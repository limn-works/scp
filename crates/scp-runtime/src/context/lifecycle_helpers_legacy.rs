...
//! 6. [`join_context_membership_legacy`] — Phase 4 membership mutations
//!    for join.
//! 7. [`capture_join_payment_legacy`] — Phase 5 escrow capture for
//!    join.
//! 8. [`leave_context_legacy`] — capability check + MLS remove + sender
//!    key cleanup + membership removal + close-on-empty.
//! 9. [`drain_and_deliver_sender_keys_legacy`] — sender-key
...
//! actor-owned shape — they live ONLY here, with no actor-shape twin:
//!
...
pub async fn restore_all_contexts_legacy(supervisor: &Supervisor) -> Result<Vec<String>, ContextError> {
    ...
    for ctx_id in &context_ids {
        ...
        match restore_context_legacy(supervisor, ctx_id, &handle).await {
...
/// (ADR-049 commit 12). Byte-identical behavior.
pub async fn flush_all_contexts_legacy(supervisor: &Supervisor) {
    if !manager_methods::has_persistence(supervisor) {
        return;
    }
    ...
    for (context_id, arc) in arcs {
        match tokio::time::timeout(FLUSH_LOCK_BUDGET, arc.lock()).await {
            ...
            Err(_elapsed) => {
                persist_degraded_snapshot_legacy(supervisor, &context_id);
                degraded += 1;
            }
        }
    }
...
/// (ADR-049 commit 12). Byte-identical behavior.
pub fn flush_all_contexts_sync_legacy(supervisor: &Supervisor) {
    if !manager_methods::has_persistence(supervisor) {
        return;
    }
    match tokio::runtime::Handle::try_current() {
        Ok(handle) => {
            tokio::task::block_in_place(|| {
                handle.block_on(flush_all_contexts_legacy(supervisor));
            });
        }
        Err(e) => {
            tracing::warn!(
...
/// vanishingly small.
pub fn shutdown_all_contexts_sync_legacy(supervisor: &Supervisor) {
    use crate::context::state::context_id_to_bytes;
...
fn persist_degraded_snapshot_legacy(supervisor: &Supervisor, context_id: &str) {
    let Some(persistence) = supervisor.persistence_ref() else {
        return;
    };
    let snapshot = build_degraded_snapshot_legacy(context_id);
...
fn build_degraded_snapshot_legacy(context_id: &str) -> crate::context::state::ContextSnapshot {
...
pub async fn shutdown_all_contexts_legacy(supervisor: &Supervisor) {
    use crate::context::state::context_id_to_bytes;
