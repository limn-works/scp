...
//! Legacy messaging-domain helpers
//! (ADR-049 Phase 2A.7, `messaging` domain migration).
//!
//! # Purpose
//!
//! This module preserves the pre-migration `&Supervisor` lock-and-call
//! messaging helper bodies for the Phase 2A shim fallback. The live
//! actor path now calls [`crate::context::messaging_helpers`], which
...
//! # Legacy twins
//!
//! 1. [`build_encrypted_envelope_legacy`] — access-key wrap, inner
//!    envelope sign+pad, sender-key + MLS + outer-envelope seal.
//! 2. [`enforce_send_economy_legacy`] — unified economy enforcement.
//! 3. [`build_broadcast_envelope_legacy`] — broadcast-mode publish.
//! 4. [`verify_and_unwrap_legacy`] — inner-signature verify +
//!    padding strip + content integrity + access-key unwrap.
//! 5. [`deliver_plaintext_or_announcement_legacy`] — buffered/drained
//!    delivery (announcement or regular).
//! 6. [`run_buffered_post_delivery_legacy`] — post-delivery governance
//!    for buffered/drained messages.
//! 7. [`send_message_legacy`] — top-level send path.
//! 8. [`deliver_incoming_legacy`] — top-level receive path.
//! 9. [`encrypt_and_send_legacy`] — Phase 2 encrypt + transport.
//! 10. [`authorize_send_payment_legacy`] — Phase 1.5 escrow auth.
//! 11. [`capture_send_payment_legacy`] — Phase 3 escrow capture.
//! 12. [`finalize_send_legacy`] — event-log append + consequence
//!     evaluation + checkpoint + persistence.
//! 13. [`decrypt_and_dispatch_legacy`] — open + sender-key processing.
//! 14. [`validate_and_drain_timeouts_legacy`] — timestamp + sequence
//!     validate + reorder-buffer timeout drain.
//! 15. [`buffer_ahead_message_legacy`] — buffer out-of-order message,
//!     overflow-force-deliver if needed.
//! 16. [`deliver_message_and_drain_buffered_legacy`] — in-order
//!     delivery + drain consecutive buffered.
//! 17. [`send_pseudonym_announcement_legacy`] — best-effort pseudonym
//!     announcement send.
...
pub fn build_encrypted_envelope_legacy(
    clock: &Arc<dyn Clock>,
...
pub fn enforce_send_economy_legacy(
...
pub fn build_broadcast_envelope_legacy(
    clock: &dyn Clock,
...
#[allow(clippy::too_many_arguments)]
pub fn verify_and_unwrap_legacy(
    key_resolver: &KeyResolver,
    inner: &InnerEnvelope,
    sender_did: &str,
    context_id: &str,
...
pub fn deliver_plaintext_or_announcement_legacy(
    ctx: &mut PerContextState,
    sender_did: &str,
    plaintext: &[u8],
    context_id: &str,
...
pub fn run_buffered_post_delivery_legacy(
    ctx: &mut PerContextState,
    context_id: &str,
...
pub async fn send_message_legacy(
    ...
    let (
        ...
        let deducted_cost = match enforce_send_economy_legacy(
            ctx,
            sender_did,
            now_secs,
            spending_ucan,
        ...
        if let Some(ref mut bc) = ctx.broadcast_context {
            ...
            let env = match build_broadcast_envelope_legacy(&**clock, bc, sender_did, payload, sk) {
                Ok(env) => env,
                Err(e) => {
                    // Use inline variant: we already hold the per-context lock.
                    crate::context::economy_logic::rollback_economy_ticket_inline(&mut ctx.governance, ticket);
    ...
    let auth = match authorize_send_payment_legacy(supervisor, &context_id, sender_did).await {
        Ok(auth) => auth,
        Err(e) => {
    ...
    let phase2_result = encrypt_and_send_legacy(
        supervisor,
        broadcast_envelope,
        signing_key,
        &context_id,
    ...
    capture_send_payment_legacy(supervisor, auth, sender_did, &context_id, deducted_cost).await;

    finalize_send_legacy(
        supervisor,
        &context_id,
        &context_id_bytes,
        sender_did,
...
pub async fn deliver_incoming_legacy(
    supervisor: &Supervisor,
    context_id: &str,
    encrypted_blob: &[u8],
    ...
    let Some(opened_envelope) =
        decrypt_and_dispatch_legacy(supervisor, context_id, &context_id_bytes, encrypted_blob)?
    else {
        return Ok(None);
    };
    ...
    let plaintext = verify_and_unwrap_legacy(
        key_resolver,
        &inner,
        &sender_did,
        context_id,
    ...
    let sequence_check =
        validate_and_drain_timeouts_legacy(supervisor, context_id, &inner, now_ms).await?;
    ...
    match sequence_check {
        SequenceCheck::Expected => {
            ...
            let consumed_as_announcement = deliver_message_and_drain_buffered_legacy(
                supervisor,
                context_id,
                &context_id_bytes,
                &sender_did,
...
///   (inside [`build_encrypted_envelope_legacy`]), the crypto provider seals
...
pub fn encrypt_and_send_legacy(
    supervisor: &Supervisor,
    ...
    let encrypted = if let Some(envelope) = broadcast_envelope {
        ...
        let result = build_encrypted_envelope_legacy(
            supervisor
                .clock_ref()
                .ok_or_else(|| ContextError::NotInitialized(ATTACHED_EXPECT.to_owned()))?,
            supervisor
...
/// Authorizes escrow for send payment (Phase 1.5 of [`send_message_legacy`]).
///
...
pub async fn authorize_send_payment_legacy(
    supervisor: &Supervisor,
    context_id: &str,
    sender_did: &DID,
...
///
/// Byte-identical to the legacy method form.
pub async fn capture_send_payment_legacy(
    supervisor: &Supervisor,
...
///
/// Extracted from the [`send_message_legacy`] orchestration so the outer
...
pub async fn finalize_send_legacy(
    supervisor: &Supervisor,
    context_id: &str,
    context_id_bytes: &[u8; 32],
    sender_did: &DID,
...
/// literal `.open(` text.
pub fn decrypt_and_dispatch_legacy(
    supervisor: &Supervisor,
    context_id: &str,
    context_id_bytes: &[u8; 32],
    encrypted_blob: &[u8],
...
pub async fn validate_and_drain_timeouts_legacy(
    supervisor: &Supervisor,
    context_id: &str,
    ...
    for (gap_info, messages) in timed_out {
        ...
        for msg in &messages {
            ...
            if let Some(event_name) = deliver_plaintext_or_announcement_legacy(
                ctx,
                &msg.sender_did,
                &msg.plaintext,
                context_id,
                ...
                run_buffered_post_delivery_legacy(
                    ctx,
                    context_id,
                    &context_id_bytes,
                    &msg.sender_did,
...
/// Byte-identical to the legacy method form.
#[allow(clippy::significant_drop_tightening)] // Legacy body held per-context guard across await points deliberately — narrowing changes lock-ordering semantics.
pub async fn buffer_ahead_message_legacy(
    supervisor: &Supervisor,
    context_id: &str,
    inner: &scp_protocol::envelope::inner::InnerEnvelope,
    sender_did: &str,
...
#[allow(clippy::significant_drop_tightening)] // Legacy body held per-context guard across await points deliberately — narrowing changes lock-ordering semantics.
pub async fn deliver_message_and_drain_buffered_legacy(
    supervisor: &Supervisor,
    context_id: &str,
    context_id_bytes: &[u8; 32],
    sender_did: &str,
...
/// [`ContextManager::send_pseudonym_announcement`](crate::context::messaging_helpers_legacy::send_pseudonym_announcement_legacy)
/// (ADR-049 commit 12). Byte-identical behavior — best-effort delivery
/// with internal logging on transport / serialization failures.
pub async fn send_pseudonym_announcement_legacy(
    supervisor: &Supervisor,
    handle: &ContextHandle,
    sender_did: &DID,
    signing_key: &ed25519_dalek::SigningKey,
    ...
    if let Err(e) = send_message_legacy(
        supervisor,
        handle,
        sender_did,
        &payload,
