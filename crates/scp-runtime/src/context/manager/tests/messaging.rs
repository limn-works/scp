...
async fn tool_invoke_escalation_via_managed_wrapper() {
    ...
    // C1b (PR #1606): paid tool invocations require signature-verified
    // spending UCANs. `mock_key_resolver` resolves the deterministic test
    // key for `did:key:invoker` so the new validation pipeline succeeds,
    // matching the send/join path fix from C1.
    let manager = ContextManager::new(
        Box::new(MockCrypto::default()),
        Box::new(MockTransport::connected()),
        Box::new(MockEventLog::default()),
    ...
    // C1b (PR #1606): tool invoke path now runs full spending-UCAN
    // signature validation inside `invoke_tool_with_economy` — unsigned
    // tokens no longer pass. Mint a signed token bound to the invoker
    // via `dummy_spending_ucan_for`, which matches `mock_key_resolver`.
...
async fn tool_invoke_respects_hard_rate_limit() {
    ...
    // C1b (PR #1606): paid tool invocations require signature-verified
    // spending UCANs. Use `mock_key_resolver` so the deterministic key
    // for `did:key:spammer` resolves during signature verification.
    let manager = ContextManager::new(
        Box::new(MockCrypto::default()),
        Box::new(MockTransport::connected()),
        Box::new(MockEventLog::default()),
    ...
    //
    // C1b (PR #1606): tool invoke path now runs full spending-UCAN
    // signature validation. Mint a fresh signed token per iteration via
    // `dummy_spending_ucan_for` so (a) signature verification passes and
    // (b) the per-context nonce tracker does not reject replays
    // (ADR-016 §6). Fresh tokens per call matches the send-path pattern
    // in `escalation_kicks_in_at_velocity_threshold_10`.
