//! governance-related manager tests.
//!
...
#[test]
#[ignore = "manager/tests/governance rewrite deferred to 12c.9f — MlsBackend injection"]
fn manager_tests_governance_deferred_to_12c_9f() {
    // Intentionally empty — placeholder so the deferral is visible to
    // test runners and so grep for `governance` tests in git blame leads
    // back to this marker.
}
