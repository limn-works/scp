...
async fn promote_context_succeeds_when_policy_is_promotable() {
    ...
    let _arc = manager.contexts.get("promo-ctx").unwrap().value().clone();
    let _g = _arc.lock().await;
    let ctx = &*_g;
    assert_eq!(
...
async fn revoke_rejected_without_member_ban_ceiling() {
    ...
    // Add bob as author.
    {
        let _arc = manager.contexts.get("bc-no-ban").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
...
async fn governance_single_admin_engine_constructed() {
    ...
    let _arc = manager.contexts.get("ctx-gov-sa").unwrap().value().clone();
    let _g = _arc.lock().await;
    let ctx = &*_g;
...
async fn governance_majority_engine_constructed() {
    ...
    let handle = manager
        ...
        .unwrap();
    assert_eq!(handle.state().await, ContextState::Active);
    let _arc = manager.contexts.get("ctx-gov-maj").unwrap().value().clone();
    let _g = _arc.lock().await;
    let ctx = &*_g;
...
async fn governance_threshold_propose_approve_lifecycle() {
    ...
    {
        let _arc = manager.contexts.get("thresh-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.role_state
            .member_capabilities
...
async fn scp274_majority_full_lifecycle() {
    ...
    let _handle = manager
        ...
        .unwrap();
    {
        let _arc = manager.contexts.get("scp274-maj").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.membership
            .add_member("did:key:target".into(), "member".into(), vec![]);
...
async fn scp274_unanimity_full_lifecycle() {
    ...
    let _handle = manager
        ...
        .unwrap();
    {
        let _arc = manager.contexts.get("scp274-unan").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.membership
...
async fn execute_modify_ceiling_sets_pending_with_72h_period() {
    ...
    let _arc = manager.contexts.get("ctx-ceiling").unwrap().value().clone();
    let _g = _arc.lock().await;
    let ctx = &*_g;
    let pending = ctx
        .governance
...
async fn apply_pending_ceiling_modification_respects_notification_period() {
    ...
    let notified_at = {
        let _arc = manager.contexts.get("ctx-apply").unwrap().value().clone();
        let _g = _arc.lock().await;
        let ctx = &*_g;
        ctx.governance
            .pending_ceiling_modification
...
async fn approve_spend_grants_budget_to_member_tracker() {
    ...
    // No budget initially.
    {
        let _arc = manager.contexts.get("budget-ctx").unwrap().value().clone();
        let _g = _arc.lock().await;
        let ctx = &*_g;
...
async fn budget_tracker_included_in_snapshot() {
    ...
    let _arc = manager.contexts.get("snap-ctx").unwrap().value().clone();
    let _g = _arc.lock().await;
    let ctx = &*_g;
...
async fn test_consequence_rule_triggers_enforcement_event() {
    ...
    {
        let _arc = manager.contexts.get("conseq-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
...
async fn test_economy_cost_deducted_on_send() {
    ...
    {
        let _arc = manager.contexts.get("econ-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.governance
            .budget_tracker
...
async fn test_participation_decay_clears_caches() {
    ...
    {
        let _arc = manager.contexts.get("decay-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
...
async fn remaining_members_keys_after_removal() {
    ...
    let _arc = manager.contexts.get("remain-ctx").unwrap().value().clone();
    let _g = _arc.lock().await;
    let ctx = &*_g;
    assert!(
...
async fn send_message_deducts_budget() {
    ...
    // Grant budget of 50.
    {
        let _arc = manager.contexts.get("deduct-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.governance
            .budget_tracker
...
async fn sybil_resistance_evaluated_on_join() {
    ...
    let _arc = manager.contexts.get("sybil-ctx").unwrap().value().clone();
    let _g = _arc.lock().await;
    let ctx = &*_g;
    assert!(
...
async fn test_send_rejected_insufficient_budget() {
    ...
    {
        let _arc = manager.contexts.get("insuf-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.governance
            .budget_tracker
...
async fn test_execute_paid_action_full_flow_with_adapter() {
    ...
    // Grant budget.
    {
        let _arc = manager.contexts.get("adpt2-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.governance
            .budget_tracker
...
async fn test_free_context_no_budget_deduction() {
    ...
    {
        let _arc = manager.contexts.get("free-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
...
async fn test_remaining_members_unaffected_after_removal() {
    ...
    let _arc = manager.contexts.get("remain2-ctx").unwrap().value().clone();
    let _g = _arc.lock().await;
    let ctx = &*_g;
...
async fn test_send_consequence_economy_round_trip() {
    ...
    // Grant budget.
    {
        let _arc = manager.contexts.get("xcut-rt-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.governance
            .budget_tracker
...
async fn velocity_escalation_raises_effective_cost() {
    ...
    macro_rules! budget_remaining {
        ($mgr:expr) => {{
            let _arc = $mgr.contexts.get("vel-esc-ctx").unwrap().value().clone();
            let _g = _arc.lock().await;
            let ctx = &*_g;
    ...
    let (velocity, aggregate) = {
        let _arc = manager.contexts.get("vel-esc-ctx").unwrap().value().clone();
        let _g = _arc.lock().await;
        let ctx = &*_g;
        let now = scp_primitives::SystemClock.now_secs();
        (
...
async fn test_consequence_evaluation_uses_full_history() {
    ...
    {
        let _arc = manager.contexts.get("hist-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.role_state
            .suspended_capabilities
...
async fn no_double_charge_on_paid_send() {
    ...
    let sender_did: DID = "did:key:sender".into();
    {
        let _arc = manager.contexts.get("double-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.governance
            .budget_tracker
...
async fn aggregate_velocity_via_manager_send() {
    ...
    {
        let _arc = manager.contexts.get("agg-vel-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.governance.budget_tracker.grant(
            &"did:key:sender".into(),
...
async fn buffer_event_timestamp_bounds_m18() {
    ...
    {
        let _arc = manager.contexts.get("bounds-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        for i in 0..10u64 {
            ctx.receive_buffer.push(ContextEvent::MessageSent {
...
async fn observable_metrics_time_of_day_populated() {
    ...
    {
        let _arc = manager.contexts.get("tod-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.governance
            .budget_tracker
...
async fn context_message_rate_from_aggregate_velocity() {
    ...
    {
        let _arc = manager.contexts.get("cmr-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.governance.budget_tracker.grant(
            &"did:key:sender".into(),
...
async fn c1_paid_context(name: &str, sender_did: &DID) -> (ContextManager, ContextHandle) {
    ...
    {
        let _arc = manager.contexts.get(name).unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.governance
            .budget_tracker
...
async fn test_capture_failure_budget_retained() {
    ...
    let handle = manager
        ...
        .unwrap();
    {
        let _arc = manager.contexts.get("capture-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        ctx.governance
            .budget_tracker
...
async fn test_consequence_failure_escalates() {
    ...
    let _arc = manager.contexts.get("esc-ctx").unwrap().value().clone();
    let _g = _arc.lock().await;
    let ctx = &*_g;
    assert!(
        ctx.role_state
...
async fn test_member_reset_rotates_sender_keys() {
    ...
    let _arc = manager.contexts.get(&context_id).unwrap().value().clone();
    let _g = _arc.lock().await;
    let ctx = &*_g;
...
async fn earned_capacity_evicts_stale_timestamps() {
    ...
    {
        let _arc = manager.contexts.get("evict-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
        let now = manager.clock.now_secs();
...
async fn consequence_evaluation_caps_buffer_events() {
    ...
    {
        let _arc = manager.contexts.get("cap-buf-ctx").unwrap().value().clone();
        let mut _g = _arc.lock().await;
        let ctx = &mut *_g;
...
async fn suspend_all_consequence_preserves_mls_membership() {
    ...
    {
        let _arc = manager.contexts.get("h20a-ctx").unwrap().value().clone();
        let _g = _arc.lock().await;
        let ctx = &*_g;
        assert!(
...
async fn suspend_capability_consequence_preserves_mls_membership() {
    ...
    // member role.
    {
        let _arc = manager.contexts.get("h20b-ctx").unwrap().value().clone();
        let _g = _arc.lock().await;
        let ctx = &*_g;
...
async fn restore_access_after_suspend_all_regrants_capabilities() {
    ...
    let pre_caps = {
        let _arc = manager.contexts.get("h20c-ctx").unwrap().value().clone();
        let _g = _arc.lock().await;
        let ctx = &*_g;
        ctx.role_state
            .member_capabilities
