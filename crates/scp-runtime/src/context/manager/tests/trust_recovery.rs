//! trust_recovery-related manager tests.
//!
...
#[test]
#[ignore = "manager/tests/trust_recovery rewrite deferred to 12c.9f — MlsBackend injection"]
fn manager_tests_trust_recovery_deferred_to_12c_9f() {
    // Intentionally empty — placeholder so the deferral is visible to
    // test runners and so grep for `trust_recovery` tests in git blame leads
    // back to this marker.
}
