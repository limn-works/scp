...
//! - [`invoke_session`] -- Invokes an outlet within an active session with
...
/// * `registry` -- The tool registry to validate the outlet exists.
    ...
    Fut: std::future::Future<Output = Result<(serde_json::Value, serde_json::Value), String>>,
{
    ...
    // Per-call UCAN governance: validate invoker has OutletCall capability.
...
mod tests {
    ...
    async fn invoke_session_validates_ucan_per_call() {
        ...
        // Second call by member (no OutletCall capability) -- should fail.
        let result = invoke_session(
            &mut store,
            &registry,
            &role_state,
