...
//! # Purpose
//!
//! This module retains the pre-actor `&Supervisor` tool bodies during
//! the Phase 2A migration window. The actor-owned hard-rate helpers live
//! in [`crate::context::tools_helpers`]; supervisor shim fallback and
//! FFI-facing sync/generic surfaces continue to call this module until
//! Phase 2A finalization removes the legacy path.
//!
//! # Behavior preservation
//!
//! Every function here is behavior-preserving by construction: this
//! file started as the pre-migration `tools_helpers.rs`, then the
//! actor-owned hard-rate entry points were renamed with a `_legacy`
//! suffix for explicit shim fallback.
//!
//! Supervisor direct FFI surfaces keep calling this module until their
//! own actor-safe design lands. The generic
//! [`invoke_tool_with_economy`] closure surface cannot cross the
//! current actor mailbox and therefore stays on the direct supervisor
//! fallback path for this Phase 2A helper-domain slice.
//!
//! # Actor-handler fallback entry points
//!
//! [`try_consume_hard_rate_limit_legacy`],
//! [`refund_hard_rate_limit_legacy`].
//!
//! # Direct supervisor / FFI surfaces
//!
...
//! [`invoke_tool_with_economy`] are reached from bridge-facing
//! supervisor methods, not from the tools actor handler.
...
#[must_use]
pub async fn try_consume_hard_rate_limit_legacy(
...
/// (ADR-049 commit 12). Byte-identical behavior.
pub async fn refund_hard_rate_limit_legacy(supervisor: &Supervisor, context_id: &str, did: &DID) {
        ...
        Ok(handle) => {
            ...
            match handle.runtime_flavor() {
                RuntimeFlavor::MultiThread => tokio::task::block_in_place(|| {
                    handle.block_on(try_consume_hard_rate_limit_legacy(
            ...
            match handle.runtime_flavor() {
                RuntimeFlavor::MultiThread => {
                    tokio::task::block_in_place(|| {
                        handle.block_on(refund_hard_rate_limit_legacy(supervisor, context_id, did));
                    });
                }
                _ => {
...
fn run_blocking_on_dedicated_thread(
    ...
    std::thread::spawn(move || {
        ...
        let result = if refund {
            rt.block_on(refund_hard_rate_limit_legacy(
                &supervisor,
                &context_id,
                &did,
            ));
            ...
            rt.block_on(try_consume_hard_rate_limit_legacy(
                &supervisor,
                &context_id,
                &did,
                now_secs,
