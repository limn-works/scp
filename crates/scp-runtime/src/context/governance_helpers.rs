...
// `manager/governance.rs` carried `#[allow(clippy::significant_drop_tightening)]`
...
//! [`crate::context::trust_recovery_helpers`] (12c.3a), and
//! [`crate::context::economy_helpers`] (12c.3a). Governance is the
//! largest of the four domains (~6300 LOC) and is therefore landed as
//! its own commit (12c.3b).
//!
...
//!
//! # Transitive helpers hoisted
//!
//! All private / `pub(super)` methods reachable from the 14 entry points:
//! [`finalize_governance_action`], [`dispatch_governance_action`],
//! [`dispatch_context_governance_action`],
//! [`dispatch_content_governance_action`],
//! [`build_governance_context`], [`propose_governance_action_inner`],
//! [`vote_on_proposal_inner`], [`detect_and_handle_conflicts`],
//! [`check_and_resolve_expired_freezes`],
//! [`try_broadcast_commit_or_enqueue`], and all per-action
//! `execute_*` helpers (`SuspendMember`, `Revoke`, `RestoreAccess`,
//! `AddMember`, `RemoveMember`, `ChangeRole`, `RegisterTool`, `RemoveTool`,
//! `ModifyCeiling`, `CloseContext`, `ExtendTtl`, `TransferAdmin`,
//! `CreateChildContext`, `ModifyPruningPolicy`, `AddSigner`, `RemoveSigner`,
//! `ModifyThreshold`, `EstablishToolInterface`, `ResetMember`,
...
pub fn build_governance_context(ctx: &PerContextState, clock: &dyn Clock) -> GovernanceContext {
...
// ---------------------------------------------------------------------------
// start_governance_timeout_task (transitive of lifecycle/governance paths)
// ---------------------------------------------------------------------------
...
pub async fn start_governance_timeout_task(supervisor: &Supervisor, context_id: &str) {
