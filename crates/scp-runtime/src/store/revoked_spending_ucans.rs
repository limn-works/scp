...
//! wholesale re-loaded (already-moot entries dropped) from the durable store via
//! [`load_all`](RevokedSpendingUcanStore::load_all) on BOTH startup hydration AND
//! every incremental global revocation — a SINGLE reload discipline, not two
//! divergent cache-write paths. So after any revocation the cache equals the
//! bounded durable set rather than growing monotonically until restart
//! (invariant 2a). An incremental global revoke does a full `identity/`-prefix
//! reload rather than a cheaper single-DID re-derive; this is affordable because
//! global revokes are rare, human-initiated, cold-path (NOT the paid-action hot
//! path). The number of *distinct* payer DIDs is the trusted-local
//! self-governance model's own bound.
//!
