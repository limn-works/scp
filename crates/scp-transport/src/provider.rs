...
//!
//! [`RelayTransportProvider`] implements the async [`ContextTransportProvider`]
//! trait from `scp-core` by wrapping a `NativeRelayAdapter` (or any
//! [`TransportAdapter`]) and delegating directly to the underlying async
//! transport operations.
//!
//! # Design
//!
//! The [`ContextTransportProvider`] trait methods are async, matching the
//! underlying transport adapter. The `is_connected` method remains sync
//! (an `AtomicBool` read).
//!
//! # Thread Safety
//!
//! `RelayTransportProvider` is `Send + Sync`. The underlying adapter is
//! stored in an `Arc<tokio::sync::Mutex<_>>` to allow mutable access for
//! send operations without blocking the async runtime.
//!
    ...
    for RelayTransportProvider<A>
{
    ...
    async fn publish_context(
        ...
        match guard.send(&envelope).await {
            Ok(_blob_id) => Ok(()),
    ...
    async fn delete_published(&self, context_id: &[u8; 32]) -> Result<(), ContextCreationError> {
        ...
        match guard.delete(&blob_id).await {
            Ok(()) => Ok(()),
            Err(e) => Err(ContextCreationError::TransportFailed(e.to_string())),
        }
    }
