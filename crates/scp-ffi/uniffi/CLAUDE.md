...
- **Authorization reads live supervisor state**: every gate that decides whether an operation is permitted reads membership, role assignment, per-member capability, suspension, or the capability ceiling through `UniffiBridgeInstance::live_role_state(context_id).await` or `UniffiBridgeInstance::live_ceiling_strings(context_id).await`, and every gate that refuses a non-active context reads `UniffiBridgeInstance::live_context_state(context_id).await`. Each helper routes through the per-context supervisor actor's mailbox, and each fails closed with `SCP-CTX-2023` when the supervisor holds no actor for the context.
  - `ContextHandle` holds no ceiling and `UcanContextState` holds no ceiling, and no ceiling-sync helper exists. The bridge stored a ceiling string set on both until governance narrowing a ceiling proved a bridge copy cannot stay current: a `ModifyCeiling` another participant committed left `ucan_validate`, `ucan_evaluate`, `ucan_mint`, `ucan_delegate`, and the outlet-invocation gate admitting a capability that context already forbade. Do NOT reintroduce a `ceiling_strings` field, a `sync_ceiling_from_params` helper, or a refresh call site — a refresh site closes no window.
  - A context the supervisor created with an EMPTY ceiling authorizes its creator for nothing. The bridge substitutes no `default_ceiling()` on a caller's behalf, so a test that needs an authorized creator passes the capabilities it needs in `ContextParams.ceiling` at creation. `crates/scp-ffi/CLAUDE.md` states the same rule for the `PyO3` reference bridge, and the NAPI bridge follows it through `runtime::live_ceiling_strings`.
  - `validate_outlet_ucan_uniffi` is `async` for this reason: it awaits `live_role_state` before it enters `with_ucan_state`, whose closure holds a `DashMap` guard across no await point. Read live state BEFORE that closure.
