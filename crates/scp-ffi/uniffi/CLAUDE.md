...
- **Close authorization**: `context_close` does NOT perform bridge-layer authorization. It delegates to `Supervisor::close_context` (the hoisted `lifecycle_helpers::close_context` body) which checks the `ContextClose` capability via `ttl::close_context`. The Supervisor is the authoritative auth layer. Its gate reads `UniffiBridgeInstance::read_live_context_state`, the `Option` form of the live read: an absent actor (a completed TTL expiry) and a `Closed`/`Expired`/`Tombstoned` supervisor state skip the dispatch and still release the per-context UCAN state, because that release has no other path after creation. A `Poisoned` supervisor state releases the same way: the crash watchdog despawned that actor and keeps only the sticky poison flag (ADR-049 §10), so a close that refused it would hold the registry entry for the life of the process, because `clear_poison` is an operator action no bridge exports. A `Closing` supervisor state does NOT skip the dispatch: `ttl::close_context` drives `Active` → `Closing` and a separate `FinalizeClose` command ends the §5.9 cooperative window, so `Closing` is a live non-terminal state, and the dispatch carries the only `ContextClose` capability check this path has. Skipping it let a caller holding no `context:close` capability release the per-context UCAN state — the event log, the nonce tracker, the revocation list — for every identity sharing the bridge instance while the context was still in use. The release path out of `Closing` is `context_finalize_close`, which transitions the context to `Closed`. Every live non-terminal state (`Creating`, `Closing`, `MigratingOut`) refuses the close.
