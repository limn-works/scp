...
- **Authorization reads live supervisor state**: every gate that decides whether an operation is permitted reads membership, role assignment, per-member capability, suspension, or the capability ceiling through `runtime::live_role_state(bi, context_id)` or `runtime::live_ceiling_strings(bi, context_id)`. Both route through the per-context supervisor actor's mailbox, and both fail closed with `SCP-CTX-2023` when the supervisor holds no actor for the context.
  - Every gate that refuses a non-active context reads `runtime::live_context_state_str(bi, context_id)`, which renders that actor's lifecycle state as the bridge's lowercase state string and renders an absent actor as `"unknown"`, so a caller comparing against `"active"` fails closed. Seven call sites read it: `context_join`, `context_leave`, `context_close`, `context_send`, and `context_receive` in `context.rs`, plus the caller and target axes of `outlet_invoke_cross_context_saga_impl` in `outlets.rs` (codes `SCP-OUTLET-6010` / `SCP-OUTLET-6011`). NONE of them reads `PyContextHandle::state`, whose getter doc marks it a best-effort cached snapshot that lags a close.
