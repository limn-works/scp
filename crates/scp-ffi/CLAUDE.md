...
**Supervisor (shared):** Owns context lifecycle state — membership, roles, governance, broadcast, TTL. All `py_context_*` functions delegate to `Supervisor::create_context`, `join_context`, `leave_context`, `close_context`, `send_message`. Built via `Supervisor::with_providers(...)` with production providers: `MlsCryptoProvider` (real OpenMLS-backed encryption, sender keys, and group management — #1324, matching NAPI bridge #1305), `NotConfiguredTransportProvider` (returns descriptive errors until relay is configured via `transport_connect`), `NoOpEventLogProvider` (bridge-level `EventLog` instances handle Merkle ops), and a `not_configured_key_resolver` that logs errors on every lookup rather than silently skipping verification (#501). The `local_did` is passed to `MlsCryptoProvider::new` as the MLS credential identity.
...
- `RoleState` — role assignments for UCAN/tool capability checking (synced with Supervisor)
...
**Backward-compatibility aliases:** `with_context` → `with_ffi_state`, `register_context` → `register_ffi_state` + Supervisor wiring (formerly `init_context_manager`), `remove_context` → `remove_ffi_state`. Existing modules (`tools.rs`, `ucan.rs`, `event_log.rs`, `mcp.rs`) use these aliases without modification.
...
`py_context_create` in `context.rs` registers FFI bridge state and delegates lifecycle to `Supervisor`. Other modules (`tools.rs`, `ucan.rs`, `event_log.rs`) look up FFI state by context ID via `with_context` (alias for `with_ffi_state`).
...
- `py_context_create` creates real `ToolRegistry` and `EventLog` objects in the runtime registry. If context creation fails partway, the registry entry must be cleaned up. It also calls `Supervisor::register_local_did` to register the creator's DID for defense-in-depth (matching NAPI behavior).
...
- **Storage provider (SCP-217)**: `py_init_storage("in_memory")` injects a global `EncryptingAdapter<InMemoryStorage>` via `OnceLock<Arc<EncryptingAdapter<InMemoryStorage>>>` in `runtime.rs`. A random AES-256-GCM key is generated via `OsRng` at initialization time. This satisfies the sealed `EncryptedStorage` bound required by `ProtocolRepository::new()`, matching the `scp-node` ephemeral mode pattern. `py_identity_create` persists identity state under `identity/{did}/state` after creation. `py_identity_load` retrieves from storage and raises `IdentityError` if the DID is not found (no silent fallback). The `Storage` trait uses RPITIT and is **not dyn-compatible** — the global must use a concrete type, not `Arc<dyn Storage>`. When `SqliteStorage` lands, this will need an enum dispatch or generic parameter.
