...
- `FfiBridgeProvider::validate_capability` performs real capability checking via `has_tool_invoke_capability` against the context's role state (SCP-210). Defense-in-depth alongside the UCAN layer. `invoke_tool` validates input against the tool's JSON schema and returns a validated response with `"status": "executed"` (no runtime handler dispatch at the FFI layer — tools are registered with metadata only).
