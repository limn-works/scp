...
**Server side:**
- `FfiBridgeProvider` implements `scp_mcp::server::ContextProvider` by reading outlet registrations, role state, and event log data from the runtime registry via `with_context()`.
...
- **Close authorization**: `py_context_close` does NOT perform bridge-layer authorization. It delegates to `Supervisor::close_context` (the hoisted helper in `lifecycle_helpers`), which checks the `ContextClose` capability via `ttl::close_context`. The Supervisor is the authoritative auth layer. Errors from the Supervisor are propagated (except "not registered"/"context not found" which are idempotent). The close gate reads `read_live_context_state`, the `Option` form of the live read: an absent actor (a completed TTL expiry) and a `Closed`/`Expired`/`Tombstoned` supervisor state skip the dispatch and still release the `FfiBridgeState`, because that release has no other path after creation. A `Poisoned` supervisor state releases the same way: the crash watchdog despawned that actor and keeps only the sticky poison flag (ADR-049 §10), so a close that refused it would hold the registry entry for the life of the process, because `clear_poison` is an operator action no bridge exports. A `Closing` supervisor state does NOT skip the dispatch: `ttl::close_context` drives `Active` → `Closing` and a separate `FinalizeClose` command ends the §5.9 cooperative window, so `Closing` is a live non-terminal state, and the dispatch carries the only `ContextClose` capability check this path has. Skipping it let a caller holding no `context:close` capability release the `FfiBridgeState` — the outlet handlers, the receive-channel sender, the event log, the nonce tracker, the revocation list — for every identity sharing the bridge instance while the context was still in use. The release path out of `Closing` is `finalize_close`, which transitions the context to `Closed`. Every live non-terminal state (`Creating`, `Closing`, `MigratingOut`) refuses the close.
- **Authorization reads the supervisor actor, never a bridge copy**: `FfiBridgeState` holds no role state, no membership set, no capability ceiling, and no creator DID. Every entry point that decides authorization, membership, a role, a capability, or a ceiling calls `runtime::live_role_state(bi, context_id)` (or `runtime::live_ceiling_strings`), which queries the per-context supervisor actor and fails closed when that actor holds no role state. The former `role_state` / `ceiling_strings` / `creator_did` fields, plus the `sync_role_state_from_manager`, `sync_role_state_from_manager_async`, and `sync_ceiling_from_params` helpers that refreshed them, are DELETED: a copy refreshed on a local mutation still went stale on every change the supervisor applied by another route (a governance execution, a broadcast-subscriber removal, a TTL expiry, a trust-recovery transition), and `outlet_stream_open` plus the cross-context streaming saga already read the actor, so one bridge answered one authorization question two ways. `live_role_state` bridges sync to async under whichever tokio regime the caller sits in, so the MCP `ContextProvider` trait methods reach it from a server task.
- **An absent ceiling and an empty ceiling are different declarations**: `PyContextParams::from_py_dict` resolves an absent `ceiling` key, and a `ceiling` key holding `None`, to `default_ceiling()` rendered as `{resource}:{action}` strings, because `default_ceiling`'s own doc comment says every FFI bridge applies it "when no explicit ceiling is provided". A supplied list stands as written, so `ceiling=[]` builds a context whose ceiling grants nothing. `build_core_context_params` substitutes nothing: substituting there read an empty list as an absent key and handed a caller who asked for a deny-all context the eleven capabilities `default_ceiling()` carries.
- **A lifecycle gate reads the supervisor actor, never the handle string**: `py_context_join`, `py_context_leave`, `py_context_send`, and `py_context_receive` call `require_active_context(bi, context_id, verb)`, which reads `runtime::live_context_state` and refuses unless that context's supervisor actor reports `Active`. It fails closed when no actor answers. `py_context_close` reads the same supervisor state through `read_live_context_state` instead, because a close that already happened has to stay reachable (see the close bullet above). The three cross-context outlet entry points — `outlet_invoke_cross_context`, `outlet_invoke_cross_context_saga`, and the streaming open — read `Supervisor::read_context_state` for the source axis (`SCP-OUTLET-6010`) and the target axis (`SCP-OUTLET-6011`) and treat a missing actor as non-active. `PyContextHandle::state` stays a cached best-effort snapshot — its documented contract says so, and `py_context_close` still writes `"closed"` into it — but no gate reads it: that string records only the transitions THIS bridge observed, so a TTL expiry the supervisor applied on its own timer, a close another member initiated, a migration, and an actor poison all leave it reading `"active"`. The supervisor's own vocabulary differs from that snapshot's: a close enters `Closing` (the §5.9 cooperative window), which is what a gate now reports.
