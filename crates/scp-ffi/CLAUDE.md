...
- **A lifecycle gate reads the supervisor actor, never the handle string**: `py_context_join`, `py_context_leave`, `py_context_close`, `py_context_send`, and `py_context_receive` call `require_active_context(bi, context_id, verb)`, which reads `runtime::live_context_state` and refuses unless that context's supervisor actor reports `Active`. It fails closed when no actor answers. `PyContextHandle::state` stays a cached best-effort snapshot — its documented contract says so, and `py_context_close` still writes `"closed"` into it — but no gate reads it: that string records only the transitions THIS bridge observed, so a TTL expiry the supervisor applied on its own timer, a close another member initiated, a migration, and an actor poison all leave it reading `"active"`. The supervisor's own vocabulary differs from that snapshot's: a close enters `Closing` (the §5.9 cooperative window), which is what a gate now reports.
