...
- **Role state sync after governance**: `FfiBridgeState.role_state` is a local copy used by UCAN/outlet capability checks. After any governance action that modifies roles (ChangeRole, ModifyCeiling, AddMember, RemoveMember, etc.), call `runtime::sync_role_state_from_manager(context_id)` to re-sync from the authoritative `Supervisor`. Without this, the FFI copy goes stale. (The helper name keeps the legacy `from_manager` suffix; rename pending Phase 2 of the post-review-round-1 plan.)
...
- UCAN validation (SCP-164) now delegates to scp-core's full 11-step ADR-016 pipeline including Ed25519 signature verification. Bridge trait implementations (`BridgeRevocationChecker`, `BridgeProofResolver`, `BridgeNonceTracker`) in `ucan.rs` adapt runtime state to scp-core's validation traits. The `py_ucan_validate` function accepts optional `presenting_agent_did` and `proof_tokens` parameters for delegation chain verification.
- **The only DID resolver is `IdentityBackedDidResolver` (#311)**: `runtime::init_did_resolver`, called from `py_identity_create`, installs it on the bridge instance. It resolves the signer's DID document and verifies the document's BEP44 signature, its self-certification, and its sequence number. Every validation site reads it through `scp_ffi_common::require_did_resolver`, which returns `NO_VERIFYING_DID_RESOLVER` when the caller reaches validation before creating an identity; all three bridges report `SCP-PERM-3031` (PyO3 raises `ScpUcanError` carrying that code, via `ScpPyError::no_verifying_did_resolver`). There is **no** substitute resolver: the deleted `DispatchDidResolver`/`BridgeDidResolver` pair read the identity key out of the `did:dht:z…` string and returned it without fetching a document, so a token signed by a key the DID document had since retired still passed signature verification. The resolver lives in `scp-ffi-common` — shared across all FFI bridges.
- **`ucan_revoke` fails closed (no revocation broadcast)**: `scp_ffi_common::UnavailableRevocationDistributor` reports step 3 of the ADR-016 revocation flow as failed, because the bridge layer reaches no seal-and-send path for a single token CID and no receiver merges a revocation list it receives (`RevocationList::merge` has no non-test caller). The recovery orchestrator does seal and send a serialized `RevocationList` (`crates/scp-runtime/src/identity/recovery.rs` through `crates/scp-runtime/src/context/trust_recovery_helpers.rs`), which the distributor's doc comment describes along with what building real distribution requires. `revoke_ucan` then rolls the `RevocationPending` entry back and `py_ucan_revoke` raises. The resolved "UCAN revocation mechanism" entry in `.docs/specs/00-open-questions.md` requires that rollback so the revoker and the other members do not disagree about the token. The predecessor logged the revocation, returned `Ok`, and committed the local list to `Revoked` while every other member kept accepting the token.
