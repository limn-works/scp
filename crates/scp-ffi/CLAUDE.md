...
- **Identity registry (SCP-214)**: `py_identity_create` registers identity state in the `BridgeInstance` identity registry (type-erased, set via `set_identity_registry`). All crypto bridge functions (`py_ucan_mint`, `py_ucan_delegate`, `py_context_send`, `py_identity_rotate_key`, `py_identity_migrate`) look up the retained `FfiKeyCustody` and `KeyHandle`s via `with_identity()`. The `KeyCustody` trait uses RPITIT (return-position impl Trait in trait), making it NOT object-safe — the registry therefore holds the concrete `FfiKeyCustody` enum (`custody.rs`), which dispatches to `InMemoryKeyCustody`, `FileKeyCustody`, or a caller-injected `KeyCustodyProvider`, rather than a `dyn KeyCustody`. One function, `build_key_custody` in `identity.rs`, maps a custody value onto a backend, and `identity_create`, `identity_create_with_agent_key`, and `identity_create_with_custody` all route through it. §3.2.2 of the identity spec gives a caller two values: `"encrypted_file"` builds a `FileKeyCustody`, and `"os_keystore"` builds a `PyCallbackKeyCustody` over the `KeyCustodyProvider` the caller injected through `identity_create_with_custody`. `"os_keystore"` with no provider returns `SCP-IDENT-1003` and falls back to neither `encrypted_file` nor an in-memory store. Every retired string — `platform`, `software`, `file`, `platform_managed`, `hardware` — returns `SCP-VALID-7005`. The NAPI and UniFFI bridges accept the same two values and return the same codes. See SCP-294, "Fail closed on the custody strings the bridges reject, and normalize the identity parameter across SDKs", in `.docs/prds/http-features.json`.
