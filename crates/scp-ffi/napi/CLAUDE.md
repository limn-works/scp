...
- **Authorization reads the supervisor actor, never a bridge copy**: `UcanContextState` holds no role state. Every entry point that decides authorization, membership, a role, a capability, or a ceiling calls `runtime::live_role_state(bi, context_id)` (or `runtime::live_ceiling_strings`), which queries the per-context supervisor actor and fails closed when that actor holds no role state. The former `role_state` field, plus the `sync_role_state_from_manager` and `sync_ceiling_from_params` helpers that refreshed it and the `core.ceiling_strings` / `core.creator_did` fields that the UCAN validate, evaluate, mint, delegate, revoke, and outlet-invocation sites read, are gone — `UcanContextStateCore` now holds the revocation list, the nonce tracker, and the event log, the three the bridge itself owns: a copy refreshed on a local mutation still went stale on every change the supervisor applied by another route (a governance execution, a broadcast-subscriber removal, a TTL expiry, a trust-recovery transition). The `PyO3` reference bridge reads the actor at the same decisions, so the two bridges answer one authorization question one way.
- **A lifecycle gate reads the supervisor actor, never the handle string**: `context_join_on`, `context_leave_on`, `context_send_on`, `context_subscribe_on`, and the nine outlet entry points that decide an authorization question — `outlet_register_on`, `outlet_invoke_on`, `outlet_verify_on`, `outlet_invoke_cross_context_on` (both axes), `outlet_invoke_cross_context_saga_on` (both axes), `outlet_session_create_on`, `outlet_session_invoke_on`, `outlet_interface_expose_on`, and `outlet_interface_accept_on` — call `runtime::require_active_context(bi, context_id, verb, mk_err)`, which reads the supervisor actor and refuses unless it reports `Active`. Two outlet entry points carry no gate, and each one's doc comment says why: `outlet_session_close_on` releases one bridge-owned session entry, and `outlet_interface_revoke_on` reads no context state and only builds an `InterfaceRevoked` event for the caller to distribute. `mk_err` keeps each operation's own `ScpNapiError` variant and error code. It fails closed when no actor answers. `NapiContextHandle::state` stays a cached best-effort snapshot, and `context_close_on` still writes `"closed"` into it, but no gate reads it: that string records only the transitions THIS bridge observed, so a TTL expiry the supervisor applied on its own timer, a close another member initiated, a migration, and an actor poison all leave it reading `"active"`.
- **Close authorization**: `context_close_on` delegates to the supervisor's `LifecycleCommand::CloseContext`, which checks the `ContextClose` capability. Its gate reads `runtime::read_live_context_state`, the `Option` form of the live read: an absent actor (a completed TTL expiry) and a `Closed`/`Expired`/`Tombstoned` supervisor state skip the dispatch and still release the `UcanContextState`, because that release has no other path after creation. A `Poisoned` supervisor state releases the same way: the crash watchdog despawned that actor and keeps only the sticky poison flag (ADR-049 §10), so a close that refused it would hold the registry entry for the life of the process, because `clear_poison` is an operator action no bridge exports. A `Closing` supervisor state does NOT skip the dispatch: `ttl::close_context` drives `Active` → `Closing` and a separate `FinalizeClose` command ends the §5.9 cooperative window, so `Closing` is a live non-terminal state, and the dispatch carries the only `ContextClose` capability check this path has. Skipping it let a caller holding no `context:close` capability release the `UcanContextState` — the outlet registry, the outlet handlers, the session store, the event log, the nonce tracker, the revocation list — for every identity sharing the bridge instance while the context was still in use. The release path out of `Closing` is `contextFinalizeClose`, which transitions the context to `Closed`. Every live non-terminal state (`Creating`, `Closing`, `MigratingOut`) refuses the close.
...
- **An absent ceiling and an empty ceiling are different declarations**: `parse_context_params` resolves an absent `ceiling` key, and a `ceiling` key holding `null`, to `default_ceiling()` rendered as `{resource}:{action}` strings, because `default_ceiling`'s own doc comment says every FFI bridge applies it "when no explicit ceiling is provided". A supplied array stands as written, so `ceiling: []` builds a context whose ceiling grants nothing, and a `ceiling` of any other JSON type, plus any non-string entry inside the array, rejects with `VALID_7000` rather than dropping out of the parsed ceiling. The supervisor installs the parsed vector verbatim and every authorization gate reads the actor's ceiling, so before this substitution existed an omitted `ceiling` built a NAPI context that refused every `ucan_mint`, `ucan_delegate`, and `ucan_validate`, while the same omission on the `PyO3` bridge granted the eleven defaults. The `PyO3` bridge applies the same rule in `PyContextParams::from_py_dict`; the UniFFI `ContextParams.ceiling` field is a required `Vec<String>`, so a UniFFI caller cannot omit it and an empty vector grants nothing there too.
