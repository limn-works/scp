...
- **Authorization reads the supervisor actor, never a bridge copy**: `UcanContextState` holds no role state. Every entry point that decides authorization, membership, a role, a capability, or a ceiling calls `runtime::live_role_state(bi, context_id)` (or `runtime::live_ceiling_strings`), which queries the per-context supervisor actor and fails closed when that actor holds no role state. The former `role_state` field, plus the `sync_role_state_from_manager` and `sync_ceiling_from_params` helpers that refreshed it and the `core.ceiling_strings` / `core.creator_did` reads at the UCAN validate, evaluate, mint, delegate, and outlet-invocation sites, are gone: a copy refreshed on a local mutation still went stale on every change the supervisor applied by another route (a governance execution, a broadcast-subscriber removal, a TTL expiry, a trust-recovery transition). The `PyO3` reference bridge reads the actor at the same decisions, so the two bridges answer one authorization question one way.
- **A lifecycle gate reads the supervisor actor, never the handle string**: `context_join_on`, `context_leave_on`, `context_send_on`, `context_subscribe_on`, and every outlet entry point call `runtime::require_active_context(bi, context_id, verb, mk_err)`, which reads the supervisor actor and refuses unless it reports `Active`. `mk_err` keeps each operation's own `ScpNapiError` variant and error code. It fails closed when no actor answers. `NapiContextHandle::state` stays a cached best-effort snapshot, and `context_close_on` still writes `"closed"` into it, but no gate reads it: that string records only the transitions THIS bridge observed, so a TTL expiry the supervisor applied on its own timer, a close another member initiated, a migration, and an actor poison all leave it reading `"active"`.
