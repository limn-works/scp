...
- **Close authorization**: `context_close_on` delegates to the supervisor's `LifecycleCommand::CloseContext`, which checks the `ContextClose` capability. Its gate reads `runtime::read_live_context_state`, the `Option` form of the live read: an absent actor (a completed TTL expiry) and a `Closing`/`Closed`/`Expired`/`Tombstoned` supervisor state skip the dispatch and still release the `UcanContextState`, because that release has no other path after creation. A `Poisoned` supervisor state releases the same way: the crash watchdog despawned that actor and keeps only the sticky poison flag (ADR-049 §10), so a close that refused it would hold the registry entry for the life of the process, because `clear_poison` is an operator action no bridge exports. Only a live non-active state (`Creating`, `MigratingOut`) refuses the close.
