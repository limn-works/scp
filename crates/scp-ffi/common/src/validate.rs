...
///
/// Outlet IDs are derived from outlet names (e.g., `tool-my-outlet`). Per spec
...
fn validate_context_params_strings(
    ...
    for tool_reg in &params.tools {
        validate_outlet_name(&tool_reg.name)?;
...
mod tests {
    ...
    #[test]
    fn valid_tool_name() {
        assert!(validate_outlet_name("my-tool").is_ok());
    }
    ...
    fn tool_name_with_braces_rejected() {
        let err = validate_outlet_name("tool-{name}").unwrap_err();
    ...
    #[test]
    fn valid_tool_id() {
        assert!(validate_outlet_id("tool-my-tool").is_ok());
    }

    #[test]
    fn valid_tool_id_with_underscores_and_digits() {
        assert!(validate_outlet_id("my_tool_42").is_ok());
    }
    ...
    fn tool_id_with_uppercase_rejected() {
        let err = validate_outlet_id("Tool-My-Tool").unwrap_err();
        assert!(err.message.contains("invalid characters"));
