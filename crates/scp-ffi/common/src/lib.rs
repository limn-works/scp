...
//!
//! # Resolver architecture (#311)
//!
//! Two resolver implementations:
//!
//! - [`BridgeDidResolver`] — String-only resolver that extracts public keys
//!   from `did:dht:z` strings via z-base-32 decode. No network calls. Used
//!   by WASM (cannot depend on scp-identity, see ADR-034) and tests.
//!
//! - [`IdentityBackedDidResolver`] — Production resolver that delegates to
//!   `scp_identity::resolver::DidResolver` (i.e., `DualLayerResolver`) for
//!   full DID document validation: BEP44 signature verification,
//!   self-certification, sequence number comparison, caching, and healing.
//!   Implements both `scp_core::crypto::ucan::validate::DidResolver` and
//!   `scp_core::trust::attestation::DidPublicKeyResolver`.
//!
