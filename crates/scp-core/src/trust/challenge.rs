...
//! [`AgentCapabilityUri`] (ADR-041), unifying the previously separate
...
use super::capability_uri::AgentCapabilityUri;

// ---------------------------------------------------------------------------
// ChallengeType
// ---------------------------------------------------------------------------

/// The type of challenge to issue, identified by an [`AgentCapabilityUri`].
///
...
impl ChallengeType {
    ...
    pub fn prompt_injection_resistance() -> Self {
        Self::Uri(AgentCapabilityUri::Protocol {
    ...
    /// Returns a reference to the inner [`AgentCapabilityUri`].
    #[must_use]
    pub const fn uri(&self) -> &AgentCapabilityUri {
...
impl<'de> Deserialize<'de> for ChallengeType {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        if let Some(custom_name) = s.strip_prefix("Custom:") {
            ...
            if let Ok(uri) = custom_name.parse::<AgentCapabilityUri>() {
                return Ok(Self::Uri(uri));
            }
            // Legacy unstructured custom strings can't be represented as
            // AgentCapabilityUri — reject them.
            return Err(serde::de::Error::custom(format!(
        ...
        let uri: AgentCapabilityUri = s.parse().map_err(serde::de::Error::custom)?;
        Ok(Self::Uri(uri))
    }
}
...
/// || challenge_type_uri || challenger_did || subject_did || parameters
