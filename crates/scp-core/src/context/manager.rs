...
impl ContextManager {
    ...
    // -------------------------------------------------------------------
    // Paid action routing (SCP-156)
    // -------------------------------------------------------------------

    /// Routes an action through the payment integration sequence when the
    /// context has an economic policy with non-zero cost, or bypasses
    /// payment entirely for free actions.
    ///
    /// This is the [`ContextManager`]-level entry point for the 9-step
    /// action-payment integration sequence described in spec section 19.2.2.
    ///
    /// # Arguments
    ///
    /// * `handle` -- The context handle.
    /// * `action_type` -- The type of paid action.
    /// * `payer_did` -- The DID of the payer.
    /// * `sender_metrics` -- Observable metrics from the payer's perspective.
    /// * `receiver_metrics` -- Observable metrics from the receiver's perspective.
    /// * `spending_auth` -- Verified spending UCAN constraints. `None` if
    ///   the payer has no spending UCAN (will fail for paid actions).
    ...
    /// # Returns
    ///
    /// The action result and an optional [`ActionOutcome`] containing the
    /// payment receipt. `None` outcome means the action was free.
    ///
    /// # Errors
    ///
    /// Returns [`IntegrationError`] if any step of the payment sequence
    /// fails. Returns [`ContextError`] (wrapped) if the context is not
    /// active or not registered.
    ///
    /// See spec section 19.2.2 and SCP-156.
    pub async fn execute_action_with_payment<T, A, F, Fut>(
        &self,
        handle: &ContextHandle,
        action_type: &crate::economy::PaidActionType,
        payer_did: &DID,
        sender_metrics: &crate::economy::ObservableMetrics,
        receiver_metrics: &crate::economy::ObservableMetrics,
        spending_auth: Option<&crate::economy::SpendingAuth>,
        adapter: &A,
        metadata: crate::economy::PaymentMetadata,
        process_action: F,
    ) -> Result<(T, Option<crate::economy::ActionOutcome<()>>), ContextError>
    where
        A: crate::economy::PaymentAdapter,
        F: FnOnce(&crate::economy::PaymentAuthorization) -> Fut,
        Fut: std::future::Future<Output = Result<T, String>>,
    {
        let context_id = handle.context_id().to_owned();

        // Verify context is active and retrieve economic policy.
        let economic_policy = {
            let contexts = self.contexts.lock().await;
            let ctx = contexts
                .get(&context_id)
                .ok_or_else(|| ContextError::MembershipFailed("context not registered".into()))?;
            require_active(&ctx.handle)?;
            ctx.handle.params().economic_policy.clone()
        };

        // Check if action is free (no policy or zero cost).
        if crate::economy::is_free_action(
            economic_policy.as_ref(),
            action_type,
            sender_metrics,
        ) {
            // Free action bypass: execute directly without payment.
            let result = crate::economy::execute_free_action(|| async {
                // Create a dummy auth for the closure.
                let dummy = crate::economy::PaymentAuthorization {
                    auth_id: [0u8; 32],
                    payer: payer_did.clone(),
                    payee: payer_did.clone(),
                    amount: crate::economy::Amount(0),
                    currency: crate::economy::CurrencyCode::from(""),
                    adapter_id: String::new(),
                    created_at: 0,
                    expires_at: 0,
                    adapter_state: Vec::new(),
                };
                process_action(&dummy).await
            })
            .await
            .map_err(|e| ContextError::MembershipFailed(e.to_string()))?;
        ...
        // Paid action: route through the integration sequence.
        crate::economy::execute_paid_action(
            economic_policy.as_ref(),
            action_type,
            payer_did,
            sender_metrics,
