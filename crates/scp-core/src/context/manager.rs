...
//! public [`create_context`](ContextManager::create_context) API. It
//! delegates to [`builder::create_context`] for the two-phase commit flow.
//!
...
use super::builder::{
    ContextCryptoProvider, ContextCreationError, ContextEventLogProvider, ContextTransportProvider,
    create_context as builder_create_context,
};
use super::{ContextHandle, ContextParams};
...
/// let handle = manager.create_context("ctx-1".into(), params).await?;
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Reusable mock providers (same as builder tests, but owned by manager)
    // -----------------------------------------------------------------------
    ...
    async fn manager_create_context_encrypted_success() {
        ...
        let handle = manager
            .create_context("mgr-ctx-1".into(), ContextParams::default())
            .await;
    ...
    async fn manager_create_context_broadcast_success() {
        ...
        let handle = manager.create_context("mgr-ctx-bc".into(), params).await;
    ...
    async fn manager_create_context_transport_disconnected() {
        ...
        let result = manager
            .create_context("mgr-ctx-dc".into(), ContextParams::default())
            .await;
    ...
    async fn manager_create_context_rollback_on_crypto_failure() {
        ...
        let result = manager
            .create_context("mgr-ctx-fail".into(), ContextParams::default())
            .await;
    ...
    async fn manager_preserves_params_on_handle() {
        ...
        let handle = manager
            .create_context("mgr-ctx-p".into(), params.clone())
            .await
            .unwrap();
