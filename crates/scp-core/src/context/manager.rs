...
use super::governance::{
    GovernanceAction, GovernanceEngine, GovernanceModelConfig, GovernanceProposal, ProposalId,
    ProposalStatus, PruningPolicy, RevocationScope, SingleAdminEngine,
    majority::MajorityVoteEngine, multisig::ThresholdEngine, unanimity::UnanimityEngine,
};
use super::membership::{ContextEvent, KeyPackage, MembershipState, ReceiveBuffer};
use super::params::{ContextMode, GovernanceModel, TemplateId, ToolRegistration};
...
struct PerContextState {
    ...
    /// Governance engine for this context (ADR-031, SCP-267).
    ///
    /// Wrapped in its own `std::sync::Mutex` (separate from the global
    /// `contexts: tokio::sync::Mutex<HashMap>`) so governance operations do
    /// not hold the global lock during engine calls. The engine is immutable
    /// for the context's lifetime (per ADR-031 §4).
    ///
    /// Consumed by the governance proposal/approval flow (SCP-268 through
    /// SCP-274). Currently constructed at creation and stored; downstream
    /// stories wire the propose/approve/reject API through this field.
    #[allow(dead_code)]
    governance_engine: std::sync::Mutex<Box<dyn GovernanceEngine>>,
}
...
/// Constructs a default [`GovernanceEngine`] from a [`GovernanceModel`] and
/// the creator DID.
///
/// For `SingleAdmin`, the creator is the admin. For other models, the creator
/// is the sole voter/signer with default voting windows. Callers who need
/// explicit configuration should use [`build_governance_engine`] instead.
fn default_governance_engine(
    model: &GovernanceModel,
    creator_did: &DID,
) -> Result<Box<dyn GovernanceEngine>, ContextCreationError> {
    match model {
        GovernanceModel::SingleAdmin => Ok(Box::new(SingleAdminEngine::new(creator_did.clone()))),
        GovernanceModel::Threshold => {
            let engine = ThresholdEngine::new(vec![creator_did.clone()], 1, 86_400)
                .map_err(|e| ContextCreationError::CreationFailed(e.to_string()))?;
            Ok(Box::new(engine))
        }
        GovernanceModel::Majority => {
            let engine = MajorityVoteEngine::new(vec![creator_did.clone()], 86_400, 5000)
                .map_err(|e| ContextCreationError::CreationFailed(e.to_string()))?;
            Ok(Box::new(engine))
        }
        GovernanceModel::Unanimity => {
            let engine = UnanimityEngine::new(vec![creator_did.clone()], 172_800)
                .map_err(|e| ContextCreationError::CreationFailed(e.to_string()))?;
            Ok(Box::new(engine))
        }
    }
...
fn build_governance_engine(
    ...
    match config {
        GovernanceModelConfig::SingleAdmin { admin_did } => {
            Ok(Box::new(SingleAdminEngine::new(admin_did)))
        }
        GovernanceModelConfig::Threshold {
            signers,
            threshold,
            ...
            let engine = ThresholdEngine::new(signers, threshold, voting_window_secs)
                .map_err(|e| ContextCreationError::CreationFailed(e.to_string()))?;
            Ok(Box::new(engine))
        }
        GovernanceModelConfig::Majority {
            ...
            let engine =
                MajorityVoteEngine::new(initial_voters, voting_window_secs, min_participation_bps)
                    .map_err(|e| ContextCreationError::CreationFailed(e.to_string()))?;
            Ok(Box::new(engine))
        }
        GovernanceModelConfig::Unanimity { voting_window_secs } => {
            let engine = UnanimityEngine::new(initial_voters, voting_window_secs)
                .map_err(|e| ContextCreationError::CreationFailed(e.to_string()))?;
            Ok(Box::new(engine))
        }
    }
...
impl ContextManager {
    ...
    pub async fn restore_context(
        ...
        // Reconstruct the governance engine from persisted state (SCP-267).
        let creator_did: DID = ctx_snapshot.role_state.creator_did.clone().into();
        let restored_engine: Box<dyn GovernanceEngine> = match ctx_snapshot
            .context_params
            .governance
        {
            GovernanceModel::SingleAdmin => Box::new(SingleAdminEngine::new(creator_did.clone())),
            GovernanceModel::Threshold => {
                // Restore from persisted threshold signers/value.
                let signers = if ctx_snapshot.threshold_signers.is_empty() {
                    vec![creator_did.clone()]
                } else {
                    ctx_snapshot.threshold_signers.clone()
                };
                let threshold = if ctx_snapshot.threshold_value == 0 {
                    1
                } else {
                    ctx_snapshot.threshold_value
                };
                Box::new(
                    ThresholdEngine::new(signers, threshold, 86_400).map_err(|e| {
                        ContextError::PersistenceFailed(format!(
                            "failed to restore ThresholdEngine: {e}"
                        ))
                    })?,
                )
            }
            GovernanceModel::Majority => {
                // Restore with current members as eligible voters.
                let voters: Vec<DID> = ctx_snapshot
                    .membership
                    .members()
                    .map(|m| m.did.clone())
                    .collect();
                ...
                Box::new(MajorityVoteEngine::new(voters, 86_400, 5000).map_err(|e| {
                    ContextError::PersistenceFailed(format!(
                        "failed to restore MajorityVoteEngine: {e}"
                    ))
                })?)
            }
            GovernanceModel::Unanimity => {
                ...
                Box::new(UnanimityEngine::new(voters, 172_800).map_err(|e| {
                    ContextError::PersistenceFailed(format!(
                        "failed to restore UnanimityEngine: {e}"
                    ))
                })?)
            }
        };
        ...
        let per_context = PerContextState {
            ...
            governance_engine: std::sync::Mutex::new(restored_engine),
        };
    ...
    pub async fn create_context(
        ...
        // Construct the governance engine from GovernanceModel (SCP-267, ADR-031)
        // and mint UCAN tokens for designated voters per ADR-031 §6.
        let governance_engine = default_governance_engine(&params.governance, &creator_did)?;
        ...
        let per_context = PerContextState {
            ...
            threshold_value: 0,
            pruning_policy: None,
            governance_engine: std::sync::Mutex::new(governance_engine),
        };
    ...
    pub async fn create_context_with_governance(
        ...
        let governance_engine =
            build_governance_engine(governance_config, vec![creator_did.clone()])?;
...
mod tests {
    ...
    async fn governance_single_admin_engine_constructed() {
        ...
        let engine = ctx.governance_engine.lock().unwrap();
        let config = engine.model_config();
        assert_eq!(
            config,
    ...
    async fn governance_threshold_engine_constructed() {
        ...
        let engine = ctx.governance_engine.lock().unwrap();
        assert_eq!(engine.model_config(), config);
    }
    ...
    async fn governance_ucan_tokens_minted_for_threshold_signers() {
        ...
        let engine = build_governance_engine(config, vec![creator.clone()]).unwrap();
    ...
    async fn governance_ucan_tokens_minted_for_single_admin() {
        let creator: DID = "did:key:creator1".into();
        let engine = Box::new(SingleAdminEngine::new(creator.clone()));
