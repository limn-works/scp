...
use scp_protocol::context::outlets::error_codes::CODE_ECONOMIC_FAULT;
...
async fn invoke_outlet_with_economy_rejects_insufficient_budget() {
    ...
    assert!(
        msg.contains(CODE_ECONOMIC_FAULT) && msg.contains("economic.budget-exceeded"),
        "expected {CODE_ECONOMIC_FAULT} economic.budget-exceeded error, got: {msg}"
    );
