...
//!    point rejects the action and returns a [`CategoryARejection`] carrying the
//!    [`CustodyViolationType::CategoryAViolation`] record a caller logs, with the
//!    violating signature as that record's evidence. A verification point signs
//!    no record itself: a [`ScpCustodyViolationAttestation`] names a
//!    `verifier_did` and carries that verifier's signature, and a caller that
//!    holds a signing key builds it from the returned record.
//!
//! # Types
//!
...
//!   No shipped artifact implements it; `InMemoryViolationStore` is compiled
//!   only under `cfg(test)` and this crate's `testing` feature, because two
//!   `HashMap`s a process drops on exit do not store the permanent record
//!   ADR-039 requires.
//!
...
/// In-memory implementation of [`ViolationStore`] for tests.
///
/// Compiled only under `cfg(test)` and under this crate's `testing` feature,
/// which no shipped artifact enables. ADR-039, shared-DID human-agent identity
/// model (`.docs/adrs/phase-1.md`), calls a custody-violation record permanent,
/// and two `HashMap`s that a process drops on exit do not store a permanent
/// record. Gating this type keeps it out of every shipped artifact, so a
/// production build has no `ViolationStore` at all rather than one that lies
/// about durability. §27.5 of `.docs/specs/27-attestations.md` records the
/// absence, and open question OQ-26 asks a human which durable store holds
/// these records.
