...
//!   signature digest — byte-identical across runs and implementations
//!   (ADR-050, the `BTreeSet` convention named in §23.16.8). Deserialization
...
//! - [`serde_sorted_capability_map`] — serializes a
//!   `HashMap<String, HashSet<Capability>>` with the inner sets sorted (the
//!   outer string-keyed map is already canonicalized by JCS object-key
//!   sorting).
...
/// makes the output stable across runs and implementations.
///
...
pub mod serde_sorted_set {
        ...
        S: Serializer,
    {
        ...
        // JSON bytes. Falls back to default-Serialize JSON only if JCS itself
        // fails, which cannot occur for well-formed set members.
        let mut keyed: Vec<(Vec<u8>, &T)> = set
            .iter()
            .map(|e| (crate::jcs::to_vec(e).unwrap_or_default(), e))
            .collect();
