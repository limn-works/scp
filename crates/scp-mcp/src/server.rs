...
/// Describes a tool available in a context, as seen by the MCP server.
///
/// Simplified view of `scp-core`'s `ToolRegistration` -- just what the MCP
/// layer needs for listing and invocation.
#[derive(Debug, Clone)]
pub struct ContextToolInfo {
    /// The tool name (without context prefix).
    pub name: String,
...
impl<P: ContextProvider> McpServer<P> {
    ...
    fn handle_tools_list(&self, request: &JsonRpcRequest) -> JsonRpcResponse {
        ...
        for context_id in self.provider.active_context_ids() {
            ...
            // Context-registered tools -- filtered by capability.
            for tool_info in self.provider.context_tools(&context_id) {
                // Skip admin-only tools for non-admin agents.
                if tool_info.admin_only && !agent_role_is_admin(&context_id) {
                    continue;
                }
                ...
                if self
                    .provider
                    .validate_capability(&context_id, &tool_info.name)
                    .is_ok()
                {
                    tools.push(context_tool_definition(
                        &context_id,
                        &tool_info.name,
                        tool_info.description.as_deref(),
    ...
    fn handle_tools_call(&self, request: &JsonRpcRequest) -> JsonRpcResponse {
        ...
        let (context_id, tool_name) = match parse_namespaced_tool(&params.name) {
            Ok(parsed) => parsed,
            Err(e) => {
...
mod tests {
    ...
    fn tools_list_filters_admin_only_tools_for_member_role() {
        ...
        assert!(!names.contains(&"ctx_b/admin_tool"));
        assert!(names.contains(&"ctx_b/member_tool"));
    }
