...
/// Configuration for starting an MCP server that exposes SCP context tools.
///
/// Specifies the identity, contexts, transport mode, and optional UCAN
/// authorization for an MCP server instance. When the UniFFI XCFramework
/// build pipeline ships (SCP-103), this definition is replaced by the
...
public nonisolated struct McpServerConfig: Sendable {
    ...
    /// immediately (UCAN is required for tool invocation per section 6.2).
    public let ucanToken: String?

    /// Optional proof tokens for UCAN delegation chain verification.
    public let proofTokens: [String]?
