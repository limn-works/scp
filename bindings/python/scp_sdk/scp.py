...
class SCP:
    ...
    async def event_log_verify(self, context_id: str, claim: dict[str, Any]) -> Any:
        ...
        - ``SCP-CTX-2139`` -- the authoritative log WAS read and no proof could
          ...
          the tree), OR no proof was issued (an EMPTY but live log, where an
          absence claim is actually TRUE but ``prove_absence`` DECLINES to
          issue a proof -- an explicit ``leaf_count == 0`` guard, a deliberate
          choice and NOT a limit of the construction).
