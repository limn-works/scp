...
async def execute_governance_action(
    ...
    Args:
        scp: The :class:`scp_sdk.SCP` instance that owns this context.
    ...
    """
    native = scp._native
    raw = await asyncio.to_thread(native.governance_execute, context._handle, proposal_json)
...
async def propose_ttl_extension(
    ...
    return await execute_governance_action(scp, context, proposal)


async def handle_ttl_expiry(scp: SCP, context: Context) -> GovernanceActionResult:
...
async def reset_ttl_timer(
    ...
    Raises:
        RuntimeError: If governance execution fails.
    """
    return await propose_ttl_extension(scp, context, ttl_seconds)
...
async def propose_governance_action(
    ...
    *,
    ...
    return await asyncio.to_thread(native.governance_propose, context._handle, did, action_json)
...
async def withdraw_governance_vote(
    ...
    *,
    ...
    did = identity_did or context._creator_did
    return await asyncio.to_thread(
        native.governance_withdraw, context._handle, did, proposal_id_hex
    )
...
async def list_governance_proposals(scp: SCP, context: Context) -> str:
    ...
    """
    native = scp._native
    return await asyncio.to_thread(native.governance_list_proposals, context._handle)
...
async def apply_pending_ceiling_modification(
    ...
    native = scp._native
    return await asyncio.to_thread(
        native.apply_pending_ceiling_modification, context._handle, current_timestamp
    )


async def finalize_close(scp: SCP, context: Context) -> None:
    ...
    """
    native = scp._native
    await asyncio.to_thread(native.finalize_close, context._handle)
...
async def create_governance_checkpoint(
    ...
    *,
    ...
    return await asyncio.to_thread(
        native.create_governance_checkpoint,
        context._handle,
        checkpoint_seq,
        merkle_root_hex,
        event_count,
...
async def add_checkpoint_cosignature(
    ...
    *,
    ...
    native = scp._native
    return await asyncio.to_thread(
        native.add_checkpoint_cosignature,
        context._handle,
        checkpoint_json,
        signer_did,
        signature_hex,
...
async def restore_context(scp: SCP, context_id: str) -> None:
    """Restore a single persisted context from storage.

    Args:
        scp: The :class:`scp_sdk.SCP` instance that owns the context store.
    ...
    """
    native = scp._native
    await asyncio.to_thread(native.restore_context, context_id)


async def restore_all_contexts(scp: SCP) -> str:
    ...
    """
    native = scp._native
    return await asyncio.to_thread(native.restore_all_contexts)
