...
For each OpSpec in SEED_OPS and each alt-bridge in {"napi", "wasm"}:

  1. Run the op on the in-process PyO3 bridge, normalize the output.
  2. Run the op via the Bun child on the alt bridge, normalize the output.
  3. Assert the two normalized dicts are equal; on failure, show a unified
     diff of the two dicts so the divergence is obvious.

Known divergences are declared on `OpSpec.xfail_bridges` and surface as
`@pytest.mark.xfail(strict=True)` — the test still runs but an unexpected
pass is also a failure. That way the xfail list stays a live snapshot of
what's broken; when a bridge is fixed upstream, removing the xfail tag
is the forcing function.

The test surface is the op library — adding a new op adds 2 more
parametrized test cases. See ADR-046.
"""
...
def _resolve(d: dict[str, Any], path: str) -> Any:
    ...
    Used to apply `OpSpec.expected_values` assertions after the canonical
    equality check. Raises AssertionError with a caller-meaningful message
    if any segment is missing or traverses a non-dict — both are bugs in
    the op definition that should surface loudly rather than None out.
    """
    parts = path.split(".")
    cur: Any = d
...
def op_context() -> OpContext:
    ...
    # actually runs — avoids coupling the rest of the Python test suite
    # to the PyO3 build availability.
    import scp_sdk._scp_core as scp_core  # type: ignore[import-not-found]

    return OpContext(scp_core=scp_core, scratch={})


def _pytest_param_for(op: OpSpec, alt_bridge: str) -> Any:
    ...
    return pytest.param(op, alt_bridge, marks=marks, id=f"{alt_bridge}-{op.name}")


_PARITY_CASES = [_pytest_param_for(op, bridge) for op in SEED_OPS for bridge in ("napi", "wasm")]


@pytest.mark.parametrize(("op", "alt_bridge"), _PARITY_CASES)
def test_parity(
    op: OpSpec,
    alt_bridge: Literal["napi", "wasm"],
    bun_runner: RunnerClient,
    ...
    response = bun_runner.call(op=str(node_op), args=args, mode=alt_bridge)
    ...
    # Guards against joint drift: if PyO3 and the alt bridge both changed
    # to the same wrong value, step 3 would pass but the spec would be
    # silently violated. See OpSpec.expected_values.
    for path, expected in op.expected_values:
        ...
        # expected_values is ever invoked for an xfail'd field, checking
        # each side independently keeps the error message precise.
