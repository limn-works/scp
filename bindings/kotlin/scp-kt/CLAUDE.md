...
- **Gradle task:** `./gradlew :scp-kt:generateUniffiBindings`
- **Output directory:** `src/main/kotlin/works/limn/scp/internal/` (gitignored, regenerated)
...
./gradlew :scp-kt:build    # Full build (detekt + ktlint + compile + test)
./gradlew :scp-kt:test     # Tests only
./gradlew :scp-kt:test --tests "works.limn.scp.conformance.*"  # Conformance only
./gradlew :scp-kt:detekt   # Static analysis only
./gradlew :scp-kt:runKtlintFormatOverMainSourceSet  # Auto-format
```
...
```bash
./gradlew :scp-kt:publishToMavenLocal    # Local testing
./gradlew :scp-kt:publish                 # Deploy to Sonatype OSSRH
```
