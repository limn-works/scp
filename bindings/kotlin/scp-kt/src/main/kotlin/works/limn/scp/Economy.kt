 ...
 * @throws BridgeException (SCP-ECON-12070) if the currency is unknown; use the
 *     [formatAmount] overload taking `decimals` for unknown/custom currencies.
 */
fun formatAmount(
    amount: ULong,
    currency: String,
    ...
    val decimals =
        KNOWN_CURRENCY_DECIMALS[currency.uppercase()]
            ?: throw BridgeException(
                "unknown currency \"$currency\" has no known decimals; " +
 ...
 * @return The human-decimal representation.
 * @throws BridgeException (SCP-ECON-12070) if [decimals] is out of range.
 */
fun formatAmount(
    amount: ULong,
    decimals: Int,
    ...
    if (decimals < 0 || decimals > 100) {
        throw BridgeException(
            "decimals must be in 0..100, got $decimals",
            "SCP-ECON-12070",
        )
    }
    return formatWithDecimals(amount, decimals)
