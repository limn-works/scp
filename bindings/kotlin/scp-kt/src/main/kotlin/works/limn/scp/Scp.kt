...
class SCP internal constructor(
     ...
     *   index past the end of the tree), OR no proof was issued (an
     *   EMPTY but live log, where an absence claim is actually TRUE but
     *   `prove_absence` DECLINES to issue a proof — an explicit
     *   `leaf_count == 0` guard, a deliberate choice and NOT a limit of
     *   the construction). Never read the empty-log case as evidence
