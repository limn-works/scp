...
//
// Grade A: starts an in-memory relay, configures relay transport, and
// exercises the full identity -> context -> messaging pipeline over a real
// WebSocket relay.
//
// Prerequisites:
//   1. Build the Rust cdylib: cargo build -p scp-ffi-uniffi --features allow_in_memory_custody,server
 ...
 *
 * Grade A: starts an in-memory relay via `relayStartInMemory()`, configures
 * relay transport via `configureRelayTransport(relayUrl, did)`, then runs the
 * full test suite over the real relay connection.
 *
    ...
    companion object {
        ...
        fun checkNativeLibraryAndStartRelay() {
            try {
                ...
                // Start an in-memory relay and configure relay transport.
                // relayStartInMemory() is a suspend function generated by UniFFI;
                // we call it via runBlocking since @BeforeAll is synchronous.
                kotlinx.coroutines.runBlocking {
                    ...
                    uniffi.scp.configureRelayTransport(
                        relayUrl = relayUrl,
                        localDid = bootstrapIdentity.did(),
                    )
                }
