...
- **New `PreRotationCustody` trait in `scp_platform`.** Pre-rotation private-key material now lives behind a separate trait (distinct from `KeyCustody`) and a distinct `PreRotationKeyHandle` type — `From`/`Into` between `PreRotationKeyHandle` and operational `KeyHandle` are NOT provided, so the type system rejects accidental cross-substrate handoff. Spec §9.7.4.1 §3 storage isolation is enforced at compile time. `InMemoryPreRotationCustody` was the only backend this change wired. ADR-062, capability injection and prove-absent dev backends, §Decision 6 later re-gated that type to `scp-platform`'s `testing` feature, so a shipped build holds no `PreRotationCustody` backend and identity creation on a shipped build fails closed with `SCP-IDENT-1059`. A backend that satisfies substrate isolation (FIDO2 / paper / cold callback) is forward work under issue #1729, production `PreRotationCustody` backends, and RFC #2130, pre-rotation recovery custody; each such backend lands as a `PreRotationCustody` impl and changes no protocol type.
