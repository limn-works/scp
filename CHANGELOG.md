...
- **`verify_migration` signature widened** to `(old_did, old_document, new_did, migration_proof, pre_rotation_proof, rotated_at, now) -> Result<bool, IdentityError>` (was 4 args returning `bool`). Always-checked invariants (1-6, MODERATE assurance): (1) Ed25519 strict signature on the migration digest, (2) `old_public_key` self-certifies to `old_did`, (3) saturating future-skew bound (5 min) on `rotated_at`, (4) saturating past-window bound (5 years) on `rotated_at`, (5) hard epoch floor (`MIGRATION_EPOCH_FLOOR_UNIX_SECS = 1_700_000_000`, 2023-11-14 UTC) rejects pre-protocol timestamps even on a faulty verifier clock, (6) `pre_rotation_proof` MUST be `Some(_)` whenever the OLD DID document publishes a `PreRotationCommitment` service entry — STRONG assurance committed to at creation cannot be silently downgraded. Conditional invariants — applied only when `pre_rotation_proof` is `Some(_)` (STRONG assurance, 7-9): (7) `SHA-256(revealed_key) == commitment`, (8) `commitment` matches the OLD document's `PreRotationCommitment` service entry, (9) `revealed_key` self-certifies to `new_did`. See ADR-003 §4c.
