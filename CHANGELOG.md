...
- **`DidDht::create_identity` return type changed** to `(Identity, PreRotationKeyHandle)`. Callers persist the returned handle alongside the identity; the handle is the only reference to the pre-rotation private bytes (which never enter `ScpIdentity` itself — only the 32-byte SHA-256 commitment does).
...
- **`verify_migration` signature widened** to `(old_did, old_document, new_did, migration_proof, pre_rotation_proof, rotated_at, now) -> Result<bool, IdentityError>` (was 4 args returning `bool`). Now enforces seven invariants: (1) Ed25519 strict signature on the migration digest, (2) `old_public_key` self-certifies to `old_did`, (3-4) saturating future-skew (5 min) and past-window (5 years) bounds on `rotated_at`, and when `pre_rotation_proof` is present (5) `SHA-256(revealed_key) == commitment`, (6) `commitment` matches the OLD document's `PreRotationCommitment` service entry, (7) `revealed_key` self-certifies to `new_did`. See ADR-003 §4c.
...
- **New SDK accessor: rotation event JSON** for distributing `DidRotationEvent` to active contexts after migration. Surfaced as `Identity.rotation_event_json` (Python), `BridgeIdentityHandle.rotationEventJson` (TypeScript), and `Identity.rotationEventJson()` (Swift).
