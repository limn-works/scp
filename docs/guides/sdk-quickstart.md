...
let params = ContextParams {
    ceiling: vec!["msg:send".into(), "msg:receive".into()],
    ttl: Some(3600),
    ..ContextParams::default()
};

// ContextManager coordinates context lifecycle
let ctx = ContextManager::create_context(&identity, params, &custody, &storage).await?;
println!("Context ID: {}", ctx.context_id);
```
...
```rust
use scp_core::envelope::InnerEnvelopeParams;

// Send a message
let payload = b"Hello from SCP";
let params = InnerEnvelopeParams::new(payload.to_vec(), &identity, &ctx);
let envelope = ctx.encrypt_and_send(params, &custody, &transport).await?;

// Receive messages via transport subscription
let mut stream = transport.subscribe(&ctx.routing_id(), None).await?;
while let Some(event) = stream.next().await {
    match event {
        TransportEvent::Envelope(env) => {
            let decrypted = ctx.decrypt(&env, &custody).await?;
            println!("Received: {:?}", decrypted.payload);
        }
        _ => {}
    }
}
