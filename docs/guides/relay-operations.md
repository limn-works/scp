...
SQLite blob storage (`SqliteBlobStore`) and node storage (`SqliteStorage`) use versioned schemas. The `ProtocolStore` wraps values in `StoredValue<T>` envelopes (spec SS17.5) for forward compatibility. Schema migrations run automatically on startup.
