...
**Measured Mainline packet sizes (bencoded BEP44 `v`, constructed from wire bytes and round-trip parsed):** 501 bytes at 1 relay entry with `wss://relay.example.com/scp/v1`; 659 at 3; 716 at 3 with a realistic operator hostname; 912 at 5 realistic. Each further relay entry costs 79 bytes (short) or 98 (realistic); a further **verification-method** record costs **84**, which is a different figure — do not reuse the service marginal for it. The 1,000-byte cap admits 7 relay entries short, 5 realistic. §18.2.3's recommended minimum of 3 fits with 284 bytes spare. RFC 1035 compression saves only 12–28 bytes here, because only the root record's name carries the 52-character DID — compression is not what makes the core fit. **These figures are the `id=`-carries-the-SCP-fragment form; an earlier set published the shorter alias form without recording which, and was wrong by 22–58 bytes.**

**SCP's two mapping choices, both decided in §18.2.2C:** `t=` carries the SCP service type string verbatim (`t=SCPRelay`, `t=PreRotationCommitment`), and `id=` carries the entry's SCP fragment with the leading `#` removed (`id=scp-relay-1`, `id=0`, `id=active`) rather than the record alias — the alias is smaller but would not round-trip §18.2.2A's `service[].id`.
