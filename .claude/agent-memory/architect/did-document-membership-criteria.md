...
3. **Mainline bootstrap-core membership.** Clause (a) admits what a resolver needs to reach a relay — the relay list, because §18.5.1 makes Mainline resolution level 2, the first place an SDK learns of any relay. Clause (b) admits the verification material a verifier reads when no relay answers: `#active`, which §18.2.2A requires of every document, and the `PreRotationCommitment` entry, which §18.2.2A caps at one and does not require. A core for an identity that published no commitment therefore carries three elements, and whether an identity may be created carrying none is Discussion #1553, the non-committing-identity question, which Alec decides alongside issue #1729. `#0` carries no key bytes (the DID string encodes them) but the DNS packet still emits a `k0` record. `#agent` is excluded because ADR-039 makes it optional; a core-only resolver reports it unresolved and fetches from a relay.
...
**THREE OPEN QUESTIONS — Alec's call. All three are registered in `.docs/specs/00-open-questions.md`; read them there, do not resolve them.**
...
2. **Does the bootstrap core carry `#agent` and `alsoKnownAs`?** Excluding them costs: an `#agent`-signed artifact cannot be verified when all relays are down, so §9.7.1 fails closed and the identity is unadmittable; and a migrated identity looks live to a Mainline-only resolver, which is §9.12's `#0`-compromise recovery path. Size is not the reason — the core is 716 bytes at three relay entries with a realistic hostname against a 1,000-byte cap, and one more verification-method record costs 84.

3. **May an identity carry no `PreRotationCommitment`?** ADR-003 §4c invariant 7 already lets `pre_rotation_proof` be `None` when the old document publishes none, and the migration verifies at MODERATE assurance. No shipped creation path produces such a document, because `DidMethod::create` (`crates/scp-identity/src/dht.rs:2012`) takes a `PreRotationCustody` implementation it cannot omit. ADR-062, capability injection, assigns the creation question to Discussion #1553. A spec table that writes "exactly one required" into §18.2.2A answers it, so do not write one.
