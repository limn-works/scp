...
- [UniFFI Swift checksum staleness](uniffi_checksum_staleness.md) — CRITICAL recurring: hand-edited throwing sig + stale checksum int → Swift SDK fatalError on init; detect by regen+diff. Found in 8bcd520c2 (#1543). ALSO: adding/inserting a `ScpError` variant renumbers UniFFI wire tags — the TRACKED `bindings/swift/Sources/SCP/Internal/ScpBindings.swift` MUST be regenerated (`bindings/swift/build-xcframework.sh`) or every variant after the insertion point silently mis-decodes in Swift. Kotlin's `scp.kt` is gitignored/regenerated, so a Kotlin-only regen hides the Swift staleness. Detect: `grep -c NewVariantName bindings/swift/Sources/SCP/Internal/ScpBindings.swift`. Found again in SCP-OUT-031 PR-2b.
- [PyO3 multi-arg exceptions break `_coded_bridge_error`](pyo3_multiarg_exception_code_loss.md) — `Exception.__str__` returns `repr(args)` when len(args) >= 2, so the Python SDK's `^\[(SCP-[A-Z]+-\d+)\]`-anchored code extraction yields `code=None` and a tuple-repr message. Any new `X::new_err((a, b, ...))` needs a dedicated `exc.args`-reading translator (like `_saga_terminal_from_bridge`).
...
### Known Bug Patterns (Jun 2026 — check-handler-no-panic.sh stack fix, commit 60692e6 review)
- **VERIFIED CORRECT:** The flag→stack fix in scan_helper_file (gated_floor[]/gated_top) and scan_dispatch_hub (testing_floor[]/testing_top) is sound. Stack arithmetic correct for: deep nesting, sibling gates (push/pop same floor), while-loop multi-pop on one line (`}}`), double-gate single item, push/pop/push at different floors. `gated_top==0` short-circuit on `&&` is POSIX-portable (verified mawk 1.3.4 — system awk is mawk not gawk; `gated_floor[-1]` never evaluated, no auto-vivify). Per-file awk invocation → no cross-file stack leak. Self-test (8) is a GENUINE regression guard: old single-bool logic emits false-positive HIT on the case-8 fixture; new stack does not. Commit-message claim verified: old scanner = 5 false positives in supervisor.rs (lines 8458/8459/8554/9223/9225), new = 0. Real tree passes exit 0. MULTITEST counting + raw-line "testing" literal detection preserved in hub.
- **PRE-EXISTING (not introduced by 60692e6) — `#[cfg(test)]` on a NON-braced item false negative (MEDIUM, latent):** gated_pending latches: a test cfg gate on a `use`/`const`/`static`/`type` (no `{}`) leaves gated_pending=1 until the NEXT braced item, which is then wrongly treated as the gated region — swallowing a real production panic in that following fn. Confirmed: `#[cfg(test)]\nuse X;\npub fn h(){ panic!() }` → NO HIT. Same in old code. NOT live today (no such pattern in tree). Root cause: gated_pending should be cleared/decided by the item that the attribute actually applies to, not the next braced line. Local fix would require attribute-to-item association (clear gated_pending if the gated item turns out non-braced, e.g. ends in `;` at same depth).
- **PRE-EXISTING (not introduced by 60692e6) — char-literal brace `'{'`/`'}'` breaks depth accounting (LOW, latent):** only double-quoted strings are stripped, not char literals. An unbalanced `'{'` inside a gated region makes depth never return to floor → region stays active over following production code → real panic hidden (under-pop false negative). Same in old code. NOT live (0 occurrences of `'{'`/`'}'` in tree). Latent fragility of brace-counting design.
- **CONCLUSION:** Commit 60692e6 is a correct, well-tested bug fix. No NEW defects introduced. Two pre-existing latent false-negative vectors in the surrounding gated-region design, neither live today.

### check-handler-no-panic.sh — 77c3296 (stack→scalar floor) RE-REVIEW (Jun 2026)
- 77c3296 replaced gated_floor[]/gated_top stack + pop-loop with a scalar latched on FIRST entry (`if(!in_gated){in_gated=1;gated_floor=depth}`), cleared via `if(in_gated && depth<=gated_floor) in_gated=0`. Both scan_helper_file and scan_dispatch_hub (testing twin).
- VERDICT: behaviorally EQUIVALENT to stack for the in-scope property. Proven via 4000-iter random differential fuzz on BOTH functions (0 mismatches) + targeted fixtures (deep 3-level nest, siblings, same-line open/close gate, outer close, prod-after-test-module, unbalanced braces). System awk IS mawk 1.3.4 — scalar is strictly MORE portable (no arrays). Self-test case(8) is non-vacuous: a buggy re-latch-inner-floor variant produces a false HIT, scalar does not. Load-bearing element is the `if(!in_gated)` guard preventing inner floor from clobbering outer. NO new defects in the delta.
- Equivalence rests on "gated cfg regions are always brace-nested" — true for compiling Rust. The 2 KNOWN preexisting vectors (non-braced cfg latch leak; char-literal brace) are UNCHANGED by this commit (scalar == stack on both). Tracked as follow-up.
### Known Bug Patterns (Jun 2026 — event-log unification Phase 2 round 2, @dc18f5899)
- **HIGH fix VERIFIED correct+complete:** run_buffered_post_delivery(event_name: Option<EventType>) — append guarded on Some, velocity/consequence/checkpoint run unconditionally. All 4 buffered drain sites (messaging_helpers.rs 2244/2324/2434/2545) call it unconditionally. No 5th site missed. No double-count (top-level msg increments checkpoint separately from drained msgs; distinct messages). In-order normal path (2504-2606) pushes top-level msg inline, not via deliver_plaintext_or_announcement — no double-push.
- **Regression test value gap (LOW, not a code defect):** buffered_application_message_runs_velocity_consequence_and_checkpoint calls run_buffered_post_delivery DIRECTLY with event_name=None. It tests the HELPER contract, not the 4 call sites where the gating bug lived. Against pre-fix code it only fails to COMPILE (old sig took non-Option EventType). A re-introduced `if let Some` gate at a call site (keeping Option sig) would NOT be caught. Ideal test drives validate_and_drain_timeouts/buffer_ahead_message/deliver_message_and_drain_buffered end-to-end with an out-of-order app message.
- **Consequence decode rewrite (consequence.rs payload_target_did/payload_starts_with) SOUND:** rmp_array_first_string tried first, then JSON. JSON objects start with `{`=0x7B=msgpack fixint → read_value returns Integer not Array → falls to JSON. No collision (JSON never starts with msgpack fixarray header 0x90-0x9f). Custom trigger dropped dead `custom_key` branch (no producer ever wrote it) — now reads target_did only; preserves effective pre-existing behavior.
- **Typed payloads wired:** AccessRevokedPayload/GovernanceActionExecutedPayload emitted by governance_helpers (805) + consequence projection (governance_logic.rs 693-714) maps AccessRevoked/GovernanceActionExecuted/Consequence* → GovernanceAction bucket so WarningCount matches. Buffer source uses JSON target_did_to_payload; persisted source uses positional msgpack encode_payload; both decode.
- **leaf_hash extraction (tree.rs) clean:** shared SHA-256(0x00 ‖ rmp_serde::to_vec(Event)) incl signature. append + append_unsigned_event both use it. verify_merkle_chain replays via append_unsigned_event (sequence + prev_hash checked) → rejects prefix-trunc/reorder/remove-middle; suffix-trunc replays but root differs → signed-root compare rejects. transport serde to_vec_named differs from leaf to_vec but both deserialize to same Event so roots match.
- **All 4 FFI bridges (pyo3/napi/uniffi/common) migrated EventLogEntry→scp_event_log::Event consistently;** event_type_label = format!("{:?}") (all EventType variants unit-like → Debug == name). napi/pyo3/uniffi recompute leaf_hash for surfaced "hash" field. No orphaned compute_entry_hash/verify_chain_integrity/EventLogEntry refs.
- **PRE-EXISTING (not Phase 2):** prune_before_checkpoint time-prune `break`-on-first-retained is non-monotonic under structural_retention_multiplier — an early structural event retained longer shields newer prunable operational events from time-pruning. Identical in 1c0ccbc7d. Errs toward over-retention. Out of Phase-2 scope.

### Event-log Phase-2 substrate swap final review (Jun 2026, HEAD 3d96058f5) — CLEAN
- Latest commit test helpers (test_set_governance/test_insert_context/test_context_event_log_events + make_bare_per_context_state/test_insert_member/test_insert_ceiling) all `#[cfg(test)]` + pub(crate). Zero production exposure. Verified compile out of wasm32 build (clean clippy, no dead-code warnings).
- `now_ms` cfg-gating: wasm32 path = captured Date.now (unchanged, hardened); `#[cfg(not(wasm32))]` SystemTime fallback is test-only (lets native cargo test drive WASM handlers w/o JS runtime). Mutually-exclusive cfg, production wasm byte-identical to before.
- Non-backdatable deadline fix (4cad781e5): is_effective gates `current >= max(effective_at, observed_at+PERIOD)` w/ saturating_add. import re-pins observed_at to local clock (untrusted), RESTORE keeps verbatim (trusted self-respawn). Correct.
- Committer-assigned convergent timestamps (88c856360): only residual SystemTime::now() in event_log provider is prune_before_checkpoint retention cutoff (local prune decision, not leaf hash — legit). WASM governance leaves use now_secs = committer's own created_at (single-member model, matches native committer semantics). Receive path ms->s = `inner.timestamp/1000` consistent native+WASM.
