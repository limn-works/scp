...
- [PR #2415 custody vocabulary](pr2415-custody-vocabulary-published-enum.md) — request side SOUND (fixes the `file`-feature root cause); published 3-value fused enum UNSOUND: reachable image is a singleton, both non-extractable values unreachable, absent state licensed by an empty signal.
