...
## fix/sdk-coverage-fail-closed-and-parity @ 341df72cc (2026-06-22) — ALIGNED, 0 blocking, REBASED onto 1f1ea7cd2
See [sdk_coverage_failclosed_parity_341df72cc.md](sdk_coverage_failclosed_parity_341df72cc.md). 59 ahead/0 behind, merge-base==origin/main==1f1ea7cd2. KEY DELTA vs 0219e5c12: ADR-051(pre-rotation custody)→**ADR-053** (ADR-051 now causal-dag on main). Rename COMPLETE/clean; all residual ADR-051 refs are causal-dag (disjoint subsystem, untouched, no conflict). §9.12(NEW-DID migrate)/§3.2.1(DID-preserving custody) split verified cross-SDK + anchors @09/03. Fail-closed gate RAN: 223 ops, EXIT 0, 11 self-tests pass; sound-by-construction (true+unmatched→ERROR, all-exempted guard, positive ALIASES whitelist); ci.yml self-test-before-gate + CLAUDE.md enforcement-list = strengthening. PERM-3030 re-raise py:770+TS:461 aligns ADR-048 §4. ADR-053 artifact-flow clean (Proposed/Phase6/downstream). Matrix honesty-improving. provider.rs doc-only. Issue-refs NET-REDUCED (removed 16, added 4 reflows). 2 lessons accurate.

## fix/sdk-coverage-fail-closed-and-parity @ 0219e5c12 (2026-06-20) — ALIGNED, 0 HIGH/MED, REBASE-CLEAN FINAL
See [sdk_coverage_failclosed_parity_0219e5c12.md](sdk_coverage_failclosed_parity_0219e5c12.md). 3 past ed14e6c77. merge-base==origin/main==dabf13364, 33 ahead/0 behind — NO phantom deletions. Migrate disambiguation VERIFIED live: §3.2.1 "Key Custody Migration Protocol" case-2 "Identity Key migration" explicitly creates NEW DID via pre-rotation (ADR-003 §4b, §9.12); scp.ts:755 identityMigrate cites §9.12+§4b. evaluateTrust §7.2–7.5 (types.ts §9.3 = ADR-017 Consequence-rules, different subsystem). Gate RAN: 222 ops, 0 errors, EXIT 0, self-tests 9/9. Matrix rotate_key text corrected (honesty; entries already false). add_relay_url Kotlin coverage_exemption = real tree-sitter grammar gap. ADR-051 Proposed/Phase6. provider.rs doc-only. #632 refs pre-existing. APPROVED.

## fix/sdk-coverage-fail-closed-and-parity @ ed14e6c77 (2026-06-20) — ALIGNED, REBASE-CLEAN (SUPERSEDED by 0219e5c12)
See [sdk_coverage_failclosed_parity_ed14e6c77.md](sdk_coverage_failclosed_parity_ed14e6c77.md). REBASE TRAP RESOLVED: merge-base==origin/main==dabf13364, 31 ahead/0 behind, no phantom deletions (the ad51633f3 stale-base illusion gone). Migrate §9.12 disambiguation CORRECT: identity_migrate→§9.12+ADR-003§4b NEW DID (removed wrong §3.2.1+spurious §4c); custody_migration stays §3.2.1 PRESERVES DID. ADR-051 coherent/Proposed/Phase6, all cited code+spec locations verified live (§9.7.4.1@655, bridge.rs:689/736). Matrix changes honesty-improving not weakening (rotate_key false is pre-existing on main; only false exemption text corrected). evaluateTrust §7.2–7.5 not §9.3. test-guard positive allowlist+fail-closed. Same work as 44eaf5d05 rebased clean. APPROVED.

## fix/sdk-coverage-fail-closed-and-parity @ ad51633f3 (2026-06-20) — ALIGNED; CRITICAL: branch is STALE (~19 commits behind main)
HEAD ad51633f3 (1 past 44eaf5d05). Re-reviewed 4 focus areas: (1) TS evaluateTrust full parity w/ py — all 6 UCAN prefix lists byte-identical (incl 19-entry SIGNATURE_CHAIN), _PASSED_BEFORE maps match, classify order matches; py dispatches by context_id, TS by Context handle (documented per-SDK NAPI idiom, not a bug). (2) identityRotateKey=same DID, identityMigrate=NEW DID + rotationEventJson distribute — both match spec §9.12 (rotate_active_key ADR-003 §4a "DID does NOT change"; migrate_identity §4b "Creates a new DID") + py already had all 5 methods on main; TS adds them = closes parity gap. (3) ADR-051 problem statement grounded in §9.7.4.1 §3-§6 + ADR-003 §4b, cites real code locations, Proposed status. (4) matrix changes accurate (rotate_key exemption text corrected, add_relay_url coverage_exemption for tree-sitter-kotlin gap). **CRITICAL FINDING: merge-base=0c8f0b06, origin/main=dabf1336.** The given `git diff origin/main..HEAD` rendered main's NEWER work (event-log #1827 EventType expansion, heartbeat #1533, OwnedIdentityDid compiler-enforcement #1826, ADR-052 phase-B, saga #1849) as PHANTOM deletions — incl fake "reconnect/heartbeat removal", fake "signing_key_id re-add to Event", fake "§9.4.1 enforcement softening". Verified via `git diff 0c8f0b06..HEAD --stat`: branch touched NONE of those files; reconnect/ReconnectReport still live on both branch AND main. Real branch scope (26 files): SDK parity TS/py + fail-closed check-sdk-coverage.py + ADR-051 + matrix + MLS provider doc. Actionable: REBASE onto current main before merge; ensure rebase does NOT drop main's reconnect/event-log/heartbeat work. See [[two-dot-diff-stale-base-trap]].

## fix/sdk-coverage-fail-closed-and-parity @ 44eaf5d05 (2026-06-20) — ALIGNED, 0 blocking, FINAL
See [sdk_coverage_failclosed_parity_44eaf5d05.md](sdk_coverage_failclosed_parity_44eaf5d05.md). 4 commits past 27d82895e resolved EVERY prior finding: §9.3→§7.2–7.5 everywhere (types.ts:785 leftover FIXED); identityMigrate NEW-DID test/doc correct; assertTestHookAllowed → positive allowlist (NODE_ENV test|development|BUN_TEST); coverage gate null-safe `.text or b""` decode + coverage_exemptions non-empty validation. Verified on branch: gate EXIT 0 (221 ops, all teeth), ruff/biome/tsc clean. 4 TS test fails = ENVIRONMENTAL (addon built sans allow_in_memory_custody, not diff defect). MLS provider.rs doc → ADR-049 actor + remove stale "override" language (no crypto trait). ADR-051 Proposed, correct artifact-flow. 2 LOW carried (Real-NAPI probe gates on method-presence not custody-success; ADR-051 cites build-generated path).

## fix/sdk-coverage-fail-closed-and-parity @ 27d82895e (2026-06-20) — ALIGNED (1 LOW), SUPERSEDED
See [sdk_coverage_failclosed_parity_review.md](sdk_coverage_failclosed_parity_review.md). Cross-SDK parity + coverage-gate fail-closed hardening + ADR-051 (pre-rotation custody). Spec anchors: §3.2.1 identity migration = NEW DID; §7.2–7.5 four-layer trust (NOT §9.3 Sybil). LOW: types.ts:785 leftover `§9.3` miscitation — FIXED in 44eaf5d05.
...
- [SDK coverage fail-closed + parity (ae1fcecaa)](sdk_coverage_failclosed_parity_ae1fcecaa.md) — gate hardening + cross-SDK parity (TS evaluateTrust, py discover_contexts/verify_receipts); ALIGNED
