...
- See [project_branch_sdk_coverage_fail_closed.md](project_branch_sdk_coverage_fail_closed.md) -- ADR-051 (pre-rotation substrate isolation, Proposed) standalone file (consistent w/ ADR-046..050) + fail-closed check-sdk-coverage.py (listed in CLAUDE.md) + capability matrix at .docs/standards/sdk-capability-matrix.json. REBASED 2026-06-20: merge-base now dabf13364, 0 behind origin/main, two-dot==three-dot diff (no stale-base deletions). rotate_key exemption text CORRECTED this branch (was falsely "UniFFI does not export rotate_key"; bridge DOES export it).
- Two durable lessons promoted from agent-memory to .docs/lessons/ during this review: `coverage-gates-must-fail-closed.md` (gates default FAIL, exact-match symbols not suffix, negative self-tests, exemptions cite provenance) + `identity-migration-cite-9.12-not-3.2.1.md` (identity_migrate=new DID via pre-rotation=§9.12/ADR-003 §4b; custody_migration=DID-preserving=§3.2.1; don't conflate).
