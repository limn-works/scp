...
- See [project_branch_sdk_coverage_fail_closed.md](project_branch_sdk_coverage_fail_closed.md) -- ADR-051 (pre-rotation substrate isolation, Proposed) + fail-closed check-sdk-coverage.py + CLAUDE.md enforcement-list +1. CRITICAL: branch is 32 commits behind origin/main (merge-base 0c8f0b065); `origin/main..HEAD` two-dot diff shows huge stale-base deletions (incl. #1826 OwnedIdentityDid AST-gate drop appearing as a revert). MUST rebase before merge; use `git diff 0c8f0b065..HEAD` to see only branch-authored changes.
