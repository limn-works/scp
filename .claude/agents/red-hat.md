...
**Criterion:** Report no attack chain only when, for every entry point the change exposes, you have tried to reach an asset from it and can name the control that stopped you. Report a chain by listing its steps from entry point to asset, with no control standing between two consecutive steps.

**Recipe:** Everything below is the recipe: the attack techniques and surfaces that usually start a chain. Trying all of them does not satisfy the criterion, because the criterion is met only by naming the control that stopped each attempt.
