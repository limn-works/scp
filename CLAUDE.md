...
**Rust is pinned, and raising the pin is a deliberate change.** Eight locations name a Rust version. Five name the stable one: `rust-toolchain.toml` (which also pins the components and every cross-compilation target CI builds for), `.mise.toml`, `Dockerfile`, the container build documented in `templates/personal-relay/README.md`, and the toolchain table in `.docs/standards/rust.md`. Three name the nightly that `fuzz/` needs: `fuzz/rust-toolchain.toml`, and `FUZZ_TOOLCHAIN` in `.github/workflows/fuzz.yml` and in `.github/workflows/ci.yml`. `scripts/check-toolchain-pin.sh` fails when any of them disagree, when the compiler a command in the repository resolves to is not the pinned one, and when `Dockerfile`'s builder stage names a different Debian release than its runtime stage. mise sets `RUSTUP_TOOLCHAIN` for the commands it runs, and that variable overrides `rust-toolchain.toml` entirely, which is why `.mise.toml` must match — check it with `mise x -- printenv RUSTUP_TOOLCHAIN`, since plain `mise env` does not print it. To raise the version: change the five stable locations together, run `mise install` so the shell picks up the new toolchain, run `bash scripts/check-toolchain-pin.sh`, run the CI clippy command from the Orchestrator verification protocol below, and fix everything the new release reports in that same pull request. Never lower the pin to make a new lint disappear. See `.docs/lessons/pin-the-rust-toolchain-or-ci-drifts-from-local.md`, which records the merge-queue outage that a floating `@stable` caused.
