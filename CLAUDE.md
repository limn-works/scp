...
- Run `python3.12 scripts/validate-prd.py` before committing PRD changes. A `prd-validate` job in `.github/workflows/ci.yml` runs it on every pull request, and `ci`, this repository's one required status check, fails when that job does not pass.
