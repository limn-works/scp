...
**Prose (MANDATORY — governs every sentence you write for a human reader: chat responses, specs, ADRs, PRD stories, commit bodies, PR descriptions, code comments, review findings, README text, artifact copy):**
**The criterion:** a reader who does not already know the answer can reconstruct, from the sentence alone, who did what to what, and why. When a reader has to already know the answer in order to parse the sentence, the sentence has failed, however well it reads. Every rule below is a way that sentences fail this criterion. Satisfying all of them still does not satisfy the criterion, so check the criterion itself last. `.docs/lessons/prose-hidden-agents-worked-example.md` works one failed passage through every rule and rewrites it.
...
- **Six forbidden constructions.** Each one substitutes a rhetorical move for a claim. Delete it and write the claim.
...
- **Check every sentence before you send it.** Ask seven questions: have I hidden a subordinate relation? Have I dropped a subject or an object? Does a passive verb hide who acted? Have I appraised the work instead of reporting it? Have I referenced anything by a bare identifier? Have I used any of the six forbidden constructions? Does this sentence admit two contradictory readings? If any answer is yes, rewrite the sentence.
