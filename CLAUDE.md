...
compiles on it, ci.yml routes every root-level file and every cargo configuration
file to a lane or declares it unread, and every script a paths-filtered job runs is
listed in a filter that guards that job; .mise.toml names no Rust version source; each
workflow's verdict job reads the result of every job that workflow declares),
check-swift-bindings-fresh.sh (the committed UniFFI Swift bindings are byte-identical
to what uniffi-bindgen emits from the current scp-ffi-uniffi source),
...
**mise installs every tool except Rust, and `.mise.toml` names no Rust version.** mise exports one `RUSTUP_TOOLCHAIN` for the whole repository, and that variable overrides a toolchain file entirely, so it cannot give `fuzz/` a nightly while the workspace compiles on stable. rustup resolves both, per directory, and README.md lists rustup among the prerequisites. A `RUSTUP_TOOLCHAIN` exported into your shell from anywhere still overrides both files, so `scripts/hooks/pre-commit` compares `rustc --version` against the pin before it runs cargo, and `fuzz/build.rs` fails the fuzz build when that crate resolves a compiler its own file does not name. `scripts/check-toolchain-wiring.sh` checks the four properties a derivation cannot supply: that every container build carries the ASSERT-PINNED-RUSTC block, which makes the build compare the compiler it resolved against the copied-in pin, since the base tag no longer names a compiler; that the `dorny/paths-filter` wiring of every workflow that guards a job with one — `.github/workflows/ci.yml` and `.github/workflows/docs.yml` today — routes a pin change to every lane that compiles on it, that `ci.yml` routes every root-level file and every cargo configuration file to a lane or declares that no compile reads it, and that every script a paths-filtered job runs appears in a filter guarding that job, since the `ci` aggregator job counts a skipped job as a pass and the pull request that edits a gate script is the one that has to run it; that `.mise.toml`, parsed as TOML rather than matched line by line, gives its `tools` table no `rust` key and registers no `rust` idiomatic version file; and that each workflow's verdict job — the job whose own `if:` names one of the four status check functions GitHub Actions defines, `success()`, `always()`, `cancelled()`, or `failure()` — names every other job of that workflow in `needs:` and reads each of their results, because a job it does not name reports `skipped` when the `changes` job it depends on fails, and the aggregator passes on `skipped`. The gate parses each workflow with PyYAML for the same reason it parses `.mise.toml` with a TOML parser: a line matcher reads one spelling per pattern, and an author who rewords a workflow without changing what GitHub Actions reads makes that matcher find nothing and report nothing. See `.docs/lessons/pin-the-rust-toolchain-or-ci-drifts-from-local.md`, which records the merge-queue outage that a floating `@stable` caused.
