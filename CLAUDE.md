...
**Read to the frontier, then stop.** Derived from the instruction Alec gave for "Review the class, not the instance" above — that paragraph quotes the instruction and this one derives the reading bound, and the bound took review rounds from agents that died at the 200k-token context limit before writing a verdict to rounds that finish. The criterion: a reviewer reads the diff, expands along the edges the change perturbs, and stops at every node whose observable contract the change leaves unchanged. The edge list names where to look and does not define the frontier: callers, when the change alters what a caller can observe; consumers of a changed value, because a wire format or a signed preimage is read wherever the bytes are consumed, not where the function is called; implementors of a changed trait; twin sites, only when a held finding points there. A frontier that does not close within the reading budget is itself a finding — report "unbounded blast radius" — not a reading failure.
