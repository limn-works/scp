...
- **Severity**: HIGH
- **Resolution (later)**: Accepted and fixed. Spec §9.10.4 now pins the `Ed25519_keygen(context_seed[0..32])` interpretation: the 32 bytes are an RFC-8032 Ed25519 seed (standard SHA-512 expansion, then clamp the lower 32 bytes to form the scalar) — NOT an already-clamped scalar. Implementations MUST NOT treat the input as a pre-clamped scalar. This single interpretation is what makes software pseudonyms agree byte-for-byte across platforms. See spec §9.10.4 and the known-answer vectors in §25.19. Line references (549-551, 568-570) are from the spec revision at audit time and have since shifted. Preserved as the historical record that drove the fix.
...
- **Severity**: HIGH
- **Resolution (later)**: Accepted and fixed. The public-key-as-HMAC-key approach was rejected outright — using public key bytes (which any party can derive from a DID) as the HMAC key created a membership-enumeration oracle. Spec §9.10.4.A now makes the HMAC key the private-derived `pseudonym_secret`, which is NOT publicly derivable: software custody derives it via `HKDF-SHA256(ed25519_private_seed, salt="scp-pseudonym-secret-v1")` and is cross-platform deterministic; hardware custody uses a device-local secret computed inside the secure boundary and is device-local by design (the underlying key is non-exportable). The false "All implementations produce identical output" claim is replaced by the explicit software-deterministic / hardware-device-local distinction. See spec §9.10.4.A and the known-answer vectors in §25.19. Line references (549, 554, 560) are from the spec revision at audit time and have since shifted. Preserved as the historical record that drove the fix.
