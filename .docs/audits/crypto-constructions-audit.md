...
This audit covers all cryptographic constructions specified in `09-security-model.md` (sections 9.3-9.17), `03-identity.md` (key management, identity, rotation), and `07-trust-validation-and-capabilities.md` (UCAN, attestations). I have identified **9 CRITICAL**, **10 HIGH**, **8 MEDIUM**, and **5 LOW** severity findings.
...
- **What's missing**: The spec describes "HPKE assembly" as 4 manual steps: (1) generate an ephemeral P-256 keypair, (2) ECDH, (3) HKDF to derive key, (4) AES-128-GCM encrypt. This is NOT RFC 9180 HPKE. RFC 9180 defines specific modes (Base, PSK, Auth, AuthPSK) with mandatory `info`, `psk`, `psk_id` parameters and a specific `ExtractAndExpand` procedure. The spec says "HKDF to derive encryption key" without specifying: which HKDF mode (Extract only? Expand only? Both?), what the salt is, what the info string is, what the key length is, or what the nonce is for AES-128-GCM. The access key section (line 916) specifies `info = "scp-sender-key-v1" || context_id || member_did || epoch` but the sender key section does not reference this info string -- the info string appears only in the access key context as a contrast example.
...
- **Severity**: HIGH
- **Resolution (later)**: Accepted and fixed. The public-key-as-HMAC-key approach was rejected outright — using public key bytes, which any party can derive from an identifier, as the HMAC key created a membership-enumeration oracle. Spec §9.10.4.A now makes the HMAC key the private-derived `pseudonym_secret`, which is NOT publicly derivable: software custody derives it via `HKDF-SHA256(private_seed, salt="scp-pseudonym-secret-v1")` and is cross-platform deterministic; hardware custody uses a device-local secret computed inside the secure boundary and is device-local by design (the underlying key is non-exportable). The false "All implementations produce identical output" claim is replaced by the explicit software-deterministic / hardware-device-local distinction. See spec §9.10.4.A and the known-answer vectors in §25.19. Line references (549, 554, 560) are from the spec revision at audit time and have since shifted. Preserved as the historical record that drove the fix.
...
- **Security impact**: An attacker could construct a valid block notification for a different (context_id, blocker, blocked) triple. Exploitation requires finding specific identifier and context_id values whose concatenation collides, which is unlikely but theoretically possible.
- **Severity**: MEDIUM
...
- **Construction**: ParticipationProfile signature
- **Location**: 07-trust-validation-and-capabilities.md, lines 144-158 (section 7.3.2.1)
- **What's missing**: The `ParticipationProfile` struct has a `signature` field and the spec says "The signature covers all fields except itself." But it does not specify: (1) the serialization format for the signed bytes; (2) the field ordering in the canonical form; (3) whether field names are included or just values; (4) the encoding of each field type (`u64` as big-endian? little-endian? variable-length?); (5) whether a domain separator is used.
...
- **Construction**: Context-specific signing key for ParticipationProfile
- **Location**: 07-trust-validation-and-capabilities.md, lines 156-161 (section 7.3.2.1)
- **What's missing**: The spec says `signer_public_key` is "context-specific -- derived from the context's identity with domain separation, not reused across contexts." It does not specify: (1) what "the context's identity" is (does the context have a key? Is it the creator's key? A group key?); (2) the derivation function (HMAC? HKDF? SHA-256?); (3) the domain separation string; (4) the input material; (5) how the derived keypair is generated from the derived material. This is a complete specification gap -- there is no way to implement this construction from the spec alone.
...
- **What's missing**: The spec says "Pre-generated and published to relays. Each KeyPackage is single-use. Signed by identity key." In MLS (RFC 9420), KeyPackages are signed by the leaf signing key (the credential key), not the identity key. The spec's key architecture (section 3.7) has `#0` (Identity Key) and `#active` (Active Signing Key). Which key signs KeyPackages? If the Identity Key (`#0`) signs them, that contradicts ADR-003, which restricts `#0` to key-event log updates and pre-rotation commitments. If the Active Signing Key signs them, the spec text "signed by identity key" is incorrect. The MLS credential must match the signing key.
...
- **Construction**: KeyDestructionAttestation signature
- **Location**: 09-security-model.md, lines 750-758 (section 9.15)
- **What's missing**: The `KeyDestructionAttestation` struct includes a `signature` field and says "signed by `#0` (Identity Key) or `#active` (Active Signing Key)." The spec does not define the canonical bytes that are signed. Is it a hash of all other fields? If so, what serialization? What domain separator?
...
- **Construction**: ConsistencyCheckpoint signature
- **Location**: 09-security-model.md, lines 462-471 (section 9.9.3)
- **What's missing**: The `ConsistencyCheckpoint` struct includes a `signature` field but does not specify what bytes are signed. Same class as CRYPTO-26. The fields include `contextID`, `senderDID` (variable-length), `eventCount`, `merkleRoot`, `epoch`, `timestamp`. No serialization format, no domain separator.
...
- **Specification**: MLS_128_DHKEMP256_AES128GCM_SHA256_P256 (RFC 9420 ciphersuite 2)
...
- **Specification**: Domain-separated SHA-256 hash, then a signature
- **Soundness**: CONDITIONALLY SOUND
...
- **Specification**: SHA-256 hash, then a signature
- **Soundness**: UNSOUND as specified
...
- **Specification**: `SHA-256(context_id || "scp-metadata")` for metadata
- **Soundness**: CONDITIONALLY SOUND for what is specified
- **Issues**: CRYPTO-23 (encrypted context routing_id derivation unspecified). Broadcast routing_id `SHA-256(context_id)` is specified. The domain separator for metadata routing is good. But the primary encrypted context routing ID -- the one used for actual message delivery -- is missing.

### 7. Pseudonym Derivation
- **Specification**: `HMAC-SHA256(identity_key_material, context_id || "scp-pseudonym")` then keygen over `seed[0..32]`
- **Soundness**: CONDITIONALLY SOUND
- **Issues**: CRYPTO-07 (HMAC key material ambiguous). Domain separation between v1 and v2 is good. Epoch-based rotation for v2 is well-designed. HSM compatibility approach is reasonable in principle but the cross-platform determinism claim is unverified.
...
- **Specification**: `SHA-256(public_key)` commitment, signed migration proof
- **Soundness**: CONDITIONALLY SOUND
...
| Active Signing Key (#active) | KeyCustody | Platform secure storage | rotate_active_key (key-event log entry) | On rotation | NOT SPECIFIED |
| Pre-Rotation Key | Keygen | Cold/offline storage | Single-use (migration) | After migration | NOT SPECIFIED |
...
| HPKE Wrapping Key | Software keygen | KeyCustody | On identity rotation | On rotation | NOT SPECIFIED |
...
Every hash or signature input that contains variable-length fields (context_id, serialized provenance) MUST use 4-byte big-endian length prefixes. The migration proof already does this correctly -- apply the same pattern to InnerEnvelope, BroadcastEnvelope, block notification, SenderKeyEpochAdvance, AAD, and all other concatenated hash inputs.
...
Replace the manual 4-step HPKE description with: "Sender key distribution uses HPKE (RFC 9180) in Base mode with suite DHKEM(P-256, HKDF-SHA256), HKDF-SHA256, AES-128-GCM. The info parameter is `"scp-sender-key-v1" || context_id_len || context_id || member_did_len || member_did || epoch_be`." This eliminates all ambiguity about the KDF, nonce derivation, and key schedule.
...
Define: "Context-specific signing key is derived as `signing_seed = HKDF-Expand(HKDF-Extract(salt="scp-participation-signer-v1", ikm=context_secret_key), info=context_id, L=32)`, and the signing key is generated from this seed."

### 8. [HIGH] Define sender key encryption wire format
Add a `SenderKeyEncryptedContent { ciphertext: Vec<u8>, nonce: [u8; 12] }` struct to the wire format specification.

### 9. [HIGH] Define UCAN CID computation
...
- `.docs/specs/03-identity.md` -- Identity, key custody, resolution
