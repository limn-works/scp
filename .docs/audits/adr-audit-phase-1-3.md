...
- **Severity**: HIGH
- **Resolution (later)**: Accepted and fixed. The normative basis is spec §23.12 items 1, 2, and 4 (`.docs/specs/23-sync-and-offline-strategy.md`): a client MUST verify a received `ConsistencyCheckpoint`'s signature *before accepting it for comparison*, MUST reject a checkpoint that fails verification without using its root or count for any comparison, and MUST only compare checkpoints that have passed verification. The ungated `compare_checkpoint` free function — whose signature could see neither a key nor a roster, and so could not satisfy that MUST — has been removed from `scp-event-log`, along with the same-class `cross_checkpoint_verify` / `verify_cross_checkpoint_with_leaves` / `CrossCheckpointResult`, which no artifact had prescribed. Comparison of a RECEIVED checkpoint is now the single gated runtime judge `queries_helpers::compare_remote_checkpoint`, which verifies the sender's Ed25519 signature, checks the sender's membership, and binds `context_id` before any count/root arithmetic runs, and which fails closed on an unreachable local log. ADR-011 acceptance criterion 8 (`.docs/adrs/phase-2.md`) carries the correction, and the refusals are regression-tested in `crates/scp-runtime/src/context/queries_helpers.rs` (`remote_checkpoint_classification_tests`). This finding is preserved as the historical record that drove the fix.
