...
Large language models produce functional applications from brief specifications. Modular agentic workflows power countless newly funded startups. Across the industry, companies are reorienting around agent tooling, orchestration, and harnesses[citation needed]. Building software no longer requires a particular skillset as much as it does access to and understanding of tools — both of which are also being trivialized. Entire businesses are being built around the “one-shot[glaze]” premise, meanwhile, model makers are preparing to sell “intelligence” (ie, tokens) as a utility[sam altman, noah smith], alongside  electricity, natural gas, and water.
Barring externalities, it is unlikely the trend will stop soon. By and large, the capability floor has been cleared. Training improvements have not begun to level off, there is debate as to whether it matters[dario amodei, noah smith, ...], as consumers are finding that investments in tooling and process are cheaper and faster pathways to performance gains.

While the cost of building software is collapsing, the cost of connecting and operating it is not. Shared identity, trust, relationships, and infrastructure still depend on platforms, accounts, authentication providers, and API-level federation — all of which assume long-lived applications and manual integration effort. These mechanisms work when software is durable and carefully maintained. They break down when software is malleable, ephemeral, and disposable; generated by people without exposure to or appetite for added complexity, or institutions that do not benefit from it.
...
We are witnessing an industrial revolution of software engineering, the implications of which being unlike any predecessor. Until now, writing software required a human to learn to think like and communicate with a computer, creating a natural sieve for software permeated only by those with the requisite combination of knowledge, time, and money. Required inputs are being reduced, while output is increasing without losing fidelity. Now that LLMs can generate average-or-higher quality code by default, at the cost of $20–200/month[citation], and faster than any human, all three barriers have been dramatically lowered. A coherent concept expressed cogently is the only filtering input.

As with any desirable commodity or service, cost decreases and greater accessibility drive up consumption. Latent demand is the final unknown, but early signs show much of it. Online life today is lived according to those who had the means to pass through the sieve; tomorrow, it will be according to personal preference and ideals. The reason is, simply, that everyone has preferences and ideas. When awareness that the cost of trying to realize them is lower than the opportunity cost of the alternative, we will see a Cambrian explosion of personal software, powered by an unstoppable cycle: describe what you want; an agent builds it. The output serves its purpose. It may be discarded, it may evolve, or it may transition into something entirely different. Agentic coding is not so much software engineering as it is the decompressing of ideas. Software ceases to be a durable artifact and becomes an infinitely malleable substrate onto which data, experiences, and interactions are projected.
