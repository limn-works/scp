# Shared Context Protocol: Infrastructure for Agent-Native Social Computing
...
Software engineering is undergoing industrialization.

Large language models produce functional applications from brief specifications. Nearly half of all new code is now AI-generated [40]. Across the industry, companies are reorienting around agent tooling, orchestration, and harnesses [28]. Building software no longer requires a particular skillset as much as it does access to and understanding of tools — both of which are also being trivialized. Entire businesses are built around the “one-shot” premise [29], while model makers prepare to sell “intelligence” (i.e., tokens) as a service on par with internet access, or even a utility [30,31] alongside electricity, natural gas, and water.

Absent external constraints, this trajectory is unlikely to reverse. The capability floor has been cleared. Training improvements show no signs of leveling off, and there is debate as to whether it matters [32,39] given that investments in tooling yield cheaper and faster performance gains [43].

While development costs collapse, the costs of connecting and operating software remain stubbornly high. Shared identity, trust, relationships, and infrastructure still depend on platforms, accounts, authentication providers, and API-level federation — all predicated on durable, manually maintained applications. These mechanisms break down when software is malleable, ephemeral, and disposable; generated by people without exposure to or appetite for added complexity, or by institutions that derive no benefit from it.

We present the Shared Context Protocol (SCP), an open protocol providing cryptographic identity (DID [10]), governed interaction spaces (contexts), end-to-end encryption as access control (MLS [2]), capability-based authorization (UCAN [12]), and verifiable provenance. All interaction occurs within contexts — bounded, encrypted, governed spaces where membership is enforced by cryptography. The protocol is designed for a world where autonomous agents are the primary actors: every agent traces to a human identity through cryptographic binding, agents are isolated per context at the protocol level, and behavioral records replace reputation scores as the primary trust input.

Key properties: no operator dependency (the protocol functions if its creators disappear), transport independence (17 adapter specifications across three tiers), human accountability for all autonomous agents, and context isolation as the security boundary. The protocol complements existing platforms and tool-level protocols — bridge connectors, transport adapters, and identity attestations enable interoperation with established distribution networks. The reference implementation is in Rust with bindings for Python, Swift, Kotlin, TypeScript, and WebAssembly. The specification is published under CC-BY 4.0; the SDK under Apache 2.0.
...
Until now, software development demanded that humans operate in a formal, invented domain better suited to machines than to human cognition [34], creating a natural sieve through which only those with the requisite knowledge, time, and money could pass. Over time, this barrier intensified: research productivity has declined for decades [36], there is evidence that scientific breakthroughs are becoming less disruptive [37], and a shrinking population of researchers threatens to compound the problem [38]. As input requirements grew, returns diminished. AI is dissolving this sieve — reducing required inputs while increasing output without compromising fidelity. Now that LLMs can generate average-or-higher quality code by default, at the cost of $20–200/month [33], and faster than any human, all three barriers have been dramatically lowered. A coherent concept, expressed cogently, is the only remaining filter.

As with any desirable commodity or service, reduced cost and greater accessibility drive consumption. Latent demand is the final unknown, but early signs are unmistakable: 73% of engineering teams now use AI coding tools daily, up from 18% two years prior [40]; in Y Combinator's Fall 2025 batch, 92% of startups integrated AI at the core of their products, and a quarter of the Winter 2025 cohort shipped codebases that are 95% or more AI-generated [41]; one "one-shot" platform reached $100M ARR in eight months [29]; and a reverse-engineering study of 200 funded AI startups found that three quarters are wrappers around foundation model APIs [42]. The software landscape today reflects the preferences of those who could pass through the sieve. As the sieve dissolves, latent demand from the rest of the population — previously priced out by knowledge, time, or money — enters the market. If the cost of generating a functional application falls below the cost of not having one, the volume of software in the world increases by orders of magnitude. Much of it will be disposable: generated for a specific purpose, used, and discarded. Some will evolve. The defining characteristic is that code becomes a commodity input rather than a durable artifact — generated on demand, shaped by intent, and replaced without friction.

What this trajectory does *not* make trivial is maintaining the projected material across substrates — a job done not by software alone, but by abstract methods of consensus and enforcement. Protocols, contracts, custody — the mechanisms that provide longevity to and connective tissue between applications. Identity that belongs to the user rather than to the application that created it, trust earned through interaction and portable across contexts, relationships that persist when the software that introduced them is discarded, and transport that works regardless of which application generated the endpoints. Building software is becoming trivial; connecting it is not. When every person and every agent generates their own software, all of those applications are islands.
...
[28] Deloitte, “Unlocking exponential value with AI agent orchestration,” in *Technology, Media, and Telecom Predictions 2026*, Deloitte Insights, 2025. Reports 74% of companies globally planning to deploy agentic AI within two years (23% currently deployed); Deloitte, “State of AI in the Enterprise,” 2026.

[29] One-shot application builders: Raycast Glaze (raycast.com/blog/introducing-glaze, March 2026), Base44 (base44.com, acquired by Wix for $80M, 2025), Lovable (lovable.dev), Bolt.new (bolt.new), Replit Agent (replit.com). Each generates functional applications from a single natural language prompt.

[30] S. Altman, remarks at BlackRock U.S. Infrastructure Summit, Washington, D.C., March 11, 2026. “We see a future where intelligence is a utility, like electricity or water, and people buy it from us on a meter.” Transcript: rev.com/transcripts/altman-speaks-at-blackrocks-us-infrastructure-summit.

[31] N. Smith, “The AI bust scenario that no one is talking about,” Noahpinion (Substack), 2025. Argues AI could become “a commoditized, low-margin business, more like solar power…or airlines”; N. Smith, “Who will actually profit from the AI boom?,” Noahpinion (Substack), 2025.

[32] D. Amodei, “Machines of Loving Grace,” darioamodei.com, October 2024. On continued scaling: “We do not see hitting the wall” (Morgan Stanley TMT Conference, March 3, 2026). See also D. Amodei, interview with D. Patel, Dwarkesh Podcast, 2025.

[33] Pricing as of March 2026: GitHub Copilot (free–$39/mo), Cursor ($20–$200/mo), Claude Code ($17–$100+/mo), ChatGPT Plus/Pro ($20–$200/mo), Windsurf ($15/mo). See, e.g., GetDX, “AI coding assistant pricing 2025,” getdx.com/blog/ai-coding-assistant-pricing.

[34] H. Moravec, *Mind Children: The Future of Robot and Human Intelligence*, Harvard University Press, 1988. Moravec's paradox: tasks that are hard for humans (mathematics, logic, chess) are easy for computers, while tasks that are effortless for humans (perception, motor control, social reasoning) — shaped by a billion years of evolution — are extremely difficult to automate. Software engineering, a human invention, falls on the easy side. See also N. Smith, “The Fall of the Nerds,” Noahpinion (Substack), February 2026.

[35] J. S. Gans, “A Model of Artificial Jagged Intelligence,” NBER Working Paper 34712, January 2026. Models how AI exhibits “highly uneven performance across tasks that appear nearby” — users relying on global quality signals cannot reliably predict local performance.

[36] N. Bloom, C. I. Jones, J. Van Reenen, and M. Webb, “Are Ideas Getting Harder to Find?,” *American Economic Review*, vol. 110, no. 4, pp. 1104–1144, 2020. Research productivity has declined by a factor of 41 since the 1930s; the number of researchers required to maintain Moore's Law has increased 18-fold since the early 1970s.

[37] M. Park, E. Leahey, and R. J. Funk, “Papers and patents are becoming less disruptive over time,” *Nature*, vol. 613, pp. 138–144, 2023. Analysis of 45 million papers and 3.9 million patents across six decades shows a sustained decline in disruptive science and technology.

[38] C. I. Jones, “The End of Economic Growth? Unintended Consequences of a Declining Population,” NBER Working Paper 26651, 2020. In standard growth models, declining population implies zero steady-state knowledge growth — fewer researchers producing fewer ideas.

[39] N. Smith, “Superintelligence is already here, today,” Noahpinion (Substack), March 2026. Argues current LLMs combined with pre-existing computer capabilities (speed, memory, corpus scanning) already constitute superintelligence for STEM: “The biggest marginal gains of AI over human brains may always come from the pieces we already had in place before 2022.” Implies further model scaling may matter less than tooling and process.

[40] AI coding tool adoption (Stack Overflow Developer Survey): 44% using AI tools (2023) → 62% (2024) → 84% using or planning to use (2025). Daily usage among engineering teams: 18% (2024) → 41% (2025) → 73% (Feb 2026, Pragmatic Engineer survey, n=15,000). GitHub Copilot: 1.8M paid subscribers (FY2024) → 4.7M (Jan 2026, +75% YoY); 15M users (early 2025) → 20M (July 2025). AI-generated code among Copilot users: 27% at 2022 launch → 42% of all code AI-assisted (SonarSource State of Code 2025); Gartner forecasts 60% by end of 2026. See Stack Overflow Developer Surveys 2023–2025; Panto, “GitHub Copilot Statistics 2026,” getpanto.ai/blog/github-copilot-statistics.

[41] Y Combinator batch composition: Fall 2025 batch, 92% of 155+ startups integrate AI at the core; Winter 2025 batch, 25% have codebases 95%+ AI-generated. Overall 2026 batches ~60% explicitly AI companies, up from ~40% in 2024. See TLDL, “YC AI Startups 2026,” tldl.io/blog/yc-ai-startups-2026.

[42] S. Karforma, “I Reverse-Engineered 200 AI Startups,” Towards AI, 2025. Found 73% (146 of 200) are wrappers around OpenAI or Anthropic APIs with custom frontends. AI wrapper startups have raised over $10B in aggregate funding. See also Machine Brief, “What Are AI Wrapper Startups? The $10 Billion Gold Rush,” machinebrief.com/analysis/ai-wrapper-startups-10-billion-gold-rush-explained.

[43] C. Snell, J. Lee, K. Xu, and A. Kumar, “Scaling LLM Test-Time Compute Optimally can be More Effective than Scaling Model Parameters,” arXiv:2408.03314, August 2024. Demonstrates that inference-time compute scaling can outperform model parameter scaling for reasoning tasks. Practical benchmarks confirm: Claude Opus 4.5 scored 42% on SWE-bench with a generic scaffold vs. 78% with Claude Code's harness — a 36-point gain from tooling alone (see M. Fowler, “Harness Engineering,” martinfowler.com/articles/exploring-gen-ai/harness-engineering.html). Similarly, simply repeating a prompt twice improved accuracy across all tested LLMs, winning 47 of 70 benchmark-model combinations with zero losses (Y. Leviathan et al., “Prompt Repetition Improves Non-Reasoning LLMs,” arXiv:2512.14982, December 2025).
