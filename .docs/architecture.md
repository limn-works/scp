...
Inner Envelope Builder: sign inside encrypted payload
    │  Sign: Ed25519 over SHA256(context_id || sender_did || signing_key_id ||
    │         epoch || generation || sequence || timestamp || payload_hash ||
    │         provenance_hash)
    │           🔒 Ed25519 inner signature — member-only verifiable (§9.8.1)
    │           🔒 signing_key_id binds to #active or #agent (ADR-039)
...
│   │
│   ├── go/                    # Go package (via UniFFI C ABI, Phase 5)
│   │   └── scp/
│   │
│   ├── csharp/                # C# package (via UniFFI C ABI, Phase 5)
│   │   └── SCP/
│   │
│   └── java/                  # Java package (via UniFFI C ABI, Phase 5)
│       └── com/limn/scp/
│
...
Go, C#, and Java bindings use UniFFI to generate language-specific bindings from the same UDL definitions as Swift and Kotlin. Each language wraps the generated interface with idiomatic bindings:
...
Build:
  ...
  • scp-core/economy/ — economic governance (ADR-033: cost attribution, sender velocity, rate escalation)
...
Build:
  ...
  • crates/scp-core/src/bridge/ — X bridge adapter (relay mode)
  • crates/scp-core/src/bridge/ — Bluesky bridge adapter (API mode)
...
- **Specific API signatures.** See sketch.md for API surfaces (§1–§17).
