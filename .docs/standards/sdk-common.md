...
| `SCP-CTX-2139` | native FFI-common registry (`PyO3` + NAPI + `UniFFI`, **one shared meaning**) | Event-log Merkle proof generation failed over a log the bridge COULD read — the counterpart to `2138`, raised by `event_log_verify` when `prove_inclusion` / `prove_absence` decline. Covers two distinct outcomes that a caller must not collapse: (a) the claim is demonstrably FALSE — an absence claimed for an event that IS present, or a leaf index past the end of the tree; (b) no proof of this shape exists in this construction — an EMPTY but live log, where an absence claim is actually TRUE but `prove_absence` has no bracketing neighbours to build a proof from, so it declines rather than fabricating one. Case (b) is "no proof," NOT "disproof": it must never be read as evidence the claimed event IS present. Malformed CLAIM input carries `SCP-VALID-7000`, not this code. NOT emitted by `scp-client-wasm` |
