...
| `SCP-CTX-2139` | native FFI-common registry (`PyO3` + NAPI + `UniFFI`, **one shared meaning**) | Event-log Merkle proof generation failed over a log the bridge COULD read — the honest-negative counterpart to `2138`. Raised by `event_log_verify` when `prove_inclusion` / `prove_absence` reject the claim: an empty log, a leaf index past the end of the tree, or an absence claimed for an event that IS present. Malformed CLAIM input carries `SCP-VALID-7000`, not this code. NOT emitted by `scp-client-wasm` |
