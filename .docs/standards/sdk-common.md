...
- **Broadcast publish** → **no in-Rust custody gate**. WASM's `SCP-PERM-3000` uses on this path are suspended-write and non-author authorization checks, a different condition entirely — not a missing-custody signal.
