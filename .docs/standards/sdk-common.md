...
| `SCP-CTX-2138` | native FFI-common registry (`PyO3` + NAPI + `UniFFI`, **one shared meaning**) | Event-log operation could not reach the AUTHORITATIVE log — FAIL CLOSED (GitHub #1933). Raised by `event_log_query` / `event_log_verify` / `event_log_checkpoint` when the instance is suspended or shut down, no supervisor is attached, or the provider reports NO LOG for the context (`Ok(None)` = UNKNOWN, never "empty"). The bridge never falls back to a bridge-local tree: an absence proof over a non-authoritative log is a forgeable false negative, and a checkpoint over one is a validly-SIGNED false commitment. NOT emitted by `scp-client-wasm` |
| `SCP-CTX-2139` | native FFI-common registry (`PyO3` + NAPI + `UniFFI`, **one shared meaning**) | Event-log Merkle proof generation failed over a log the bridge COULD read — the counterpart to `2138`, raised by `event_log_verify` when `prove_inclusion` / `prove_absence` decline. Covers two distinct outcomes that a caller must not collapse: (a) the claim is demonstrably FALSE — an absence claimed for an event that IS present, or a leaf index past the end of the tree; (b) no proof was issued — an EMPTY but live log, where an absence claim is actually TRUE but `prove_absence` DECLINES to issue a proof, via an explicit `leaf_count == 0` guard. Case (b) is "no proof," NOT "disproof": it must never be read as evidence the claimed event IS present. The decline is a deliberate choice, NOT a limit of the construction — `AbsenceProof` admits `lower: None, upper: None`, and `leaf_count == 0` with `root == SHA-256("")` is in fact the one fully self-contained, off-box-checkable non-membership answer this append-order log can produce (the verifier recomputes the empty-tree root; no adjacency assumption needed). Malformed CLAIM input carries `SCP-VALID-7000`, not this code. NOT emitted by `scp-client-wasm` |
