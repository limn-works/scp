...
- **Never reference anything by an identifier alone.** Name the thing first, then give the identifier so the reader can find it. Write "ADR-062, capability injection", not "ADR-062". Write "§17.17 of the persistence spec, which classifies a capability as durability-only or as a nullifier", not "§17.17". Write "pull request #2293, the CLAUDE.md prose rules", not "#2293". Write "SCP-OUT-046, the streaming-saga seal-phase FSM", not "SCP-OUT-046". A reader who has not memorized the ledger learns nothing from a bare number, and has to open the artifact before finishing your sentence. This rule binds every kind of identifier: issue and pull-request numbers, ADR numbers, spec and section numbers, story IDs, commit hashes, branch names, and task IDs. Open the target before you write the citation and confirm the heading exists: a number that resolves to nothing leaves the reader unable to decide whether the artifact lost the section or you invented it, which is worse than the sentence carrying no citation at all. When you cite a section an artifact used to carry and no longer does, write the reference followed by `[no such section]` on the same line, so the reader stops looking. `scripts/check-doc-citations.py` reads every `§N.M` citation in `.docs/standards/`, `.docs/lessons/`, `.claude/agents/`, and `CLAUDE.md`, and fails when one names a section its spec file does not contain.
...
- **Check every sentence before you send it.** Ask seven questions: have I hidden a subordinate relation? Have I dropped a subject or an object? Does a passive verb hide who acted? Have I appraised the work instead of reporting it? Have I referenced anything by a bare identifier? Have I used any of the eleven forbidden constructions? Does this sentence admit two contradictory readings? If any answer is yes, rewrite the sentence.
