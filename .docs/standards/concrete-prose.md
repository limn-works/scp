...
- **Never reference anything by an identifier alone.** Name the thing first, then give the identifier so the reader can find it. Write "ADR-062, capability injection", not "ADR-062". Write "§17.17 of the persistence spec, which classifies a capability as durability-only or as a nullifier", not "§17.17". Write "pull request #2293, the CLAUDE.md prose rules", not "#2293". Write "SCP-046, the Python SDK package root", not "SCP-046". A reader who has not memorized the ledger learns nothing from a bare number, and has to open the artifact before finishing your sentence. This rule binds every kind of identifier: issue and pull-request numbers, ADR numbers, spec and section numbers, story IDs, commit hashes, branch names, and task IDs. Open the artifact and copy the name out of it before you write the pair. A name you recalled instead of read attaches a real identifier to the wrong artifact, and a reader who sees a name beside a number stops checking, so the wrong pair costs the reader more than the bare number does.
...
- **Check every sentence before you send it.** Ask seven questions: have I hidden a subordinate relation? Have I dropped a subject or an object? Does a passive verb hide who acted? Have I appraised the work instead of reporting it? Have I referenced anything by a bare identifier, or written a name beside an identifier without opening the artifact to confirm that the name belongs to that artifact? Have I used any of the eleven forbidden constructions? Does this sentence admit two contradictory readings? If any answer is yes, rewrite the sentence.
