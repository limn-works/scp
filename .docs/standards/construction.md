...
**`storage` carries an inherited handle, not a fresh backend choice.** Alec Marcus ruled on 2026-08-14 that storage is selected once at `SCP`-instance construction and that a node receives an already-opened handle, so `NodeConfig.storage` holds `StorageSlot::Custom(handle)` for a node an `SCP` instance owns. M4 is unaffected: `storage` stays required and non-`Option`, and M3 fail-closed behaviour still governs an open a caller performs at `SCP`-instance construction. `.docs/specs/17-persistence-and-storage.md` §17.6 states a rule and its two grounds (a `{dir}/scp.db.lock` exclusive lock, and divergence between two independently-opened handles); ADR-052 "Amendment (2026-08-14)" records what changes for a construction surface.
