...
| Scenario | Action |
|---|---|
| Active Signing Key compromised | The root signs one `KeyState` carrying a new key `current` and the stolen key's `Compromised{from: N}` (§3.2.1). The identifier is unchanged. MLS Update everywhere. |
