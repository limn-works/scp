...
- **~~Capability declaration format.~~** ✅ **Resolved.** JSON Schema (MCP-compatible) with SCP-specific extensions. §8.4 specifies the contract; §8.5 establishes MCP compatibility. The declaration is SDK-local and never crosses the wire — no app DID, no signature, no protocol state (§8.4.1). See `00-open-questions.md`.
