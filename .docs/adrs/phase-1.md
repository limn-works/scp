...
- **Self-certifying property:** The DID string `did:dht:cyuoqaf7itop8ohww4yn5ojg13qaq83r9zihgqntc5i9zwrfdfoo` is the z-base-32 encoding of the public key. Verification requires no network call — just decode the DID and compare to the resolved public key. This eliminates DID-to-key MITM attacks at the identifier level. **This illustration was corrected.** It previously read `did:dht:z6Mk...`, which is a **did:key** string: `z6Mk` is multibase base58btc over a multicodec-prefixed Ed25519 key, and did:dht prepends no multibase character at all (§18.2.2A `id` row). `did_dht_from_public_key` (`crates/scp-did/src/lib.rs:173`) formats `did:dht:z{}` and emits a 53-character suffix, and the illustration above is where that leading `z` came from.
...
- **Language:** Rust
- **Libraries:** the `mainline` crate (v6.1.1+) for Mainline DHT publish and resolve, `simple-dns` for DNS packet construction, and `z-base-32` for DID string encoding. SCP implements the did:dht-specific DID Document to DNS resource record encoding (~300 lines) directly over those three crates. No existing Rust crate implements did:dht directly — `did-dht` does not exist on crates.io, `web5-rs` is abandoned (Block/TBD shut down Nov 2024), and `veilid-did` does not exist. The `web5-rs` `document_packet/` module may be referenced for encoding patterns but is not a dependency. `did:web` remains contingency fallback only. **This bullet was amended; see "Amendment — `pkarr` is unusable, and the two document encodings are two, not one" below for what it previously said and why it changed.**
- **Key generation:** Ed25519 via platform adapter (Secure Enclave on iOS, Keystore on Android, software keys for testing).
- **DID document format:** two encodings, one per resolution layer (§18.2.2A). The SCP relay layer carries a standard W3C DID Document with Ed25519 verification methods, serialized as canonical JSON. The Mainline DHT layer carries the bootstrap core (§18.2.2C) as the did:dht method's DNS-packet encoding. **This bullet was amended; see the amendment below.**
- **Crate:** `scp-core`
- **Module:** `scp-core/identity/`

### Amendment — `pkarr` is unusable, and the two document encodings are two, not one

This amendment records two corrections to the Implementation section above. It changes no decision in the Decision section: did:dht remains the DID method, Mainline DHT remains the fallback resolution layer, and the identity module still abstracts the method behind a trait.

**1. `pkarr` cannot be a dependency of this design, and never could have been.** The Libraries bullet named `pkarr` as the primary dependency. Every `pkarr::SignedPacket` constructor requires a `Keypair` holding the Ed25519 **private** key, and the one constructor that does not — `SignedPacket::from_relay_payload` — only verifies an already-signed packet. SCP never holds a private key in a library: `KeyCustody` signs detached, so the identity module hands a byte buffer to custody and receives a signature back (`bep44_signable` → the caller's `sign_fn` → `MutableItem::new_signed_unchecked`, `crates/scp-identity/src/dht.rs:851-868`). A library that requires the private key to build the packet cannot participate in that flow, so this ADR mandated an impossible dependency from the day it was written.

The implementation resolved this by using `mainline` directly and never adding `pkarr`. That resolution is correct and this amendment adopts it. Two facts about the tree as it stands: `pkarr` appears in no manifest dependency table and in no `Cargo.lock` entry — the only in-repo trace is the keyword string at `crates/scp-dht/Cargo.toml:10` — and `PkarrDhtClient` (`crates/scp-dht/src/dht_client/pkarr_client.rs:56`) is a misnomer that wraps `mainline::Dht`. The name carries only the pkarr HTTP-gateway URL convention. **This amendment requires the rename**, along with the module name `pkarr_client`, the builder `PkarrDhtClientBuilder`, and the `pkarr` keyword at `crates/scp-dht/Cargo.toml:10` — a name that asserts a dependency this amendment establishes was never usable is the kind of misnomer that sends the next reader looking for a crate that is not there. The rename is a code change, so it lands in the code step rather than here; it is not optional and not a follow-up.

**The `mainline` floor of v6.1.1+ stands, and the tree does not meet it.** `Cargo.lock` resolves `mainline` at **6.0.1** against the requirement `mainline = { version = "6", optional = true }` (`crates/scp-dht/Cargo.toml:19`, behind the `production-dht` feature), which admits 6.0.1. An earlier draft of this amendment dropped the floor because the lockfile disagreed with it — that is code informing an ADR, the inversion the artifact-flow invariant forbids, and nothing here established that the floor was arbitrary. The floor is a decision this ADR made and no evidence retires it, so the **code moves**: the manifest requirement rises to `mainline = { version = "6.1.1", optional = true }` and the lockfile follows. An implementer who finds a concrete reason 6.1.1 is wrong amends this ADR on that reason, before changing the manifest.

The ~300-line estimate for the DNS resource record encoding stands. Removing `pkarr` does not raise it, because the encoding was always SCP's own code; what `pkarr` would have supplied is the signing and the DHT transport, and `mainline` supplies both without holding a key.

**2. The DNS encoding and the JSON-LD document are both correct, at different layers.** The Libraries bullet specified "the did:dht-specific DID Document to DNS resource record encoding (~300 lines)" while the DID document format bullet specified "Standard W3C DID Document JSON-LD". Read as one encoding those two clauses contradict each other, and the implementation resolved the contradiction by building only the JSON side and publishing JSON to Mainline, where BEP44 rejects it.

Under the two-layer resolution model (§3.10) both clauses are true and they describe different layers. The SCP relay network carries the full W3C DID document as canonical JSON inside a DID-record frame, bounded by `MAX_DID_RECORD_VALUE_LEN` (262,039 bytes). Mainline carries the bootstrap core as the did:dht method's DNS-packet encoding, bounded by BEP44's 1,000 bytes. Each layer signs its own bytes and advances its own sequence number (§3.10.5), and no clause anywhere requires the two layers to carry identical bytes.

The DNS encoding is the did:dht method's own, and the method specification fixes it: the packet is compressed per RFC 1035 §4.1.4 and bencoded into the BEP44 `v` value, and the method specifies no other compression. §18.2.2C states the record layout, the `@context` prohibition the method imposes on that layer, and the one mapping question the method leaves to SCP.
...
4. **Key Rotation — Three-Layer Architecture**
   ...
   **4a. `rotate_active_key(identity, key_custody) -> Identity`** (Layer 1 — common case)
   ...
   - Updates the DID document: adds the new key as a verification method, moves `authentication` and `assertionMethod` references to the new key.
   - Retains the old key as `#retired-{sequence}`, which keeps verifying **content signatures** the identity made before the rotation, indefinitely. §9.7.1 of `.docs/specs/09-security-model.md` is unchanged and binds the other direction: a verifier MUST NOT accept a **KeyPackage attestation** signed by any `#retired-*` method, because an attestation is a bearer capability rather than a statement about the past. No section of `.docs/specs/03-identity.md` or `.docs/specs/09-security-model.md` states which fragment any signature class other than those two verifies against, and `.docs/specs/00-open-questions.md` registers that gap as open item (d) of "Retired verification methods: four rules the corpus does not state", which a human decides. This criterion therefore states retention and the two settled classes, and decides no third class. Retention also reaches a compromised key. §9.12 of `.docs/specs/09-security-model.md`, the compromise recovery protocol, routes an `#active` compromise through `rotate_active_key`, which retires the attacker-held key into the document under this criterion, and §9.12 states that the retired key keeps verifying content signatures. Alec's ruling of 2026-08-17, recorded in `.docs/specs/00-open-questions.md`, states the one method state that stops a retired method from verifying content — `Compromised { from: N }`, which fails for content at or from sequence N — and no clause of any spec and no Rust type carries that state today, so an identity that recovers from an `#active` compromise has no built way to mark the retired entry. A DID document retains every `#retired-{sequence}` entry its rotations have produced, under no cap, and no rotation prunes one.

     **Alec deleted the cap on 2026-08-17, dissolving the open question that had suspended this whole clause.** He ruled out all three of a stated count, a per-identity byte budget, and a pointer to a key-history document a verifier would fetch. This amendment records his decision and restores the retention the suspension had withdrawn. The capped clause read: "Retired key retention is bounded: the document retains at most the 2 most recent retired active keys; older ones are pruned on rotation to prevent unbounded DID document growth within DHT size constraints." Its justification was the 1,000-byte BEP44 cap on a Mainline record (§18.2.2D of `.docs/specs/18-addressability-and-deployment.md`). The two-encoding split moved every entry outside the Mainline bootstrap core onto the relay layer (§18.2.2C), and a retired verification method sits outside that core, so the bound that reaches it is the relay layer's 262,039-byte cap.

     **What the artifact records in place of the cap.** One retired verification method costs **263 bytes** of minified relay-layer JSON at a one-digit sequence number: the object `{"controller":"<60 chars>","id":"<60 chars>#retired-N","publicKeyMultibase":"<48 chars>","type":"Ed25519VerificationKey2020"}`, where §18.2.2A fixes the DID string at 60 characters, the `publicKeyMultibase` value at 48, and `verificationMethod[].type` at the 26 characters of `Ed25519VerificationKey2020`. The entry joins a JSON array, so it costs one more byte for the separator and one more for each digit the sequence carries past the first. The agent-side entry costs 6 bytes more, because `#retired-agent-N` is six characters longer than `#retired-N`. §18.2.2D measures a document carrying `#0`, `#active`, a `PreRotationCommitment` and one `SCPRelay` entry at 1,255 bytes minified — measured against the 61-character DID string the shipped code emits, so the same document is 1,245 bytes at §18.2.2A's conformant 60-character string — which leaves 260,794 bytes under the relay layer's 262,039-byte cap. Roughly **990 rotations** of one identity fit inside that: 991 if a reader divides by the 263-byte object alone, and 980 once the separator and the sequence digits are counted (9 entries at 264 bytes, 90 at 265, then 881 at 266). The figure is illustrative, because the ruling states no count. A cap of 2 stated a number that the relay-layer bound does not produce, which is why Alec's ruling leaves no number behind.

     **The code the criterion governs, and what it now does.** `retire_active_key` (`crates/scp-did/src/document.rs`) emits `#retired-{sequence}` and matches this criterion as restored, and this criterion authorizes no active-key pruner, so nobody writes one. On the agent side the cap of 2 is deleted from the tree: `prune_retired_agent_keys`, `MAX_RETIRED_AGENT_KEYS = 2`, the call `rotate_agent_key` made to the pruner, and the `IdentityError::TooManyRetiredAgentKeys` variant are all removed, and the two tests that asserted the cap — `rotate_agent_key_retains_every_retired_key_uncapped` in `crates/scp-did/src/document.rs` and `agent_key_retention_is_uncapped` in `crates/scp-testing/tests/integration/identity.rs` — now assert that the retained count equals the number of rotations performed and that every `#retired-agent-{sequence}` fragment resolves after every later rotation. Issue #2365 is building the verifier side of retired-key verification.

     **What §18.2.2D still owes, which this criterion does not gate.** `SCP-IDENT-1061` appears in no Rust file, so neither of §18.2.2D's caps is built: the publish-side gate is an acceptance criterion of story SCP-007 in `.docs/prds/main.json` and of story SCP-239 in `.docs/prds/reachability.json`, and the parse-side cap on `DidDocument::from_json` (`crates/scp-did/src/document.rs:399`) is a bare `serde_json::from_str` with no length bound. The Mainline bootstrap-core encoder is unbuilt too, so `DidDht::publish_document` (`crates/scp-identity/src/dht.rs:830`) still serializes the whole document — every retired key included — and publishes those bytes to Mainline under the 1,000-byte BEP44 cap, where an over-cap put surfaces to the caller as a network timeout. §18.2.2D owns both caps and the encoder owes the split; this criterion states retention and the measurement, and orders nothing.
   ...
   **4a′. `rotate_agent_key(identity, key_custody) -> Identity`** (Layer 1 — agent key rotation, ADR-039)
   ...
   - Updates the DID document: replaces the `#agent` verification method with the new key.
   - Retains the old agent key as `#retired-agent-{sequence}`, on §4a's terms: it keeps verifying **content signatures only**, a DID document retains every such entry under no cap, and no rotation prunes one. §9.7.1 of `.docs/specs/09-security-model.md` still forbids a verifier from accepting a KeyPackage attestation signed by a `#retired-agent-*` method, and that rule is what makes agent-key rotation a revocation. ADR-003 previously capped this retention at 2 as well, with the identical "within DHT size constraints" justification, and Alec's ruling of 2026-08-17 deletes both.
   ...
   **4c. `verify_migration(old_did, old_document, new_did, migration_proof, pre_rotation_proof, rotated_at, now) -> Result<bool, IdentityError>`**
   ...
   - Always-checked invariants (MODERATE assurance):
     ...
     3. **Self-cert binding of `old_did`.** `migration_proof.old_public_key` MUST z-base-32-encode, prefixed with `did:dht:` and **no further character**, to exactly the `old_did` argument. (This invariant previously read "with the `did:dht:z` prefix"; §18.2.2A fixes the format as `did:dht:` + `Z-BASE-32(raw-public-key-bytes)`, 52 characters, and names the leading `z` the shipped code emits as a defect the DID-string slice removes.) did:dht is self-certifying — without this check, an attacker could substitute their own pubkey and a valid signature and forge "MODERATE assurance" migrations.
   ...
   - Conditional invariants — applied only when `pre_rotation_proof` is `Some(_)` (STRONG assurance):
     ...
     10. **Self-cert binding of `new_did`.** `pre_rotation_proof.revealed_key` MUST z-base-32-encode, prefixed with `did:dht:` and **no further character**, to exactly the `new_did` argument (same correction as invariant 3 above) — preventing a valid proof for one new DID from being substituted under a different `new_did` string.
   - **Assurance levels.**
   ...
   **Identity structure at creation:**
   ...
   pub struct ScpIdentity {
       ...
       /// The DID string: did:dht:<z-base-32(identity_key.public)>, 52 chars, no
       /// prepended `z` (§18.2.2A)
       pub did: String,
   }
   ```
...
What keeps the restart case availability-only (never integrity loss) is that the slot decisions are **storage-authoritative**, not index-only: the QUERY path (rule (c)) re-derives the slot from the durable blob on a cold-index read and returns only the genuine record — **largely closing the suppression window on the read path itself**, not merely relying on the client to sort out a flood; the DELETE path (rule (d)) and cold-index establish are likewise storage-backed, so a cold index cannot be used to purge or roll back the durable record. Independently, integrity holds by client re-verification in *both* cases: a *replayed old-but-genuine* record is owner-signed and passes the resolver's DID-derived-key BEP44 verify, so the rollback defense is the resolver's **client-side sequence-number monotonicity** freshness check (accept only `seq >= last_known_seq` **for the layer that served the record** — §3.10.7 states that a sequence number "carries no authority across layers", and §9.6.1 requires the comparison to be against the last number observed "on the layer that served the record"; only the identity owner can advance either number), backed by multi-relay publishing. An earlier version of this line read "the highest valid `seq` is authoritative across the relay *and* the DHT" and attributed that to §9.6.1, which never contained the sentence it quoted; the cross-layer comparison is now forbidden, and Mainline's own number is a wall-clock timestamp rather than a publish counter (§18.2.2C). (An arbitrary *forged* blob that is not owner-signed fails the BEP44 verify outright — the always-on integrity guarantee, distinct from what handles a genuine replay.)
