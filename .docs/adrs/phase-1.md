...
4. **Key Rotation — Three-Layer Architecture**
   ...
   **4a. `rotate_active_key(identity, key_custody) -> Identity`** (Layer 1 — common case)
   ...
   - Retains the old key as `#retired-{sequence}`, which keeps verifying **content signatures** the identity made before the rotation, indefinitely. §9.7.1 of `.docs/specs/09-security-model.md` is unchanged and binds the other direction: a verifier MUST NOT accept a **KeyPackage attestation** signed by any `#retired-*` method, because an attestation is a bearer capability rather than a statement about the past. §3.9 of `.docs/specs/03-identity.md` states the criterion that decides any other signature class, and states the hard path a compromise takes: a compromised key is removed from the document rather than retired into it, so this criterion's retention never keeps a key an attacker holds. A DID document retains every `#retired-{sequence}` entry its rotations have produced, under no cap, and no rotation prunes one.
     ...
     **The code work this criterion authorizes.** `retire_active_key` (`crates/scp-did/src/document.rs:893`) already emits `#retired-{sequence}` and matches this criterion as restored, and this criterion authorizes no active-key pruner, so nobody writes one. On the agent side an implementer removes `prune_retired_agent_keys` (`:1154`), `MAX_RETIRED_AGENT_KEYS = 2` (`:293`), the call at `:1104`, the `IdentityError::TooManyRetiredAgentKeys` variant (`crates/scp-identity/src/lib.rs:276`), and the assertions that encode the count of 2 — `crates/scp-did/src/document.rs:1997`, `:2001`, `:2013`, `:2020`, `:2029`, their explanatory comments at `:1992` and `:2000`, and `crates/scp-testing/tests/integration/identity.rs:203` with its comments at `:195` and `:202` — replacing each assertion with one that the retained count equals the number of rotations performed. Issue #2365, which is implementing the verifier side of retired-key verification, and this ADR amendment are the two artifacts that authorize those edits; this file authorizes them and does not perform them.
