...
The implementation resolved this by using `mainline` directly and never adding `pkarr`. That resolution is correct and this amendment adopts it. Two facts about the tree as it stands: `pkarr` appears in no manifest dependency table and in no `Cargo.lock` entry — the only in-repo trace is the keyword string at `crates/scp-dht/Cargo.toml:10` — and `PkarrDhtClient` (`crates/scp-dht/src/dht_client/pkarr_client.rs:56`) is a misnomer that wraps `mainline::Dht`. The name carries only the pkarr HTTP-gateway URL convention. Renaming it is a code change this amendment authorizes and does not perform.

The `mainline` version this ADR named, v6.1.1+, does not match the tree: `Cargo.lock` resolves `mainline` at **6.0.1** against the requirement `mainline = { version = "6", optional = true }` (`crates/scp-dht/Cargo.toml:19`, behind the `production-dht` feature). The requirement is what governs, and the amended Libraries bullet states no patch floor.
