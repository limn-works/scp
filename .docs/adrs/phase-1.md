...
5. **`POLICY { }`** (**added 2026-09-13**)
   ...
   - **The residue an unsigned answer leaves reaches every field a publisher spends money against** (**amended 2026-09-14**). A party on the network path of a relay running without TLS (`18-addressability-and-deployment.md` §18.6.3) rewrites the address a publisher settles to, the publisher pays that party, the settlement is final on the rail, and the relay's own adapter reports no settlement to the account its operator configured, so the relay refuses `payment` and the publisher's money is gone. The same party rewrites an empty `receipt_exempts` into `["identifier"]`, and the publisher pays the genuine price to the genuine payee believing it bought an exemption the relay does not honour; the rewrite in the other direction makes a publisher decline to pay at a relay that would have exempted it. It rewrites a rent term the same way, and the controller's rent buys a quota or a period the relay never declared. A publisher that has not confirmed a relay's declared terms through a channel it authenticates publishes without paying and takes the refusal.
...
6. **`RENT { identifier, units, payment_receipt, signature }`** (**added 2026-09-14**, **amended 2026-09-18**)
   ...
   - **It refuses with `4041` scope `payment`, whose `value` carries this entry's rent terms**, on the three receipt grounds R9 states, and with `4042` on the three grounds no payment satisfies.
