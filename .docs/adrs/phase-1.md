...
6. `UcanHeader` includes a `kid: Option<SigningKeyId>` field, so a JWT header naming any verification method outside `#active` and `#agent` fails to parse and no reader downstream decodes a fragment string. It serializes as the same `"#active"` / `"#agent"` string a `kid` header carries on the wire. `MintParams` includes a `key_scope: Option<String>`, because `fct.scp_key_scope` is a free JSON facts value rather than a header field; a minter decodes it through `SigningKeyId::from_fragment` and rejects a scope naming any other method, so a header and the facts beside it always name the same one.
