...
1. **`PUBLISH { routing_id, recipient_hint, blob_ttl, retain, payment_receipt, blob }`**
   ...
   - `payment_receipt` (optional `bin`, **added 2026-09-13**, **amended 2026-09-15**): the `PaymentReceipt` bytes of `19-economic-governance.md` §19.2.1, tagged with the adapter that issued them. <!-- scp:include id="publish-charge-trigger" from=".docs/specs/09-security-model.md" -->**A relay charges a PUBLISH at `scp:did:` or `scp:svc:` where it declares `per_publish`, and it charges one on no other declaration.** **It charges no `scp:wit:` and no `scp:wcf:` write** (§9.7.4.3), which are the other two retained kinds, and no party pays for either.<!-- scp:end id="publish-charge-trigger" --> It runs every check `09-security-model.md` §9.7.4.2 R9 states, in the order `03-identity.md` §3.10.2 gives, and refuses the write with `4041` scope `payment` on the grounds that rule states. "Paid" names a receipt this relay verified, which is the term R9's `Payment` scope reads. **A receipt presented here pays the publish fee and nothing else, and rent reaches the relay on `RENT`** (R9).
   - Store it for `blob_ttl` seconds **where the write carries one**; a `retain` write carries none, and `09-security-model.md` §9.7.4.2 R9's ring buffer is what removes such a record, under the coverage R9 states an identity's current rent buys.
...
5. **`POLICY { }`** (**added 2026-09-13**)
   ...
   - **The residue an unsigned answer leaves reaches every field a publisher spends money against** (**amended 2026-09-14**). A party on the network path of a relay running without TLS (`18-addressability-and-deployment.md` §18.6.3) rewrites the address a publisher settles to, the publisher pays that party, the settlement is final on the rail, and the relay's own adapter reports no settlement to the account its operator configured, so the relay refuses `payment` and the publisher's money is gone. The same party rewrites an empty `receipt_exempts` into `["identifier"]`, and the publisher pays the genuine price to the genuine payee believing it bought an exemption the relay does not honour; the rewrite in the other direction makes a publisher decline to pay at a relay that would have exempted it. It rewrites a rent term the same way, and the controller's rent buys a quota or a period the relay never declared. **It rewrites `operator_key`, and that case ends elsewhere than the other three**: the controller signs its `RENT` over the rewritten operand, the relay refuses on the signature ground ahead of `PaymentAdapter::verify`, so the relay reads no receipt and no settlement is lost, and the key the refusal returns rides back over the same rewritable channel. A publisher that has not confirmed a relay's declared terms through a channel it authenticates publishes without paying and takes the refusal.

6. **`RENT { identifier, units, payment_receipt, signature }`** (**added 2026-09-14**, **amended 2026-09-18**)
   - Pay rent for one identity at this relay, or read that identity's rent state. <!-- scp:fragment id="rent-wire-fields" -->**The `RENT` operation carries four fields and no others**: `identifier`, the 32 raw digest bytes of the identity the rent is credited to; `units`, a `u32` count of declared quotas, at least 1 on a message carrying a receipt and 0 on a message carrying none; `payment_receipt`, optional, the `PaymentReceipt` bytes of `19-economic-governance.md` §19.2.1 tagged with the adapter that issued them and bounded by the Constraints line below; and `signature`, the 64-byte beneficiary signature `09-security-model.md` §9.7.4.2 R9 defines.<!-- scp:end id="rent-wire-fields" --> <!-- scp:include id="rent-preimage-operands" from=".docs/specs/09-security-model.md" -->**The `RENT` signature preimage is `SHA-256("SCP-RENT-V1:" ‖ identifier ‖ BE32(units) ‖ receipt_digest ‖ operator_key)`, and each operand is stated here so two implementations compose one preimage.** **The preimage is 113 bytes**: the twelve separator bytes and 32 + 4 + 32 + 33 = 101 field bytes. `identifier` is the 32 raw digest bytes. `BE32(units)` is the count as a four-byte big-endian integer, at least 1 on a message carrying a receipt and 0 on a message carrying none. **`receipt_digest` is `SHA-256(payment_receipt)` where the receipt is present and the 32 bytes `SHA-256(0x00)` where it is absent**, under §9.5.1's rule for an absent fixed-length optional field. **`operator_key` is the 33-byte SEC1 compressed operator key the addressed relay declares in its own `POLICY` answer**, which the controller reads before it sends and which the relay already holds as the key it proves control of.<!-- scp:end id="rent-preimage-operands" -->
   - **A `RENT` whose `payment_receipt` is absent credits nothing and answers with the identity's current rent state**, which is how a controller reads whether it is covered without paying for the answer.
   - The relay runs the checks `09-security-model.md` §9.7.4.2 R9 states, in the order that rule gives, credits that identity's quota and rent expiry under the one arithmetic that rule states, and answers `OK` with a `RentState` (`03-identity.md` §3.10.10), so a controller learns what it now holds without a second round trip. The arithmetic: <!-- scp:include id="rent-arithmetic-quota" from=".docs/specs/09-security-model.md" -->**One verified rent payment of `units` sets the identity's quota to `units` times the declared quota, and the standing quota is the figure the relay recorded while that identity's rent is current and is zero for an identity whose rent has lapsed.** **A relay refuses, under the `Payment` scope, a `RENT` whose `units` times the declared quota is below that identity's standing quota while that identity's rent is current**, and the refusal's value carries that entry's rent terms, so a controller computes from them and from the quota its rent state reports the smallest `units` that entry accepts.<!-- scp:end id="rent-arithmetic-quota" --> <!-- scp:include id="rent-arithmetic-period" from=".docs/specs/09-security-model.md" -->**The same payment sets the identity's rent expiry to one declared period past the later of its standing expiry and the instant the relay verified the payment, and the declared period is the duration one payment buys whatever `units` that payment names.**<!-- scp:end id="rent-arithmetic-period" --> <!-- scp:include id="rent-arithmetic-price" from=".docs/specs/09-security-model.md" -->**The relay verifies the receipt against `units` times the declared rent price, so the arithmetic multiplies the declared quota and the declared price by `units` and adds the period once.**<!-- scp:end id="rent-arithmetic-price" -->
   - **It refuses with `4041` scope `payment`, whose `value` carries this entry's rent terms**, on the payment grounds R9 states; **with `4041` scope `rate_limit` where the declared rate limit refuses the message**, because `rate_limit_publish` is a declared term and a `RENT` counts against it like every other message; and with `4042` on the three grounds no payment satisfies.
   - **`RENT` stores no bytes**, so no retained kind and no ring rule reads it, and it displaces nothing. <!-- scp:include id="rate-limit-check" from=".docs/specs/09-security-model.md" -->**The rate-limit check counts every PUBLISH and every `RENT` against the publishing address, reads no field of a receipt, and decodes nothing.** No declaration exempts any party from it, because a relay sells relief from a storage refusal and sells no relief from the rate limit.<!-- scp:end id="rate-limit-check" -->
...
| `RENT` | `identifier: bin32`, `units: u32`, `payment_receipt: bin?`, `signature: bin64` | OK with `rent_state` |
| `DELETE` | `blob_id: bin32` | OK (best-effort, does not confirm existence) |
| `ACK` | `blob_id: bin32` | None (fire-and-forget) |
| `PING` | `ts: u64` | PONG |

**Constraints:** `blob_ttl` 1–604800 (7 days), and absent on a `retain` write. `blob` 1–262144 bytes (256KB). `payment_receipt` 1–8192 bytes on PUBLISH and on RENT alike, so the field a charging relay decodes carries a declared ceiling as `blob` and `ref` do. `units` is at least 1 on a RENT that carries a receipt and is exactly 0 on a RENT that carries none, so the two implementations that compose a receipt-absent message compose one preimage. `signature` is exactly 64 bytes. `routing_id`, `recipient_hint`, `blob_id` are exactly 32 bytes, encoded as MessagePack `bin 32` (not hex/base64 strings).
...
| `ERR` | `ref: string?`, `code: u16`, `msg: string`, `scope: BudgetScope?`, `ground: RentRefusalGround?`, `operator_key: bin33?`, `value` | Error response. `msg` is for logging, not parsing, which is why `4041`'s two typed fields ride beside it. <!-- scp:include id="budget-scope-token-mapping" from=".docs/specs/03-identity.md" -->`BudgetScope` serializes under one spelling, one wire token per variant: `Identifier` takes `identifier`, `Total` takes `total`, `RateLimit` takes `rate_limit`, and `Payment` takes `payment`.<!-- scp:end id="budget-scope-token-mapping" --> The retry rule below, a publisher's decision to attach a receipt, and `PublishOutcome`'s per-entry list each read one of those two fields. `operator_key` rides on `4042`'s signature ground alone (**amended 2026-09-18**). |
...
**Client errors (4xxx):** `4000` INVALID_MESSAGE, `4001` UNKNOWN_OP, `4002` MISSING_FIELD, `4003` INVALID_FIELD, `4010` BLOB_TOO_LARGE, `4011` TTL_TOO_LONG, `4012` LIMIT_EXCEEDED, `4020` RATE_LIMITED, `4021` TOO_MANY_SUBSCRIPTIONS, `4041` STORAGE_POLICY_REFUSED (a validating SCP-native relay refused a PUBLISH or a `RENT` that failed one term of the write policy it declares. <!-- scp:include id="budget-scope-token-mapping" from=".docs/specs/03-identity.md" -->`BudgetScope` serializes under one spelling, one wire token per variant: `Identifier` takes `identifier`, `Total` takes `total`, `RateLimit` takes `rate_limit`, and `Payment` takes `payment`.<!-- scp:end id="budget-scope-token-mapping" --> <!-- scp:include id="budget-value-variants" from=".docs/specs/03-identity.md" -->`BudgetValue` carries one variant per `BudgetScope` and never a string: `Identifier(u64)` and `Total(u64)`, each in bytes; `RateLimit{ceiling_per_minute: u64, retry_after_seconds: u64}`; and `Payment{currency, per_publish, per_byte_stored, payment_adapters, payee, rent}`, which carries every declared term a publisher needs to mint what would satisfy the refusal.<!-- scp:end id="budget-value-variants" --> <!-- scp:include id="refusal-value" from=".docs/specs/09-security-model.md" -->**`value` carries the declared term the refusal names — the budget figure, the rate, or the price — so the one object a publisher holds after a refusal states what would satisfy it in every term but the rail-native address it settles to.**<!-- scp:end id="refusal-value" --> The checks that answer it, and the ones that answer `4040` instead, sit in the order <!-- scp:include id="publish-check-order" from=".docs/specs/03-identity.md" -->On a PUBLISH a validating relay runs, in this order, the declared rate limit, the structural decode, the identifier-to-routing-id binding, the declared storage budgets, `PaymentAdapter::verify` with its three comparisons, then chain verification, at which the relay evicts, places, and spends the pair of the rail-bound receipt identifier and the operation the receipt paid for.<!-- scp:end id="publish-check-order" --> (**`proof_of_work` removed 2026-09-14**); the SDK surfaces both as `IdentityError::StorageBudgetExceeded{scope, value}`, `03-identity.md` §3.10.10. **Added 2026-09-13** under Alec's storage ruling, **`value` amended 2026-09-13**: the refusal is never a verdict about the identity, and a publisher that read it as `4040` would tell a controller its own recovery event is malformed. **A refusal under any term of a declared write policy takes `4041` and takes no other code**, and **every operation this ADR declares that a declared term can refuse can take it**: a PUBLISH under any of the four scopes, and a `RENT` under `RateLimit` or `Payment`. `4020` covers a rate limit on an operation that declares no policy and `5001` covers a relay out of room rather than at a declared ceiling), `4042` RENT_REFUSED (**added 2026-09-15**, **amended 2026-09-18**: a validating SCP-native relay refused a `RENT` on a ground no payment satisfies, carrying a `ground` field typed as `03-identity.md` §3.10.10's `RentRefusalGround`, whose three variants ride as one token each — `signature_absent`, `signature_invalid` and `identifier_unknown_to_entry`. **On the `signature_invalid` ground the message also carries `operator_key`, the 33-byte key the relay reconstructed the preimage with**, so a controller whose shipped community-relay-list entry predates an operator-key replacement composes its next message against the relay's current key rather than rotating a key that was never stale. `09-security-model.md` §9.7.4.2 R9 states the three grounds and what a controller does about each; the SDK surfaces the refusal as `IdentityError::RentRefused{ground, terms, operator_key}`, which carries the key on the signature ground and nothing on the other two. **These three are not terms of a declared write policy**, so `4041`'s universal above stands and a controller reading one of them settles no payment, where `4041` would have told it a price would satisfy a stale signing key), `4040` DID_RECORD_REJECTED (a validating SCP-native relay rejected an operation at an identity-domain `routing_id`: a PUBLISH of a frame that failed the identifier-to-routing-id binding or chain verification, a frame the relay could place on no chain, any blob published to a `routing_id` that already holds a chain and that is not a frame passing every validation step `03-identity.md` §3.10.2 orders, or a DELETE of a stored frame whose chain verifies — see the subsection below on what may open or extend a key-event chain at a routing id. **`03-identity.md` §3.10.2 orders those steps** (**amended 2026-09-14**): <!-- scp:include id="publish-check-order" from=".docs/specs/03-identity.md" -->On a PUBLISH a validating relay runs, in this order, the declared rate limit, the structural decode, the identifier-to-routing-id binding, the declared storage budgets, `PaymentAdapter::verify` with its three comparisons, then chain verification, at which the relay evicts, places, and spends the pair of the rail-bound receipt identifier and the operation the receipt paid for.<!-- scp:end id="publish-check-order" --> The policy checks that order runs first answer `4041` and never `4040`, so a relay that returned `4040` for a budget or a price told a controller its own recovery event was malformed. **Amended 2026-09-10:** the code's name carries the retired identifier-record vocabulary, and this ADR names the wire constant as it ships rather than inventing one).

**Server errors (5xxx):** `5000` INTERNAL_ERROR, `5001` STORAGE_FULL, `5002` SHUTTING_DOWN. **`5001` is what a relay declaring no `storage_budget_total` answers a write it cannot take after displacing every uncovered identity it may displace**, because that code names a relay out of room and names no declared ceiling a publisher could satisfy (**added 2026-09-18**). A relay that declares one answers `4041` scope `total` instead: <!-- scp:include id="total-scope" from=".docs/specs/09-security-model.md" -->**`Total` refuses a write where displacing every uncovered identity the relay may displace still leaves the retained capacity short of the arriving frame's bytes, and refuses no write while one such identity remains**, because the ring below displaces the oldest-established uncovered identity instead. That one refusal covers the write that carried a receipt and the write that carried none, and it returns `StorageBudgetExceeded{Total, value}` carrying the declared total budget, which is the figure the `POLICY` query returns and the figure that states what would satisfy the refusal.<!-- scp:end id="total-scope" -->

Clients MUST handle unknown codes by category: 4xxx = do not retry same request, 5xxx = retry with backoff or switch relay. Codes are extensible within these ranges. **`4041` is the one 4xxx code a client retries at the same relay** (**amended 2026-09-14**). A retry is permitted where the publisher changed the request to satisfy the term the `value` names — attaching a receipt for the declared price, or renting the quota the entry declared — **or where it waited the seconds a `rate_limit` value states**. The category rule's prohibition binds a retry that repeats the refused request without either act. **A client does not retry `4042` at the same relay by paying**, because no payment changes any of its three grounds; its recourses are to publish the chain that entry lacks, or the events that advance the head it holds, and then to sign under the key that entry's head lists; or to re-compose the preimage against the operator key the refusal carried. **A publish that advances a stale head is an ordinary PUBLISH and the entry's declared per-identifier budget binds it** (`09-security-model.md` §9.7.4.2 R9), so where that budget refuses the frame the entry is too small for that identity and the controller writes to the next entry of its publication set (**added 2026-09-18**, **amended 2026-09-21**).
...
| `rate_limit_publish` | 6000/min | The rate per publishing address, counting every PUBLISH and every `RENT` (`09-security-model.md` §9.7.4.2 R9). Per-IP shared across connections; operator-adjustable. Priced relays have economic DDoS resistance (§19.7). |
...
pub enum ClientMessage {
    ...
    Rent { ref_id: Option<String>, identifier: [u8; 32], units: u32, payment_receipt: Option<Vec<u8>>, signature: [u8; 64] },
...
#### What may open or extend a key-event chain at a routing id (validating SCP-native relays, OPTIONAL)

**Amended 2026-09-10.** ADR-063, inception-derived self-certifying identity over a key-event log, retired the record form this subsection transcribed, and the rules now live in `03-identity.md` §3.10.2. That section states relay-side validation as an ordered list of checks run cheapest first. The chain-facing ones are the structural decode of the frame, the identifier-to-routing-id binding check, chain verification, and placing the frame's events on a chain, and §3.10.2 orders the payment verification, the declared rate limit and the declared storage budgets ahead of them. The chain authorizes the write, and no key the writer supplies does. The same section states what may open or extend a chain at a routing id: once a binding-valid frame whose chain verifies establishes a chain there, the relay rejects a later publish there that is not a frame passing every check §3.10.2 orders, evicts the opaque blobs already stored there when the first chain is established, returns on query every chain that routing id holds and nothing else, and rejects a client delete of any stored frame whose chain verifies, on a gate that is storage-derived, rate-limited, and fails closed on a storage read error. `09-security-model.md` §9.10.12 defines the frame and the obligations its decoder carries.

**What this ADR still decides** is the storage model those rules run on, and the substrate leaves it unchanged. Which frames make which chain is an index the validating relay keeps over its blob store, so enforcement is backend-agnostic and applies uniformly across every configured backend (in-memory, SQLite, redb, S3, Postgres) with no per-backend code, and it changes neither the opaque store's `(routing_id, blob_id)` keying nor its multi-blob-per-`routing_id` contract at other addresses. Every decision about what a routing id already holds reads storage rather than the index, because a frame is content-addressed and verifies against its own bytes: after a restart the index is cold while a durable backend still holds the genuine record, and a storage-authoritative decision keeps that window availability-only instead of letting a cold index purge or roll back the durable record.

**Optional, never a trust dependency.** Relay-side validation is an OPTIONAL capability of SCP-native relays, and the protocol MUST NOT require it. A foreign transport or a non-validating SCP transport stores the frame as an ordinary opaque blob, and a resolver verifies every event itself, so a relay that skips, botches, or lies about validation degrades availability alone and never integrity (`03-identity.md` §3.10.2 and §3.10.8).
