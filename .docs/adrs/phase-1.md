...
6. `UcanHeader` includes a `kid: Option<SigningKeyId>` field, so a JWT header naming any verification method outside `#active` and `#agent` fails to parse and no reader downstream decodes a fragment string. It serializes as the same `"#active"` / `"#agent"` string a `kid` header carries on the wire. `MintParams` includes a `key_scope: Option<String>`, because `fct.scp_key_scope` is a free JSON facts value rather than a header field; a minter decodes it through `SigningKeyId::from_fragment` and rejects a scope naming any other method. Which verification method a scoped self-delegation is signed by — and therefore whether step 5b's equality check is right — is an open question recorded in `.docs/specs/00-open-questions.md` under Crypto and Security, because the artifacts that answer it are specs this ADR does not govern.
