...
- **(d)** the relay rejects an (unauthenticated) **DELETE of the current slot blob** — only a superseding PUBLISH may replace a slot. DID records are public, so an attacker can compute `blob_id = SHA-256(genuine_record)` and issue a DELETE to force the slot to revert, then replay the owner's older genuine frame to pin a stale slot that rule (c) would then serve exclusively. Since DELETE is unauthenticated on every transport, the relay gates it: a `blob_id` that is the current slot blob of any claimed DID-domain `routing_id` is refused (`DID_RECORD_REJECTED`) on every validating transport that shares the slot registry (WebSocket, QUIC, UDP/DTLS, WebTransport). Integrity still holds regardless (resolver `seq`-monotonicity + DHT, see Reversion), but rule (d) removes the on-demand slot-reversion primitive. DELETE of any non-slot blob (all encrypted context blobs, pre-seed junk) is unaffected.

**Reversion.** Slot-exclusivity is a property of a *claimed* slot. If the slot record's own blob TTL (§9.10.2) expires while the owner is offline past the 6-day republish cycle (§3.10.2) — or, for an in-memory slot index, on relay restart — the `routing_id` reverts to an unclaimed opaque-blob address and the pre-seed window reopens. This is **not a suppression bypass**: during that window the genuine record is already absent (owner offline, not attacker action). The rollback defense is **not** signature verification — a *replayed old-but-genuine* record is owner-signed and passes the resolver's DID-derived-key BEP44 verify — it is the resolver's **client-side sequence-number monotonicity** freshness check: a record is accepted only when its BEP44 `seq >= last_known_seq`, and the highest valid `seq` is authoritative across both the relay and the DHT (§3.10.7, spec §9.6.1 "the BEP44 sequence number is the sole authority for document freshness"; only the identity owner can increment it). So a stale attacker-replayed record cannot roll a resolver back during a reversion window; resolution also falls through to the DHT, and the owner's next republish re-establishes the slot and re-fires rule (b). (An arbitrary *forged* blob that is not owner-signed still fails the BEP44 verify outright — but that is the always-on integrity guarantee, not what protects the reversion window against a genuine replay.)
...
**Cold-index reconciliation.** The index is in-memory, but a *durable* blob backend outlives it: after a relay restart the index is empty (cold) while a genuine slot record is still persisted in storage. To keep the "availability only, never integrity" invariant, establishing a slot at a cold `routing_id` first reconciles against storage — it enumerates the co-located blobs, re-validates each as a frame, and **adopts the highest-`seq` binding-valid frame already present** rather than letting an incoming lower-`seq` frame win. Consequently a *replayed old-but-genuine* frame (owner-signed, so it passes validation) can never evict a fresher genuine record that survived the restart: the newer record is adopted and the replay is rejected as non-superseding. This is a one-time O(N) scan on the first establish after a cold start (N bounded by the co-located blobs, behind the per-IP rate limit).
