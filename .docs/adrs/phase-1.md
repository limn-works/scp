...
4. **Key Rotation — Three-Layer Architecture**
   ...
   **4a. `rotate_active_key(identity, key_custody) -> Identity`** (Layer 1 — common case)
   ...
   - Updates the DID document: adds the new key as a verification method, moves `authentication` and `assertionMethod` references to the new key.
     ...
     **This criterion's whole retention clause is SUSPENDED, both the retention and the bound, pending the open question in `.docs/specs/00-open-questions.md` ("Retired verification methods").** It read: "Retains the old key as `#retired-{sequence}` for historical verification. Retired key retention is bounded: the document retains at most the 2 most recent retired active keys; older ones are pruned on rotation to prevent unbounded DID document growth within DHT size constraints."

     Suspending the whole clause, rather than only its bound, is deliberate. An earlier draft of this amendment suspended §18.2.2A's prohibition and retracted this criterion's count while leaving its retention `MUST` live. The composite contract that produced — emit `#retired-*` entries, retain them, under no cap — is neither of the two answers the open question is asking between, and §18.2.2B criterion 2 forbids it outright ("every entry class that appears as a value carries a maximum count this specification states"). Retracting half a contradiction manufactures a third position; both halves are therefore suspended together.

     **The shipped implementation is frozen in place, not de-authorized.** `retire_active_key` (`crates/scp-did/src/document.rs:882`) emits `#retired-{sequence}` today, `rotate_agent_key` (`:1093`) calls `prune_retired_agent_keys`, and `MAX_RETIRED_AGENT_KEYS = 2` (`:281`) enforces the agent-side count. That code is **authorized to keep behaving exactly as it does** for the duration of the suspension — an implementer neither removes it nor extends it — because a suspension is not a repeal and changing shipped behaviour is a decision the open question owns. What no implementer may do is add the missing **active**-key pruner, extend retention, or change the count, since each of those acts on the answer.

     **The reason the original bound gave has expired regardless of how the question resolves.** Under the two-layer model (§3.10, §18.2.2C) every entry outside the Mainline bootstrap core lives on the relay layer under a 262,039-byte bound, and retired keys are outside the core, so "DHT size constraints" constrain nothing here. `MAX_RETIRED_AGENT_KEYS`'s own doc comment cites ADR-039 for that same size rationale, which ADR-039's amendment further down this file retracts; the constant keeps its behaviour under the freeze above, and its stated justification does not survive.
   ...
   **4a′. `rotate_agent_key(identity, key_custody) -> Identity`** (Layer 1 — agent key rotation, ADR-039)
   ...
   - Updates the DID document: replaces the `#agent` verification method with the new key.
     ...
     **This criterion's retention clause is SUSPENDED on the same terms as §4a above** — both the retention and the bound, together. It read: "Retains the old agent key as `#retired-agent-{sequence}` for historical verification. Retired key retention is bounded: the document retains at most the 2 most recent retired agent keys; older ones are pruned on rotation (same policy as active key rotation) to prevent unbounded DID document growth within DHT size constraints." It carried the identical count of 2 and the identical expired "within DHT size constraints" justification. The shipped `prune_retired_agent_keys` and `MAX_RETIRED_AGENT_KEYS = 2` are frozen in place under §4a's freeze rather than de-authorized. See the open question in `.docs/specs/00-open-questions.md`.
