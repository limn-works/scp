...
1. **`PUBLISH { routing_id, recipient_hint, blob_ttl, retain, payment_receipt, blob }`**
   ...
   - `payment_receipt` (optional `bin`, **added 2026-09-13**, **amended 2026-09-15**): the `PaymentReceipt` bytes of `19-economic-governance.md` §19.2.1, tagged with the adapter that issued them. **A relay declaring `per_publish` charges the write**, runs every check `09-security-model.md` §9.7.4.2 R9 states, in the order `03-identity.md` §3.10.2 gives, and refuses the write with `4041` scope `payment` on the grounds R9 states. "Paid" names a receipt this relay verified, which is the term R9's `Payment` scope reads. **A receipt presented here pays the publish fee and nothing else, and rent reaches the relay on `RENT`** (R9).
...
6. **`RENT { identifier, units, payment_receipt, not_after, chain_segment, signature }`** (**added 2026-09-14**, **amended 2026-09-15**)
   - Pay rent for one identity at this relay, or read that identity's rent state. **The operation carries six fields and no others**: `identifier`, the 32 raw digest bytes of the identity the rent is credited to; `units`, the count of declared quotas the payment buys; `payment_receipt`, optional, the `PaymentReceipt` bytes of `19-economic-governance.md` §19.2.1 tagged with the adapter that issued them and bounded by the Constraints line below; `not_after`, a Unix-seconds instant past which the signer will not have the message honoured; `chain_segment`, optional, a frame carrying a contiguous segment of the signer's own chain, which lets a controller establish its chain at an entry that holds none in the same round trip; and `signature`, the 64-byte signature `09-security-model.md` §9.7.4.2 R9 defines over six operands.
   ...
   - The relay runs the checks `09-security-model.md` §9.7.4.2 R9 states, in the order that rule gives, credits that identity's quota and rent expiry under the one arithmetic R9 states, and answers `OK` with a `RentState` (`03-identity.md` §3.10.10), so a controller learns what it now holds without a second round trip. **It refuses with `4041` scope `payment`, whose `value` carries this entry's rent terms**, on the grounds R9 states.
   - **`RENT` stores no bytes**, so no retained kind and no ring rule reads it, and it displaces nothing. **The declared rate limit counts it like every other message**, because a relay sells relief from a storage refusal and sells no relief from the rate limit, so no payment buys a party a higher message rate.
...
**Constraints:** `blob_ttl` 1–604800 (7 days), and absent on a `retain` write. `blob` 1–262144 bytes (256KB). `payment_receipt` 1–8192 bytes on PUBLISH and on RENT alike, so the field a charging relay decodes carries a declared ceiling as `blob` and `ref` do. `units` is at least 1 on a RENT that carries a receipt and is exactly 0 on a RENT that carries none, so the two implementations that compose a receipt-absent message compose one preimage. `not_after` is a Unix-seconds instant, and `09-security-model.md` §9.7.4.2 R9 states the window a relay admits it in. `chain_segment` is bounded as `blob` is. `signature` is exactly 64 bytes. `routing_id`, `recipient_hint`, `blob_id` are exactly 32 bytes, encoded as MessagePack `bin 32` (not hex/base64 strings).
...
| `ERR` | `ref: string?`, `code: u16`, `msg: string`, `scope: BudgetScope?`, `ground: RentRefusalGround?`, `value` | Error response. `msg` is for logging, not parsing, which is why `4041`'s two fields ride beside it: `scope` is typed as `03-identity.md` §3.10.10's `BudgetScope` and names the term the refusal failed, and `value` carries that term's own value, typed per scope as the same section's `BudgetValue` declares. The retry rule below, a publisher's decision to attach a receipt, and `PublishOutcome`'s per-entry list each read one of those two fields (**amended 2026-09-14**). |
