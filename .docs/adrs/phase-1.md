...
1. **`PUBLISH { routing_id, recipient_hint, blob_ttl, retain, payment_receipt, blob }`**
   ...
   - Store it for `blob_ttl` seconds **where the write carries one**; a `retain` write carries none, and `09-security-model.md` §9.7.4.2 R9's ring buffer is what removes such a record, under the coverage R9 gives an identity a verified receipt names.
...
6. **`RENT { identifier, units, payment_receipt, signature }`** (**added 2026-09-14**, **amended 2026-09-15**)
   - Pay rent for one identity at this relay, or read that identity's rent state. **The operation carries four fields and no others**: `identifier`, the 32 raw digest bytes of the identity the rent is credited to; `units`, the count of declared quotas the payment buys; `payment_receipt`, optional, the `PaymentReceipt` bytes of `19-economic-governance.md` §19.2.1 tagged with the adapter that issued them and bounded by the Constraints line below; and `signature`, the 64-byte signature `09-security-model.md` §9.7.4.2 R9 defines over the identifier, the units and the receipt digest.
...
| `ERR` | `ref: string?`, `code: u16`, `msg: string`, `scope: BudgetScope?`, `value` | Error response. `msg` is for logging, not parsing, which is why `4041`'s two fields ride beside it: `scope` is typed as `03-identity.md` §3.10.10's `BudgetScope` and names the term the refusal failed, and `value` carries that term's own value, typed per scope as the same section's `BudgetValue` declares. The retry rule below, a publisher's decision to attach a receipt, and `PublishOutcome`'s per-entry list each read one of those two fields (**amended 2026-09-14**). |
