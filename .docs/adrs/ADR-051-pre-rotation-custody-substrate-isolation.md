...
- `consume(handle) -> Zeroizing<[u8; 32]>` — destroy-and-export the private bytes atomically at migration time (the §9.7.4.1 §6 / `migrate_identity` step-5 "post-rotation key cycling" operation).
