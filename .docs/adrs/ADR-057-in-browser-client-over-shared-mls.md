...
- **T1 — Crate-topology split.** Dissolve `scp-primitives` into `scp-clock` / `scp-crypto`; move the DID data model into a new wasm-safe `scp-did` (out of `scp-protocol` and `scp-primitives`); `scp-identity` keeps its native DHT/resolution/lifecycle subsystem unchanged, now importing the DID model from `scp-did` (no `scp-dht` — the DHT is not separable; see Amendment). Delete every re-export shim (including `scp-runtime/src/crypto/mls/mod.rs`'s `pub use scp_mls::*`). No behavior change; acyclicity + fence enforced.
...
|---|---|---|---|
| `scp-clock` | `Clock`, `SystemClock`, `TestClock`, `ClockError` (+ the hardened wasm `Date.now` clock, Prereq 1) | `scp-primitives/time.rs` | sync — wasm-safe leaf |
| `scp-crypto` | `verify_ed25519_signature` + low-level crypto anchor | `scp-primitives/crypto.rs` | sync — wasm-safe leaf |
| `scp-did` | `DID`, `SigningKeyId`, `extract_public_key_from_did`, `DidDocument`, rotation/migration proofs, `attestation`, `DidError` | `scp-primitives/identity.rs` **+** the `scp-protocol` strays (`identity/document.rs`, `identity/did_attestation.rs` — as-built, the DID model was moved to `scp-protocol` in Slice-1a, not left in `scp-identity`) | sync — **wasm-safe**, deps = `scp-crypto` |
...
- **No re-exports, no back-compat shims.** Every consumer imports the real crate (`scp_clock::Clock`, `scp_crypto::verify_ed25519_signature`, `scp_did::DID`, …). This also **deletes** the `scp-runtime/src/crypto/mls/mod.rs` `pub use scp_mls::{…}` shim left by Slice 1 — `scp-runtime` call sites import from `scp_mls` directly.
...
5. *Split the DHT transport into its own `scp-dht` crate (the original three-way hypothesis).* Rejected on implementation ground-truth: the DHT machinery is not separable from the identity model. `DidDht` `impl`s the async `DidMethod` trait, embeds/constructs `ScpIdentity` (custody `KeyHandle`s), and returns `IdentityError` (`scp_platform`-coupled), while `IdentityError`/`config` reference `DidDht`/`MigrationPartialState` and `MigrationPartialState` embeds `ScpIdentity` — a hard crate cycle. Splitting is impossible without either moving async/`scp_platform`-coupled types into the wasm-safe `scp-did` (defeating its purpose) or a logic change (out of scope for a behavior-preserving split). The identity subsystem is one native capability and stays one crate. *The type graph decides crate boundaries — an aesthetically-appealing split that the compiler rejects is not a boundary.*
