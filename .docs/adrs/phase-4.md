...
Alec ruled that a key-destruction attestation declaring a hardware-backed method reads as software-backed unless a verified platform attestation proof accompanies it. §27.4.6 of the attestations spec (`.docs/specs/27-attestations.md`) states the ruling in four clauses and gives the reasoning Alec used to reach it, and §9.15 of the security spec assigns a confidence rating to the method those clauses produce. This ADR states no reading rule of its own.
