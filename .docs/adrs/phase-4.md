...
No SCP implementation verifies a destruction platform proof today, so every hardware level a member publishes ranks at software-only. Open questions OQ-2 and OQ-29 of the attestations spec ask what a verification of such a proof would check, and open question OQ-8 of that spec asks what key identifier binds a published record to the material it claims was destroyed. The `KeyDestructionAttestation` row in §9.5.2 of the security spec puts `platformAttestation` inside the signing preimage, so a holder who strips or substitutes the proof invalidates the signature; what a passing verification of that proof would establish about the destruction is the three properties §27.4.6 of the attestations spec derives, and those three stay open. The close path records `KeyDestructionLevel::SoftwareOnly` from the disposal outcome it observed, which is the level the ruling produces for that path either way.

Alec ruled separately on 2026-08-28, when an agent put to him that closing the detachment meant changing the record's wire format. He wrote, in his words: "idc about wire format changes. if you need to do it, and it's not a doa choice, do it." §9.5.2 of the security spec carries the resulting signed-structure row and §9.18.2 carries the `"SCP-KEY-DESTRUCTION-V2:"` increment §9.5.1 requires for it. He wrote nothing about the field order, the encodings, or the increment itself; §27.4.6 of the attestations spec quotes the same statement and records what it left undecided.
