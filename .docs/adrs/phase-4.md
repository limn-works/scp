...
Alec ruled that a hardware-backed declaration reads as software-backed unless a verified platform attestation proof accompanies it. §27.4.6 of the attestations spec (`.docs/specs/27-attestations.md`) quotes the statements he wrote, names the binary an agent posed to him, and states the clauses the ruling sets; the sentence before this one is that section's statement of what he decided and is not his wording. §9.15 of the security spec assigns a confidence rating to the method those clauses produce. This ADR states no reading rule of its own.
