...
- **Scope note:** The initial application of this ADR covers `verify_participation_requirements` (Op A) and `check_capability_requirements` (Op B). Remaining trust-input operations that still take raw JSON at an SDK surface are brought onto this convention as they are touched.
