...
- **Per-PR compilation check** (`cargo +nightly-2026-05-03 check --manifest-path fuzz/Cargo.toml`): catches API breakage without running the fuzzer. Runs in `.github/workflows/ci.yml` as `fuzz-build`.
...
1. `cargo +nightly-2026-05-03 fuzz list --fuzz-dir fuzz` lists all 19 targets.
2. `cargo +nightly-2026-05-03 check --manifest-path fuzz/Cargo.toml` succeeds on stable CI (nightly toolchain via the `+nightly-2026-05-03` flag; nightlies after that date reject openmls 0.8.1's prelude re-export).
