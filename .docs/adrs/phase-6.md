...
- **Per-PR compilation check** (`cargo +$FUZZ_TOOLCHAIN check --manifest-path fuzz/Cargo.toml`, where `$FUZZ_TOOLCHAIN` is the nightly `fuzz/rust-toolchain.toml` pins): catches API breakage without running the fuzzer. Runs in `.github/workflows/ci.yml` as `fuzz-build`.
...
1. `cargo +$FUZZ_TOOLCHAIN fuzz list --fuzz-dir fuzz` lists all 19 targets, where `$FUZZ_TOOLCHAIN` is the nightly `fuzz/rust-toolchain.toml` pins.
2. `cargo +$FUZZ_TOOLCHAIN check --manifest-path fuzz/Cargo.toml` succeeds on a stable CI runner, because the explicit `+toolchain` flag selects the pinned nightly for that command.
...
5. `.github/workflows/ci.yml` includes a `fuzz-build` job that runs `cargo +$FUZZ_TOOLCHAIN check --manifest-path fuzz/Cargo.toml`.
