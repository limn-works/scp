...
**Context migration action.** `GovernanceAction` also declares `ProposeContextMigration`, through which a context proposes its own migration to a destination context. §5.11A.1 of the contexts spec, migration initiation, declares that action's payload fields — `new_context_params`, `reason`, `grace_period` and `auto_invite` — and this ADR restates none of them. `GovernanceAction` declares no action that accepts a migration, because §5.11A.3 of the contexts spec, member migration, makes each member join the destination context through the standard join flow and gives governance no part in that acceptance.

**Bridge lifecycle actions.** `GovernanceAction` also declares the five actions through which a context governs a bridge connector: `RegisterBridge`, `SuspendBridge`, `ReactivateBridge`, `RevokeBridge` and `UpdateBridgePlatformKey`. The platform-bridge-connectors spec declares each one's payload fields and states what a context does when it executes an approval of it: §12.2.1, bridge registration protocol, declares `RegisterBridge`; §12.2.2, bridge removal protocol, declares `RevokeBridge`, `SuspendBridge` and `ReactivateBridge`; and §12.10.2 step 5, authentication, declares `UpdateBridgePlatformKey`. This ADR restates none of those payload fields, as it restates no field of the `EconomicPolicy` that ADR-033, the economic policy model, declares for `SetEconomicPolicy`. An approved `RegisterBridge`, `SuspendBridge`, `ReactivateBridge` or `RevokeBridge` action appends the bridge lifecycle leaf that ADR-011, the verifiable event log, declares for it, and an approved `UpdateBridgePlatformKey` action appends no bridge lifecycle leaf, because the `GovernanceActionExecuted` leaf records that execution and §12.10.2 step 5 computes the key-rotation deadline from that leaf.
...
2. **`GovernanceProposal` struct and `GovernanceAction` enum:**
   ...
   - `GovernanceAction` with every variant section 3 declares (including content access actions: RevokeReadAccess, RestoreReadAccess, RevokeWriteAccess, RestoreWriteAccess, RotateContentKeys; broadcast action: BlockAuthor; economic actions: SetEconomicPolicy, ApproveSpend, LockEconomicPolicy; migration action: ProposeContextMigration; bridge lifecycle actions: RegisterBridge, SuspendBridge, ReactivateBridge, RevokeBridge, UpdateBridgePlatformKey) and `RevocationScope` enum (Full, FutureOnly). This criterion counts no variants, and the list it reads is all of section 3 rather than that section's `GovernanceAction` code block alone: the code block, the paragraph that opens with the words Context migration action, and the paragraph that opens with the words Bridge lifecycle actions each declare variants, so an implementation that omits a variant any one of those three declares fails this criterion.
