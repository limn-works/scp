...
2. **`GovernanceProposal` struct and `GovernanceAction` enum:**
   ...
   - `GovernanceAction` with every variant section 3 lists (including content access actions: RevokeReadAccess, RestoreReadAccess, RevokeWriteAccess, RestoreWriteAccess, RotateContentKeys; broadcast action: BlockAuthor; economic actions: SetEconomicPolicy, ApproveSpend, LockEconomicPolicy; migration actions: ProposeContextMigration, AcceptContextMigration; bridge lifecycle actions: RegisterBridge, SuspendBridge, ReactivateBridge, RevokeBridge, UpdateBridgePlatformKey) and `RevocationScope` enum (Full, FutureOnly). This criterion counts no variants, because the enum block of section 3 is the list it reads.
