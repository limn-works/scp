...
8. CLAUDE.md's enforcement-files list will be updated in the merge commit that lands this PR to add: `scripts/check-bridge-symmetry.sh`, `scripts/bridge-aliases.json`, and `scripts/tests/bridge-symmetry/` fixture tree. Until that merge commit lands, these files are protected only by the PreToolUse hook in `.claude/settings.json`.
