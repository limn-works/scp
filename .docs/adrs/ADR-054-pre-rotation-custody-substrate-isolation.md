...
**Status:** Proposed — the recovery-authority-residence RULE is canonical in spec §9.7.4.1 item 3a; this ADR's realization (per-profile floors, per-backend §3-soundness table, conformance PAIR, §5 ceremony) is PROPOSED in RFC #2130, to be validated when executed (#1729 / #1777). Not accepted-canonical because the realization is not being executed now.

**Scope note (2026-07-14):** the §3a residence RULE this ADR authored lives canonically in spec §9.7.4.1 item 3a (core residence paragraph + the fail-closed-no-fallback rule + the migration-not-a-daily-operation sub-paragraph + the at-rest paragraph); everything else in this ADR — the per-profile floors, the approved-backend menu realization, the §3-soundness table, the conformance PAIR, and the §5/§6 ceremony — is the PROPOSED realization, now homed in RFC #2130 (pre-rotation recovery custody) for validation when production pre-rotation custody is executed (tracked by #1729 / #1777). The ADR body below is retained as the design record, but it is Proposed, not Accepted.
**Date:** 2026-06-14
**Phase:** Phase 6 (production readiness, security)
**Amended (2026-07-14) — recovery-authority residence; OQ2/OQ3 resolved; realization PROPOSED in RFC #2130:** A cryptographer analysis established that substrate isolation (§9.7.4.1 §3) is a property of WHERE THE RECOVERY AUTHORITY RESIDES, not of the cipher wrapped around the key — so §4's parameter checklist does not by itself satisfy §3, and encrypted-offline with an auto-generated passphrase is degenerate (isomorphic to InMemory) on a non-interactive server. The spec now owns this rule as normative **§9.7.4.1 item 3a (recovery-authority residence)**, added spec-first ahead of this amendment; this ADR realizes and schedules it. OQ2 is resolved with **per-profile floors** (non-interactive server floor = independent-principal KMS/HSM/cloud-vault, NOT encrypted-offline-auto-gen; interactive clients may use user-supplied-passphrase encrypted-offline), the full §4 menu remaining available per profile under the §3a residence rule. OQ3 is resolved **YES** (§3 prose constrained the provider object, not the secret's residence; the new §3a clause is required and lands spec-first). With OQ2/OQ3 resolved the design is complete; but because production pre-rotation custody is not being executed now, this amendment's realization is **PROPOSED in RFC #2130** (not accepted-canonical) — only the §3a residence RULE is canonical (spec §9.7.4.1 item 3a). See the Amendment section for the per-backend §3-soundness table, the strengthened conformance-test pair, and the at-rest caveat. Prior content below is retained; this note and the Amendment section amend it.
...
- **Defer to a bare tracking issue with no design.** Rejected per CLAUDE.md ("never create tracking issues instead of doing the work"): the design must be done, not parked in an empty ticket. This ADR *is* that design step — now **Proposed**, with its realization homed as a fully-specified proposal in RFC #2130 (tracked by #1729 / #1777). A designed-and-Proposed realization awaiting execution is not the rejected pattern; an undesigned deferral is.

## Amendment (2026-07-14): Recovery-authority residence — RULE canonical (spec §9.7.4.1 item 3a); realization PROPOSED (RFC #2130)

This amendment does not delete or contradict the design above; it sharpens the security property the design must satisfy and resolves OQ2 and OQ3 at the design level, but **leaves the ADR Proposed** — the realization is homed in RFC #2130, not accepted-canonical. The design is complete and review-converged; production pre-rotation custody is punted to a later e2e-collab stage, so this amendment's realization is recorded as PROPOSED in RFC #2130 rather than accepted-canonical; only the §3a residence rule is canonical (spec §9.7.4.1 item 3a). Provenance: the normative rule is authored spec-first as **§9.7.4.1 item 3a** in `../specs/09-security-model.md`; this ADR cites and realizes it, per the artifact-flow invariant. The rule is upstream; ADR-054 does not author it.
...
The shipped `InMemoryPreRotationCustody` default (traits.rs doc-comment "default in production … until backends wired in") is, under §3a, a fail-closed VIOLATION — not a tolerable degraded default. ADR-062 (§Decision 6) SEVERS it: `InMemoryPreRotationCustody` is re-gated test-harness-only (`#[cfg(feature = "testing")]`) and is ABSENT from every shipped binary, and the `traits.rs` "default in production" doc-comment is deleted. Because no real production pre-rotation backend exists yet (this ADR's realization is PROPOSED in RFC #2130 / #1729 / #1777), production identity creation MUST fail closed with a typed error — it must never fall back to the nullifier. This is the normative target realized by ADR-062 §Decision 6 (Proposed, pending execution); it is not yet the shipped behavior. This note records the design-authority consequence; the code edits are ADR-062's (the nullifier weld is severed there, not merely documented).

## Open questions (for review)

> **Resolution status (Amendment 2026-07-14):** OQ1 resolved by ADR-055 (below). **OQ2 resolved** — per-profile floors; see the Amendment section. **OQ3 resolved YES** — §9.7.4.1 item 3a authored spec-first; see the Amendment section. All open questions are resolved AT THE DESIGN LEVEL; the ADR is **Proposed** (realization in RFC #2130, to be validated when executed) — only the §3a residence rule is canonical (spec §9.7.4.1 item 3a).
