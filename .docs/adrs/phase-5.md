...
3. **`AppleDeviceAttestation` — App Attest:**
   ...
   - `verify(token: DeviceAttestationToken) -> Bool`: Decides whether the bytes are an Apple App Attest attestation object that this app's App Attest key produced. `verify` decides the structure of that object, the authority behind its certificate chain, and whether that chain and that object name this app's stored App Attest key, all three from bytes the caller holds: the adapter carries Apple's App Attest root certificate, so this decision reaches no relay and no network. The relay decides what `verify` cannot: it validates the receipt, reads the nonce out of the credential certificate, and binds that nonce to the challenge it issued. `verify` returns `false` whenever a clause below does not hold.
     ...
     **The criterion.** `verify` returns `true` for a token that satisfies every clause below, and `false` for every token that does not. There is no other accepting branch.
     ...
     3. `attStmt` is a definite-length CBOR map carrying exactly two entries, no two of which repeat a key, and whose keys are `"x5c"` and `"receipt"`. `x5c` is a definite-length array of at least two byte strings, each of which parses as a DER-encoded X.509 certificate; the first is the credential certificate and the second is the Apple App Attest intermediate certificate. Those certificates evaluate as an X.509 certification path that terminates at Apple's App Attest root certificate, and that path's leaf is element 0 and its next certificate is element 1. `receipt` is a byte string.
        ...
        **How the adapter decides which authority issued a chain.** It carries Apple's App Attest root certificate — the one Apple publishes as `Apple_App_Attestation_Root_CA.pem` at `https://www.apple.com/certificateauthority/private/` — sets that certificate as the sole trust anchor through `SecTrustSetAnchorCertificates` plus `SecTrustSetAnchorCertificatesOnly`, forbids network fetching during the evaluation, and evaluates `x5c` under a basic X.509 policy. It then reads back the path the evaluation built and requires that path to begin with element 0 and element 1, because `SecTrustCreateWithCertificates` reads its certificate argument as one leaf plus an unordered bag and so decides no positions on its own.

        **What this clause does not state.** It fixes no identity for element 1: no name, no public key, and no fingerprint. Apple signs and replaces its App Attest intermediate certificate on its own schedule, so an adapter that compared element 1 against a stored copy would reject every genuine attestation from the day Apple issued a replacement. Anchoring at a root that expires in 2045 accepts whichever intermediate Apple signed.
...
5. **`AppleStorage` — SQLCipher:**
   ...
   - Every one of the six methods above throws when SQLite rejects a parameter bind, and none of them reports a result computed from an unbound parameter. SQLite reports a rejected bind through a return code and leaves that parameter reading `NULL`, and a statement carrying `NULL` where a key belongs still steps to `SQLITE_DONE`: `delete` would then remove no row and return, `exists` would answer `false` for a key the database holds, and `retrieve` would answer `nil` for it. Reading each bind's return code is what turns those three answers into a thrown `StorageError.databaseError`.
   - Every one of the six methods above names a key by that key's exact UTF-8 byte count, and `listKeys` returns each key at the byte count SQLite reports for it. Two keys that differ only after a zero byte therefore name two rows, and `listKeys` returns each of them whole. A C string ends at its first zero byte, so a method that handed SQLite a C string pointer and a negative length would answer `store("a\0b", x)` by writing the one-byte key `a`, `store("a\0c", y)` would overwrite that same row, and `retrieve("a\0b")` would then answer `y`. SQLite answers `SQLITE_OK` for that bind and steps that statement to `SQLITE_DONE`, so the return code the bullet above reads rejects nothing here, and the byte count is what separates those two keys. The same byte count also forbids a null pointer, which SQLite reads as a request to bind `NULL` while still answering `SQLITE_OK`.
...
6. **`PlatformAdapter` initialization:**
   ...
   public final class ApplePlatformAdapter {
       ...
       ///   Clause 4 of acceptance criterion 3 binds `verify(token:)` to it, and
       ///   no runtime API on iOS reads a team ID, so a caller supplies it.
       public static func make(appId: String) throws -> ApplePlatformAdapter {
           let keyCustody = AppleKeyCustody()
           let attestation = AppleDeviceAttestation(appId: appId)
   ...
   - `ApplePlatformAdapter.make(appId:)` is called by `SCP.init()` when `custody: "platform"` is specified (ADR-026), and `SCP.init()` takes that App ID from its caller.
   - All four providers are initialized before the Rust engine starts. If any provider fails to initialize (e.g., Keychain inaccessible at boot), `make(appId:)` returns a descriptive `PlatformError`.

7. **Conformance test suite:**
   - A Swift test suite under `bindings/swift/Tests/SCPTests/Platform/` covers each of the four providers by applying this ADR's acceptance criteria to it directly. A suite covers a provider when, for each constraint that provider's criteria state and a `swift test` host reaches, one case exists that fails if that constraint is removed. Two constraints fall outside a `swift test` host, because it runs no App Attest service and holds no APNs entitlement: `attest()` reaching Apple's App Attest service, and `register()` reaching APNs. The physical-device lane below covers those two. The Rust conformance macros in `crates/scp-testing/src/conformance/` — `key_custody_conformance!()`, `storage_conformance!()`, `attestation_conformance!()`, `push_conformance!()` (ADR-006) — bind Rust implementations of the `scp-platform` traits and expand into Rust `#[cfg(test)]` modules, so no Swift class is an argument a Rust macro can take and no Swift class passes one.
