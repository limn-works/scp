...
3. **`AppleDeviceAttestation` — App Attest:**
   ...
   - `verify(token: DeviceAttestationToken) -> Bool`: Decides whether the bytes are an Apple App Attest attestation object that this app's App Attest key produced. `verify` decides the structure of that object and the authority behind its certificate chain, both from bytes the caller holds: the adapter carries Apple's App Attest root certificate, so this decision reaches no relay and no network. The relay decides what `verify` cannot: it validates the receipt, reads the nonce out of the credential certificate, and binds that nonce to the challenge it issued. `verify` returns `false` whenever a clause below does not hold.
     ...
     **Four consequences of the criterion.** These follow from the four clauses; a reader applies the clauses, not this list. An empty token fails clause 1. A UTF-8 string fails clause 1. A CBOR map carrying an extra key alongside the three named keys fails clause 1, because clause 1 fixes an entry count of three rather than requiring the three keys to be present. A CBOR map carrying four entries that spell one key twice fails clause 1 twice over, on that entry count and on that repeat.
...
7. **Conformance test suite:**
   - A Swift test suite under `bindings/swift/Tests/SCPTests/Platform/` covers each of the four providers by applying this ADR's acceptance criteria to it directly. The Rust conformance macros in `crates/scp-testing/src/conformance/` — `key_custody_conformance!()`, `storage_conformance!()`, `attestation_conformance!()`, `push_conformance!()` (ADR-006) — bind Rust implementations of the `scp-platform` traits and expand into Rust `#[cfg(test)]` modules, so no Swift class is an argument a Rust macro can take and no Swift class passes one. A Swift suite covers a provider when a case exists that fails if any single constraint that provider's criteria state is removed from it.
