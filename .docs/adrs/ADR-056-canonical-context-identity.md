...
- **Otherwise** `context_id` is not a real context id — a synthetic namespace (`"identity-private-state"` — cited here as the §9.12 PSK-rotation namespace; that send was deleted when step 6 was made to fail closed, so the literal now has no production producer and survives only as a resolver test label), a standing-pair display id (`"standing-" + hex`, which carries the prefix and so is never bare 64-hex), or an arbitrary test label (`"ctx-…"`). These fall through to `SHA-256(id)` via the raw primitive, producing **byte-for-byte the same value as before this change** — they were never 64-hex, so their behavior is unchanged.
