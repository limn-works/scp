...
- **`identity_rotate_key`.** WASM (`crates/scp-ffi/wasm/src/identity.rs`) rotates the *identity* key (`#0`, the DID-deriving key): a fresh Ed25519 keypair is drawn, a new `did:dht:z…` DID is derived from the new public key, the old DID is recorded in the migration links so `identity_resolve` surfaces it under `alsoKnownAs`, and a new `WasmIdentity` is returned. This is DID-migration semantics — the operation native bridges expose under `identity_migrate`. PyO3 / NAPI / UniFFI's `identity_rotate_key` calls `DidDht::rotate_active_key`, which rotates only the `#active` signing key per spec §3.7; the identity key, the DID, and the document subject all stay stable. WASM consumers needing active-key rotation cannot reach it through this name — the op is tracked for a true active-key rotation in WASM under issue [#1717](https://github.com/limn-works/scp/issues/1717) (which superseded the stub-replacement attempt in PR #1720). WASM consumers needing DID migration should call `identity_rotate_key` (current behavior); a future WASM `identity_rotate_key` aligned to native semantics will be a breaking change to the WASM surface and will need to land alongside a WASM `identity_migrate` to preserve both capabilities.
