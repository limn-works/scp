...
The signed `ContextExport` snapshot signature is **Ed25519 over `SHA-256("SCP-CONTEXT-EXPORT-V1:" || JCS(ContextSnapshot))`**, where `JCS` is the RFC 8785 (JSON Canonicalization Scheme) canonical-JSON serialization of the *entire* embedded `ContextSnapshot` — every field, not a subset. This is specified normatively in spec **§23.16.8 (Signed Context Export)**, and §17.5 now references §23.16.8 instead of §23.16.4.
