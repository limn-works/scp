...
```
   ...
   - **Dual signing keys — `#active` OR `#agent` (normative).** `.docs/specs/09-security-model.md` §9.9.3 specifies the `ConsistencyCheckpoint.signature` field as "signed by sender's `#active` or `#agent` key (ADR-039); equivocation detection applies to both". This binds the JUDGING side as much as the producing side: a checkpoint signed under `#agent` is as valid as one signed under `#active`, and equivocation by an `#agent`-key signer MUST be detectable. The runtime judge does not yet satisfy this — `queries_helpers::verify_remote_checkpoint_authenticity` resolves `scp_did::SigningKeyId::Active` only, so an `#agent`-signed checkpoint is REFUSED with `ContextError::CryptoFailed` (fail-closed — a refusal to judge, not a forgeable acceptance) and equivocation by an `#agent`-key signer therefore goes undetected. Resolving both verification methods before verification is the real fix; see #2325.
