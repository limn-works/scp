...
8. **Five-language equivalence:** each config object and its enums map to Rust `struct`+`enum`, Python `@dataclass`+sum type, TS interface+discriminated-union, Swift `struct`+associated-value-enum, Kotlin `data class`+`sealed class`. The existing `StorageConfig` enum — defined per FFI bridge (`crates/scp-ffi/uniffi/src/runtime.rs`, the napi bridge, and the PyO3 dict form) as an identical-shape mirror of the core config — is the worked precedent for this cross-language equivalence. A new advanced-injection capability slot (a `Custom(concrete)` variant carrying a caller-supplied Rust provider) is a Rust-core-only design element of this pattern: a Rust trait object cannot cross the FFI boundary, so the bridge mirror enums omit it and expose only the named/convenience variants. This Rust-core-only/FFI-named split mirrors the established `parse_custody` asymmetry (the FFI surface accepts named custody selectors, never a caller-supplied Rust custody object).
