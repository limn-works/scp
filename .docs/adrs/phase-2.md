...
```
   ...
   **Signing-key identification (ADR-039).** The verification method that signed
   ...
   any other Ed25519 signature in the protocol. (The same holds for the
   `ConsistencyCheckpoint` of criterion 8: it carries no `signing_key_id` field
   and `generate_checkpoint` takes no such parameter — the verification method is
   resolved from the signer's DID document. *Which* verification methods a
   verifier MUST accept for a checkpoint is a separate, normative question; see
   the dual-signing-key requirement under criterion 8.)
...
```
   ...
   - **`async generate_checkpoint(log: &EventLog, sender_did: &DID, epoch: u64, signer: &(impl EventLogSigner + ?Sized)) -> Result<ConsistencyCheckpoint, EventLogError>`**: Creates and signs a checkpoint from the current log state. Signing goes through the key-custody-backed `EventLogSigner` (hence `async`), so the private key never crosses this boundary. There is no `signing_key_id` parameter: as in the signing-key-identification note under criterion 1, the verification method that signed is carried by the signature apparatus and resolved from the signer's DID document, never passed alongside the key. Which verification methods a verifier must ACCEPT is the separate normative requirement below.
   - **Dual signing keys — `#active` OR `#agent` (normative).** `.docs/specs/09-security-model.md` §9.9.3 specifies the `ConsistencyCheckpoint.signature` field as "signed by sender's `#active` or `#agent` key (ADR-039); equivocation detection applies to both". This binds the JUDGING side as much as the producing side: a checkpoint signed under `#agent` is as valid as one signed under `#active`, and equivocation by an `#agent`-key signer MUST be detectable.
     ...
     - **Status: SATISFIED.** The judge previously resolved `scp_did::SigningKeyId::Active` unconditionally, which refused honest `#agent` signers and — worse — exempted an `#agent`-key equivocator from the §9.9.3 tier-(a) detection MUST entirely. It now resolves the declared method, so detection applies under both.
