...
```
   ...
   **Signing-key identification (ADR-039).** The verification method that signed
   ...
   any other Ed25519 signature in the protocol: the signature apparatus DECLARES
   the verification method, and the DID document is then read for the public KEY
   published under exactly that method. The document supplies the key; it never
   selects the method. (The same holds for the `ConsistencyCheckpoint` of
   criterion 8: it carries no `signing_key_id` field and `generate_checkpoint`
   takes no such parameter — its declaration is the `signing_key_id` of the
   `InnerEnvelope` that carries it, and the DID document is read for that one
   method's key. *Which* verification methods a verifier MUST accept for a
   checkpoint is a separate, normative question; see the dual-signing-key
   requirement under criterion 8.)
...
```
   ...
   - **`async generate_checkpoint(log: &EventLog, sender_did: &DID, epoch: u64, signer: &(impl EventLogSigner + ?Sized)) -> Result<ConsistencyCheckpoint, EventLogError>`**: Creates and signs a checkpoint from the current log state. Signing goes through the key-custody-backed `EventLogSigner` (hence `async`), so the private key never crosses this boundary. There is no `signing_key_id` parameter on the checkpoint STRUCT: as in the signing-key-identification note under criterion 1, the verification method is declared by the signature apparatus that carries the checkpoint — the `signing_key_id` of the enclosing `InnerEnvelope` — rather than duplicated as a struct field a second signature would have to cover. The signer's DID document is then read for the public key published under that declared method; the document never selects the method (selecting it there is precisely the try-both mechanism the requirement below forbids). Which verification methods a verifier must ACCEPT is the separate normative requirement below.
   - **Dual signing keys — `#active` OR `#agent` (normative).** `.docs/specs/09-security-model.md` §9.9.3 specifies the `ConsistencyCheckpoint.signature` field as "signed by sender's `#active` or `#agent` key (ADR-039); equivocation detection applies to both". This binds the JUDGING side as much as the producing side: a checkpoint signed under `#agent` is as valid as one signed under `#active`, and equivocation by an `#agent`-key signer MUST be detectable.
     - **How a judge accepts both: resolve the DECLARED method, never try both.** ADR-039 (§MLS Impact, `.docs/adrs/phase-1.md`) is normative here — "Verifiers resolve the correct public key from the DID document **based on this field**." The verifier therefore resolves the *one* verification method the sender declared and verifies against it; it does NOT resolve `#active`, then fall back to `#agent`, and accept whichever verifies. Try-both would decouple the persona stamp from the signing key — exactly the divergence ADR-039's Enforcement-Stack layer 2 exists to prevent, where the stamp and the key "are chosen together from one persona and cannot diverge" — and would let an `#agent`-signed checkpoint be accepted while declaring the `#active` persona, laundering an agent action into a human attribution. Accepting both methods and binding each signature to its declared method are the same requirement, not competing ones.
     - **Where the declared method comes from.** The `ConsistencyCheckpoint` struct carries no `signing_key_id` (see the `generate_checkpoint` note above: the verification method rides the signature apparatus, not the struct). The declaration is the `signing_key_id` of the `InnerEnvelope` that carries the checkpoint — the same field the envelope's own signature is verified under. The runtime threads it from the receive path (`messaging_helpers::deliver_incoming` → `deliver_checkpoint_message`) and from `Supervisor::compare_remote_checkpoint` / `MessagingCommand::CompareRemoteCheckpoint` into `queries_helpers::compare_remote_checkpoint` → `classify_remote_checkpoint` → `verify_remote_checkpoint_authenticity`, which resolves exactly that method. A method absent from the sender's DID document, or a signature that does not verify against the key it resolves to, is refused with `ContextError::CryptoFailed` — the judge fails closed, it does not retry under the other method. `scp_did::SigningKeyId` admits only `Active` and `Agent`, so no other key class is declarable; a checkpoint actually signed by any other key fails verification.
     - **Status: SATISFIED — on BOTH sides.** The requirement binds the judging side and the producing side, and each had its own break; satisfying only one leaves `#agent` checkpoints undetectable just the same.
       - *Judging side.* The judge previously resolved `scp_did::SigningKeyId::Active` unconditionally, which refused honest `#agent` signers and — worse — exempted an `#agent`-key equivocator from the §9.9.3 tier-(a) detection MUST entirely. It now resolves the declared method.
       - *Producing side.* `messaging_helpers::send_checkpoint` and `send_heartbeat` previously discarded the caller's `MessageSigner` and re-asserted `MessageSigner::Active`, so a checkpoint produced under `#agent` went out stamped `#active` while signed with the agent key. Every peer resolves the DECLARED method, so the envelope failed verification and the checkpoint never reached the judge — a fixed judge alone would still have seen nothing. Both now carry the caller's `MessageSigner` verbatim, and `Supervisor::build_local_checkpoint` / `send_heartbeat` take that coupled value at the public boundary (as `send_message` already did) instead of a bare key.
       - *Evidence.* `scp-testing`'s `reconnect_sync::agent_persona_checkpoint_survives_the_real_cross_member_receive_path` drives a real `#agent`-signed checkpoint from one member's actor through the other's full receive path against a resolver that publishes DIFFERENT keys for the two methods, and asserts a stamp/key mismatch is rejected; `::supervisor_compare_remote_checkpoint_honors_the_declared_method` pins the same property on the `Supervisor` → `MessagingCommand::CompareRemoteCheckpoint` → judge path. `queries_helpers` covers every verdict arm (`Consistent` / `Divergent` / `Behind` / `Ahead`) under `#agent`, plus refusal of an absent declared method and of a key class outside both methods.
   - ~~**`compare_checkpoint(local_log: &EventLog, remote_checkpoint: &ConsistencyCheckpoint) -> CheckpointComparison`**: Compares a received checkpoint against local state. Returns `Consistent`, `Divergent { first_divergent_event: Option<u64> }`, `Behind { missing_events: u64 }`, or `Ahead { extra_events: u64 }`.~~ **SUPERSEDED — this signature was unsound; the free function has been removed. Historical text struck through to the left; the correction is below.**
     - **Correction.** As prescribed, `compare_checkpoint` takes no signer, no roster, and no context binding, so it classifies an *unsigned, unverified, wrong-context* checkpoint from a *non-member* as `Divergent` — letting a relay or any unauthenticated sender forge an equivocation alert. This violates a spec `MUST` that governs this ADR: `.docs/specs/23-sync-and-offline-strategy.md` §23.12 item 1 — "Clients MUST verify the signature on every received `ConsistencyCheckpoint` **before accepting it for comparison**" — restated for cross-relay comparison in item 4: "Cross-relay checkpoint comparison (section 9.9.3) MUST only use checkpoints that have passed signature verification." A function whose signature cannot see a key or a roster cannot satisfy that `MUST`. (`.docs/audits/adr-audit-phase-1-3.md` independently recorded the same gap against this signature — "the ADR does not address it" — but the spec is the normative basis; the correction flows spec → ADR → code.) Comparison of a RECEIVED checkpoint now lives in the runtime as `queries_helpers::compare_remote_checkpoint`, which verifies the sender's signature, checks the sender's membership, and binds `context_id` before returning the same `CheckpointComparison`. The `CheckpointComparison` shape itself is unchanged. The related `cross_checkpoint_verify` / `verify_cross_checkpoint_with_leaves` / `CrossCheckpointResult` — never prescribed by any artifact — were removed for the same reason; the sound same-log prefix mechanism is the RFC 6962 consistency proof (`proof::prove_consistency` / `proof::verify_consistency`; RFC 6962 §2.1.2, invoked by spec §23.7 step 3).
