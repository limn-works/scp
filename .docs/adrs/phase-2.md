...
**Amended (2026-08-14) — a node inherits a storage handle instead of selecting storage a second time.** Alec Marcus ruled that storage is selected once, at `SCP`-instance construction, and that a node receives an already-opened handle; a deployment wanting a node on different storage runs a second `SCP` instance. §AC-3 below stands unchanged — selection stays mandatory, explicit, and never defaulted, and `NodeConfig` still carries a required non-`Option` storage slot. What a ruling settles is which value fills a slot. `.docs/specs/17-persistence-and-storage.md` §17.6 owns a rule normatively (§"A Node Inherits Storage; It Never Re-Selects Storage", written spec-first per an artifact-flow invariant); see "Amendment (2026-08-14)" below.
...
### Amendment (2026-08-14): node storage, blob storage, and configuration layering

Alec Marcus ruled on three points that constrain `NodeConfig` shape. `.docs/specs/17-persistence-and-storage.md` §17.6 carries all three normatively; ADR-052 records what changes for a construction surface.

1. **Node storage is inherited.** Storage is selected once at `SCP`-instance construction; a node receives an already-opened handle. In Rust core a slot §AC-3 requires holds `StorageSlot::Custom(handle)` — a variant `.docs/standards/construction.md` §Storage vocabulary already defines for a caller-supplied `Storage` implementation. Two mechanisms drive a ruling: `SqliteStorage::new` takes a non-blocking exclusive lock on `{dir}/scp.db.lock`, so a second open on one directory fails; and two independently-opened handles carry two event logs, two saga journals, and two MLS stores that diverge with no reconciliation path. §17.6 states both.
2. **Blob storage stays a `NodeConfig` field.** `NodeConfig.blob_storage` remains required and non-`Option`. Blob selection does not move onto an `SCP` instance, because a relay needs blob storage and a relay is not an `SCP` instance. Selection happens through a configuration key with an environment-variable override; a bespoke `SCP_RELAY_STORAGE_BACKEND` variable is not a supported mechanism (approved by Alec Marcus, 2026-08-14).
3. **One typed configuration struct serves both entry points.** An SDK caller constructs it programmatically; a `scp-node` binary loads TOML and applies environment-variable overrides on top. `BlobStorageBackend` is an enum over opened stores rather than a description of stores to open, so "one struct" means a deserializable descriptor plus a constructed `NodeConfig`, related by one construction step. §17.6 records that wrinkle.

M1 through M5 are unaffected: a config object stays flat, required fields stay required and non-`Option`, and no builder or typestate marker returns.
