...
|---|---|
| "stop trying to solve root theft. it's not solvable." | Every mechanism whose reason was a party holding a threshold of a victim's root leaves the corpus, and no sentence claims a defence against one. The newest-first accepted-chain read, its backward cursor, and the claim that a backward read keeps a pad from freezing an identity go with it. `MAX_RETAINED_BYTES` stays as a plain per-source, per-resolution download cap. |
...
| "ok ring buffer." | A relay's retained storage is a first-in-first-out ring: the newest write displaces the oldest, and there is no idle clock, no refresh write and no pressure ordering, so `EVICTION_IDLE_SECONDS` is deleted. The ring's unit, its order and the pinning of a paid write are the executor's derivations, and entries 21 and 22 of the executor-call register below state each one. |
| "slot is a bad name for the chain unit. why do we need special names? identity, and chain, no?" | The per-fork unit a relay stores is a **chain**, and the unit the ring buffer displaces is everything the relay holds for one **identity**. The corpus coins no replacement for either word: it states what may open or extend a chain at a routing id rather than naming that rule, and it writes out the divergent suffixes a verifier retains rather than naming that set. |

**The word his fifth ruling deleted covered two different objects, and the corpus now distinguishes them**: every record a relay holds for one identity, which is what the ring displaces, and the divergent suffixes a verifier retains under R9's count and byte bounds, which the ring never reaches. A sentence that named the first is rewritten around the identity, and a sentence that named the second is rewritten around the suffixes the party holds. **His ruling names those two objects and no third.**
...
16. **The accepted chain is not one of the divergent suffixes a party retains** and carries no eviction rank; at a first-contact verifier the chain it adopts under R11 becomes its accepted chain, and the fetch bound governs until then. A verifier never evicts it, and at a relay the identity does not pay it is displaceable by the ring buffer and by nothing else.
...
21. **The unit the ring buffer displaces is one identity: everything the relay holds for it, displaced together.** **The ring's order is a monotonic per-relay counter and no clock**: the relay increments one counter on each identity's first binding-valid frame, records that value durably beside the records it orders, and never decreases it, so no wall-clock correction inverts the order, two identities never take one value, and a restart restores the order rather than leaving the relay with none. A later accepted write extends what the relay holds for that identity and leaves its position unchanged, and a paid identity is pinned outside the ring. **A relay never displaces the identity whose write it is admitting**, and where displacing every other unpaid identity still leaves the ring short it refuses that frame for `Total`. Displacing a single frame, or a single chain, would leave a chain without its inception event and separate a fault proof's two cosigned heads.
22. **`Identifier` bounds what the relay holds for one identity, and `Total` is the whole retained capacity a relay declares, the ring and the pinned set being two partitions of that one figure.** **`Total` refuses an unpaid write where the relay holds no unpaid identity it may displace, and refuses a paid write where the pinned set already fills the declared total budget and the ring holds no unpaid identity to displace**; a relay that declares no total budget runs no ring, refuses nothing under `Total`, and answers a full host with ADR-004's `5001`. The superseded "declared pinned capacity" named a term `relay_config` never carried and `POLICY` never returned, so no operator could declare it and no publisher could read it. Under `Identifier` pressure the relay evicts that identifier's retained divergent suffixes by eviction rank before it refuses.
23. **PUBLISH carries a `payment_receipt`, and "paid" names a receipt the relay verified and bound to this write.** **Five checks decide it, each one reading a value the presenting party did not choose**, which `09-security-model.md` §9.7.4.2 R9 states: the verified payee binds the receipt to this relay, the write binding binds it to the frame's value bytes, the unspent rail-bound identifier binds it to one use, the verified amount covers the declared price, and the verified currency stops an amount in one money satisfying a price quoted in another. **The relay reads the terms `PaymentAdapter::verify` returns rather than the receipt's own fields**, because `payee`, `write_binding` and `receipt_id` are fields the presenting party composes and the payer's signature over them reaches no rule on the relay's path. A verified receipt bound none of these before.
