...
**Alec settled what bounds a relay's key-event storage on 2026-09-13, and the decision is his and not the executor's.** A per-identifier byte ceiling misses the actor — "the risk is bad actors not theives" — because a party minting identities fills a relay under any per-identifier figure. He asked for both halves of the answer, "we need both eviction and denial", named rent as a bound a relay already has, "relays can already charge rent.", and closed the exchange with **"ok."**

The decision has five parts, and `09-security-model.md` §9.7.4.2 R9 carries each one with its rule. A validating relay **declares** its write policy in `relay_config` and refuses a key-event PUBLISH that fails a declared term with a typed error naming the term, so the protocol fixes each term's shape and never its value. A PUBLISH of a key-event record carries a **proof-of-work** nonce, and the difficulty is per relay rather than a network constant, which leaves nothing to govern: "how do we maintain and govern pow" is answered by the relay's own configuration. A relay **evicts** a record it was not paid to hold once the identity has gone silent, which drains a flood after it stops; denial is what holds during one. **Rent** buys exemption from eviction and from budget refusal. **Free listed relays** keep accepting writes under the policy they declare, and a rank-1 reveal-authorized event is never refused for a storage budget, so an owner's recovery lands while a flood is running.

`MAX_ACCEPTED_CHAIN_BYTES` is withdrawn. `MAX_RETAINED_BYTES` stays, as two counters sharing one value: R9's byte bound on divergent-suffix bytes, and the verifier's fetch bound on the accepted-chain bytes it has read.
...
Nineteen calls the orchestrator made and surfaced to Alec rather than asked about, being the calls with consequence at this record's level. He may reverse any of them. Calls 1 through 10 were made on or before 2026-09-10; calls 11 through 19 on 2026-09-11, after round 13's and round 14's reviewers read the fresh corpus. The identity-substrate plan of record carries the full register of the executor's resolutions, of which these nineteen are the subset that changes a decision stated here. **Every ruling of Alec's takes its own heading in the Decision section above, and no entry below is one of his.**
