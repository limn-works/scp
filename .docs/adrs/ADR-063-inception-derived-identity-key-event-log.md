...
| "ok ring buffer." | A relay's retained storage is a first-in-first-out ring of the total budget's bytes, whose unit of displacement is one identity's whole retained set and whose order is each set's establishment time. A paid set is pinned outside the ring. There is no idle clock, no refresh write and no pressure ordering, so `EVICTION_IDLE_SECONDS` is deleted. |

**Alec was told the bandwidth residue before he ruled on the ring**: a party minting distinct identities displaces every unpaid retained set at a free relay by writing binding-valid frames at that relay's full ingest rate, the per-address rate limit bounds the rate, no rule bounds the duration, and the curator is the party that answers it. R9 states that residue where a reader meets the ring.
...
21. **The unit the ring buffer displaces is one identity's whole retained set, ordered by each set's establishment time.** A later accepted write extends the set it lands in and moves nothing, and a paid set is pinned outside the ring. Displacing a single frame would leave a chain without its inception event and separate a fault proof's two cosigned heads.
22. **`Identifier` bounds one identity's retained set and `Total` sizes the ring**, so `Total` refuses no unpaid write and refuses a paid write only where the relay's declared pinned capacity is full. Under `Identifier` pressure the relay evicts that identifier's retained divergent suffixes by eviction rank before it refuses.
23. **PUBLISH carries a `payment_receipt`, and "paid" names a receipt the relay verified and bound to this write.** Four checks decide it: the receipt's `payee` equals the relay's declared `economic.payee`, its `write_binding` equals the digest over that payee and the frame's value bytes, its `receipt_id` is unspent at this relay, and `verified_amount` covers the declared price. Three of the four are the bindings — to this relay, to these bytes, and to one use — and a verified receipt bound none of them before.
