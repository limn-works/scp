...
23. **PUBLISH carries a `payment_receipt`, and "paid" names a receipt the relay verified and bound to this write.** **Five checks decide it, each one reading a value the presenting party did not choose**, which `09-security-model.md` §9.7.4.2 R9 states and this register does not repeat. **The relay reads the terms `PaymentAdapter::verify` returns rather than the receipt's own fields**, because `payee`, `write_binding` and `receipt_id` are fields the presenting party composes and the payer's signature over them reaches no rule on the relay's path. A verified receipt bound none of these before.
