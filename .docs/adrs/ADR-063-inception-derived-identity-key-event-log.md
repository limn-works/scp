...
**The relay's storage budget is Alec's call and not the executor's.** He settled it on 2026-09-12. A per-identifier byte ceiling misses the actor — "the risk is bad actors not theives" — because a party minting identities fills a relay under any per-identifier figure, and what bounds a relay instead is what it charges: "relays can already charge rent." **Alec: "ok."** `MAX_ACCEPTED_CHAIN_BYTES` is withdrawn and `MAX_RETAINED_BYTES` stays as the verifier's fetch bound. `09-security-model.md` §9.7.4.2 R9 carries the three budgets a validating relay configures, the typed refusal and the fallback-set recourse, and `19-economic-governance.md` §19.8 carries why a retained record's stored byte buys permanence.
...
**Bounds and encoding.** KERI bounds no key list. SCP caps the root set, the next set, and the witness set, and `09-security-model.md` §9.18.17 carries every bound with its reason. KERI leaves storage to the witness relationship, where a witness serves only the controllers it agreed to serve; SCP's community relays accept a publish from anyone, so a relay's refusal past its storage budget is a protocol rule here rather than a term of a relationship. SCP writes its own byte layouts under the canonical hash construction of §9.5.1 rather than adopting CESR, and registers every domain separator in one table.
...
**A per-identifier byte ceiling on the accepted chain, or a KERI-style verified checkpoint.** The orchestrator put both to Alec on 2026-09-11 as the two shapes an accepted-chain bound could take, with proof of work and designation-as-write-permit offered beside them. He rejected the ceiling on 2026-09-12, because a party minting identities fills a relay under any per-identifier figure; the checkpoint fell with it, because it weakens the recompute-from-inception property `09-security-model.md` §9.7.4.2 R2 states. Proof of work was withdrawn before he ruled, because rent is a per-relay cost that does not tax a phone. Designation-as-write-permit was withdrawn because it would make the shipped community relay list an admission service, which the protocol-requires-no-operator tenet forbids, and because it constrains no relay that charges nothing.
