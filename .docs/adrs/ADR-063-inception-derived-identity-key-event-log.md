...
**Status:** Accepted 2026-08-30, the day Alec settled the identity form and delegated the choice of a freshness anchor. He confirmed the witness and watcher layer in scope on 2026-08-31. Four later rulings by Alec changed decisions this record already carried, one executor call rewrote a fifth, and his relay-storage ruling of 2026-09-13 added a sixth decision under its own heading below. Each amendment sits beside the decision it changed, with its date. Accepted records that the identity model is settled and not that the code exists.
...
**Alec settled what bounds a relay's key-event storage on 2026-09-13, and the decision is his and not the executor's.** A per-identifier byte ceiling misses the actor — "the risk is bad actors not theives" — and `09-security-model.md` §9.7.4.2 R9 states why. He asked for both halves of the answer, "we need both eviction and denial", named rent as a bound a relay already has, "relays can already charge rent.", asked "how do we maintain and govern pow", and closed the exchange with **"ok."**

The decision has five parts — a declared write policy, proof of work, eviction, rent, and the free-listed-relay invariant — and R9 carries each one with its rule and its reason.

`MAX_ACCEPTED_CHAIN_BYTES` is withdrawn. `MAX_RETAINED_BYTES` stays, and R9 states the two bounds that read it. **`EVICTION_IDLE_SECONDS` is the eviction window**, a relay constant in the relay's own clock, because a window computed from the subject's own `witnessing_interval` reads a field the published identity itself writes.
...
Twenty-four calls the orchestrator made and surfaced to Alec rather than asked about, being the calls with consequence at this record's level. He may reverse any of them. Calls 1 through 10 were made on or before 2026-09-10; calls 11 through 19 on 2026-09-11, after round 13's and round 14's reviewers read the fresh corpus; calls 20 through 24 on 2026-09-13, after round 16's reviewers read it again. The identity-substrate plan of record carries the full register of the executor's resolutions, of which these twenty-four are the subset that changes a decision stated here. **Every ruling of Alec's takes its own heading in the Decision section above, and no entry below is one of his.**
...
16. **The accepted chain sits outside the retained set** and carries no eviction rank; at a first-contact verifier the chain it adopts under R11 becomes its accepted chain, and the fetch bound governs until then. A verifier never evicts it, and at a relay the identity does not pay it is evictable on the idleness rule alone, which call 21 states.
17. **The fault proof gains a fifth condition**, excluding the designating event's digest, and the five conditions are the whole test. Without the fifth, one witness seeding twice at one designating event indicted an honest operator. Conditions one through four read the two heads' fields and the fifth reads the subject's chain, so no condition reads a prior head of the witness and any party holding the pair and the chain decides the predicate.
...
20. **The budget exemption reads the event's kind and never a rank.** A rank belongs to a divergent suffix, and the ordinary recovery extends the accepted chain and carries none, so a rank-keyed exemption was granted to the key material a thief holds and withheld from the owner who lost its root. A relay reads the kind off R3's type discriminator before any rank exists.
21. **`EVICTION_IDLE_SECONDS`, a relay constant in the relay's own clock, replaces a multiple of the subject's own witnessing interval**, which read a field the published identity itself writes, and R10's self-observation cadence carries the re-publication the window reads. Eviction reaches the four retained kinds, and storage pressure evicts unpaid records longest idle first.
22. **The proof-of-work preimage names four operands and binds a key-event PUBLISH alone.** The relay term makes one solution serve one relay, and the ceiling is stated as a bound on the honest client's cost and as nothing else. A client refuses to solve above it and moves to the next fallback-set entry.
23. **PUBLISH carries a `payment_receipt`, and "paid" names a receipt the relay verified through the adapter the receipt is tagged with.** Rent buys exemption from eviction and from the `Identifier` and `RateLimit` scopes and buys none from `Total`, so the capacity ceiling an operator declared bounds a funded party.
24. **The verifier reads an accepted chain newest-first, per source and per resolution**, and both fields of the `MissingEvents` payload a bound-reached stop returns carry a value the verifier holds.
...
**A per-identifier byte ceiling on the accepted chain, or a KERI-style verified checkpoint.** The orchestrator put both to Alec on 2026-09-11 as the two shapes an accepted-chain bound could take, with proof of work and designation-as-write-permit offered beside them. He rejected the ceiling on 2026-09-13, for the reason the storage-policy heading above records; the checkpoint fell with it, because it weakens the recompute-from-inception property `09-security-model.md` §9.7.4.2 R2 states. **The verifier-side fetch bound R9 states is not that ceiling**: the ceiling he rejected was a relay-side refusal to store past a figure, and the fetch bound is one verifier's limit on what it reads in one resolution from one source, which stores nothing and refuses nothing. R9 reads that bound newest-first for the reason it states there. **Proof of work was withdrawn once and then adopted, and the orchestrator both withdrew it and brought it back.** The orchestrator withdrew it on the ground that rent is a per-relay cost that does not tax a phone. Alec's own statement about the mechanism was a reservation against it — "pow sounds like more surface area than i would like" — and the orchestrator reintroduced it four turns later, answering his observation that "attackers can force users to start paying while attacking for free" with proof of work as the one cost that needs no account and no money. He then asked "how do we maintain and govern pow", took the per-relay-difficulty answer, and settled the mechanism. Designation-as-write-permit stays withdrawn, because it would make the shipped community relay list an admission service, which the protocol-requires-no-operator tenet forbids, and because it constrains no relay that charges nothing.
