...
17. **A cosigned head carries a one-byte `seed` marker, and the fault proof's fifth condition is that neither head sets it.** A witness sets the byte on the first head it cosigns for a subject after a designation or a store loss and clears it on every later head, so a declared re-seed stays outside the predicate while an equivocation does not. Every one of the five conditions reads the two heads' own fields, so any party holding the pair decides the predicate without the subject's chain. The superseded fifth condition excluded the designating event's digest, and a witness that wrote that digest on both heads escaped attribution for the price of one value any party can compute.
...
21. **The unit the ring buffer displaces is one identity: everything the relay holds for it, displaced together, ordered by each identity's establishment time.** A later accepted write extends what the relay holds for that identity and moves nothing, and a paid identity is pinned outside the ring. Displacing a single frame, or a single chain, would leave a chain without its inception event and separate a fault proof's two cosigned heads.
22. **`Identifier` bounds what the relay holds for one identity and `Total` sizes the ring**, so `Total` refuses no unpaid write and refuses a paid write only where the relay's declared pinned capacity is full. Under `Identifier` pressure the relay evicts that identifier's retained divergent suffixes by eviction rank before it refuses.
23. **PUBLISH carries a `payment_receipt`, and "paid" names a receipt the relay verified and bound to this write.** Three bindings decide it, which `09-security-model.md` §9.7.4.2 R9 states with the amount check beside them: the receipt's `payee` binds it to this relay, its `write_binding` binds it to the frame's value bytes, and its unspent `receipt_id` binds it to one use. A verified receipt bound none of the three before.
...
25. **A per-index signature is an indexed signature, renamed on 2026-09-14 by the orchestrator and by no ruling of Alec's.** Two of the orchestrator's own agents wrote the corpus at once and left one object named two ways: `09-security-model.md` called it a slot while `25-test-vectors.md` and `scripts/gen-test-vectors-p256.py` called it an indexed signature. A corpus that names one object two ways is worse than either name, which is why the orchestrator settled it rather than leaving it. The name follows the term the prior art uses for a signature carrying an index into a key list, so `11-prior-art.md`'s comparison reads in one vocabulary, and §9.7.4.2's definitions already called the group an indexed signature group.
