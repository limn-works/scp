...
**Four parts of that decision are his: denial, eviction, rent, and the frame that holds the three together.** The free-listed-relay invariant is not among them, because it predates this redesign as `19-economic-governance.md`'s economic-governance invariant. R9 carries each part with its rule and its reason.
...
He gave five rulings that day. Four changed the storage decision above or removed a mechanism the corpus carried, and the fifth named the two objects the first four are about.
...
| "ok ring buffer." | A relay's retained storage is a first-in-first-out ring: the newest write displaces the oldest, and there is no idle clock, no refresh write and no pressure ordering, so `EVICTION_IDLE_SECONDS` is deleted. The ring's unit and its order are the executor's derivations, and entries 21 and 22 of the executor-call register below state each one. What a payment does against that ring is his payment ruling of the same day, stated under its own heading below. |
| "slot is a bad name for the chain unit. why do we need special names? identity, and chain, no?" | The per-fork unit a relay stores is a **chain**, and the unit the ring buffer displaces is everything the relay holds for one **identity**. |

**The word his fifth ruling deleted covered two different objects, and the corpus now distinguishes them**: every record a relay holds for one identity, which is what the ring displaces, and the divergent suffixes a verifier retains under R9's count and byte bounds, which the ring never reaches. A sentence that named the first is rewritten around the identity, and a sentence that named the second is rewritten around the suffixes the party holds.

### Alec's payment ruling of 2026-09-14 — what a payment buys is the operator's to declare

The orchestrator asked what a payment buys and offered three candidate answers: eviction protection alone; that plus the per-identifier budget; those plus the rate limit. Alec answered **"suppose its up to the operator?"**, and confirmed the stated resolution with **"ok"**.

| Ruling | What it decided |
|---|---|
| "suppose its up to the operator?" | A relay that charges declares which refusal scopes a verified receipt exempts, named with the scope names that already exist. The declaration sits in `relay_config` and the `POLICY` query returns it, so a publisher reads what its money buys before it spends. A relay that charges and declares nothing exempts nothing. The relay's declaration governs, so an implementer reads that declaration and builds no fixed answer of its own. |

**Two rules are his, stated to him and confirmed inside that same answer rather than decided separately.** A receipt covers the identity it names up to the bytes it paid for, and the excess beyond that is ordinary evictable data. A relay never evicts the identity it is currently writing for.

**That ruling replaced the reading under which a payment pinned a record or an identity outside the ring**, and replaced the reading under which a payment bought a fixed set of refusal scopes the protocol chose. `09-security-model.md` §9.7.4.2 R9 carries the declared exemption set, the covered-byte sum and the coverage rule.

**Six calls sit inside that ruling, each the orchestrator's and each on the register below**: entries 25 through 30.

**Alec was told the bandwidth residue before he ruled on the ring**, and `09-security-model.md` §9.7.4.2 R9 states that residue where a reader meets the ring.
...
Each call below is one the orchestrator made and surfaced to Alec rather than asked about, being the calls with consequence at this record's level. He may reverse any of them. **This register carries every call still live and carries no entry for a call Alec reversed**, so a call he overturned leaves the register rather than staying in it under a superseded note. Calls 1 through 10 were made on or before 2026-09-10; calls 11 through 19 on 2026-09-11, after round 13's and round 14's reviewers read the fresh corpus; calls 20 through 24 on 2026-09-13 and 2026-09-14, after round 16's and round 17's reviewers read it again; calls 25 through 30 on 2026-09-14, inside Alec's payment ruling above. The identity-substrate plan of record carries the full register of the executor's resolutions, of which these thirty are the subset that changes a decision stated here. **Every ruling of Alec's takes its own heading in the Decision section above.**
...
23. **PUBLISH carries a `payment_receipt`, and "paid" names a receipt the relay verified and bound to this write.** **Five checks decide it, each one reading a value the presenting party did not choose**, which `09-security-model.md` §9.7.4.2 R9 states. **The relay reads the terms `PaymentAdapter::verify` returns rather than the receipt's own fields**, because `payee`, `write_binding` and `receipt_id` are fields the presenting party composes and the payer's signature over them reaches no rule on the relay's path. A verified receipt bound none of these before.
...
25. **`receipt_exempts` is the `relay_config` field's name, and a set of scope names is its type**, with an absent or empty set exempting nothing. Alec ruled that the relay declares what a payment buys and named no field; a publisher reads the set through ADR-004's `POLICY` query.
26. **`Identifier` and `RateLimit` are the two members a relay may declare, and `Total` and `Payment` are the two it may not.** A relay cannot sell past the capacity ceiling its own operator declared, which excludes `Total`, and the receipt is the object a `Payment` refusal reads, which makes exempting `Payment` circular.
27. **A declared `Identifier` exemption stops both arms of the per-identifier budget — the suffix eviction and the refusal — and a declared `RateLimit` exemption stops the relay counting the write against the address's rate and never stops the rate-limit check.** Both arms of the per-identifier budget read the one term the exemption names. A relay that skipped the rate-limit check would have to verify the receipt before it ran the check, which puts `PaymentAdapter::verify` ahead of the limit and reinstates the flood `03-identity.md` §3.10.2's check order exists to stop.
28. **The relay holds a covered-byte sum per identity rather than a per-record pin**, because Alec's rule names the identity and not the record.
29. **An identity holding more bytes than its receipts covered is an ordinary displacement candidate the ring takes whole.** His sentence says the excess is evictable and does not say whether a partly covered identity is split or displaced whole; a split would leave a chain without its inception event and would separate a fault proof's two cosigned heads, which is the reason the ring's unit is the identity.
30. **A receipt's paid bytes running out mid-chain draws no distinct answer from the relay**: it accepts the write under its ordinary budgets, returns no new error, and leaves the identity uncovered until its controller pays for the bytes it published. Refusing would contradict his own sentence that the excess is stored and evictable.
