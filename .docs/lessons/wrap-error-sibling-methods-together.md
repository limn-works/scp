...
drop retained identity state for an identity. Wrapping only the first makes it raise a typed
...
inconsistency is invisible at the call site and appears on the error path alone.
...
- **When you wrap one method's errors, wrap its entire behavioral family in the same
  ...
  create and remove, get and list variants — and confirm every member routes errors
