...
The passage also cites a section no merged spec carries.
`.docs/specs/18-addressability-and-deployment.md` numbers §18.11 up to §18.11.12, Site
Configuration, and stops there, so §18.11.13.2 [no such section] resolves to nothing on
`main`. The agent was reading an unmerged draft that proposes a thirteenth subsection under
§18.11, and it wrote the draft's number into a review a human read against the merged
