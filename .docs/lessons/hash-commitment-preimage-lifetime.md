...
**Resolution (2) is what landed.** `scp-platform` exposes `PreRotationCustody` as a sibling trait of `KeyCustody`; `InMemoryPreRotationCustody` is the shipped default backend (with substrate isolation — FIDO2, paper-backup callbacks — landing as additional `PreRotationCustody` impls without protocol churn). `ScpIdentity` carries `pre_rotation_commitment: [u8; 32]` (public hash) and the four operational handles (`identity_key`, `active_signing_key`, `agent_signing_key`); the pre-rotation private bytes are never on `ScpIdentity` directly. `DidDht::create_identity` now returns `(ScpIdentity, PreRotationKeyHandle)` and `migrate_identity` takes the handle plus a `&impl PreRotationCustody` parameter and consumes the OLD entry to mint the new `#0`.
