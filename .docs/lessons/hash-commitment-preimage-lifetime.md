...
|------------|----------|---------------|----------|
| `pre_rotation_commitment` (32-byte SHA-256) | Pre-rotation key public bytes | §9.7.4.1 item 2 (pre-rotation key custody) and §9.12 (migration and recovery) of the security-model spec; §4b of ADR-003, the DID method | Identity creation → next Layer 2 migration (potentially years) |
...
## What went wrong in SCP-1717, the pre-rotation custody-handoff story, and what holds now

`DidDht::create_new_identity_keys` published `SHA-256(pre_rotation_public)` as a commitment, then called `key_custody.destroy_key(&pre_rotation_key)`. It read item 5f of §9.7.4.1 of the security-model spec, which specifies pre-rotation key custody, literally: "Destroy the pre-rotation private key from the creating device's memory after backup is confirmed." `InMemoryKeyCustody` carried no backup callback, so that destroy ran unconditionally. At migrate time native bridges generated a fresh keypair, which broke the equality `SHA-256(revealed_key) == commitment` that a verifier must check. ADR-003, the DID method, states that equality as invariant 8 of `verify_migration` at `.docs/adrs/phase-1.md:430`, and its §4b builds the pre-rotation proof that carries it at `.docs/adrs/phase-1.md:385`. §9.12 of the security-model spec restates it at `.docs/specs/09-security-model.md:783`. Item 2 of §9.7.4.1 covers the publication of the commitment and states neither side of that equality.
...
**What remains unbuilt.** `InMemoryPreRotationCustody` is this repository's only implementation of `PreRotationCustody`, and it sits under a testing module — `crates/scp-platform/src/testing/pre_rotation_custody.rs:67` holds its sole `impl`, gated to a test harness by ADR-062 §Decision 6. A shipped build therefore reaches no pre-rotation backend at all. Identity creation fails closed with typed code SCP-IDENT-1059 rather than minting that nullifier, which `crates/scp-ffi/common/src/error_codes.rs:284` documents and attributes to CLAUDE.md's builder tenet forbidding a dev stand-in on a production path. No hardware-backed or callback-based implementation exists yet, and building one is what closes this gap. Two artifacts track that work: issue #1729, which validates the per-profile custody realization at execution, and RFC #2130, which proposes the pre-rotation recovery custody design.

That state satisfies this lesson's rule rather than violating it. The rule requires a code path that publishes a commitment to retain the preimage through the reveal site. A shipped build publishes no commitment, because identity creation fails before it, so no unretained preimage exists. Shipping `InMemoryPreRotationCustody` instead would publish a commitment whose preimage no production custody holds, which is the failure this lesson names. `.docs/lessons/custody-substrate-isolation-holds-at-rest-not-in-transit.md` states the constraint that binds a real backend once one lands.
