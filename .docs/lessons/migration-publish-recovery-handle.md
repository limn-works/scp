...
> **Dating note (2026-09-02):** this lesson describes `migrate_identity` and the pre-rotation model as Pre-Rotation Key Custody, §9.7.4.1 of `09-security-model.md`, read before the key-event-log recovery amendment. R10 of Root-Authority Recovery and Fork Precedence, §9.7.4.2 of that spec, states the controller's procedure now. The lesson's principle stands; its spec citations are historical.
...
`migrate_identity` performs eight steps. Step 5 (`destroy_after_migration`) consumes the OLD pre-rotation private bytes from cold custody — the operation is irreversible by design, and the ceremony order lives in §9.7.4.2 R10. Step 7 (publish the NEW identity's record) and step 8 (republish the OLD identity's record) BOTH happen AFTER step 5. If either publish fails, the caller cannot retry: step 1 would fail at `reveal_public_key` against a missing handle.
...
- `resume_migration_publish(state, key_custody)` — the resume entry point.
...
- **Spec authority.** The recovery handle is documented in the controller's procedure, §9.7.4.2 R10 of `.docs/specs/09-security-model.md`. Code without spec coverage is phantom provenance — the lesson here is that this kind of structural error decision belongs in the spec as much as the byte-format decisions do.
...
- **Forgetting the retention the final step needs.** The destroy step retires the replaced operational key, and the final publish still needs the key that signs it. Resume documentation MUST surface that retention contract.
