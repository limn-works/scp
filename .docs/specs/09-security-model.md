...
| Agent keys allowed | `true` | `{true, false}` | `ContextParams::agent_keys_allowed`. When `false`, the context admits no artifact signed by `#agent`. No struct declares the field; story SCP-AB-028 of `.docs/prds/agent-binding.json`, which adds `agent_keys_allowed`, declares it. | §4.9.3 |
| Agent action rate limit | Absent (no cap) | A count in [1, u32 max]; §4.9.3 states no window length and no range for one | `ContextParams::agent_rate_limit`. Caps actions signed by `#agent` per member within a rolling window; an `#active`-signed action does not count against it. A rate limit needs a count and a window, and §4.9.3 states only that the window rolls. No struct declares the field; story SCP-AB-030 of `.docs/prds/agent-binding.json`, which adds `agent_rate_limit`, declares it. | §4.9.3 |
| Agent co-signature requirement | Empty list | A list of capabilities; §4.9.3 states no ceiling bound and §5.9 gives the list no collection-size row | `ContextParams::agent_cosign_required`. Capabilities a member exercising them through `#agent` must additionally have signed with `#active`. No struct declares the field; story SCP-AB-031 of `.docs/prds/agent-binding.json`, which adds `agent_cosign_required`, declares it. | §4.9.3 |
