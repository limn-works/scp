...
**KeyDestructionAttestation** — domain: `"SCP-KEY-DESTRUCTION-V2:"`

The record §9.15 specifies. The `V2` suffix replaces `"SCP-KEY-DESTRUCTION-V1:"`, whose preimage covered neither `platform_attestation` nor `signing_key_id`, encoded `method` as a length-prefixed variant name, and carried no SHA-256; the versioning rule in §9.5.1 requires the increment, and no `V1` signature exists to invalidate, because no producer ever signed one.

| Order | Field | Encoding |
|-------|-------|----------|
| 1 | `context_id` | 4-byte BE length + UTF-8 bytes |
| 2 | `member_did` | 4-byte BE length + UTF-8 bytes (the DID of the member who destroyed the key material) |
| 3 | `destroyed_at` | 8-byte BE u64 (Unix seconds) |
| 4 | `platform_attestation` | 32 bytes, raw, no length prefix — `SHA-256("SCP-DESTRUCTION-PROOF-V1:" \|\| BE32(len(attestation_data)) \|\| attestation_data \|\| BE32(len(platform)) \|\| platform)` if present, or the `SHA-256(0x00)` sentinel if absent. `attestation_data` is the opaque platform proof, hashed as the raw bytes a transport decodes to, whatever encoding that transport uses to carry them; `platform` is UTF-8 |
| 5 | `method` | 1-byte U8 discriminator (0x00=SoftwareOnly, 0x01=HardwareBacked) |
| 6 | `signing_key_id` | 4-byte BE length + UTF-8 bytes (`"#0"` or `"#active"` — the DID verification-method fragment that produced the signature, resolved against `member_did`, field 2) |

The Ed25519 signature covers the 32-byte §9.5.1 canonical hash of the six fields above. The order matches the order §9.15 lists the record's fields in, so an implementer transcribing either produces the same bytes. This row fixes the preimage and not the record's serialized form: how a published record encodes `method`, `signature`, or a `PlatformAttestation` on the wire is unstated, and open question OQ-25 of the attestations spec, which asks who publishes an F6 record, owns it. A byte-exact known-answer vector for each arm of field 4 is in §25.25 (Vector 38, proof absent; Vector 39, proof present).

`platform_attestation` (field 4) is inside the signed scope, and the reason is §9.15's trust-level split. §9.15 grants High confidence only where a verification of the platform attestation returns a pass; a proof the signature does not cover arrives beside the declaration rather than under it, so a holder strips it or substitutes another one and the signature still verifies. Binding the proof's hash makes either edit invalidate the signature.

**What field 4 does not establish.** It binds which proof the signer chose. It does not bind that proof to this destruction event: the proof body hashes `attestation_data` and `platform` and mixes in no `context_id`, no `member_did`, and no `destroyed_at`, so the same signer attaches one platform proof to any number of destruction records and signs each validly. Whether a proof reports a destruction, whether it was issued to the DID that published the record, and whether it covers the key material the record claims are the three properties §27.4.6 of the attestations spec derives. Open question OQ-29 of that spec asks whether the platform proof has one type or three and routes the answer to whichever artifact open question OQ-2 produces, so OQ-2's artifact is where the verification procedure lands even though OQ-2 states its own question about F3. A rule binding the proof's own challenge to a value derived from fields 1 through 3 would change field 4's encoding, which §9.5.1's versioning rule then takes to `"SCP-KEY-DESTRUCTION-V3:"`.

Three further properties the preimage leaves to a verifier. No field ties `member_did` to membership of `context_id`, so any DID signs a well-formed record naming any context. No rule bounds `destroyed_at`, so a record claiming a destruction at Unix time 0 or at `u64::MAX` verifies. And field 6 binds a fragment rather than a public key or a document version, so a verifier that resolves the fragment against `member_did`'s current document reads whatever key that document publishes at read time. §9.15's clause — these records "remain verifiable after context keys are destroyed" — is about context keys, and `#0` and `#active` are both identity-layer keys, so both satisfy it. What §9.15 does not reach is rotation of the identity-layer key itself: §9.7.4's `rotate_active_key` replaces the `#active` verification method in the published document, and no section states which verification methods a migrated identity's old document retains, so how long a record of either fragment stays verifiable is a question no artifact answers. Open question OQ-25 of the attestations spec asks who verifies an F6 record, and the acceptance rules a verifier applies — which document it resolves, and at what time — belong to that answer.

The field enters as a 32-byte hash rather than as inline bytes because an Android Keystore certificate chain runs to kilobytes, and hashing bounds the preimage a verifier reads regardless of proof size. Three preimages in §5.4.5 of the contexts spec do the same thing, each carrying `caveats_binding` — itself the output of the separately registered `"SCP-OUTLET-CAVEAT-BIND-V1:"` hash — as 32 raw bytes with no length prefix: `"SCP-OUTLET-CHUNK-SIG-V1:"`, `"SCP-OUTLET-CREDIT-V1:"`, and `"SCP-OUTLET-CANCEL-V1:"`. Nothing about this section's other optional fields follows from that choice: `Attestation.expires_at` and `InnerEnvelope.provenance_hash` each encode their present arm at a width their absent arm never shares, and `Attestation.evidence` encodes its present arm at whatever width its content takes, which for a 28-byte value is the sentinel's own width. This section sanctions all three. What every field of this preimage does share is that its boundaries are unambiguous, which the length prefixes and the fixed widths supply. Field 4 supplies a weaker property than the inline fields do: it is a SHA-256 image, so two records carrying different proofs map to one preimage exactly when SHA-256 collides.

No field-level bound governs `attestation_data`: the 4-byte length prefix admits up to `u32::MAX` bytes, and neither §9.15 nor this row states a smaller one. Two bounds reach a published record from elsewhere. The §9.10.3 envelope bound does not, because §9.15 publishes this record to relays outside a context rather than inside an envelope. The §9.18.11 `Max blob size` of 262,144 bytes does, because a record published to a relay is a relay blob. What no artifact states is the bound a verifier applies to a record it reads from any other source, which the section open question OQ-2 of the attestations spec asks for states together with the checks that verifier runs.

`method` (field 5) takes the 1-byte discriminator that `InnerEnvelope.message_type` takes, and not one of the three longer enum encodings this section sanctions elsewhere, because `DestructionMethod` is a closed two-variant enum with no payload. The values follow the enum's declaration order, `SoftwareOnly` then `HardwareBacked`, as `InnerEnvelope.message_type`'s do, so an implementation that reaches for a variant ordinal reads the same byte.

`signing_key_id` (field 6) binds the verification method a verifier resolves, exactly as it does in `InnerEnvelope` (position 11) and `KeyPackageAttestation` (position 6). The values this field encodes are `"#0"` and `"#active"`, and no other. §9.15 excludes `"#agent"`, attributing the exclusion to ADR-039; ADR-039's Category A lists four actions and a destruction attestation is not among them, and its key-properties table gives `#agent` "Yes (within permission scope)" under "Signs operational actions" (`.docs/adrs/phase-1.md:1271`), so §9.15 is the only artifact that states this exclusion. Open question OQ-53 of the attestations spec carries that alongside the `#0` question. Which of the two a destruction attestation may carry is a question this spec answers twice and incompatibly: §9.15 permits `#0` or `#active`, and §9.7.4 states that `#0` is "Used ONLY for DID document updates and signing pre-rotation commitments." §27.4.6 of the attestations spec records the pair as contradiction C35, and open question OQ-53 of that spec asks a human which sentence stands.

The encoding above is the union of what the two sentences admit, so the *type* of the field commits to neither. A verifier is a different matter: a verifier that admits both values admits a `#0`-signed record, which is §9.15's sentence winning and §9.7.4's losing, and a verifier that admits only `#active` is the reverse. This section states no admission rule, because stating one would answer OQ-53. Until a human answers it, an implementation that must choose is choosing, and it says which sentence it chose.
...
3a. **Recovery-authority residence (normative).** Item 3's storage-isolation requirement constrains *where the recovery authority resides*, not merely which provider object holds the key handle. The **recovery authority** is the minimal secret, key handle, or authorization capability sufficient to recover the pre-rotation private key. It MUST reside in a substrate whose compromise is **independent** of operational-custody compromise: there MUST NOT exist any secret, key handle, or authorization capability reachable **(directly or transitively, following any key-wrapping or key-derivation chain to its root)** from the operational `KeyCustody` provider or the daily-operations authentication flow that — alone or in combination with publicly-stored artifacts (including the encrypted-offline ciphertext of item 4) — suffices to recover the pre-rotation private key. Wrapping the key in an approved cipher (item 4) does NOT by itself satisfy this requirement: the §3 adversary is one who has already compromised operational custody, and if the decryption authority for that cipher is itself reachable from operational custody, the cipher provides zero protection against that adversary. The load-bearing property is **principal-distinctness of the recovery authority**, not the strength of the cipher wrapped around the key.
   ...
   **Migration is not a daily operation.** Identity migration (§9.12) is a rare, separately-authorized event, not a daily operation. A distinct migration-time authorization principal — even one that is fully automated (e.g., a separate KMS role assumed only for migration, which the operational path holds no grant to assume) — satisfies this requirement. What does NOT satisfy it is the daily operational signing flow itself being able to reach the pre-rotation key.

   This is an **at-rest / daily-operations** property. It constrains where the recovery authority lives when no migration is in progress; it cannot and does not prevent the revealed pre-rotation seed from being live in operational process memory during an authorized migration (`consume` by construction yields plaintext, which is then imported as the new operational `#0`; §9.12, ADR-003 §4b).
...
**Partial-publish recovery.** The migration described above performs two DHT publishes that BOTH occur AFTER the cold-custody consumption step (§9.7.4.1 item 6 "post-rotation key cycling"; `migrate_identity` step 5 in the reference implementation, which destroys-and-exports the old pre-rotation private bytes atomically). The two publishes are: a publish of the NEW DID document, then a republish of the OLD DID document with `alsoKnownAs` pointing at the new DID and the OLD operational verification methods retired. If either publish fails after the old pre-rotation key is consumed, the caller cannot simply re-invoke `migrate_identity` — the only key whose hash satisfies `SHA-256(revealed_key) == commitment` is gone. The protocol MUST surface a typed recovery handle in this case carrying the byte-identical artifacts (new identity, new document, rotation event with the step-2 migration proof and pre-rotation proof, new pre-rotation handle registered at step 4, plus the old identity and old document) so a resume entry point can finish the migration without re-deriving keys or re-signing proofs. The resume operation MUST preserve byte parity: the `revealed_key` carried in the rotation_event and the commitment published on the OLD document MUST be byte-identical between the failed-first-pass and the successful-resume artifacts. The OLD `#0` is retained in operational custody for exactly this case — it signs the OLD-document republish; the OLD `#active` and `#agent` (if present) are destroyed before the OLD-document republish (defense-in-depth, §9.12). Reference implementation: `DidDht::migrate_identity` returns `IdentityError::MigrationPublishFailed { phase, partial, source }` and `DidDht::resume_migration_publish` consumes the partial state.
...
KeyDestructionAttestation {
  ...
  signingKeyId:          "#0" | "#active"       // the DID verification-method fragment that produced the signature; §9.7.4 contests "#0" for this use (see below)
...
PlatformAttestation {
  attestationData:       Bytes                  // opaque, platform-specific
  platform:              String                 // UTF-8 platform identifier, e.g. "apple-secure-enclave"
}
```

4. Attestations are published to relays (outside the now-destroyed context). They are signed by a key that survives the destruction — an identity-layer key rather than a context key — so they remain verifiable after context keys are destroyed. `signingKeyId` records which one, and the paragraph below states that the choice between `#0` and `#active` is unsettled.

**Signing construction.** §9.5.2 states the field order and every field's encoding under the domain separator `"SCP-KEY-DESTRUCTION-V2:"`, and the signature covers the §9.5.1 canonical hash of that preimage. Three properties of that row carry rules this section states: field 4 puts `platformAttestation` inside the signed scope, so a holder cannot substitute a different proof for the one the signer chose; field 5 encodes `method` as a one-byte discriminator, so two implementations produce the same bytes; and field 6 binds `signingKeyId`, so a verifier reads the intended verification method out of the signed bytes. §25.25 of the test-vector spec pins Vector 38 and Vector 39, the known-answer vectors for the absent-proof and present-proof arms.

**Which verification method may sign is unsettled.** The signature annotation above permits `#0` or `#active`. §9.7.4 of this spec states of the Identity Key: "Used ONLY for DID document updates and signing pre-rotation commitments." A destruction attestation is neither, so the two sentences cannot both hold. §27.4.6 of the attestations spec (`.docs/specs/27-attestations.md`) records the pair as contradiction C35, quotes both sides, and leaves the choice to a human. `signingKeyId` binds whichever fragment signed, so a verifier enforces the rule that human writes.
...
No SCP implementation verifies a destruction `platformAttestation` today, and no artifact states the checks such a verification would run — open questions OQ-2 and OQ-29 of the attestations spec own that procedure. Every published `.hardwareBacked` record therefore rates as Moderate confidence today. Field 4 of the §9.5.2 row binds the proof the signer chose, so a holder who strips or substitutes it invalidates the signature; what a passing verification of that proof would establish about this destruction is the three properties §27.4.6 of the attestations spec derives, and those three stay open.
...
| `"SCP-KEY-DESTRUCTION-V2:"` | KeyDestructionAttestation signing — replaces `"SCP-KEY-DESTRUCTION-V1:"`, which no producer ever signed under. `"SCP-KEY-DESTRUCTION-V1:"` is retired: no future structure takes that string | §9.5.2, §9.15 |
| `"SCP-DESTRUCTION-PROOF-V1:"` | Hash of a KeyDestructionAttestation `platformAttestation` proof body, which field 4 of the preimage carries | §9.5.2, §9.15 |
