...
**Failure modes.** Each row names the keys the attacker holds and the outcome the rules of §9.7.4.2 produce over key material alone. **Two parties who each hold the pre-rotation key can each reveal it, and key material cannot tell their events apart: those rows are contested (§9.7.4.2 R7), and a party applying a witness policy over a recognized-independent set resolves them for itself (§9.7.4.2 R6).** Where a row says the root cannot be recovered by key material and no witness policy applies, the person establishes a new identity — declaring the old one abandoned (§9.7.4.2 R4) — and each context's admins remove the old identity and admit the new one. The social recovery of `03-identity.md` §3.3 does not apply to those rows, because §3.3 re-establishes custody of the same identity. Rank numbers refer to §9.7.4.2 R6: rank 1 is a suffix that reveals the shared prefix's standing commitment, rank 2 one that does not.
...
- **Pre-rotation backup lost, `#0` compromised.** Both suffixes are rank 2 and the tie is pending (§9.7.4.2 R7), and no party can end it, because no party can reveal the standing commitment. The root cannot be recovered by key material.
- **Pre-rotation key copied, `#0` intact and trusted.** Both the controller and the attacker can reveal the standing commitment, so both suffixes reach rank 1 and the identity is contested. The controller's `CommitmentRollover` carries the standing `#0`'s co-signature as evidence, and the witness set that cosigned the controller's head refuses to cosign a fork anchored behind it; the rollover also fixes a commitment the attacker cannot reveal, so the attacker cannot repeat the attempt on the controller's suffix.
- **Pre-rotation key copied, `#0` lost.** Both parties sign a `RootRecovery{Lost}`; both suffixes are rank 1 and the identity is contested, with no co-signature on either side as evidence.
- **Pre-rotation key taken and the controller's copy lost, `#0` intact.** The attacker reveals and the controller cannot, so the attacker's suffix is rank 1 against the controller's rank 2 and the attacker wins uncontested: relying parties see a valid, uncontested root change. Item 3a exists to keep this case from arising through operational-custody compromise; no key-material mitigation exists once the key is exclusively the attacker's.
- **`#0` compromised, pre-rotation key held only by the controller.** The controller signs a `RootRecovery` — `CoSigns` while it still holds `#0`, `Lost` otherwise. Its suffix is rank 1 against the attacker's rank 2, so it wins wherever the attacker forked, and the recovery's state snapshot (§9.7.4.2 R8) replaces every install the attacker made.
- **`#0` compromised and pre-rotation key copied.** The attacker can reveal too, so both suffixes are rank 1 and the identity is contested. The controller's co-signature by `#0`, where it still holds it, is evidence and not a win, because the attacker holds that same root and can co-sign its own fork.
- **`#0` compromised and pre-rotation key exclusively taken.** The attacker reveals and the controller cannot: rank 1 against rank 2, an uncontested attacker win. The root cannot be recovered by key material.
- **`#0` compromised, owner recovered, and a backup of the spent pre-rotation key later reaches the `#0` thief.** The owner signed a `RootRecovery` at some position r that revealed P_prev and installed K′; the thief, who holds the superseded root K0 by the recovery's own premise, later obtains a copy of P_prev — a Shamir share set, a paper mnemonic, a cloud-store snapshot, or a copy taken before r — and forks before r revealing P_prev. Both suffixes reveal the shared prefix's standing commitment, so both are rank 1 and the identity is contested at any later date; the thief's co-signature by K0 wins it nothing, because K0 is the root the owner's recovery declared lost. Destruction that reaches every backup medium (item 4's destructibility axis, §9.7.4.2 R5) is what prevents the contest from arising, and a party's witness policy is what resolves it once it has.
- **A backup of a spent pre-rotation key reaches an attacker who does not hold the superseded root.** The attacker forks before the event that spent the key and reveals its commitment: both suffixes are rank 1 and the identity is contested, on the same terms as the row above. §9.7.4.2 R5 destruction and item 5f destroy, and item 5d verifies, only the controller's own copies.
- **Two devices both holding `#0`, no attacker.** Neither suffix reveals the standing commitment, so both are rank 2 and the tie is pending (§9.7.4.2 R7). The controller ends the tie by signing a `CommitmentRollover` or a `RootRecovery{CoSigns}` on the suffix it keeps (§9.7.4.2 R10).
...
This section governs a **non-delegated** identifier. For a delegated identifier the delegator's chain carries the seal that authorizes the delegate's establishment events, so precedence for a delegated chain reads the delegator's chain; ADR-064, cooperative delegation, specifies SCP's delegation model, and this section states no delegated rule beyond that reading.
...
- The **key state** of an identity is: the standing `#0`; every operational key by role (`#active`, `#agent`) and by custody-tier declaration; every key that ever appeared on the chain as a root or an operational key, each with a condition from the vocabulary of §9.7.1 (`Superseded`, or `Compromised{from:M}`); the **witness set** — the relays the controller designates to cosign the identity's log head — and their cosigning parameters; and the **relay list** — the relays that serve the identity's log — together with the designation of the operational key that signs the identity's service record.
...
- A **seal** is a digest an event carries that anchors external content at that event's position in the log. A seal gives the content it anchors a verifiable position in the log's order, which is what lets a verifier compare that content against a compromise boundary (§9.7.1). A `KeyState` event carries seals.
...
**R3 — Four event kinds; each kind fixes its signature set; every signature MUST verify.** A verifier MUST derive an event's kind from the type discriminator in its preimage, never from which signatures it carries. The kinds are `Inception`, `KeyState`, `CommitmentRollover`, and `RootRecovery`; these four names, and the condition names of §9.7.1, are the protocol's identifiers, and every SDK binding carries them unchanged as its variant names. The set of discriminator values is not closed until the planned-root-move question of `03-identity.md` §3.2.1 is decided.
...
| `KeyState` | the standing `#0` | carries a complete key state and any seals; changes no root and no commitment |
...
A `RootRecovery` carries a required field `standing_root` with exactly two values, `CoSigns` and `Lost`, bound in the preimage. The kind together with that field fixes which signatures the event carries, so a `RootRecovery{CoSigns}` whose `#0` signature is missing or fails to verify is a copy-level defect — a required signature that does not verify — and never a `RootRecovery{Lost}`: the value in the preimage, not the presence of a signature, says which signatures the event names. **The `CoSigns` co-signature is evidence, not rank.** It says that the party who produced the event also held the `#0` standing at that point, which is a fact a relying party's witness policy weighs (R6); it does not decide a divergence, because in every divergence that matters the attacker holds that same root by the recovery's own premise. A `RootRecovery` also carries P's public key, K′'s public key with K′'s signature over the event as proof of possession, the complete post-recovery key state (definitions), and the next commitment or an abandonment declaration (R4); P is never installed as a key, and no `#0` is ever the preimage of any commitment, because K′ is generated fresh and the commitment names a separate key. A `CommitmentRollover` carries no key state: it changes the commitment and nothing else, so one operation has one shape.

**Every signature the kind names MUST verify against the key the kind names for it**: P's signature against the revealed public key, the `#0` signature against the `#0` standing at the event's predecessor, and K′'s signature against the root the event installs. A signature that does not verify is absent. **An additional valid signature by a key the kind does not name is an endorsement, not a defect**: a verifier ignores it for validity and for rank, and MAY read it as third-party evidence. An earlier rule counted any surplus signature as a defect, which let any relay invalidate an intact event by appending one signature. Two classes of invalidity remain, and a verifier MUST treat them differently. A **copy-level defect** — a signature the kind names absent or failing to verify, or a corrupted byte — is producible by any relay with no key, so the verifier discards that copy, treats the position as unserved under R11 (`Inconclusive`), and never demotes the suffix or records anything against the identity; an intact copy from another source supersedes the mutilated one, and events are identified by preimage digest for that purpose. An **author-attributable defect** — a revealed key whose commitment does not equal the standing commitment at the predecessor, or a violation of R4's invariants — is signed content, so the verifier rejects the chain at that event (`Invalid{AuthorAttributable}`, R14). Authorization is the signature by P's private key and never the reveal alone: the revealed public key is public the instant the event appears, so a matching value without the signature proves nothing about who produced the event. Rationale for two reveal-authorized kinds: a controller whose pre-rotation key was copied while `#0` stays intact and trusted needs the rollover; a controller whose `#0` is compromised needs the recovery, co-signing with `#0` when it still holds it so that the co-signature stands as evidence.

**R4 — Every reveal-authorized event fixes the next commitment or declares the identity abandoned, under two closed invariants.** A reveal-authorized event either fixes the next pre-rotation commitment, in which case the controller MUST generate that key in a substrate satisfying §9.7.4.1 item 3a before it signs the event (R10 states when), or carries an explicit **abandonment declaration** in place of a commitment. An event that carries neither is invalid and a verifier MUST reject it. **Abandonment is terminal:** after an event that declares abandonment, no further key event on that chain is valid, and a verifier MUST reject any event whose predecessor chain contains one. The abandoning event is state-carrying or not according to its kind, and the derived key state after it is the state the latest state-carrying event at or before it carries; content signed before the abandonment verifies against that final state exactly as before, because abandonment ends the identity's ability to produce events and revokes nothing retroactively. The failure-mode table of §9.7.4.1 depends on this: the rows that direct a person to establish a new identity presuppose that the old one can be declared dead rather than left indefinitely live. Two invariants hold over the whole chain, and a verifier MUST reject the event that breaks either: **key uniqueness** — a public key appears on a chain in at most one event and in one role (an installed root, an installed operational key, or a revealed P); and **commitment freshness** — a fixed commitment equals no commitment fixed earlier on the chain and equals the commitment of no key that appears on the chain. Rationale: a chain whose standing commitment has been consumed and neither replaced nor declared abandoned has no recovery path and no terminal state; a key that reappears or a commitment that repeats would let a spent or stolen key satisfy a later reveal.

**R5 — The controller destroys the spent pre-rotation key.** After the controller has retained the signed event (R10's retention invariant), the controller MUST destroy P's private key from every custody location and backup medium it controls, for a `RootRecovery` and a `CommitmentRollover` alike, and MUST NOT destroy it before that retention. R5 reaches only the controller's own copies: a Shamir share held by a contact, a paper mnemonic already photographed, and a cloud key store's historical snapshot are outside its reach (§9.7.4.1 item 4's destructibility axis), so a controller using a non-destructible medium cannot fully comply and MUST be warned at custody selection (§9.7.4.1 item 4). Rationale: a spent key that survives can produce a second reveal of a commitment already consumed, which under R6 makes the identity contested at that fork rather than resolvable.

**R6 — Fork precedence over key material, and the slot a witness policy fills.** When a verifier holds two chains that diverge from a shared prefix, it MUST rank them as follows. Let C be the standing commitment at the shared prefix. On each divergent suffix, the verifier locates the first event that reveals C's preimage; at most one such event exists per suffix, because R1 makes C unique on the shared prefix, a reveal consumes it, and R4 forbids fixing it again. The verifier ranks each suffix:

1. the suffix carries an event that reveals C;
2. the suffix carries no event that reveals C.

A rank-1 suffix beats a rank-2 suffix, and the winning chain supersedes the losing chain in its entirety. **Two rank-1 suffixes do not resolve: the identity is contested (R7), and no property of either event breaks the tie.** In particular the `standing_root: CoSigns` co-signature does not break it. This replaces an earlier rule that ranked a co-signed reveal above a reveal alone, which was unsound: a `RootRecovery` leaves its superseded root in the attacker's hands by its own premise, so the attacker can co-sign with the very key the owner's recovery declared lost, and in the rows where the owner's root was gone the rule adopted the attacker's chain outright. Contested is the honest verdict, because over key material alone the two reveals are indistinguishable: both parties held the same pre-rotation key. A verifier MUST NOT decide a divergence by head sequence, by the first event after the shared prefix, or by the highest rank found anywhere on a suffix. When a verifier holds more than two chains, it ranks every pair against the standing commitment of that pair's own shared prefix; the winner is the chain that wins every pairwise comparison it is party to, and the identity is contested when no such chain exists (R7).

- *Pin A.* Rank reads one property of a suffix — whether it carries an event revealing the shared prefix's standing commitment — and no signature beyond the one that authorizes that event. The `#0` co-signature a `CommitmentRollover` or a `RootRecovery{CoSigns}` carries is evidence a witness policy weighs, never a rank.
...
**This rule reads key material only, and key material cannot resolve a contest between two reveals of one commitment.** The witness layer is the axis that can: a witness that cosigned the owner's head refuses to cosign a fork anchored behind it under its consistency rule, and a `standing_root: CoSigns` signature is further evidence that a policy weighs. **A party applying a witness policy over a set it recognizes as independent of the `#0` holder MAY, on a contested divergence, adopt the suffix that set has cosigned; a party applying no such policy holds the key-material verdict, which is contested.** Detection by the witness layer is the mandatory base of that layer; a witness gate is a relying party's opt-in policy and never a condition of an identity's resolvability. The witness protocol itself is specified separately in this corpus.
...
**R7 — Ties and the contested verdict.** Equal rank resolves no winner over key material, and the identity is **contested**. The tie's class is a function of the tied rank: a tie at rank 1 is **terminal by key material** — C is consumed on both suffixes, so no key event on either suffix changes the comparison; a tie at rank 2 is **pending** — neither suffix has revealed C, and a later C-revealing event on either suffix decides the divergence under R6. A party's witness policy MAY resolve a tie of either class for that party (R6's slot). For a contested identity a verifier MUST return the `Contested` verdict (R14), which carries the tied rank and the digests of every tied head; MUST NOT adopt any tied suffix as the current key state; MUST NOT return the shared prefix's key state as the current key state; and MUST retain every tied head and suffix as evidence. Criterion for what a tie proves: a rank-1 tie proves that the pre-rotation private key signed two divergent events; a rank-2 tie proves that `#0` signed two divergent suffixes. Indicator, not criterion: two parties holding the key is the usual cause of a double signature, and one holder with two devices also produces one. When R6 later resolves a divergence that a verifier had recorded as a rank-2 tie, the verifier MUST retire that equivocation record and MUST NOT count it against the identity. Rationale for refusing the shared prefix's key state: if `#0` was compromised before the fork point, the shared prefix already carries the attacker's events, so serving its key state as current would serve a hostile key state.

**R8 — Key state is the latest state-carrying event; nothing is voided; a controller may not repudiate what a relying party already accepted.** The derived key state at any position IS the key state the latest state-carrying event at or before that position carries; no earlier event contributes, and a verifier applies no diff, no undo, and no voiding. A `RootRecovery` therefore replaces the entire key state in one event: every operational key the attacker installed, every condition the attacker asserted, every witness-set and relay-list change the attacker made, is absent from the recovery's snapshot and so is not current after it. **The default is deny (§9.7.1):** a key that appears on the chain and is absent from the latest state-carrying event has no condition, and a verifier rejects every content signature by it; the controller lists its own historical keys as `Superseded`, and a key it forgets to list has its content rejected — the fail-safe direction, corrected by a later state event. The recovery's operational-key set replaces the derived set in full: an operational role the snapshot does not name is not current after the recovery.

**The repudiation floor.** Each `Compromised{from:M}` entry carries its own boundary M. M MUST NOT be earlier than the position of any content the identity's relying parties have already accepted as accountable — content the chain itself anchors by a seal at or before that position (definitions, §9.7.1). A verifier MUST reject a state-carrying event whose entry names an M above the event's own sequence, and MUST reject a `Compromised{from:M}` entry that would invalidate content the chain seals at or before M. A controller recovers control of a compromised key; it does not thereby unmake what its relying parties already accepted. Rationale: without the floor a dishonest controller names an M far in its own past and repudiates every commitment it has made since, which is a harm to relying parties that the compromise rule was never meant to enable. M remains a content boundary only (§9.7.1): an M set too high leaves the attacker's content under that key accepted, and an M set too low is rejected where it crosses the floor and otherwise rejects the controller's own content.

A `CommitmentRollover` carries no key state and changes the derived state in no way. Chain validity under R1–R4 is evaluated over the full chain with every event present. Rationale for the snapshot: an attacker holding `#0` who appends `KeyState` events to the one genuine log produces no divergence, so R6 never runs; a rule that voided the attacker's events by boundary needed an expanding list of exemptions and re-opened for each of them the window it was written to close, and one recovery can express `#agent` compromised at 30 and `#0` at 50, which a single boundary could not. A state-carrying event costs O(keys) bytes — a few kilobytes on a log whose key events are rare — and the relay list becoming key state is a consequence of the snapshot: the DID document of the identity spec is a projection of the latest state-carrying event, not a separately signed artifact.
...
| has lost `#0` | `RootRecovery{Lost}` | 1 |
| is retiring the identity permanently | a reveal-authorized event declaring abandonment (R4) | 1 |

Every reveal-authorized event reaches rank 1, so the choice among the first three rows is not a choice of rank: it is a choice of what the event does to the key state and what evidence it carries. The SDK MUST treat any reveal-authorized event on its identity's chain that it did not sign, and any `#0`-signed event it did not author, as evidence of compromise and alert at maximum severity; and it MUST be able to extend a suffix the controller authored even after its own verifier adopted or contested a competing chain, because the controller's answer to a hostile chain is an event on its own suffix. A controller SHOULD roll over on a schedule, because a standing copy of P lets its holder contest the identity at any later date.
...
1. **A `#0` thief forks early; the owner's device holding K0 is gone.** The owner's chain is e0 through e9 (K0-signed `KeyState` events). The thief, holding K0, forks at e5 and appends a5 through a50, all K0-signed. The owner no longer holds K0, so it signs a `RootRecovery{Lost}` at e10 that reveals C with P's signature, installs a fresh root K′, carries a complete key state, and fixes C′. The shared prefix is e0 through e4 and its standing commitment is C. The owner's suffix reveals C, so its rank is 1; the thief's suffix reveals nothing, so its rank is 2. The owner's chain wins under R6 wherever e10 sits, and the thief's head sequence 50 decides nothing. Under R8 the derived key state after e10 is e10's snapshot, so nothing the thief installed survives.
2. **A copied pre-rotation key, thief forks early.** The owner's chain is e0 through e60 with K0 intact and trusted. The thief holds a copy of P, forks at e10 with a `RootRecovery{Lost}` a11, and appends to a50. The owner signs a `CommitmentRollover` at e61 that reveals C and carries K0's signature. The shared prefix is e0 through e10. Both suffixes reveal C, so both are rank 1 and the identity is **contested** over key material (R7, terminal by key material): the owner and the thief held the same P, and no property of either event distinguishes them by key material alone. The owner's co-signature by K0 is evidence a relying party's witness policy weighs (R6's slot), and the witness set that cosigned the owner's head e60 refuses to cosign a fork anchored at e10 under its consistency rule. The rollover still fixes a commitment the thief cannot reveal, so the thief cannot repeat the attempt on the owner's suffix.
3. **`#0` compromised and the pre-rotation key copied; the owner still holds K0.** The owner's chain is e0 through e30, and a thief holding both K0 and a copy of P forks at e12 with a `RootRecovery{Lost}` a13. The owner signs a `RootRecovery{CoSigns}` at e31 that reveals C, installs K′, and carries K0's signature. Both suffixes reveal C: the identity is contested (R7). The co-signature does not break the tie, because the thief holds K0 too and could co-sign its own fork. A witness policy resolves it for the party that applies one; a party applying none holds the contested verdict.
4. **Two `#0`-only suffixes.** The owner's two devices both hold K0. Device A appends a `KeyState` e10a and device B appends a different `KeyState` e10b. The shared prefix is e0 through e9. Neither suffix reveals C, so both are rank 2 and the tie is pending (R7). The owner, holding P, signs a `CommitmentRollover` e11a on device A's suffix; that suffix is now rank 1 against rank 2, and it wins. A thief holding K0 alone could not have ended the tie. An owner who has lost P faces a permanent rank-2 tie, and the root cannot be recovered by key material.
5. **A backup of a spent key later reaches the `#0` thief.** The owner's chain carries a `RootRecovery{Lost}` r20 that reveals an earlier commitment C_prev over P_prev and installs K′, because K0 was stolen and the owner's device holding it was gone; the chain continues to e40. The thief, who holds K0 by r20's own premise, later obtains a backup of P_prev — a Shamir share set the owner's contacts still hold, a paper mnemonic, or a cloud-store snapshot — and forks at e19 with a `CommitmentRollover{P_prev, K0}` a20. The shared prefix is e0 through e19, and its standing commitment is C_prev. Both suffixes reveal C_prev, so both are rank 1 and the identity is **contested**, at any later date. The thief's co-signature by K0 does not win it the chain, because K0 is the root r20 declared lost. Over key material neither chain is adopted; a party applying a witness policy over a recognized-independent set adopts the suffix that set cosigned — a witness that cosigned e40 refuses a fork anchored at e19 — and a party applying none holds the contested verdict. Destruction that reaches every backup medium (§9.7.4.1 item 4's destructibility axis, R5) is what prevents the contest from arising.
...
- **Pre-rotation key copied, `#0` intact and trusted:** The controller signs a `CommitmentRollover` (§9.7.4.2 R3) immediately. The attacker can reveal the same commitment, so the identity is contested over key material (§9.7.4.2 R7) and a party's witness policy resolves it; the rollover fixes a commitment the attacker cannot reveal, so the attempt cannot be repeated on the controller's suffix. No operational key changes, so steps 2–6 do not run.
- **Pre-rotation key taken and the controller's copy lost, `#0` intact:** The attacker's `RootRecovery{Lost}` wins uncontested over the controller's chain, which reveals nothing (§9.7.4.2 R6). The root cannot be recovered by key material.
- **`#0` compromised with the pre-rotation key copied:** both parties can reveal, so the identity is contested (§9.7.4.2 R7). **`#0` compromised with the pre-rotation key exclusively taken:** the attacker's `RootRecovery` wins uncontested (§9.7.4.2 R6). In every row where the root cannot be recovered by key material, a party's witness policy MAY still adopt the controller's suffix (§9.7.4.2 R6's slot); otherwise the person establishes a new identity — declaring the old one abandoned (§9.7.4.2 R4) — each context's admins remove the old identity and admit the new one, and §3.3's social recovery does not apply, because it re-establishes custody of the same identity.
