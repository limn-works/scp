...
| Agent co-signature requirement | Empty list | A list of capabilities; §4.9.3 states no further bound | `ContextParams::agent_cosign_required`. Capabilities a member exercising them through `#agent` must additionally have signed with `#active`. Story SCP-AB-031 adds it. | §4.9.3 |
