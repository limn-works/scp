...
**A relay charges a PUBLISH at `scp:did:` or `scp:svc:` where it declares `per_publish`, and it charges one on no other declaration.** **It charges no `scp:wit:` and no `scp:wcf:` write** (§9.7.4.3), which are the other two retained kinds, and no party pays for either.
