...
- **For every identifier, whatever its method:** the SDK records the identifier with the digest of its root-installing event on first contact, and applies §9.11's key-change rule to every `RootRecovery` and every unauthored operational-key rotation it later observes for that identifier. TOFU is not a did:web-only measure.
...
**Key conditions, the content boundary, and the relying party's obligation.** The latest state-carrying event of an identity's key-event log (§9.7.4.2 definitions, R8) lists every key that has appeared on the chain as a root member or an operational key, and each listed key is in exactly one of three conditions: **current** — listed in a role; **`Superseded`** — listed as a historical key; or **`Compromised{from:M}`** — listed with an asserted boundary M, a position in the key-event log at or before the sequence of the event that carries the entry. A key that an event the controller did not author installed, and that the latest state-carrying event omits, has no condition, and a verifier MUST reject every signature by it. A state-carrying event that omits a key a controller-authored event installed is malformed (§9.7.4.2 R8). `Superseded` is derived: a key becomes `Superseded` when a state-carrying event lists it outside every role. The only assertion a controller makes about a key is `Compromised{from:M}`, and it withdraws a false alarm by a later state-carrying event that lists the key as `Superseded` or in a current role. An entry identifies its key by the public key bytes; the verification-method fragment a signature's `signing_key_id` names (§9.8.1) resolves to a key through the latest state-carrying event, so a verifier locates a `Superseded` key by that lookup and never from a current document alone.

**Where the content boundary lives.** Content signed by an operational key is committed inside a context through MLS, and the compromise recovery protocol (§9.12 step 2) MUST issue an MLS Update — a Commit, an epoch boundary every member of that context observes identically — in every context the identity belongs to. **That Commit's epoch is the identity's compromise boundary in that context**, and abandonment (§9.7.4.2 R4) fixes a boundary the same way. A `Compromised{from:M}` entry names the log position M; the Commit epoch names where that assertion took effect in each context, and content is ordered against it by the epoch under which it was encrypted, which every member reads from the envelope. No further ordering structure is needed: the epoch is already in every envelope and every member derives the same epoch sequence, so the boundary converges by the same mechanism that makes MLS work.

**The relying party's obligation, stated over evidence it holds.** For a key listed `Superseded` or `Compromised{from:M}`, a relying party accepts a signature **iff** it accepted that content through MLS, with a valid `membership_tag` (§9.8.1 check 2), in an epoch before that context's boundary; content it first sees after the boundary, or under a later epoch, it MUST reject whatever the key's condition. A party that cannot place content against the boundary — because it did not observe the boundary Commit — treats content it accepted before it observed the recovery as before-boundary, and rejects the rest. This bounds `Superseded` and `Compromised` alike: a retired key is not a live signing capability, and a compromise entry never unmakes what the party already accepted. The controller's key event is never rejected on account of content. A party with no membership in a context holds no evidence and verifies no content of that context; that is the cost of a boundary that lives where the content lives.
...
|---|---|---|
| **Attestation** — current key only | verifies against the identity's current key for that role (check 1 above); a `Superseded` or `Compromised` key's signature never verifies | `SCP-KEYPACKAGE-ATTESTATION-V1:`, `SCP-ATTESTATION-V1:`, `SCP-PARTICIPATION-V1:`, `SCP-PARTICIPATION-PROFILE-V1:`, `SCP-KEY-REQUEST-V1:`, `SCP-ACCESS-KEY-REQUEST-V1:`, `SCP-EPOCH-ADVANCE-V1:`, `SCP-BLOCK-NOTIFICATION-V1:`, `SCP-CHALLENGE-REQ-V1:`, `SCP-CHALLENGE-RESP-V1:`, `SCP-CHALLENGE-VERIFY-V1:`, `SCP-BRIDGE-REGISTER-V1:`, `SCP-PUSH-REGISTER-V1:`, `SCP-PUSH-DEREGISTER-V1:`, `SCP-INVITATION-BUNDLE-V1:`, `SCP-JOIN-RESPONSE-V1:`, UCAN tokens, and every separator this table does not list |
| **Content** — accepted before the boundary | the relying-party obligation above: verifies under a current key, and under a `Superseded` or `Compromised{from:M}` key only for content the party accepted through MLS before that context's boundary | `SCP-INNER-ENVELOPE-V1:`, `SCP-BROADCAST-ENVELOPE-V1:`, `SCP-VOTE-V1:`, `SCP-PROPOSAL-V1:`, `SCP-RECEIPT-V1:`, `SCP-XCTX-RECEIPT-V1:`, `SCP-XCTX-STREAM-RECEIPT-V1:`, `SCP-XCTX-DIVERGENCE-V1:`, `SCP-OUTLET-CHUNK-SIG-V1:`, `SCP-OUTLET-CREDIT-V1:`, `SCP-OUTLET-CANCEL-V1:`, `SCP-RESET-REQUEST-V1:`, `SCP-COMMIT-RANGE-REQ-V1:`, `SCP-COMMIT-RANGE-RESP-V1:`, `SCP-CONTEXT-SNAPSHOT-V1:`, `SCP-CONTEXT-EXPORT-V1:` |
| **Key event** — the log's own rules | §9.7.4.2 R3 | `SCP-KEL-EVENT-V1:` |

Governance votes and proposals are content: a vote cast under a key that is later marked compromised stays counted for the members who accepted it before the boundary, because the alternative — every vote in a context's history rejected the moment a key is asserted compromised — would let a compromise assertion rewrite governance outcomes. A key-continuity fingerprint (§9.11) and a DID-record frame are not signed by an operational key and are outside this table. Rationale for the default: a structure nobody classified is safer read as current-only than as content, because the content class admits historical keys.

**Two verdicts the resolution policy must consume.** The resolution-failure policy of check 2 has, for each trigger, a branch for resolution failure and a branch for a resolution that returns a rotated key. Two verdicts of §9.7.4.2 R14 are neither, and the policy MUST treat them as follows. `Inconclusive` is the **failure** branch: an Add rejects, and an already-admitted member's Update falls to the bounded last-known-good grace. `Contested` is its own branch: no new Add of that identity is admitted, no UCAN is issued to it, and no sender key is distributed to it while the contest stands; its existing leaves are not evicted by the verdict alone (§9.12's leaf re-verification step is what evicts an attacker's leaf); and content verification for that identity uses the shared prefix's key state as its basis for content the context's boundary places before the fork, and verifies nothing after it (§9.7.4.2 R7). A `RootRecovery` that is `Adopted`, and a `Contested` identity, both fire §9.11's key-change rule for every party that observes them.

**A content-signature freshness bound.** Content signed by a newly installed operational key can reach a verifier before the verifier has the `KeyState` that installed it, and §9.8.1 rejects an unresolvable signature rather than holding it, so the content is lost. A verifier MUST therefore refresh the signer's key-event log before rejecting a content signature whose `signing_key_id` it cannot resolve, and MUST treat a key-event log older than `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (§9.18.7) as stale for that purpose — the same bound check 2 places on attestation resolution.
...
- Active Signing Key: Rotated by a `KeyState` event signed by the standing root (§9.7.4.2 R3) that lists the new `#active` current and the old one `Superseded`. All active MLS groups receive an Update proposal with the new credential. The identifier does not change. A peer's SDK that did not author the event treats it as a key change under §9.11.
...
- **P copied, root intact and trusted.** Both parties can reveal: contested. The controller's `CommitmentRollover` carries the root's co-signature as evidence, and it fixes a commitment the attacker cannot reveal, so the attempt cannot be repeated on the controller's suffix.
- **P copied, root lost.** Both parties sign a `RootRecovery{Lost}`: contested, with no co-signature on either side.
- **P exclusively the attacker's, whatever the state of the root.** The attacker reveals and the controller cannot: the attacker's suffix is rank 1 against rank 2 and the attacker wins uncontested — relying parties see a valid, uncontested root change. The attacker cannot abandon the identity, because abandonment requires the standing root's co-signature (§9.7.4.2 R4); with the root also compromised, it can. Item 3a exists to keep this case from arising through operational-custody compromise; no key-material mitigation exists once the key is exclusively the attacker's.
- **Root compromised, P held only by the controller.** The controller signs a `RootRecovery` — `CoSigns` while it still holds the root, `Lost` otherwise. Its suffix is rank 1 against the attacker's rank 2, so it wins wherever the attacker forked; the recovery's snapshot (§9.7.4.2 R8) replaces every install the attacker made, and its K′-signed head has its own store slot (R9), so the attacker cannot evict it.
- **Root compromised and P copied.** Both parties can reveal: contested. The controller's co-signature, where it still holds the root, is evidence and not a win, because the attacker holds that same root.
...
This section governs a **non-delegated** identifier. For a delegated identifier the delegator's chain anchors the delegate's establishment events by a key-event seal (definitions), so precedence for a delegated chain reads the delegator's chain; the delegation model is specified elsewhere in this corpus, and until it is, a verifier MUST reject a chain that claims delegation (`Invalid{AuthorAttributable}`).

**Definitions.**

- The **root** of an identity is a **root set** — a list of public keys — with a **signing threshold** t. A **root signature** is a set of signatures over one preimage by at least t distinct members of the root set. Every statement below that says a root "signs" means a root signature. A personal identity is the 1-of-1 case: one key, threshold one, and every rule below reads exactly as it would for a single key. An organization sets a list and a threshold so that no one officer acts alone. The **standing root** at a position in a chain is the root set and threshold the latest root-installing event at or before that position installed: the inception event installs the first, and a `RootRecovery` installs each later one.
...
- The **next set** is the list of pre-rotation public keys whose commitments the chain fixes (below), with a **next threshold** n. A **reveal** is the disclosure of at least n members of the committed next set, each signing the revealing event. The **pre-rotation key** P names one member of that set; each is a one-shot authorizer that signs exactly one reveal-authorized event and is then spent and destroyed (R5), and never becomes a root member or any other key. The name is retained because §9.7.4.1 item 3a, ADR-054, RFC #2130, and the shipped custody trait carry it; this definition, not the name, governs.
- The **key state** of an identity is: the standing root; every operational key by role (`#active`, `#agent`) and by custody-tier declaration; **every key that has ever appeared on the chain** as a root member or an operational key, each with a condition from §9.7.1 (`Superseded`, or `Compromised{from:M}`); the **witness set** — the relays the controller designates to cosign the identity's log head — and their cosigning parameters; and the **relay list** — the relays that serve the identity's log — together with the designation of the operational key that signs the identity's service record.
...
- A **reveal-authorized event** carries a reveal against the standing commitment at the event's predecessor: at least n revealed keys whose commitments are members of the standing list, each signing the event.
...
- Every key event's signed preimage binds the identifier, the event's sequence, the digest of its predecessor event, the event-type discriminator, and every field a rule in this section reads (the revealed keys, the `standing_root` field, the installed root set and threshold, the key-state snapshot, any key-event seal, the abandonment declaration, and the next commitment list and threshold, where the kind carries them). Every signature an event carries is over that one identical preimage. In the inception event the identifier field and the predecessor-digest field are the all-zero 32-byte placeholder (R13). Rationale: without predecessor binding, a party holding a root could copy the owner's genuine revealed `RootRecovery` bytes onto its own fork at the same sequence and present them as its own; without the declaration in the preimage, a relay could strip the next commitment and substitute abandonment with every named signature still verifying.
...
**The signature set closes by construction.** Each kind — and for a `RootRecovery`, the kind together with its `standing_root` value — fixes the number, order, and role of the signature slots the event carries, and each slot is one 64-byte Ed25519 signature; the signature field's length is therefore a function of the kind and the declared set sizes, and any other bytes in it are a copy-level defect. An event carries no signature its kind does not name. Endorsements by third parties and cosignatures by witnesses are **separate signed objects over the event's preimage digest**, never carried on the event copy; the witness layer names the channel they travel. Rationale: a signature carried on the copy can be stripped or stuffed by any relay with no detectable defect, so evidence that rides the copy is neither reliably present nor reliably absent, and an unbounded signature field is a verification-cost amplifier; a separate signed object over the digest is unstrippable without trace and unstuffable.

A `RootRecovery` carries a required field `standing_root` with exactly two values, `CoSigns` and `Lost`, bound in the preimage. The value declares which set the event carries, so a `RootRecovery{CoSigns}` whose root signature is missing or fails to verify is a copy-level defect and never a `RootRecovery{Lost}`; the declaration in the preimage, not the presence of a signature, says which signatures the event names. The SDK derives the value from whether it holds a threshold-satisficing subset of the standing root and MUST refuse, with a typed error, to sign `standing_root: Lost` while it does. The `CoSigns` signature is evidence a party's witness policy reads (R6, Pin A); it is not a rank. A `RootRecovery` also carries the revealed keys, the installed root set K′ and its threshold with a root signature by K′ as proof of possession, the complete post-recovery key state (definitions), and the next commitment (R4); no revealed key is ever installed, and no root member is ever the preimage of any commitment, because K′ is generated fresh and the commitment names separate keys. A `CommitmentRollover` carries no key state: it changes the commitment, or declares abandonment, and nothing else.

**Every signature the kind names MUST verify against the key the kind names for it**: each revealed key's signature against that revealed public key, each standing-root member's signature against that member, and each installed-root member's signature against the installed set. A signature that does not verify is absent. Two classes of invalidity follow. A **copy-level defect** — a named signature absent or failing to verify, a corrupted byte, or extra bytes in the signature field — is producible by any relay with no key, so the verifier discards that copy, treats the position as unserved under R11 (`Inconclusive`), and never demotes the suffix or records anything against the identity; an intact copy from another source supersedes the mutilated one, and events are identified by preimage digest for that purpose. An **author-attributable defect** — a revealed key whose commitment is not a member of the standing list, fewer revealed keys than the next threshold, or a violation of any R4 obligation — is signed content, so the verifier rejects the chain at that event (`Invalid{AuthorAttributable}`, R14). Authorization is the signature by the revealed private keys and never the reveal alone: a revealed public key is public the instant the event appears.

**R4 — Every reveal-authorized event fixes the next commitment or declares abandonment, under two closed invariants.** A reveal-authorized event either fixes the next commitment list and next threshold — the controller MUST generate those keys in a substrate satisfying §9.7.4.1 item 3a before it signs (R10 states when) — or carries an explicit **abandonment declaration** in place of a commitment. **Only a `CommitmentRollover` co-signed by the standing root may declare abandonment**: the party retiring an identity holds its root, and a controller whose root is lost recovers first (a `RootRecovery{Lost}` installs K′) and abandons under a second reveal co-signed by K′. A reveal alone cannot abandon. An event that fixes no commitment and declares no abandonment is invalid. **Abandonment is terminal:** after an event that declares abandonment, no further key event on that chain is valid, and a verifier MUST reject any event whose predecessor chain contains one. The derived key state after abandonment is the state the latest state-carrying event before it carries, and the abandoning event changes nothing in it — abandonment retires the identity and does not rewrite its history. Content signed before abandonment verifies against that final state under §9.7.1; the abandonment fixes each context's boundary (§9.7.1) so that a key the final state lists current verifies nothing signed after it. A relying party MUST NOT act socially on an abandonment — remove the identity from a context, revoke standing — until it has confirmed the abandoning head from a source the abandoning chain's own snapshot does not name. The failure-mode table of §9.7.4.1 depends on abandonment: the rows that direct a person to establish a new identity presuppose that the old one can be declared dead. Two invariants hold over the whole chain, and a verifier MUST reject the event that breaks either: **key uniqueness** — a public key appears on a chain in at most one event and in one role (a root member, an operational key, or a revealed key); and **commitment freshness** — a fixed commitment equals no commitment fixed earlier on the chain and equals the commitment of no key that appears on the chain. Rationale: a chain whose standing commitment has been consumed and neither replaced nor declared abandoned has no recovery path and no terminal state; a key that reappears or a commitment that repeats would let a spent or stolen key satisfy a later reveal; and abandonment authorized by a reveal alone would let anyone who ever obtained a copy of one pre-rotation key kill a live identity whose root is intact.

**R5 — The controller destroys the spent pre-rotation keys.** After the controller has retained the signed event (R10's retention invariant), the controller MUST destroy every revealed private key from every custody location and backup medium it controls, for a `RootRecovery` and a `CommitmentRollover` alike, and MUST NOT destroy them before that retention. R5 reaches only the controller's own copies: a Shamir share held by a contact, a paper mnemonic already photographed, and a cloud key store's historical snapshot are outside its reach (§9.7.4.1 item 4's destructibility axis), so a controller using a non-destructible medium cannot fully comply and MUST be warned at custody selection. Rationale: a spent key that survives can join a second reveal of a commitment already consumed, which under R6 makes the identity contested at that fork.
...
**R7 — Ties and the contested verdict.** Equal rank resolves no winner over key material, and the identity is **contested**. The tie's class is a function of the tied rank: a tie at rank 1 is **terminal by key material** — C is consumed on both suffixes, so no key event on either suffix changes the comparison; a tie at rank 2 is **pending** — neither suffix has revealed C, and a later reveal on one suffix decides the divergence under R6, while a later reveal on both escalates the tie to rank 1 and terminal. A party's witness policy MAY resolve a tie of either class for that party (R6's slot). For a contested identity a verifier MUST return the `Contested` verdict (R14), which carries the tie class and the digests of every tied head; MUST NOT adopt any tied suffix as the **current** key state; and MUST retain every tied head and suffix as evidence. The verifier MAY use the shared prefix's key state — the state both suffixes agree on — as the verification basis for content the relevant context's boundary (§9.7.1) places before the fork, and for nothing after it; it MUST NOT serve that state as current. Criterion for what a tie proves: a rank-1 tie proves that a threshold of the next set signed two divergent events; a rank-2 tie proves that the standing root signed two divergent suffixes. Indicator, not criterion: two parties holding the keys is the usual cause, and one holder with two devices also produces one. When R6 later resolves a divergence that a verifier had recorded as a rank-2 tie, or the tie escalates to rank 1, the verifier MUST retire the rank-2 equivocation record and MUST NOT count it against the identity. Rationale for refusing the shared prefix's state as current: if the root was compromised before the fork point, the shared prefix already carries the attacker's events.

**R8 — Key state is the latest state-carrying event; nothing is voided; omission of the controller's own keys is malformed.** The derived key state at any position IS the key state the latest state-carrying event at or before that position carries; no earlier event contributes, and a verifier applies no diff, no undo, and no voiding. A `RootRecovery` therefore replaces the entire key state in one event: every operational key an attacker installed, every condition an attacker asserted, every witness-set and relay-list change an attacker made, is absent from the recovery's snapshot and so is not current after it. Two rules govern absence, and they differ by who installed the key. **A key that an event the controller did not author installed, and that the snapshot omits, has no condition, and a verifier rejects every content signature by it** — the default is deny for attacker-installed keys. **A key that a controller-authored event installed MUST appear in every later state-carrying event with a condition; a state-carrying event that omits one is malformed, and a verifier MUST reject it** (`Invalid{AuthorAttributable}`) — the controller lists its own historical keys as `Superseded` or `Compromised{from:M}`, and R10's enumeration step is how it comes to know them. The recovery's operational-key set replaces the derived set in full: an operational role the snapshot does not name is not current after the recovery. Each `Compromised{from:M}` entry carries its own boundary M, a position in this log, bounded M ≤ the sequence of the event that carries it; a verifier MUST reject an entry whose M exceeds that. M is the log-side half of the content boundary; §9.7.1 states the context-side half and the relying party's obligation, and no rule in this section rejects a controller's event on account of content. A `CommitmentRollover` carries no key state and changes the derived state in no way; the derived state after an adopted rollover is the state of the latest state-carrying event before it, which after a divergence may be an event on the shared prefix. Chain validity under R1–R4 is evaluated over the full chain with every event present. Rationale: an attacker holding the root who appends `KeyState` events to the one genuine log produces no divergence, so R6 never runs, and only a full replacement of the state removes what it installed; a rule that voided by boundary needed an expanding list of exemptions; and omission of the controller's own keys would let a controller repudiate its own history by silence, which the malformed rule forbids. A state-carrying event costs O(keys) bytes, a few kilobytes on a log whose key events are rare, and the relay list becoming key state means the identity spec's DID document is a projection of the latest state-carrying event.

**R9 — Retention of evidence, the store, and the retention bound.** A verifier MUST NOT bound the suffix it retains at its last reveal-authorized event, because a party holding a spent pre-rotation key can anchor a fork before that event (R5). **A validating relay keeps one highest-sequence slot per (routing id, standing root set)**, not one per identity: a chain whose head is signed under a different standing root than another chain's head occupies a different slot, so an attacker holding a superseded root cannot evict the owner's recovered head by out-appending it, and every verifier receives both heads and runs R6. Slot count is bounded by the number of `RootRecovery` events, which is small. A verifier MUST retain at most **eight** divergent suffixes per identity; a candidate that would exceed that bound is `Inconclusive`. Rationale for eight: two devices plus several attacker forks fit with margin, and more concurrent forks than that is itself the signature of an attack rather than a state a verifier must hold; the bound caps the pairwise comparison a flood would otherwise drive quadratic. Indicator, not criterion: key-event logs are short, so a verifier that retains its full accepted chain satisfies R11's evidence obligation with no fetch.
...
| is retiring the identity permanently and holds its root | `CommitmentRollover` declaring abandonment (R4) |

A reveal-authorized event puts its suffix at rank 1 against any divergence whose shared prefix's standing commitment it reveals (Pin B); the choice among rows is a choice of what the event does to the key state and what evidence it carries. The SDK MUST treat any reveal-authorized event on its identity's chain that it did not sign, and any root-signed event it did not author, as evidence of compromise and alert at maximum severity; it MUST be able to extend a suffix the controller authored even after its own verifier adopted or contested a competing chain; and when its own identity is contested it MUST tell the human which suffix a witness policy would resolve for and keep extending its own. A controller SHOULD roll over on suspicion, not on a schedule: every rollover mints a spent key whose later leak contests the identity, so a schedule multiplies that exposure with no threat-driven gain.

*Self-observation.* The SDK MUST fetch its own identity's log at a stated cadence — no less often than the witnessing interval, and on every launch — from at least one source its own relay list does not name (the fallback set), and MUST treat an inability to complete that check as an alert, not as silence.

*Before composing a snapshot.* The controller MUST hold its chain from the inception event, obtained from at least one source outside its own relay list; MUST enumerate every key that a controller-authored event installed, and every custody-tier declaration, so that the snapshot lists each one (R8); MUST diff the chain's current relay list and witness set against the inception's or against a pre-compromise record it holds and surface every entry introduced at or after the asserted M; and MUST refuse to sign, with a typed error, if that chain is `Inconclusive` or `Contested` at any position it needs.

*Device boundary.* A reveal-authorized event MUST be produced on a device, and its successor keys — K′, the operational keys the snapshot installs, and the next set — generated in custody, that the controller has established the asserted compromise did not reach. When the compromise reached a platform account (a passkey or cloud-keystore account, a device-management enrolment, a restore image), the controller MUST create a fresh platform account and MUST NOT restore any state from the compromised one before proceeding; a controller that cannot establish the boundary by that or another stated means MUST NOT proceed. The controller MUST confirm, before the reveal signs, that the event carries the commitments of the keys it just backed up and the K′ it generated — on a device with a display that is not the assembling device, or by comparing a displayed digest of the event against one the pre-rotation substrate computes, because a hardware token with no display signs whatever it is shown and the assembling device is the one the compromise may have reached. The SDK's `RootRecoveryConfig` names the fresh custody as a required field with no default and no fallback to the daily provider.

*Ceremony order.* Enumerate (above); generate the next set under §9.7.4.1 items 1, 3a, 4, and 5b–5d; for a `RootRecovery`, generate K′ and the operational keys under the device boundary; sign — each pre-rotation key signs where it resides and is never imported into operational custody; persist the recovery handle (below); publish; retain; destroy; submit to the witness set. **Retention is the invariant that makes destruction safe:** the controller MUST durably retain the byte-identical signed event as the head of its own chain BEFORE it destroys the spent keys (R5). **Confirmed publication** is durable self-retention together with acceptance of the event by at least one relay **outside the snapshot's own relay list** — a relay in the fallback set that no snapshot on any suffix names, and that an attacker's `KeyState` therefore could not have introduced — and the controller MAY publish to any additional relay at any time. The witness set the controller submits to is the set the event's snapshot names; witness submission never gates destruction, because an identity's resolvability never depends on a witness (the witness layer detects; a witness gate is a relying party's opt-in policy).

*The recovery handle.* The controller MUST durably persist a typed recovery handle BEFORE it publishes the event, and phase 1 begins only after that persist. The handle is a tagged union with one variant per phase — signed but published to no relay; published but the spent keys not yet destroyed; destroyed but not yet submitted to the witness set — and each variant carries only what its resume consumes: the byte-identical signed event in every phase, the spent keys' custody handles in the first two, and, for an event that fixes a commitment, the next set's custody handles and the expected commitment list so a resume re-checks it; an abandoning event's handle carries no next-set field. Exactly one named accessor loads a pending handle after a restart, and exactly one resume entry point consumes the handle by value and continues from its phase without re-signing.

**R11 — Divergence is a positive finding.** A candidate head is **on the accepted chain** when either chain is a prefix of the other. A candidate head is **divergent** when the verifier holds both chains back to a common ancestor and they diverge (definitions). A candidate that is neither — because the events linking it to the accepted chain were not served, because a served copy failed verification as a copy-level defect (R3), or because retaining it would exceed R9's bound — is **inconclusive**: the verifier holds its baseline, records nothing, and returns the `Inconclusive` verdict (R14), which names the candidate head, the accepted head, the missing sequence range, and every source that failed to serve the link; before returning it the verifier MUST have tried at least one source that the candidate's own chain does not name. A verifier MUST record equivocation evidence only from two chains it holds in full back to their common ancestor, and only when R6 resolves them as a tie (R7). Rationale: a single relay that withholds linking events, or serves a mutilated copy, could otherwise cause a verifier to record a durable verdict against an honest identity.

**R12 — Sequence governs the accepted chain only.** For a candidate head on the accepted chain, a verifier MUST reject a head at a sequence strictly lower than the highest it has accepted (`RejectedStale`, R14), and an equal sequence with an identical head confirms the baseline (`Confirmed`). A candidate whose chain is neither a prefix of the accepted chain nor extended by it is classified under R11 and decided under R6 and R7. A divergent candidate's head sequence decides nothing. Adopting an R6 winner MAY lower the accepted head sequence (`Adopted{baseline_decreased}`), and a verifier's stored baseline MUST be able to express that decrease. Rationale: a monotone maximum cannot record the adoption of a lower-sequence winner.

**R13 — Domain separators and the identifier's construction.** The key-event signature preimage uses `"SCP-KEL-EVENT-V1:"` under the §9.5.1 construction, and the first field after the separator is the event-type discriminator byte, because a verifier derives the kind before it parses the fields the kind carries. The pre-rotation commitment uses `"SCP-PREROTATION-COMMITMENT-V1:"` as a commitment-hash domain, and the key-event seal uses `"SCP-KEL-SEAL-V1:"`. The **identifier** is `SHA-256("SCP-KEL-ID-V1:" || inception_signed_preimage)`, where `inception_signed_preimage` is the inception event's complete signed preimage with its identifier and predecessor-digest fields set to the all-zero placeholder (definitions); a verifier recomputes it under R2. The identifier separator is distinct from the event separator so that no signature preimage's digest is ever an identifier and no identifier is ever a digest a key signed. All four separators are registered in §9.18.2. The root is a key list with a threshold from the first version, so that an organization and a person incept under one schema; the inception preimage's shape is frozen by the identifier construction, and a shape absent at inception cannot be added to an existing identity.

**R14 — The verdict taxonomy.** A resolution returns exactly one of the following, and every rule above cites its variant; every SDK binding carries the six names unchanged. `Confirmed` — the candidate is the accepted head (R12). `Adopted{head, baseline_decreased}` — the verifier adopted a new head, on the accepted chain or by R6, and the flag says whether the accepted sequence decreased (R12). `RejectedStale` — a head on the accepted chain at a sequence strictly lower than the accepted head's (R12). `Contested{tie: TerminalByKeyMaterial | Pending, heads}` — equal rank under R6, with the tie class and the digests of every tied head (R7). `Inconclusive{candidate, accepted, missing_range, failed_sources}` — the candidate could be neither placed on the accepted chain nor shown divergent (R11). `Invalid{at_event}` — the chain fails R1–R4 at a named event by an author-attributable defect (R3); a copy-level defect never produces this verdict. The contested verdict, the retained suffixes it rests on, and the accepted-head baseline MUST survive the verifier's restart and MUST be persisted before any verdict is acted on; a persist failure returns an error and not the verdict.
...
1. **A root thief forks early; the owner's device holding R0 is gone.** The owner's chain is e0 through e9 (R0-signed `KeyState` events). The thief, holding R0, forks at e5 and appends a5 through a50, all R0-signed. The owner no longer holds R0, so it signs a `RootRecovery{Lost}` at e10 that reveals C, installs a fresh root K′, carries a complete key state, and fixes C′. The shared prefix is e0 through e4 and its standing commitment is C. The owner's suffix reveals C, so its rank is 1; the thief's suffix reveals nothing, so its rank is 2. The owner's chain wins under R6 wherever e10 sits, and the thief's head sequence 50 decides nothing. Under R8 the derived key state after e10 is e10's snapshot, so nothing the thief installed survives; under R9 the owner's K′-signed head occupies its own store slot, so the thief cannot evict it by appending.
2. **A copied pre-rotation key, the thief forks early.** The owner's chain is e0 through e60 with R0 intact and trusted. The thief holds a copy of P, forks at e10 with a `RootRecovery{Lost}` a11, and appends to a50. The owner signs a `CommitmentRollover` at e61 that reveals C and carries R0's signature. Both suffixes reveal C, so both are rank 1 and the identity is contested, terminal by key material (R7); the owner's R0 co-signature is evidence for a witness policy, and the witness set that cosigned e60 refuses a fork anchored at e10. The same verdict follows when the thief also holds R0 and the owner signs a `RootRecovery{CoSigns}` instead: the co-signature does not break the tie, because the thief holds that root too.
...
- The SDK records, on first encounter, the identifier together with the digest of the event that installed its standing root (TOFU); the record is keyed to that pair, not to the identifier alone.
- **A `RootRecovery` on a peer's chain — whether the verifier `Adopted` it or the identity is `Contested` — IS a key change under this rule and is never a legitimate-change exemption.** On observing one the SDK MUST: (a) alert the user at maximum severity that the identity's root has changed; (b) invalidate every recorded continuity verification for that identifier; (c) refuse to send encrypted content to that identifier, issue a UCAN to it, admit it to a context, or distribute a sender key to it until the user completes re-verification or explicitly accepts the change. An uncontested `RootRecovery` is exactly the attacker's outcome when the pre-rotation key was exclusively taken (§9.7.4.1's failure-mode table), so a verdict of `Adopted` exempts nothing.
- **Any rotation of an operational key that the local SDK did not itself author is a key change under this rule on the same terms.** A `KeyState` signed by the standing root is not privileged as legitimate: a thief holding the root produces exactly such an event, and the honest owner's later `RootRecovery` is the one that would otherwise alarm. The trigger is the event kind and its author, not "a different public key."
- A `KeyState` the local SDK authored, and a `RootRecovery` the local SDK signed, are the two changes that fire no alert on the device that made them; every other device and every peer treats them as above.
- If fingerprints do not match on re-verification, a MITM is actively intercepting resolution or the identity has been taken over. The SDK MUST alert with maximum severity and MUST keep the refusals of (c) in force.
- For social recovery, trusted contacts independently confirm the change out of band before accepting it.

The gates that enforce this rule: §9.7.1's Update trigger (a replaced leaf after a root change is admitted only after re-consent); §9.16.2's key response (no sender key to an identifier under (c)); §9.6.4's first-contact bootstrapping (the TOFU record above applies to every identifier, not to did:web alone); and the standing-pair consent gate and the known-identity allowlist of the contexts spec, which MUST re-verify under this rule on a peer's `RootRecovery` because a stable identifier keeps matching an allowlist entry that a rename used to invalidate.
...
- **Root compromise (rare, severe), with or without operational-key compromise:** The controller signs a `RootRecovery` (§9.7.4.2 R3) with the pre-rotation key from independent custody — `standing_root: CoSigns` while it still holds a threshold of its root, `Lost` otherwise (§9.7.4.2 R10). The event installs a fresh root K′ generated under R10's device boundary and carries the complete post-recovery key state (§9.7.4.2 R8): every operational key by role — a fresh key for any key the compromised device also held, with the replaced key listed `Compromised{from:M}`, and the controller's own key re-listed otherwise; every historical key listed `Superseded`; the witness set; the relay list, diffed against the pre-compromise list per R10. The recovery supersedes any chain the attacker's root signed, wherever the attacker forked (§9.7.4.2 R6). The identifier does not change: no new identity is created and no forwarding record exists. Steps 1a–6 then run under the operational keys the snapshot names. **What the snapshot replaces is key state and nothing else.** It does not remove an MLS leaf the attacker added under the stolen `#active` in any context (step 1a covers it); it does not withdraw a KeyPackage the attacker published (the controller cannot enumerate them; they expire, and step 4 replaces the controller's own); it does not revoke a UCAN the attacker issued (step 3 covers the controller's tokens; a token the attacker issued is revoked as the relying party discovers it under §9.11's refusal, with no protocol enumeration); it does not undo auto-accept or contact-graph standing the attacker earned on peers (§9.11's key-change rule invalidates the standing at each peer); it does not reach private-state events written under a stolen PSK (§3.7's re-key); and it does not withdraw an attestation the attacker issued (attestation verification is current-key-only, so the replaced key's attestations fail from the recovery on).
- **Operational-key compromise (agent or active key), root intact:** the `KeyState` that rotates the key MUST list the replaced key `Compromised{from:M}`, with M the sequence of that `KeyState`, so that §9.7.1's boundary applies to content under it; listing it `Superseded` would leave the attacker's copy verifying as a historical key.
- **Pre-rotation key copied, root intact and trusted:** The controller signs a `CommitmentRollover` (§9.7.4.2 R3) immediately. The attacker can reveal the same commitment, so the identity is contested over key material (§9.7.4.2 R7) and a party's witness policy resolves it; the rollover fixes a commitment the attacker cannot reveal. No operational key changes, so steps 1a–6 do not run.
- **Pre-rotation key copied, root lost:** The controller signs a `RootRecovery{Lost}`; the attacker can too, and the identity is contested. Steps 1a–6 run under the controller's suffix for the parties that adopt it.
...
**1a. Leaf re-verification after a root recovery.** Every member of a context who resolves a `RootRecovery` for a fellow member — `Adopted` or `Contested` — MUST re-verify every leaf in that group carrying that identity against the post-recovery key state and MUST propose Remove for any leaf whose attesting `signing_key_id` resolves to a key the snapshot does not list current; a PCS Update heals a leaked leaf secret and does not evict a member, so this step is the only thing that removes a leaf the attacker added. The recovering controller's own SDK MUST enumerate every leaf under its identity in every context it can reach and propose Remove for each one whose attestation it did not sign. Where the attacker is the sole admin of a context, no member can issue the Remove and the context is re-created (§5.9); that outcome is stated, not hidden.

**2. MLS Update in all active contexts.** Issue MLS Update proposals in every context. This Commit is the identity's compromise boundary in that context (§9.7.1). This provides post-compromise security: new epoch keys are derived from the new key material, making the compromised old key useless for future messages. If the old key is unavailable (device stolen), a trusted co-member with admin role must remove and re-add the member.
...
3. **Key response.** The key holder's SDK processes the request: verifies the signature, checks the block list, and checks that the requester's identifier is not under §9.11's refusal (an unaccepted `RootRecovery` or unauthored rotation, or a `Contested` identity) — a sender key is never distributed to an identifier under that refusal. If the requester is not blocked, responds with `SenderKeyResponse { sender_did, epoch, hpke_sealed_key, ephemeral_pubkey, request_nonce }` via an MLS application message with `recipient_hint` to the requester. The sender key is sealed using HPKE Base mode (RFC 9180) to the requester's ephemeral wrapping public key. If blocked, no response — the blocked party cannot obtain the key. The `ephemeral_pubkey` field carries the HPKE encapsulated key (`enc` in RFC 9180 terminology) and `hpke_sealed_key` carries the AEAD ciphertext (`ct`).
