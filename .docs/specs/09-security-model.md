...
**One record form, and the SCP relay network carries it.** A key-event record carries the log itself: the signed `value` of an identity's relay record carries the chain from the inception event to the latest state-carrying event, or — when the resolver already holds a state-carrying event on that chain — the segment from that event forward (§9.7.4.2 R9 states how a relay serves a chain too long for one frame). A publisher that cannot tell what a resolver holds publishes from inception, which is the form every first-contact resolution reads. The protocol runs no second publication or resolution layer beside the relay network, and it uses no distributed hash table.
...
**The record signature is the write mechanism, never the resolver's trust decision.** A key-event record carries an Ed25519 signature by the operational key the key state designates for the service record, over `SHA-256("SCP-KEY-EVENT-RECORD-V1:" || identifier || value)` under the §9.5.1 construction — the 32-byte identifier raw and `value` under the variable-length rule — with the separator registered in §9.18.2. That signature serves one purpose: it lets a relay decide which write to keep at a routing ID (§9.10.12, §3.10.2). A resolver's trust decision reads the log under steps 2 through 4 above and never the record signature.
...
**Stale record prevention:** the frame carries no sequence number, and a relay keeps the chain that extends the chain it holds (§9.10.12). The client discards, without changing its accepted state, a candidate whose chain is a head of the accepted chain at a sequence strictly lower than the highest it has accepted (§9.7.4.2 R12). Two key-event chains that diverge from a shared prefix are not comparable by sequence, and a client settles them by the fork-precedence rule of §9.7.4.2 R6, which may adopt a chain whose head sequence is lower than the head the client previously held (§9.7.4.2 R12).
...
| **Log-anchored evidence** — anchored to a key-state position | the artifact carries the digest of the signer's key-state head at the moment it signed, inside the signed preimage (§9.5.2); the signature verifies **iff** the signing key was `current` at that position, and, where the entry is `Compromised{from: N}`, only under the further test the paragraph below states. A key the state lists `Superseded` or `Retired` keeps its signature valid for this class, because the artifact names when it was made | `SCP-KEY-DESTRUCTION-V1:`, `SCP-CONTEXT-SNAPSHOT-V1:`, `SCP-CONTEXT-EXPORT-V1:` |
...
Governance votes and proposals are content: a vote cast under a key that is later marked compromised stays counted for the members who accepted it before the boundary, because the alternative — every vote in a context's history rejected the moment a key is asserted compromised — would let a compromise assertion rewrite governance outcomes. The key-event record frame's record signature under `SCP-KEY-EVENT-RECORD-V1:` (§9.10.12) is outside this table and takes no default class from it, because no relying party reads it as evidence. It is the relay's write rule, made by the operational key the key state designates for the service record and checked by a relay against the designated key of the chain the frame carries, and a resolver authenticates the record by the key-event log instead (§9.6.1). A key-continuity fingerprint (§9.11) is a hash, not a signature, and is outside this table. Rationale for the default: a structure nobody classified is safer read as current-only than as content, because the content class admits historical keys.
...
- The **fallback set** for an identity is the set of relays the identity's own service record (`03-identity.md` §3.10.13) does not name. The SDK draws a fallback relay from the bootstrap relay list of `18-addressability-and-deployment.md` §18.5.1, taking any entry of that list the identity's service record does not name. This definition is the one home of the fallback set, and every rule below cites it. **The set can be empty**, because an identity whose service record names every bootstrap relay leaves none outside it; the consequence is fail-closed and stated where each rule reads the set — R11 returns `Inconclusive{SingleSource}` and R10's ceremony refuses to sign. A controller reads its own chain from the fallback set (R10) and a verifier tries a fallback source before it returns `Inconclusive` (R11), because a source an attacker's `KeyState` could have introduced is a source the attacker chose.
...
**Storage model — a raw relay blob.** A key-event record is stored as a **raw relay blob** at its routing ID. It is **NOT** wrapped in an `OuterEnvelope`, and its bytes are **NOT** MLS-encrypted — it is a public, self-certifying record whose authenticity comes from the key-event log its `value` carries (§9.6.1), not from encryption and not from its record signature. Publication and resolution use the existing PUBLISH/QUERY operations (ADR-004) verbatim — no new wire types. `ClientMessage::Publish { routing_id, blob_ttl, blob }` writes the frame bytes at the identity's routing ID, and `RelayMessage::Blob { …, blob }` returns them.

**Relay-side validation (SCP-native relays, OPTIONAL).** Unlike an opaque context blob, a key-event record is a public, self-certifying record an SCP-native relay MAY validate on PUBLISH. The whole path sits behind the existing per-IP PUBLISH rate limit (ADR-004) and runs **cheapest-first**, so junk is rejected before any Ed25519 verify. A validating relay:

1. **Structurally decodes** the blob as a key-event record frame. A blob that does not decode is not a candidate key-event record (it is governed by slot-exclusivity, below).
2. **Confirms the identifier→routing_id binding**: `routing_id` equals `SHA-256("scp:did:" || identifier_bytes)` over the 32 raw digest bytes the frame's `identifier` field carries — the derivation §9.7.4.2 R13 states. The identifier encodes no key (R13), so no key is derived from it and no field of the frame is hashed to the routing ID. This binding is the discriminant: a validating relay recognizes a key-event record by the frame decoding *and* the binding holding — it needs no new wire type and no a-priori knowledge of `routing_id` semantics, and a context `OuterEnvelope` (whose first byte is a MessagePack map marker, never `version = 1`) is never mistaken for one. Being a plain hash, it runs **before** any signature verify.
...
4. **Verifies the record signature** over `SHA-256("SCP-KEY-EVENT-RECORD-V1:" || identifier || value)` against the **designated service key of the chain the `value` itself carries**, in every case — the operational key that chain's latest state-carrying event designates for the service record (§9.7.4.2 R3 rejects a state-carrying event whose designation names a key it does not list `current`). Step 3 has already verified that chain from its inception event, so the key is one the relay read out of a verified chain and never one the writer supplied; reading it out of the stored slot's chain instead would fail every genuine first frame after a `#active` rotation, whose new designated key the stored chain does not yet name. The signature is the write rule and never a resolver's trust input (§9.6.1).
5. **Places the frame's chain in a slot** under §9.7.4.2 R9, which states the slot key, what opens a slot, what a slot holds, and the eviction rule; this section applies R9 and restates none of it. The relay compares the frame's chain against each stored chain at that routing id and, where the two are prefix-related, writes to that slot under one test and no other: **the frame's chain must extend the stored chain.** A frame whose chain the stored chain extends is rejected, and a frame whose chain is byte-identical to the stored one is an idempotent TTL refresh the relay accepts. The rule reads no sequence number: sequence is position on a chain (§9.7.4.2 definitions), so an extending chain has the strictly higher head sequence by construction and a separate comparison would only restate the extension test in a weaker form a writer could supply. A QUERY returns every slot at the routing id, because a resolver runs the fork-precedence rule (§9.7.4.2 R6) over the chains it is served and a relay that returned one slot would decide that rule for it.

**Slot-exclusivity.** The base relay stores multiple opaque blobs per `routing_id` with no per-`routing_id` cap (ADR-004), so validating only frame-shaped blobs would leave non-frame or pre-seeded junk sitting alongside the slot and returnable by QUERY. A validating relay therefore makes a `routing_id` **slot-exclusive** once a binding-valid frame first establishes a slot there: (a) it rejects any later PUBLISH there that is not a frame passing steps 1–5 above; (b) it evicts any pre-existing opaque blobs at that `routing_id` when the first slot is established; (c) QUERY there returns the slots and nothing else — every slot the routing id holds, one per divergent suffix; (d) it **rejects a DELETE of any slot blob** — only a PUBLISH whose chain extends the slot's chain may replace it, and only R9's rank eviction may remove one. Rule (d) is **storage-derived, not index-derived**: because DELETE addresses a blob by `blob_id` and the in-memory slot index is a cache that a relay restart or a store-sharing peer leaves cold, the relay decides rule (d) by re-reading the blob at `blob_id` and rejecting the DELETE if it decodes as a frame whose chain verifies from inception under R2 and R3 (the chain is self-certifying, so protection is reconstructible from the bare bytes) — immune to a cold index. The DELETE gate runs behind the per-IP rate limit and fails closed on a storage read error (§3.10.8 "Attacker DELETEs the slot blob"). Before the first valid frame the relay cannot recognize the `routing_id` as identity-domain (`SHA-256` is one-way), so pre-seeded junk simply sits as ordinary opaque blobs until the first valid publish establishes the slot and evicts it (§3.10.2). This is a relay **storage** behavior; the relay-storage mechanics are transcribed in the companion **ADR-004 "DID-Record Slot-Exclusivity" subsection** (implemented under #482 / SCP-RELAYRES-003), whose own text still names the superseded sequence rule and the superseded frame name and which Track U4's ADR sweep reconciles. This section and §3.10.2/§3.10.8 are authoritative for the behavior and its threat model.
...
```
KEY-EVENT-RECORD (KeyEventRecordV1) :=
  ...
  value:            [u8]                     # trailing remainder = signed payload = one contiguous segment of the identity's key-event log (§9.6.1, §9.7.4.2 R9)
```
...
2. **Require the fixed prefix in full.** A decoder MUST reject any frame shorter than the 97-byte fixed prefix (`version + identifier + signature`) — truncation is never a partially-valid frame.
3. **Bound-check the `value` length only after the prefix check.** `value` is the trailing remainder, `len(value) = total_frame_len − 97`. A decoder MUST NOT compute `total_frame_len − 97` before rule 2 has confirmed `total_frame_len >= 97`: computing it first can underflow, which diverges between a debug-build panic and a release-build wrap across bindings. With rule 2 satisfied the subtraction cannot underflow. A decoder MUST reject an empty `value` (`total_frame_len == 97`) and MUST reject `len(value) > Max blob size (262144, §9.18.11) − 97`. No widening is required: `len(value)` is an actual buffer length (a valid `usize`, already bounded by the transport's Max blob size) and the bound is a compile-time constant — there is no wire-supplied length field to overflow (unlike a length-prefixed layout).
...
- **Publish** = wrap `(identifier, signature, value)` in a key-event record frame and PUBLISH the frame bytes at the identity's routing ID via the existing PUBLISH operation (§3.10.5). The relay blob MUST be the key-event record frame — not the bare key-event log bytes, and not an `OuterEnvelope`.
...
- **Distinct from the outer envelope.** An identity's routing ID carries key-event record frames, not `OuterEnvelope`s; a resolver MUST NOT deserialize one as the other. The two are disjoint at two independent levels: **(a) routing-ID space** — key-event records occupy `SHA-256("scp:did:" || identifier_bytes)` (§9.7.4.2 R13), disjoint from context routing IDs; **(b) byte level** — a key-event record frame begins with `version = 0x01`, whereas an `OuterEnvelope` is MessagePack-serialized as a map (`rmp_serde::to_vec_named`) whose first byte is always a map marker (fixmap `0x80`–`0x8f`, or `0xde`/`0xdf` for larger maps) — never `0x01`. A key-event record frame therefore can never be mistaken, byte-for-byte, for an `OuterEnvelope`, or vice versa — a defense-in-depth backstop beneath the routing-ID separation.

**The frame never enters the signed bytes.** The frame wraps `value` for relay transport only; it never reorders or alters the bytes the record signature covers, so a relay verifies the same preimage the publisher signed. `03-identity.md` §3.10.4's rule that two valid records serving heads of one chain at one sequence MUST be byte-identical is applied to two copies of one record; the sequence it compares is the chain's own, read out of the verified `value`, never a field of the frame.
...
| `"SCP-KEY-EVENT-RECORD-V1:"` | Key-event record signature preimage — `SHA-256("SCP-KEY-EVENT-RECORD-V1:" \|\| identifier \|\| value)`, signed by the operational key the key state designates for the service record; it is a relay's write rule and never a resolver's trust input | §9.6.1, §9.10.12 |
...
| Key-event record frame version | 1 | Current key-event record relay frame version (§9.10.12); bumped on any field-encoding change. The frame has no magic tag and no record-kind byte — the identity's `routing_id` domain is the type discriminant. Unsigned framing — grants no authority. See also §9.18.11 Transport and Relay for the shared blob-size/TTL bounds the frame reuses. | §9.10.12 |
| Key-event record frame fixed prefix | 97 | Fixed-width prefix length in bytes (`version` 1 + `identifier` 32 + `signature` 64); the frame carries no sequence field, and `value` is the trailing remainder, `total_frame_len = 97 + len(value)` | §9.10.12 |
