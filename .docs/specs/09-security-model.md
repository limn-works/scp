...
- The SDK MUST keep the `ContinuityStanding` of §9.11 for the pair (identifier, root-installing event), and MUST set it `PendingReverify` on any `RootRecovery` and on any rotation of `#active` or `#agent` it did not author, exactly as for every other identifier; did:web adds no per-verification-method pin of its own.
...
- **For every identifier, whatever its method:** the SDK records a `ContinuityStanding` for the pair (identifier, root-installing event) on first contact (§9.11) and sets it `PendingReverify` on every `RootRecovery` and every unauthored operational-key rotation it later observes. TOFU is not a did:web-only measure.
...
3a. **Recovery-authority residence (normative).** Item 3's storage-isolation requirement constrains *where the recovery authority resides*, not merely which provider object holds the key handle. The **recovery authority** is the minimal secret, key handle, or authorization capability sufficient to recover the pre-rotation private key. It MUST reside in a substrate whose compromise is **independent** of operational-custody compromise: there MUST NOT exist any secret, key handle, or authorization capability reachable **(directly or transitively, following any key-wrapping or key-derivation chain to its root)** from the operational `KeyCustody` provider or the daily-operations authentication flow that — alone or in combination with publicly-stored artifacts (including the encrypted-offline ciphertext of item 4) — suffices to recover the pre-rotation private key. Wrapping the key in an approved cipher (item 4) does NOT by itself satisfy this requirement: the §3 adversary is one who has already compromised operational custody, and if the decryption authority for that cipher is itself reachable from operational custody, the cipher provides zero protection against that adversary. The load-bearing property is **principal-distinctness of the recovery authority**, not the strength of the cipher wrapped around the key.
   ...
   **A reveal is not a daily operation.** A reveal-authorized event (§9.7.4.2) is a rare, separately-authorized event, not a daily operation. A distinct reveal-time authorization principal — even one that is fully automated (e.g., a separate KMS role assumed only for a rollover or recovery, which the operational path holds no grant to assume) — satisfies this requirement. What does NOT satisfy it is the daily operational signing flow itself being able to reach the pre-rotation key.
   ...
   The fail-closed-no-fallback rule above and the recovery-authority-residence property are canonical and normative here. What this section does not settle is the per-profile conformance *realization* of this rule — the per-profile floors, which of the item-4 methods conform on which profile, and the profile-specific selection ceremony of item 5. Items 4 (approved custody methods) and 5 (SDK presentation) below are canonical; only their per-profile conformance filtering is unsettled, and when it is settled it MUST rank post-spend destructibility, item 4's primary axis, as a conformance axis (§9.7.4.2 R5).
...
4. **Approved custody methods.** Under the key-event log a copy of a pre-rotation key alone does more lasting damage than a copy of the root alone: a root leak is recoverable by a reveal, while a leaked pre-rotation key lets its holder contest the identity at any later date and, held exclusively, take it over and then kill it permanently (the failure-mode table below). The pre-rotation key is therefore the backstop against root compromise and at the same time the key whose leak is least recoverable, and its custody is ranked on two axes with **post-spend destructibility** as the primary one: whether the controller can destroy every copy of a spent key (§9.7.4.2 R5) and, where the substrate supports it, obtain the §9.15 attested destruction; and resistance to theft at rest. Any one method is sufficient **subject to the recovery-authority-residence requirement of item 3a**. Every method MUST hold an Ed25519 key that signs the raw §9.5.1 preimage of a key event (§9.7.4.2 R3). A WebAuthn/FIDO2 authenticator signs only `authenticatorData || SHA-256(clientDataJSON)` under its own counter, and Apple's Secure Enclave and Android StrongBox hold P-256 keys only, so none of those substrates can produce the signature R3 requires, and this table names none of them.
   ...
   The SDK MUST warn at custody selection that a method marked not destructible leaves the spent-key rows of the failure-mode table below open for every reveal the key later authorizes, and that the witness layer is the only other mitigation (§9.7.4.2 R6).
...
- A **reveal-authorized event** carries a reveal against the standing commitment at the event's predecessor: each revealed key's commitment MUST be the member of the standing list at the index the preimage names.
...
2. **Confirms the identifier→routing_id binding**: the frame names the identifier, and `routing_id` equals the registered routing derivation over it — the derivation over an inception-derived identifier is fixed in a later revision of this section, because it waits on the identifier's byte layout; the identifier encodes no key (§9.7.4.2 R13), so no key is derived from it. A frame whose `public_key` does not hash to the `routing_id` it is published at is rejected. This binding is the discriminant: a validating relay recognizes a DID record by the frame decoding *and* the binding holding — it needs no new wire type and no a-priori knowledge of `routing_id` semantics, and a context `OuterEnvelope` (whose first byte is a MessagePack map marker, never `version = 1`, and whose bytes do not hash a `public_key` to a context pseudonym) is never mistaken for one. Being a plain hash, it runs **before** the signature verify.
...
- **Query** = the existing QUERY operation (§3.10.4) with `limit: N` (N = 16) returns the blob(s) stored at the routing ID; the resolver decodes each at the **single** decode-and-verify site into `(value, signature, seq)` and then verifies the signature against the operational key the identity's latest state-carrying event designates for the service record (§9.7.1: attestation-class, `current` key only) — never against a key derived from the identifier, which encodes none (§9.7.4.2 R13). Against a validating relay the routing ID is slot-exclusive, so one record is returned regardless of `N`; against non-validating or foreign storage `limit: N` lets the resolver retrieve up to N candidates and sift them to the highest-seq *valid* one (§3.10.4 step 5). See §3.10.2 for why `limit: N` dominates `limit: 1`.
...
The formula above is defined for a 1-of-1 root, where `a_identity_key` is the single root member. For a root set of more than one member the fingerprint MUST cover every member key in list order, count-prefixed, and MUST take the identifier rather than a DID string; both changes wait on the byte layout of the identifier, which a later revision of §9.7.4.2 fixes, so an organization's continuity fingerprint is undefined until that revision and the SDK MUST report it as unavailable rather than compute the 1-of-1 form over one member. Every root member and both operational keys are included so that substitution of any single key changes the fingerprint. If a DID has no `#agent` verification method (no agent bound), a domain-derived sentinel `SHA-256("SCP-ABSENT-AGENT-KEY")` (truncated to 32 bytes) is used instead. This avoids collision with the Ed25519 identity point (all zeros) and provides domain separation from legitimate key values. The `"SCP-KEY-CONTINUITY-V1:"` domain separator prevents cross-protocol signature confusion.
...
- The SDK keeps, for every identifier it has encountered, one **`ContinuityStanding`** keyed to the pair (identifier, digest of the event that installed its standing root): `Verified`, or `PendingReverify`. On first encounter the standing is `Verified` by trust-on-first-use, and the pair is recorded.
- **A `RootRecovery` on a peer's chain — whether the verifier `Adopted` it or the identity is `Contested` — IS a key change under this rule and is never a legitimate-change exemption.** On observing one the SDK MUST set the standing to `PendingReverify`, invalidate every recorded continuity verification for that identifier, and alert the user at maximum severity that the identity's root has changed. An uncontested `RootRecovery` is exactly the attacker's outcome when the pre-rotation key was exclusively taken (§9.7.4.1's failure-mode table), so a verdict of `Adopted` exempts nothing.
- **Any rotation of an operational key that the local SDK did not itself author is a key change under this rule on the same terms.** A `KeyState` signed by the standing root is not privileged as legitimate: a thief holding the root produces exactly such an event. The trigger is the event kind and its author, not "a different public key."
- A `KeyState` the local SDK authored, and a `RootRecovery` the local SDK signed, are the two changes that leave the standing `Verified` on the device that made them; every other device and every peer sets `PendingReverify`.
- **One gate reads the standing.** While an identifier's standing is `PendingReverify`, the SDK MUST NOT send encrypted content to it, issue a UCAN to it, admit it to a context, or distribute a sender key to it. The standing returns to `Verified` only when the user completes re-verification against the identity's current root set or explicitly accepts the change. If fingerprints do not match on re-verification, a MITM is intercepting resolution or the identity has been taken over; the SDK MUST alert with maximum severity and the standing stays `PendingReverify`.
- For social recovery, trusted contacts independently confirm the change out of band before accepting it.

The gates that read `ContinuityStanding`: §9.7.1's Update trigger (a replaced leaf after a root change is admitted only when the standing is `Verified`); §9.16.2's key response (no sender key to an identifier whose standing is `PendingReverify`); §9.6.4's first-contact bootstrapping (the TOFU record above applies to every identifier); §9.6.2's did:web mitigations (the same standing, not a per-verification-method pin); and the standing-pair consent gate and the known-identity allowlist of the contexts spec, which MUST read the standing on a peer's `RootRecovery` because a stable identifier keeps matching an allowlist entry that a rename used to invalidate.
...
3. **Key response.** The key holder's SDK processes the request: verifies the signature, checks the block list, and reads the requester's `ContinuityStanding` (§9.11) — a sender key is never distributed to an identifier whose standing is `PendingReverify`. If the requester is not blocked, responds with `SenderKeyResponse { sender_did, epoch, hpke_sealed_key, ephemeral_pubkey, request_nonce }` via an MLS application message with `recipient_hint` to the requester. The sender key is sealed using HPKE Base mode (RFC 9180) to the requester's ephemeral wrapping public key. If blocked, no response — the blocked party cannot obtain the key. The `ephemeral_pubkey` field carries the HPKE encapsulated key (`enc` in RFC 9180 terminology) and `hpke_sealed_key` carries the AEAD ciphertext (`ct`).
...
| `MAX_RETAINED_SUFFIXES` | 8 | Maximum divergent suffixes a verifier retains per identity; a candidate beyond it is `Inconclusive{RetentionBound}` | §9.7.4.2 R9 |
