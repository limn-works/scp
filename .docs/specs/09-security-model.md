...
Note: the `KeyPackageAttestation` binds **all four** of the leaf's own public keys to the member's `did` (field 1), and is signed by the DID verification method named in `signing_key_id` (field 6, `#active` or `#agent` — never a root member). The attestation must vouch for the *whole* leaf, not a subset of its keys — each of the leaf's HPKE public keys is a distinct decryption capability, and any key left unbound is one an attacker who holds only the leaf **signing** key can substitute with a key of their own:
...
| 4 | `key_state_head` | 32 bytes (fixed-size, the digest of the signer's key-state head at signing) |
...
An inception-derived identifier is **self-certifying through its key-event log**: the identifier is the digest of the inception event's signed preimage (§9.7.4.2 R13), and it encodes no key. The DID record a resolver fetches carries that log, and the log is what authenticates the record.

**Two record forms, one per layer.** A relay record carries the log itself: the BEP44-signed `value` of an identity's relay record carries the chain from the inception event to the latest state-carrying event, or — when the resolver already holds a state-carrying event on that chain — the segment from that event forward (§9.7.4.2 R9 states how a relay serves a chain too long for one frame). A publisher that cannot tell what a resolver holds publishes from inception, which is the form every first-contact resolution reads.

A **DHT-form record** carries a signed head pointer instead of the log, because BEP 44 caps a mutable item's value at `MAX_DHT_RECORD_BYTES` (§9.18.17 — 1000 bytes, the external BitTorrent constraint) and one state-carrying event already exceeds that (§3.10.3). The pointer is `identifier ‖ head digest ‖ head sequence ‖ the relay list, or the digest of the relay list where the list does not fit the bound`, BEP44-signed by the operational key the key state designates for the service record. **The two layers therefore carry different bytes, and no rule requires byte-identity across them.** The pointer names a chain; it never substitutes for one, and a resolver adopts no key state from a pointer alone. **Which BEP44 public key addresses the mutable item that holds an identity's pointer is open**, because a BEP44 mutable item is addressed by the publishing key and an inception-derived identifier encodes no key; the identifier byte-layout revision of §9.7.4.2 R13 fixes it. Until it does, the DHT layer is an availability path: it delivers a second source under R11's two-source rule wherever it resolves, and it delivers no eviction resistance of its own.

**How a resolver authenticates a served record.** When resolving an identifier:

1. The client queries the Mainline DHT (§3.10.3) and every relay it knows for the record at the identifier's routing ID. A client that holds no accepted baseline for the identifier obtains the chain from at least two sources under §9.7.4.2 R11's two-source rule, and the DHT head pointer is one such source.
2. The client recomputes the identifier from the served chain's inception event and rejects a chain whose recomputed identifier differs from the identifier it is resolving (§9.7.4.2 R2). The recomputation runs against a chain and never against a head pointer; the client checks a pointer it holds against the chain by comparing the pointer's head digest and head sequence with the chain's head.
...
4. The client derives the key state, and with it the DID document, from the latest state-carrying event of the chain it adopted (§9.7.4.2 R8).

**A pointer no served chain matches is evidence of suppression.** When the client holds a head pointer naming a head that no reachable relay serves, it returns `Inconclusive` (§9.7.4.2 R14) naming the pointer's head and adopts nothing, because the party that can serve a chain shorter than the pointer names is a party suppressing the rest of it.
...
**The BEP44 signature is the write mechanism, never the resolver's trust decision.** The record's BEP44 signature, by the operational key the key state designates for the service record, and its `seq` serve one purpose: they let a relay and a DHT node decide which write to keep at a routing ID (§9.10.12, §3.10.2). A resolver's trust decision reads the log under steps 2 through 4 above and never the BEP44 signature.

**MITM on resolution is impossible given the correct identifier.** A DHT node or a relay cannot serve a fraudulent chain under an identifier it does not control, because step 2 recomputes the identifier from the inception event the chain carries and a different inception event yields a different identifier. Tampering is detectable without trusting any intermediary.

**Stale record prevention:** BEP44 records include a sequence number, and a relay or DHT node keeps the highest it holds. The client discards, without changing its accepted state, a candidate whose chain is a head of the accepted chain at a sequence strictly lower than the highest it has accepted (§9.7.4.2 R12). Two key-event chains that diverge from a shared prefix are not comparable by sequence, and a client settles them by the fork-precedence rule of §9.7.4.2 R6, which may adopt a chain whose head sequence is lower than the head the client previously held (§9.7.4.2 R12).
...
**Relay-stored DID records.** The log-based authentication above applies equally to DID records stored on SCP relays (§3.10.2). A resolver runs steps 2 through 4 on the chain the record carries whether it retrieved the record from the Mainline DHT or from an SCP relay. The storage backend does not affect the trust model.
...
- The SDK MUST pin the TLS certificate of the did:web resolution server. This section is the one home of that obligation, and §9.13 cites it.
- The SDK MUST keep the `ContinuityStanding` of §9.11 for the identifier, and MUST set it `PendingReverify` on any `RootRecovery` and on any rotation of `#active` or `#agent` it did not author, exactly as for every other identifier; did:web adds no per-verification-method pin of its own. A did:web identifier carries no key-event log and so no root-installing event: its **change trigger** is the resolved document's set of verification-method keys, and the SDK MUST set the standing `PendingReverify` when that set differs from the set it recorded, whichever method changed.
- **A did:web identifier's standing exits `PendingReverify` by out-of-band confirmation of the resolved verification-method key set, and never by the §9.11 fingerprint.** §9.11's fingerprint reads a 32-byte inception-derived identifier and a root set, and a did:web identifier has neither, so the SDK MUST NOT derive a substitute for either input: two SDKs inventing two substitutes would compare two different values and raise a maximum-severity alert on every honest pair. The SDK surface that carries the exit displays the resolved verification-method key set — each method's fragment with its raw public key — and takes the user's confirmation that the remote party read the same set aloud; on confirmation the standing returns to `Verified`, and on a mismatch the SDK alerts at maximum severity and the standing stays `PendingReverify`.
...
**For an inception-derived identifier:** the relay list is a field of the key state (§9.7.4.2 definitions), so the latest state-carrying event of the identity's log carries it and a root signature covers it. Substituting a relay list requires a threshold of the identity's standing root.
...
**Attack: relay list substitution.** A compromised DHT node or relay could serve a stale relay list, directing messages to relays the recipient no longer uses. Defense: a client takes the relay list from the latest state-carrying event of the chain it adopted (§9.7.4.2 R6, R8), and it discards a chain that is a head of the accepted chain at a strictly lower sequence without changing its accepted state (§9.7.4.2 R12).
...
**Rollback-resistance assumption.** The ≤ 5-min freshness guarantee (check 2) presupposes **rollback-resistant DID resolution**. An attacker who serves an **OLD but validly-signed, lower-`seq`** chain as if it were a "fresh resolution" would return a **pre-rotation** key that the freshness check cannot catch — the document is genuinely signed, only stale, so it passes as fresh. This is out of scope for the attestation verifier; it is mitigated at the **resolution layer** — the sequence rule of §9.6.1, which rejects a head of the accepted chain at a lower sequence than the accepted head, together with chain verification, which rejects a chain that recomputes to another identifier; and did:web fetch integrity (§9.6.2) — which the attestation verifier **assumes**, rather than re-checking as a separate attestation check.
...
**Key conditions, the content boundary, and the relying party's obligation.** The latest state-carrying event of an identity's key-event log (§9.7.4.2 definitions, R8) lists every key that has appeared on the chain as a root member or an operational key, and each listed key is in exactly one of two conditions: **`current`** — listed in a role; or **`Retired{compromised_from}`** — listed outside every role, carrying an optional position `compromised_from` = M in the key-event log, at or before the sequence of the event that carries the entry. R8 states the obligation to list every installed key exactly once, and a verifier applies R8's text rather than a paraphrase of it. **The criterion the one retired condition states:** the key signs nothing new; content the party accepted before that key's boundary in a context stays valid there; and, when the entry carries M, log-anchored evidence anchored at or after M is rejected. Retirement is derived from the roles: a key becomes retired when a state-carrying event lists it outside every role. The only assertion a controller adds to that derivation is `compromised_from`, and it withdraws a false alarm by a later state-carrying event that lists the key retired with no `compromised_from`, or in a current role. An entry identifies its key by the public key bytes; the verification-method fragment a signature's `signing_key_id` names (§9.8.1) resolves to a key through the latest state-carrying event, so a verifier locates a retired key by that lookup and never from a current document alone.

**Where the content boundary lives.** Content signed by an operational key is committed inside a context through MLS, and **the retiring identity owes an MLS Update to every context it belongs to** — a Commit, an epoch boundary every member of that context observes identically. Three rules place that obligation on it: the compromise recovery protocol (§9.12 step 2) on a compromise, the rotation rule of §9.7.3 on a planned rotation, and §9.7.4.2 R4 on the abandoning controller before it publishes. **The obligation is the identity's, and a context where the identity did not discharge it records no Commit**; the paragraph below states what a member reading such a context does. **The epoch of the Commit that follows a key's retirement is that key's boundary in that context**, whatever the reason for the retirement. A `compromised_from` position names where in the log the controller asserts the compromise began; the Commit epoch names where that assertion took effect in each context, and content is ordered against it by **the MLS epoch the message decrypted under, never the envelope's `epoch` field** — a compromised key's holder signs that field, so a verifier that read it would take its ordering input from the attacker (§9.8.1 inner check 1 rejects an envelope whose `epoch` field differs from the epoch that decrypted it). No further ordering structure is needed: the decryption epoch is what MLS already gives every member, and every member derives the same epoch sequence, so the boundary converges by the same mechanism that makes MLS work.

**The boundary in a broadcast context.** A broadcast context (§5.14) runs no MLS group and advances no epoch: its authors distribute per-author sender keys and stamp each chunk with the author's `key_epoch`. **In a broadcast context the boundary for a retired author key is that author's `key_epoch` at the rotation, read from the sender-key distribution the author published and never from the chunk's own field**, for the same reason the MLS rule reads the decryption epoch: the holder of the retired key signs the chunk. A member with no sender-key distribution recording a `key_epoch` step for that author holds no boundary there, and the rule below decides what it does.
...
**"No boundary here" is a fact about the context, not a gap in the evidence.** The boundary for key K in context C **exists iff C's Commit history records a retirement Commit from that identity for K** — or, in a broadcast context, a `key_epoch` step for that author. A member reading C's Commit history, live or through a snapshot that carries that history, and finding no such Commit **accepts** K's content in C: either K was never retired in C, or the retiring party's SDK still owes C the Update, and in both readings nothing in C bounds K. `Invalid{no_boundary}` is the outcome in one case only — **the member holds no Commit history covering the span it must judge**, so it can neither find a retirement Commit nor establish that none exists. That is the fail-closed direction, because a member that accepted content over a span it cannot see would accept everything an attacker signed after a boundary it never read.

**The boundary travels with the content it bounds, and observation outranks it.** A context snapshot and a context export carry, for every identity whose key the context retired, that key's boundary epoch in that context, and the producer signs those values with the key the snapshot or export is signed under. A member that joins after the boundary Commit therefore learns the boundary from the snapshot it syncs rather than from an observation it could not have made. **Precedence:** a member's own observation of the boundary Commit outranks any snapshot value for that key in that context, so no other member rewrites a boundary the member watched happen. Where a member holds two snapshot values for one key in one context and they disagree, it takes the **earliest** and surfaces the disagreement to the user, because the earliest value bounds the most content and a member that took the later one would accept content the other producer says came after the boundary.
...
A retired key is not a live signing capability, and a `compromised_from` position never unmakes what the party already accepted before the boundary. The controller's key event is never rejected on account of content.
...
| **Log-anchored evidence** — anchored to a key-state position | the artifact carries the digest of the signer's key-state head at the moment it signed, inside the signed preimage (§9.5.2); the signature verifies **iff** the signing key was `current` at that position, and, where the entry carries `compromised_from` = M, only under the further test the paragraph below states. A retired key with no `compromised_from` keeps its signature valid for this class, because the artifact names when it was made | `SCP-KEY-DESTRUCTION-V1:`, `SCP-CONTEXT-SNAPSHOT-V1:`, `SCP-CONTEXT-EXPORT-V1:` |
| **Key event** — the log's own rules | §9.7.4.2 R3 | `SCP-KEL-EVENT-V1:` |

**Why the log-anchored class exists, and the limit it carries.** An artifact a party publishes as evidence against a future version of itself cannot verify under the attestation class, because the signer would void it by any routine rotation of the key it controls. §9.15's destruction attestation is such an artifact: it is published to relays, outside any context, so no context epoch orders it and the content class has no test to apply. A durable context snapshot and a durable context export are such artifacts too, and for a second reason: a snapshot signed under a key the producer later rotates is the very record that teaches a later joiner where that key's boundary lies, and the content class would reject it at every member who received it after the boundary. Anchoring these three artifacts to a key-state position gives them an ordering the signer cannot revise after the fact, because the key-state head digest is fixed by the log. **The limit, stated because it is not a guarantee against the compromised key's holder:** the signer chooses `key_state_head`, so a party holding a key the state lists `Retired{compromised_from: M}` can anchor a forged artifact before M. A verifier therefore accepts such an artifact **only** when it held that artifact before it adopted the event that carried M — first-seen, recorded with the adoption — **or** when a party independent of the signer countersigned it. Against a compromised key with neither condition met, this class gives no guarantee, and a verifier rejects. `SCP-CHECKPOINT-V1:` (§9.9.3) needs no anchor at all: a member accepts a consistency checkpoint through MLS inside the context, so the content class already orders it. **Which artifacts are durable:** a context snapshot and a context export are durable artifacts, stored and re-served after the epoch that produced them, which is why they carry per-item epochs and their own key-state anchor; every separator in the content row is live traffic a context ordered when it arrived.

Governance votes and proposals are content: a vote cast under a key that is later marked compromised stays counted for the members who accepted it before the boundary, because the alternative — every vote in a context's history rejected the moment a key is asserted compromised — would let a compromise assertion rewrite governance outcomes. The DID-record frame's BEP44 signature (§9.10.12) is outside this table: it is not made under an SCP domain separator, and no relying party reads it as evidence. It is the relay and DHT write rule, made by the operational key the key state designates for the service record and checked by a relay against the designated key of the chain the frame carries, and a resolver authenticates the record by the key-event log instead (§9.6.1). A key-continuity fingerprint (§9.11) is a hash, not a signature, and is outside this table. Rationale for the default: a structure nobody classified is safer read as current-only than as content, because the content class admits historical keys.

**Every verdict the resolution policy must consume.** The resolution-failure policy of check 2 has, for each trigger, a branch for resolution failure and a branch for a resolution that returns a rotated key. The five verdicts of §9.7.4.2 R14 partition across those branches as follows, and the policy MUST route each one.

- `Confirmed` and `Adopted` are the **success** branch, and both refresh the accepted key state. **A candidate at a strictly lower sequence on the accepted chain is no verdict at all**: the verifier discards it and stays on the baseline it already held (§9.7.4.2 R12), so the resolution refreshed nothing and check 2's freshness bound decides what happens next — where the held baseline is older than `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS`, the resolution has not succeeded and the failure branch applies. Routing a discarded stale candidate to the success branch would let a party that suppresses the newer head and serves an older genuine one pin every verifier on a baseline listing a stolen key `current`, at no cost and repeatably.
...
- `Contested` is its own branch. §9.11's gate list is authoritative for the acts a verifier withholds from a contested identifier, and this paragraph adds no act to it. Two things are decided here rather than there. **The basis an already-admitted member's Update is checked against:** the shared prefix's key state — the state both suffixes agree on, which the verdict's shared-prefix head identifies — and an Update whose attestation does not verify against it is rejected. R7 gives a contested identity no `current` key state, so a verifier that read "the adopted key state" here would have nothing to read and would either admit the Update unchecked or censor the member. **Leaf eviction:** the verdict alone evicts no leaf, and §9.12 step 1a proposes Remove for no leaf on a `Contested` verdict, for the same missing basis. Content verification for that identity uses the shared prefix's key state as its basis for content the member accepted under an epoch no later than the last Commit that context observed from the identity before the member first observed any divergent suffix (§9.7.4.2 R7), and verifies nothing after that anchor. **A contested identity has no §9.7.1 boundary at all**, because it retired no key and issued no §9.12 step-2 Update, so a member that cannot place content before that anchor reports `Invalid{no_prefork_basis}` and never `Invalid{no_boundary}`.
...
- Active Signing Key (Ed25519): Generated via KeyCustody. Used for MLS KeyPackage attestations (§9.7.1), inner-envelope signatures, UCAN issuance. It does NOT sign the MLS leaf/credential directly — the leaf is self-signed by the ephemeral MLS leaf signature key (below), and the DID↔leaf binding is carried by a `#active`/`#agent`-signed KeyPackage attestation. Rotated by a `KeyState` event signed by the standing root (§9.7.4.2 R3). The identifier does not change on active key rotation.
...
- Agent Signing Key (Ed25519, optional): Generated by agent runtime software. Listed as `#agent` in the key state by a `KeyState` event signed by the standing root (§9.7.4.2 R3). Software-held — no HSM requirement (agent runtimes typically lack hardware security). Used for agent-autonomous message signing and scoped UCAN delegation. Rotated by a `KeyState` event signed by the standing root. The identifier does not change on agent key rotation.
...
- Root set and operational keys: carried in the identity's key-event log; a resolver derives them from the latest state-carrying event (§9.7.4.2 R8), and the identity spec's DID document is a projection of that event.
...
- Active Signing Key: Rotated by a `KeyState` event signed by the standing root (§9.7.4.2 R3) that lists the new `#active` current and the old one retired with no `compromised_from` (§9.7.1). All active MLS groups receive an Update proposal with the new credential. The identifier does not change. §9.11 states which observers set `PendingReverify` on that event and which do not.
- Agent Signing Key: Rotated by a `KeyState` event signed by the standing root that lists the replacement `#agent` current (or lists no `#agent`, which revokes agent access). All scoped UCANs with `fct.scp_key_scope: "#agent"` signed by the old `#agent` key are revoked and reissued with the new key. All active MLS groups receive an Update proposal with a new credential containing the updated `signing_key_id`. The identifier does not change.
...
- UCAN tokens: Expire per their `exp` field. Re-issued by the human's Active Signing Key. On active key rotation, all UCAN tokens signed by the old Active Signing Key are revoked and reissued under the new key. On agent key rotation, all scoped UCANs with `fct.scp_key_scope: "#agent"` are revoked and reissued with the new `#agent` key; root UCANs signed by `#active` are unaffected. Revocations are added to the per-context `RevocationList` and distributed as MLS application messages (see §9.5 UCAN revocation, ADR-016 criterion 5).
...
- **A copy of P forks before the controller's abandoning rollover.** The controller holds its root and publishes an abandoning `CommitmentRollover`; a holder of a copy of P forks before it with a `RootRecovery{Lost}` revealing the same commitment. Both suffixes are rank 1: the abandonment is never adopted, the controller may not extend past its own abandonment (§9.7.4.2 R4), and relying parties may not act on an unconfirmed head — the retirement never takes effect. The remedy is order: a controller that suspects a copy signs a `RootRecovery{CoSigns}` on its own suffix first, retiring the contested root set and consuming the commitment the copy could reveal, and then abandons under K′ (§9.7.4.2 R10).
...
This section governs a **non-delegated** identifier. For a delegated identifier the delegator's chain anchors the delegate's establishment events by a key-event seal (definitions), so precedence for a delegated chain reads the delegator's chain; the delegation model is not yet specified in this corpus, and until it is a verifier MUST reject a chain that claims delegation (`Invalid{at_event}`).
...
- An **operational key** is a signing key other than a root member — currently the Active Signing Key `#active` and the Agent Signing Key `#agent`.
...
- The **fallback set** for an identity is the set of sources that no snapshot on any suffix of that identity's chain names — every source outside the relay list the chain carries. A controller reads its own chain from the fallback set (R10) and a verifier tries a fallback source before it returns `Inconclusive` (R11), because a source an attacker's `KeyState` could have introduced is a source the attacker chose.
...
- The **key state** of an identity is: the standing root; every operational key by role (`#active`, `#agent`) and by custody type (above), one value per key; **every key that has ever appeared on the chain** as a root member or an operational key, each in one of the two conditions of §9.7.1 (`current`, or `Retired{compromised_from}` with an optional log position); the **witness set** — the relays the controller designates to cosign the identity's log head — with two **cosigning parameters**, the **witnessing interval** (`u32` seconds; the interval at which each designated witness re-cosigns the head, and the floor on the controller's self-observation cadence under R10) and the **accountability threshold** (`u8`; the number of cosignatures at which the controller treats its own head as established, which R10's self-observation reads); the **relay list** — the relays that serve the identity's log — with the designation of the operational key that signs the identity's service record; and, for a delegated identifier, the **delegator** (a 32-byte identifier, or the all-zero placeholder for a non-delegated identity). Each of these is in the inception event's preimage because the identifier freezes that preimage: the initial witness set and relay list are what a first-contact resolver reads, custody type is what trust evaluation reads, and delegation must be declared from the first event because a delegated chain's precedence reads the delegator's chain. **The inception fixes the initial value of each field; it does not freeze the field.** Every state-carrying event carries the whole key state, so a later `KeyState` changes the witness set, the two cosigning parameters, the relay list, the operational roles, and each key's condition, exactly as R8 describes. The one field the inception freezes for the life of the identity is the delegator, because a delegated chain's precedence reads a different chain and a verifier must know from the first event which chain that is.
...
- a state-carrying event whose service-key designation names a key that same snapshot does not list `current`, or names no key at all. A `RootRecovery` therefore installs at least one operational key, which R10's ceremony already generates. Rationale: the designation is what a relay and a DHT node check a write against (§9.10.12), so a designation naming a retired or absent key leaves the identity unable to publish its own recovery;
- a witnessing interval below `MIN_WITNESSING_INTERVAL` (§9.18.17), or an accountability threshold greater than the number of witnesses the same snapshot's witness set names. Rationale: an interval of zero makes R10's self-observation an unbounded fetch loop, and a threshold above the set size is a permanent alert the controller cannot satisfy by cosigning;
...
| finds its identity contested and wishes to retire it | `RootRecovery{CoSigns}` on its own suffix first, retiring the contested root set, then abandonment under K′ (the failure-mode table's two-phase row) |
...
*Self-observation.* The SDK MUST fetch its own identity's log at a stated cadence — no less often than the witnessing interval the key state names, and on every launch — from at least one source in the fallback set (definitions), and MUST treat an inability to complete that check as an alert, not as silence. **The accountability threshold is read here:** the controller treats its own head as established once it holds at least that many cosignatures for it from the witness set its key state names, and MUST alert the human while it holds fewer. That is the whole reader of the field. **Until the witness protocol is specified in this corpus, the threshold's value is zero**, so an SDK that can collect no cosignature raises no alert it has no means to clear; a controller that sets a nonzero threshold before the protocol lands is asking for an alert it cannot satisfy, and R3's bound against a threshold above the witness set's size is what stops the value from being unreachable by construction. The controller changes both cosigning parameters by a `KeyState` (R8), so neither value is fixed for the life of the identity.

*Before composing a snapshot.* The controller MUST hold its chain from the inception event, obtained from at least one source in the fallback set (definitions); MUST enumerate **every key any event at or before the snapshot's position installed**, with each key's custody type and the condition the snapshot will give it, so that the snapshot lists each one exactly once (R8); MUST diff the chain's current relay list and witness set against the inception's or against a pre-compromise record it holds and surface every entry introduced at or after the asserted M. **The refusal splits by verdict.** On `Inconclusive` the controller MUST refuse to sign, with a typed error, because it cannot see the events whose keys R8 requires it to enumerate. On `Contested` the controller refuses only when it cannot identify which suffix it authored; when it can, it composes over that suffix and signs, which is what the paragraph below tells the SDK to keep doing. A blanket refusal on `Contested` would lock the controller out of the first row of the table above, whose whole premise is a root-signed event some other party wrote onto the chain.
...
*Abort.* **Abort is available in the two phases before a publish is sent, and in no phase after.** In those two phases it consumes the handle by value and MUST destroy the composed preimage, the signed event if one exists, and the next set the ceremony generated, and it leaves the standing commitment unconsumed and the chain unchanged — which is the guarantee abort carries and the only place that guarantee is true. A controller that discovers, before it sends a publish, that its device-boundary establishment was wrong aborts under it.

**The phase splits at PUBLISH SENT, not at acknowledged**, because an acknowledgement can be lost while the relay holds the event: a controller that aborted from a sent-but-unacknowledged publish would destroy its own copy of an event the relay serves, believe the standing commitment unconsumed, later reveal that commitment a second time, and contest its own identity permanently under R7 — the failure the persist-before-sign clause prevents at the signing boundary, reappearing at the publishing one. From the sent state the controller's paths are: re-read its chain from the fallback set (self-observation above), then complete the ceremony from the handle, or extend its suffix under the table above. Never abort.

**R11 — Divergence is a positive finding.** A candidate head is **on the accepted chain** when either chain is a prefix of the other. A candidate head is **divergent** when the verifier holds both chains back to a common ancestor and they diverge (definitions). A candidate that is neither — because the events linking it to the accepted chain were not served, or because a served copy failed verification as a copy-level defect (R3) — is **inconclusive**: the verifier holds its baseline, records nothing, and returns the `Inconclusive` verdict (R14), which names the candidate head, the accepted head, the missing sequence range, and every source that failed to serve the link; before returning it the verifier MUST have tried at least one source in the fallback set (definitions). R9's retention bound never produces this verdict, because R9 admits the candidate and evicts by rank instead.

**First contact takes two sources, one of them outside the served chain's own relay list.** A verifier holding no accepted baseline for an identifier MUST obtain that identifier's chain from at least two sources, of which at least one is in the fallback set (definitions) — a source the served chain's own relay list does not name — and it adopts the highest-sequence chain that verifies across them. **When it can reach no second source, the verdict is `Inconclusive{SingleSource}` (R14) and the verifier adopts no head**, and the SDK reports that cause rather than a generic failure, so the human sees that the identity was not resolved rather than that it does not exist. Rationale: a single source can always serve a genuine prefix truncated before the event the reader most needs — the `KeyState` that retires a stolen key, or the recovery that replaced a root. Such a prefix forks nothing, so R6 never runs; it verifies clean under R2 and R3; and a first-contact verifier holds no baseline, so R12 has nothing to compare. Neither the chain's own rules nor any relay-side gate detects it, because a relay-side gate is code an untrusted operator omits. Two sources are what a first-contact reader has instead, and a later reader holding a baseline has R12 and R6. The Layer-B witness layer is what would let one source suffice, and the witness protocol is not yet specified in this corpus. A verifier MUST record equivocation evidence only from two chains it holds in full back to their common ancestor, and only when R6 resolves them as a tie (R7). Rationale: a single relay that withholds linking events, or serves a mutilated copy, could otherwise cause a verifier to record a durable verdict against an honest identity.

**R12 — Sequence governs the accepted chain only.** For a candidate head on the accepted chain, a verifier **discards** a head at a sequence strictly lower than the highest it has accepted, changing no stored state and returning no verdict, and an equal sequence with an identical head confirms the baseline (`Confirmed`). A discarded stale candidate is not a resolution outcome: the verifier stays on the baseline it already held, so a consumer that treated a discard as a completed resolution would run its next check against a baseline the candidate did nothing to refresh. A candidate whose chain is neither a prefix of the accepted chain nor extended by it is classified under R11 and decided under R6 and R7. A divergent candidate's head sequence decides nothing. Adopting an R6 winner MAY lower the accepted head sequence (`Adopted{baseline_decreased}`), and a verifier's stored baseline MUST be able to express that decrease. Rationale: a monotone maximum cannot record the adoption of a lower-sequence winner.
...
The **DID-record frame** is the unencrypted counterpart to the §9.10.2 Minimal Outer Envelope: where the outer envelope is *confidential by encryption*, a DID record is *self-certifying through the key-event log it carries* (§9.6.1). It carries an identity's key-event log (§3.10.2) as a public relay record. It is specified here in §9.10 because §9.10 is where relay-stored record formats live: the outer envelope (§9.10.2) is the encrypted format, and this is the public-record format. This design is issue #482. (See also §9.18.11 Transport and Relay, which fixes the shared relay blob-size and blob-TTL bounds this frame reuses.)

The frame is **minimal and DID-specific — there is no magic tag and no record-kind byte.** DID records live at their own `routing_id` domain (`SHA-256("scp:did:" || identifier_bytes)`, §9.7.4.2 R13), so the address is the type discriminant; a frame needs no self-describing tag to say what it is. (A future MLS KeyPackage relay record, if built — issue #2202 — would define its OWN minimal frame at its OWN `routing_id` domain; DID records and KeyPackages do NOT share a tagged multi-kind envelope family, and this frame introduces no such taxonomy.)

**Storage model — a raw relay blob.** A DID record is stored as a **raw relay blob** at its routing ID. It is **NOT** wrapped in an `OuterEnvelope`, and its bytes are **NOT** MLS-encrypted — it is a public, self-certifying record whose authenticity comes from the key-event log its `value` carries (§9.6.1), not from encryption and not from its BEP44 signature. Publication and resolution use the existing PUBLISH/QUERY operations (ADR-004) verbatim — no new wire types. `ClientMessage::Publish { routing_id, blob_ttl, blob }` writes the frame bytes at the DID routing ID, and `RelayMessage::Blob { …, blob }` returns them.
...
2. **Confirms the identifier→routing_id binding**: `routing_id` equals `SHA-256("scp:did:" || identifier_bytes)` over the 32 raw digest bytes the frame's `identifier` field carries — the derivation §9.7.4.2 R13 states. The identifier encodes no key (R13), so no key is derived from it and no field of the frame is hashed to the routing ID. This binding is the discriminant: a validating relay recognizes a DID record by the frame decoding *and* the binding holding — it needs no new wire type and no a-priori knowledge of `routing_id` semantics, and a context `OuterEnvelope` (whose first byte is a MessagePack map marker, never `version = 1`) is never mistaken for one. Being a plain hash, it runs **before** any signature verify.
3. **Verifies the chain the `value` carries** under §9.7.4.2 R2 and R3: it recomputes the identifier from the chain's inception event, rejects a chain whose recomputed identifier differs from the `identifier` field of step 2, and rejects a chain any of whose events carries an author-attributable defect. This is the relay's write authorization, and it authorizes against the chain rather than against any key the frame supplies.
4. **Verifies the BEP44 signature** over `bencode(seq, value)` against the **designated service key of the chain the `value` itself carries**, in every case — the operational key that chain's latest state-carrying event designates for the service record (§9.7.4.2 R3 rejects a state-carrying event whose designation names a key it does not list `current`). Step 3 has already verified that chain from its inception event, so the key is one the relay read out of a verified chain and never one the writer supplied; reading it out of the stored slot's chain instead would fail every genuine first frame after a `#active` rotation, whose new designated key the stored chain does not yet name. The signature and `seq` are the write rule and never a resolver's trust input (§9.6.1).
5. **Places the frame's chain in a slot** under §9.7.4.2 R9, which states the slot key, what opens a slot, what a slot holds, and the eviction rule; this section applies R9 and restates none of it. The relay compares the frame's chain against each stored chain at that routing id and, where the two are prefix-related, writes to that slot under **this section's own `seq` rule**: reject a `seq` at or below the stored slot's `seq` unless an equal-`seq` frame is byte-identical (an idempotent TTL refresh), and replace the slot only on a strictly-higher valid `seq` whose chain extends the stored one. A QUERY returns every slot at the routing id, because a resolver runs the fork-precedence rule (§9.7.4.2 R6) over the chains it is served and a relay that returned one slot would decide that rule for it.

**Slot-exclusivity.** The base relay stores multiple opaque blobs per `routing_id` with no per-`routing_id` cap (ADR-004), so validating only frame-shaped blobs would leave non-frame or pre-seeded junk sitting alongside the slot and returnable by QUERY. A validating relay therefore makes a `routing_id` **slot-exclusive** once a binding-valid frame first establishes a slot there: (a) it rejects any later PUBLISH there that is not a frame passing steps 1–5 above; (b) it evicts any pre-existing opaque blobs at that `routing_id` when the first slot is established; (c) QUERY there returns the slots and nothing else — every slot the routing id holds, one per divergent suffix; (d) it **rejects a DELETE of any slot blob** — only a superseding PUBLISH may replace a slot, and only R9's rank eviction may remove one. Rule (d) is **storage-derived, not index-derived**: because DELETE addresses a blob by `blob_id` and the in-memory slot index is a cache that a relay restart or a store-sharing peer leaves cold, the relay decides rule (d) by re-reading the blob at `blob_id` and rejecting the DELETE if it decodes as a frame whose chain verifies from inception under R2 and R3 (the chain is self-certifying, so protection is reconstructible from the bare bytes) — immune to a cold index. The DELETE gate runs behind the per-IP rate limit and fails closed on a storage read error (§3.10.8 "Attacker DELETEs the slot blob"). Before the first valid frame the relay cannot recognize the `routing_id` as DID-domain (`SHA-256` is one-way), so pre-seeded junk simply sits as ordinary opaque blobs until the first valid publish establishes the slot and evicts it (§3.10.2). This is a relay **storage** behavior; the relay-storage mechanics are transcribed in the companion **ADR-004 "DID-Record Slot-Exclusivity" subsection** (implemented under #482 / SCP-RELAYRES-003). This section and §3.10.2/§3.10.8 are authoritative for the behavior and its threat model.
...
DID-RECORD (DidRecordV1) :=
  ...
  value:            [u8]                     # trailing remainder = BEP44-signed payload = one contiguous segment of the key-event log the record projects (§9.6.1, §9.7.4.2 R9)
```
...
- **value** — the sole variable-length field, carried as the **trailing remainder** of the frame (no `value_len` prefix). Because every preceding field is fixed-width, `value` is unambiguously `frame_bytes[105..]`; a length prefix would be redundant with the blob's own length and a determinism footgun (two disagreeing lengths). `value` MUST be non-empty and its length MUST NOT exceed `Max blob size` (262144, §9.18.11) `− 105`. It carries one contiguous chain segment, naming the segment's first and last sequence and the events between them in order; a relay serves a slot's chain as the ordered sequence of frames covering inception through head (§9.7.4.2 R9).

The fixed prefix is `1 + 32 + 8 + 64 = 105` bytes, and the total frame length is `105 + len(value)`. There is deliberately **no** `sig_len` field: a BEP44 Ed25519 signature is unconditionally 64 bytes, and §9.5.1 gives fixed-width fields no length prefix; a different signature length would be a new version, not a per-frame field. **The frame carries no public key and no root-set digest**: a relay reads both the designated service key and the chain's standing root set out of the chain it verified at step 3, so a value supplied by the writer would authorize the writer against itself, and a digest supplied by the writer would let a keyless party replay a genuine frame under many spellings of the same chain and claim a slot for each.

**Framing is outside the signed authority.** The BEP44 signature covers only the **BEP44-canonical bencoded buffer `bencode(salt?, seq, value)`** — the sequence number precedes the value, serialized as `3:seqi<seq>e1:v<value>` (with an optional preceding `4:salt<salt>` where the DID method uses one; did:dht uses none). **BEP44 (BitTorrent BEP 44) is authoritative for this field ordering** — `seq` before `value` — and every SCP binding MUST reproduce that exact octet sequence, since the signature is hand-rolled across bindings and must be byte-reproducible. The frame's `version`, its `identifier` field, and its field boundaries are unsigned transport framing. **A decoder MUST NOT derive any security-relevant conclusion from an unsigned field, and a relay MUST cross-check the one unsigned field a decision reads — `identifier` — against the verified chain before any decision reads it**: step 3 recomputes the identifier from the chain's inception event and rejects a frame whose `identifier` field differs. Every other input a decision reads, the standing root set and the designated service key included, the relay takes from the verified chain and never from the framing. After decoding a frame, the resolver authenticates the record by the key-event log the `value` carries — recompute the identifier from the inception event and verify every event under §9.7.4.2 R2 and R3 (§9.6.1) — and reads the BEP44 signature for no trust purpose at all; a frame whose chain fails that verification is discarded exactly as an invalid DHT record is (§3.10.4). Record substitution fails because a chain served under an identifier recomputes to that identifier only if its inception event is the one that identifier names.
...
2. **Require the fixed prefix in full.** A decoder MUST reject any frame shorter than the 105-byte fixed prefix (`version + identifier + seq + signature`) — truncation is never a partially-valid frame.
...
- **Publish** = wrap `(identifier, seq, signature, value)` in a DID-record frame and PUBLISH the frame bytes at the DID routing ID via the existing PUBLISH operation (§3.10.5). The DID relay blob MUST be the DID-record frame — not the bare key-event log bytes, and not an `OuterEnvelope`.
- **Query** = the existing QUERY operation (§3.10.4) with `limit: N` (N = 16) returns the blob(s) stored at the routing ID; the resolver decodes each at the **single** decode-and-verify site, verifies the chain each `value` carries under §9.7.4.2 R2 and R3, and settles two chains that diverge by the fork-precedence rule (§9.7.4.2 R6). It reads no key from the frame, and it treats the BEP44 signature as the relay's write rule, never as evidence (§9.6.1). Against a validating relay the routing ID holds one slot per divergent suffix, bounded by `MAX_RETAINED_SUFFIXES` (§9.18.17), so `N` covers every slot a routing id can hold; against non-validating or foreign storage `limit: N` lets the resolver retrieve up to N candidates and sift them by R2, R3, and R6 (§3.10.4 step 5). See §3.10.2 for why `limit: N` dominates `limit: 1`.
- **Distinct from the outer envelope.** A DID routing ID carries DID-record frames, not `OuterEnvelope`s; a resolver MUST NOT deserialize one as the other. The two are disjoint at two independent levels: **(a) routing-ID space** — DID records occupy `SHA-256("scp:did:" || identifier_bytes)` (§9.7.4.2 R13), disjoint from context routing IDs; **(b) byte level** — a DID-record frame begins with `version = 0x01`, whereas an `OuterEnvelope` is MessagePack-serialized as a map (`rmp_serde::to_vec_named`) whose first byte is always a map marker (fixmap `0x80`–`0x8f`, or `0xde`/`0xdf` for larger maps) — never `0x01`. A DID-record frame therefore can never be mistaken, byte-for-byte, for an `OuterEnvelope`, or vice versa — a defense-in-depth backstop beneath the routing-ID separation.

**The frame never enters the signed bytes.** The frame wraps `value` for relay transport only; it never reorders or alters the bytes the BEP44 signature covers, so a relay verifies the same `bencode(seq, value)` buffer the publisher signed. **The two layers carry different payloads and no rule compares them byte for byte**: a relay record's `value` carries a chain segment, and a DHT record's value carries the signed head pointer §9.6.1 defines, which the 1000-byte BEP44 cap is what forces. §3.10.4's rule that two valid records sharing a sequence number MUST be byte-identical is a rule within one layer, applied to two copies of one record, and it says nothing across layers.
...
**Fingerprint format — one construction, for inception-derived identifiers.** The fingerprint takes the parties' **32-byte identifiers** (§9.7.4.2 R13), never their DID strings. The identifier is already a fixed-length 32-byte digest, so under §9.5.1 it carries no length prefix and the construction waits on nothing.

```
fingerprint = SHA256("SCP-KEY-CONTINUITY-V1:" || id_a || count(a_root_set) || a_root_set_members || a_active_key || a_agent_key
                                             || id_b || count(b_root_set) || b_root_set_members || b_active_key || b_agent_key)
```

Where `id_a, id_b` are the two 32-byte identifiers ordered by unsigned byte comparison, and the block for the lower identifier comes first; `count()` is a 4-byte big-endian member count (§9.5.1's repeated-field rule); `a_root_set_members` is every member of that root set as a raw 32-byte key, **in the list order the key state carries**, because the root set is an ordered list and its order is signed; and every key is a raw 32-byte Ed25519 public key. **This is the only form, and it covers a root set of any size, the one-member set included**: a personal identity contributes `count = 1` and its single member. An SDK MUST NOT omit the count for a one-member set. A second, count-free encoding for the 1-of-1 case would make two SDKs that each chose a different encoding compute two different values for one pair and raise a maximum-severity MITM alert on an honest pair. Every root member and both operational keys are included so that substitution of any single key changes the fingerprint.

**A did:web identifier is outside this fingerprint's scope**, because it has neither a 32-byte inception-derived identifier nor a root set; §9.6.2 states how such an identifier's standing exits `PendingReverify` and names the SDK surface that carries the exit.

**Sentinels for an absent key.** If a key state names no `#agent`, the domain-derived sentinel `SHA-256("SCP-ABSENT-AGENT-KEY")` (truncated to 32 bytes) stands in its place. If a key state names no `#active` — which R8 permits, because a snapshot may designate `#agent` for the service record and name no `#active` at all — the sentinel `SHA-256("SCP-ABSENT-ACTIVE-KEY")` (truncated to 32 bytes) stands in its place. Each sentinel avoids collision with the Ed25519 identity point (all zeros) and provides domain separation from legitimate key values, and each makes the fingerprint defined for every key state R8 admits. The `"SCP-KEY-CONTINUITY-V1:"` domain separator prevents cross-protocol signature confusion.
...
1. Alice and Bob each compute the fingerprint over the two identifiers, the two root sets, and the two pairs of operational keys, taking the other party's values from the key state each derived from the chain it adopted.
...
**Where each gate lives.** The bullet list above is authoritative for which acts the standing withholds; this paragraph names the section that owns each gate's implementation and adds no act. §9.7.1's admission and grant gates (Add, UCAN, sender key to a new leaf); §9.16.2's key response; §5.14.8's broadcast per-author key cache, which is where a cached `author_keys` entry is read; §9.6.4's first-contact bootstrapping; §9.6.2's did:web mitigations, whose change trigger is the verification-method key set; inbound invitation acceptance and contact-graph promotion; and the standing-pair consent gate and the known-identity allowlist of the contexts spec, which MUST read the standing on a peer's `RootRecovery` because a stable identifier keeps matching an allowlist entry that a rename used to invalidate. An implementer that builds a gate set from this paragraph alone would miss an act; the list above is what a gate enforces.
...
**Certificate pinning:** The SDK SHOULD support certificate pinning for known relays. §9.6.2 states the did:web resolution server's pinning obligation, and this section adds nothing to it.
...
**Self-hosted relay exception:** **a relay URL may use `ws://` if and only if the resolver took it from a key-event log it verified under §9.7.4.2 R2 and R3** — the relay list is key state, so a verified chain's latest state-carrying event is a self-certifying source for it, whichever storage layer served that chain (§9.6.1, §9.6.3). The SDK MUST reject a `ws://` relay URL from every other source, `.well-known/scp` among them, because such a source carries no signature binding the URL to the identity and a substituted URL would downgrade the transport unnoticed. This section is the one home of that criterion, and §10.12.6 applies it to the self-hosted relay tiers by citation. Such relays have no domain and cannot obtain CA-signed certificates; MLS provides the confidentiality boundary, and TLS on a dumb pipe protects already-encrypted traffic.
...
KeyDestructionAttestation {
  ...
  key_state_head:        [u8; 32]              // digest of the signer's key-state head at signing (§9.7.1, log-anchored evidence class)
  platform_attestation:  PlatformAttestation?  // hardware-backed if available
  method:                .hardwareBacked | .softwareOnly
  signature:             Ed25519Signature       // signed by #active (Active Signing Key); NOT a root member (the root signs establishment events only, §9.7.4.2 definitions); NOT #agent — agents cannot sign destruction attestations (ADR-039)
}
```

§9.5.2 fixes the field order and encoding of the signed preimage, and every field above except `signature` is inside it — `key_state_head` included, so the anchoring position is signed rather than supplied beside the signature.

4. Attestations are published to relays, outside the now-destroyed context, so they remain readable after the context keys are destroyed. They are **log-anchored evidence** (§9.7.1): the `key_state_head` field carries the digest of the signer's key-state head at the moment it signed, and a verifier accepts the signature iff the signing `#active` was `current` at that position. A later routine rotation of `#active` therefore leaves every earlier destruction attestation verifiable, which is the property the attestation class would not give it — a signer that could void its own past destruction claims by rotating a key it controls would be publishing no evidence at all. **Where the key state lists the signing key retired with `compromised_from` = M, the anchor alone proves nothing**, because the signer chooses `key_state_head` and the holder of a compromised key chooses a position before M. §9.7.1 states the two conditions under which a verifier accepts such an artifact — it held the artifact before it adopted the event carrying M, or a party independent of the signer countersigned it — and states that the class gives no guarantee where neither holds.
...
| `"SCP-ABSENT-ACTIVE-KEY"` | Sentinel for absent `#active` key in continuity fingerprint — `SHA-256("SCP-ABSENT-ACTIVE-KEY")`; R8 permits a snapshot that names no `#active` while designating `#agent` for the service record, so the fingerprint is defined for every key state | §9.11 |
...
| DID-record frame fixed prefix | 105 | Fixed-width prefix length in bytes (`version` 1 + `identifier` 32 + `seq` 8 + `signature` 64); `value` is the trailing remainder, `total_frame_len = 105 + len(value)` | §9.10.12 |
...
| `MAX_KEY_EVENTS_PER_CHAIN` | 4096 | Maximum key events on one chain; a verifier rejects a chain that exceeds it. R8 makes every state-carrying event list every key the chain installed, so a chain's bytes grow with the square of this count, and 4096 events bounds a chain at a few hundred megabytes assembled from segments — far past the rate at which a controller that rolls over on suspicion rather than on a schedule (R10) reaches it | §9.7.4.2 R9 |
| `MAX_DHT_RECORD_BYTES` | 1000 | Maximum bytes of a DHT-form record's value. External constraint: BEP 44 caps a mutable item's `v` field at 1000 bytes, which is why the DHT layer carries a signed head pointer and not the log (§9.18.18 records the external constraint itself) | §9.6.1 |
...
| BEP44 mutable-item value cap | 1000 bytes | External constraint (BitTorrent BEP 44). Not configurable — a DHT node rejects a larger value, which is what makes `MAX_DHT_RECORD_BYTES` (§9.18.17) the bound on the DHT-form record. | §9.6.1 |
