...
| Agent keys allowed | `true` | `{true, false}` | `ContextParams::agent_keys_allowed`. When `false`, the context admits no artifact signed by `#agent`. No struct declares the field; story SCP-AB-028 of `.docs/prds/agent-binding.json` adds it. | §4.9.3 |
| Agent action rate limit | Absent (no cap) | [1, u32 max] per rolling window | `ContextParams::agent_rate_limit`. Caps actions signed by `#agent` per member; an `#active`-signed action does not count against it. No struct declares the field; story SCP-AB-030 adds it. | §4.9.3 |
| Agent co-signature requirement | Empty list | Any subset of the context ceiling | `ContextParams::agent_cosign_required`. Capabilities a member exercising them through `#agent` must additionally have signed with `#active`. No struct declares the field; story SCP-AB-031 adds it. | §4.9.3 |
| User-authentication requirement | Empty list | Any subset of the context ceiling | `ContextParams::user_authentication_required`. Capabilities the context requires a member to exercise under a signing key its platform gates behind a device-local authentication gesture. A verifier rejects every listed capability: §4.9.3 states no admission rule, and issue #2417 carries eight open questions that decide one. No struct declares the field; story SCP-AB-037 adds it and the rejection. | §4.9.3 |
