...
- **Capability is unduplicable, unforgeable, and opaque.** The capability proof MUST satisfy all of:
  1. **Not duplicable.** No API, trait, impl, or ergonomic conversion returns a copy, clone, or alternative instance of the proof. Language-specific: Rust impls MUST NOT derive or implement `Clone`, `Copy`, `Serialize`, `Deserialize`, `Default`, `Hash`, `PartialEq`, `Eq`, `Borrow`, or `From`/`Into` for the capability type; other languages MUST apply the equivalent set of restrictions. A `&self`-only reissue/attenuation method that takes no raw identity input is permitted: it duplicates a proof the caller already holds without broadening which identities are reachable, so it is security-equivalent to holding the existing token twice and is not a forgery surface (see ADR-049 §5). Such a reissue/attenuation method's visibility MUST be inherited-private or exactly `pub(in crate::context)` — never `pub`, `pub(crate)`, or any narrower path-restricted form — matching the capability struct's own name-visibility, so the clone/accessor is reachable exactly where the type is nameable and no wider, and (for `reissue`) callable by `ActorDeps::clone_for_spawn` in the sibling `actor` module. The "never *wider*" half (no `pub`/`pub(crate)`) is the security/isolation bound; the "never *narrower*" half is a reachability/functional-correctness bound, not a security one — a narrower form (e.g. `pub(super)`) is strictly *safer* in isolation terms but would break the one sibling-module caller (`ActorDeps::clone_for_spawn` in `crate::context::actor`), so it is rejected to keep that caller reachable, not to close a leak. Within that tree it MAY be reachable by handler code, which is safe because it only duplicates an already-held proof and broadens nothing; the bound exists solely to prevent a `pub`/cross-crate reissue that would let code outside the module tree clone a held proof. This is a *weaker* bound than the point-3 mint constructor (which is supervisor-module-only because it accepts a raw identity); the matching-name-visibility allowed-set suffices for reissue precisely because reissue takes no raw identity input.
  2. **Not inspectable.** No API (including any `Debug`, `Display`, `Deref`, `AsRef`, or equivalent accessor) returns the inner DID or its bytes in a form that allows reconstruction of the proof, fabrication of a new proof, or addressing of *another* identity's per-identity state. The proof — as a proof — is opaque to handler code; handler code cannot consume it to mint, duplicate (beyond the point-1 `&self` reissue), or key into a different identity's state. A single carve-out is permitted: a borrowed `&DID` accessor (e.g. `as_did`) that returns a read-only reference to the token's *own* owning identity — the only DID the token holds — MAY be reachable by handler code, on the same matching-name-visibility allowed-set as the point-1 reissue method (inherited-private or exactly `pub(in crate::context)`, never `pub`, `pub(crate)`, or any narrower path-restricted form). For `as_did` the "never *narrower*" half rests on a different rationale than for `reissue`: `as_did` has no sibling-module caller forcing a particular width (it is consumed only by `SupervisorHandle` methods that are themselves `pub(in crate::context)`), so for it the lower bound is a pure uniformity/consistency choice — matching the cap struct's own name-visibility — not a reachability necessity and not a security one (a narrower `as_did` would be strictly safe, exposing only the actor's own public DID); only the "never *wider*" half carries isolation weight. This is safe because (a) it cannot reconstruct or forge the proof — there is no `From<&DID>` and no public constructor (point 3) — and (b) it exposes only the actor's *own* owning DID, which the actor already operates as; it cannot be used to key into a *different* identity's state, which is the actual isolation boundary (the "No API returns per-identity state given an arbitrary DID" rule below still holds, because the supervisor's per-identity-state API takes the *proof*, not a `&DID`). The prohibition stands in full for any accessor that would instead hand out the DID in a form usable to mint a token or to address another identity's state.
