...
- When an Identity Key migrates (ADR-003 §4b), the agent MUST send a `DidRotationEvent` in every active context and issue MLS Updates with the new credential and a **re-issued KeyPackage attestation** under the new DID.
...
- Pre-Rotation Key (Ed25519): Generated at identity creation, stored in cold/offline custody (see §9.7.4.1 for custody requirements). Its commitment (§9.7.4.2 definitions) is a field of the inception event and of every reveal-authorized event. The key is revealed only inside a reveal-authorized event — a commitment refresh or a `#0` recovery (§9.7.4.2 R3) — and the controller destroys it after that event is published (§9.7.4.2 R5).
...
- Identity Key (`#0`): Changes only by a recovery event (§9.7.4.2 R3) — a rare operation. The recovery event reveals the pre-rotation public key whose commitment equals the standing commitment, carries a signature by that key's private half, installs that key as the new `#0`, fixes the next commitment (§9.7.4.2 R4), and carries the compromise boundary N (§9.7.4.2 R8). The identifier does not change, because it derives from the inception event and a recovery event does not rewrite the inception event. No migration proof exists: the recovery event is signed under `"SCP-KEL-EVENT-V1:"` like every other key event (§9.7.4.2 R13). A relying party re-verifies key continuity (§9.11) on a recovery event, because the identifier no longer signals that the root moved.
...
3a. **Recovery-authority residence (normative).** Item 3's storage-isolation requirement constrains *where the recovery authority resides*, not merely which provider object holds the key handle. The **recovery authority** is the minimal secret, key handle, or authorization capability sufficient to recover the pre-rotation private key. It MUST reside in a substrate whose compromise is **independent** of operational-custody compromise: there MUST NOT exist any secret, key handle, or authorization capability reachable **(directly or transitively, following any key-wrapping or key-derivation chain to its root)** from the operational `KeyCustody` provider or the daily-operations authentication flow that — alone or in combination with publicly-stored artifacts (including the encrypted-offline ciphertext of item 4) — suffices to recover the pre-rotation private key. Wrapping the key in an approved cipher (item 4) does NOT by itself satisfy this requirement: the §3 adversary is one who has already compromised operational custody, and if the decryption authority for that cipher is itself reachable from operational custody, the cipher provides zero protection against that adversary. The load-bearing property is **principal-distinctness of the recovery authority**, not the strength of the cipher wrapped around the key.
   ...
   **A reveal is not a daily operation.** A reveal-authorized event (§9.7.4.2) is a rare, separately-authorized event, not a daily operation. A distinct reveal-time authorization principal — even one that is fully automated (e.g., a separate KMS role assumed only for a refresh or recovery, which the operational path holds no grant to assume) — satisfies this requirement. What does NOT satisfy it is the daily operational signing flow itself being able to reach the pre-rotation key.

   This is an **at-rest / daily-operations** property. It constrains where the recovery authority lives when no reveal-authorized event is in progress; it cannot and does not prevent the revealed pre-rotation private key from being live in operational process memory during an authorized refresh or recovery (revealing the key yields its plaintext, which a recovery event then installs as the new operational `#0`; §9.7.4.2 R3 and R10).
...
**Failure modes.** Each row names the keys the attacker holds and the outcome the rules of §9.7.4.2 produce.

- **Pre-rotation backup lost, `#0` intact.** The identity keeps operating under `#0`. The controller can sign no commitment refresh and no recovery, because both require the pre-rotation private key, and `#0` alone cannot fix a new commitment (§9.7.4.2 R1). The SDK MUST warn the controller that root recovery is no longer possible. The retention contract of §9.7.4.2 R10 and item 5d above is the mechanism that prevents this state.
- **Pre-rotation backup lost, `#0` compromised.** The attacker's `#0`-signed suffix and the controller's `#0`-signed suffix tie at rank 3 (§9.7.4.2 R7), and the tie is permanent because no party can reveal the standing commitment. Social recovery (§3.3) is the only remaining path: trusted contacts with admin roles remove the compromised identity from each context and re-add the person under a new identity.
- **Pre-rotation key copied, `#0` intact.** The attacker can sign a recovery (rank 2). The controller MUST sign a commitment refresh (rank 1) as soon as it suspects the copy; the refresh wins under §9.7.4.2 R6 wherever the attacker forked, and it fixes a new commitment whose preimage the attacker does not hold.
- **Pre-rotation key taken and the controller's copy lost, `#0` intact.** The attacker's recovery (rank 2) stands, because the controller can sign no reveal-authorized event. This is root takeover. Item 3a exists to keep this case from arising through operational-custody compromise; no mitigation exists once the key is exclusively the attacker's, and the spec states the outcome.
- **`#0` compromised, pre-rotation key held only by the controller.** The controller signs a recovery (rank 2) carrying the compromise boundary N. The recovery wins under §9.7.4.2 R6 against any `#0`-signed suffix wherever the attacker forked, and §9.7.4.2 R8 voids the attacker's establishment events from N on the controller's own chain.
- **`#0` compromised and pre-rotation key copied.** Both parties can sign every event kind. Two refreshes tie at rank 1 and two recoveries tie at rank 2, and either tie is terminal (§9.7.4.2 R7). The identity is unrecoverable, and social recovery (§3.3) is the outcome.
- **Two devices both holding `#0`, no attacker.** The two suffixes tie at rank 3 and the identity is contested but the tie is pending (§9.7.4.2 R7). The controller ends the tie by signing a commitment refresh on the suffix it keeps.

**Partial-publish recovery.** A reveal-authorized event has one irreversible step, the reveal, followed by fallible publication steps. §9.7.4.2 R10 fixes the ceremony order and requires a typed recovery handle that carries the byte-identical signed event on any failure after signing, so a resume republishes the same bytes without re-deriving a key or re-signing. The controller retains the spent pre-rotation key until publication is confirmed and destroys it then (§9.7.4.2 R5).
...
- The **root key** `#0` is the identity's establishment authority.
- An **operational key** is a signing key other than `#0` — currently the Active Signing Key `#active` and the Agent Signing Key `#agent`.
- An **establishment event** is a key event that changes the identity's key state or its declared witness set: the inception event, an operational-key rotation, a key-status assertion, a witness-set update, a commitment refresh, and a recovery. Every establishment event other than a reveal-authorized event is signed by `#0` alone. (The witness layer that consumes the witness set is specified under Track U4 of the identity-substrate execution plan; this section names the witness-set update only as an event class R8 voids.)
- A **pre-rotation commitment** is `SHA-256("SCP-PREROTATION-COMMITMENT-V1:" || len(pre_rotation_public_key) || pre_rotation_public_key)`, where `len()` is a 4-byte big-endian unsigned integer (§9.5.1). The separator is registered in §9.18.2 as a commitment-hash domain.
- The **standing commitment** at a position in a chain is the commitment fixed by the latest reveal-authorized event at or before that position. The inception event fixes the first commitment.
- A **reveal-authorized event** carries a pre-rotation public key P whose commitment equals the standing commitment at the event's predecessor, carries a signature over the event by P's private key, and fixes the next commitment.
...
**R2 — Inception fixes the first commitment.** A verifier MUST reject an inception event that fixes no commitment. Rationale: the identifier is the inception event's digest, so an inception event without a commitment would create a permanently valid identifier with no recovery path.

**R3 — Exactly two reveal-authorized event kinds exist.** A **commitment refresh** carries P's signature and the signature of the `#0` standing at the event's predecessor, and leaves `#0` unchanged. A **recovery** carries P's signature, installs P as the new `#0`, and carries the compromise boundary N (R8). No other event kind reveals a commitment. Authorization is the signature by P's private key and never the reveal alone: the revealed public key is public the instant the event appears, so a matching value without the signature proves nothing about who produced the event. Rationale: a controller whose pre-rotation key was copied while `#0` stayed intact needs the refresh; a controller whose `#0` was compromised needs the recovery; no third compromise case needs a reveal.

**R4 — Every reveal-authorized event fixes the next commitment in the same event.** The controller MUST generate the next pre-rotation key in a substrate satisfying §9.7.4.1 item 3a before it signs the event. A verifier MUST reject a chain whose latest reveal-authorized event fixes no successor commitment. Rationale: a chain whose standing commitment has been consumed and not replaced has no recovery path, and a separate follow-on event to fix the next commitment would have to be signed by `#0` alone, which R1 forbids.

**R5 — The controller destroys a spent pre-rotation key.** After the controller has published a reveal-authorized event and consumed any recovery handle for it (R10), the controller MUST destroy the revealed private key from every custody location it controls. Rationale: a spent key that survives can sign a second reveal of a commitment already consumed and tie the identity at that fork (R7). The controller cannot destroy a copy an attacker holds; R5 removes the controller's own copy and closes the two-device case.

**R6 — Fork precedence: the event that consumes the shared prefix's standing commitment decides.** When a verifier holds two chains that diverge from a shared prefix, the verifier MUST decide between them as follows and by no other comparison. Let C be the standing commitment at the shared prefix. On each divergent suffix, the verifier locates the event that reveals C's preimage. At most one such event exists per suffix, because R1 makes C unique on the shared prefix and a reveal consumes it. The verifier ranks each suffix:

1. the suffix carries a C-revealing event that also carries the signature of the `#0` standing at the shared prefix (a commitment refresh);
2. the suffix carries a C-revealing event that carries no such signature (a recovery);
3. the suffix carries no C-revealing event.
...
- *Pin A.* Rank 1 names the `#0` standing at the shared prefix, because each suffix may have rotated `#0` differently after the fork.
- *Pin B.* A suffix's rank is fixed by its C-revealing event and does not change when the suffix is extended. A reveal on a suffix that consumes a commitment the suffix itself fixed confers no rank, because the shared prefix never committed to that key.

Rationale, stated as the three comparisons this rule replaces: a comparison of the first event after the shared prefix lets the party that chooses the fork point choose which events are compared, so an attacker who forks among the owner's ordinary rotations ties or wins before the owner's recovery is read; a comparison of the highest rank found anywhere on a suffix lets a party whose recovery installed its own root and its own commitment mint a refresh on its own suffix and tie the owner; a comparison of head sequences lets the party that appends the most events win.

**R7 — Ties.** Equal rank resolves no winner, and the identity is **contested**. A tie at rank 1 or rank 2 is **terminal**: C is consumed on both suffixes, so no later event on either suffix changes the comparison. A tie at rank 3 is **pending**: neither suffix has revealed C, and a later C-revealing event on either suffix decides the divergence under R6. For a contested identity a verifier MUST return a typed contested verdict, MUST NOT adopt either suffix as the current key state, MUST NOT return the shared prefix's key state as the current key state, and MUST retain both heads and both suffixes as evidence. Criterion for what a tie proves: a rank-2 tie proves that the pre-rotation private key signed two divergent events; a rank-1 tie proves that both the pre-rotation private key and the `#0` standing at the shared prefix signed two divergent events; a rank-3 tie proves that `#0` signed two divergent suffixes. Indicator, not criterion: two parties holding the key is the usual cause of a double signature, and one holder with two devices also produces one. Rationale for refusing the shared prefix's key state: if `#0` was compromised before the fork point, the shared prefix already carries the attacker's events, so serving its key state as current would serve a hostile key state.

**R8 — A recovery voids the superseded `#0`'s establishment events from the compromise boundary.** A recovery event MUST carry N, the sequence from which the controller asserts the superseded `#0` compromised. When a verifier replays a chain through a recovery event, every establishment event on that chain that the superseded `#0` signed at a sequence greater than or equal to N and before the recovery MUST NOT contribute to the derived key state: an operational key such an event installed is not current, and a witness-set change such an event made does not take effect. The key state after the recovery is the key state at sequence N−1 plus the recovery event's own content. The `Compromised{from:N}` carve-out for content signatures (§9.7.1) evaluates content signed by an operational key that a voided event installed. A commitment refresh carries no N, because a refresh supersedes a competing suffix in its entirety under R6 and voids nothing on its own chain. Rationale: an attacker holding `#0` who appends rotations and witness-set changes to the one genuine log produces no divergence, so R6 never runs, and without R8 the controller's recovery would extend the attacker's events and leave them current. The alternative this rule rejects — leaving those events in force and requiring the new root to retire each one by a follow-on event — requires the controller to enumerate the attacker's events and leaves a window open between the recovery and the last retirement.

**R9 — A verifier holds the evidence the comparison reads.** To apply R6 a verifier MUST hold, or obtain, both divergent suffixes back to their common ancestor. A verifier that cannot MUST reach no divergence verdict: it holds its current baseline, records no equivocation, and treats the candidate as inconclusive (R11). Indicator, not criterion: key-event logs are short, so a verifier that retains its full accepted chain satisfies R9 with no fetch, and a verifier that retains only a head digest re-fetches its own chain from a relay, which cannot forge a self-certifying chain but can withhold it. A verifier MUST NOT bound the retained suffix at its last reveal-authorized event, because a party holding a spent pre-rotation key can anchor a fork before that event (R5), and the comparison then reads earlier events.

**R10 — Ceremony order and the recovery handle.** A controller MUST produce a reveal-authorized event in this order: generate the next pre-rotation key in a substrate satisfying §9.7.4.1 item 3a; sign the event; publish the event to the identity's relays and submit it to the identity's witness set; on confirmed publication, destroy the spent key (R5). The controller retains the spent key between signing and confirmed publication. A failure after signing MUST surface a typed recovery handle that carries the byte-identical signed event, so that a resume republishes the same bytes without re-signing; a resume that re-derives a key or re-signs the event produces artifacts that differ from the ones already published. Rationale: the reveal is irreversible once the event is public, and a bare error after it would leave the controller with no path to complete the ceremony.

**R11 — Divergence is a positive finding.** A candidate head is **on the accepted chain** when its chain links to the verifier's accepted head. A candidate head is **divergent** when the verifier holds both chains back to a common ancestor and each chain carries, at the first position after that ancestor, an event the other does not. A candidate that is neither — because the events linking it to the accepted chain were not served — is **inconclusive**: the verifier holds its baseline, requests the missing events, and records nothing. A verifier MUST record equivocation evidence only from two chains it holds in full back to their common ancestor. Rationale: a single relay that withholds linking events could otherwise cause a verifier to record a durable equivocation verdict against an honest identity.

**R12 — Sequence governs the accepted chain only.** For a candidate head on the accepted chain, a verifier MUST reject a head at a sequence strictly lower than the highest it has accepted; an equal sequence with an identical head confirms the baseline; an equal sequence with a different head is two `#0`-signed heads at one position, and the verifier records it as equivocation evidence subject to R11. A divergent candidate goes to R6, and its head sequence decides nothing. Adopting an R6 winner MAY lower the accepted head sequence, and a verifier's stored baseline MUST be able to express that decrease. Rationale: a monotone maximum cannot record the adoption of a lower-sequence winner, and a verifier that stored only a maximum would discard the outcome R6 reached.

**R13 — Domain separators.** The key-event signature preimage uses the separator `"SCP-KEL-EVENT-V1:"` under the §9.5.1 construction, and the first field after the separator is the event-type discriminator byte. The pre-rotation commitment uses `"SCP-PREROTATION-COMMITMENT-V1:"` as a commitment-hash domain. Both are registered in §9.18.2. Whether the identifier derivation uses the inception event's canonical digest under the event separator or a distinct identifier-construction prefix is fixed by the substrate byte-layout amendment (Track U4 of the identity-substrate execution plan); §9.18.2 marks that row pending until then.

**Worked examples.** Notation: e0 is the inception event fixing commitment C; a number is a sequence; the owner is the legitimate controller.

1. **A `#0` thief forks early.** The owner's chain is e0 through e9 (ordinary `#0`-signed rotations), and the owner signs a recovery at e10 that reveals C with N = 6. The thief, holding the compromised `#0`, forks at e5 and appends a5 through a50, all `#0`-signed. The shared prefix is e0 through e4 and its standing commitment is C. The owner's suffix carries e10, which reveals C, so its rank is 2. The thief's suffix carries no C-revealing event, so its rank is 3. The owner's chain wins under R6, wherever e10 sits, and the thief's head sequence 50 decides nothing. Under R8, the owner's own events e6 through e9 are voided as key-state contributors, because N = 6.
2. **A copied pre-rotation key, thief forks early.** The owner's chain is e0 through e60 with `#0` intact. The thief holds a copy of C's preimage, forks at e10 with a recovery a11 (rank 2), and appends to a50. The owner signs a commitment refresh at e61 that reveals C and carries `#0`'s signature. The shared prefix is e0 through e10. The owner's suffix carries e61, which reveals C with the shared prefix's `#0` signature, so its rank is 1. The thief's suffix carries a11, so its rank is 2. The owner wins under R6 wherever e61 sits. The refresh fixes a new commitment whose preimage the thief does not hold, so the thief cannot repeat the attempt.
3. **Two recoveries revealing the same commitment.** Both suffixes carry a recovery that reveals C, and neither carries the shared prefix's `#0` signature. The tie is at rank 2 and terminal (R7): C's preimage signed twice. This is the "`#0` compromised and pre-rotation key copied" failure mode, and the identity is unrecoverable.
4. **Two `#0`-only suffixes.** Neither suffix reveals C. The tie is at rank 3 and pending (R7). An owner holding the pre-rotation key ends it with a commitment refresh on the suffix it keeps — the two-device case. A thief holding `#0` alone cannot end it. An owner who has lost the pre-rotation key faces a permanent tie, and social recovery (§3.3) is the remaining path.
5. **A fork anchored before the latest reveal.** The owner's chain carries a reveal-authorized event r20 that reveals an earlier commitment C_prev and fixes C, and continues to e40. A party holding C_prev's preimage forks at e19 and reveals C_prev at a20. The shared prefix is e0 through e19, and its standing commitment is C_prev. The owner's suffix carries r20, which reveals C_prev; the forker's suffix carries a20, which also reveals C_prev. If r20 was a commitment refresh, the owner's rank is 1 against the forker's rank 2 and the owner wins. If r20 was a recovery, both ranks are 2 and the tie is terminal — which means the owner's `#0` and pre-rotation key were both compromised at the time of r20, the unrecoverable failure mode. R5 removes the owner's own spent copy of C_prev's key so the owner cannot produce this fork by accident; it cannot remove a copy an attacker holds.
...
- **Root key (`#0`) compromise (rare, severe):** The controller signs a recovery event (§9.7.4.2 R3) with the pre-rotation key from independent custody, carrying the compromise boundary N. The recovery supersedes any chain the attacker's `#0` signed, wherever the attacker forked (§9.7.4.2 R6), and voids the attacker's establishment events from N on the controller's own chain (§9.7.4.2 R8). The identifier does not change: no new identity is created and no forwarding record exists. The controller then completes steps 2–6 under the new `#0` and the operational keys it installs.
- **Pre-rotation key copied, `#0` intact:** The controller signs a commitment refresh (§9.7.4.2 R3) immediately. The refresh outranks any recovery the attacker signs (§9.7.4.2 R6) and fixes a commitment the attacker cannot reveal. No operational key changes, so steps 2–6 do not run.
- **Pre-rotation key taken and the controller's copy lost, `#0` intact:** The attacker's recovery stands (§9.7.4.2 failure modes). Social recovery (§3.3) is the remaining path.
- **Root key and operational key compromised, pre-rotation key held only by the controller:** As root-key compromise above; the recovery event's new `#0` rotates the operational keys in the same or a following event, and steps 2–6 run.
- **All keys compromised, or `#0` compromised with the pre-rotation key copied or lost:** Social recovery via trusted contacts with admin roles removing and re-adding the member under a new identity (§9.7.4.2 failure modes).
...
| `"SCP-MIGRATION-V1:"` | DID migration proof — **retired**: no migration proof exists in the key-event-log model, because a root-key change is a recovery event signed under `"SCP-KEL-EVENT-V1:"` | §9.7.4.2 R13 |
| `"SCP-KEL-EVENT-V1:"` | Key-event signature preimage (inception, operational-key rotation, key-status assertion, witness-set update, commitment refresh, recovery); the first field after the separator is the event-type discriminator byte | §9.7.4.2 R13 |
| `"SCP-PREROTATION-COMMITMENT-V1:"` | Pre-rotation commitment over the pre-rotation public key — a commitment-hash domain, NOT a §9.5.1 signature-preimage separator | §9.7.4.2 definitions, R13 |
| *(identifier derivation)* | **Pending.** Whether the inception-derived identifier is the inception event's canonical digest under `"SCP-KEL-EVENT-V1:"` or a distinct identifier-construction prefix is fixed by the substrate byte-layout amendment (Track U4 of the identity-substrate execution plan); no separator is registered for it until then | §9.7.4.2 R13 |
