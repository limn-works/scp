...
**No relay is trusted.** The log authenticates the record under §9.7.4.2 R2 and R3, so a resolver's integrity guarantee never rests on which relay served the record. Freshness at first contact comes from the two distinctly-operated relays R11 requires; freshness afterwards comes from the accepted baseline R12 holds against a candidate and from the freshness bound the witness layer's cosigned heads carry (§9.7.4.2 definitions, §9.7.4.3).
...
4. **Approved custody methods.** Under the key-event log a copy of a pre-rotation key alone does more lasting damage than a copy of the root alone: a root leak is recoverable by a reveal, while a leaked pre-rotation key lets its holder contest the identity at any later date and, held exclusively, take it over and then kill it permanently (the failure-mode table below). The pre-rotation key is therefore the backstop against root compromise and at the same time the key whose leak is least recoverable, and its custody is ranked on two axes with **post-spend destructibility** as the primary one: whether the controller can destroy every copy of a spent key (§9.7.4.2 R5) and, where the substrate supports it, obtain the §9.15 attested destruction; and resistance to theft at rest. Any one method is sufficient **subject to the recovery-authority-residence requirement of item 3a**. Every method MUST hold an Ed25519 key that signs the raw §9.5.1 preimage of a key event (§9.7.4.2 R3). A WebAuthn/FIDO2 authenticator signs only `authenticatorData || SHA-256(clientDataJSON)` under its own counter, and Apple's Secure Enclave and Android StrongBox hold P-256 keys only, so none of those substrates can produce the signature R3 requires, and this table names none of them.
   ...
   **A non-destructible sole custody has a failure mode the witness layer mitigates conditionally, and the condition is stated here rather than assumed.** When the controller cannot destroy every copy of a spent pre-rotation key, any copy that later reaches any party lets that party fork before the event that spent the key and reveal the same commitment. **The witness layer covers that fork for an identity that a recognized threshold witnessed past the fork position and that remains witnessed when the fork is presented** (§9.7.4.3): every honest witness of the standing set refuses the fork under its consistency check, the fork's suffix reaches no accountability threshold, and §9.7.4.2 R6 never ranks it, so no tie forms. **The failure stays terminal for an identity in direct mode, for an identity whose witnessing has lapsed past the freshness bound, and for an identity a threshold of whose designated witnesses the forking party also corrupted**: both suffixes are then rank 1 and R7 makes the identity contested and terminal by key material at any later date. The SDK MUST state that outcome at custody selection, in those terms, whenever the user selects a method the table marks not destructible or destructible only by destroying every physical copy. This section discloses the choice and does not forbid it: a controller MAY select a non-destructible method as its sole custody, having read the disclosure.
...
**Failure modes.** Each row names the keys the attacker holds and the outcome the rules of §9.7.4.2 produce. Two parties who each hold enough of the next set to reveal can each reveal, and key material cannot tell their events apart, so what decides whether the two events ever meet in a rank comparison is establishment: against a witnessed identity the fork reaches no accountability threshold and is never ranked (§9.7.4.3), and against a direct-mode identity, a lapsed one, or one whose witness set the forking party also corrupted at threshold, those rows are contested and stay contested (§9.7.4.2 R7). Where a row says the root cannot be recovered by key material and the fork established, the person establishes a new identity and each context's admins remove the old identity and admit the new one — after confirming the head from a relay in the fallback set (§9.7.4.2 definitions), which the hostile identity's service record does not name (§9.7.4.2 R4). The social recovery of `03-identity.md` §3.3 does not apply to those rows, because §3.3 re-establishes custody of the same identity. Rank numbers refer to §9.7.4.2 R6. "P" below means enough of the next set to reveal. **A 1-of-1 next set is the single-custody-loss configuration:** for a personal identity P is one key, so one loss of one key hands a party everything every P row below needs. A next threshold of two or more, with the members in independent custody, raises the precondition of every one of those rows from one loss to that many, and §9.7.4.2 R3 already permits it — `MAX_NEXT_SET_SIZE` is 16 and the only bound on the threshold is that it be at least the standing root's threshold. This sentence states what an existing mechanism costs and restricts nothing. **Every row below that names a fork behind a spent commitment additionally requires the forking party to reach the accountability threshold of the standing witness set from operators the relying party recognizes** (§9.7.4.3); against a witnessed identity the fork is pending rather than ranked, and the rows state the outcome for an identity in direct mode or one whose witnessing has lapsed.
...
- **P copied, root intact and trusted.** Both parties can reveal. Against a witnessed identity the copy holder's fork sits behind a head the standing witness set already cosigned, so it reaches no threshold, stays pending, and is never ranked (§9.7.4.3); the controller's own chain keeps establishing and the identity keeps working. Against a direct-mode or lapsed identity both suffixes are rank 1 and the identity is `Contested`, terminal by key material (§9.7.4.2 R7), and §9.11 sets the identifier `PendingReverify` at every party. An intact root does not contain that outcome: the root's co-signature on the controller's `CommitmentRollover` is evidence and never a rank (§9.7.4.2 R6, Pin A). The rollover fixes a commitment the attacker cannot reveal, which stops the attempt from being repeated on the controller's suffix; it does not win the contest already standing.
...
- **Root compromised, owner recovered, and a backup of a spent pre-rotation key later reaches an attacker — whether or not the attacker also holds the superseded root.** The attacker forks before the event that spent the key and reveals it again. Against a witnessed identity every honest witness refuses the fork and it is never ranked (§9.7.4.3); against a direct-mode or lapsed identity both suffixes reveal the shared prefix's standing commitment, so the identity is contested at any later date. Destruction that reaches every backup medium (item 4's destructibility axis, §9.7.4.2 R5) is what prevents the contest from arising. §9.7.4.2 R5 destruction and item 5f destroy, and item 5d verifies, only the controller's own copies.
...
This section states how an identity's root recovers from compromise and how a verifier decides between two key-event chains that diverge from a shared prefix. §9.7.4.1 supplies the recovery key's custody rules; this section states the events that consume that key and the rule that ranks competing chains. Every statement below is a verifier obligation or a controller obligation. The identity spec and the identity ADRs cite this section and do not restate its rules. **The rules of this section decide an event's acceptability from key material together with the threshold witness cosignatures §9.7.4.3 specifies.** Validity under R1 through R4 is a function of signed bytes alone. Establishment (definitions below) is a function of signed bytes, of the relying party's own recognized set, and of the wall clock, which enters through the freshness bound and nowhere else. Two parties whose clocks differ therefore compute different established heads, and the divergence runs one way: a party whose clock is further ahead treats more cosignatures as stale, so it establishes less and never more, which is the fail-closed direction. §9.14 governs clock skew for the rest of the protocol.
...
- A **witness** is a relay an identity's key state designates to cosign that identity's log head. A **cosigned head** is one witness's signed statement that it held this identity's chain at one head and its service record at one sequence at one moment; a **conflict statement** is one witness's signed statement that two chains for one subject were offered to it. §9.7.4.3 states both objects' fields, their signature preimages, their addresses, and the five checks a witness applies before it cosigns; this section reads them and restates none of it.
- A relying party's **recognized set** is the list of operator identities it treats as independent of the identities it resolves. The set is the party's own policy, held locally, and it defaults to the community relay list above. It is never derived from any suffix's own key state, because that suffix's author chose what its key state names.
- The **standing witness set** at an event is the witness set and the accountability threshold carried by the latest state-carrying event **at or before that event's predecessor**. The inception event has no predecessor, so its standing witness set is the set the inception itself carries (R2). **This is the prior-set rule**, and it takes the shape R4 already uses for the threshold a reveal is checked against, for the same reason: an event checked against the witness set it itself names would let a party holding the authorizing key name three witnesses it operates, collect three cosignatures, and establish its own takeover, so the check would test nothing. §9.7.4.3 states the one bounded exception, which covers a controller whose designated set is gone.
- A cosigned head is **fresh** at a relying party when `now − observed_at ≤ WITNESS_STALENESS_INTERVALS × witnessing_interval` (§9.18.17), where `witnessing_interval` is the value the standing key state carries, and when `observed_at` sits no further in the future than §9.14's clock-skew tolerance. Freshness is a property of one cosigned head at one moment and enters no other definition below.
- An event is **established** at a relying party when that party holds, for at least `t` distinct witnesses that are members of both the event's standing witness set and that party's recognized set, a cosigned head over this chain at a sequence at or above the event's own sequence, where `t` is the accountability threshold of the event's standing witness set. **A chain's established head** at that party is the highest sequence `h` such that every event of the chain at sequence `h` or below is valid under R1 through R4 and established. Every event above `h` is **pending**: the party retains it, derives no key state from it, and ranks nothing by it. **An unestablished event is never `Invalid`** (R14): `Invalid` names an author-attributable defect in signed bytes and is terminal, while establishment is a property of evidence a party may acquire later, so classifying a missing cosignature as `Invalid` would make a verdict that reverses itself.
- **Establishment decides what a party derives and ranks; freshness decides whether that party treats the result as current evidence, and the two are separate tests.** A party derives the key state at its established head under R8 whatever the age of the cosignatures that established it. **An identity is lapsed at a party when no cosigned head that establishes that party's established head is fresh**: the party keeps that key state and keeps verifying content against it, and §9.11 sets `ContinuityStanding::Unresolved` with stale witnessing as the cause, which closes that section's admission and grant gate. **A historical position needs establishment and never freshness.** A party verifying an artifact under §9.7.1's log-anchored evidence class needs the position that artifact's `key_state_head` names to be established alone, so cosignatures of any age establish a historical position — without which an offline importer verifying a context export (`23-sync-and-offline-strategy.md` §23.16.8) could verify nothing, holding no fresh cosignature by construction.
- **Direct mode** is the state of an identity whose key state carries an empty witness set and an accountability threshold of zero. Under direct mode every valid event is established, so a direct-mode identity's established head is its chain head and every rule below behaves as it would with no witness layer. §9.7.4.3 states when an identity is in direct mode, what the SDK does about it, and what such an identity gives up.
...
**R2 — Inception fixes the first commitment and the identifier is bound to the chain.** The inception event installs the first root set and threshold, fixes the first commitment list and next threshold, and carries the initial key state. Its root signature verifies against the root set the inception itself installs — the one event whose signers are named by the event they sign. A verifier MUST reject an inception event that fixes no commitment. A verifier MUST recompute the identifier from the inception event (R13) and MUST reject a chain whose recomputed identifier differs from the identifier under which the chain was served. **The inception event's standing witness set is the set the inception itself carries, at the accountability threshold it carries**, which is the one position the prior-set rule cannot reach because the event has no predecessor. The inception still needs that threshold of cosignatures to be established, so an identity whose inception no witness has cosigned is pending at every relying party (definitions), and §9.7.4.3 states what the SDK's identity-creation flow does about it. Rationale: without the recomputation a relay could serve any self-consistent chain, including one whose inception is the attacker's own, under any identifier.
...
- **a witness set that is empty while the accountability threshold is nonzero, or nonempty while the threshold is zero.** Rationale: the two fields declare one choice, and a state that mixes them names a mode no rule interprets;
- **an accountability threshold `t` and a witness set of size `n` where `2t ≤ n`. This is the majority bound, and it is the criterion that makes every duplicity attributable to a named witness.** Where two divergent suffixes are both established, each drew `t` cosignatures from the same `n`-member set, and `2t > n` forces at least one witness into both quorums by the pigeonhole principle, so a relying party holding both suffixes names a witness that signed two divergent heads. Under `2t ≤ n` two disjoint quorums exist and a contested identity would name nobody. The bound costs availability — a controller needs more than half its designated set live to publish a key event at all — and this section buys attributability with that cost;
...
**R6 — Fork precedence over established, valid suffixes.** A verifier first evaluates validity (R1–R4) over each chain in full and establishment (definitions) over each divergent suffix. A chain invalid at any event is `Invalid` and is not ranked. **A suffix carrying no established event is neither `Invalid` nor ranked**: the verifier retains it as pending, and R14's `Inconclusive{NotEstablished}` is what it reports where no suffix of the identity carries an established event. The rank comparison below is unchanged in every respect, and so are Pin A and Pin B. When a verifier holds two valid chains that diverge from a shared prefix, it MUST rank them as follows. Let C be the standing commitment at the shared prefix. On each divergent suffix, the verifier locates the first event that reveals C; at most one such event exists per suffix, because R1 makes C unique on the shared prefix, a reveal consumes it, and R4 forbids fixing it again. The verifier ranks each suffix:
...
**Establishment is what keeps most reveals of one commitment from ever meeting in this comparison.** A witness refuses to cosign a chain that does not carry the head it last cosigned (§9.7.4.3), so a party that forks behind an established head reaches no threshold, its suffix stays pending, and it is never ranked. **A relying party may not decline that requirement for an identity that declared a witness set**, because establishment is a property of the event and not a policy a party applies to it; what a party decides is which operators its recognized set carries (definitions). A tie that survives to this comparison therefore means something narrower than it once did, and R7 states what.
...
**The retained set holds two further object kinds, and both stay bounded by construction.** A verifier retains, per retained suffix, at most one cosigned head per witness of that suffix's standing witness set — the highest sequence it verified from that witness on that suffix — and at most one conflict statement per (witness, offered head digest) pair. `MAX_WITNESS_SET_SIZE` × `MAX_RETAINED_SUFFIXES` (§9.18.17) bounds each product at 256 objects per identity, which is 71,680 bytes of cosigned heads and 63,488 bytes of conflict statements at the fixed widths §9.7.4.3 states, so this rule needs no constant of its own.

**Why a flood of slots takes no identity.** Every event a slot holds stands under a root the relay verified from the inception event forward. A party that opens many slots without reaching the accountability threshold on any of them has produced only pending suffixes, which no relying party derives a key state from, ranks, or acts on, and which the eviction rule below discards first; a party that opens many established slots has already corrupted a majority of the identity's designated witness set, so the identity is contested by R7 whatever the relay stores. A flood of slots is therefore a symptom of an already-contested or already-taken identity, never a way to take an intact one.
...
| cannot reach its designated witness set at threshold, and its identity has therefore lapsed past the freshness bound at relying parties | `RootRecovery` carrying a fresh witness set, which §9.7.4.3's escape establishes under the set the event itself carries |

A reveal-authorized event puts its suffix at rank 1 against any divergence whose shared prefix's standing commitment it reveals (Pin B); the choice among rows is a choice of what the event does to the key state and what evidence it carries. The SDK MUST treat any reveal-authorized event on its identity's chain that it did not sign, and any root-signed event it did not author, as evidence of compromise and alert at maximum severity; it MUST be able to extend a suffix the controller authored even after its own verifier adopted or contested a competing chain — the SDK extends its own suffix as the controller and, as a relying party on its own identity's state, holds the `Contested` verdict like any other party; and when its own identity is contested it MUST tell the human which operators of the standing witness set cosigned both suffixes, which R7's verdict names. **A controller's device that finds its own event pending, while a different event of that identity is established at the same sequence, MUST discard its own event, fetch the established chain, and re-sign its intended change on top of the established head.** Because a `KeyState` carries a complete key state (R8), re-signing costs one event and loses nothing, and the obligation is what makes two devices racing at one sequence resolve without a tie. A controller SHOULD roll over on suspicion, not on a schedule: every rollover mints a spent key whose later leak contests the identity, so a schedule multiplies that exposure with no threat-driven gain.

*Self-observation.* The SDK MUST fetch its own identity's log at a stated cadence — no less often than the witnessing interval the key state names, and on every launch — from at least one relay in the fallback set (definitions), and MUST treat an inability to complete that check as an alert, not as silence. **On its own chain the controller reads the accountability threshold as every relying party does:** it treats its own head as established once it holds at least that many cosignatures for it from the witness set its key state names, and it alerts the human while it holds fewer. That alert is what tells a controller its witnessing has lapsed, which is the condition §9.7.4.3's witness-set-replacement escape turns on, so a controller that ignores it is the controller whose identity that escape makes takeable. The controller changes both cosigning parameters by a `KeyState` (R8), so neither value is fixed for the life of the identity.
...
**R14 — The verdict taxonomy.** A resolution returns exactly one of the following, and every rule above cites its variant; every SDK binding carries the six names unchanged. A resolution either delivers a verified key state at or newer than the accepted head, or it does not; `Invalid` and `Contested` reject on their own terms. `Confirmed{pending_beyond}` — the candidate is the accepted head (R12). `Adopted{head, baseline_decreased, pending_beyond}` — the verifier adopted a new head, on the accepted chain or by R6, and the flag says whether the accepted sequence decreased (R12). **`pending_beyond` names the lowest sequence of a valid but unestablished event the verifier holds on the adopted chain, and is absent where it holds none**; a consumer that ignores the field behaves as it would with no witness layer, and a controller's own SDK reads it and alerts, because a pending event on its own chain is either its own unwitnessed publication or another party's. `Contested{tie: TerminalByKeyMaterial | Pending, heads, shared_prefix_head, fork_position, residue, equivocating_witnesses}` — equal rank under R6, with the tie class, the digests of every tied head the verifier retained, the shared prefix's head digest, the fork position (R7), the count of rank-1 candidates the retention bound left unretained (R9; zero in every case but a rank-1 flood), and the identifiers of the witnesses whose cosignatures appear on more than one tied suffix together with the digests of their conflicting cosigned heads (R7). `Inconclusive{cause: MissingEvents{range, failed_sources} | CopyDefect{at_event} | SingleSource{reached_sources} | NotEstablished{key_valid_head, witnesses_held, threshold}}` — the resolution delivered no key state the verifier may act on, for the named cause: the events linking a candidate to the accepted chain were not served, a served copy failed verification as a copy-level defect (R3), a first contact reached fewer than the two relays R11 requires (R11), or the verifier holds a valid chain no event of which reached the accountability threshold from witnesses its recognized set carries (definitions). **`NotEstablished` is the fail-closed direction and is where a never-witnessed identity, an identity whose declared witnesses the party's recognized set carries none of, and an identity the party holds no cosignature for at all land**; the SDK reports the cause so the human reads which of the three it met. **An identity that once established and has since lapsed does not land here**: the party holds an established head, returns `Adopted` or `Confirmed` for it, and §9.11 records the lapse as the standing (definitions). `Invalid{at_event}` — the chain fails R1–R4 at a named event by an author-attributable defect (R3); this is the only cause, a copy-level defect never produces this verdict, and **an unestablished event never produces it either** (definitions). `Discarded{accepted_head}` — the candidate is a head of the accepted chain at a sequence strictly lower than the accepted head, which the variant names; the verifier changed no stored state (R12). A discard is a verdict so that a consumer routes it, and §9.7.1 routes it as no refresh of the key state: it is not the success branch, because the resolution delivered nothing newer than the baseline the verifier already held. The contested verdict, the retained suffixes it rests on, and the accepted-head baseline MUST survive the verifier's restart and MUST be persisted before any verdict is acted on; a persist failure returns an error and not the verdict.
...
1. **A root thief forks early; the owner's device holding R0 is gone.** The owner's chain is e0 through e9 (R0-signed `KeyState` events). The thief, holding R0, forks at e5 and appends a5 through a50, all R0-signed. The owner no longer holds R0, so it signs a `RootRecovery{Lost}` at e10 that reveals C, installs a fresh root K′, carries a complete key state, and fixes C′. The shared prefix is e0 through e4 and its standing commitment is C. The owner's suffix reveals C, so its rank is 1; the thief's suffix reveals nothing, so its rank is 2. The owner's chain wins under R6 wherever e10 sits, and the thief's head sequence 50 decides nothing. Under R8 the derived key state after e10 is e10's snapshot, so nothing the thief installed survives. The thief cannot evict the recovery by appending, because R9 never evicts a rank-1 suffix; nor can it make the recovery unpublishable by inflating its own chain, because the owner's segments attach by predecessor digest to e4, which every relay holding either chain already has. **The scope of that guarantee is a validating relay**: a non-validating relay and a foreign transport deliver availability and no eviction resistance, so a verifier querying only those may receive one chain.
2. **A copied pre-rotation key, the thief forks early.** The owner's identity designates five community witnesses at an accountability threshold of three, which satisfies R3's majority bound, and the relying party's recognized set carries all five. The owner's chain is e0 through e60 with R0 intact and trusted. Each witness has cosigned heads along the owner's chain at every witnessing interval, so each holds a last-cosigned head well above sequence 10. The thief holds a copy of P, forks at e10 with a `RootRecovery{Lost}` a11, and appends to a50. Every honest witness applies §9.7.4.3's third check: the offered chain carries a11 at sequence 11, and the witness's own last cosigned head is an event of the owner's chain at a higher sequence the offered chain does not carry. Each refuses and emits a conflict statement naming the owner's head as held and a11 as offered. The thief's suffix therefore carries no cosignature from the standing set, carries no established event, and is never ranked. The verifier returns `Adopted{head: e60, …}` and the relying party sees up to five signed conflict statements naming a fork attempt at sequence 11. **What the attacker must do instead:** obtain the accountability threshold of the standing witness set — three of the five named operators — before any of them cosigns a head above sequence 10, because after that the consistency check refuses. The witnessing interval bounds how long the chain goes uncosigned, so that window is one interval wide at the fork position and it closed long before the attacker acquired the spent key.
3. **Two root-only suffixes.** The owner's two devices both hold R0, under the same five-witness three-threshold configuration. Device A appends a `KeyState` e10a and device B a different `KeyState` e10b. Each witness cosigns whichever of the two it verifies first and refuses the other under §9.7.4.3's third check, emitting a conflict statement. R3's majority bound leaves no room for two disjoint quorums of size `t` inside an `n`-member set, so at most one of e10a and e10b reaches the accountability threshold: at most one becomes established and the other stays pending forever. The rank comparison never runs and neither suffix is contested. The losing device applies R10's obligation — it discards its own event, fetches the established chain, and re-signs its intended change on top of the established head — which costs it one event.
4. **A backup of a spent key later reaches the root thief.** The owner's chain carries a `RootRecovery{Lost}` r20 that reveals C_prev over P_prev and installs K′; the chain continues to e40 and every witness holds a cosigned head above sequence 19. The thief, who holds R0 by r20's own premise, later obtains a backup of P_prev and forks at e19 with a `CommitmentRollover` a20 that reveals P_prev and carries R0's signature. a20 is offered at sequence 20 on a chain that does not carry the head each witness last cosigned, so every honest witness refuses. The thief's suffix is pending, is never ranked, and produces no contest. **This is the case §9.7.4.1 item 4 calls terminal for an identity that is not witnessed**, and item 4 states the condition under which this layer covers it.
5. **An identity lapses, and its controller recovers it.** The owner's five designated witnesses go offline permanently. The owner's chain stops establishing above its last cosigned head, every relying party's freshness bound expires, and each sets `ContinuityStanding::Unresolved` (§9.11) with stale witnessing as the cause, which closes that section's admission and grant gate while leaving UCANs the identity issued acceptable. The owner then signs a `RootRecovery` carrying a fresh witness set and threshold. At a relying party that holds no fresh cosignature from any witness of the standing set, and for which the recovery's predecessor is the highest established event it holds, and which met R11's floor, §9.7.4.3's escape establishes the recovery under the set the event itself carries. Every such party sets `PendingReverify` and alerts the human at maximum severity under §9.11, and the exit is an out-of-band fingerprint comparison. **The same path is open to a holder of a spent pre-rotation key against an owner who stopped maintaining the identity**, and it is loud at every party for exactly that reason.
6. **A witness set corrupted at threshold.** The thief holds a copy of P and also corrupts three of the owner's five designated witnesses, which the relying party's recognized set carries. Those three cosign both the owner's chain and the thief's fork, so two divergent suffixes are both established and both rank 1. R7 returns `Contested{tie: TerminalByKeyMaterial, …, equivocating_witnesses}` naming the three operators that appear on both suffixes, together with the digests of their conflicting cosigned heads. The verdict is terminal by key material, §9.12 states the controller's remedy, and the named operators are publicly accountable. This is the residual §9.7.4.3 states first.

### 9.7.4.3 Witness and Watcher Layer (Layer B)

**This section carries the witness protocol, and a threshold of witness cosignatures is a condition of an event's acceptability.** An event that a threshold of the identity's designated witnesses has not cosigned derives no key state, wins no fork comparison, and enters no verdict a relying party acts on. §9.7.4.2's definitions state what establishment is, and each rule of that section states how it reads establishment; this section states the two objects the layer produces, what a witness does before it signs one, which cosignatures a relying party counts, how an identity incepts and how one operates with no witnesses at all, how a controller replaces a witness set, and what the layer leaves uncovered. ADR-063, the inception-derived key-event-log identity substrate, settles the model this section states.

**The requirement lands in acceptability and never in rank.** R6 already refuses to rank a chain it found invalid, and establishment is the second reason a suffix is never ranked, so Pin A survives verbatim: rank still reads one property of a suffix, and the set of suffixes that reach the rank comparison narrows. **A relying party may not decline the requirement for an identity that declared a witness set**, because establishment is a property of the event rather than a policy a party applies to it. What a party decides is which operators its recognized set carries, and whether it deals with an identity that declared no witness set at all.

**The role split, and where a watcher's function lives.** A witness answers to the controller that designated it in that controller's own signed key state; a relying party answers to itself and counts only the operators its own recognized set carries. The comparison a separate watcher role would perform — holding two divergent suffixes side by side and naming the divergence — is R9's retention and R7's verdict at the relying party, so this layer defines no watcher pool, no watcher object, and no out-of-band introduction channel.

**The cosigned head.** A cosigned head is one witness's signed statement that, at one moment, it held one identity's key-event chain at one head and that identity's service record at one sequence. Its fields, in preimage order:

| Field | Width | Meaning |
|---|---|---|
| `witness` | 32 bytes | The witness operator's inception-derived identifier (R13) |
...
| `service_record_sequence` | 8 bytes, big-endian u64 | The sequence of the service record the witness held, or zero where it held none |
| `service_record_digest` | 32 bytes | The §9.5.1 preimage digest of that service record, or the all-zero placeholder where the witness held none |
...
The signature preimage is `SHA-256("SCP-COSIGNED-HEAD-V1:" || witness || witness_key_state_head || subject || sequence || event_digest || service_record_sequence || service_record_digest || previous_cosigned_digest || observed_at)` under the §9.5.1 construction. Every field is fixed-width, so no field carries a length prefix. The signature is one Ed25519 signature by the key the witness operator's key state lists `current` in the `#active` role at the position `witness_key_state_head` names. The object is 216 preimage bytes and one 64-byte signature, 280 bytes in all.

**One cosignature covers every event at or below the head it names.** Each key event's preimage binds its predecessor's digest (§9.7.4.2 definitions), so the head's digest determines the whole chain back to the inception event, and no second chain reaching that head exists without a SHA-256 collision. That is why a witness cosigns a head on an interval rather than receipting each event, and it is why one cosignature carries freshness, anti-rollback, and split-view detection for the whole chain at once.

**Where the two objects live.** Cosigned heads are addressed at `wit_routing_id = SHA-256("scp:wit:" || subject_identifier_bytes)` and conflict statements at `SHA-256("scp:wcf:" || subject_identifier_bytes)`, two further routing derivations beside `scp:did:` for the key-event record (R13) and `scp:svc:` for the service record (`03-identity.md` §3.10.13). The address is the type discriminant, which is the rule §9.10.12 states, so no decoder reads a cosigned head as a key-event frame. §9.10.12 states the two frames' bytes.

**What a witness verifies before it signs.** A witness applies five checks in order and signs only where every one passes.

1. **Chain validity.** The chain the witness holds for the subject recomputes to the subject identifier under R2, and every event verifies under R1 through R4. A witness that cannot verify a chain does not cosign it.
2. **Designation.** The key state derived at the head names this witness in its witness set. A witness that is not designated refuses, because a cosignature no key state names counts toward no threshold and because signing for undesignated subjects is an unbounded workload.
3. **Consistency with its own last cosigned head.** The witness looks up the head it last cosigned for this subject, and the chain it is now asked to cosign MUST carry that exact event digest at that exact sequence. Where the chain does not, the witness refuses and emits a conflict statement. **This check is the security property of the layer**, and it is what a party holding a spent pre-rotation key must overcome, which means corrupting a threshold of named third parties rather than obtaining one key. SCP's log is a hash chain rather than a Merkle tree, so the chain segment from the previously cosigned sequence to the new head is itself the consistency proof and the witness verifies it by walking predecessor digests; this layer therefore needs no Merkle inclusion proof and no Merkle consistency proof.
4. **Service-record monotonicity.** Where the witness holds a service record for the subject, it verifies that record's signature against the key the head's key state designates (`03-identity.md` §3.10.13), and it refuses a record whose sequence is below the `service_record_sequence` of its own last cosigned head under the same designated key bytes. This is what the service record's inheritance of this layer amounts to.
5. **Its own clock is monotone.** The witness refuses to stamp an `observed_at` at or below the `observed_at` of its own last cosigned head for the subject.

**The witness's durable per-subject state, and what it does when it loses it.** A witness retains, per subject it has ever cosigned, the sequence, the event digest, the service-record sequence, and the `observed_at` of its own last cosigned head, which is about 60 bytes, so a witness serving ten million identities holds under a gigabyte. **A witness that has lost that record for a subject MUST NOT cosign that subject until it rebuilds the record.** It rebuilds by fetching the other designated witnesses' latest cosigned heads for that subject from the subject's `scp:wit:` address, verifying them, and taking the highest sequence among those it verified as its own floor; where it can reach no other designated witness it refuses and reports that it refuses. A witness with amnesia is a witness that will cosign a fork, which is why `17-persistence-and-storage.md` §17.17.4 classifies an in-memory witness store as a security nullifier rather than as a durability-only affordance.

**The conflict statement.** When a witness refuses under check 3 it emits a conflict statement, which is this layer's fault proof. Its fields, in preimage order:
...
**A conflict statement is portable evidence that two chains for one subject were offered to one named witness, and it is not a verdict.** The witness makes no claim about which chain the controller authored. A relying party that holds a conflict statement together with the two chains reads the divergence the statement names and applies R6 and R7 to the chains themselves. The statement exists rather than a silent refusal because a refusal that produced no object would leave duplicity as the witness's private knowledge, and the statement is what lets a relying party learn that a party attempted a fork without being online at the moment of the attempt.

**Two alternatives to the refusal, each rejected.** **A witness adopts the later chain and re-cosigns** — rejected, because a witness that can be talked out of a head it already cosigned supplies no non-equivocation at all: an attacker would ask twice. **A witness decides which chain the controller authored** — rejected, because a witness holds no evidence that distinguishes two chains both signed by one set of key material, so its signature attests what it saw and when and attests nothing about authorship.

**Which cosignatures a relying party counts.** A relying party counts a cosignature only where its witness sits in both the subject's standing witness set and that party's own recognized set (§9.7.4.2 definitions). **The subject's accountability threshold governs, and the recognized set filters which cosignatures count toward it.** A relying party MUST NOT demand a threshold higher than the one the controller declared, because a party that demanded more would hold an identity to a bar its controller cannot read from its own key state; a party MAY decline to recognize an operator, because whose observation it trusts is its own judgement and no rule here takes it. **A cosignature from an operator the running SDK does not list counts toward nothing, and it is not an error.** A party's recognized set is what its own release carries (`18-addressability-and-deployment.md` §18.5.1), so the party leaves such a cosignature out of the threshold count, reads the rest of the chain unchanged, and reports no defect: the object is a valid signature by an operator this party did not choose to recognize, and it attributes nothing to the subject. **A release boundary may therefore change what a party establishes.** An identity established under one release is unestablished under another until operators the new release lists cosign it, and until they do the party returns `Inconclusive{NotEstablished{…}}` (§9.7.4.2 R14) for that identity.

**What a cosignature proves.** Producing a cosignature under an operator identity requires that operator's own `#active` key, so control of the declared operator identity is proven by construction, with no separate attestation to check and no self-asserted field to trust. This is the same proof-of-control R11 already requires of a relay that serves a response, and it is why a cosigned head is signed under the operator identity rather than under a bare relay key.

**Independence of an operator from a subject's controller is not provable, and no sentence in this corpus implies a check covers it.** Nothing in the protocol proves that two operators are different parties: an inception-derived identifier is a self-signed digest, so distinct names cost one party nothing to mint. What is mechanical is a local set-membership test — the SDK ships the community relay list (`18-addressability-and-deployment.md` §18.5.1), and a relying party counts only operators its own copy of that list carries — and that test is the whole of the independence property. **Anti-self-witnessing follows from the membership test and from no ownership assertion**, because a controller's own relays are not entries of the shipped list. **A badly curated list is a supply-chain risk this protocol does not cover:** where the shipped list is curated badly, or where several entries on it are secretly one operator, nothing here detects it, and the exposure belongs to the class a compromised browser root store belongs to. The protocol supplies the enumeration of named operators and never the judgement about them.

**What a controller that names its own relays as witnesses gets.** Its cosignatures come from operators a relying party's recognized set does not carry, so that party counts none of them and returns `Inconclusive{NotEstablished{…}}` (R14), reporting the cause so the human reads that the identity's witnesses are not ones the SDK recognizes rather than that the identity does not exist. The same identity establishes at a party whose recognized set does carry those operators, which is that party's own judgement about operators it chose to add. **A self-hoster that names only its own relays is therefore unestablished everywhere by default**, and the resolution is that transport goes to the self-hoster's own relay while witnessing goes to the community set. The SDK MUST warn when a controller replaces the default witness set with operators the shipped list does not carry, and MUST state that consequence in the warning.

**Inception.** An identity's first event commits its witness set and no witness has cosigned anything yet, so an inception event is established under the set and threshold it itself carries (R2). The controller's identity-creation flow therefore ends at establishment rather than at publication: the SDK publishes the inception event, submits it to each designated witness, collects cosignatures, and reports the identity created once the threshold is met. **An SDK that cannot reach the threshold returns a typed error and creates no usable identity.** An identity nobody can establish is one nobody will transact with, and reporting success would ship the false guarantee the no-stand-ins tenet forbids. Self-witnessing at inception buys an attacker nothing, because the recognized-set test above leaves an identity witnessed by nobody a relying party recognizes unestablished at that party whatever its own key state says.

**Direct mode.** An empty witness set with an accountability threshold of zero declares direct mode (§9.7.4.2 definitions), under which every valid event is established and §9.7.4.2's rules behave as they would with no witness layer at all. The mode exists because two parties on an isolated network must still be able to create usable identities and transact. **The mode is legible from the identifier's own first event**, because the witness set and the accountability threshold sit in the inception preimage that the identifier freezes, so a first-contact resolver reads the mode before it decides anything, which serves the legibility-before-opt-in tenet mechanically. **A direct-mode identity gets nothing from this layer**, the non-destructible-custody failure of §9.7.4.1 item 4 included: a party holding a copy of its spent pre-rotation key still forks behind the spending event and produces a rank-1 tie.

**What the SDK ships.** The SDK's default witness set for a new identity is the entries of the community relay list under distinct declared operators, capped at `MAX_WITNESS_SET_SIZE` (§9.18.17), at an accountability threshold of `⌊n/2⌋ + 1`, which satisfies R3's majority bound at every set size. **Direct mode is available, is never the default, and selecting it requires an explicitly named field value rather than an omission**, because the agent-first API rule forbids a silent security default and an identity's witnessing posture is the clearest instance of one. The SDK's default recognized set is that same shipped artifact in its second role. The SDK's default posture toward a direct-mode counterparty is to resolve it, report it as unwitnessed, and route the acts §9.11's gate list withholds through the manual path rather than refusing outright, so two parties who witness nothing transact and each of them sees that it is doing so.

**A witness set that is down, partitioned, or refusing.** The chain's established head stops advancing while the chain itself grows, so relying parties keep the last established head, derive the key state from it (R8), and the identity keeps working at that key state while new key events do not take effect. **That failure runs in the dangerous direction for a rotation:** a controller rotating `#active` away from a compromised key finds the rotation ineffective while its witnesses are down, so the compromised key keeps verifying. Three things bound the damage. The freshness bound turns a lapsed identity's standing to `ContinuityStanding::Unresolved` at every relying party (§9.11), which closes that section's admission and grant gate. `MAX_WITNESSING_INTERVAL` (§9.18.17) bounds how long an identity may go uncosigned before every party treats it as lapsed. R3's majority bound means a controller loses establishment only where more than half its designated set is unreachable, and the default set is the community relay list under distinct operators. **A controller whose witnesses refuse cannot be distinguished by anyone from a controller whose witnesses are down**, because a refusal that emits no conflict statement is silence and no party compels a signature; the remedy is the same in both cases, and it is the replacement below.

**Rotating the witness set.** A `KeyState` that changes the witness set, the accountability threshold, or the witnessing interval is established under the **prior** set at the **prior** threshold, under the prior-set rule (§9.7.4.2 definitions). The outgoing witnesses cosign the event that retires them, which is safe because they cosign a chain extension and adjudicate nothing. R8 already makes a state-carrying event replace every key-state field in full, so the set changes by the event carrying new values and no diff or merge rule is needed.

**The one case the prior-set rule cannot serve.** Where the outgoing set is unreachable at threshold, refuses at threshold, or has been corrupted at threshold, the prior-set rule deadlocks: establishing the event that would replace the set requires the set it is replacing. **A `RootRecovery` is therefore established under the witness set and accountability threshold it itself carries where all three of the following hold at the relying party:**

1. **It is reveal-authorized.** A `RootRecovery` carries a reveal against the standing commitment (R3), so only a holder of the standing next set at threshold produces one.
2. **Its predecessor is the highest established event of the chain the relying party holds**, and that party satisfied R11's first-contact floor before it evaluated this condition. A party forking behind a live identity's established head fails here, because that identity's established head sits above the fork.
3. **The relying party holds no fresh cosignature from any witness of the standing set, at any sequence, on any retained suffix of this identity.** The escape is available only for an identity the party itself observes to be lapsed, and an attacker cannot manufacture that condition against a live identity, because the owner's own witnesses keep stamping fresh heads.

**The escape admits a `RootRecovery` and no other event kind, because §9.11 makes an observed root recovery loud at every party**: every relying party sets `ContinuityStanding::PendingReverify`, invalidates every recorded continuity verification, closes the admission and grant gate, and alerts the human at maximum severity, whether its verifier adopted the chain or found it contested. The exit from `PendingReverify` is an out-of-band fingerprint comparison against the current root set, which an attacker does not pass with the people who knew the owner. **What the escape costs is stated here rather than implied:** a party holding a spent pre-rotation key takes over an identity whose owner stopped maintaining it past the freshness bound, and every relying party sees a maximum-severity root change and closes its gates until a human re-verifies. The controller's countermeasure is to keep the identity witnessed, and R10's self-observation alert is what tells it that witnessing has lapsed. **The escape also costs the controller its standing pre-rotation commitment**, because a `RootRecovery` is reveal-authorized: a controller replacing a dead witness set spends one next set and generates its successor under §9.7.4.1 item 3a, and no cheaper event carries the escape, for the reason the rejected alternatives below give.

**Four alternatives to the escape, each rejected.** **Establish a witness-set change under the set it carries** — rejected outright, because a party holding the authorizing key would name three witnesses it operates and establish its own takeover. **Establish it under both the old and the new set** — rejected, because it deadlocks in exactly the same way, still requiring the old set. **Admit no escape, and leave a controller whose witnesses are gone frozen permanently** — rejected under the no-DOA-decisions tenet: a controller that designated five witnesses and lost three would be dead, which is a foreseeable single-user disaster, and the portability of an identity's trust basis is the property designated witnesses exist to deliver. **Establish any reveal-authorized event under the set it carries** — rejected, because the fork of the fourth worked example is a reveal-authorized event whose predecessor is established, so that rule would establish an attacker's takeover under the attacker's own witnesses.

**Rotating away from a set that is corrupted and still cosigning.** The third condition fails, because fresh cosignatures exist. The identity is contested under R7 with the equivocating witnesses named, and the controller's remedy is §9.12's: the person establishes a new identity, while the conflict statements and the `Contested` attribution make the failed operators publicly accountable.

**Cosigning is free.** `18-addressability-and-deployment.md` §18.5.1 states that rule beside the free-entry invariant it companions, and this section reads it. Economic rules (`19-economic-governance.md`) may price a witness's cadence and its availability guarantees; they may never price the act, because an identity that could not pay would be unwitnessable and therefore unresolvable, which would put identity itself behind the economic layer.

**What this layer does not cover, stated plainly rather than implied.**

1. **A witness set corrupted above its accountability threshold takes the identity.** Two established divergent suffixes produce `Contested`, terminal by key material, with the failed operators named (R7). The controller's remedy is §9.12's establishment of a new identity, and R7 states why no construction inside the log resolves it.
2. **A lapsed identity is takeable by a holder of a spent pre-rotation key**, under the escape above, loudly, and the controller's countermeasure is to keep the identity witnessed.
...
4. **A witness's own chain is resolved without the witness requirement.** Applying the requirement recursively has no base case, so a party verifying a cosignature resolves that operator's chain under R1 through R4 alone, and the trust anchor for a witness operator is its community-relay-list entry rather than a witnessed resolution.
5. **A direct-mode identity gets nothing from this layer**, the non-destructible-custody failure of §9.7.4.1 item 4 included.
6. **A party that silences a majority of a controller's designated witnesses freezes that controller's rotations** for as long as it sustains the silencing, which is a wider version of the publication-suppression exposure that exists without this layer.
7. **The witness's durable per-subject store is a correctness dependency** on infrastructure the protocol does not control, and the rebuild obligation above is a controller-independent mitigation rather than a proof.
8. **Wall-clock time enters acceptability** through the freshness bound (§9.7.4.2 definitions), which states the direction of the resulting divergence and why that direction is fail-closed.
...
**A relay that witnesses is accountable, not trusted, and the untrusted-relay invariant holds over it unchanged.** A relay an identity designates in its key state cosigns that identity's log head (§9.7.4.3), and every relying party verifies that cosignature itself against the operator's own key-event log rather than taking the relay's word for anything. A witness that cosigns two divergent heads is proven to have done so by the two objects it signed, so its equivocation is attributable to it by name and a relying party learns of it through R7's `Contested` payload. A witness supplies no confidentiality property, reads no encrypted content, and decides nothing about which chain a controller authored; what it supplies is one signed observation of what it held and when.
...
COSIGNED-HEAD-RECORD (CosignedHeadRecordV3) :=
  ...
  value:      [u8]                     # trailing remainder = one or more 280-byte cosigned heads (§9.7.4.3)
...
- **`Unresolved` means nothing was learned about the identifier, not that the identifier is suspect.** It clears on any later resolution that adopts a chain, with no human step, because a verifier that reached one relay learned nothing about the identity that a verifier reaching two would have contradicted. **Two causes set it: a resolution that reached too few sources, and an identity whose established head carries no fresh cosignature** — a witnessing outage or a lapsed witness set (§9.7.4.3), which R14 reports as `Inconclusive{NotEstablished}`. The second cause reads the same way as the first, and for the same reason: a witnessing outage is no evidence against the tokens the identity issued, so withholding them would reach every token the victim ever issued on the strength of a relay outage.
...
- **A resolved contest clears the standing in one case and no other, and this bullet is the one home of the clearing rule.** When R6 ranks one suffix above the other and the verifier retires the rank-2 equivocation record under §9.7.4.2 R7, the clearing rule reads that retirement. **The standing stays `PendingReverify` where the winning suffix carries a `RootRecovery` the verifier had not already trusted on first use**, and the verifier records the winner's root-install digest; the trigger is stated over the adopted chain, because a first encounter holds no prior digest and a trigger stated as a digest mismatch could never fire for it — which is the reading that hands a takeover a clean `Verified`. Where the winning suffix carries no `RootRecovery`, the verifier restores the standing the identifier held before the contest, and where the contest was the identifier's first encounter it takes the standing trust-on-first-use would have set for the chain the resolution then adopts (§9.6.4), recording that chain's root-install digest at the same moment. **An escalation clears nothing.** Where the rank-2 record is retired because the divergence escalated to a rank-1 tie, the standing stays `PendingReverify` under the new `Contested` verdict, which is terminal by key material (§9.7.4.2 R7): a rank-2 divergence escalating to rank 1 is the takeover completing. The two-device race of §9.7.4.2's third worked example therefore unblocks itself.
...
- **Pre-rotation key copied, root intact and trusted:** The controller signs a `CommitmentRollover` (§9.7.4.2 R3) immediately. The attacker can reveal the same commitment. **Against a witnessed identity the attacker's fork sits behind a head the standing witness set already cosigned, so it reaches no accountability threshold, stays pending, and is never ranked** (§9.7.4.3): the controller's own chain keeps establishing and no contest arises. Against a direct-mode identity, or one whose witnessing has lapsed past the freshness bound, both suffixes are rank 1 and the identity is `Contested`, terminal by key material (§9.7.4.2 R7), and every peer holds the identifier at `PendingReverify` (§9.11). The rollover fixes a commitment the attacker cannot reveal, which stops the attempt from being repeated; it does not win the contest. The root's co-signature is evidence a human weighs out of band (§9.7.4.2 R6, Pin A) and never a rank: a rule that let a co-signature confer rank would hand a thief holding both the root and a copied pre-rotation key the win over a controller who lost its root and holds the genuine key, so the two-value rank keeps that case contested. No operational key changes, so for a rollover that fixes a commitment steps 1a–6 do not run; an abandoning rollover runs step 2 before it is published (§9.7.4.2 R4).
...
- **Pre-rotation key copied, root lost:** The controller signs a `RootRecovery{Lost}`; the attacker can too. Against a witnessed identity the attacker's fork establishes at no party and the controller's recovery is adopted. Where the identity is in direct mode, or its witnessing has lapsed, or the attacker also reached the accountability threshold of the standing witness set, both suffixes are rank 1 and the identity is `Contested` (§9.7.4.2 R7). Step 1a proposes Remove for no leaf while the contest stands, because R7 gives no party a `current` key state to test leaves against, and steps 2–6 do not run while it stands.
...
- **Root compromised with the pre-rotation key copied:** both parties can reveal, so the identity is contested (§9.7.4.2 R7). In every row where the root cannot be recovered by key material and the witness layer did not keep the attacker's fork pending, the person establishes a new identity, and each context's admins — after confirming the head from a relay in the fallback set, which the hostile identity's service record does not name (§9.7.4.2 R4, definitions) — remove the old identity and admit the new one; §3.3's social recovery does not apply, because it re-establishes custody of the same identity.
- **The controller's own identity is contested:** the SDK keeps extending the controller's suffix (§9.7.4.2 R10) and tells the human which operators of the standing witness set cosigned both suffixes, which R7's verdict names; it does not sign a further reveal, because both suffixes have already consumed the standing commitment.
...
**1b. Witness submission.** After the controller has published a key event and durably retained it, it submits that event to the witness set its snapshot names and re-submits until the accountability threshold is met (§9.7.4.2 R10, §9.7.4.3). **Until the threshold is met the event is pending at every relying party**, so a `KeyState` retiring a compromised `#active` has not taken effect anywhere and the compromised key keeps verifying; the SDK alerts the human while it holds fewer cosignatures than the threshold. Witness submission gates neither the destruction of spent keys nor the completion of R10's ceremony. **Where the identity's designated witness set is itself corrupted at threshold and still cosigning**, the identity is contested with those operators named (§9.7.4.2 R7), no witness-set replacement is available to the controller (§9.7.4.3 states why), and the remedy is the new-identity path the contested rows below already state.
...
| `"SCP-COSIGNED-HEAD-V1:"` | Cosigned head — one witness's signed statement of the subject chain's head and the subject service record's sequence at one moment; a **log-anchored evidence** separator, keyed on the witness's own `witness_key_state_head` | §9.7.4.3 |
...
| `MAX_KEYS_PER_CHAIN` | 4096 | Maximum keys one chain installs; R3 rejects a state-carrying event whose snapshot lists more. R8 makes every state-carrying event list every key the chain installed, so a chain's bytes grow with the square of this count and one event's snapshot is what the `value` bound of one frame constrains. **The arithmetic, at 43 bytes per key-state entry** (a 32-byte public key, a one-byte role discriminator, a one-byte condition discriminator, the eight-byte position a `Compromised{from: N}` entry carries, and a one-byte custody type): 4096 entries occupy about 176 KB; add the witness set's worst case (`MAX_WITNESS_SET_SIZE` × 32 bytes plus a 4-byte count = 1,028 bytes), a reveal group (`MAX_NEXT_SET_SIZE` × 96 bytes = 1,536), two root-signature groups (`MAX_ROOT_SET_SIZE` × 64 bytes each = 2,048), the commitment list (16 × 32 = 512), and the index lists and sequence fields, and the largest event occupies about 181 KB inside the 262,111-byte `value` bound of one frame (§9.10.12). The frame would admit about 5,900 entries; 4096 sits below that so the ceiling is one round checkable number rather than a byte edge. **What the ceiling costs an identity:** a personal identity installs one key per rotation and reaches about 4,000 rotations; an organization whose root set holds `MAX_ROOT_SET_SIZE` members installs 17 keys per root recovery and reaches about 240 recoveries. R10 tells a controller to roll over on suspicion rather than on a schedule, which is what keeps both figures out of reach. The assembled chain then totals about 360 MB and a relay serves it as about 1,400 frames (§9.7.4.2 R9); the assembled chain has no frame bound, because R9's write rule appends segments | §9.7.4.2 R3, R9 |
...
| `WITNESS_STALENESS_INTERVALS` | 3 | Multiplier on the witnessing interval past which a cosigned head is no longer fresh (§9.7.4.2 definitions). A witness misses two consecutive rounds without the identity going stale, which is the two-fault margin the rest of the witness layer uses, and a third miss is a signal rather than noise | §9.7.4.2 definitions, §9.7.4.3 |
| Majority bound on the accountability threshold | `2 × accountability_threshold > witness set size` | A protocol invariant R3 enforces on every state-carrying event, and R3 states why: it forces at least one witness into any two quorums, so every contested identity names an operator that cosigned two divergent heads, and below it two disjoint quorums make duplicity unattributable | §9.7.4.2 R3, R7 |
