...
- **Auto-accept policies (§5.12.2)** handle the common case autonomously. For contexts matching a known template from a known identity with acceptable TTL, the SDK joins without human involvement. The human is only in the loop for novel or high-risk invitations.
- **Invitation rate limiting.** The SDK rate-limits inbound invitations per source identity and globally. An attacker flooding invitations from multiple identities is bounded by the global rate limit. Invitations that exceed the rate limit are queued (not dropped) with decreasing priority.
...
- **Capability is unduplicable, unforgeable, and opaque.** The capability proof MUST satisfy all of:
  ...
  2. **Not inspectable.** No API (including any `Debug`, `Display`, `Deref`, `AsRef`, or equivalent accessor) returns the inner identifier or its bytes in a form that allows reconstruction of the proof or its use as a lookup key elsewhere. The proof is opaque to handler code; only supervisor-module code consumes it. A borrowed identifier accessor returning a read-only reference to the token's own owning identity is permitted: it cannot reconstruct or forge the proof (there is no public conversion and no public constructor) and exposes only the actor's own identifier, never a key into another identity's state. Language-specific (Rust): such an accessor's visibility MUST be exactly `pub(in crate::context)`, the same bound as the reissue method above.
...
- **No API returns per-identity state given an arbitrary identifier.** The only supervisor API that returns per-identity state takes the capability proof as a parameter. Callers cannot pass another identity's identifier and receive that identity's state.
...
- Broadcast per-author **subscriber block** and **governance subscriber ban** (§5.14.8): each advances the per-author broadcast key epoch and adds the target identity to a block list (the ban to ALL authors' block lists, plus registry removal), revoking future key access — sync-persisted fail-closed, atomic with `read_exclusion_list`. The UNBLOCK / `RestoreAccess` direction is upward and MAY be coalesced.
...
Every signer MUST emit the low form, converting `(r, s)` to `(r, n − s)` whenever `s` exceeds half the group order, and every verifier MUST reject a signature whose `s` exceeds half the group order. The obligation binds a hardware signer too. A reveal-authorized event spends the standing commitment when the controller publishes it and R10 of §9.7.4.2 confirms it, so a signer that skipped normalization would publish an event no verifier accepts and spend the commitment for nothing.
...
An absent optional field encodes as the 32 bytes `SHA-256(0x00)` under the same rule the present form takes: length-prefixed where the present form is variable-length, bare where the present form is fixed-length. **On a fixed-length optional field narrower than 32 bytes the two forms occupy different widths and neither carries a prefix, so the preimage stays injective only while the fields that follow cannot absorb the difference.** A field added after such an optional field, or a type widened behind one, MUST therefore be length-prefixed or fixed at a width no run of following bytes can re-parse into: `Attestation` field 8, `expires_at`, is the one such field today, at 8 bytes present against 32 absent, and `revocation_status` behind it is far shorter than the 24-byte difference. The key-event preimage takes no field under this rule: the definitions of §9.7.4.2 fix a sentinel of their own for a field a kind does not carry.

Changing any field's encoding, adding a field, or removing a field requires incrementing that separator's version suffix, and every signature made under the old separator becomes invalid. The definitions of §9.7.4.2 state which rules of the key-event log rest on that cost. The rule that the key state carries no relay endpoint rests on where the retired document's contents went instead, which §18.2.2A of `18-addressability-and-deployment.md` records.

### 9.5.2 Signed Structure Definitions

**Sixteen preimages wait on the identifier's textual form.** R13 of §9.7.4.2 fixes the identifier as 32 raw digest bytes and defers its textual form. Every derivation the protocol performs consumes those 32 bytes, so the routing derivations and both continuity-fingerprint forms of §9.11 are settled. A preimage that takes an identifier as UTF-8 bytes is not yet computable, and a reader MUST NOT treat one as computable until a later revision of R13 fixes the encoding. Fourteen of the sixteen sit below in this section: `InnerEnvelope`, `BroadcastEnvelope`, `SenderKeyRequest`, `SenderKeyEpochAdvance`, `AccessKeyRequest`, `Attestation`, `KeyPackageAttestation`, `KeyDestructionAttestation`, `BlockNotification`, the governance-proposal id, `SignedVote`, `ParticipationProfile`, `ScpCustodyViolationAttestation` and `CounterAttestation`. Two sit outside it. The first is the `"SCP-DID-AUTH-V1:"` challenge of `03-identity.md` §3.11.3, whose reconstruction step §3.11.9 states and whose three public signatures §3.11.8 states; that section carries the deferral and cites this one. **The second is the payment receipt's signed payload**, whose construction `19-economic-governance.md` §19.15.5 states: the payer signs `payer_did` and `payee` as UTF-8 bytes, so no party computes that preimage until R13's later revision fixes the encoding, and §9.7.4.2 R9 states that no relay verifies the payer's signature on its own path. **That construction carries no write-binding operand**, which changes the operands the payload takes and leaves this register's count where it stands, because the payer and payee operands are what the deferral reads. The `derived_context_id` of `05-contexts.md` §5.15.8 is not among the sixteen: it consumes the two 32-byte digests, so its preimage is computable today.
...
A subscriber resolves the `#active` fragment under the arm §9.7.1's classification table selects for this separator, which is R8's content arm of §9.7.4.2, and verifies the signature against the key that arm selects.
...
| 3 | `issuer` | 4-byte BE length + UTF-8 bytes (identifier) |
| 4 | `subject` | 4-byte BE length + UTF-8 bytes (identifier) |
...
| Order | Field | Encoding |
|-------|-------|----------|
| 1 | `did` | 4-byte BE length + UTF-8 bytes (the attested identifier; MUST equal the leaf's `ScpCredential.did`) |
...
| 4 | `key_state_head` | 32 bytes, no length prefix; First-seen is the sole acceptance condition against a key the state lists `Compromised` (§9.7.1) |
...
ADR-039, the shared human-agent identity model ADR-063 supersedes, defines a custody violation at enforcement-stack layer 4 as a permanent record that one verifier writes about a subject who never consented to it. A reader must be able to establish which verifier wrote a given record, and must be able to detect a record that any other party altered after that verifier signed it. Fields 1 through 7 below feed one P-256 ECDSA signature that carries both properties.
...
**`signer_key_id` names a verification method the identity model superseded.** The shipped `SigningKeyId` type renders a second role beside `"#active"`, the shared agent key ADR-039 defined, and §9.7.4.2 gives a human identity no such key, because an agent holds its own key-event log and its own `#active`. Which key a `CategoryAViolation` names once the offending signature comes from a delegated agent identity is undecided, and this section decides nothing about it.
...
**The record carries no signature of its own** (§9.10.12 states the frame's bytes and why it carries none), so nothing authenticates the frame beside the chain inside it. A relay's write decision is chain verification under §9.7.4.2 R2 and R3 plus §9.7.4.2 R9's rule for what may open or extend a chain at a routing id (`03-identity.md` §3.10.2 states the procedure), and a resolver's trust decision reads the log under steps 2 through 4 above.
...
A party meeting an identifier for the first time resolves it under §9.7.4.2, which states how that party obtains the chain, recomputes the identifier under R2, derives the key state under R8, and reaches one of the six verdicts R14 defines. This section reads that verdict and assigns the identifier's first standing from it, and §9.11's gate list states every condition a gate reads. The assignment is not a gate condition: a verifier that recorded `Verified` on a verdict under which it adopted no chain would fix a standing against a chain it never adopted, after which the key-change trigger of §9.11 could never fire for that identifier.
...
| LeafNode credential | identifier + UCAN + signing_key_id | The MLS credential field carries the member's identifier, their context-scoped UCAN token, and the `signing_key_id` (`#active`) naming which role signed the leaf's **KeyPackage attestation** (§9.7.1). The leaf node is self-signed by its own ephemeral MLS key and never by an identity key. |
...
1. the `signing_key_id` names the role the identity's key state lists `#active` and `Current`, and resolution binds to **that key only** — a verifier MUST NOT accept an attestation signed by a key the resolved current key state lists in any of the three conditions of §9.7.1 that are not `Current`, nor by any key that key state does not list `Current` in the `#active` role. (Without this, an attestation signed by a rotated-away key would still verify, and the revocation-by-rotation of §9.12 would not bite.) This current-key binding is enforced on **both** triggers, and the **Resolution failure policy** below states what a verifier does on each when the resolution fails and when it succeeds against a rotated-away key;
2. **current-key resolution freshness:** the key state used to satisfy check 1, derived from the identity's key-event log (§9.7.4.2 R8), MUST be no older than `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (§9.18.7 — 300s / 5 minutes, tied to the §9.14 clock-skew tolerance). A resolver-cache entry older than this bound MUST NOT be used for the current-key check — it MUST trigger a **fresh** resolution, which the **Resolution failure policy** below governs. The ≤ 5-minute freshness guarantee presupposes **rollback-resistant identity resolution** — see the **Rollback-resistance assumption** below. This hard-bounds revocation latency — a retired `#active` cannot keep verifying attestations past rotation for longer than this bound (§9.12), regardless of how long the privacy cache would otherwise retain the pre-rotation document;
3. the attestation's P-256 signature verifies against that resolved current verification method (over the §9.5.1 canonical hash under `"SCP-KEYPACKAGE-ATTESTATION-V1:"`). On an already-admitted member's Update the verifier re-resolves with its cache bypassed before it rejects on this check (**Resolution failure policy** below);
...
9. the attestation's `did` equals the identifier carried in the leaf's `ScpCredential`;
...
**Standing gates the identity layer, never MLS processing.** §9.11 states that rule, its RFC 9420 reason, and the acts a standing withholds. The one act that takes a human step before it happens is an Add of a **new leaf for a flagged identifier**, which §9.11's outbound gate withholds; that is a question about admitting an identity, not about processing MLS.
...
**Content verification returns `ContentVerdict`.** Its three variants are `Valid`, `Unverified{reason}` and `Invalid{cause}`, whose two causes are `NoPreforkBasis` and `NoBoundary`, defined by §9.7.4.2 R7 and by `03-identity.md` §3.2.1 step 5 respectively. `ContentInvalidCause` names the second payload type. `UnverifiedReason` names the first, and **`UnverifiedReason`'s** two values are `NoFreshKeyState` — the verifier holds the artifact and has resolved no key state fresh enough to test it — and `ResolutionInconclusive{cause: InconclusiveCause}`, the resolution having returned `Inconclusive`. All three type names are name-bound under the criterion §9.7.4.2's definitions state. An `Invalid` payload here is a verdict about one artifact inside one context and never about a chain, so a resolution never returns this type and content verification never returns R14's.
...
**How a verifier resolves a `signing_key_id` fragment: three arms, and the class above selects the arm.** **The class this table assigns the artifact's separator selects the arm, and the presence or the absence of a `key_state_head` field selects nothing.** **Attestation arm:** the verifier reads the `Current` key of the named role at the latest state-carrying event and reads no other key. **Log-anchored arm:** the verifier replays the chain to the position the artifact's `key_state_head` names and reads, at that position, the key that held the named role and the condition it held there. **Content arm:** the verifier replays the adopted chain, takes every key that held the named role at any position, takes the one key among them whose bytes verify the signature, and then applies that key's condition and that context's boundary under the content rule above. No attestation-class separator carries a `key_state_head` field, so a field-presence test would send every one of them to the content arm, whose procedure admits a key the controller has already dropped. **No arm other than the attestation arm resolves a fragment from the latest key state's current keys**, and the key state carries no set of every key that ever held a role for the log-anchored arm or the content arm to read.

**Why the log-anchored class exists, and the limit it carries.** An artifact a party publishes as evidence against a future version of itself cannot verify under the attestation class, because the signer would void it by any routine rotation of the key it controls. §9.15's destruction attestation is such an artifact: it is published to relays, outside any context, so no context epoch orders it and the content class has no test to apply. A durable context snapshot and a durable context export are such artifacts too, for a second reason: a snapshot signed under a key the producer later rotates is the record that teaches a later joiner where that key's boundary lies, and the content class would reject it at every member who received it after the boundary. Anchoring these artifacts to a key-state position gives them an ordering the signer cannot revise after the fact, because the log fixes the key-state head digest. **The three objects a community-relay-list operator signs need no anchor**: the key their row names does not rotate, so no rotation can void a cosignature, a conflict statement or a relay proof, and a reader that holds the shipped list holds the key each one verifies under. **The limit, stated because it is not a guarantee against the compromised key's holder:** the signer chooses `key_state_head`, so a party holding a key the state lists `Compromised{from: N}` can anchor a forged artifact before N. A verifier therefore accepts such an artifact **only** when it held that artifact before it adopted the event that carried N — first-seen, recorded with the adoption. **The class reads that entry wherever on the chain it sits**, so a `KeyState` carrying it arms this test exactly as a `RootRecovery` carrying it does, which is what reaches the commonest compromise the protocol has, a stolen `#active` the controller drops by a `KeyState` (§9.12 step 1). First-seen is the sole acceptance condition, because this design constructs no countersignature over one of these artifacts, so a verifier told to weigh one would invent the object and accept the exact forgery the sentence above describes. Against a compromised key with first-seen unmet, this class gives no guarantee, and a verifier rejects. `SCP-CHECKPOINT-V1:` (§9.9.3) needs no anchor at all: a member accepts a consistency checkpoint through MLS inside the context, so the content class already orders it. **Which artifacts are durable:** a context snapshot and a context export are durable artifacts, stored and re-served after the epoch that produced them, which is why they carry per-item epochs and their own key-state anchor; every separator in the content row is live traffic a context ordered when it arrived.
...
- When an Active Signing Key rotates by a `KeyState` event (§9.7.4.2 R3), the SDK MUST issue an MLS Update in every active context with a new credential AND a **KeyPackage attestation re-issued under the new `#active` key** (a fresh `#active` signature over the leaf's ephemeral MLS key). This synchronizes key rotation with MLS-level post-compromise security. The MLS leaf `signature_key` is ephemeral and never equals the rotated identity key — only the attestation over it is re-signed.
...
- MLS leaf HPKE keys (DHKEM(P-256), 65 bytes each) — these are **three distinct** keys, NOT one; RFC 9420 keeps them separate and so does SCP:
  ...
  All three are bound to the identifier by the KeyPackage attestation (§9.7.1, §9.5.2) and are distinct from the leaf signature key below.
- MLS leaf signature key (P-256, in RFC 9420's uncompressed SEC1 encoding): The leaf's `signature_key` — an **ephemeral, context-scoped** key generated by the MLS layer (`SignatureKeyPair::new()`), one per group, re-generated on every Update/PCS rotation (§9.7.3). It self-signs the LeafNode and is DISTINCT from every root member and from the Active Signing Key (`#active`) — it is never a key the key state lists and MUST NOT be expected to resolve to one. It is bound to the member's identifier out-of-band by the `#active`-signed KeyPackage attestation (§9.7.1, §9.5.2), which is re-issued whenever this key is re-generated. Never persisted beyond the MLS group state; destroyed with the leaf on Update or group exit.
...
- Active Signing Key: Rotated by a `KeyState` event signed by the standing root (§9.7.4.2 R3) whose key list carries the new `#active` `Current` and drops the old one, which a verifier reads by replay as `Superseded` (§9.7.4.2 R8). All active MLS groups receive an Update proposal with the new credential. §9.11 states which observers set `PendingReverify` on that event and which do not.
- Root set: Changes only by a `RootRecovery` (§9.7.4.2 R3), which installs a fresh root set and carries the complete post-recovery key state (§9.7.4.2 R8) — a rare operation. No migration proof exists: the recovery event is signed under `"SCP-KEL-EVENT-V1:"` like every other key event (§9.7.4.2 R13). §9.11's key-change rule governs what every peer does on observing it.
- MLS epoch keys: Rotated automatically on every Commit (membership change or Update).
- UCAN tokens: Expire per their `exp` field. Re-issued by the human's Active Signing Key. On active key rotation, all UCAN tokens signed by the old Active Signing Key are revoked and reissued under the new key. A delegated agent identity's own key rotation revokes no token the human issued, because a scoped UCAN names that agent's identifier as its audience (§9.7.4.2 R2). Revocations are added to the per-context `RevocationList` and distributed as MLS application messages (see §9.5 UCAN revocation, ADR-016 criterion 5).
...
Three readings of that rule cover the substrates item 4 admits: a substrate that generates its own key receives no key material from the SDK and returns none; a substrate the SDK fills takes a key the SDK generated on the device from the platform CSPRNG; and a network HSM under a separate principal is the substrate and not a remote server.
...
**3a. Independent residence.** The recovery authority is the minimal secret, key handle, or authorization capability sufficient to recover the pre-rotation private key. It MUST reside in a substrate whose compromise is independent of operational-custody compromise: **no secret, handle, or capability the custody holding the `#active` key can reach, directly or transitively, may suffice**, alone or together with publicly stored artifacts, to recover it. Wrapping the key in a cipher whose decryption authority is itself reachable from operational custody gives zero protection against the adversary this rule names.

Where no substrate satisfying that requirement is available, identity creation MUST fail closed with `IdentityError::NoIndependentSubstrate` (`03-identity.md` §3.10.10), and the same refusal binds at every later reveal: the SDK MUST refuse, under that error, to compose a reveal-authorized event at all, before it signs. There is no fallback to co-located operational storage and no fallback to a development stand-in. The ceremony order of §9.7.4.2 R10 puts successor generation ahead of the persist and the signature, so the refusal lands in the one phase where an abort still leaves the standing commitment unconsumed.

A reveal-authorized event is a rare, separately authorized event and not a daily operation, so a distinct reveal-time authorization principal satisfies the residence requirement even where that principal is fully automated. What fails the requirement is the custody holding the `#active` key being able to reach the pre-rotation key. **A read or a write against the pre-rotation credential's `largeBlob` store is not a daily operation either**: R10's one-reveal-at-a-time rule performs both, once per composition, under the same reveal-time authorization principal, so exercising that store is the act this item already separates from the daily flow and adds no reach the item forbids.
...
**4. Custody selection.** Under the key-event log a copy of a pre-rotation key alone does more lasting damage than a copy of the root alone. A root leak is recoverable by a reveal wherever the leak did not also reach the pre-rotation credential, which on the shipped consumer default means wherever it did not reach the one platform account item 4 names, while a leaked pre-rotation key lets its holder contest the identity at any later date and, held exclusively, take the identity over and then kill it permanently.
...
R10's one-reveal-at-a-time rule reads a store set keyed on the identity's **pre-rotation credential**, and this value says which stores that credential syncs through. **The `largeBlob` store that rule reads is the pre-rotation credential's and never the root credential's**, because a reveal is what the rule gates and item 3a above separates the reveal-time authorizer from daily operations. The platform syncs that store with the credential, over the origin span R10 states. **Where the authenticator supports no `largeBlob`, the platform gives the SDK no durable store spanning those origins and the SDK's set is its own local store, which spans one device.** A profile is a property of a running SDK and never of an identity. The SDK MUST re-surface the copyable-custody disclosure whenever it opens an identity whose standing pre-rotation custody type is one of the three copyable values, on whatever profile it runs, because an identity incepted headless under a copyable method keeps that key when its operational key migrates to a phone.

**Consumer methods.** The consumer profile's pre-rotation methods are these three, and every one of them is non-exportable: a passkey, which is the default; a FIDO2 security token; and an HSM-resident key under a principal the operational path holds no grant to assume. The conformance table below states where each of the three is offered, and an SDK MUST NOT offer one the table marks as not conforming on the running profile. A non-exportable substrate is one from which no code path extracts the private key. The controller holds no exportable copy, and the provider's sync fabric may retain an encrypted copy reachable only through the platform account. Alec ruled on 2026-09-10 that self custody "is writing down a key, yes" and is "not a concern", which is why the three copyable methods sit on the headless profile alone.

A passkey is a discoverable WebAuthn credential under the universal relying-party identifier the SDK ships, `ctx.network` by default, held by the platform's passkey provider and synced across that account's devices. Its private key is non-exportable, so the controller holds no second copy of it. Alec named the substrate on 2026-09-10, "a passkey in your apple passwords or similar", and typed the domain rule the same day: "roots would get created under a universal identifier by default like ctx.network". Two questions about that identifier are open at his instruction, and `00-open-questions.md` §Identity and Custody carries both as a dated entry.
...
The root credential and the pre-rotation credential share one platform account by default, which places the identity's root authority and its recovery authority behind one authentication factor. Alec chose that default on 2026-09-10 from two options the orchestrator put to him: keep the two together and tell the user. The SDK MUST state the co-residence at custody selection, and MUST offer the pre-rotation credential on a FIDO2 security token or under a second platform account; that offer binds the SDK and not the controller.
...
Item 3a's test reads the custody holding the `#active` key and never the root, so it does not fire on the root-and-recovery co-residence, which is the default Alec chose on 2026-09-10. **The shipped consumer default satisfies that test**, because the `#active` key's custody holds no grant that recovers the pre-rotation credential. The SDK therefore creates the identity on the default, states the co-residence, and offers the two separations the paragraph above requires.
...
No row of the table turns on whether a human attends the reveal. Every conforming answer stays subject to item 3a, and the SDK MUST reject a selection whose recovery authority the custody holding the `#active` key can reach, whatever the table says.
...
The reachability confirmation of step (d) for an offline method is a re-entry or a re-scan of the backup. Where step (d) runs on a second device, that device reads the credential-keyed store set R10's one-reveal-at-a-time rule names, and refuses while it holds either object that rule reads. For a non-copyable substrate it is one test signature from the credential over `SHA-256("SCP-CUSTODY-PROBE-V1:" ‖ 32 fresh random bytes)`, verified against the public key the commitment covers; for the assertion form the probe's WebAuthn challenge is that same prefix over those same random bytes. Without a defined confirmation the gate had a precondition for a mnemonic and none for a passkey.
...
On a non-copyable substrate no private key ever entered the SDK's memory, so step (f) has nothing to destroy and the SDK discharges it by holding no key material.
...
| Two of the controller's devices each reveal the standing commitment | Rank 1 on both sides; terminal. **R10's one-reveal-at-a-time rule closes the race across exactly the set of SDKs that one store spans, and leaves it open across every pair of SDKs no one store spans.** The rule's store set is keyed on the pre-rotation credential: the platform account's synced credential store where that account syncs the credential, and the credential's `largeBlob` store where the authenticator supports the extension. A controller whose SDKs all read one of those stores runs no race; a controller running two SDKs that read two different stores, or one SDK that reads neither, runs it. A `Mobile` SDK under a platform account beside a `Browser` SDK holding only `largeBlob` is such a pair, and so is any pair including an SDK whose set is its own device-local store |

Where the root cannot be recovered by key material, the person establishes a new identity, and each context's admins remove the old identity and admit the new one after confirming the head from a relay in the fallback set. The social mechanisms of `03-identity.md` §3.3 do not reach this row, because none of them authorizes a root change.
...
**A root signature is an indexed signature group.** The signed preimage names the root-set indices that sign and the signature form each named index uses. The signature field carries exactly one signature per named index, in index order. Every indexed signature MUST verify against the member at its index, and the named indices MUST number at least t and MUST be distinct. A fixed-layout signature set and a threshold of "at least t members" contradicted each other, and KERI's `spec-body` §Indexed Signatures resolves that without a variable layout.
...
**Signature form.** An indexed signature's form is a one-byte discriminator carried in a signature-form list that the preimage binds beside the signer index list of its group. Its type name is `SignatureForm` and its two variant names are `Raw` and `WebAuthnAssertion`, name-bound under the criterion below. A verifier MUST reject any other value.

`SignatureForm::Raw`, discriminator `0x01`: the indexed signature a named index contributes is the 64-byte raw ECDSA signature that §9.5 defines, low-`s` normalized by the signer, verified against the key at that index over the event's preimage digest.

`SignatureForm::WebAuthnAssertion`, discriminator `0x02`: the indexed signature a named index contributes is
...
A verifier of an assertion signature parses `clientDataJSON` as RFC 8259 JSON and MUST reject a document carrying a duplicate member name at any nesting level, because JSON parsers differ over which duplicate wins and two honest verifiers holding one chain would then disagree about one event's validity.

A verifier MUST reject an assertion signature unless the `type` member equals `"webauthn.get"`, and unless the `challenge` member equals the unpadded base64url encoding of `"SCP-KEY-EVENT-V1:" ‖ D`, where `D` is that event's own preimage digest. Both comparisons run against the decoded JSON string value and never against the raw bytes. The challenge prefix is what makes a sign-in assertion unusable as a key-event signature, and the decoded-value comparison is what stops two verifiers comparing different things, because RFC 8259 admits an escape for any character.

A verifier MUST reject an assertion signature unless both flags of byte 32 of `authenticatorData` are set: the user-presence flag, which is bit 0, and the **user-verification flag, which is bit 2**. A passkey prompt that returned no user verification therefore signs no key event, and the SDK requests user verification when it composes one. A verifier MUST also reject a signature that fails the low-`s` rule of §9.5. It then verifies the 64-byte signature against the key at that signature's index over `authenticatorData ‖ SHA-256(clientDataJSON)`, which is the message WebAuthn defines and is not `D`. `MIN_AUTHENTICATOR_DATA_BYTES` is what guarantees byte 32 exists before the verifier indexes it, and it guarantees it for both flags, which share that byte. Under the shipped consumer default the root credential and the pre-rotation credential sit in one platform account, so user verification is what separates a root signature by the human from one by anything running on an unlocked device.
...
**Custody type.** A key's custody type names the substrate that holds its private half. Its type name is `CustodyType`, name-bound under the criterion below, and **each of its nine variants carries a registered byte, because that byte rides in every key entry of the key-state snapshot and therefore inside the inception preimage the identifier digests**: `Passkey` `0x01`, `Fido2Token` `0x02`, `Hsm` `0x03`, `SecureEnclave` `0x04`, `AndroidKeystore` `0x05`, `EncryptedOfflineBackup` `0x06`, `ShamirShares` `0x07`, `PaperBackup` `0x08`, `Software` `0x09`. R3 rejects an unrecognized value under its unrecognized-discriminator clause. Without a registered byte a Swift author numbering from `0x00` and a Rust author numbering from `0x01` derive two identifiers for one identity, and each binding rejects every identity the other created.
...
The custody type is a declaration its author chose, and every consumer reads it as `Software` unless a platform proof verifies. The orchestrator proposed the sentence "either the platform proof is attached and verified, or the custody model reads as software no matter what string was written." on 2026-08-25, and Alec approved it that day with his own three words, "yes do that." The sentence is the orchestrator's and the approval is his.
...
**The key state** of an identity is: the standing root with each member's custody type; every operational key that is `Current`, by role and by custody type; the standing commitment as of the latest reveal-authorized event at or before that position, which is the commitment digest list, the next threshold, and each committed member's custody type and key algorithm; the witness set with the witnessing interval; the service-key designation; and the delegator. The key list names every key `Current` at that position and, for each key the event drops, one entry naming that key's new condition, which R8 states for a `KeyState` and a `RootRecovery` alike. It names no other key, so the key state's size follows the identity's current configuration and never the chain's length. A verifier replays the log under R8, so restating every key the chain installed would make every state-carrying event grow with the square of the installed-key count and would tell a verifier nothing the replay does not. KERI's key-state notice carries the same shape, under `spec-body` §Validator and KID0003 §Key State Notice Messages.
**The key state's fields, one name and one type each.** Every SDK binding carries these names, because `IdentityBackend::resolve` returns this object to a caller (`03-identity.md` §3.10.10). Each field's type is the one the key-state snapshot's encoding below already fixes for it.

| Field | Type | What it holds |
|---|---|---|
| `root_threshold` | `u32` | The signing threshold t of the standing root |
| `keys` | `Vec<KeyEntry>` | One entry per key the state names: every root member and every operational key |
| `witnesses` | `Vec<[u8; 32]>` | The designated witnesses, each a community-relay-list operator identifier |
| `witnessing_interval` | `u32` | The cadence in seconds at which each witness cosigns the head |
| `service_key_role` | `KeyRole` | The role whose `Current` key signs the service record |
| `delegator` | `[u8; 32]` | The delegating identity's identifier, all-zero where the identity is undelegated |
| `standing_commitment` | `StandingCommitment` | The commitment the latest reveal-authorized event at or before this position fixed |

`KeyEntry` carries `key: [u8; 33]`, `role: KeyRole`, `condition: KeyCondition`, `compromised_from: u64`, `custody_type: CustodyType` and `key_algorithm: KeyAlgorithm`. `StandingCommitment` carries `commitments: Vec<CommitmentEntry>` and `next_threshold: u32`, and `CommitmentEntry` carries `commitment: [u8; 32]`, `custody_type: CustodyType` and `key_algorithm: KeyAlgorithm`.

`18-addressability-and-deployment.md` §18.2.2A states where the retired document's contents went; the key state expresses its verification-relationship arrays: a key's role and its condition say what that key may do, and no separate relationship list exists.
...
**State-carrying events.** Three kinds carry a key-state snapshot: the inception event, a `KeyState`, and a `RootRecovery`. A snapshot carries every component of the key state except the standing commitment, which the continuation field of the latest commitment-fixing event carries instead. The derived key state at any position is that position's latest snapshot together with the standing commitment at that position, which the inception event or a reveal-authorized event fixed, so a `CommitmentRollover` replaces the commitment component and changes nothing else. No other earlier event contributes.
...
The seal has no live reader today, because R3 rejects every chain whose delegator is nonzero until the delegation model is specified. ADR-063's tracked item 3 states why it ships now.
...
**A witness** is a relay that an identity's key state designates to cosign that identity's log head. §9.7.4.3 states what a witness does and what it decides. Neither the cosigned head nor the conflict statement enters any rule of this section.

**A community-relay-list operator's identifier is non-transferable.** The operator's list entry carries the operator's P-256 public key, that key does not rotate, and an operator that changes its key takes a new list entry at the next release. A party verifying any object an operator signs reads the key from that operator's list entry and resolves no chain of the operator's own. **A party that finds one `operator` value under two entries declaring different `key` bytes reads neither entry as that operator**, counts that operator as absent from its list, and refuses every object naming it, because the singular "that operator's entry" would otherwise name two keys and two verifiers holding one list would reach two verdicts on one cosigned head. **One operator key is one witness:** where one operator runs two listed relays under one key, every rule that names a witness names that operator once, and the two relays' cosigned heads are one witness's heads. KERI states the non-transferability rule in one sentence under `spec-body` §Backer list: "When the Backers are Witnesses, then the AIDs themselves MUST be non-transferable, fully qualified public keys… Consequently, the Witness does not need a KEL because its key state is fixed and is given by its AID."

**The relay proof of control**, fields in preimage order: `operator` (32 bytes), `nonce` (32 bytes, echoed verbatim from the QUERY), `routing_id` (32 bytes), `value_digest` (32 bytes, `SHA-256` over the response's blob bytes under the repeated-field rule of §9.5.1, the elements in the order the relay returned them, so a resolver that sorts or deduplicates its response buffer still digests the order it received), and `observed_at` (8 bytes, big-endian u64, the relay's own clock). Its signature preimage is

```
SHA-256("SCP-RELAY-PROOF-V1:" ‖ operator ‖ nonce ‖ routing_id ‖ value_digest ‖ observed_at)
```

Every field is fixed width, so the object is 136 preimage bytes and one 64-byte signature, 200 bytes in all. This object's `value_digest` covers a response's blob bytes.

The proof verifies under the community-relay-list operator-key class of §9.7.1, so its signature verifies against the P-256 public key the operator's community-relay-list entry declares, under neither the attestation class nor the log-anchored class. It carries no position field, because a non-transferable operator key has no position to name, and it names no role, because a non-transferable operator holds no `#active`.
...
**Head provenance.** How a party came to hold the head it adopted is a three-valued record named `HeadProvenance`, whose variants are `WitnessRead`, `TwoOperatorRead`, and `SingleRelay`, name-bound under the criterion below. **A party assigns the record from the source it used for the head it adopted, and from nothing else**: `WitnessRead` where it adopted a head a witness cosigned, `TwoOperatorRead` where it adopted a head two list entries carrying distinct operator keys served, and `SingleRelay` otherwise. `TwoOperatorRead` counts operator keys and never declared operator names, which is R11's count and the non-transferable-operator rule above. The SDK records the value as a field of the per-identifier record §9.11 states, and surfaces it beside the standing. **No rule reads the record.** A rule that withheld an act on the record would withhold that act on a property of the reading party's own network, and it would restore the missing-cosignature gate that Alec's watch-and-report ruling removed, under the name of a second source.
...
**The key-event preimage binds twelve fields in this order, and every kind carries all twelve.**
...
12. The continuation: one byte, `0x00` for abandonment or `0x01` for a commitment list, and where `0x01` a 4-byte count, then per member one 32-byte commitment followed by that member's one-byte custody type and one-byte key algorithm, then a 4-byte next threshold. The commitment-fixing event is the one event whose author knows which substrate holds each committed key, which is why the committed members' custody metadata travels here rather than in the snapshot.

**The absent-field sentinel.** A field a kind does not carry is present with its sentinel: all-zero bytes at the field's own fixed width where the field is fixed-width, which is one `0x00` byte for `standing_root` and 32 zero bytes for a digest; and `BE32(0)` where the field is a count-prefixed list or the key-state snapshot, whose absent form is `BE32(0)` standing in place of the whole snapshot encoding. **Two fields are composites that are neither fixed-width nor a bare count-prefixed list, and each one's absent form is the same four bytes `BE32(0)` standing in place of the whole composite.** Field 9, the installed root set and its threshold, writes `BE32(0)` and no trailing threshold, so its sentinel is four bytes and not eight. Field 12, the continuation, writes `BE32(0)` and never the one-byte `0x00`, because `0x00` is the registered live value declaring abandonment and a one-byte sentinel would be byte-identical to a declaration that the identity is terminal. The key-event preimage takes this rule and never §9.5.1's absent-optional-field rule, so no key-event field ever encodes as `SHA-256(0x00)`. Without one sentinel an absent field admits three conforming encodings, which gives one inception event three preimage lengths and three identifiers, and R2 then makes each implementation reject every other implementation's chain.

**One rule bounds every count-prefixed list a party parses out of unverified bytes** — every such list of a key event, of a key-state snapshot, and of a service record, and **every list a publisher reads out of a relay's own answer**, which is the `DeclaredWritePolicy` the `POLICY` query returns and the `BudgetValue` a `4041` refusal carries (`03-identity.md` §3.10.10). A relay is untrusted on the read path exactly as a publisher is on the write path, so an answer carrying a ten-million-member `receipt_exempts` set reaches the same decoder. The list is bounded by the `MAX_*` constant §9.18.17 registers for it, and the bytes the count implies MUST NOT run past the field that encloses the list. A decoder rejects on either condition before it allocates, and sizes no allocation from any wire count before it has read that many elements. Where §9.18.17 registers no `MAX_*` for a list, the enclosing field's remaining bytes are its whole bound. R3's set-size caps do not reach this, because R3 applies them after the parse the frame must first survive: one 283-byte frame declaring a snapshot key-list count of `0xFFFFFFFF` would otherwise make a conforming decoder attempt 193 GB and kill the relay for every identity it serves.

**Which fields each kind carries in substance.** `Inception` carries 1, 2, 3, 4, 6, 7 (one root group), 9, 10 and 12, and carries 5, 8 and 11 as sentinels. `KeyState` carries 1, 2, 3, 4, 6, 7 (one root group), 10 and 11, and carries 5, 8, 9 and 12 as sentinels. `CommitmentRollover` carries 1, 2, 3, 4, 6, 7 (the reveal group then the standing-root group), 8 and 12, and carries 5, 9, 10 and 11 as sentinels. `RootRecovery{CoSigns}` carries 1, 2, 3, 4, 5, 6, 7 (the reveal group, the K′ group, then the standing-root group), 8, 9, 10 and 12, and carries 11 as a sentinel. `RootRecovery{Lost}` carries the same fields with two groups rather than three, the reveal group and the K′ group.

**The key-state snapshot's own order** is: the 4-byte root threshold; the key list as a 4-byte count then one 45-byte entry per key the snapshot names; the witness set as a 4-byte count then each 32-byte operator identifier; the 4-byte witnessing interval; a one-byte service-key role discriminator; and the 32-byte delegator. Each 45-byte key entry is `key` (33-byte compressed point), `role` (one byte), `condition` (one byte: `Current` `0x01`, `Superseded` `0x02`, `Retired` `0x03`, `Compromised` `0x04`), `compromised_from` (8 bytes, the position a `Compromised` entry carries and zero otherwise), `custody_type` (one byte), and `key_algorithm` (one byte), with root members in the root set's own list order, which is what makes a root signer index resolvable from the snapshot.

**Key role.** A key's role is a one-byte discriminator named `KeyRole`, whose two variant names are `RootMember` (`0x01`) and `Active` (`0x02`), carried by every key entry of a key-state snapshot. The snapshot's service-key role discriminator takes the same one-byte encoding and carries `Active` as its one registered value, because `#active` is the identity's one operational role. A verifier MUST reject any other value in either field. Both names are name-bound under the criterion below.

**Why each binding is in the preimage.** Every signature an event carries is over one identical preimage. Without predecessor binding a root holder could copy the owner's genuine revealed `RootRecovery` bytes onto its own fork at the same sequence. Without the index list in the preimage a relay could strip one of several signatures with no detectable defect. Without the form list a relay could rewrite one signature's declared form and re-split the signature field into a second self-consistent parse. Without the continuation in the preimage a relay could strip the next commitment and substitute abandonment with every named signature still verifying.

**The name-binding criterion.** <!-- scp:fragment id="name-binding-criterion" -->**An enumeration this design defines is name-bound — its type name and every variant name identical in every SDK binding — where a binding surfaces it to callers or where it serializes on the wire, and a name is bound as the spec spells it, PascalCase, which every binding carries verbatim.** No binding recases a variant and no rule splits one on word boundaries, so a shared conformance fixture comparing a variant across languages compares one string.<!-- scp:end id="name-binding-criterion" --> Without one criterion, four bindings inventing four names would register one later-added variant under four different names. That sentence decides membership, so an enumeration added later is name-bound the moment it meets the criterion and needs no edit here. The criterion reaches every section of this design and is not scoped to the section that states it. **A name is bound as this spec spells it, and every binding carries that spelling verbatim: PascalCase in Rust, Swift, Kotlin, TypeScript and Python alike.** No binding recases a variant and no rule splits one on word boundaries, so a conformance fixture comparing a variant across languages compares one string. The verbatim spelling overrides a language's own enum convention wherever the two differ, Python's `SCREAMING_SNAKE_CASE` and Swift's `lowerCamelCase` included, because a split-and-recase rule spells `Fido2Token`, `EcdsaP256Sha256`, `Rank1Terminal`, `Rank2Terminal` and `Rank3Pending` two or three defensible ways each, which is the choice this criterion exists to take away from a binding author. The naming table of `.docs/standards/conventions.md` carries the enum-variant row and states the mapping.

**Eight further enumerations carry a type name.** The event kind is `EventKind`; the key condition is `KeyCondition`; the continuation is `Continuation`, whose two variants are `FixesCommitment`, which field 12 encodes as `0x01`, and `Abandons`, which it encodes as `0x00`; the resolution verdict is `ResolutionVerdict`; the inconclusive cause is `InconclusiveCause`; the tie class is `TieClass`, whose three variants are `Rank1Terminal`, `Rank2Terminal` and `Rank3Pending`, one per R7 sentence; the unverified reason is `UnverifiedReason`, whose two variants §9.7.1 names; and the content-invalid cause is `ContentInvalidCause`. All eight are name-bound under the criterion above, and R14 and §9.7.1 state the variants of `ResolutionVerdict`, `InconclusiveCause`, `UnverifiedReason` and `ContentInvalidCause`.
...
**The signature layout is closed.** Each kind, and for a `RootRecovery` the kind together with its `standing_root` value, fixes which groups the event carries and in what order; the index list fixes how many indexed signatures the field carries and the form list fixes each indexed signature's layout. Bytes past the last indexed signature the form list accounts for are a copy-level defect, and so is an indexed signature whose declared length prefix runs past the field or past its bound. Any relay can strip or stuff an indexed signature the copy carries with no detectable defect, and an unbounded signature field is a verification-cost amplifier.

An event carries no indexed signature that its kind, its index lists, and its form lists do not name. A third party's endorsement and a witness's cosignature are separate signed objects over the event's preimage digest, because evidence that rides the copy is neither reliably present nor reliably absent.

**`standing_root`.** A `RootRecovery` carries a required field `standing_root` with exactly two values, `CoSigns` and `Lost`, bound in the preimage. Its type name is `StandingRootDeclaration`, name-bound under the criterion the definitions state. The declaration, and not the presence of a signature, says which signatures the event names, so a `RootRecovery{CoSigns}` missing its root signature is a copy-level defect and never a `RootRecovery{Lost}`.

`Lost` is a controller obligation and no verifier checks it. The signing quorum MUST establish that fewer than t members of the standing root are reachable before it declares `Lost`, and where the SDK itself holds at least t members it MUST refuse, with `IdentityError::RootThresholdHeld`, to sign `standing_root: Lost`. That refusal is the one mechanical realization, and it cannot run for an organization whose root members sit in separate substrates. Every relying party reads `Lost` as the controller's declaration and never as a checked fact.
...
**Author-attributable defects.** A verifier rejects the chain at the event where any of these holds: a signer index list naming fewer indices than the indexed set's own threshold; a repeated index; an index naming no member of the set; a root set, next set, or commitment list whose entries are not pairwise distinct; a root set over `MAX_ROOT_SET_SIZE` members; a next set or commitment list over `MAX_NEXT_SET_SIZE` members, which the verifier reads in the continuation field of the preimage and never in the key-state snapshot, because the snapshot carries no next-set list; a key-state snapshot whose root set exceeds `MAX_ROOT_SET_SIZE`; an event whose encoded length exceeds `MAX_EVENT_BYTES`, which is what bounds the snapshot's key list; a threshold outside the range one to the size of the set it indexes; a sequence that is not its predecessor's plus one; a revealed key whose commitment is not the standing list's member at the named index; a revealed-keys list whose element count differs from the length of the reveal group's signer index list, which would leave the commitment-match rule with no operand at the indices the list does not reach; a `KeyState` installing a root set or threshold differing from the standing root at its predecessor; a `CommitmentRollover` carrying a key state; a state-carrying event listing one public key more than once; a key-state list naming a key that is neither `Current` at that position nor dropped by that event (R8); an unrecognized key-algorithm value; a form list whose length differs from its index list, or naming an unrecognized form; an unrecognized discriminator; a next threshold lower than the threshold of the root standing after that event; a service-key role discriminator that resolves to no key that snapshot lists `Current` in that role, the discriminator being the one-byte field the snapshot carries and never a key; a witnessing interval outside `MIN_WITNESSING_INTERVAL` and `MAX_WITNESSING_INTERVAL`; a witness set over `MAX_WITNESS_SET_SIZE` entries or not pairwise distinct; a snapshot identical to its predecessor's; a witness set naming the subject identifier itself; more than one key `Current` in any operational role; a key installed that appeared earlier as a revealed next-set key; a commitment whose preimage is a key that same event installs; a nonzero delegator; and any violation of an R4 or R8 obligation.
...
A relying party MUST NOT act socially on a chain it adopted, for an adopted takeover or an abandonment, until it has confirmed that chain's head **from an entry of the fallback set the definitions above state**, because the holder of the designated key chose the relays that record names. The condition names the fallback set and never the exclusion alone: a party that takes an identity over holds the designated key, and a service record naming every listed entry would otherwise leave no qualifying source and bar every relying party permanently from the removal the failure-mode table of §9.7.4.1 prescribes. The definitions close that by making the fallback set the whole community relay list where the exclusion empties it.
...
A lower rank number beats a higher one, and the winning chain supersedes the losing chain in its entirety. Alec ruled it on 2026-09-07: "Root wins sounds like a good solution."
...
**R8 — deriving the key state.** A relying party derives the key state from the latest state-carrying event at or before the head of the chain it adopted. No earlier event contributes, and a verifier applies no diff, no undo, and no voiding, so a `RootRecovery` replaces the entire key state in one event.

**What pre-rotation defends, and what it does not.** **A party holding a threshold of the standing root signs a `KeyState` that lists its own key `Current` in the `#active` role and drops the controller's key, and every verifier that resolves the identity then reads that party's key as the identity's one operational key.** That event carries a root signature alone, it produces no divergence, and R6 therefore never runs on it, so only a full replacement of the key state removes what that party installed. **Pre-rotation stops that party from installing a new root set and stops nothing else**: the controller holding the pre-rotation key installs one under a `RootRecovery` and wins the fork under the root rule of R6. **A party holding a threshold of the standing root takes the identity's operational keys, and this design does not solve root theft.** Alec ruled on 2026-09-14: "stop trying to solve root theft. it's not solvable."

A verifier derives every historical key's condition by replaying the log. A state-carrying event asserts only the condition changes it makes: its key list carries every key `Current` at that position and, for each key the event drops, one entry naming that key's new condition — `Superseded` where a successor takes its role, `Retired` where none does, or `Compromised{from: N}` where the standing root asserts the compromise. The list names no key it neither keeps `Current` nor drops, and every state-carrying event — a `KeyState` and a `RootRecovery` alike — records a dropped key this way. A `RootRecovery` also carries the fresh root set and the operational keys it installs. No event erases an earlier event.

§9.7.1's classification table states the three arms by which a verifier resolves a `signing_key_id` fragment and states which arm an artifact's class selects. A verifier applying them here reads a historical key's condition out of the replay this paragraph describes.

A `Compromised{from: N}` entry carries a position N bounded by the sequence of the event that carries it, and a verifier MUST reject an entry whose N exceeds that. **N is the controller's record of where the standing root asserts the compromise began, and no rule compares N's value against anything.** The log-anchored evidence class of §9.7.1 reads the presence of the entry and applies its first-seen test, which is that class's sole acceptance condition; it does not read N. No content rule reads N either, and no rule in this section rejects a controller's event on account of content. A verifier that invented a comparison — rejecting an artifact whose `key_state_head` names a position at or after N — would reject an honest party's own snapshot that it held first.

**R9 — storage, service, and retention.** **A validating relay keeps each of an identity's chains, at that identity's routing id.** Two prefix-related chains are one chain and the relay keeps the longer; two divergent chains are two chains. A chain runs from the inception event to its head, and a relay serving a head serves every predecessor. Keying what the relay keeps on the root set alone would put every divergence that has not yet installed a new root into one chain and hand it to whoever published first, so no verifier would hold both chains and R6 would never run.

**The write rule.** A relay accepts a frame on one of two preconditions: its first event is an inception event that recomputes to the routing id's identifier, or its first event's predecessor digest names an event the relay already holds at that routing id. A relay rejects a frame meeting neither. **The publisher's remedy is to publish from the first segment the relay does not hold, which the publisher learns by querying the routing id**, so the remedy reads the relay's current state and never the publisher's memory of an acknowledgement. Without that rule a relay receiving a second frame holds no test deciding whether to accept it, and two relays accept different second frames at one routing id.
...
**The three write outcomes.** For an accepted frame the relay compares each event's preimage digest against the event the chain holds at that position. Every digest matching inside that chain's range leaves the relay's stored state unchanged, because the relay keeps the copy it holds and that identity keeps the position in the ring its first write gave it, **so a re-PUBLISH of bytes the relay already holds moves nothing**, which is what makes the re-publication R10 states conditional rather than a cadence every SDK runs. Every held digest matching, with the frame continuing past the head, extends that chain. A first differing position makes the frame's events a second chain.

**Serving order and the page walk.** **A relay serves the frames a routing id holds in ascending order of each segment's first sequence across every chain it holds there, interleaved and never one chain at a time.** The shared prefix therefore reaches a resolver once and both divergent suffixes reach it from the fork position upward, so a fetch bound reached mid-walk truncates every chain at one sequence rather than truncating one chain whole. A relay that served one chain whole before it served the next would hand a resolver a complete chain from an inception event to the head of whichever chain it served first, and a resolver that reached the fetch bound there would stop before it read one frame of the other chain, adopt the head it holds, and never run R6 for want of a second suffix. The walk is over a routing id and not over one chain: a resolver re-issues QUERY with `since` set to the `stored_at` of the last frame it accepted at that routing id **from that relay**, takes every frame each page returns, and assembles frames into chains itself by predecessor digest. **`stored_at` is the issuing relay's own cursor and is opaque to the resolver**, which keeps one `since` per relay and never carries one relay's value to another: one listed relay that stamped a near-maximal `stored_at` on page one would otherwise advance a shared cursor past every later page at every relay, and the walk would stop on the truncated prefix that relay served. It stops where every chain it opened runs from inception to a head, or where a page returns no new frame. A resolver advancing `since` per chain would skip the frames of a second chain interleaved below its own high-water mark.

**The two further object kinds.** A verifier keys a retained cosigned head on the subject, the witness and the `event_digest` — the same three terms the relay uses (`03-identity.md` §3.10.2) — and never on the retained suffix, so evicting a suffix drops no cosigned head and both heads of a fault-proof pair survive at one key. It retains at most one conflict statement per pair of witness and retained suffix, keeping the highest `observed_at`. `MAX_WITNESS_SET_SIZE` multiplied by `MAX_RETAINED_SUFFIXES` bounds each at 256 objects per identity: the `event_digest` fan-out is bounded by `MAX_RETAINED_SUFFIXES`, because an honest witness releases one successor per baseline and a faulty one releases at most one head per divergent suffix the verifier retains.

**Why a flood of chains takes no identity.** Every event a chain holds stands under a root the relay verified from the inception event forward, so a party that opens many chains has signed every one of them under a root it holds. Chain count confers nothing, because rank reads two properties and neither gets easier by publishing more forks.

**Segmentation.** A frame carries a contiguous segment of one chain, naming the segment's first and last sequence. A chain carries no ceiling on the keys it installs and none on its sequence, because a state-carrying event names only the keys that are current and the conditions it changes, so an event's size follows the identity's configuration and never the chain's length.

**The retention bound.** A verifier and a validating relay retain per identifier at most `MAX_RETAINED_SUFFIXES` divergent suffixes and at most `MAX_RETAINED_BYTES` of divergent-suffix bytes. Each bound reads the eviction rank below, and neither refuses a suffix on arrival without reading that rank. Both count divergent suffixes and count no byte of the accepted chain, which the storage budget below governs instead. A relay MAY configure a lower per-identifier byte value and MUST NOT configure a higher one. **The byte bound governs a rank-2 and a rank-3 suffix and governs no rank-1 suffix**: a rank-1 suffix is admitted whatever value the relay configured, so a lower configured value evicts rank-2 and rank-3 suffixes alone. **A rank-1 suffix is therefore bounded by the protocol ceiling alone and by no value an operator configures**, and where one rank-1 suffix reaches that ceiling the party refuses further arrivals on it and returns `Contested{…, residue: n}`, which is the answer this rule already states for two rank-1 suffixes at the ceiling. **The residue that leaves, stated rather than repaired, and the key its party holds:** rank 1 takes a root signature verifying against the root standing at the revealing event's predecessor, and `RootRecovery{Lost}` carries a reveal and a root signature by the installed set K′ and no signature by the standing root, so **a party holding one unspent pre-rotation key composes a rank-2 suffix and never a rank-1 one**. A rank-2 suffix sits inside the byte bound and is evictable in the rank order, so a lower configured value does reach it and that party's flood is bounded by whatever figure the operator chose. **The party that fills `MAX_RETAINED_BYTES` at a victim's routing id with bytes no configured value reaches is the party holding the standing root**, which this design does not defend against. **The residue that remains inside this design is the identity's own controller forking its own chain**, which holds its own root by construction, composes rank-1 suffixes, and reaches the protocol ceiling at every relay; the coverage test below counts those bytes, so that identity is uncovered and the ring reclaims it whole. A verifier reading such a relay can spend its whole fetch allowance on that suffix and return `Inconclusive{MissingEvents}`, which is the collision the fetch bound below discloses as open and names a third decision to close. **That exemption is from a configured value and never from the protocol ceiling**, and it is no exemption from the fetch bound below: where the rank-1 suffixes a verifier or a validating relay retains fill the protocol ceiling, that verifier or that relay refuses the arrival, as the rank-1 flood paragraph states. Without a byte bound the count bound leaves a suffix's bytes unbounded: a holder of one spent pre-rotation key forks below the event that spent it, signs a `RootRecovery{Lost}` installing its own root, and appends four million `KeyState` events on that rank-2 suffix, so every verifier that resolves the victim fetches about 1.1 GB and performs four million signature verifications before R6 can rank. A byte bound with no rank term would be defeated by the cheapest suffix to manufacture, which is why the rank term the eviction rank defines is what this bound reads.

**The relay's declared write policy.** A relay whose retained storage fills displaces an uncovered identity under the ring buffer below, and refuses only where no such identity remains. ADR-063, the inception-derived key-event-log identity substrate, carries the rulings that settled a relay's denial terms, its ring buffer and the rent that covers an identity against displacement, and carries the withdrawal of `MAX_ACCEPTED_CHAIN_BYTES`, so no per-chain byte ceiling stands.

A validating relay declares its write policy in `relay_config` (`18-addressability-and-deployment.md` §18.3.3): `rate_limit_publish`, a per-identifier storage budget, a total storage budget, `per_publish` and `per_byte_stored` (`19-economic-governance.md` §19.8), and the three rent terms the ring-buffer paragraph below states. A publisher reads that policy through ADR-004's `POLICY` query (§9.10.12), whose answer carries the `DeclaredWritePolicy` type `03-identity.md` §3.10.10 declares. The relay refuses a retained-kind PUBLISH that fails a declared term with `IdentityError::StorageBudgetExceeded{scope, value}` (`03-identity.md` §3.10.10), whose four scopes are `Identifier`, `Total`, `RateLimit` and `Payment`, and it keeps serving every record it already holds. The refusal reaches every one of the four retained kinds, because the coverage test below counts a service record's bytes and the ring below displaces all four together. <!-- scp:fragment id="refusal-value" -->**`value` carries the declared term the refusal names — the budget figure, the rate, or the price — so the one object a publisher holds after a refusal states what would satisfy it in every term but the rail-native address it settles to**<!-- scp:end id="refusal-value" -->, which the two open clauses below hold open and which `03-identity.md` §3.10.6's `Payment` row discloses to the publisher beside the remedy it states. `03-identity.md` §3.10.10 declares that field as the `BudgetValue` enumeration, which types it per scope, and the wire code `4041` carries the same field.

<!-- scp:fragment id="receipt-exempts-declaration" -->
**A relay that charges declares which refusal scopes a current rent exempts an identity from.** `relay_config` carries `receipt_exempts`, a set whose members are scope names, and ADR-004's `POLICY` query returns it, so a publisher reads what its money buys before it spends. **A relay that charges and declares nothing exempts nothing**: an absent or empty `receipt_exempts` exempts no scope, and an implementer reads that declaration rather than assuming a set. **`Identifier` is the one member a relay may declare.**
<!-- scp:end id="receipt-exempts-declaration" -->
**This protocol asserts nowhere what a payment buys; it asserts that the relay declares it and that the declaration governs.** **Alec ruled on 2026-09-14: "it should really only apply to storage (amount, duration). not to traffic."** What a relay sells relief from is therefore a storage refusal, in amount and in duration, and it sells no relief from the rate limit. **`RateLimit` names a rate of writes, which is traffic, so no declaration exempts any party from the rate limit**, and Alec's sentence is what removes it. `Payment` names a receipt rather than storage, which his sentence excludes on the same ground, because a declaration naming it would make the receipt the object that exempts the check reading the receipt. `Total` names a storage amount and stays undeclarable on a ground his sentence does not reach and does not need to: it is the capacity the operator's own host holds, and an exemption from it names an outcome no relay can produce whatever its operator declared. The exclusion of `Total` is therefore the executor's and the other two are his.

**The currency of the exemption is the identity's current rent and never a receipt a write carried.**

<!-- scp:fragment id="identifier-exemption" -->
**A declared `Identifier` exemption stops the refusal arm of the per-identifier budget and stops no eviction**, for the identity whose rent is current, for as long as that rent is current, and up to the quota that rent bought and no further. **Its refusal test is one arithmetic: the relay tests, against the declared per-identifier budget, the bytes it holds for that identity less the smaller of that identity's counted bytes and its current quota.** **A rank-2 and a rank-3 divergent suffix stay evictable in the eviction-rank order below whatever that identity's coverage**, because the coverage test counts none of their bytes, so an exemption over them would shelter bytes nobody paid for.
<!-- scp:end id="identifier-exemption" -->

A relay that subtracted nothing would shelter every byte it holds behind one unit of rent, and a relay that subtracted the whole counted figure would shelter bytes no rent bought; the smaller of the two figures is the bytes the rent actually covers, and it is the one reading under which a controller that pays the `units` `03-identity.md` §3.10.6 derives is admitted.

**`Identifier` refuses no frame whose events all extend the accepted chain the relay already holds at that routing id**, and the relay counts those bytes against the per-identifier budget and against `Total` like every other byte it holds. The refusal arm therefore binds a frame that opens a chain, a frame that forks one, and a service-record write, and binds no frame that advances an accepted chain. **The bytes that admission puts past the budget are priced and reclaimable rather than free**: the identity's held bytes then exceed its quota, so it is uncovered from that moment and is an ordinary displacement candidate the ring reclaims whole; `Total` still refuses the frame where displacing every uncovered identity leaves the relay short; and the per-address rate limit above bounds the rate at which any party reaches this arm. **What this admission closes is the rotation deadlock**: an entry holding a stale head for an identity serves the key state the `RENT` signature check below resolves against, so a controller that rotated its `#active` key can sign no `RENT` that entry accepts, and its one remedy is the PUBLISH that advances the head — which is the write this arm admits, whatever the per-identifier budget, at every entry that has not refused it under `RateLimit`, `Payment` or `Total`. **The bound the per-identifier budget no longer places on an identity's own accepted chain is placed by `Total`, by the ring and by the rate limit**, and this rule states that trade rather than hiding it.

A rule that lifted a refusal on a receipt a write carried would lapse when that write ended, which is the one-time purchase Alec's rent ruling replaced; an unbounded exemption would let one rent payment fill a relay's declared capacity at no marginal cost, which "get quota a of b thing" does not say. An exemption an identity bought lapses with the rent that bought it, and the per-identifier budget binds that identity again from that moment. **A `relay_config` whose `receipt_exempts` names `Identifier` and which declares no rent terms is invalid**, because rent is the one thing that buys the exemption and a publisher refused at such a relay holds a remedy nothing can satisfy.

**A controller's own rent buys room for the controller's own bytes, and every divergent suffix at that routing id stays evictable exactly as it is at a relay selling no coverage.** A stop over the eviction arm would shelter bytes the coverage test does not count, which a controller's own fork reaches as readily as a third party's, and it would leave a controller that wants a third party's suffix evicted no act but stopping its rent — which arms the ring against that controller's whole record set in the same act, so the controller could not tell which loss it was choosing.

<!-- scp:fragment id="rate-limit-check" -->
**The rate-limit check counts every PUBLISH and every `RENT` against the publishing address, reads no field of a receipt, and decodes nothing.** No declaration exempts any party from it, because a relay sells relief from a storage refusal and sells no relief from the rate limit.
<!-- scp:end id="rate-limit-check" -->

The relay counts the arriving message against the publishing address and refuses it where the count passes `rate_limit_publish`, whatever that message carries and whatever the relay declared. A check that admitted a message for carrying a `payment_receipt` field would admit every message carrying arbitrary bytes under that name, and each one would reach `PaymentAdapter::verify`, which hands an unauthenticated party one outbound rail query per request. A relay that skipped the check would have to know a receipt is valid before it ran the check, which drives that call ahead of the limit and reaches the same flood. The check reads one address's counter, decodes nothing, and runs before any check that leaves the relay's host. `rate_limit_publish` is a declared term, so the refusal it produces is `StorageBudgetExceeded{RateLimit, value}` on a PUBLISH and on a `RENT` alike, and the wire code is `4041`.

**The protocol fixes each term's shape and never its value.** A term declared zero is off, and an absent term is off at every term but one: an absent `rate_limit_publish` is the 6,000-per-minute default `18-addressability-and-deployment.md` §18.3.3 states, and a relay turns that term off by declaring zero. **A refusal is never a verdict**: the publisher's recourse is the next entry of its fallback set, and a publish cycle is confirmed when at least one entry accepts it rather than when one answers (`03-identity.md` §3.10.6). **No event kind is exempt from any scope.** A kind-keyed exemption let an identity's own holder write unbounded exempt permanent bytes by rolling over its own commitment. **No per-publisher scope exists**, because a PUBLISH carries no key (ADR-004, the SCP native relay protocol) and a minting party's signing key is fresh for every identity it mints.

**`Total` is the whole retained capacity a relay declares, and the identities the ring below holds covered and the identities that ring may displace are two partitions of that one figure**, so a relay stores no more than the ceiling its operator declared.
<!-- scp:fragment id="total-scope" -->
**`Total` refuses a write where displacing every uncovered identity the relay may displace still leaves the retained capacity short of the arriving frame's bytes, and refuses no write while one such identity remains**, because the ring below displaces the oldest-established uncovered identity instead. That one refusal covers the write that carried a receipt and the write that carried none, and it returns `StorageBudgetExceeded{Total, value}` carrying the declared total budget, which is the figure the `POLICY` query returns and the figure that states what would satisfy the refusal.
<!-- scp:end id="total-scope" -->

**Every validating relay runs the ring, and the declared total budget sets the ring's size only where the operator declares one.** Where a relay declares no `storage_budget_total` it runs its ring over the retained capacity its own host holds: it displaces uncovered identities to make room exactly as a declaring relay does, and it answers a write it cannot take after displacing every uncovered identity it may displace with ADR-004's `5001` STORAGE_FULL rather than with `StorageBudgetExceeded{Total, value}`, because `5001` names a relay out of room and names no declared ceiling a publisher could satisfy. A publisher records that answer as `EntryResult::Failed{code}` (`03-identity.md` §3.10.10). **An absent `storage_budget_total` is the host's own capacity with the ring beneath it, and it is not "no limit".**

<!-- scp:fragment id="identifier-eviction" -->
**Under `Identifier` pressure the relay evicts before it refuses:** it evicts that identifier's retained divergent suffixes in the eviction-rank order below, **evicting the retained suffix carrying the highest rank number first**, and refuses only where evicting every suffix it may evict still leaves the per-identifier budget exceeded.
<!-- scp:end id="identifier-eviction" -->
Rank 1 is the strongest claim and rank 3 the weakest, so a relay evicting by the highest rank number evicts the weakest claim first and evicts a controller's own second device's rank-1 suffix last. **The two pressures act on two units and neither is an exception to the other.** The ring acts when the retained capacity fills and the unit it moves is one identity; the `Identifier` eviction acts when one identity exceeds its per-identifier budget and the unit it moves is one divergent suffix. **`Identifier` bounds what the relay holds for one identity**, so one identity holds no more than the per-identifier budget wherever an operator declares one and no exemption stands. **A declared `storage_budget_identifier` clears the floor `MIN_STORAGE_BUDGET_IDENTIFIER` registers (§9.18.17), and a declaration below it is invalid** (`18-addressability-and-deployment.md` §18.3.3); §9.18.17 discloses that the floor's own figure is open and names the two encoded widths it waits on.

**The ring buffer.** A relay's retained storage is a first-in-first-out ring of `Total` bytes, **and the unit it displaces is one identity: everything the relay holds for that identity, displaced together.** That is its chains at `scp:did:`, its service record at `scp:svc:`, and the `scp:wit:` and `scp:wcf:` records naming it as subject. The ring displaces no single frame and no single chain, so a chain the relay still holds never lacks its inception event and a fault proof's two cosigned heads are never separated by a displacement. **The ring is ordered by a monotonic per-relay counter and by no clock.** The relay increments one counter on each identity's first binding-valid frame and records that value as the identity's establishment position. The counter never decreases, so no correction to the host's wall clock inverts the order. Two identities never take one value, so the order is total and two relays holding identical identities displace the same one. The relay stores each position durably beside the records it orders, so a restart restores the order rather than leaving the relay with none. A later accepted write adds to what the relay holds for that identity and leaves its position unchanged, so no write refreshes anything and a quiet identity's position is the position its first write gave it. **A re-PUBLISH of bytes the relay already holds moves nothing**: the relay keeps the copy it holds and the identity keeps its position, which is what makes the re-publication R10 states conditional rather than a cadence every SDK runs. **An identity the ring displaced and which then re-publishes takes a fresh counter value, because the relay is establishing that identity again.** **When the ring is full, the relay displaces the oldest-established uncovered identity other than the identity whose write it is admitting, and takes every record it holds for that identity together. A relay never displaces the identity it is writing for.** Where displacing every uncovered identity it may displace still leaves the ring short of the arriving frame's bytes, the relay refuses that frame for `Total`, returning `StorageBudgetExceeded{Total, value}` carrying the declared total budget, so the publisher reads that this relay is too small for the chain it is publishing and moves to the next fallback-set entry. Without those two sentences a relay whose ring is smaller than one chain drops the segments it just accepted in order to store the segment whose predecessor those segments carried, the arriving frame then fails the write rule for a predecessor the relay no longer holds, and the publisher's re-send reaches the same state on every attempt. **A junk write at an identity's address displaces nothing, because the relay refuses it before it stores it**: relay-side validation and this rule's own test for what may open or extend a chain (`03-identity.md` §3.10.2) admit only a binding-valid frame.

**Coverage is rent.** Alec ruled on 2026-09-14: "it's rent. as long as you keep paying. operator says, \"pay x at y frequency, get quota a of b thing\"." **A relay that sells coverage declares three terms in `relay_config` — a rent price, a rent period, and the quota of stored bytes one rent payment covers — and an identity is covered up to that quota while its rent is current.**
<!-- scp:fragment id="rent-arithmetic-quota" -->
**One verified rent payment of `units` sets the identity's quota to the greater of its standing quota and `units` times the declared quota, and the standing quota is the figure the relay recorded while that identity's rent is current and is zero for an identity whose rent has lapsed.**
<!-- scp:end id="rent-arithmetic-quota" -->
<!-- scp:fragment id="rent-arithmetic-period" -->
**The same payment sets the identity's rent expiry to one declared period past the later of its standing expiry and the instant the relay verified the payment, and the declared period is the duration one payment buys whatever `units` that payment names.**
<!-- scp:end id="rent-arithmetic-period" -->
<!-- scp:fragment id="rent-arithmetic-price" -->
**The relay verifies the receipt against `units` times the declared rent price, so the arithmetic multiplies the declared quota and the declared price by `units` and adds the period once.**
<!-- scp:end id="rent-arithmetic-price" -->

A lapse therefore ends the quota with the coverage, and the greater-of clause keeps the one property it was written for: a renewal inside the term never lowers what a controller already bought. Without the lapse arm the greater-of clause makes one large payment buy a quota floor the identity then renews at one unit's price for ever, which falsifies this rule's own sentence that the operator's declared price bounds the rate at which a funded party converts declared capacity into covered bytes. **A relay drops an identity's rent record once that rent has lapsed and the relay next reads it**, so a lapsed record does not stand indefinitely beside records it no longer covers, and a relay holding no chain and no unlapsed record names one state rather than two.

That one arithmetic answers four questions. A controller raises its quota by naming a larger `units` on its next payment, which is the reachable act the `Identifier` refusal row of `03-identity.md` §3.10.6 states. A controller that renews at a smaller `units` than the quota it already holds keeps that quota, because the arithmetic takes the greater of the two figures, so neither a routine renewal nor a party replaying an older signed `RENT` lowers what a controller bought. A controller that renews early keeps the remainder of the term it already holds and gains one period past it. A controller that renews late starts a fresh term one period from the moment of verification. **ADR-004's `RENT` operation carries `units` so the relay reads which quota the controller asked for, and the relay supplies `units` times the declared rent price as the price it verifies the receipt against.** **A relay refuses a `RENT` under the `Payment` scope where `units` times the declared rent price or `units` times the declared quota exceeds what a `u64` holds, and it computes both products under a checked multiplication**, because `units` is a `u32` the payer chooses and both declared terms are `u64`, so a relay that multiplied without the check wrapped in a release build, panicked on a signed message in a checked build, or recorded the true product, which is three answers to four fields on the wire. **An identity whose rent lapses is uncovered from that moment and is an ordinary displacement candidate again**, so no payment buys coverage that outlives the period it paid for. **A relay that declares no rent terms sells no coverage, and every identity it holds is uncovered.** `19-economic-governance.md` §19.8 obliges the shipped community relay list to carry free entries, so a free entry is one at which no identity can be covered and every identity is a displacement candidate in establishment order. **A controller wiring `pay_rent` across its fallback set therefore reads which entries answer `EntrySellsNoCoverage` from the three terms each entry declares**, by construction rather than by discovering it one entry at a time (`03-identity.md` §3.10.10). **Coverage is a cap on what rent protects and never a credit a write earns**: the relay reads, for each identity, the bytes it holds for that identity against that identity's current quota, and the identity is uncovered where the first figure exceeds the second or where the rent has lapsed. **The rent terms supersede `per_byte_stored` as what a controller pays for storage of a retained kind**, because that member is a one-time fee and a retained kind never expires; `per_byte_stored` keeps its meaning for a blob the relay expires under a TTL, and `per_publish` keeps its meaning as the fee for the publish operation and buys no coverage. **The relay records each identity's current quota and the instant its rent expires, both durably beside the records they cover**, and it reads its own clock to decide whether a rent term has elapsed. **A relay that cannot read an identity's rent record treats that identity as uncovered**, which is the fail-closed direction for the ring below; the payer's recourse is that relay's operator rather than any rule of this protocol, the relay answers no distinct error for it, and a controller reading that relay's rent state sees a zero quota it cannot tell from a lapse. **It treats a term as elapsed only past that instant plus the clock-skew tolerance of §9.14, the clock and ordering model**, which is the same tolerance the relay proof of control and a witness's `observed_at` take, so a controller renewing inside that tolerance is never lapsed for a clock it cannot read. **That clock decides which partition of the displacement order below an identity falls in, and it orders nothing inside a partition**: the ring's order inside a partition is the monotonic counter above, and no relay measures how long it has held a record. **Two relays whose clocks differ therefore displace different identities where one relay reads a term as elapsed and the other does not**, which is a disagreement about an operator-declared term, and the clock-skew tolerance above is what bounds it.

<!-- scp:fragment id="coverage-test" -->
**The coverage test counts the bytes of the accepted chain the relay holds at `scp:did:` for that identity, the bytes of its service record at `scp:svc:`, and the bytes of every rank-1 divergent suffix the relay holds at that routing id.** It counts no byte of a rank-2 or rank-3 divergent suffix and no byte of the `scp:wit:` and `scp:wcf:` records other parties write about it. **The bytes it counts are the `value` bytes of the frames the relay stores**, and the relay's own establishment position, its chain index and its rent record count against neither the coverage test nor the per-identifier budget.
<!-- scp:end id="coverage-test" -->
**A rank-1 suffix is counted because the relay can attribute it**: rank 1 takes a root signature verifying against the root standing at the revealing event's predecessor on the accepted chain, which is a signature the identity's own standing root produces and no other party's key produces, so those bytes are the identity's own by a check the relay already runs. **The party that writes a rank-1 suffix at an identity's routing id holds that identity's standing root**, which sits outside what this design defends against, so counting a rank-1 suffix hands no third party a lever it did not already hold.

A displacement still takes all four kinds together, which is what keeps a chain with its inception event and a fault proof's two heads at one relay, and the bytes the test does not count still count against `Total` like any other byte the relay holds. **The test names a set a relay can compute**, because a PUBLISH carries no key and no relay can attribute a stored frame to a publisher. Counting a rank-2 or a rank-3 suffix would let a party holding one pre-rotation key write a fork at a paying controller's routing id, drive that controller past its rented quota, and hand its whole identity to the ring for the price of one frame; counting the witness-written records made a controller uncovered at its first designated witness's cosignature, for bytes it cannot compose, cannot refuse and cannot buy. **A rank-2 and a rank-3 suffix are bounded by the per-identifier budget's eviction above and by the retention bound above, and by no coverage term**, and a declared exemption stops neither eviction, so those suffixes are evictable whatever an identity's coverage and no payment holds bytes the test does not count. **A rank-1 suffix is bounded by the coverage test instead**: the eviction above never reaches one and the protocol ceiling is the only figure that caps it, so an identity that composes rank-1 suffixes to the ceiling exceeds its quota, is uncovered from that moment, and is an ordinary displacement candidate the ring reclaims whole. Without that count a controller forking its own chain would hold up to `MAX_RETAINED_BYTES` at every relay for one unit of rent, with no eviction rule reaching those bytes and no coverage term counting them, and two hundred such identities would take a 100 GiB relay permanently.

**The ring displaces uncovered identities alone, and it never splits a partly covered one.** An identity holding more published bytes than its current quota, and an identity whose rent has lapsed, are each an ordinary displacement candidate, and the ring displaces either whole, taking every record the relay holds for it together, because a split would leave a chain without its inception event and would separate a fault proof's two cosigned heads. **The ring reaches every wholly uncovered identity, in establishment order, before it reaches any partly covered one, and reaches partly covered identities in establishment order after that**, so a controller that rented most of its bytes loses them only after no identity that rented none remains. **An identity is wholly uncovered where it holds no unlapsed rent record at this relay, and partly covered where it holds one whose quota its counted bytes exceed**, so the two classes partition the displaceable set and a relay reads which class an identity falls in from the two figures it already stores. **A displacement takes the records the relay holds for an identity and leaves that identity's quota and rent expiry standing until the rent lapses**, so a controller that re-publishes inside the period it paid for is covered again at that relay and pays no second rent for it. **An identity whose held bytes pass its quota, or whose rent lapses, keeps the establishment position it already holds**: the relay refuses no write on that ground and answers no distinct error, because no declared term refused it. **A controller keeps its identity covered by paying rent at the frequency the relay declares, and it raises its quota by naming a larger `units` on its next payment**; neither act requires it to publish a byte, and the rent paragraph below states how a receipt that pays rent reaches the relay. **Coverage's rule is protocol and coverage's terms are the operator's declaration**: the `receipt_exempts` declaration above names refusal scopes and a displacement is not a refusal, so a relay's rent terms rather than its exemption set decide whether an identity is covered, and a relay that declares no rent terms holds every identity uncovered. **There is no idle clock, no refresh write and no pressure ordering**: no relay measures how long it has held a record, no party re-publishes a record to reset a clock, and the relay displaces by coverage and then by establishment order and by nothing else.

**What a displacement leaves each party to do.** A controller whose self-check finds its record absent at a fallback-set entry re-publishes its chain there (R10), which establishes that identity at the newest position in the ring, and it re-publishes its service record with the chain, because the displacement took every record the relay held for it. A witness re-cosigns the subject's head on the next offer that subject's controller makes it, which the self-observation cadence above drives, because a witness fetches nothing on its own account and cosigns nothing it was not offered (§9.7.4.3); that cosignature is an ordinary write into what the relay holds for the subject, and its bytes count against `Total` and against no identity's coverage. **A relay that holds no chain for a subject refuses every `scp:wit:` and `scp:wcf:` write naming that subject** (`03-identity.md` §3.10.2), so the controller's re-publication of its chain is what lets a witness record land there again. A conflict statement is held by the witness that wrote it and by the controller that fetched it. **A third party's writes raise nobody's coverage**, because the coverage test counts the bytes of the accepted chain and the service record this relay holds for the identity; a third party's *payment* is a different act, and the `RENT` paragraph below states who may make it. Every controller holds its full log (R10), so a displacement changes where a record is stored and destroys no record, and a controller that lost the device holding that log has lost the log.

**The two residues the ring does not bound.** **The bandwidth residue.** A party minting distinct identities displaces every uncovered identity at a relay by writing binding-valid frames at that relay's full ingest rate, for as long as it wants honest records absent: free at a relay that charges nothing, and at the declared price per write at a relay that charges a per-write fee. The per-address rate limit above bounds the rate, and no rule of this protocol bounds the duration. That is the bandwidth attack every free public service faces, and the party that answers it is the curator, with more free entries under distinct operators or with paid ones (`19-economic-governance.md` §19.8); who curates the shipped list and what obligations that curator carries are open (`18-addressability-and-deployment.md` §18.5.1).

**The funded-capacity residue.** A funded party converts a relay's declared retained capacity into covered bytes by renting quota for identities it mints, at the relay's own declared price, and while those rents are current the ring holds no uncovered identity it may displace and `Total` refuses every write from every other party. Alec's rent ruling bounds that in time where a one-time fee did not, because every such coverage lapses at the end of the period it paid for. The operator's declared price is what bounds the rate and the declared period is what bounds the duration, and that is the sentence an operator reads when it sets its rent price against its declared total budget. **No rule relates the quota a relay sells to the capacity it holds**, so an operator that sells more quota than `Total` holds reaches the same state honestly.

**When a relay charges a write, the price it verifies against, and the `Payment` refusal.** A PUBLISH carries an optional `payment_receipt` (§9.10.12): the `PaymentReceipt` bytes of `19-economic-governance.md` §19.2.1, tagged with the adapter that issued them.
<!-- scp:fragment id="publish-charge-trigger" -->
**A relay charges a PUBLISH at `scp:did:` or `scp:svc:` where it declares `per_publish`, and it charges one on no other declaration.** **It charges no `scp:wit:` and no `scp:wcf:` write** (§9.7.4.3), which are the other two retained kinds, so no sentence of this rule reaches a write no party can pay for.
<!-- scp:end id="publish-charge-trigger" -->
It supplies that fee as the price it verifies the receipt against, and it verifies the receipt through the named adapter, which is `PaymentAdapter::verify` of §19.2.1. Alec named rent as a bound a relay already holds, on 2026-09-13: "relays can already charge rent." **It refuses `Payment` where a write it charges for carried no receipt, where the verification failed, or where any of the three receipt checks below failed.** **`per_byte_stored` triggers no receipt check on a retained kind**, because the rent terms superseded it there and it prices nothing a retained write stores; it keeps its trigger on the blob path it still governs. **A receipt pays one thing, and the operation carrying it says which**: a receipt presented on a PUBLISH pays the publish fee, and a receipt presented on `RENT` pays rent. No receipt pays both, because no field of either message distinguishes an overpaying write receipt from a write-and-rent receipt, so a relay reading the sum refused every honest write-only payer and a relay reading the fee alone took a controller's rent money and credited nothing. A controller that wants both sends one PUBLISH and one `RENT`. **Every relay that charges anything, a rent price included, declares an `economic` object**, so a relay that sells storage and charges nothing per write still holds the `economic.currency` the currency check reads; a `relay_config` declaring a rent price and no `economic` object is invalid, and a publisher that reads one records that entry as `Skipped`. **"Paid" means "carried a receipt this relay verified" in every storage rule of this section, and "covered" means what the coverage test above states over an identity**, so the `Payment` scope reads a field the wire carries and the ring reads the bytes the relay holds against the bytes a controller rented.

**Three checks decide a receipt, and every one of the three reads a term `PaymentAdapter::verify` returned.** That call takes the receipt together with the relay's own declared currency and the price the relay computed, and returns a `VerificationResult` carrying what it verified against the rail (`19-economic-governance.md` §19.2.1); the relay's checks read those returned values and never the receipt's own fields, because a presenting party composes every field of the receipt.
<!-- scp:fragment id="receipt-checks" -->
The three: the **rail-bound receipt identifier** the call returned is one this relay has not already spent; **`amount_covers`** is true, which is the call's answer to whether the settlement covers the price this relay supplied; and **`currency_matches`** is true, which is its answer to whether the settlement is denominated in the currency this relay supplied.
<!-- scp:end id="receipt-checks" -->
A relay compares no verified amount and no verified currency of its own, because the call performs both comparisons against the terms the relay passed in and returns the answers; two rails denominate one integer in two monies, and an amount compared without its currency compares nothing. A relay that charges accepts the message only where the call returns valid and all three hold, and refuses `Payment` otherwise. **A relay refuses, under `Payment`, a receipt whose adapter tag names no member of its declared `economic.payment_adapters`**, and the refusal's value carries that set; otherwise the presenting party picks which of the relay's verifiers runs.

**The payee comparison is deleted, with the `payee_matches` term and the payee parameter it read.** The operator configures its adapter with its own account, so the call answers whether the settlement paid this relay, and a comparison against 32 raw digest bytes of an SCP identifier had no work to do and no registered rail that could answer it. `economic.payee` stays as the identifier a relay declares as the party it receives payments under, and no check on the relay's path reads it.

**The relay spends the rail-bound receipt identifier at the check that commits the message's effect** — the placement of a frame's events on a chain, or the crediting of a rent quota and expiry — so a message the relay rejects costs the payer no receipt. **The relay holds that spent set durably**, because a restart that lost it would let one settled payment buy a second write. **The set is keyed on the pair of the rail-bound receipt identifier and the operation the receipt paid for**, which is `PUBLISH` or `RENT`, so one settled payment is spendable once per operation at one relay and a write fee and a rent payment are never spent against each other. **The relay reads the operation off the message it is processing and never off a field of the receipt**, so the key costs no returned term and no receipt field. **What the spent set does, stated exactly: it stops one settled payment being spent twice at one relay under one operation, and it decides nothing about which party's first spend counts.** A settled payment on a public rail is presentable by any party that reads that rail until it is spent, and the relay spends it for whichever message arrives first under that operation. **On the write path no field binds a receipt to a presenting party, and the residue that leaves is one write's fee taken from the payer**, which this rule states rather than repairs, because a PUBLISH carries no key. **That fee is the whole loss on the write path, and the pair key is what bounds it**: a party presenting a rent payer's settlement on a PUBLISH spends the `PUBLISH` half of the key and takes one write, and the payer's own `RENT` still spends the `RENT` half and takes the coverage it paid for. **Nothing on the relay's path verifies the payer's signature over the receipt**, because that preimage waits on the identifier's textual form and because a relay resolving `payer_did` would resolve a key the presenting party chose; the receipt's signed payload therefore joins §9.5.2's register of preimages that wait on that form.

**Two clauses of these checks are open, and this revision writes neither**, in the shape the fetch bound below uses for its own. **First, how a publisher learns the rail-native address it settles to.** Deleting the payee comparison removes the only reader of `economic.payee` as an SCP identifier, and a publisher's own adapter cannot pay an SCP identifier on any rail `19-economic-governance.md` §19.2.7 registers. The two candidate shapes, and this revision chooses neither: `relay_config` gains a rail-native payee address per registered adapter, which the `POLICY` query returns; or the adapter holds the mapping from the relay's declared identifier to that relay's address on the adapter's own rail, and the corpus states who writes that mapping. **Second, what binds a receipt to one relay rather than to one operator.** The spent set is per relay, so one settled payment is spendable once at each relay one operator runs, and the operator's declared price bounds nothing across its own fleet. The two candidates, and this revision chooses neither: the receipt binds to the 33-byte operator key a community-relay-list entry carries, which one operator key may cover two listed entries; or `relay_config` gains a relay identifier distinct from its operator and the receipt binds to that.

**A receipt that pays for a write accompanies one PUBLISH, and the relay spends it once, for the identity that PUBLISH names.** **A receipt that pays rent reaches a relay on `RENT`, an ADR-004 client operation of its own, and never on a PUBLISH**, because an identity that publishes nothing for a whole rent period would otherwise hold no way to pay and would go uncovered by the next period. The rejected alternative was a PUBLISH whose blob is absent: the retained-kind path reads that blob at the `retain` flag's scope, at the routing-id derivation and at chain verification, so each of those checks would need an arm for a frame that is not there.

**`RENT` names its beneficiary under a signature.** **The message carries four fields and no others** (ADR-004, the SCP native relay protocol): the identifier the rent is credited to, the `units` the payment buys, the `PaymentReceipt` bytes where the message pays, and a signature over four operands.

<!-- scp:fragment id="rent-preimage-operands" -->
**The `RENT` signature preimage is `SHA-256("SCP-RENT-V1:" ‖ identifier ‖ BE32(units) ‖ receipt_digest ‖ operator_key)`, and each operand is stated here so two implementations compose one preimage.** **The preimage is 113 bytes**: the twelve separator bytes and 32 + 4 + 32 + 33 = 101 field bytes. `identifier` is the 32 raw digest bytes. `BE32(units)` is the count as a four-byte big-endian integer, at least 1 on a message carrying a receipt and 0 on a message carrying none. **`receipt_digest` is `SHA-256(payment_receipt)` where the receipt is present and the 32 bytes `SHA-256(0x00)` where it is absent**, under §9.5.1's rule for an absent fixed-length optional field. **`operator_key` is the 33-byte SEC1 compressed operator key the addressed relay declares in its own `POLICY` answer**, which the controller reads before it sends and which the relay already holds as the key it proves control of.
<!-- scp:end id="rent-preimage-operands" -->

**The signature is by the key the key state at the head of the accepted chain lists `Current` in the `#active` role**, which at a validating relay is the chain the routing id's first binding-valid frame established, so a routing id holding two chains resolves to one key rather than to whichever chain a reader picks. **Where the addressed relay is a community-relay-list entry the controller also holds that list's declared key for it, compares that key against the one the `POLICY` answer carries, and surfaces a mismatch as a release-skew signal rather than signing against either key silently**; where the addressed relay sits outside that list the `POLICY` answer is the whole source, and the residue an unsigned `POLICY` answer leaves is the one §9.10.12 states over that answer. **The operand binds one signature to one operator and never to one relay**: where one operator key covers two listed entries, a message a controller signed for the first entry verifies at the second, which is the residue the second open clause above names and which this paragraph closes nothing of. **A relay refuses a `RENT` that carries no signature, one that does not verify against that key, or one naming an identifier it holds no chain for.** **Each of those three refusals answers with a wire code and an error variant of its own, and none of the three answers with the price refusal's**, because no payment satisfies any of them. **A relay holding a rent record and no chain for an identifier refuses every `RENT` naming that identifier, the receipt-absent read included, on that third ground**, because the relay holds no accepted chain, therefore no key state, therefore no key the signature check resolves against; a relay that skipped the check would hand any party naming an identifier that identity's held bytes, quota and expiry, and would let any party credit quota to a displaced identifier. **The record itself stands through the displacement**, and the controller reads it once the PUBLISH that re-establishes its chain there has landed, which is the act R10's duty already obliges that controller to perform. **A relay writes a rent record only for an identity whose chain it holds**, which is what the signature check establishes, so no party writes durable per-identity state at a relay for an identifier the relay never stored, and it keeps a record it already wrote through a displacement.

**The preimage carries no freshness operand and the message carries no chain bytes, and the residue each absence leaves is stated here rather than repaired.** A message carrying a receipt is replayable exactly once per relay per operation, which the spent set above spends. A receipt-absent read message carries no receipt to spend, so it is replayable at the addressed operator's relays for as long as the signing key stands: a party that replays one reads that identity's held bytes, quota and expiry, and credits nothing.

**What the signature establishes: the controller of the identity named asked for this credit. It establishes nothing about which party settled the payment**, and the residue that leaves is stated here rather than repaired, in the shape the two open clauses above take. A party that watches a public rail for a settlement to a relay's declared payee composes a receipt over that settlement, names an identity it controls itself, signs with that identity's own `#active` key, and takes the coverage the settling party bought; that party's own message is then refused for a spent receipt identifier. The signature does not reach that party, because it authenticates the beneficiary and this attack names the attacker as the beneficiary. **Whether a third party may pay rent for an identity that is not its own is undecided, and this revision writes no rule that assumes either answer**, so what binds a receipt to the identity it credits stays unwritten here and at `19-economic-governance.md` §19.2.1, which discloses the same gap at `PaymentAdapter::verify`.

**The relay runs its checks on a `RENT` in this order**: the declared rate limit, which counts the message and reads nothing else; the structural decode of the message's fields; the signature check above, which reads the chain the relay already holds; then `PaymentAdapter::verify` with the three comparisons against its return. **Every check a relay can run before it decides whether to spend an outbound query runs ahead of that call**, which is the same universal the PUBLISH order of `03-identity.md` §3.10.2 carries. The relay then credits the identity's quota and rent expiry under the one arithmetic the coverage paragraph above states, **spends the rail-bound receipt identifier at that same crediting step**, and answers with that identity's rent state after the payment, so a controller learns what it now holds without a second round trip. **A `RENT` the relay refuses costs the payer no receipt.** **A `RENT` refused on one of the three receipt grounds answers `StorageBudgetExceeded{Payment, value}`, whose value carries that entry's rent terms**, so a controller refused for a price holds the figure that would satisfy it. **A `RENT` refused on one of the three grounds above — the absent signature, the signature that does not verify, and the identifier the relay holds no chain for — answers `RentRefused` under wire code `4042`, which names which of the three it was** (ADR-004, the SCP native relay protocol; `03-identity.md` §3.10.10). Routing those three to the price refusal would tell a controller whose signing key is stale that a payment would fix it, and it would send that controller to settle again on a rail for a message no payment reaches. **A `RENT` whose `payment_receipt` is absent credits nothing and answers with the identity's current rent state**, which is how a controller reads whether it is covered without paying for the answer.

**`RENT` stores no bytes, so it sits outside the ring, outside the four retained kinds, and outside the `Identifier` and `Total` scopes**: neither of those two refusals reads it and it displaces nothing. A `Payment` refusal does read it, on the three grounds the receipt checks above state, and a `RateLimit` refusal reads it like every other message. **The declared rate limit counts `RENT` like every other message**, because a relay sells relief from a storage refusal and sells no relief from the rate limit, so no payment buys a party a higher message rate.

**`03-identity.md` §3.10.2 orders every check a relay can run before it decides whether to spend an outbound query ahead of `PaymentAdapter::verify`, the one call that leaves the relay's host, and gives that reason: the call is an asynchronous query to a payment rail, a PUBLISH carries no key, and a relay that called the rail first would drive one outbound query per request an unauthenticated party sent.** **Every check after that call reads a value it returned or a decision it produced**, which is why the chain verification sits behind it; the declared storage budgets sit ahead of it, because every term they read is local. **A receipt and a current rent buy no exemption from `Total`: rented bytes count against the declared total budget**, and the `Total` paragraph above states when that term refuses, so the ceiling an operator declared bounds what that operator stores against a funded party. The blob-TTL clause of `19-economic-governance.md` §19.8 reaches no retained kind.

**Free listed relays** accept writes under their whole declared policy, `Total` included, and owe no party any particular write (`18-addressability-and-deployment.md` §18.5.1). The community relay list carries at least one free entry, and the curator keeps enough free capacity under distinct operators that a flood at one entry does not take first contact with the list (`19-economic-governance.md` §19.8), an obligation whose holder `18-addressability-and-deployment.md` §18.5.1 records as open. **A relay keeps one service record per identifier** at `scp:svc:`: it replaces the record it holds when a record signed by the key the adopted key state lists `Current` in the designated role arrives at a higher sequence, and it drops a record signed under a key that key state no longer lists `Current` in that role. **That count is keyed on the identifier and never on the pair of identifier and designated key bytes**, so a party designating a fresh key on every `KeyState` plants no second record; the reader-side high-water mark of `03-identity.md` §3.10.13 stays keyed on the pair, because that mark decides which of two records a reader accepts rather than how many records a relay stores. The record counts against the same budgets (`03-identity.md` §3.10.2, §3.10.13).

**The fetch bound.** **A verifier MUST stop fetching once the bytes it has read for one identifier, from one source, in one resolution, reach `MAX_RETAINED_BYTES`. It counts every byte it read, the accepted chain's and every divergent suffix's alike, because it is a download cap and the party downloading cannot tell which class a byte belongs to until it has read it.** **The bound exists because a party mints an identity of its own, fills its chain to whatever a relay's declared policy admits, and names that identity to a victim in an invitation or a standing-pair request**, so a resolver under no such bound fetches to `MAX_RETAINED_BYTES` from each of the two sources R11's floor requires and verifies one P-256 signature per event, for a payload that party published once. The bound is per source and per resolution: it neither spends one source's allowance on another, which would leave R11's two-source floor unmeetable for every long chain, nor carries a count forward from an earlier resolution, which would take the bound from a first contact to every established peer. It treats the unread remainder as absent rather than as a copy-level defect, because a chain the verifier did not read is not a chain it found a defect in, and the verdict is never `Invalid{at_event}`. **A verifier that stopped without assembling a complete chain from an inception event to a head returns `Inconclusive{MissingEvents{range, failed_sources}}`. A verifier that assembled a complete chain and stopped while reading beyond it has reached a verdict on the bytes it read and returns that verdict** — `Confirmed`, `Adopted`, `Contested` or `Discarded` under R6, R12 and R14 — because `Inconclusive` names the state where a verifier reached no verdict. **Both fields of the `MissingEvents` payload carry a value the verifier holds.** `range` names the sequences the verifier holds no event for: **its `from` is the first such sequence and is absent where the verifier holds an event at every sequence of the chain it assembled**, and its `to` is the last such sequence where the verifier knows that sequence and is absent where the stop left the chain's head sequence unknown (`03-identity.md` §3.10.10 declares the `SequenceRange` type and types both fields `Option<u64>`). **A verifier that assembled a complete chain from an inception event to a head and stopped while reading beyond it holds no sequence it lacks an event for**, and it returns its verdict rather than this payload. `failed_sources` is empty, because the verifier stopped itself and no source failed to serve; naming the relay it was reading would record an honest listed relay as having failed to serve, which is the signal §9.9.2's suppression reasoning reads. A verifier reads the chain in whatever order the page walk of `03-identity.md` §3.10.4 returns it. **An `Inconclusive` verdict blocks no recovery**, because R10 composes a reveal-authorized event from the log the controller itself retains and never from a resolution verdict. **The bound reaches every suffix a verifier fetches, a rank-1 suffix included**, whose exemption above is from retention and from eviction alone; a reveal sits at the first event of the suffix, so a verifier decides the rank before the bound binds. **The fetch bound and the byte bound above share one value and measure two different things**: the byte bound measures what a party retains and counts divergent-suffix bytes alone, and the fetch bound measures what a verifier downloads in one resolution from one source and counts every byte of both classes. A verifier runs both at once and neither reduces to the other. §9.18.17's registry row for `MAX_RETAINED_BYTES` states what each reader of that constant measures. **The residue this bound leaves, stated rather than repaired**: a verifier resolving an identity whose accepted chain approaches the cap has little allowance left for a divergent suffix and stops with `Inconclusive{MissingEvents}`, which §9.18.17 derives an honest chain never to approach and which is the fail-safe direction. **Whether the two bounds keep one value is open**, and this revision fixes no second figure: a verifier must read both classes of byte inside the fetch bound while a validating relay may hold divergent-suffix bytes up to the whole of the retention bound, so a verifier reading a relay whose retention is full of one party's suffix can stop before it finishes the accepted chain and return `Inconclusive{MissingEvents}` at every source that party wrote to. Raising the fetch bound above the retention bound by the margin an honest accepted chain occupies, and lowering the retention bound below the fetch bound by that same margin, are the two shapes reviewers reached, and §9.18.17's registry row records that the figure each bound takes is undecided. **Neither shape closes the question on its own.** The margin each names is what an honest accepted chain occupies, and no rule of this design bounds an accepted chain: a chain carries no ceiling on the keys it installs and none on its sequence, the retention bound counts no byte of the accepted chain, `MAX_ACCEPTED_CHAIN_BYTES` stays withdrawn, and the declared storage budgets are the operator's and optional. **The only figure that bounds an honest accepted chain is the fetch bound itself, so each shape defines its margin in terms of the quantity it is trying to set.** Closing the question therefore takes a third decision this revision does not make: a stated ceiling on an accepted chain, or a fetch bound that is not one scalar. The fetch bound is not a sequence ceiling, for the reason the segmentation paragraph above gives.

**The eviction rank** is defined for a divergent suffix and for nothing else. It is a scalar: the R6 rank the suffix carries against the accepted chain, read against the commitment standing at that suffix's own fork position. Eviction reads that one term and nothing else, and R6's pairwise comparison decides adoption while no eviction reads it. **The accepted chain is not one of the divergent suffixes a party retains, and it carries no eviction rank.** It is the chain the verifier held before it observed any divergence; at a first-contact verifier, which held none, it is the chain that verifier adopts under R11, and until that moment every chain it fetches is bounded by the fetch bound above, which is a MUST, and by nothing else; and at a validating relay it is the chain the routing id's first binding-valid frame established. **A verifier never evicts that chain. A validating relay never evicts it for a budget term; while the ring buffer above holds that identity covered no write displaces it, and while that identity is uncovered that ring displaces it with everything else the relay holds for that identity, and nothing else displaces it**, so one record has one fate under both rules. R9 counts every arrival against it. Without the accepted-chain clause the rank term was defined only by reference to a chain the suffix forked from, so the chain every fork hangs off had either no rank or eight, and one conforming reading evicted it first. A suffix is rank 1 in one pair and rank 2 in another, so "never evict a rank-1 suffix" had no referent and two relays evicted different chains from identical inputs.

**The eviction rule, over both bounds.** A candidate that would exceed the count bound or the byte bound is admitted wherever the verifier already retains a suffix it may evict whose rank number is at least the candidate's, and the verifier evicts the retained suffix carrying the highest rank number, breaking a tie among equal ranks by evicting the suffix it observed most recently. Evicting the earliest-observed equal-rank suffix would evict the owner's own chain first at every verifier that had been following the identity. A rank-1 suffix is never evicted. A candidate the party can evict no suffix for is refused — a rank-3 candidate against rank-2 suffixes alone, for one — and the party returns `Contested{…, residue: n}` with that candidate counted in n, which is the one signal a refusal produces; a validating relay in that state rejects the frame. An eviction returns no signal to the evicted suffix's publisher, and a suffix a party evicted and a party presents again later is re-ranked as if the party saw it for the first time.

**The rank-1 flood.** Where the rank-1 suffixes a party retains fill either bound and another rank-1 candidate arrives, the verifier keeps the suffixes it holds and returns `Contested{…, residue: n}`, where n counts the rank-1 candidates it could not retain. A validating relay in that state rejects the further frame. A resolver MAY stop fetching once it holds two rank-1 suffixes.
...
**Self-observation.** **The SDK MUST fetch its own identity's log from every entry of its fallback set** at a stated cadence, no less often than the witnessing interval the key state names and on every launch, and MUST treat an entry it could not reach as an alert. **The duty reads the head and never the record's presence**, and it decides all four relations an entry's head can hold to the controller's own retained log: absent, superseded, extending and diverged. **Which head the SDK reads at an entry serving more than one chain: the head of the chain the controller's own retained log is a prefix of**, because R9's serving order interleaves every chain a routing id holds and R12 makes two diverged chains incomparable by sequence, so "the head" names nothing at an entry holding a divergent suffix until this clause names it.

**The four arms, each naming an act.**

- **Absent** — the fetch returned no record for this identity. The SDK re-publishes its chain from the inception event and re-publishes its service record.
- **Superseded** — the entry's head sits on the chain the controller authored and the controller's own log holds events past it. The SDK re-publishes from the first event that entry does not hold, walking predecessor digests along that chain rather than comparing sequence numbers, and re-publishes its service record where that entry holds none or holds one the controller's own copy supersedes.
- **Extending** — the entry's head sits on the chain the controller authored, every event on it verifies under the controller's standing root, and the controller's own log does not hold it, which is the controller's own second device having published past this device's backup. The SDK adopts that chain into its own log before it composes its next event, and publishes nothing.
- **Diverged** — the entry's head sits on a chain the controller's log does not contain. The SDK raises §9.11's device-compromise signal at maximum severity, surfaces that entry to the controller, and re-publishes its own head there, so the entry holds both chains and R6 can rank them at every resolver reading it.

**The publication duty is the absent, the superseded and the diverged arm, and the extending arm publishes nothing.** The superseded arm fires because an entry that missed one publish cycle holds a record and holds a superseded one: it serves a key state whose `#active` key the controller has replaced, every resolver reading it adopts that key state, two such entries carrying distinct operator keys meet R11's first-contact floor, and the identity's own cadence reports success at every one of them. **A re-publication under the superseded arm is not a re-PUBLISH of bytes the relay already holds**, because the events that superseded that head are events the entry does not hold. The publisher learns the first event an entry lacks from the same fetch this cadence already performs. **The fetch ranges over the set the duty acts on, because a fetch from one entry establishes nothing about the others**: a fetch from one chosen entry left the duty unfired wherever that entry still held the record, so an identity a flood had displaced everywhere else stayed absent at every other entry, R11's two-source first-contact floor was met at none of them, and the SDK's own check reported success. The cost is the per-entry fetch cost §9.18.17 derives, multiplied by the entries in the set, and it carries no publish at an entry that holds the controller's own current head. **There is no unconditional re-PUBLISH on every cadence**, because a re-PUBLISH of bytes the relay already holds leaves the relay's stored state unchanged and moves the identity nowhere in the ring, for the reason R9's three write outcomes and R9's ring buffer state, so it costs the relay an ingest slot and the publisher a rate-limit allowance and buys the identity nothing. **That reason bounds an entry holding the controller's own current head and reaches no entry holding a superseded one.** The fetch establishes what the controller is standing on, and the conditional re-publication is what restores a record the ring displaced and what carries a stale entry forward to the current head. **Where satisfying that publication duty at an entry would cost money, the SDK surfaces the condition to the controller and mints no receipt**: an entry's claim that it holds no record is a claim the resolver did not prove, so an SDK that minted on it would let any charging entry bill a controller once a cadence by answering every self-check with an empty response, which `19-economic-governance.md` §19.14's invariant that agents never silently incur costs forbids. That cadence is also the submission cadence of §9.7.4.3.

**Before composing a snapshot** the controller MUST hold its chain from the inception event, MUST diff the chain's witness set and service-key designation against a pre-compromise record, and MUST diff its service record's relay list against a pre-compromise copy. It holds that chain in the log it durably retained, so a fetch bound reached on a read, a relay's refusal, or an `Inconclusive` verdict that any party's read of this identity returns changes nothing about what it can compose. On `Contested` it refuses only where it cannot identify which suffix it authored, because a blanket refusal would lock the controller out of the first row above, whose premise is a root-signed event another party wrote onto the chain.
...
A controller that cannot establish the boundary MUST NOT proceed, and on the non-exportable arm it MUST NOT sign until it has regained exclusive control of the platform account holding the root and pre-rotation credentials. Exclusive control is a controller obligation and no verifier checks it, exactly as `standing_root: Lost` is. Where the fallback-set refusal below persists instead, the SDK MUST surface the unreachable sources to the controller rather than retry silently, because a party holding the controller's network path can hold that refusal open for as long as it holds the path. Before the reveal signs, it MUST confirm the commitment list, the next threshold, the K′ set and root threshold, and whether the event declares abandonment, on a device with a display that is not the assembling device, or by comparing a displayed digest against one the pre-rotation substrate computes. **Where that confirmation runs on a second device, that device MUST read the store set the one-reveal-at-a-time rule below names and MUST refuse to confirm while it holds an unpublished reveal-authorized event or a pending `RecoveryHandle` for the same identity**, which is that rule read from the confirming side. That confirmation binds the bytes the signing substrate consumes to the event the human read, and establishes nothing about where the committed successor keys came from.

**Ceremony order.** Read the standing commitment and the current key list from the adopted key state, **from the log the controller durably retains itself and never from a resolution verdict**; generate the next set under §9.7.4.1 items 1, 3a, 4, and 5b through 5d; for a `RootRecovery` generate K′ and the operational keys under the device boundary; for an abandoning rollover issue the retirement Update in every context; persist the composed preimage, the next commitment list, the next threshold and the signed event into the SDK's own store keyed on the composed event's preimage digest, then persist the `RecoveryHandle` carrying the identifier, the phase `Composed` and that digest into every store the one-reveal-at-a-time rule below names for the identity's pre-rotation credential; sign, and update that handle's `RecoveryPhase` to `Signed` in every one of those stores; publish, and update it to `PublishSent`; on the publish cycle's confirmation update it to `Confirmed`; on the reveal's submission update it to `Submitted`; retain; destroy. **The ordered list names the write of every one of the five phases `RecoveryPhase` carries**, because an SDK that wrote only the first restarts every interrupted ceremony at the compose step. The persist precedes the signature, because a controller that crashed between signing and persisting would resume by composing a fresh next set, and its second event would reveal the standing commitment a second time.

The controller MUST durably retain the signed event byte for byte before it destroys the spent keys. **That retained log is what the ceremony above composes from**: the controller holds its chain from the inception event because it retained every event it signed. Confirmed publication is durable self-retention together with acceptance by at least one relay in the fallback set, the ceremony completes at confirmed publication plus destruction, and witness submission gates neither.

The SDK MUST refuse, with `IdentityError::NoFallbackSourceReachable`, to compose a reveal-authorized event where it can reach no source in the fallback set, because a controller that cannot read its own chain from a source an attacker did not choose cannot establish what it is recovering from.

**One reveal at a time.** An SDK MUST NOT compose a reveal-authorized event for an identity, and refuses with `IdentityError::RevealAlreadyPending`, while it can observe, for that identity, either an unpublished reveal-authorized event or a pending `RecoveryHandle`. **The SDK MUST write the pending `RecoveryHandle` into every store the identity's pre-rotation credential syncs through before it signs, and MUST update that handle's `RecoveryPhase` in every one of those stores after it signs; and it MUST read every one of those stores before it composes.** **Where the SDK reaches some members of that set and not every member, it MUST refuse to compose with `IdentityError::StorePartiallyUnreadable` and MUST NOT compose against a partial view**, because a partial read is indistinguishable to it from an empty one. **A member that answers with bytes the SDK cannot decode as a `RecoveryHandle` is a member it treats as unreadable**, and it takes the same refusal, because an SDK that read such a value as no pending handle would compose a second reveal at one predecessor. **`RecoveryHandle` is the one name for the one object every store of that set holds**, and the rules, the accessor and the three entry points below name it and name no second object. **The store set is keyed on the credential and never on the running profile**: it is every store the identity's pre-rotation credential syncs through, and an SDK on any profile writes to and reads from every member of that set it can reach. Two stores are members. The platform account's synced credential store is a member wherever that account syncs the credential. **WebAuthn's `largeBlob` store of the pre-rotation credential is a member wherever the authenticator supports the extension** — the pre-rotation credential and not the root, because a reveal is what this rule gates and §9.7.4.1 item 3a already separates the reveal-time authorizer from daily operations, so a compose-time read exercises the authorizer the compose is about. The platform syncs that store with the credential, and every origin whose effective domain is the credential's relying-party identifier, or a registrable-domain suffix of it, reaches it. **The page's origin storage is never a member**, because the browser partitions it per origin and `17-persistence-and-storage.md` §17.8 calls it ephemeral in a private window. Where a credential syncs through neither store — a `Headless` profile, or a `Mobile` or `Desktop` profile under no platform account with an authenticator supporting no `largeBlob` — the set is the SDK's own local store and spans one device. **`RecoveryHandle` is fixed-width at `RECOVERY_HANDLE_BYTES` (§9.18.17) and carries the composed preimage and the signed event never**: it carries the identifier, the `RecoveryPhase`, and the preimage digest of the composed event, which answers the one observable condition this rule states. **The composed preimage, the next commitment list, the next threshold and the signed event sit in the SDK's own store, keyed on that digest.** CTAP 2.1 floors an authenticator's whole large-blob array at 1,024 bytes across every credential it holds, and `MAX_EVENT_BYTES` is 65,536, so a duty to write the event itself would ask for a write between one and two orders of magnitude larger than the store. **A `largeBlob` write that does not land has an arm: the SDK MUST NOT sign, and MUST surface the condition.** A `largeBlob` write reports failure as `largeBlob: { written: false }` with no error, and an authenticator at capacity, an authenticator that advertises the extension and refuses the write, and a declined verification gesture each produce that report silently; any origin the controller visits can register a credential and consume the array, so the condition is routine and not an attack signature. **Origin storage is the wrong home on `Browser` for two reasons a page cannot work around**: the browser partitions it per origin, so two applications under one relying-party identifier read two stores and each composes a reveal believing none is pending, and §17.8 calls it ephemeral in a private window. **Where the authenticator supports no `largeBlob`, the platform gives the SDK no durable store spanning the origins that credential serves, the profile is per-device for this rule**, and §9.7.4.1's failure-mode table says so rather than leaving a reader to conclude the rule closed the race there. **The rule therefore observes across a controller's SDKs exactly where one store spans every SDK the controller runs, and observes one SDK alone everywhere else**; §9.7.4.1's failure-mode table states that condition over SDK sets rather than as a list of profiles closed and open independently. Without the rule the shipped consumer default destroys an identity with no attacker and no leaked key: the root credential and the pre-rotation credential share one platform account and the platform syncs both to every device of that account, so a controller told to roll over on suspicion signs a `CommitmentRollover` on a phone and another on a laptop, each revealing the same standing commitment under the same standing root at the same predecessor. Neither may be discarded, R6 ranks both at 1, and R7 makes a rank-1 tie terminal.

**The recovery handle.** <!-- scp:fragment id="recovery-handle-declaration" -->**`RecoveryHandle` is one enum whose variants are `FixesCommitment` and `Abandons`, each carrying the identity's 32-byte identifier, a `RecoveryPhase`, and the 32-byte preimage digest of the composed event, and `RecoveryPhase`'s five values are `Composed`, `Signed`, `PublishSent`, `Confirmed` and `Submitted`.**<!-- scp:end id="recovery-handle-declaration" --> **It is fixed-width at `RECOVERY_HANDLE_BYTES`** (§9.18.17, which states the breakdown), and that width is what every store of the credential-keyed set holds. **The composed preimage, the next commitment list, the next threshold and the signed event sit in the SDK's own store, keyed on that digest, and no variant of `RecoveryHandle` carries any of them.** The continuation is the variant tag so that `Abandons` has no next-set field by construction, which is why the discriminant byte is load-bearing and counts toward the width. Both type names and all seven variant names are name-bound under the criterion the definitions state. Exactly one accessor, `load_pending_recovery`, loads a pending handle, and exactly one entry point, `resume_recovery`, consumes it by value.

**This section declares the ceremony's two types, the type its three entry points sit on, and those three entry points, every field and every parameter carrying a type; `03-identity.md` §3.10.10's `IdentityBackend` carries none of the three.** <!-- scp:fragment id="recovery-entry-points" -->**The three recovery entry points sit on a type named `Recovery`: `load_pending_recovery`, `resume_recovery` and `abort_recovery`**, so a binding author reads where they live rather than placing them as free functions on one platform and as statics on another.<!-- scp:end id="recovery-entry-points" --> **`resume_recovery` reads the composed preimage, the next commitment list, the next threshold and the signed event out of the SDK's own store, keyed on the handle's `event_digest`**, and no synced store carries them: <!-- scp:include id="large-blob-arithmetic" from=".docs/specs/17-persistence-and-storage.md" -->CTAP 2.1 floors an authenticator's whole large-blob array at 1,024 bytes across every credential it holds, and `MAX_EVENT_BYTES` is 65,536, so a duty to write the event itself would ask for a write between one and two orders of magnitude larger than the store.<!-- scp:end id="large-blob-arithmetic" --> **An SDK whose own store holds no preimage under that digest MUST NOT resume and MUST surface that condition**, which is the state a controller reaches on a second device after it loses the device that composed the ceremony; `abort_recovery` reaches that handle like any other and clears it, so losing the composing device costs the controller the ceremony and never the identity.

```rust
pub enum RecoveryPhase { Composed, Signed, PublishSent, Confirmed, Submitted }

/// `RECOVERY_HANDLE_BYTES` in either variant (§9.18.17): one discriminant
/// byte, the identifier, the phase byte, and the composed event's preimage
/// digest.
...
/// The two synced stores the credential-keyed set above names, and the SDK's
/// own local store, which is that set wherever the credential syncs through
/// neither and which holds the ceremony's four other values under
/// `RecoveryHandle::event_digest` on every profile.
pub enum CredentialStore { PlatformAccount, LargeBlob, SdkLocal }

pub struct StoreRemoval { pub store: CredentialStore, pub removed: bool }

pub struct RemovalOutcome { pub per_store: Vec<StoreRemoval> }

/// What a resumed ceremony returns: the outcome of the publish cycle it ran
/// and the outcome of the per-store removal that followed it.
pub struct RecoveryCompletion {
    pub publish: PublishOutcome,
    pub removal: RemovalOutcome,
}

/// The three entry points of the ceremony. The type takes the capabilities the
/// ceremony reads through its initializer, so a caller reads which stores a
/// call clears from the value it constructed.
pub struct Recovery<S> { /* persistence slot, custody slot, identity backend */ }

impl<S: EncryptedStorage> Recovery<S> {
    /// Each slot carries the type `IdentityConfig` carries for it
    /// (`03-identity.md` §3.10.10), so a caller holding the config that created
    /// the identity constructs this value from what it already holds.
    pub fn new(
        persistence: Option<StorageSlot<S>>,
        custody: KeyCustodySlot,
        backend: IdentityBackendSlot,
    ) -> Self;

    pub fn load_pending_recovery(&self, identifier: &[u8; 32])
        -> Result<Option<RecoveryHandle>, IdentityError>;

    /// Asynchronous, because it drives a publish cycle.
    pub fn resume_recovery(&self, handle: RecoveryHandle)
        -> impl Future<Output = Result<RecoveryCompletion, IdentityError>> + Send;

    pub fn abort_recovery(&self, identifier: &[u8; 32])
        -> Result<RemovalOutcome, IdentityError>;
}
```

**`Recovery` takes the persistence slot, the custody slot and the identity backend through its initializer, each carrying the type `IdentityConfig` carries for that slot, and each of the three entry points takes `&self`**, which is the shape `03-identity.md` §3.10.10's `IdentityBackend` already uses for the same capabilities: a free function holding no capability cannot say which store set it clears, and `.docs/standards/construction.md` injects every dependency through an initializer. **No parameter takes a boxed trait object**, because that standard states that `KeyCustody`, `Storage` and `IdentityBackend` return position-`impl Trait` and are therefore not object-safe, so `Arc<dyn IdentityBackend>` does not compile and a caller holding the config that created the identity holds none of the three in that shape. **`resume_recovery` is declared asynchronous in the shape `03-identity.md` §3.10.10 uses**, returning `impl Future<Output = Result<RecoveryCompletion, IdentityError>> + Send`, because it drives a publish cycle, and the scaffold's per-language rows carry an asynchronous suffix on exactly the entry points that return a future. **`abort_recovery` returns a `RemovalOutcome` carrying one `StoreRemoval` per store the SDK reached, and `resume_recovery` returns a `RecoveryCompletion` carrying that outcome beside the `PublishOutcome` of the cycle it ran**, so the three publication duties `03-identity.md` §3.10.6 states read one per-entry list wherever a publish cycle ran and a caller reads which stores the removal landed in. An `Err` records that the SDK could not run the removal at all, which is the same split `PublishOutcome` draws between a per-entry refusal and an error. Without the per-store value one binding returned `()` and surfaced a partial removal by logging it, and another returned `Err` on a removal that had reached two stores of three. **`load_pending_recovery` reads every store the credential-keyed set above names and returns a handle any SDK of that set wrote, at every phase from `Composed` through `Confirmed`.** **`Submitted` records a submission the SDK surfaces and holds no handle**: the accessor returns nothing at that phase, and its range ends at `Confirmed`. **`abort_recovery` removes the handle from every store it was written to. `resume_recovery` removes it the same way once the ceremony completes, and the ceremony completes at `Confirmed` plus destruction** — confirmed publication together with destruction of the spent keys, the completion this rule defines above, which witness submission gates neither. The removal duty names that phase and never the verb "completes", because a reader taking completion at `RecoveryPhase`'s last value leaves a handle in every synced store of a controller whose designated witnesses go silent. **A removal that does not land has an arm**: the handle stays in the store it was written to, the `StoreRemoval` for that store carries `removed: false`, and the SDK MUST surface that. `abort_recovery` reaches a handle at every phase from `Composed` through `Confirmed`, so the controller has one entry point that clears it.

**Abort.** `load_pending_recovery` returns nothing for that identity after an abort that landed in every store, because a handle an abort left in place would refuse every later reveal with `RevealAlreadyPending` and the controller could never roll over again. Abort at `Composed` leaves the standing commitment unconsumed, which is what §9.7.4.1 item 3a's fail-closed refusal rests on. Abort after the signature clears the handle and does not unsign the event: a software zeroize of a signed event is not verifiable, and a surfaced copy beside a recomposed one is two reveals of one commitment and a permanent self-contest, so the remedy for a wrong device-boundary establishment discovered after signing is to publish the signed event and sign a further reveal-authorized event under the commitment it fixed.

**R11 — classifying a candidate head.** A candidate head is on the accepted chain where either chain is a prefix of the other, divergent where the verifier holds both chains back to a common ancestor and they diverge, and inconclusive otherwise. On inconclusive the verifier holds its baseline, records nothing, and returns `Inconclusive{MissingEvents{range, failed_sources}}`, having tried at least one relay in the fallback set. **The chain a first-contact verifier adopts under the floor below becomes that verifier's accepted chain from that moment**, and R9's eviction rank reads it from there; until then the verifier holds no accepted chain, so it bounds the bytes it downloads from one source in one resolution by R9's fetch bound.

**The first-contact floor.** A verifier holding no accepted baseline MUST obtain the chain from at least two entries of the community relay list carrying **distinct operator keys**, each serving a relay proof of control the verifier checked. The floor counts operator keys and never declared operator names: two entries whose `key` members hold equal bytes count once however many operator identities they declare, which is the one-operator-key-is-one-witness rule the definitions state, and the entry's own bytes carry the key, so the check is a local byte comparison. A relay outside the list counts toward neither number. A single relay can always serve a genuine prefix truncated before the event the reader most needs, which forks nothing, verifies clean, and faces a verifier holding no baseline.

Every degree below the floor is fail-closed: the verdict is `Inconclusive{SingleSource}` and the verifier adopts no head. **The verifier counts distinct 33-byte operator keys in the `proven` member of `ResolutionSources` (`03-identity.md` §3.10.10) and counts declared operator identifiers nowhere.** The degrees are reaching no second relay, reaching two relays whose entries carry equal `key` bytes, reaching a second proven relay that served no chain, holding a fallback set with fewer than two entries under distinct operator keys, and rejecting every proof on the resolver's own clock. The clock-skew degree is counted separately so a party tells its own clock from a network failure. A fresher head does not relax the floor. **Every QUERY a verifier sends in a first-contact page walk carries a fresh `proof_nonce` and reads the relay proof the response returns, and not the first QUERY alone**, and the verifier counts an operator proven only while every page that operator served carried a proof it verified: a walk whose later pages carry no nonce proves control of the relay that served page one and proves nothing about the relay that served the page the verifier adopted from, which is where a truncation sits.
...
The routing derivations are `routing_id = SHA-256("scp:did:" ‖ identifier_bytes)` over the identifier's 32 raw digest bytes, with `SHA-256("scp:wit:" ‖ identifier_bytes)` for cosigned heads, `SHA-256("scp:wcf:" ‖ identifier_bytes)` for conflict statements, and `SHA-256("scp:svc:" ‖ identifier_bytes)` for the service record. The address is the type discriminant and is fixed now, because each derivation consumes the digest and never a textual encoding.

**What waits.** Display, URLs, and every signed structure that takes an identifier as UTF-8 bytes, which §9.5.2 enumerates. Because that form is unfixed, a writer prints no identifier in a prose example: an illustrative body writes `<scp-identifier:name>`, and a field whose encoding is this deferral writes `<the … identifier, in the textual form 09 §9.7.4.2 R13 defers>`. `25-test-vectors.md` §25.1 states the reason its fixture strings print an identifier.

**R14 — the verdicts.** A resolution returns exactly one of six verdicts of the type `ResolutionVerdict`: `Confirmed`, where the candidate head equals the accepted head at an equal sequence; `Adopted`, where the verifier replaced its baseline; `Contested`, where R6 resolved two suffixes as a tie and R7 classified it; `Inconclusive`, where the verifier reached no verdict on the bytes it read; `Invalid`, where an event failed R1 through R4; and `Discarded`, where the candidate head sat at a strictly lower sequence on the accepted chain. **`03-identity.md` §3.10.10 declares `ResolutionVerdict`, `InconclusiveCause`, `TieClass`, `SequenceRange` and `BudgetValue`, each field carrying a type, and this rule declares no type of its own.** R9's fetch bound states what each field of `MissingEvents` carries on a verifier that stopped itself. A discard is a verdict so that a consumer routes it, because the resolution delivered nothing newer than the baseline.

`Invalid{at_event}` is the only cause of a resolution verdict named `Invalid`, and a copy-level defect never produces it. Content verification returns a second result type, `ContentVerdict`, which §9.7.1 defines.

The contested verdict, the retained suffixes it rests on, and the accepted-head baseline MUST survive the verifier's restart, and a verifier MUST persist them before it acts on any verdict. A persist failure returns `IdentityError::PersistFailed` and not the verdict. A resolver that published an adopted head, acted on it, and then died would restore a stale head and re-open the rollback the first-contact floor closes.

#### Worked examples

Each example names the one table row it traces and states the order of the parties' acts. Four trace a row of §9.7.4.1's failure-mode table, and the fifth traces a row of R10's situation table.

1. **A root thief forks early against a controller whose root is gone**, tracing the failure-mode row "Root compromised, P copied". The thief forks below the controller's last event and signs a `RootRecovery{CoSigns}` there under the root it stole; the controller, holding no root, signs `RootRecovery{Lost}` on its own suffix.
2. **A copied pre-rotation key against an intact trusted root**, tracing the row "P copied, root intact and trusted". The copy holder reveals the standing commitment on a fork below the event that spent it; the controller's own revealing event carried the standing root's signature.
3. **Two root-only suffixes**, tracing the row "Two of the controller's devices both hold the root". Two of the controller's devices each append a root-signed `KeyState` at one sequence, and neither suffix reveals the standing commitment.
4. **A backup of a spent key reaches the root thief**, tracing the row "P copied, root lost". The thief holds the root and a copy of a key a past reveal spent, and reveals that same commitment on a fork below the event that spent it.
...
The earlier model, under which threshold cosignatures conditioned an event's acceptability, was the orchestrator's design call and not Alec's. He asked on 2026-09-08, "So root, witness required, witness decides?", the orchestrator answered "root decides, witnesses required, witnesses never decide", and he did not confirm the "required" half in his own words.

**The cosigned head**, fields in preimage order: `witness` (32 bytes), `subject` (32), `sequence` (8, big-endian), `event_digest` (32), `previous_cosigned_digest` (32, never zero), `seed` (1 byte, `0x01` where this head is the first the witness cosigned for this subject after a designation or a store loss and `0x00` on every later head), `observed_at` (8, big-endian). Its signature preimage is

```
SHA-256("SCP-COSIGNED-HEAD-V1:" ‖ witness ‖ subject ‖ sequence ‖ event_digest ‖ previous_cosigned_digest ‖ seed ‖ observed_at)
```

signed by the P-256 key the witness operator's community-relay-list entry declares, giving 145 preimage bytes and one 64-byte signature, 209 bytes in all. It names no position in the witness's own key state, because a non-transferable operator key has no position to name. **A witness sets `seed` on the first head it cosigns for a subject after a designation or after a store loss, and clears it on every later head.** A witness that sets the byte declares a re-seed to every reader of that head, and a witness declaring many seeds for one subject is re-seeding where any party holding those heads can see it.
...
**What a relay's ring buffer leaves a witness to do.** A witness whose cosigned head a relay displaced re-cosigns the subject's head on the next offer that subject's controller makes it, which is an ordinary write against that subject at that relay and carries no new duty; §9.7.4.2 R9 states the duties a displacement leaves each party. <!-- scp:fragment id="witness-write-charge-exemption" -->**A relay charges no `scp:wit:` and no `scp:wcf:` write, and no party pays for those two kinds.** A relay that declares a per-write fee accepts a PUBLISH at either witness address without a receipt and refuses it for no payment term, because the witness filter already proves the write is one a designated witness made about a subject whose chain that relay holds. **The bytes still count against `Total` and against no identity's coverage.**<!-- scp:end id="witness-write-charge-exemption" --> The filter accepts such a write only where the object's signature verifies against the P-256 key that operator's community-relay-list entry declares, where the `witness` field names a member of the witness set the key state at that subject's head carries, and where the address is not already full for that pair: it accepts such a write only where the object's signature verifies against the P-256 key that operator's community-relay-list entry declares, where the `witness` field names a member of the witness set the key state at that subject's head carries, and where the address is not already full for that pair. **No party pays for those two kinds, and this design names no payer for them.** A witness holds no payment adapter and mints no receipt, and no message carries a receipt from one party to a frame another party composes, so a rule naming any payer would name an act no party can perform. **An equivocating controller cannot suppress the conflict statement that accuses it**, which is what a rule making that controller the payer would have let it do by declining to pay. **The bytes still count against `Total` and against no identity's coverage** (§9.7.4.2 R9), so the operator's declared capacity ceiling still bounds what a witness may write and the per-subject caps below bound the fan-out. **A conflict statement is held by the witness that wrote it and by the controller that fetched it**, so neither party depends on a relay retaining one.
...
Two preconditions sit under the check and neither is a second judgement: the witness verifies the chain exactly as any relying party does, and it refuses a subject whose key state at the head does not name it. Its `observed_at` MUST exceed that of its last cosigned head and MUST sit no further ahead of its own clock than the skew tolerance. A witness whose stored `observed_at` exceeds its own clock past that tolerance keeps the record, refuses, reports the condition, and does not re-seed, because a party that moved that host's clock forward and back could otherwise make a chosen witness discard the one value its check reads. **A witness whose durable store fails to read the record for a subject takes the same four verbs:** it treats that subject as one it holds no baseline for, refuses to cosign, reports the condition, and does not re-seed. It cosigns that subject again only once the record reads, or once it re-seeds under case 1 from the event that designates it; a read error never re-seeds a witness by itself. **A store that reads and holds no record for a subject is the witness's newly-designated case and takes case 1**, because a witness that never served that subject and a witness whose record for it was dropped hold byte-identical state and no test separates them; the refusal above reaches a store that fails to read and reaches no absent record. A conforming witness that read a storage fault as case 2's lost store would re-seed at the designating event and cosign a fork, and the two heads it had then released are a valid fault proof against an operator that never equivocated.

**Durable per-subject state.** A witness retains, per subject it has ever cosigned, the subject identifier, the sequence and event digest of its last cosigned head, that cosigned head's preimage digest, its `observed_at`, and **one bit saying whether the record is a seed floor the next head declares or a released head the next head follows**. The bit is what tells the witness whether to set `seed` on the head it releases next. Each record is about 113 bytes and a witness holds at most `MAX_SUBJECTS_PER_WITNESS`. The witness persists the record durably before it releases the cosignature that record accounts for, because a witness cosigning on a cold baseline could sign both sides of a fork with no malice.
...
**A witness that lost its store takes case 1.** It holds no record naming the subjects it served and no address at which to look for its own published heads, because those heads sit at each subject's `scp:wit:` address and that address derives from the subject identifier, so it can enumerate nothing and no rule may tell it to. It therefore treats every subject as newly designated. **It seeds on a chain offered to it that it has verified from the inception event exactly as any relying party does and whose key state names this witness in its witness set, and it seeds at the latest event on that chain that designates it, which is case 1. The rule names no offering party, because a witness can identify none**: a PUBLISH carries no key (ADR-004, the SCP native relay protocol), a relay authenticates no client, and the honest controller and a party holding a forked suffix that names this witness offer objects between which the witness holds no test. **The fault-proof predicate's exclusion of the seed marker is what keeps a re-seed from minting a proof.** What that leaves open is evidence denial and not theft: a party holding a forked suffix that reaches a restarted witness first gets it to seed on that suffix and cosign it, and the witness then refuses the honest controller and emits a conflict statement naming the honest chain as the offered one. No fault proof arises, and the controller's exit is R10's row for witnesses unreachable, silent, or refusing. **A store that is gone sends the witness here; a store that is present and fails to read sends it to the refusal above.**

A recovery reaches a fresh witness set: the recovery event is the latest event designating them, it sits on the recovered suffix, and the outgoing witnesses' heads sit on a suffix the recovery does not carry, so they enter no fresh witness's floor. A party holding a spent pre-rotation key takes the same path.
...
A conflict statement is portable evidence that two chains for one subject were offered to one named witness: the witness makes no claim about which chain the controller authored, and a party holding it applies R6 and R7 to the chains themselves. KERI splits the same three roles under `spec-body` §Indirect exchange via witnesses and watchers, where a Juror records evidence, a Judge evaluates it, and the validator decides.

**The fault proof** is two cosigned heads carrying one `witness` value, one `subject` value, **either one shared `previous_cosigned_digest` value or one shared `sequence` value**, two different `event_digest` values, and **neither head setting `seed`**. Those five conditions prove the witness released two successors of one baseline, and **the five conditions are the test**. Every condition reads the two heads' own fields, so any party holding the pair decides the predicate without the subject's chain and without a prior head of the witness. **Condition three is a disjunction because `previous_cosigned_digest` is a value the accused witness alone chooses and no rule constrains beyond "never zero"**: a witness writing one fresh random value into each of two heads failed a shared-digest test and released two successors of one baseline anyway. Two heads at one sequence for one subject carrying different `event_digest` values are two successors of one baseline, because the one check above admits one event at one sequence. The false positive the disjunction admits is the backup restore the closing residue names. A witness that seeds twice at one designating event releases two heads at one sequence with `seed` set on both, so condition five excludes that pair under the disjunction exactly as it does without it. **A witness that sets `seed` on a head makes no proof out of that head**, which is what separates a re-seed from an equivocation: a witness that re-seeded declared it in the byte, and a witness that released two successors of one baseline declared nothing. Two heads for two subjects share no baseline, and the never-zero field keeps a witness's first head after a seed from colliding with a zero placeholder. Two listed relays under one operator are not a second path to an equivocation, because one operator key is one witness for this predicate (§9.7.4.2 definitions). A relay stores heads under the key `03-identity.md` §3.10.2 states, so a third party fetches both heads from one address. The proof is portable and self-contained against the shipped list, because verifying either signature takes the P-256 key the witness's community-relay-list entry declares. It decides the operator's conduct and never the fork.

**Worked example 6 — the predicate applied, and it states no sixth condition.** A witness seeds on a subject and publishes a first head carrying `seed: 0x01`, then a second head carrying `seed: 0x00` and the first head's preimage digest. It then releases two heads at one sequence for that subject, each carrying a different `event_digest`, both carrying the second head's digest as `previous_cosigned_digest`, and both carrying `seed: 0x00`. A third party holding the two heads reads one `witness` value, one `subject` value, one shared `previous_cosigned_digest` value, two different `event_digest` values, and `seed` clear on both, so all five conditions hold and the pair attributes the equivocation to that operator. Two heads at one sequence carrying two fresh `previous_cosigned_digest` values satisfy condition three by its second term and attribute the same equivocation. **The same party holding the first head beside any later head reads `seed: 0x01` on one of them, so that pair makes no proof**, which is how a declared re-seed stays outside the predicate. A witness inventing a `previous_cosigned_digest` no head of its own ever carried is attributable under the same five conditions, because no condition reads what the witness published before.
...
**What a relying party reads a cosignature for.** Two cosigned heads for one subject where neither extends the other, or a conflict statement, tell a party that a fork exists, and R6 and R7 on the chains themselves are what that party applies. A cosigned head also tells its holder how recently a recognized operator served this chain, which the SDK records and surfaces and no rule reads. KERI's `spec-body` §Indirect exchange via witnesses and watchers has a validator trust absent evidence of duplicity and withhold trust in its presence; the freshness reading is SCP's and that citation does not reach it.

A cosignature from an operator the running SDK does not list is not an error, so a release boundary changes what evidence a party holds. A cosignature proves control of its declared operator identity by construction, because producing one requires the private half of the key that operator's list entry declares.
...
The SDK's default witness set for a new identity is the entries of the community relay list under **distinct operator keys**, capped at `MAX_WITNESS_SET_SIZE`, at `DEFAULT_WITNESSING_INTERVAL`. It counts keys and never declared operator names, which is the one-operator-key-is-one-witness rule the definitions state: a set built over declared names would install one operator key twice under two identifiers, which R3's pairwise-distinctness clause admits, and a controller submitting to either of that operator's two relays would then get two first heads at one baseline, reading as a fault proof against an honest operator. Identity creation completes at publication and waits on no witness.

The SDK MUST warn a controller that replaces the default set with unlisted operators, that designates no witness, or **whose designated set answers capacity to every submission**, and MUST state the consequence in these terms: the identity's freshness then rests on that peer's own relay reads. The warning MUST NOT say that such an identity is unresolvable, unadmittable, or held at `Unresolved`. What the controller gives up is portability.

**What this layer does not cover.** The one check gives no evidence at or above a witness's last cosigned head, so two events at one sequence each extending that head record a race rather than a wrong. The witness's durable store is a correctness dependency on infrastructure the protocol does not control. A witness that knows it lost its store seeds under case 1 and sets `seed` on its next head, so its re-seed makes no proof. A witness restored from an older backup it believes current knows of no loss, sets no `seed`, and cosigns from the sequence that backup recorded, which releases a second successor of one baseline and meets all five conditions, so that restore indicts an operator that never equivocated. **A witness operator escapes attribution by setting `seed` on every head it ever releases**: no pair of its heads then satisfies the condition that neither head sets the byte, so no fault proof exists against it however many baselines it equivocates on. No rule refuses a head carrying the byte, no rule caps the seeds one witness declares, and no verdict reads the count, exactly as no party checks a declared `standing_root: Lost` under R3. What a reader holds instead is visibility: a witness setting the byte on every head releases a stream of heads none of which carries another's digest, which any party holding two of them sees. No rule bounds the rate at which one party designates witnesses, so a party minting identities in bulk drives every operator to capacity and holds that capacity; a sound rate limit would have to bound designations per designating party without pricing the act and without reading any field an inception event's author chooses.
...
**Attestation forgery:** Attestations (§7.4) are signed by the issuer's Active Signing Key. Forgery requires the issuer's private key.
...
**A relay that witnesses is accountable, not trusted, and the untrusted-relay invariant holds over it unchanged.** A relay an identity designates in its key state cosigns that identity's log head (§9.7.4.3), and every relying party verifies that cosignature itself against the P-256 key the operator's community-relay-list entry declares, rather than taking the relay's word for anything. A witness that equivocates while recording one baseline truthfully is attributable by name, under the five-condition fault proof §9.7.4.3 defines; a witness that writes a previous digest of its own choosing fails that predicate and is not. Either way the objects say nothing about which of the two chains is authoritative. A witness supplies no confidentiality property and reads no encrypted content; what it supplies is one signed observation of what it held and when.

### 9.9.1 Relay Capabilities and Limitations

A relay CAN:

- **Read metadata:** routing IDs (per-context pseudonyms, §9.10.4), recipient hints (pseudonyms), blob TTLs, padded blob sizes, and connection timing. Context IDs, sender and recipient identifiers, and timestamps are inside the encrypted payload and NOT visible to relays (§9.10.2). Relay CANNOT read encrypted content.
...
- **Read broadcast content.** Broadcast content is AES-256-GCM encrypted with author broadcast keys. Relays see encrypted blobs and author identifiers, but cannot decrypt without the broadcast key (which is distributed only to registered, non-blocked subscribers via HPKE).
...
**Broadcast mode confidentiality differences:** In broadcast contexts, the following are NOT confidential (by design): author identifier (visible in BroadcastEnvelope), routing_id (publicly derived from context_id via SHA-256), key epoch number. These are acceptable because broadcast authors are public figures and the routing_id must be discoverable for subscribers to subscribe. Message content, subscriber identities (in key request/response exchanges), and block lists remain confidential.
...
Each participant derives a per-context keypair that replaces their identifier in all outer-envelope fields:
...
- **Verification:** Sender includes its full identifier inside the MLS-encrypted payload. Group members verify the pseudonym-to-identifier mapping on first encounter and cache the association.
...
**Threat: publicly derivable pseudonyms enable membership enumeration.** If `identity_key_material` in the HMAC were the raw P-256 public key bytes (which are public by definition), any party knowing a `context_id` and an identity's public key could compute `HMAC-SHA256(public_key_bytes, context_id || "scp-pseudonym")` and test whether the resulting pseudonym appears as an active subscription on a relay. This constitutes a membership enumeration oracle.
...
### 9.10.7 Identity Resolution Privacy
...
2. **A first contact spreads its two queries across two operators.** R11's floor (§9.7.4.2) sends the two queries to community relays under distinct operator keys, so no operator observes a whole first contact.
...
The **key-event record frame** is the unencrypted counterpart to the §9.10.2 Minimal Outer Envelope: where the outer envelope is *confidential by encryption*, a key-event record is *self-certifying through the key-event log it carries* (§9.6.1). The frame is named for what it carries: a segment of the key-event log, never a W3C document, which ADR-063 defers and this protocol does not produce. It is specified in §9.10 because that is where relay-stored record formats live. This design is issue #482. (§9.18.11 fixes the shared relay blob-size bound this frame reuses.)
...
**Storage model — a raw relay blob.** A key-event record is stored as a **raw relay blob** at its routing ID, never wrapped in an `OuterEnvelope` and never MLS-encrypted: its authenticity comes from the key-event log its `value` carries (§9.6.1). Publication and resolution use the existing PUBLISH and QUERY operations (ADR-004, the SCP native relay protocol), and this frame introduces no wire type of its own. PUBLISH is the operation's one name. `ClientMessage::Publish { routing_id, retain, payment_receipt, blob }` writes the frame bytes, and `RelayMessage::Blob { …, blob }` returns them.

**PUBLISH gains two fields.** A PUBLISH carries `retain: bool`, REQUIRED true for the four retained kinds — `scp:did:`, `scp:svc:`, `scp:wit:` and `scp:wcf:` — and absent for every other kind. Without that one wire bit a relay cannot tell a retained record from a blob under the shared TTL. A PUBLISH also carries an optional `payment_receipt`, the `PaymentReceipt` bytes of `19-economic-governance.md` §19.2.1 tagged with the adapter that issued them, which is the field a charging relay verifies before it refuses `Payment` and the field R9's receipt checks read. A client obtains the policy both fields answer to through ADR-004's `POLICY` query: **the relay answers with a `DeclaredWritePolicy`, the type `03-identity.md` §3.10.10 declares, carrying the `relay_config` fields of `18-addressability-and-deployment.md` §18.3.3 and carrying no field of the relay's own operational configuration.** It signs that answer with nothing, because the relay is the authority on its own policy and no third party attests it, and a client that cannot reach the query publishes anyway and lets the relay's own typed refusal be the authority. ADR-004 states the residue an unsigned answer leaves at its `economic.payee` field. **The flag binds a validating relay and binds no other relay.** A validating relay stores a `retain` PUBLISH under R9's retention rules and gives it no TTL; R9's ring buffer is what removes such a record, and R9 states what a verified receipt buys. The relay rejects a PUBLISH at one of the four retained kinds that does not set the flag, and rejects the flag on any other kind, both of which it can execute because it decodes the blob and confirms the routing id against the derivation the kind registers (`03-identity.md` §3.10.2). **A relay that does not validate rejects a PUBLISH carrying `retain: true`**, which is the one test it can run: it holds the address digest and never the preimage, so it can neither read the kind off the address nor scope the retention the flag asks for. It stores a PUBLISH carrying no flag as an opaque blob under the shared TTL, as it stores any other blob. `blob_ttl` is absent on a `retain` write, and a validating relay rejects a PUBLISH that carries both.

**Two further fields carry the relay proof of control.** <!-- scp:fragment id="relay-proof-wire-fields" -->`QUERY` carries an optional `proof_nonce`, 32 bytes the resolver drew freshly for that query, REQUIRED on a first contact, and `BLOB` carries an optional `relay_proof`, 200 bytes, which a relay that received a `proof_nonce` and can sign under the operator identity its community-relay-list entry declares MUST return. **A resolver MUST NOT read a `relay_proof` whose nonce differs from the one it sent, and the proof covers the response and not the frame.**<!-- scp:end id="relay-proof-wire-fields" --> `ClientMessage::Query` carries that `proof_nonce: [u8; 32]`, which a resolver sets to 32 bytes it drew freshly for that query and to nothing else, and which `03-identity.md` §3.10.4 step 2 makes REQUIRED on a first contact. `RelayMessage::Blob` carries an optional `relay_proof`, the object §9.7.4.2's definitions state, at the width that section fixes, and a relay that received a `proof_nonce` and can sign under the operator identity its community-relay-list entry declares MUST return one. Both fields are optional on the wire because a relay outside that list declares no operator and has nothing to prove: it omits `relay_proof`, and §9.7.4.2 R11 counts it as one unattributed source. **A resolver MUST NOT read a `relay_proof` whose `nonce` differs from the one it sent**, which is why the nonce is a request field rather than a value the relay chooses. **The proof covers the response and not the frame**: its `value_digest` is taken over the blob bytes the response returned, in the order the relay returned them, so one proof covers a whole QUERY response at one routing id whatever number of frames that response carries. **The relay attaches that proof to the response's final BLOB message, and a resolver evaluates it after `EVENT query_complete`**, because a proof attached to an earlier message covers bytes the relay had not yet sent and two resolvers would then digest two byte strings for one response.

**The wire change this section makes to ADR-004** is six items: `retain` and `payment_receipt` on PUBLISH, `proof_nonce` on QUERY, `relay_proof` on BLOB, the `POLICY` query a publisher reads the relay's declared write policy through, and the `RENT` operation a controller pays rent on. ADR-004 carries all six, and states the last two: <!-- scp:include id="rent-wire-fields" from=".docs/adrs/phase-1.md" -->**The `RENT` operation carries four fields and no others**: `identifier`, the 32 raw digest bytes of the identity the rent is credited to; `units`, a `u32` count of declared quotas, at least 1 on a message carrying a receipt and 0 on a message carrying none; `payment_receipt`, optional, the `PaymentReceipt` bytes of `19-economic-governance.md` §19.2.1 tagged with the adapter that issued them and bounded by the Constraints line below; and `signature`, the 64-byte beneficiary signature `09-security-model.md` §9.7.4.2 R9 defines.<!-- scp:end id="rent-wire-fields" --> <!-- scp:include id="policy-answer-type" from=".docs/adrs/phase-1.md" -->**The `POLICY` answer's declared type is `DeclaredWritePolicy`, which `03-identity.md` §3.10.10 declares, and never `RelayConfig`, the relay server's own flat configuration object, which carries the internal connection-admission secret.** The answer carries the `relay_config` terms of `18-addressability-and-deployment.md` §18.3.3 that state a term of the write policy, carries no field of the relay's own operational configuration, and carries no signature, because the relay is the authority on its own policy and no third party attests it.<!-- scp:end id="policy-answer-type" --> **No wire field binds a receipt to a frame**: R9's receipt checks read the terms `PaymentAdapter::verify` returns.

**Relay-side validation.** `03-identity.md` §3.10.2 states the checks a validating SCP-native relay runs on PUBLISH and the rules that govern what may open or extend a chain at a routing id once a frame establishes one there; §9.7.4.2 R9 states what a chain holds, the write rule, the serving order and the ring buffer. This section states the frame's bytes.
...
- **version** — a `u8`, currently 2. Any change to field encoding bumps the version. **This frame starts at 2 because `0x01` is a live wire version for a different grammar**: the superseded identity-record frame the `scp-protocol` crate still implements and the key-event record replaces. This is not migration compatibility, which this pre-release protocol does not carry — the superseded frame is deleted when the key-event record lands in the crates, and `0x01` is then retired rather than reused.
- **identifier** — the identity's 32-byte inception-derived identifier. The relay's identifier-to-routing-id binding check compares the registered routing derivation over this field against the `routing_id` the frame is published at (`03-identity.md` §3.10.2). The identifier encodes no key (§9.7.4.2 R13), so nothing about the frame's authorization follows from it alone.
- **The frame carries no signature.** A signature over the frame would be checked against a key read out of the writer's own chain, and no frame that passes chain verification could fail it except by a byte change that chain verification already rejects, so the frame carries none and the relay's write decision is chain verification plus R9's rule for what may open or extend a chain (`03-identity.md` §3.10.2, §9.7.4.2 R9). **The frame carries no sequence number** either. An event's sequence is its position on the chain (§9.7.4.2 definitions), so a chain that extends a stored chain has a strictly higher head sequence by construction and the relay's write rule reads the extension rather than a number beside it; a frame-level sequence would restate the chain's own order in a field a writer supplies.
- **value** — the sole variable-length field, carried as the **trailing remainder** of the frame (no `value_len` prefix). Because every preceding field is fixed-width, `value` is unambiguously `frame_bytes[33..]`; a length prefix would be redundant with the blob's own length and a determinism footgun (two disagreeing lengths). `value` MUST be non-empty and its length MUST NOT exceed `Max blob size` (262144, §9.18.11) `− 33`. It carries one contiguous chain segment, and a relay serves a chain as the ordered sequence of frames covering inception through head (§9.7.4.2 R9).
...
| 4 | `events` | `event_count` records in ascending sequence order, each a 4-byte big-endian length prefix over the event's §9.5.1 signed preimage bytes, followed by the indexed signature field the event's kind, index lists and signature-form lists fix (§9.7.4.2 R3) |

A decoder MUST reject a frame whose `event_count` is zero, whose `last_sequence` is below its `first_sequence`, or whose declared range does not equal the sequences its events carry — `last_sequence − first_sequence + 1` events, each one sequence above its predecessor. The decoder reads the event's kind, its signer index lists, and its per-signature form lists out of the preimage it just read, and derives the signature field's length from those three together with the two length prefixes each assertion-form indexed signature carries inside itself (§9.7.4.2 R3, the closed signature layout). It walks the field one indexed signature at a time: a `0x01` indexed signature is 64 bytes, and a `0x02` indexed signature is a 4-byte length and that many bytes of `authenticatorData`, then a 4-byte length and that many bytes of `clientDataJSON`, then 64 bytes. A decoder MUST reject an indexed signature whose `authenticatorData` length is below `MIN_AUTHENTICATOR_DATA_BYTES` or exceeds `MAX_AUTHENTICATOR_DATA_BYTES`, whose `clientDataJSON` length is zero or exceeds `MAX_CLIENT_DATA_JSON_BYTES` (§9.18.17), or whose declared length runs past the field, and MUST reject an event whose signature field does not end exactly where the last indexed signature ends. **No length outside an indexed signature states the field's size**, so the frame carries no second length for a decoder to disagree with. **The three lengths `value` itself supplies are bounded here too**, because `value`'s own trailing-remainder shape bounds none of them: a decoder MUST reject an event whose 4-byte preimage length prefix runs past the remaining `value` bytes; MUST reject a frame whose events do not end exactly at the end of `value`, so a relay cannot append bytes after the last event and have one binding accept the frame while another rejects it; and MUST NOT size any allocation from `event_count` before it has read that many events.
...
**Every byte of the framing is unsigned, and the chain inside it carries the whole authority.** The frame's `version`, its `identifier` field, and its field boundaries are transport framing that no key signs. **A decoder MUST NOT derive any security-relevant conclusion from an unsigned field.** **A relay reads the unsigned `identifier` field to bind the frame to its routing id, and on the inception branch of `03-identity.md` §3.10.2's chain check it recomputes the identifier from the inception event and rejects a frame that recomputes to anything but that field.** **On the extension branch the relay recomputes no identifier and verifies the frame's events against the standing root the assembled chain carries at that position**, which is what makes the unsigned field unable to carry an extension a key did not authorize. Every other input a decision reads, the standing root set included, comes from the verified chain and never from the framing.
...
- **Query** = the existing QUERY operation (§3.10.4) with `limit: N` (N = 16) returns blobs stored at the routing ID; the resolver decodes each at the **single** decode-and-verify site, verifies the chain each `value` carries under §9.7.4.2 R2 and R3, and settles two chains that diverge by the fork-precedence rule (§9.7.4.2 R6). It reads no key from the frame. **A relay stores one chain as an ordered sequence of frames, so one QUERY does not necessarily return a whole chain**, and the resolver pages it by the walk §9.7.4.2 R9 states, re-issuing QUERY with `since` set to the `stored_at` of the last frame it accepted. `N` bounds one page and bounds no chain (§9.7.4.2 R9), so a long chain spans far more frames than `N`. Against non-validating or foreign storage `limit: N` also lets the resolver retrieve up to N candidates and sift them by R2, R3, and R6 (§3.10.4 step 5). See §3.10.2 for why `limit: N` dominates `limit: 1`.
...
**Two further frames carry the witness layer's objects, each at its own routing derivation.** §9.7.4.3 states both objects' fields and both signature preimages; this section states the frames' bytes.

```
COSIGNED-HEAD-RECORD (CosignedHeadRecordV3) :=
  version:    u8         = 3
  subject:    [u8; 32]                 # the cosigned identity's identifier (§9.7.4.2 R13)
  value:      [u8]                     # trailing remainder = one or more cosigned heads, each of the width §9.7.4.3 fixes

WITNESS-CONFLICT-RECORD (WitnessConflictRecordV4) :=
  version:    u8         = 4
  subject:    [u8; 32]                 # the subject identifier
  value:      [u8]                     # trailing remainder = one or more conflict statements, each of the width §9.7.4.3 fixes
...
A cosigned-head record is published at `SHA-256("scp:wit:" || subject_identifier_bytes)`, a conflict-statement record at `SHA-256("scp:wcf:" || subject_identifier_bytes)`, and a service-record frame at `SHA-256("scp:svc:" || identifier_bytes)` (§9.7.4.2 R13), so the address is the type discriminant for each exactly as it is for the key-event record above. **The service-record frame's `value` is the record's §9.5.1 signature preimage bytes after the separator, followed by the 64-byte signature** that `03-identity.md` §3.10.13's designated key produced over them, and a decoder MUST reject a frame whose `value` does not end exactly where that signature ends. `03-identity.md` §3.10.13 states the record's fields and its per-entry encoding. Its entry count and its three per-entry string lengths take the one parse bound §9.7.4.2's definitions state, so a decoder rejects a count or a length whose implied bytes run past the enclosing field and sizes no allocation from either before it has read that many elements. **The witness layer's two objects are fixed-width, so each of those two `value` fields is a whole number of objects and the decoder needs no per-object length.** For those two a decoder MUST reject a frame whose `value` length is not a positive multiple of the object width its `version` names, and for all three it MUST read and validate `version` before any subsequent byte, MUST reject a frame shorter than the 33-byte fixed prefix, and MUST reject a `value` longer than `Max blob size` (262144, §9.18.11) `− 33`; the four decoder rules above govern these two frames unchanged. A cosigned-head record's objects are served in ascending `sequence` order. **The version bytes are distinct from the key-event record's `0x02`** so that a frame misdelivered across routing derivations fails on its first byte rather than on a field boundary, which is the same byte-level backstop the outer-envelope disjointness paragraph below states. Neither frame carries a signature of its own: each object inside `value` carries one.

**Divergence is defined over events, never over framing.** Where `03-identity.md` §3.10.4 compares two records serving heads of one chain, it compares the two chains **event by event over each event's preimage digest**, never over the chain bytes and never over a field of the frame. A hardware signer draws its own nonce, so two copies of one event differ in their signature bytes, and a byte comparison would read one honest identity's duplicate signature as two divergent suffixes.

**Retention, not expiry, and it reaches four kinds.** A record at `scp:did:`, `scp:svc:`, `scp:wit:` or `scp:wcf:` carries no TTL. A validating relay retains all four under §9.7.4.2 R9, which is what the `retain` flag on the PUBLISH declares, so no republication cycle makes any of them permanent and the six-day cycle is deleted for all four. **R9's ring buffer is what removes such a record**, and R9 states what a current rent buys against it. The chain's own order governs a key-event record's supersession (§3.10.7). The other three cost a bounded figure per subject — one service record per identifier, one cosigned head per subject, witness and `event_digest`, and one conflict statement per subject and witness — so what a relay retains for one identity is bounded by that identity's configuration, while under the shared TTL a quiet identity's service record became unroutable and a fault proof's two heads expired before a third party could assemble them. The shared relay `blob_ttl` of §9.10.2 governs every other kind and reaches none of these four: that TTL bounded a single small mutable record a controller republished cheaply, and carrying it onto an append-only chain would expire an identity's whole key history from every relay unless the controller re-uploaded the chain twice a fortnight.
...
Every key state a verifier accepts names exactly one `#active` key, because §9.7.4.2 R3's author-attributable-defect list rejects each of the two snapshots that would break that count. No sentinel stands in for an absent operational key.
...
**The standing.** The SDK keeps, for every identifier it has encountered, one `ContinuityStanding` with three values and no fourth: `Verified`; `PendingReverify`, meaning the SDK observed a key change; and `Unresolved`, meaning the resolution delivered no chain. `Contested` is a `ResolutionVerdict` variant and never a standing, and a `Contested` verdict has already set `PendingReverify` by the time any gate reads a standing, so a gate branches on these three values and on no fourth. Beside the standing the SDK records two further fields of that record: the root-install digest, never as part of the record's key, and the `HeadProvenance` of §9.7.4.2's definitions. The digest is a field and not a key because keying the record to a pair that includes the digest would make every root change look like a first encounter, which is the takeover this rule exists to catch. §9.6.4 assigns the standing on first contact.
...
**The authoritative gate list.** While an identifier's standing is `PendingReverify` or `Unresolved`, the SDK MUST NOT: add a new leaf for it to a context; issue a UCAN to it; distribute a sender key to a new leaf of it; grant anything to a leaf it replaced; auto-accept it at the standing-pair consent gate, the known-identity allowlist, or a cached `author_keys` entry; or auto-accept an invitation, a standing-pair request, or a contact-graph promotion from it. `PendingReverify` withholds three further acts: decrypting under an `author_keys` entry the subscriber already holds, accepting a UCAN that identifier issued, and accepting a UCAN carrying that identifier anywhere on its delegation chain. Alec approved the scope of the first three auto-accept gates on 2026-09-03. The two UCAN acts follow from a recovering controller being unable to enumerate the tokens an attacker issued, which makes refusal on the relying party's side the one act that stops them.

`Unresolved` reaches none of those three acts, because the verifier learned nothing about the identifier: withholding the two UCAN acts would reach every token the victim ever issued on the strength of one relay's outage, and withholding the decryption would take an honest subscriber's read access to an honest author's content for the length of a third party's outage.
...
**The gates that read a `ContinuityStanding` take their conditions from the list above.** They are the Add gate, the UCAN gate, the sender-key-to-a-new-leaf gate, the broadcast per-author key cache, the standing-pair consent gate, the known-identity allowlist, and the key response of §9.16.2 step 3. The gates that also name the act in the section that owns it — `05-contexts.md` §5.14.2, `05-contexts.md` §5.15.8, `05-contexts.md` §5.12.2, and §9.16.2 step 3 — cite the list above there and state no condition of their own, so no section can drop a standing the list carries. §9.6.4 reads no standing: it assigns the identifier's first standing from the resolution verdict, which is not a gate condition.
...
**Clock skew tolerance:** 5 minutes. Messages with timestamps more than 5 minutes in the future are rejected. This is generous enough to handle devices with poorly-set clocks while tight enough to limit replay windows. **Three further readers apply the tolerance, and one of them gates a verdict.** A witness compares the `observed_at` it is about to write against its own clock and against its stored value, which decides whether that witness cosigns and decides nothing else (§9.7.4.3). **A relay treats an identity's rent term as elapsed only past the expiry it recorded plus this tolerance**, which decides whether the ring may displace that identity and decides nothing else (§9.7.4.2 R9). **A resolver compares the `observed_at` of a relay proof of control against its own clock, and that comparison decides an R11 verdict** (§9.7.4.2 definitions, R11): a resolver whose clock is out by more than this tolerance counts no proven source, meets no first-contact floor, and returns `Inconclusive{SingleSource}` with the clock-skew rejections counted separately. The direction is fail-closed. No key event's own acceptability reads a clock.
...
3. **Key response.** The key holder's SDK processes the request: verifies the signature, checks the block list, and reads the requester's `ContinuityStanding` (§9.11), applying the authoritative gate list that section carries. This step names no standing value and states no exception of its own. If the requester is not blocked, the key holder responds with `SenderKeyResponse { sender_did, epoch, hpke_sealed_key, ephemeral_pubkey, request_nonce }` via an MLS application message with `recipient_hint` set to the requester. The sender key is sealed using HPKE Base mode (RFC 9180) to the requester's ephemeral wrapping key.
...
This suite matches the MLS ciphersuite (§9.5) and the identity-to-identity HPKE suite, minimizing the cryptographic surface area.
...
3. Alice publishes `SenderKeyEpochAdvance { sender_did: alice_identifier, epoch: N, signature }` as an MLS application message. **O(1) cost** — no per-recipient HPKE payloads. All group members see the epoch advance.
...
The block list (§9.16.3) is per-identity. A Sybil attacker — one human controlling several identities — can create a fresh identity that no block list names and use it to request the new sender key after a block event. An agent is a separate delegated identity, and its key state names the delegator that anchors it (§9.7.4.2 definitions), so the delegation anchor ties each agent identity to one human identity and a blocker reads that anchor from either chain, once the delegation model lands and R3 stops rejecting every chain whose delegator is nonzero (§9.1 invariant 4). The Sybil cost of an agent identity is therefore the cost of the human identity whose chain anchors it, which is how key material carries the "every agent traces to a human" tenet. The block list alone does not prevent all bypass. This section specifies the mitigations.

**Mitigation 1: Membership gate.** `handle_sender_key_request` MUST verify that the requester's identity is a current member of the context before distributing sender keys. In Encrypted contexts, MLS group membership already gates who can observe application messages (including `SenderKeyRequest`), so this is defense-in-depth redundancy. In Broadcast contexts, where key requests travel as relay messages outside MLS, the membership gate is the primary defense: a Sybil identity that has not been admitted through normal subscription controls (identity-authentication for open contexts, UCAN validation for gated contexts) cannot request keys. The Sybil attacker must first pass the context's admission controls — earned capacity thresholds, UCAN gating, device attestation requirements, or whatever the context mandates — before they can even attempt a key request. This raises the cost of Sybil bypass from "create an identity" to "create an identity AND satisfy context admission requirements."

**Mitigation 2: Identity-linked block expansion.** When blocking an identity, the blocker's SDK SHOULD expand the block list to include every identity known to be linked to the same identity. Identity linkage sources include:
- **Delegation anchors** (§9.7.4.2 definitions; no delegated chain resolves until the delegation model lands, because R3 rejects every chain whose delegator is nonzero): a delegated identity's key state names its delegator, and the delegator's chain carries the key-event seal that anchors the delegate, so a blocker who blocks either end reads the other from the chain.
- **Attestation chains** (§3.5, §7.4): identities with shared social attestations, mutual endorsements, or attestations from the same issuer linking to the same external identity.
- **Governance records**: identities flagged as Sybil aliases by context governance (e.g., admin-initiated Sybil reports).
- **Behavioral correlation**: identities exhibiting correlated activity patterns (same message timing, same relay, same device attestation) that context-level detection flags.
...
1. The blocker removes the target identity from their block list (identity private state, §3.7.1).
...
pub struct WrappedCek {
    /// First 8 bytes of SHA-256(member_did) — prevents identifier publication.
    pub member_id: [u8; 8],
...
2. Each member's SDK, upon receiving and verifying the `AccessKeyRevoked` event:
   ...
   b. Adds the target's identifier to the local access key revocation list.
...
1. The target's identifier is added to the exclusion list for future CEK wrapping.
...
|----------|-------|-------|----------------|
| Signature algorithm | ECDSA on NIST P-256 with SHA-256 (FIPS 186-5) | All identity keys, envelope signatures, UCAN (ES256), MLS leaf credentials. §9.5 scopes the RFC 6979 and low-`s` rules | §9.5 |
| MLS ciphersuite | `MLS_128_DHKEMP256_AES128GCM_SHA256_P256` (RFC 9420 ciphersuite 2) | RFC 9420 §17.1 | §9.5 |
| HPKE suite (identity to identity) | DHKEM(P-256, HKDF-SHA256) (KEM ID `0x0010`), HKDF-SHA256, AES-128-GCM | RFC 9180 Base mode | §9.5 |
...
| `"SCP-KEYPACKAGE-ATTESTATION-V1:"` | KeyPackage attestation (ephemeral MLS leaf key ↔ identity binding) signing | §9.5.2 |
...
| `"SCP-MIGRATION-V1:"` | Identity migration proof — **retired**: no migration proof exists in the key-event-log model, because a root-key change is a `RootRecovery` signed under `"SCP-KEL-EVENT-V1:"` | §9.7.4.2 R13 |
...
| `"SCP-RENT-V1:"` | The `RENT` message's beneficiary signature. <!-- scp:include id="rent-preimage-operands" from=".docs/specs/09-security-model.md" -->**The `RENT` signature preimage is `SHA-256("SCP-RENT-V1:" ‖ identifier ‖ BE32(units) ‖ receipt_digest ‖ operator_key)`, and each operand is stated here so two implementations compose one preimage.** **The preimage is 113 bytes**: the twelve separator bytes and 32 + 4 + 32 + 33 = 101 field bytes. `identifier` is the 32 raw digest bytes. `BE32(units)` is the count as a four-byte big-endian integer, at least 1 on a message carrying a receipt and 0 on a message carrying none. **`receipt_digest` is `SHA-256(payment_receipt)` where the receipt is present and the 32 bytes `SHA-256(0x00)` where it is absent**, under §9.5.1's rule for an absent fixed-length optional field. **`operator_key` is the 33-byte SEC1 compressed operator key the addressed relay declares in its own `POLICY` answer**, which the controller reads before it sends and which the relay already holds as the key it proves control of.<!-- scp:end id="rent-preimage-operands" --> The signer is the key the key state at the head of the chain the relay holds lists `Current` in the `#active` role (§9.7.4.2 R9) | §9.7.4.2 R9 |
...
| `"SCP-KEY-DESTRUCTION-V1:"` | Key destruction proof — a **log-anchored evidence** separator, one of the three §9.7.1's classification table lists, with `"SCP-CONTEXT-SNAPSHOT-V3:"` and `"SCP-CONTEXT-EXPORT-V3:"`; the artifact carries the digest of the signer's key-state head at signing | §9.15 |
...
| Member ID size | 8 bytes | Truncated SHA-256 of the member identifier | §9.17 |
...
| `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` | 300s (5 min) | Maximum age of the resolved key state used to satisfy the KeyPackage-attestation **current-key** check (§9.7.1 verifier checks 1–2), and the same bound the §9.11 auto-accept gates and the §9.7.1 content-signature path read. Tied to the §9.14 clock-skew tolerance (also 5 min). **Decoupled from — and far tighter than — the §9.10.7 identity-resolution privacy cache TTL (24h / 7d)**, which MUST NOT satisfy the current-key check beyond this bound. §9.7.1's "Resolution failure policy" states what a verifier does when the resolution fails. | §9.7.1 |
...
| Key-event record frame version | 2 | Current key-event record relay frame version (§9.10.12); bumped on any field-encoding change. §9.10.12's `version` bullet states why this frame starts at 2. The frame has no magic tag and no record-kind byte — the identity's `routing_id` domain is the type discriminant. Unsigned framing — grants no authority. See also §9.18.11 Transport and Relay for the shared blob-size bound the frame reuses. | §9.10.12 |
...
|----------|-------|------------|---------|
| `MAX_ROOT_SET_SIZE` | 16 | An organization's officer set fits, and a cap of 64 would let one event carry 4 KB of signatures. The cap MUST stay at or below 255: the preimage's signer index list carries one byte per index, so a root set past 255 makes a threshold-satisficing subset inexpressible in the encoding the event itself uses | §9.7.4.2 R3's defect list; the installed-root-set field of the key-event preimage |
| `MAX_NEXT_SET_SIZE` | 16 | Same derivation as the root-set cap, and the same one-byte signer index ceiling of 255 binds the reveal group's indices, so this cap MUST also stay at or below 255 | §9.7.4.2 R3's defect list; the reveal group; the next-threshold recommendation of §9.7.4.1 item 4 |
| `MIN_STORAGE_BUDGET_IDENTIFIER` | **Open — this specification states no figure.** The floor's terms are `MAX_WITNESS_SET_SIZE` × `MAX_RETAINED_SUFFIXES` cosigned heads at one address and the same count of conflict statements at a second, each at its own fixed width, plus `MAX_EVENT_BYTES`. **The two encoded widths the product multiplies — a cosigned head's and a conflict statement's — are what no section of this corpus registers, and the figure waits on both.** The two candidate shapes, and this revision chooses neither: this section registers each object's encoded width as a constant of its own and the floor cites the product; or this section registers the floor as one figure and states the derivation beside it. | The floor a declared `storage_budget_identifier` clears, because two byte classes count against the per-identifier budget and no act of the identity's controller removes either: the `scp:wit:` and `scp:wcf:` records other parties write about it, which the coverage test counts for nobody and the eviction arm does not reach, and the identity's own inception event. The floor bounds what an operator may declare and bounds nothing any party writes, so it adds no refusal and removes one | §9.7.4.2 R9; `18-addressability-and-deployment.md` §18.3.3; `03-identity.md` §3.10.10 |
| `MAX_RETAINED_SUFFIXES` | 8 | Two of a controller's devices plus several attacker forks fit with margin | §9.7.4.2 R9's admission and eviction rules and its two-object bound; the per-identifier chain cap of a validating relay; the conflict-statement cap of `03-identity.md` §3.10.2 |
| `MAX_RETAINED_BYTES` | 512 MiB | The product of three registered constants: `MAX_RETAINED_SUFFIXES` (8) × `MAX_EVENTS_PER_SUFFIX_RETAINED` (1,024) × `MAX_EVENT_BYTES` (65,536) = 536,870,912 bytes | §9.7.4.2 R9's byte bound, which measures what a party **retains** and counts divergent-suffix bytes per identifier; §9.7.4.2 R9's fetch bound, which measures what a verifier **downloads** for one identifier from one source in one resolution and counts every byte it read; the per-routing-id retention of `03-identity.md` §3.10.2. **Whether the two readers keep one figure is open** (§9.7.4.2 R9), so an implementer choosing a lower per-identifier retention reads R9 before it also lowers what a verifier downloads |
...
| `MAX_PAYMENT_ADAPTERS` | 16 | The ceiling on the adapters one relay declares, and on the `payment_adapters` list a publisher reads out of a `POLICY` answer or a `4041` refusal. `19-economic-governance.md` §19.2.7's registry names five, and a relay that accepted every registered rail plus room for three revisions' additions stays inside this figure | The parse bound of §9.7.4.2's definitions; `03-identity.md` §3.10.10's `BudgetValue::Payment` and `DeclaredWritePolicy` |
...
| `MAX_CLIENT_DATA_JSON_BYTES` | 512 | The members SCP reads run near 160 bytes once a browser adds `origin` and `crossOrigin` | The assertion-form layout; the frame decoder of §9.10.12 |
...
| `MAX_WITNESSING_INTERVAL` | 604800 s (7 d) | The interval is the floor on the controller's self-observation cadence, and seven days is the point past which a controller reads its own chain less often than once a week, so the device-compromise signal of §9.11 would reach it after the working week in which the compromise happened. The cadence fixes a fetch and fixes no publish: R10 re-publishes to a fallback-set entry only where the fetch shows that entry holding no record | §9.7.4.2 R3's defect list; R10's self-observation floor |
| `DEFAULT_WITNESSING_INTERVAL` | 3600 s (1 h) | Chosen against the self-observation cadence it fixes: hourly reaches the device-compromise signal of §9.11 within one working day and costs a quiet identity 24 fetches a day and no publish, because R10 re-publishes only to a fallback-set entry the fetch found holding no record | The SDK's default witness set in §9.7.4.3; §9.7.4.2 R10 |
| `MAX_SUBJECTS_PER_WITNESS` | 10,000,000 | At about 113 bytes per record the store stays near a gigabyte, which makes that figure a derivation rather than an illustration | The witness's admission and eviction rules in §9.7.4.3; `17-persistence-and-storage.md` §17.17.4 |
...
| `RECOVERY_HANDLE_BYTES` | 66 | The encoded width of a `RecoveryHandle`: one discriminant byte, the 32-byte identifier, the one-byte `RecoveryPhase`, and the 32-byte preimage digest of the composed event. The discriminant is load-bearing, because the continuation is the variant tag and `Abandons` therefore has no next-set field by construction. The figure sits two orders of magnitude below CTAP 2.1's 1,024-byte floor on an authenticator's whole large-blob array, which is the bound that decides what the credential-keyed store set can hold | §9.7.4.2 R10's declaration block; `17-persistence-and-storage.md` §17.8 |
...
|-----------|---------|-------|-----------|----------------|
| `rate_limit_publish` | <!-- scp:include id="relay-config-rate-limit-absence" from=".docs/specs/18-addressability-and-deployment.md" -->An absent `rate_limit_publish` is 6,000 a minute, and a relay turns the term off by declaring zero.<!-- scp:end id="relay-config-rate-limit-absence" --> | Any value the operator sets; zero turns the term off | <!-- scp:include id="rate-limit-check" from=".docs/specs/09-security-model.md" -->**The rate-limit check counts every PUBLISH and every `RENT` against the publishing address, reads no field of a receipt, and decodes nothing.** No declaration exempts any party from it, because a relay sells relief from a storage refusal and sells no relief from the rate limit.<!-- scp:end id="rate-limit-check" --> | §9.7.4.2 R9; §18.3.3 |
| Per-identifier storage budget, total storage budget | <!-- scp:include id="relay-config-storage-absence" from=".docs/specs/18-addressability-and-deployment.md" -->An absent or zero `storage_budget_identifier` turns that term off, and an absent `storage_budget_total` is the retained capacity the operator's own host holds with the ring beneath it and never "no limit".<!-- scp:end id="relay-config-storage-absence" --> | Any value the operator sets; zero turns a term off | <!-- scp:include id="refusal-value" from=".docs/specs/09-security-model.md" -->**`value` carries the declared term the refusal names — the budget figure, the rate, or the price — so the one object a publisher holds after a refusal states what would satisfy it in every term but the rail-native address it settles to**<!-- scp:end id="refusal-value" --> <!-- scp:include id="total-scope" from=".docs/specs/09-security-model.md" -->**`Total` refuses a write where displacing every uncovered identity the relay may displace still leaves the retained capacity short of the arriving frame's bytes, and refuses no write while one such identity remains**, because the ring below displaces the oldest-established uncovered identity instead. That one refusal covers the write that carried a receipt and the write that carried none, and it returns `StorageBudgetExceeded{Total, value}` carrying the declared total budget, which is the figure the `POLICY` query returns and the figure that states what would satisfy the refusal.<!-- scp:end id="total-scope" --> <!-- scp:include id="identifier-eviction" from=".docs/specs/09-security-model.md" -->**Under `Identifier` pressure the relay evicts before it refuses:** it evicts that identifier's retained divergent suffixes in the eviction-rank order below, **evicting the retained suffix carrying the highest rank number first**, and refuses only where evicting every suffix it may evict still leaves the per-identifier budget exceeded.<!-- scp:end id="identifier-eviction" --> | §9.7.4.2 R9; §18.3.3 |
| `receipt_exempts` | <!-- scp:include id="relay-config-receipt-exempts-absence" from=".docs/specs/18-addressability-and-deployment.md" -->An absent or empty `receipt_exempts` names no scope.<!-- scp:end id="relay-config-receipt-exempts-absence" --> | The scope names §9.7.4.2 R9 permits a relay to declare | <!-- scp:include id="receipt-exempts-declaration" from=".docs/specs/09-security-model.md" -->**A relay that charges declares which refusal scopes a current rent exempts an identity from.** `relay_config` carries `receipt_exempts`, a set whose members are scope names, and ADR-004's `POLICY` query returns it, so a publisher reads what its money buys before it spends. **A relay that charges and declares nothing exempts nothing**: an absent or empty `receipt_exempts` exempts no scope, and an implementer reads that declaration rather than assuming a set. **`Identifier` is the one member a relay may declare.**<!-- scp:end id="receipt-exempts-declaration" --> <!-- scp:include id="identifier-exemption" from=".docs/specs/09-security-model.md" -->**A declared `Identifier` exemption stops the refusal arm of the per-identifier budget and stops no eviction**, for the identity whose rent is current, for as long as that rent is current, and up to the quota that rent bought and no further. **Its refusal test is one arithmetic: the relay tests, against the declared per-identifier budget, the bytes it holds for that identity less the smaller of that identity's counted bytes and its current quota.** **A rank-2 and a rank-3 divergent suffix stay evictable in the eviction-rank order below whatever that identity's coverage**, because the coverage test counts none of their bytes, so an exemption over them would shelter bytes nobody paid for.<!-- scp:end id="identifier-exemption" --> | §9.7.4.2 R9; §18.3.3 |
| Rent price, rent period, rent quota | <!-- scp:include id="relay-config-rent-absence" from=".docs/specs/18-addressability-and-deployment.md" -->An absent `rent` member, and an absent `economic` object, each mean the relay sells no coverage and charges nothing.<!-- scp:end id="relay-config-rent-absence" --> | Any values the operator sets | <!-- scp:include id="rent-arithmetic-quota" from=".docs/specs/09-security-model.md" -->**One verified rent payment of `units` sets the identity's quota to the greater of its standing quota and `units` times the declared quota, and the standing quota is the figure the relay recorded while that identity's rent is current and is zero for an identity whose rent has lapsed.**<!-- scp:end id="rent-arithmetic-quota" --> <!-- scp:include id="rent-arithmetic-period" from=".docs/specs/09-security-model.md" -->**The same payment sets the identity's rent expiry to one declared period past the later of its standing expiry and the instant the relay verified the payment, and the declared period is the duration one payment buys whatever `units` that payment names.**<!-- scp:end id="rent-arithmetic-period" --> <!-- scp:include id="rent-arithmetic-price" from=".docs/specs/09-security-model.md" -->**The relay verifies the receipt against `units` times the declared rent price, so the arithmetic multiplies the declared quota and the declared price by `units` and adds the period once.**<!-- scp:end id="rent-arithmetic-price" --> | §9.7.4.2 R9; §18.3.3 |
