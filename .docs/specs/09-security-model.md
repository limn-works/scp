...
**Content verification returns `ContentVerdict`, and this section is its one home.** Its three variants are `Valid`, `Unverified{reason}` — the resolution-failure posture this section states, where a verifier holds the artifact and has not yet resolved a key state fresh enough to test it — and `Invalid{cause}`, whose two causes are `no_prefork_basis` and `no_boundary`, defined by §9.7.4.2 R7 and by `03-identity.md` §3.2.1 step 5 respectively. `UnverifiedReason` names the first payload type and `ContentInvalidCause` the second. **Every SDK binding carries `ContentVerdict`, its three variant names, and its two cause names unchanged** — five names in all. An `Invalid` payload here is a verdict about one artifact inside one context and never about a chain, so a resolution never returns this type and content verification never returns R14's.
...
- Active Signing Key: Rotated by a `KeyState` event signed by the standing root (§9.7.4.2 R3) that lists the new `#active` `current` and the old one `Superseded` (§9.7.1). All active MLS groups receive an Update proposal with the new credential. §9.11 states which observers set `PendingReverify` on that event and which do not.
...
**State-carrying events.** A state-carrying event carries the complete key state. Three kinds carry state: the inception event, a `KeyState`, and a `RootRecovery`. The derived key state at any position is the key state that the latest state-carrying event at or before that position carries, with one exception the reveal cycle forces: the next-set component is the standing commitment of the latest reveal-authorized event at or before that position, so a `CommitmentRollover` replaces that component and changes nothing else. No other earlier event contributes.
...
A verifier derives every historical key's condition by replaying the log. A state-carrying event asserts only the condition changes it makes: its key list carries every key `current` at that position, and a `RootRecovery` additionally carries one entry per key whose condition it changes, `Compromised{from: N}` for each key it names, together with the fresh root set and the operational keys it installs. No event erases an earlier event. **A verifier resolves a `signing_key_id` fragment by replaying the chain to the position the artifact's `key_state_head` names and reading, at that position, the key that held the named role and the condition it held there**; it never resolves a fragment from the latest key state's current keys alone, and the key state carries no set of every key that ever held a role for it to read.
...
**The retention bound.** A verifier retains at most `MAX_RETAINED_SUFFIXES` divergent suffixes per identity and at most `MAX_RETAINED_BYTES` of divergent-suffix bytes per identity. The count bound and the byte bound are the whole of the retention rule, each reads the eviction rank below, and neither refuses a suffix on arrival without reading that rank. Both bounds count divergent suffixes and count no byte of the accepted chain.

**The byte bound.** A verifier and a validating relay retain per identifier at most `MAX_RETAINED_BYTES` of divergent-suffix bytes. A relay MAY configure a lower per-identifier value and MUST NOT configure a higher one: a lower cap evicts only lower-ranked suffixes, and rank is what every verdict reads, so no configured value changes a verdict any party reaches. On overflow the party evicts the lowest-ranked retained suffix first — the retained suffix carrying the highest rank number — breaking a tie among equal ranks by evicting the suffix it observed most recently. It evicts neither the accepted chain nor a rank-1 suffix, so where the retained rank-1 suffixes fill the bound, and two of them suffice, the party refuses the arrival and returns `Contested{…, residue: n}`. An eviction returns no signal to the evicted suffix's publisher, and a suffix a party evicted and a party presents again later is re-ranked as if the party saw it for the first time. Without a byte bound the count bound leaves a suffix's bytes unbounded: a holder of one spent pre-rotation key forks below the event that spent it, signs a `RootRecovery{Lost}` installing its own root, and appends four million `KeyState` events on that rank-2 suffix, so every verifier that resolves the victim fetches about 1.1 GB and performs four million signature verifications before R6 can rank. A byte bound with no rank term would be defeated by the cheapest suffix to manufacture, so this bound reads the eviction rank below and never refuses a rank-1 suffix on bytes.

**The eviction rank** is defined for a divergent suffix and for nothing else. It is a scalar: the R6 rank the suffix carries against the accepted chain, read against the commitment standing at that suffix's own fork position. Eviction reads that one term and nothing else, and R6's pairwise comparison decides adoption while no eviction reads it. **The accepted chain — the chain the verifier held before it observed any divergence, and at a validating relay the chain whose slot the routing id's first binding-valid frame established — sits outside the retained set, carries no eviction rank, and is never evicted.** R9 counts every arrival against it. Without that clause the rank term was defined only by reference to a chain the suffix forked from, so the chain every fork hangs off had either no rank or eight, and one conforming reading evicted it first. A suffix is rank 1 in one pair and rank 2 in another, so "never evict a rank-1 suffix" had no referent and two relays evicted different slots from identical inputs.
...
**The one deferral.** What waits is display, URLs, and every signed structure that takes an identifier as UTF-8 bytes, which §9.5.2 enumerates and states once. Because that form is unfixed, no prose example anywhere in the corpus prints an identifier: an illustrative body writes `<scp-identifier:name>`, and a field whose encoding is this deferral writes `<the … identifier, in the textual form 09 §9.7.4.2 R13 defers>`. `25-test-vectors.md` §25.1 carries the one carve-out, for the fixture strings its vectors pin, and states the reason there.

**R14 — the verdicts.** A resolution returns exactly one of six verdicts of the type `ResolutionVerdict`, and every SDK binding carries the type name and the six variant names unchanged: `Confirmed`; `Adopted{head, baseline_decreased}`; `Contested{tie, heads, shared_prefix_head, fork_position, residue}`, whose `tie` field carries a `TieClass`; `Inconclusive{cause}`, whose cause type is `InconclusiveCause` with three variants, `MissingEvents{range, failed_sources}`, `CopyDefect{at_event}`, and `SingleSource{reached_sources, proven_sources, clock_skew_rejected}`; `Invalid{at_event}`; and `Discarded{accepted_head}`. A discard is a verdict so that a consumer routes it, because the resolution delivered nothing newer than the baseline.

`Invalid{at_event}` is the only cause of a resolution verdict named `Invalid`, and a copy-level defect never produces it. Content verification returns a second result type, `ContentVerdict`, which §9.7.1 defines; this rule cites that definition and restates none of it.
...
- **Query** = the existing QUERY operation (§3.10.4) with `limit: N` (N = 16) returns blobs stored at the routing ID; the resolver decodes each at the **single** decode-and-verify site, verifies the chain each `value` carries under §9.7.4.2 R2 and R3, and settles two chains that diverge by the fork-precedence rule (§9.7.4.2 R6). It reads no key from the frame. **A slot is an ordered sequence of frames, so one QUERY does not necessarily return a whole chain**, and the resolver pages the slot by the walk §9.7.4.2 R9 states, re-issuing QUERY with `since` set to the `stored_at` of the last frame it accepted. `N` bounds one page and bounds no slot; a chain carries no ceiling on the keys it installs and none on its sequence (§9.7.4.2 R9), so a long chain spans far more frames than `N`. Against non-validating or foreign storage `limit: N` also lets the resolver retrieve up to N candidates and sift them by R2, R3, and R6 (§3.10.4 step 5). See §3.10.2 for why `limit: N` dominates `limit: 1`.
...
**Retention, not expiry.** A key-event record carries no TTL. A validating relay retains it under §9.7.4.2 R9 and expires it never, which is what the `retain` flag on its PUT declares, so no republication cycle makes the record permanent. The chain's own order governs supersession (§3.10.7). The shared relay `blob_ttl` of §9.10.2 governs every other kind and reaches no record at the `scp:did:` kind: that TTL bounded a single small mutable record a controller republished cheaply, and carrying it onto an append-only chain would expire an identity's whole key history from every relay unless the controller re-uploaded the chain twice a fortnight.
...
**Eight gates read a `ContinuityStanding`, and the list above is the one home of every one of their conditions.** The eight are the Add gate, the UCAN gate, the sender-key-to-a-new-leaf gate, the broadcast per-author key cache, the standing-pair consent gate, the known-identity allowlist, the key response of §9.16.2 step 3, and the first-contact bootstrapping of §9.6.4. The five that also name the gate in the section that owns the act — `05-contexts.md` §5.14.2, `05-contexts.md` §5.15.8, `05-contexts.md` §5.12.2, §9.16.2 step 3, and §9.6.4 — cite the list above there and restate no condition of their own, so no section can drop a standing the list carries.
...
| `"SCP-COSIGNED-HEAD-V1:"` | Cosigned head — one witness's signed statement of the subject chain's head at one moment; it verifies against the P-256 key the signing operator's community-relay-list entry declares, under the third class of §9.7.1's classification table | §9.7.4.3 |
...
| `MAX_RETAINED_BYTES` | 64 MiB | The protocol ceiling on divergent-suffix bytes per identifier. A relay MAY configure a lower per-identifier value and MUST NOT configure a higher one; a lower value evicts only lower-ranked suffixes and changes no verdict. `MAX_RETAINED_SUFFIXES` is 8, one frame's `value` bound is 262,111 bytes, and the stated segment count is 32 frames per retained suffix, so one suffix's ceiling is 32 × 262,111 = 8,387,552 bytes, which is 8 MiB, and 8 × 8 MiB is 64 MiB | §9.7.4.2 R9's byte bound; the per-routing-id retention of `03-identity.md` §3.10.2 |
