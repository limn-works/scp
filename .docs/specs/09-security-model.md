...
| `"SCP-RENT-V1:"` | The `RENT` message's beneficiary signature — the signed statement by which the controller of the identity a rent payment credits names that identity, over its 32 raw digest bytes, the `units` the payment buys, and the SHA-256 digest of the receipt bytes. The signer is the key the key state at the head of the chain the relay holds lists `Current` in the `#active` role | §9.7.4.2 R9 |
