...
A party meeting an identifier for the first time resolves it under §9.7.4.2, which states how that party obtains the chain, recomputes the identifier under R2, derives the key state under R8, and reaches one of the six verdicts R14 defines. This section reads that verdict and assigns the identifier's first standing from it; §9.11's gate list states every condition a gate reads, and this section states none. The assignment is not a gate condition: a verifier that recorded `Verified` on a verdict under which it adopted no chain would fix a standing against a chain it never adopted, after which the key-change trigger of §9.11 could never fire for that identifier.
...
**The relay's declared write policy.** Alec settled the relay's storage rules across two days. On 2026-09-13 he ruled that a per-identifier byte ceiling misses the actor — "the risk is bad actors not theives" — because a party minting identities fills a relay under any per-identifier figure, so the protocol fixes a policy frame and never a figure, and he fixed eviction and denial together because "we need both eviction and denial". On 2026-09-14 he chose the ring buffer below as what a relay does when its storage fills: "ok ring buffer." `MAX_ACCEPTED_CHAIN_BYTES` is withdrawn and no per-chain byte ceiling stands.
...
**The one deferral.** What waits is display, URLs, and every signed structure that takes an identifier as UTF-8 bytes, which §9.5.2 enumerates. Because that form is unfixed, a writer prints no identifier in a prose example: an illustrative body writes `<scp-identifier:name>`, and a field whose encoding is this deferral writes `<the … identifier, in the textual form 09 §9.7.4.2 R13 defers>`. `25-test-vectors.md` §25.1 carries the one carve-out, for the fixture strings its vectors pin, and states the reason there.
...
**Clock model:** SCP does not require synchronized clocks. Timestamps are best-effort for message-ordering hints and replay detection. The paragraph below names the two readers that apply the clock-skew tolerance and says what each one decides.
