...
**Ruling (2026-08-25): Alec cut the custody-violation record and `CounterAttestation` from the protocol.** Issue #2422, a filed future idea for a general subject-side counter-attestation, records the cut and its date, and Alec closed that issue on 2026-08-31 with "keep CounterAttestation cut. The axe from 2026-08-25 stands". §27.1.3 of the attestations spec states the ruling in full and lists every artifact that still specifies either record. The two preimage tables below, the `violation_reference` derivation between them, and the two domain-separator rows §9.18.2 registers describe what `crates/scp-protocol/src/trust/custody_violation.rs` carries on `origin/main` today; a reader building a new implementation builds neither record, and a reader carrying out the removal reads them as the construction sites that come out.
...
4. **Trust score impact.** The equivocating relay's trust score (§9.3) is reduced. Members who operate relay infrastructure and whose relay is implicated in equivocation receive a `RelayEquivocationViolation` record in the `ViolationStore` (ADR-039). This violation is durable and affects the operator's trust score across all contexts where other members observe the violation record. **Ruling (2026-08-25): Alec cut the custody-violation record and `CounterAttestation` from the protocol, and `RelayEquivocationViolation` and the `ViolationStore` leave with them.** §27.1.3 of the attestations spec states the ruling in full and cites the issue that records it. No source file has ever defined `RelayEquivocationViolation`, and the removal that carries out the ruling takes `ViolationStore` out of the workspace, so the two sentences above name a record and a store this step cannot write to. What durable artifact carries the cross-context trust penalty in place of a violation record is a decision the ruling does not make; OQ-13 and OQ-42 of §27.7 of the attestations spec record it.
