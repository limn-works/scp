...
**Self-observation.** **The SDK MUST fetch its own identity's log from every entry of its fallback set** at a stated cadence, no less often than the witnessing interval the key state names and on every launch, and MUST treat an entry it could not reach as an alert. **The one publication duty this cadence carries: for every entry the fetch showed holding no record for this identity, the SDK re-publishes its chain and its service record to that entry.** **The fetch ranges over the set the duty acts on, because a fetch from one entry establishes nothing about the others**: a fetch from one chosen entry left the duty unfired wherever that entry still held the record, so an identity a flood had displaced everywhere else stayed absent at every other entry, R11's two-source first-contact floor was met at none of them, and the SDK's own check reported success. The cost is the per-entry fetch cost §9.18.17 derives, multiplied by the entries in the set, and it carries no publish at an entry that already holds the record. **There is no unconditional re-PUBLISH on every cadence**, because a re-PUBLISH of bytes the relay already holds moves nothing, for the reason R9's three write outcomes state, so it costs the relay an ingest slot and the publisher a rate-limit allowance and buys the identity nothing. The fetch establishes what the controller is standing on, and the conditional re-publication is what restores a record the ring displaced. **Where satisfying that publication duty at an entry would cost money, the SDK surfaces the condition to the controller and mints no receipt**: an entry's claim that it holds no record is a claim the resolver did not prove, so an SDK that minted on it would let any charging entry bill a controller once a cadence by answering every self-check with an empty response, which `19-economic-governance.md` §19.14's invariant that agents never silently incur costs forbids. That cadence is also the submission cadence of §9.7.4.3.
...
pub struct Recovery { /* persistence slot, custody slot, identity backend */ }

impl Recovery {
    pub fn new(
        persistence: StorageSlot,
    ...
    pub fn resume_recovery(&self, handle: RecoveryHandle)
        -> Result<RecoveryCompletion, IdentityError>;
