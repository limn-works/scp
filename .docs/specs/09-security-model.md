...
**The relay spends the rail-bound receipt identifier at the check that commits the message's effect** — the placement of a frame's events on a chain, or the crediting of a rent quota and expiry — so a message the relay rejects costs the payer no receipt. **The relay holds that spent set durably**, because a restart that lost it would let one settled payment buy a second write. **What the spent set does, stated exactly: it stops one settled payment being spent twice at one relay, and it decides nothing about which party's first spend counts.** A settled payment on a public rail is presentable by any party that reads that rail until it is spent, and the relay spends it for whichever message arrives first. On the rent path the signature the `RENT` paragraph below requires binds the credit to the identity whose controller signed for it. **On the write path no field binds a receipt to a presenting party, and the residue that leaves is one write's fee taken from the payer**, which this rule states rather than repairs, because a PUBLISH carries no key. **Nothing on the relay's path verifies the payer's signature over the receipt**, because that preimage waits on the identifier's textual form and because a relay resolving `payer_did` would resolve a key the presenting party chose; the receipt's signed payload therefore joins §9.5.2's register of preimages that wait on that form.
...
**`RENT` names its beneficiary under a signature.** **The message carries the identifier the rent is credited to, the `units` the payment buys, the `PaymentReceipt` bytes, and a signature over `SHA-256("SCP-RENT-V1:" ‖ identifier ‖ units ‖ SHA-256(payment_receipt))` by the key the key state at the head of the chain that relay holds for that identifier lists `Current` in the `#active` role.** **A relay refuses a `RENT` that carries no signature, one that does not verify against that key, or one naming an identifier it holds no chain for.** That signature is what binds a settled payment to the identity it credits: the receipt checks above read the terms the rail returned and read no party's identity, so without it any party that watches a public rail for a transfer to a relay's declared payee sends one message and takes the rent the payer bought, leaving the payer refused for a spent receipt identifier. **The payer and the beneficiary are separate parties by construction**: the payer mints the receipt and the beneficiary's controller signs the message, so a sponsor may rent for an identity it hosts and no party credits an identity whose controller did not ask for the credit. **A relay holds rent state only for an identity whose chain it holds**, which is what the signature check already establishes, so no party writes durable per-identity state at a relay for an identifier the relay never stored.

**The relay runs its checks on a `RENT` in this order**: the declared rate limit, which counts the message and reads nothing else; the structural decode of the message's fields; the signature check above, which reads the chain the relay already holds; then `PaymentAdapter::verify` with the three comparisons against its return. **Every check a relay can run before it decides whether to spend an outbound query runs ahead of that call**, which is the same universal the PUBLISH order of `03-identity.md` §3.10.2 carries. The relay then credits the identity's quota and rent expiry under the one arithmetic the coverage paragraph above states, **spends the rail-bound receipt identifier at that same crediting step**, and answers with that identity's rent state after the payment, so a controller learns what it now holds without a second round trip. **A `RENT` the relay refuses costs the payer no receipt.** **A refused `RENT` answers `StorageBudgetExceeded{Payment, value}`, whose value carries that entry's rent terms**, so the one object a controller holds after a refusal states the price that would satisfy it. **A `RENT` whose `payment_receipt` is absent credits nothing and answers with the identity's current rent state**, which is how a controller reads whether it is covered without paying for the answer.
