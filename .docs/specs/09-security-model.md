...
| Agent action rate limit | Absent (no cap) | A count in [1, u32 max]; §4.9.3 states no window length and no range for one | `ContextParams::agent_rate_limit`. Caps actions signed by `#agent` per member within a rolling window; an `#active`-signed action does not count against it. A rate limit needs a count and a window, and §4.9.3 states only that the window rolls. No struct declares the field; story SCP-AB-030 of `.docs/prds/agent-binding.json` adds it. | §4.9.3 |
| Agent co-signature requirement | Empty list | Any subset of the entries the context's ceiling declares, each named at most once; §4.9.3 tests an entry against the ceiling's declared entries rather than against §5.3.1 and §5.3.1.1 wildcard coverage, which is what holds the list to one entry per capability the ceiling declares | `ContextParams::agent_cosign_required`. Capabilities a member exercising them through `#agent` must additionally have signed with `#active`. No struct declares the field; story SCP-AB-031 of `.docs/prds/agent-binding.json` adds it. | §4.9.3 |
