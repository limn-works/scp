...
**A declared `Identifier` exemption stops the refusal arm of the per-identifier budget and stops no eviction**, for the identity whose rent is current, for as long as that rent is current, and up to the quota that rent bought and no further. **It exempts a covered identity's covered bytes alone**: the bytes the coverage test counts for that identity, measured against the quota its rent bought. **That identity's divergent suffixes stay evictable in the eviction-rank order below whatever its coverage**, because the coverage test counts none of their bytes, so an exemption over them would shelter bytes nobody paid for.
...
**`RENT` names its beneficiary under a signature.** **The message carries the identifier the rent is credited to, the `units` the payment buys, the `PaymentReceipt` bytes where the message pays, the instant past which the signer will not have the message honoured, an optional chain segment (ADR-004, the SCP native relay protocol), and a signature over six operands.**

<!-- scp:fragment id="rent-preimage-operands" -->
**The `RENT` signature preimage is `SHA-256("SCP-RENT-V1:" ‖ identifier ‖ BE32(units) ‖ receipt_digest ‖ operator_key ‖ BE64(not_after_seconds))`, and each operand is stated here so two implementations compose one preimage.** `identifier` is the 32 raw digest bytes. `BE32(units)` is the count as a four-byte big-endian integer, at least 1 on a message carrying a receipt and 0 on a message carrying none. **`receipt_digest` is `SHA-256(payment_receipt)` where the receipt is present and the 32 bytes `SHA-256(0x00)` where it is absent**, under §9.5.1's rule for an absent fixed-length optional field. `operator_key` is the 33-byte SEC1 compressed operator key the addressed entry's community-relay-list entry declares, which the controller reads from that list before it sends and which the relay already holds as the key it proves control of. `BE64(not_after_seconds)` is a Unix-seconds instant the signer chooses.
<!-- scp:end id="rent-preimage-operands" -->

**The signature is by the key the key state at the head of the accepted chain lists `Current` in the `#active` role**, which at a validating relay is the chain the routing id's first binding-valid frame established, so a routing id holding two chains resolves to one key rather than to whichever chain a reader picks. The `operator_key` operand is what binds one signature to one relay, so a message a controller signed for one entry buys nothing at a second entry, and `not_after` is what stops a party replaying a controller's earlier message against a relay that has not spent its receipt. **A relay refuses a `RENT` that carries no signature, one that does not verify against that key, one whose `not_after` its own clock has passed or which names an instant further ahead of its own clock than §9.14's clock-skew tolerance admits, or one naming an identifier it holds neither a chain nor a rent record for.** **Each of those four refusals answers with a wire code and an error variant of its own, and none of the four answers with the price refusal's**, because no payment satisfies any of them. **A relay holding a rent record and no chain for an identifier answers a receipt-absent `RENT` with the rent state it holds**, which is the state a displacement leaves standing and the one window in which a controller most needs to read it. **A relay writes a rent record only for an identity whose chain it holds**, which is what the signature check establishes, so no party writes durable per-identity state at a relay for an identifier the relay never stored, and it keeps a record it already wrote through a displacement.
...
| Per-identifier storage budget, total storage budget | Absent for the per-identifier term, which turns it off; absent for the total term, which is the retained capacity the operator's own host holds | Any value the operator sets; zero turns a term off | <!-- scp:include id="total-scope" from=".docs/specs/09-security-model.md" -->**`Total` refuses a write where displacing every uncovered identity the relay may displace still leaves the retained capacity short of the arriving frame's bytes, and refuses no write while one such identity remains**, because the ring below displaces the oldest-established uncovered identity instead. That one refusal covers the write that carried a receipt and the write that carried none, and it returns `StorageBudgetExceeded{Total, value}` carrying the declared total budget, which is the figure the `POLICY` query returns and the figure that states what would satisfy the refusal.<!-- scp:end id="total-scope" --> | §9.7.4.2 R9; §18.3.3 |
| `receipt_exempts` | Absent, which exempts no scope | The scope names §9.7.4.2 R9 permits a relay to declare | <!-- scp:include id="receipt-exempts-declaration" from=".docs/specs/09-security-model.md" -->**A relay that charges declares which refusal scopes a current rent exempts an identity from.** `relay_config` carries `receipt_exempts`, a set whose members are scope names, and ADR-004's `POLICY` query returns it, so a publisher reads what its money buys before it spends. **A relay that charges and declares nothing exempts nothing**: an absent or empty `receipt_exempts` exempts no scope, and an implementer reads that declaration rather than assuming a set. **`Identifier` is the one member a relay may declare.**<!-- scp:end id="receipt-exempts-declaration" --> | §9.7.4.2 R9; §18.3.3 |
| Rent price, rent period, rent quota | Absent, and §9.7.4.2 R9 states what an absent set of the three means | Any values the operator sets | <!-- scp:include id="rent-arithmetic" from=".docs/specs/09-security-model.md" -->**One verified rent payment of `units` sets the identity's quota to the greater of its standing quota and `units` times the declared quota, and sets its rent expiry to one declared period past the later of its standing expiry and the instant the relay verified the payment.** **The declared period is the duration one rent payment buys, whatever `units` that payment names**: the arithmetic multiplies the declared quota and the declared price by `units` and adds the period once.<!-- scp:end id="rent-arithmetic" --> | §9.7.4.2 R9; §18.3.3 |
