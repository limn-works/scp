...
**Storage split:**
- Device attestation, social attestation and endorsements live in the DID document. The owner publishes them; peers verify the cryptographic proofs rather than the owner's word. §18.2.2B criterion 1 states the rule this split follows and governs the boundary case: an entry belongs in the document when a reader can check it without trusting the owner, which each of these three satisfies through the signature of whoever controls the fact. Endorsements qualify on the endorser's signature, so the table's "Self-asserted? No" column records who produced the value rather than deciding where it lives.
- Protocol-derived signals (participation history, participation records, economic activity) live in context state. They are computed, not self-asserted, and a reader checks them only against the records the participating contexts hold. You publish your credentials; the network records your behavior. The DID document MAY carry a pointer naming where a verifier fetches them (§18.2.2B criterion 1), which is how `ParticipationStatements` works.
...
**Stale document prevention:** BEP44 records include a sequence number. The client MUST reject a DID record whose sequence number is lower than the last number that client observed for the same DID **on the layer that served the record**. This prevents a layer from serving an outdated record. The comparison is per layer because the relay layer and the Mainline layer sign different payloads and advance separate counters (§3.10.5, §3.10.7); a number from one layer decides nothing about a record from the other.
...
**Relay-stored DID documents.** Self-certification applies equally to the full DID document stored on SCP relays (§3.10.2). Each layer's record carries its own BEP44 signature over its own encoding, and the client verifies each against the public key encoded in the DID string. The storage backend does not affect the trust model, and neither does the encoding: what the client trusts is the signature under the DID-derived key, so it verifies a relay-served JSON document and a Mainline-served DNS packet by the same rule and never by comparing one against the other (§18.2.2A).
...
It is the **attestation** that is verified against the DID document — never the leaf signature itself. **Fail-closed scope (positive whitelist).** Attestation verification is MANDATORY and fail-closed for **all DIDs**: a leaf whose attestation is absent, malformed, expired, or failing any check above MUST be rejected. This applies to **every** production DID method without exception — `did:dht:*` **and** `did:web:*` (§9.6.2) are both in scope; enforcement MUST NOT be keyed on a single method prefix such as `did:dht:`, which would let a `did:web`-claiming leaf bypass verification. The **only** exemption is a narrow testing carve-out — mirroring the existing creator-identity validation carve-out for non-resolvable test DIDs, and **gated behind the `testing` feature so it is never present in shipped production artifacts** — which permits the `did:test:` and `did:key:` prefixes to skip DID-document resolution. No production DID method is exempt.

**A resolution that returns only the bootstrap core is a third state, and it is not a failure.** §3.10.10 gives a resolution two shapes: `Full`, which a relay served and which carries every entry the identity published, and `BootstrapCore`, which Mainline served and which carries `#active`, the pre-rotation commitment and the relay list alone (§18.2.2C). Checks 1 and 2 above name the DID's "**current** `#active` or `#agent`", and the two named keys land differently:

- **`signing_key_id` names `#active`.** The bootstrap core carries it, so checks 1 and 2 run against the core exactly as they run against a full document, subject to the staleness rule below.
- **`signing_key_id` names `#agent`.** The core does not carry `#agent`, and its silence is **not** evidence that the identity has no agent key (§3.10.10). A verifier MUST NOT read a `BootstrapCore` result as a resolution success that shows `#agent` rotated away — that reading would reject a valid attestation on the strength of an entry the layer never carries, which is the failure mode §3.10.10's two-variant type exists to prevent. The verifier resolves the relay layer for the full document, using the relay list the core just gave it. If no relay answers, the verifier has **not resolved** the signer's document for the purpose of these checks, and the failure policy below applies unchanged: fail-closed on an Add, bounded last-known-good grace on an already-admitted member's Update.

**A `Stale` bootstrap core does not satisfy check 2.** §3.10.4 step 5e marks a Mainline record stale when its timestamp sequence number is more than 2 hours 30 minutes behind the verifier's clock. Check 2 requires a document no older than `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (300 seconds), and a record the resolver has marked stale by a far wider bound cannot satisfy the tighter one. The verifier resolves the relay layer, which §3.10.4 step 5d makes authoritative whenever a relay answers, and falls to the policy below if none does.

**Resolution failure policy — Add is fail-closed; an already-admitted member's Update gets a bounded last-known-good grace.** When the verifier cannot resolve the signer's DID document — DHT timeout, unreachable authority, `did:web` HTTPS/TLS failure, a `BootstrapCore` result for an `#agent` signer with no relay answering, or any other resolution error — the policy depends on whether the leaf's DID is a **new member** (Add) or an **already-admitted member** (Update / their own leaf-changing Commit), because the availability trade-off is opposite in the two cases. In **both** cases a resolution *success* takes precedence: a document that resolves and shows the attesting `#active`/`#agent` **rotated away** fails check 1 and rejects — the grace below is for resolution *failure* only, never for a successful resolution that returns a new key.
...
**Rollback-resistance assumption.** The ≤ 5-min freshness guarantee (check 2) presupposes **rollback-resistant DID resolution**. An attacker who serves an **OLD but validly-signed, lower-`seq`** DID document as if it were a "fresh resolution" would return a **pre-rotation** key that the freshness check cannot catch — the document is genuinely signed, only stale, so it passes as fresh. This is out of scope for the attestation verifier; it is mitigated at the **resolution layer** — did:dht BEP44 `seq` monotonicity (§9.6.1: reject any record whose sequence number is lower than the last number observed on the layer that served it) and did:web fetch integrity (§9.6.2) — which the attestation verifier **assumes**, rather than re-checking as a separate attestation check. Because monotonicity is per layer (§3.10.7), that assumption is a per-layer assumption: a verifier that resolved `#active` from the Mainline bootstrap core inherits Mainline's rollback resistance for that key, and a verifier that resolved it from a relay inherits the relay layer's. Neither layer's counter covers the other, so a deployment that publishes to only one layer narrows this mitigation to that layer alone.
...
- Active Signing Key: Rotated via `rotate_active_key` (ADR-003 §4a). DID document is updated with the new verification method, signed by the Identity Key, and republished to BOTH layers in their own encodings (§3.10.5) — each advancing its own sequence number, Mainline's being a Unix timestamp fixed by the did:dht method (§18.2.2C). All active MLS groups receive an Update proposal with the new credential. The DID string does NOT change.
...
3. **Aggressive caching:** 24-hour refresh for active contacts, 7-day for inactive. The cache is keyed per (DID, layer), and a stale record is detected by comparing its sequence number against the cached number **for the same layer** (§3.10.7). Key change alerts trigger immediate re-resolution.
...
- **Active Signing Key compromise (common case):** Call `rotate_active_key` (ADR-003 §4a). Generate new active signing keypair, update the DID document signed by the Identity Key, and publish to both layers in their own encodings (§3.10.5). The DID string does NOT change. No identity migration needed.
