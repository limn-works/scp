...
Every field is fixed width, so the object is 136 preimage bytes and one 64-byte signature, 200 bytes in all. This object's `value_digest` covers a response's blob bytes. R9 names the frame operand a payment receipt binds `frame_value_digest`, so an SDK computing both reads one name for each preimage.
...
**The relay's declared write policy.** The protocol fixes each term's shape and never its figure, and a relay whose storage fills displaces under the ring buffer below rather than refusing. ADR-063, the inception-derived key-event-log identity substrate, carries the two rulings that settled both and carries the withdrawal of `MAX_ACCEPTED_CHAIN_BYTES`, so no per-chain byte ceiling stands.

A validating relay declares its write policy in `relay_config` (`18-addressability-and-deployment.md` §18.3.3): `rate_limit_publish`, a per-identifier storage budget, a total storage budget, and `per_publish` and `per_byte_stored` (`19-economic-governance.md` §19.8). A publisher reads that policy through ADR-004's `POLICY` query (§9.10.12). The relay refuses a key-event PUBLISH that fails a declared term with `IdentityError::StorageBudgetExceeded{scope, value}` (`03-identity.md` §3.10.10), whose four scopes are `Identifier`, `Total`, `RateLimit` and `Payment`, and it keeps serving every record it already holds. **`value` carries the declared term the refusal names — the budget figure, the rate, or the price — so the one object a publisher holds after a refusal states what would satisfy it.** `03-identity.md` §3.10.10 declares that field as the `BudgetValue` enumeration, which types it per scope, and the wire code `4041` carries the same field. **A relay that charges also declares which refusal scopes a verified receipt exempts.** `relay_config` carries `receipt_exempts`, a set whose members are scope names, and ADR-004's `POLICY` query returns it, so a publisher reads what its money buys before it spends. **A relay that charges and declares nothing exempts nothing**: an absent or empty `receipt_exempts` exempts no scope, and an implementer reads that declaration rather than assuming a set. **The two members a relay may declare are `Identifier` and `RateLimit`.** `Total` is not declarable, because the declared total budget is the whole capacity the operator's host holds and a relay stores nothing past it; `Payment` is not declarable, because the receipt is the object a `Payment` refusal reads. `18-addressability-and-deployment.md` §18.3.3 states the two permitted members, and a `relay_config` naming a third is invalid. **A declared `Identifier` exemption stops both arms of the per-identifier budget for a write this relay verified a receipt for** — the suffix eviction this paragraph states and the refusal that follows it — because both arms read the one term the exemption names. **A declared `RateLimit` exemption stops the relay counting such a write against that address's rate and never stops the rate-limit check itself**, so a party at or below the declared rate sustains a higher rate by paying and a party above it is refused at the first check whatever its PUBLISH carries. A relay that skipped the check would have to know a receipt is valid before it ran the check, which drives `PaymentAdapter::verify` ahead of the limit and hands an unauthenticated party one outbound rail query per request; `03-identity.md` §3.10.2 states the order and its reason. **The protocol fixes each term's shape and never its value.** A term declared zero is off, and an absent term is off at every term but one: an absent `rate_limit_publish` is the 6,000-per-minute default `18-addressability-and-deployment.md` §18.3.3 states, and a relay turns that term off by declaring zero. **A refusal is never a verdict**: the publisher's recourse is the next entry of its fallback set, and a publish cycle is confirmed when at least one entry accepts it rather than when one answers (`03-identity.md` §3.10.6). **No event kind is exempt from any scope.** A kind-keyed exemption let an identity's own holder write unbounded exempt permanent bytes by rolling over its own commitment, and the ring buffer below is what lets an owner's recovery land instead, because a free relay displaces an older identity rather than refusing the arrival. **Under `Identifier` pressure the relay evicts before it refuses:** it evicts that identifier's retained divergent suffixes in the eviction-rank order below, lowest-ranked first, and refuses only where evicting every suffix it may evict still leaves the per-identifier budget exceeded. **`Total` is the whole retained capacity a relay declares, and the identities the ring below holds covered and the identities that ring may displace are two partitions of that one figure**, so a relay stores no more than the ceiling its operator declared and no rule of this design names a second storage term. **`Total` refuses a write where displacing every uncovered identity the relay may displace still leaves the retained capacity short of the arriving frame's bytes, and refuses no write while one such identity remains**, because the ring below displaces the oldest-established uncovered identity instead. That one refusal covers the write that carried a receipt and the write that carried none, and it returns `StorageBudgetExceeded{Total, value}` carrying the declared total budget, which is the figure the `POLICY` query returns and the figure that states what would satisfy the refusal. **Where a relay declares no total budget the term is off and the relay runs no ring**: it displaces nothing, refuses nothing under `Total`, and answers a write its host storage cannot take with ADR-004's `5001` STORAGE_FULL, which names a relay out of room and names no declared ceiling, so a publisher reads it as a condition to retry or to route past rather than as a declared term it could satisfy. **No per-publisher scope exists**, because a PUBLISH carries no key (ADR-004, the SCP native relay protocol) and a minting party's signing key is fresh for every identity it mints.

**The ring buffer.** A relay's retained storage is a first-in-first-out ring of `Total` bytes, **and the unit it displaces is one identity: everything the relay holds for that identity, displaced together.** That is its chains at `scp:did:`, its service record at `scp:svc:`, and the `scp:wit:` and `scp:wcf:` records naming it as subject. The ring displaces no single frame and no single chain, so a chain the relay still holds never lacks its inception event and a fault proof's two cosigned heads are never separated by a displacement. **The ring is ordered by a monotonic per-relay counter and by no clock.** The relay increments one counter on each identity's first binding-valid frame and records that value as the identity's establishment position. The counter never decreases, so no correction to the host's wall clock inverts the order. Two identities never take one value, so the order is total and two relays holding identical identities displace the same one. The relay stores each position durably beside the records it orders, so a restart restores the order rather than leaving the relay with none. A later accepted write adds to what the relay holds for that identity and leaves its position unchanged, so no write refreshes anything and a quiet identity's position is the position its first write gave it. **An identity the ring displaced and which then re-publishes takes a fresh counter value, because the relay is establishing that identity again.** **When the ring is full, the relay displaces the oldest-established uncovered identity other than the identity whose write it is admitting, and takes every record it holds for that identity together. A relay never displaces the identity it is writing for.** Where displacing every uncovered identity it may displace still leaves the ring short of the arriving frame's bytes, the relay refuses that frame for `Total`, returning `StorageBudgetExceeded{Total, value}` carrying the declared total budget, so the publisher reads that this relay is too small for the chain it is publishing and moves to the next fallback-set entry. Without those two sentences a relay whose ring is smaller than one chain drops the segments it just accepted in order to store the segment whose predecessor those segments carried, the arriving frame then fails the write rule for a predecessor the relay no longer holds, and the publisher's re-send reaches the same state on every attempt. **A junk write at an identity's address displaces nothing, because the relay refuses it before it stores it**: relay-side validation and this rule's own test for what may open or extend a chain (`03-identity.md` §3.10.2) admit only a binding-valid frame. **A receipt covers the identity it names up to the bytes it paid for, and the excess beyond that is ordinary evictable data.** The identity a receipt names is the `identifier` of the frame its write binding binds, which the rent paragraph below states. **The relay holds, for each identity, the sum of the byte counts the receipts it verified for that identity paid for, and that identity is covered while the bytes the relay holds for it are at or below that sum.** What the relay holds for a covered identity counts against the declared total budget. **The ring displaces uncovered identities alone, and it never splits a partly covered one**: an identity holding more bytes than its receipts covered is an ordinary displacement candidate, and the ring displaces it whole in establishment order, taking every record the relay holds for it together, because a split would leave a chain without its inception event and would separate a fault proof's two cosigned heads. **An identity whose held bytes pass its covered sum keeps the establishment position it already holds**: the relay refuses no write on that ground and answers no distinct error, because no declared term refused it, and a controller keeps its identity covered by paying for the bytes it publishes at the price the `POLICY` query states. **Coverage against displacement is protocol and no relay declares it**: the `receipt_exempts` declaration above names refusal scopes, and a displacement is not a refusal, so a relay that charges and declares nothing still gives a verified receipt the coverage this paragraph states. **There is no idle clock, no refresh write and no pressure ordering**: no relay measures how long it has held a record, no party re-publishes a record to reset a clock, and the relay displaces by establishment order alone, except under the `Identifier` pressure the paragraph above states.

**What a displacement leaves each party to do.** A controller whose self-check finds its record absent at a fallback-set entry re-publishes its chain there (R10), which puts that identity at the newest position in the ring, and it re-publishes its service record with the chain, because the displacement took everything the relay held for it. A witness re-cosigns the subject's head on the next offer the controller makes it, which the self-observation cadence above drives, because a witness fetches nothing on its own account and cosigns nothing it was not offered (§9.7.4.3). That cosignature is an ordinary write against the subject at that relay. **A relay that holds no chain for a subject refuses every `scp:wit:` and `scp:wcf:` write naming that subject** (`03-identity.md` §3.10.2), so the controller's re-publication of its chain is what lets a witness record land there again, and a third party that keeps a conflict statement covered by paying re-publishes the subject's chain first. A conflict statement is held by the witness that wrote it and by the controller that fetched it, so a party needing one durable at a relay pays for the bytes that record occupies and keeps that identity covered. Every controller holds its full log (R10), so a displacement changes where a record is stored and destroys no record, and a controller that lost the device holding that log has lost the log.

**The one attack the ring does not bound.** A party minting distinct identities displaces every uncovered identity at a free relay by writing binding-valid frames at that relay's full ingest rate, for as long as it wants honest records absent. The per-address rate limit above bounds the rate, and no rule of this protocol bounds the duration. That is the bandwidth attack every free public service faces. What answers it is free capacity on the list under distinct operators, or paid entries (`19-economic-governance.md` §19.8); who curates the shipped list and what obligations that curator carries are open (`18-addressability-and-deployment.md` §18.5.1).

**Rent.** A PUBLISH carries an optional `payment_receipt` (§9.10.12): the `PaymentReceipt` bytes of `19-economic-governance.md` §19.2.1, tagged with the adapter that issued them. A relay declaring `per_publish` or `per_byte_stored` verifies that receipt through the named adapter, which is `PaymentAdapter::verify` of §19.2.1 — "relays can already charge rent." **"Paid" means "carried a receipt this relay verified and bound to this write" in every storage rule of this section, and "covered" means what the ring-buffer paragraph above states over an identity**, so the `Payment` scope reads a field the wire carries and the ring reads a byte sum the relay holds. **The payment receipt names one write at one relay, and five checks decide it, each one reading a value the presenting party did not choose.** `PaymentAdapter::verify` returns the terms it verified against the rail in a `VerificationResult` (`19-economic-governance.md` §19.2.1), and the relay's checks read those returned values rather than the receipt's own fields. The relay applies them in this order: the **verified payee** equals this relay's declared `economic.payee`; the receipt's **write binding** equals `SHA-256(payee_identifier ‖ frame_value_digest)`, where `payee_identifier` is that declared payee as its 32 raw digest bytes and `frame_value_digest` is `SHA-256` over the `value` bytes this PUBLISH carries, §9.10.12's layout fixing `value` as the frame's trailing remainder; the **rail-bound receipt identifier** is one this relay has not already spent, and the relay spends it at the check that places the frame's events on a chain and at no earlier check, so a frame the relay rejects costs the publisher no receipt; the **verified amount** covers the declared price for this write's byte count; and the **verified currency** equals the relay's declared `economic.currency`, because two rails denominate one integer in two monies and an amount compared without its currency compares nothing. The relay refuses `Payment` where the PUBLISH carried no receipt, where the verification failed, or where any of those five checks fails. **Three of them are the bindings**: the payee term binds the receipt to this relay, the write binding binds it to these bytes, and the unspent identifier binds it to one use. **The operand is named `frame_value_digest` and never `value_digest`**, because the relay proof's 200-byte layout carries a `value_digest` taken over a response's blob bytes and one SDK computes both. **Nothing on the relay's path authenticates a field the publisher composed**: the payer's P-256 signature over the receipt covers `write_binding`, and no relay verifies it, because that preimage waits on the identifier's textual form and because a relay resolving `payer_did` would resolve a key the presenting party chose. **The receipt's signed payload therefore joins §9.5.2's register of preimages that wait on the identifier's textual form**, and that is the residue this rule leaves. **`economic.payee` is the relay identifier every charging relay declares** and every publisher reads through the `POLICY` query, so a self-hosted relay that charges has one and a relay that charges nothing needs none. **`03-identity.md` §3.10.2 orders every check that reads bytes the relay already holds ahead of `PaymentAdapter::verify`, the one call that leaves the relay's host, and states that reason once.** **The declared-write-policy paragraph above states the declaration a charging relay makes and the two scopes it may name, and the ring-buffer paragraph above states the coverage against displacement a verified receipt gives.** **The byte count this receipt paid for is the figure that covered sum reads**, because the verified amount covers the declared price for this write's byte count, so the bytes this write occupies are the bytes this receipt paid for. **It buys no exemption from `Total`: covered bytes count against the declared total budget**, and the declared-write-policy paragraph above states when `Total` refuses, so the ceiling an operator declared bounds what that operator stores against a funded party. The blob-TTL clause of `19-economic-governance.md` §19.8 reaches no retained kind.
...
**This section declares the ceremony's two types and its three entry points, every field and every parameter carrying a type, and `03-identity.md` §3.10.10's `IdentityBackend` carries none of the three.** A resume consumes the composed preimage, the next commitment list and the next threshold, so the handle carries each of them: `largeBlob` holds a 65-byte marker alone, as the paragraph above states, and the rest rides in the handle the SDK's own store holds.
...
pub enum RecoveryHandle {
    FixesCommitment {
        identifier: [u8; 32],
        phase: RecoveryPhase,
        event_preimage: Vec<u8>,
        event_digest: [u8; 32],
        next_commitments: Vec<[u8; 32]>,
        next_threshold: u8,
        signed_event: Option<Vec<u8>>,
    },
    Abandons {
        identifier: [u8; 32],
        phase: RecoveryPhase,
...
/// The three stores the credential-keyed set above names.
...
**What a relay's ring buffer leaves each party to do.** A relay displaces everything it holds for one identity together, the `scp:wit:` and `scp:wcf:` records included (§9.7.4.2 R9). A witness whose cosigned head a relay displaced re-cosigns the subject's head on the next offer that subject's controller makes it, which is an ordinary write against that subject at that relay and carries no new duty. R9 states what a relay holding no chain for that subject does with the write. **A conflict statement is held by the witness that wrote it and by the controller that fetched it**, so a party that needs one durable at a relay pays for the bytes that record occupies, which keeps that identity covered under §9.7.4.2 R9.
...
**The wire change this section makes to ADR-004** is five items: `retain` and `payment_receipt` on PUBLISH, `proof_nonce` on QUERY, `relay_proof` on BLOB, and the `POLICY` query a publisher reads the relay's declared write policy through. ADR-004 carries all five.
...
| `MAX_RETAINED_BYTES` | 512 MiB | The product of three registered constants: `MAX_RETAINED_SUFFIXES` (8) × `MAX_EVENTS_PER_SUFFIX_RETAINED` (1,024) × `MAX_EVENT_BYTES` (65,536) = 536,870,912 bytes | §9.7.4.2 R9's byte bound, which measures what a party **retains** and counts divergent-suffix bytes per identifier; §9.7.4.2 R9's fetch bound, which measures what a verifier **downloads** for one identifier from one source in one resolution and counts every byte it read; the per-routing-id retention of `03-identity.md` §3.10.2 |
...
|-----------|---------|-------|-----------|----------------|
| `rate_limit_publish` | 6,000/min, the default `18-addressability-and-deployment.md` §18.3.3 states | Any value the operator sets; zero turns the term off | The relay declares the term in `relay_config` and refuses a key-event PUBLISH that exceeds it with `StorageBudgetExceeded{scope: RateLimit, value}`, whose `value` carries the rate | §9.7.4.2 R9; §18.3.3 |
| Per-identifier storage budget, total storage budget | Off (absent) | Any value the operator sets; zero or absent means the term is off | The relay declares each term in `relay_config` and refuses a key-event PUBLISH that fails it with `StorageBudgetExceeded{scope, value}`, whose `value` carries the declared figure. The protocol fixes each term's shape and never its value, and no event kind is exempt from either scope: under `Identifier` pressure the relay evicts by R9's eviction rank before it refuses, and `Total` refuses a write where displacing every uncovered identity the relay may displace still leaves the retained capacity short of the arriving frame's bytes | §9.7.4.2 R9; §18.3.3 |
