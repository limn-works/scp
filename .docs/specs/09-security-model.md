...
Changing any field's encoding, adding a field, or removing a field requires incrementing that separator's version suffix, and every signature made under the old separator becomes invalid. Three rules of the key-event log rest on that cost, and the definitions of §9.7.4.2 state each one: the key-algorithm discriminator travels beside every key from the first schema version, the key state carries no platform proof, and the key-event seal ships with no live reader. The rule that the key state carries no relay endpoint rests on where the retired document's contents went instead, which §18.2.2A of `18-addressability-and-deployment.md` records.
...
A subscriber resolves the `#active` fragment under R8's content arm of §9.7.4.2 and verifies the signature against the key that arm selects.
...
Note: the `KeyPackageAttestation` binds **all four** of the leaf's own public keys to the member's `did` (field 1), and is signed by the key the role named in `signing_key_id` (field 6, `#active` — never a root member). The attestation must vouch for the *whole* leaf, not a subset of its keys — each of the leaf's HPKE public keys is a distinct decryption capability, and any key left unbound is one an attacker who holds only the leaf **signing** key can substitute with a key of their own:
...
**Content verification returns `ContentVerdict`.** Its three variants are `Valid`, `Unverified{reason}` and `Invalid{cause}`, whose two causes are `no_prefork_basis` and `no_boundary`, defined by §9.7.4.2 R7 and by `03-identity.md` §3.2.1 step 5 respectively. `UnverifiedReason` names the first payload type and `ContentInvalidCause` the second, and its two values are `NoFreshKeyState` — the verifier holds the artifact and has resolved no key state fresh enough to test it — and `ResolutionInconclusive{cause: InconclusiveCause}`, the resolution having returned `Inconclusive`. **Every SDK binding carries the type name and every variant name unchanged.** An `Invalid` payload here is a verdict about one artifact inside one context and never about a chain, so a resolution never returns this type and content verification never returns R14's.
...
The independence test of item 3a reads operational custody and never the root, so it does not fire on the root-and-recovery co-residence, and item 3a's fail-closed rule governs the operational path alone. **The shipped consumer default does not satisfy item 3a's independence test by construction:** the root credential and the pre-rotation credential sit in one platform account reachable on the same device unlock that authorizes the operational signature. The SDK proceeds on the default, states the co-residence, and offers the two separations the paragraph above requires. The override rule of this item therefore rejects a selection whose recovery authority the profile's operational custody reaches, and never rejects the default.
...
| Two of the controller's devices each reveal the standing commitment | Rank 1 on both sides; terminal. R10's one-reveal-at-a-time rule is what stops it |
...
**A community-relay-list operator's identifier is non-transferable.** The operator's list entry carries the operator's P-256 public key, that key does not rotate, and an operator that changes its key takes a new list entry at the next release. A party verifying any object an operator signs reads the key from that operator's list entry and resolves no chain of the operator's own. **One operator key is one witness:** where one operator runs two listed relays under one key, every rule that names a witness names that operator once, and the two relays' cosigned heads are one witness's heads. KERI states the non-transferability rule in one sentence under `spec-body` §Backer list: "When the Backers are Witnesses, then the AIDs themselves MUST be non-transferable, fully qualified public keys… Consequently, the Witness does not need a KEL because its key state is fixed and is given by its AID."
...
**Head provenance.** How a party came to hold the head it adopted is a three-valued record named `HeadProvenance`, whose variants are `WitnessRead`, `TwoOperatorRead`, and `SingleRelay`, and every SDK binding carries those names unchanged. **A party assigns the record from the source it used for the head it adopted, and from nothing else**: `WitnessRead` where it adopted a head a witness cosigned, `TwoOperatorRead` where it adopted a head two list entries carrying distinct operator keys served, and `SingleRelay` otherwise. `TwoOperatorRead` counts operator keys and never declared operator names, which is R11's count and the non-transferable-operator rule above. The SDK records the value and surfaces it beside the standing. **No rule reads the record**, in this section or any other. A rule that withheld an act on the record would withhold that act on a property of the reading party's own network, and it would restore the missing-cosignature gate that Alec's watch-and-report ruling removed, under the name of a second source.
...
**A verifier resolves a `signing_key_id` fragment under one of two arms, and the artifact's own class selects the arm** (§9.7.1's classification table). **Log-anchored arm:** where the artifact carries `key_state_head` inside its signed preimage, the verifier replays the chain to the position that field names and reads, at that position, the key that held the named role and the condition it held there. **Content arm:** where the artifact carries no `key_state_head`, the verifier replays the adopted chain, takes every key that held the named role at any position, takes the one key among them whose bytes verify the signature, and then applies that key's condition and that context's boundary under §9.7.1's content rule. Neither arm resolves a fragment from the latest key state's current keys alone, which would reject every message an author signed before its last rotation, and the key state carries no set of every key that ever held a role for either arm to read.
...
6. **A witness that cosigns two divergent heads.** One witness releases two cosigned heads carrying one `witness` value, one `subject` value, one non-zero `previous_cosigned_digest`, and two different `event_digest` values, and that previous digest is not the digest of the event designating that witness for that subject. Those five conditions are the fault proof §9.7.4.3 defines, and a third party checks the fifth by fetching the witness's prior head from that subject's `scp:wit:` address.
...
**The authoritative gate list.** While an identifier's standing is `PendingReverify` or `Unresolved`, the SDK MUST NOT: add a new leaf for it to a context; issue a UCAN to it; distribute a sender key to a new leaf of it; grant anything to a leaf it replaced; auto-accept it at the standing-pair consent gate, the known-identity allowlist, or a cached `author_keys` entry; decrypt under an `author_keys` entry it already holds; or auto-accept an invitation, a standing-pair request, or a contact-graph promotion from it. `PendingReverify` withholds two further acts: accepting a UCAN that identifier issued, and accepting a UCAN carrying that identifier anywhere on its delegation chain. Alec approved the scope of the first three auto-accept gates on 2026-09-03. The two UCAN acts follow from a recovering controller being unable to enumerate the tokens an attacker issued, which makes refusal on the relying party's side the one act that stops them.
...
| `MAX_RETAINED_BYTES` | 512 MiB | The protocol ceiling on divergent-suffix bytes per identifier, the product of three registered constants: `MAX_RETAINED_SUFFIXES` (8) × `MAX_EVENTS_PER_SUFFIX_RETAINED` (1,024) × `MAX_EVENT_BYTES` (65,536) = 536,870,912 bytes | §9.7.4.2 R9's byte bound; the per-routing-id retention of `03-identity.md` §3.10.2 |
...
| `MAX_EVENTS_PER_SUFFIX_RETAINED` | 1,024 | The ceiling on the events a party retains for one divergent suffix. A suffix records key changes and no identity makes a thousand of them, so a longer suffix is one an attacker manufactured; a party that reaches the ceiling evicts that suffix under R9's eviction rule rather than refusing the arrival | §9.7.4.2 R9's retention bound; the derivation of `MAX_RETAINED_BYTES` |
| `MAX_ACCEPTED_CHAIN_BYTES` | 2 GiB — **proposed 2026-09-11; awaiting Alec** | The per-identifier ceiling on accepted-chain bytes at a validating relay, and the figure a verifier MAY stop fetching at. Four times `MAX_RETAINED_BYTES`: the accepted chain is the one chain every party holds in full, and the divergent suffixes it retains beside that chain are capped at `MAX_RETAINED_BYTES` in total. The constant ships only if Alec takes the byte-cap shape of R9's accepted-chain proposal | §9.7.4.2 R9's accepted-chain bound; the per-routing-id retention of `03-identity.md` §3.10.2 |
| `MAX_SERVICE_RECORD_ENTRIES` | 64 | The ceiling on the entries one service record carries, and the ceiling on the concurrent queries a resolver opens against an identity's own relays. `18-addressability-and-deployment.md` §18.2.3 asks an identity for at least three relay entries, and 64 covers every honest configuration of relays, private-state relays, broadcast advertisements, capability URIs and the two pointers with margin, while a 262,111-byte frame would otherwise admit about five thousand entries pointing at one host | §9.7.4.2 R3's defect list; the record's entry encoding in `03-identity.md` §3.10.13; the resolver's query plan in `03-identity.md` §3.10.1 |
