...
**No relay is trusted.** The log authenticates the record under §9.7.4.2 R2 and R3, so a resolver's integrity guarantee never rests on which relay served the record. Freshness at first contact comes from the two distinctly-operated relays R11 requires; freshness afterwards comes from the accepted baseline R12 holds against a candidate and from the staleness bound the witness layer's cosigned heads carry, which §9.11 reads as a standing and no verdict reads at all (§9.7.4.2 definitions, §9.7.4.3).
...
- A cosigned head is **fresh** at a relying party when `now − observed_at ≤ WITNESS_STALENESS_INTERVALS × witnessing_interval` (§9.18.17), where `witnessing_interval` is the value the derived key state carries, and when `observed_at` sits no further in the future than §9.14's clock-skew tolerance. Freshness is a property of one cosigned head at one moment. **One rule outside this section reads it**: §9.11 sets `ContinuityStanding::Unresolved` for an identity whose adopted head carries no fresh cosigned head from an operator in the party's recognized set, which is that section's lapse standing. No rule in this section reads it, so a party derives the same key state and reaches the same rank whatever the age of the cosignatures it holds, and a party holding none reaches them too. **An identity that designates no witnesses produces no cosigned head**, so §9.11's lapse standing is the only rule that treats it differently from any other identity, and every rule of this section behaves for it exactly as it does for a witnessed one.
...
| finds its designated witnesses unreachable, silent, or refusing, so relying parties hold no fresh cosigned head for it | a `KeyState` naming a fresh witness set. No cosignature gates that event, so the replacement takes effect the moment relying parties adopt it; §9.7.4.3 states how the fresh witnesses reach a baseline |
...
*Self-observation.* The SDK MUST fetch its own identity's log at a stated cadence — no less often than the witnessing interval the key state names, and on every launch — from at least one relay in the fallback set (definitions), and MUST treat an inability to complete that check as an alert, not as silence. **On its own chain the controller reads freshness as every relying party does:** it fetches its own cosigned heads from the `scp:wit:` address §9.7.4.3 names and alerts the human while it holds no fresh cosigned head from an operator its own recognized set carries. That alert is what tells a controller its witnessing has lapsed, which is the state §9.11 records as `Unresolved` at every peer and which closes that section's admission and grant gate against the identity. The remedy is the witness-set replacement of the table above, and it is a `KeyState` that spends no commitment. The controller changes the witness set and the witnessing interval by a `KeyState` (R8), so neither value is fixed for the life of the identity.
...
5. **An identity's witnesses go silent.** The owner's designated witnesses stop cosigning. **The chain keeps working**: the owner publishes key events, every relying party verifies and adopts them under R1 through R4 and R12, and the key state derives exactly as before, because no rule of this section reads a cosignature. What changes is §9.11's standing: each relying party holds no fresh cosigned head, sets `ContinuityStanding::Unresolved` with stale witnessing as the cause, and closes that section's admission and grant gate while leaving UCANs the identity issued acceptable. The owner's remedy is a `KeyState` naming a fresh witness set (R10's table), which spends no commitment and takes effect the moment relying parties adopt it. The fresh witnesses hold no last-cosigned head for the subject, so §9.7.4.3's base case admits them and each cosigns from a verified chain.
...
1. **Freshness.** A cosigned head from a recognized operator bounds how stale the head that party holds may be. §9.11 reads that bound: an identity whose adopted head carries no fresh cosigned head (§9.7.4.2 definitions) takes `ContinuityStanding::Unresolved` with stale witnessing as the cause, which closes that section's admission and grant gate. **That is the only place in this corpus where a cosignature's presence or absence changes what a party does**, and it changes a standing rather than a verdict.
...
**A witness operator's own chain is resolved without recursion.** A party verifying a cosignature resolves that operator's chain under §9.7.4.2 R1 through R4, and under R6 and R7 where it holds two divergent chains for the operator; the trust anchor is the identifier the operator's community-relay-list entry declares. **The floor here is that entry plus one other list entry** under a different declared operator, and a party reaching only one reads no cosignature. Applying R11 unchanged would make counting the first source require resolving an operator's log, which would require two proven sources, each requiring another log; exempting the chain from every floor would let one relay serve it truncated before the `KeyState` that retired a stolen operator key.

**Which subjects a witness admits, and what it does at capacity.** A witness admits a subject when that subject's key state first designates it, in arrival order, to `MAX_SUBJECTS_PER_WITNESS` subjects. **It never evicts an admitted subject**, because eviction would put that subject into the no-baseline state at every witness under the same storage pressure. **A witness at capacity refuses and reports capacity as the reason**, so a controller designates a different operator rather than reading the refusal as a fork.

**What the SDK ships.** The SDK's default witness set for a new identity is the entries of the community relay list under distinct declared operators, capped at `MAX_WITNESS_SET_SIZE` (§9.18.17), at a witnessing interval of `DEFAULT_WITNESSING_INTERVAL` (§9.18.17). **An identity is usable from the moment its inception event is published**, because no rule reads a cosignature, so identity creation completes at publication and waits on no witness. **The SDK MUST warn a controller that replaces the default set with operators the shipped list does not carry**, and MUST state the consequence: such an identity produces no evidence any relying party reads, so §9.11 holds it at `ContinuityStanding::Unresolved` at every peer, which closes that section's admission and grant gate. A self-hoster's own relays therefore serve its transport while the community set serves its witnessing.

**Cosigning is free.** `18-addressability-and-deployment.md` §18.5.1 states that rule and this section reads it. Economic rules (`19-economic-governance.md`) may price a witness's cadence and its availability guarantees; they may never price the act.
...
4. **A silenced witness set costs the identity its §9.11 standing and nothing else.** Every peer moves to `ContinuityStanding::Unresolved`, which closes the admission and grant gate; the identity's key events keep taking effect, and the remedy is a `KeyState` naming a fresh set (§9.7.4.2 R10).
...
- **`Unresolved` means nothing was learned about the identifier, not that the identifier is suspect.** **Two causes set it, and each carries its own clearing condition.** The first is a resolution that reached too few sources, which R14 reports as `Inconclusive`; it clears on any later resolution that adopts a chain, with no human step, because a verifier that reached one relay learned nothing about the identity that a verifier reaching two would have contradicted. The second is an identity whose adopted head carries no fresh cosigned head from an operator this party's recognized set carries — a witnessing outage, a silent witness set, or an identity that designates no witnesses at all (§9.7.4.3). **That second cause reports no `Inconclusive` verdict of any kind**: no rule of §9.7.4.2 reads a cosignature, so a lapsed identity resolves normally and returns `Adopted` or `Confirmed`, and the party keeps deriving key state from the head it adopted (§9.7.4.2 R8). **The second cause clears only on a resolution whose adopted head carries a fresh cosigned head**, and never on the bare fact that a resolution adopted a chain, because every resolution of a lapsed identity adopts one and a clearing rule reading adoption alone would clear this standing in the same act that set it. The second cause reads the same way as the first, and for the same reason: a witnessing outage is no evidence against the tokens the identity issued, so withholding them would reach every token the victim ever issued on the strength of a relay outage.
...
| `WITNESS_STALENESS_INTERVALS` | 3 | Multiplier on the witnessing interval past which a cosigned head is no longer fresh (§9.7.4.2 definitions). A witness misses two consecutive rounds without the identity going stale, and a third miss is a signal rather than noise. **The one rule that reads freshness is §9.11's lapse standing**, so this constant decides when a peer closes its admission and grant gate against an identity and decides no verdict | §9.7.4.2 definitions, §9.11 |
...
| `DEFAULT_WITNESSING_INTERVAL` | 3600s (1h) | The witnessing interval the SDK writes into a new identity's inception event, identical in every binding of one release. It is a default and not a bound: R3 admits any value between the floor and the ceiling above. **The value is chosen against the lapse window it fixes**, `WITNESS_STALENESS_INTERVALS × DEFAULT_WITNESSING_INTERVAL` = 3 hours, after which §9.11 moves the identity to `ContinuityStanding::Unresolved` at every peer and closes that section's admission and grant gate. `MIN_WITNESSING_INTERVAL` would set that window at 15 minutes, which one relay restart exceeds, and `MAX_WITNESSING_INTERVAL` would set it at three weeks, which is longer than a peer should go without noticing that an identity stopped being watched | §9.7.4.3, §9.11 |
