...
Changing any field's encoding, adding a field, or removing a field requires incrementing that separator's version suffix, and every signature made under the old separator becomes invalid. Three rules of the key-event log rest on that cost, and the definitions of §9.7.4.2 state each one. The rule that the key state carries no relay endpoint rests on where the retired document's contents went instead, which §18.2.2A of `18-addressability-and-deployment.md` records.
...
|-----------|---------|-------|-----------|----------------|
| `rate_limit_publish` | 6,000/min, the default `18-addressability-and-deployment.md` §18.3.3 states | Any value the operator sets; zero turns the term off | The relay declares the term in `relay_config` and counts every PUBLISH and every `RENT` against the publishing address, refusing a message that exceeds it with `StorageBudgetExceeded{scope: RateLimit, value}`, whose `value` carries the ceiling and the seconds to wait. An operator that charges buys no party relief from this term, because §9.7.4.2 R9 admits no declaration that exempts one | §9.7.4.2 R9; §18.3.3 |
| Per-identifier storage budget, total storage budget | Absent for the per-identifier term, which turns it off; absent for the total term, which is the retained capacity the operator's own host holds | Any value the operator sets; zero turns a term off | The relay declares each term in `relay_config` and refuses a key-event PUBLISH that fails it with `StorageBudgetExceeded{scope, value}`, whose `value` carries the declared figure. §9.7.4.2 R9 states when each scope refuses and what the relay displaces or evicts first | §9.7.4.2 R9; §18.3.3 |
| `receipt_exempts` | Absent, which exempts no scope | The scope names §9.7.4.2 R9 permits a relay to declare | The relay declares the set in `relay_config` and ADR-004's `POLICY` query returns it, so a publisher reads which refusal a current rent lifts before it spends | §9.7.4.2 R9; §18.3.3 |
| Rent price, rent period, rent quota | Absent, and §9.7.4.2 R9 states what an absent set of the three means | Any values the operator sets | The relay declares the three terms in `relay_config`, and a controller pays rent against them on ADR-004's `RENT` operation. §9.7.4.2 R9 states what rent covers and when it lapses | §9.7.4.2 R9; §18.3.3 |
