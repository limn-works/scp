...
**One record form, and the SCP relay network carries it.** A DID record carries the log itself: the signed `value` of an identity's relay record carries the chain from the inception event to the latest state-carrying event, or — when the resolver already holds a state-carrying event on that chain — the segment from that event forward (§9.7.4.2 R9 states how a relay serves a chain too long for one frame). A publisher that cannot tell what a resolver holds publishes from inception, which is the form every first-contact resolution reads. The protocol runs no second publication or resolution layer beside the relay network, and it uses no distributed hash table.

**How a resolver authenticates a served record.** When resolving an identifier:

1. The client queries every relay it knows for the record at the identifier's routing ID: the relays the identity's key state lists, and the bootstrap relays of the fallback set (definitions; `18-addressability-and-deployment.md` §18.5.1). A client that holds no accepted baseline for the identifier obtains the chain from two independent relays under §9.7.4.2 R11's two-relay rule, at least one of them in the fallback set.
...
**The record signature is the write mechanism, never the resolver's trust decision.** A DID record carries an Ed25519 signature by the operational key the key state designates for the service record, over `SHA-256("SCP-DID-RECORD-V1:" || identifier || seq || value)` under the §9.5.1 construction — the 32-byte identifier raw, `seq` as a big-endian `u64`, and `value` under the variable-length rule — with the separator registered in §9.18.2. That signature and its `seq` serve one purpose: they let a relay decide which write to keep at a routing ID (§9.10.12, §3.10.2). A resolver's trust decision reads the log under steps 2 through 4 above and never the record signature.
...
**Attack: relay list substitution.** A compromised relay could serve a stale relay list, directing messages to relays the recipient no longer uses. Defense: a client takes the relay list from the latest state-carrying event of the chain it adopted (§9.7.4.2 R6, R8), and it discards a chain that is a head of the accepted chain at a strictly lower sequence without changing its accepted state (§9.7.4.2 R12).
...
**Resolution failure policy — Add is fail-closed; an already-admitted member's Update gets a bounded last-known-good grace.** When the verifier cannot resolve the signer's key-event log — every relay it reached timed out, it reached fewer relays than R11's two-relay rule requires, or any other resolution error — the policy depends on whether the leaf's DID is a **new member** (Add) or an **already-admitted member** (Update / their own leaf-changing Commit), because the availability trade-off is opposite in the two cases. In **both** cases a resolution *success* takes precedence: a document that resolves and shows the attesting `#active`/`#agent` **rotated away** fails check 1 and rejects — the grace below is for resolution *failure* only, never for a successful resolution that returns a new key.

- **New member (Add) — fail-closed, no stale fallback.** A resolution failure on an Add is a **REJECT** (fail-closed), never accept-if-uncertain, and the verifier **MUST NOT fall back to a stale or pre-rotation cached DID document**. Falling back would open a **rotation-bypass**: an attacker who can *induce* resolution failures (intermittently disrupting the reachability of the identity's relays) could pin verifiers onto a **pre-rotation** cached key state in which the retired `#active`/`#agent` still resolves, so old attestations continue to verify past a rotation — up to the cache TTL — defeating the revocation-by-rotation lever of §9.12. This is the cross-group **Add** path — the actual leaf-reuse threat (§9.7.3 "Scope of the PCS bound") — so it keeps the tight, fail-closed **≤ 5-min** current-key bound. Delaying a not-yet-member forks and censors nothing, so the correct availability posture is to **retry/queue the join**, not to admit on a possibly-retired key. The resolver cache is retained **only as a positive same-or-fresher optimization** for this current-key check (never as a fallback *for* a failed resolution), with a freshness TTL **no longer than `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (§9.18.7 — 300s / 5 min; check 2)**.
...
**Rollback-resistance assumption.** The ≤ 5-min freshness guarantee (check 2) presupposes **rollback-resistant DID resolution**. An attacker who serves an **OLD but validly-signed, lower-`seq`** chain as if it were a "fresh resolution" would return a **pre-rotation** key that the freshness check cannot catch — the document is genuinely signed, only stale, so it passes as fresh. This is out of scope for the attestation verifier; it is mitigated at the **resolution layer** — the sequence rule of §9.6.1, which rejects a head of the accepted chain at a lower sequence than the accepted head, together with chain verification, which rejects a chain that recomputes to another identifier — which the attestation verifier **assumes**, rather than re-checking as a separate attestation check.
...
- The **fallback set** for an identity is the set of relays that no snapshot on any suffix of that identity's chain names — every relay outside the relay list the chain carries. The SDK draws a fallback relay from the bootstrap relay list of `18-addressability-and-deployment.md` §18.5.1, taking any entry of that list the identity's own chain does not name. A controller reads its own chain from the fallback set (R10) and a verifier tries a fallback source before it returns `Inconclusive` (R11), because a source an attacker's `KeyState` could have introduced is a source the attacker chose.
...
5. **Residual: a relay in the fallback set sees first contacts it serves no traffic for.** A bootstrap relay that stores no chain of its own still learns which `routing_id` a resolver IP asked for. A resolver that requires IP anonymity uses a VPN or Tor at the transport layer, the same control §9.10.11 names for message traffic.
...
4. **Verifies the record signature** over `SHA-256("SCP-DID-RECORD-V1:" || identifier || seq || value)` against the **designated service key of the chain the `value` itself carries**, in every case — the operational key that chain's latest state-carrying event designates for the service record (§9.7.4.2 R3 rejects a state-carrying event whose designation names a key it does not list `current`). Step 3 has already verified that chain from its inception event, so the key is one the relay read out of a verified chain and never one the writer supplied; reading it out of the stored slot's chain instead would fail every genuine first frame after a `#active` rotation, whose new designated key the stored chain does not yet name. The signature and `seq` are the write rule and never a resolver's trust input (§9.6.1).
...
DID-RECORD (DidRecordV1) :=
  ...
  seq:              u64                      # 8 bytes big-endian — record sequence
  signature:        [u8; 64]                 # Ed25519 record signature, raw fixed-width, NO length prefix
  value:            [u8]                     # trailing remainder = signed payload = one contiguous segment of the key-event log the record projects (§9.6.1, §9.7.4.2 R9)
```
...
- **seq** / **signature** — the record's sequence and its §9.6.1 signature, written raw with no length prefix (fixed-width fields, §9.5.1).
...
**The frame never enters the signed bytes.** The frame wraps `value` for relay transport only; it never reorders or alters the bytes the record signature covers, so a relay verifies the same preimage the publisher signed. §3.10.4's rule that two valid records sharing a sequence number MUST be byte-identical is applied to two copies of one record.

**TTL.** DID records reuse the shared relay `blob_ttl` (§9.10.2), bounded by `Max blob TTL` = 604800s / 7d (§9.18.11). There is no record-specific TTL and no change to `Max blob TTL`. TTL governs storage lifetime; the record's `seq` governs supersession (§3.10.7), and a validating relay's single-slot rule enforces supersession on write (§3.10.2). Record permanence is achieved by republication on the 6-day cycle (§3.10.2), not by TTL.
...
4. Alice sends a **signed** block notification to Bob as an MLS application message: `{"type": "block", "blocker": "<Alice's identifier>", "blocked": "<Bob's identifier>", "signing_key_id": "#active", "timestamp": unix_ms, "signature": "<Ed25519>"}`. The signature covers the canonical hash `SHA-256("SCP-BLOCK-NOTIFICATION-V1:" || len(context_id) || context_id || len(blocker_did) || blocker_did || len(blocked_did) || blocked_did || len(signing_key_id) || signing_key_id || timestamp_BE)` — see the BlockNotification row in §9.5.2 for field order and encoding. `alice_signing_key` is Alice's `#active` (Active Signing Key) or `#agent` (Agent Signing Key) — either is valid (ADR-039); the `signing_key_id` field tells the verifier which DID document verification method to resolve. The signature prevents forgery — without it, any group member could impersonate Alice and trick Bob into rotating his sender key. MLS application messages prove group membership but not individual sender identity within the message payload.
...
| `"SCP-DID-RECORD-V1:"` | DID-record signature preimage — `SHA-256("SCP-DID-RECORD-V1:" \|\| identifier \|\| seq \|\| value)`, signed by the operational key the key state designates for the service record; it is a relay's write rule and never a resolver's trust input | §9.6.1, §9.10.12 |
