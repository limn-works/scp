...
A validating relay declares its write policy in `relay_config` (`18-addressability-and-deployment.md` §18.3.3): `rate_limit_publish`, a per-identifier storage budget, a total storage budget, `per_publish` and `per_byte_stored` (`19-economic-governance.md` §19.8), and the three rent terms the ring-buffer paragraph below states. A publisher reads that policy through ADR-004's `POLICY` query (§9.10.12), whose answer carries the `DeclaredWritePolicy` type `03-identity.md` §3.10.10 declares. The relay refuses a key-event PUBLISH that fails a declared term with `IdentityError::StorageBudgetExceeded{scope, value}` (`03-identity.md` §3.10.10), whose four scopes are `Identifier`, `Total`, `RateLimit` and `Payment`, and it keeps serving every record it already holds. **`value` carries the declared term the refusal names — the budget figure, the rate, or the price — so the one object a publisher holds after a refusal states what would satisfy it.** `03-identity.md` §3.10.10 declares that field as the `BudgetValue` enumeration, which types it per scope, and the wire code `4041` carries the same field.
...
**Coverage is rent.** Alec ruled on 2026-09-14: "it's rent. as long as you keep paying. operator says, \"pay x at y frequency, get quota a of b thing\"." **A relay that sells coverage declares three terms in `relay_config` — a rent price, a rent period, and the quota of stored bytes one rent payment covers — and an identity is covered up to that quota while its rent is current.** **One verified rent payment of `units` sets the identity's quota to `units` times the declared quota and sets its rent expiry to the later of its standing expiry and the instant the relay verified the payment, plus one declared period.** That one arithmetic answers three questions. A controller raises its quota by naming a larger `units` on its next payment, which is the reachable act the `Identifier` refusal row of `03-identity.md` §3.10.6 sends it to. A controller that renews early keeps the remainder of the term it already holds. A controller that renews late starts a fresh term from the moment of verification. **ADR-004's `RENT` operation carries `units` so the relay reads which quota the controller asked for, and the relay supplies `units` times the declared rent price as the price it verifies the receipt against.** **An identity whose rent lapses is uncovered from that moment and is an ordinary displacement candidate again**, so no payment buys coverage that outlives the period it paid for. **A relay that declares no rent terms sells no coverage, and every identity it holds is uncovered.** **Coverage is a cap on what rent protects and never a credit a write earns**: the relay reads, for each identity, the bytes it holds for that identity against that identity's current quota, and the identity is uncovered where the first figure exceeds the second or where the rent has lapsed. **The rent terms supersede `per_byte_stored` as what a controller pays for storage of a retained kind**, because that member is a one-time fee and a retained kind never expires; `per_byte_stored` keeps its meaning for a blob the relay expires under a TTL, and `per_publish` keeps its meaning as the fee for the publish operation and buys no coverage. **The relay records each identity's current quota and the instant its rent expires, both durably beside the records they cover**, and it reads its own clock to decide whether a rent term has elapsed. **It treats a term as elapsed only past that instant plus the clock-skew tolerance of §9.14, the clock and ordering model**, which is the same tolerance the relay proof of control and a witness's `observed_at` take, so a controller renewing inside that tolerance is never lapsed for a clock it cannot read. **That clock decides which partition of the displacement order below an identity falls in, and it orders nothing inside a partition**: the ring's order inside a partition is the monotonic counter above, and no relay measures how long it has held a record. **Two relays whose clocks differ therefore displace different identities where one relay reads a term as elapsed and the other does not**, which is a disagreement about an operator-declared term, and the clock-skew tolerance above is what bounds it.
