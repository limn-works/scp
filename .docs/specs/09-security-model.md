...
- **Agent Signing Key compromise (most common case):** The agent runtime is typically less secure than device HSM, making `#agent` the most likely key to be compromised. Recovery: (1) Human uses `#0` (Identity Key) to publish a new DID document that **removes the compromised `#agent` method from `verificationMethod` entirely**, then installs the replacement — remove first, add second. A `rotate_agent_key` retains the compromised key as `#retired-agent-{sequence}`, which §23.13 paragraph 1 of the sync spec accepts on an event-log leaf, so retiring an `#agent` key an attacker holds leaves that attacker signing content every honest verifier accepts. The order matters because `DidDocument::remove_agent_key` reaches `#agent` and not `#retired-agent-{n}`: rotating first moves the compromised key beyond what that operation removes. See **Hygiene rotation versus compromise removal** below. (2) Revoke all UCANs with `fct.scp_key_scope: "#agent"` — add to per-context `RevocationList` and distribute via MLS application messages. (3) Issue MLS Update proposals in all active contexts with new credentials (updated `signing_key_id`). (4) Publish fresh KeyPackages carrying attestations re-issued under the new `#agent` key (§9.7.1). The human's `#active` key, root UCANs, and Identity Key are unaffected — only agent-scoped material is rotated (ADR-039). This is the cheapest recovery scenario: no identity migration, no root UCAN reissuance.
- **Active Signing Key compromise (common case):** Generate a new active signing keypair, publish a DID document signed by the Identity Key that installs it as `#active` and **removes the compromised method from `verificationMethod` entirely**, and publish that document to the DHT. The DID string does NOT change. No identity migration needed. Removal, rather than the `rotate_active_key` retirement of ADR-003 §4a, is what this step requires: §4a retains the old key as `#retired-{sequence}`, and §23.13 paragraph 1 of the sync spec accepts a retained `#retired-{n}` method on an event-log leaf, so a compromised key an owner merely retired keeps signing content that verifies. See **Hygiene rotation versus compromise removal** below.
...
- **Leaf-key / MLS-state compromise (ephemeral leaf key leaked, identity keys intact):** An attacker who extracts a member's MLS leaf state holds the leaf `signature_key`/`encryption_key` and the standalone **KeyPackage attestation** over them (§9.7.1). Beyond in-group PCS (§9.7.3), the standalone attestation lets the attacker reuse the leaf to join **other** groups until it expires. Remediation is an **existing operation, not a new mechanism**: the victim rotates `#active` (or `#agent`, for an agent leaf) via a DID-document update signed by `#0` (ADR-003 §4a / ADR-039). A **rotation** is the correct act here, and this is the one bullet in this step where it is: the two bullets above recover a compromised **identity signing key** and therefore require removal, while a leaked *leaf* key leaves `#active` and `#agent` uncompromised. Rotation revokes what this bullet needs revoked — every outstanding KeyPackage attestation, which §9.7.1 check 1 binds to the current `#active`/`#agent` — and the retained `#retired-*` method keeps verifying the victim's own past event-log leaves, which is the outcome §23.13 paragraph 1 of the sync spec wants. This **invalidates every outstanding KeyPackage attestation within `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (§9.18.7 — 5 min, the hard current-key freshness bound of §9.7.1 check 2, NOT the 24h §9.10.7 privacy cache TTL)** — near-immediate, not instantaneous — because every one of them was signed by the now-retired verification method and verifiers resolve the DID's **current** `#active`/`#agent` only (§9.7.1 check 1), from a document no more than 5 minutes stale (§9.7.1 check 2); once the retired key no longer resolves (and no fresh-enough pre-rotation cache entry remains), every attestation signed by it fails verification everywhere. The victim then re-issues fresh KeyPackages/attestations under the new key (step 4) and issues MLS Updates in active contexts (step 2). **Containment property:** a leaked *leaf* key does NOT expose `#active`/`#agent` — separating the ephemeral, context-scoped leaf key from the identity signing keys is the entire point of the attestation model (§9.7.4), so this recovery is a **routine key rotation, not an identity-key exposure**: no `migrate_identity`, no pre-rotation-key consumption, no new DID. This is a **Tier-1 revocation**: the DID document already carries the current verification method, and rotating it is the sole, sufficient revocation act *for the attestations this bullet is about* — **no `attestations_valid_after` watermark or other new DID-document field is introduced** (a document-level revocation watermark was considered and rejected as over-engineering; rotation of an already-present key is a complete and cheaper revocation).

**Hygiene rotation versus compromise removal — the two acts differ, and only one revokes a content signature.** ADR-003 §4a and ADR-039 define a Layer 1 *rotation*: an owner installs a new `#active` or `#agent` key and retains the old one as `#retired-{sequence}` or `#retired-agent-{sequence}`, referenced by neither `authentication` nor `assertionMethod`. A rotation an owner performs for hygiene — a scheduled key change, no compromise suspected — is **soft**: the retained method keeps verifying an event-log leaf indefinitely (§23.13 paragraph 1 of the sync spec), because a leaf records what an actor did at the sequence it occupies and a later rotation must not retroactively unmake that authorship. The retained method still verifies nothing else: it authenticates no live session (§3.11.4 steps 7 and 8 of the identity spec) and it verifies no KeyPackage attestation (§9.7.1 check 1 above), because an attestation is a bearer capability a holder presents now.

**How an owner performs the removal.** The reference implementation composes it from three published operations rather than a fourth: `DidDht::rotate_active_key` (or `rotate_agent_key`) installs the replacement key and retains the compromised one under a `#retired-*` identifier, `DidDocument::remove_verification_method` drops that identifier from the returned document — `DidDocument::historical_assertion_keys` reports which identifier the rotation assigned — and `DidDht::publish_document` signs the result with `#0` and writes it to the DHT. An owner who stops after the first call has performed the soft act, and the compromised key keeps signing event-log leaves that every honest verifier accepts.

Compromise is **hard**, and this step requires the owner to remove the compromised verification method from `verificationMethod` entirely rather than retain it. Removal is the revocation act for a content signature: §23.13 paragraph 1 accepts only a retired method the resolved document still carries, so a method the document no longer carries verifies nothing, at any sequence, for any reader. An owner who reaches for a hygiene rotation to handle a compromise leaves the compromised key able to sign event-log leaves that every honest verifier accepts.

**No DID-document status field records compromise, and none is introduced.** A `Compromised { from: N }` marker — a field naming the sequence from which a method stops verifying — was considered and rejected. Nothing in an SCP record carries the DID-document sequence a reader would compare it against: ADR-011 acceptance criterion 1 gives `Event` a Unix `timestamp` and a per-log `sequence` and no DID-document sequence, whoever holds the key writes that `timestamp`, and §9.14 above confines a timestamp to ordering hints and replay detection rather than security-critical decisions. A verifier therefore has nothing sound to compare against `N`, so the marker would read as a revocation control while enforcing nothing. Presence in `verificationMethod` is the whole test, which makes it a fact a verifier can check from the document it already resolved.
