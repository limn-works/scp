...
**Three checks decide a receipt, and every one of the three reads a term `PaymentAdapter::verify` returned.** That call takes the receipt together with the relay's own declared currency and the price the relay computed, and returns a `VerificationResult` carrying what it verified against the rail (`19-economic-governance.md` §19.2.1); the relay's checks read those returned values and never the receipt's own fields, because a presenting party composes every field of the receipt. The three: the **rail-bound receipt identifier** the call returned is one this relay has not already spent; **`amount_covers`** is true, which is the call's answer to whether the settlement covers the price this relay supplied; and **`currency_matches`** is true, which is its answer to whether the settlement is denominated in the currency this relay supplied. A relay compares no verified amount and no verified currency of its own, because the call performs both comparisons against the terms the relay passed in and returns the answers; two rails denominate one integer in two monies, and an amount compared without its currency compares nothing. A relay that charges accepts the message only where the call returns valid and all three hold, and refuses `Payment` otherwise. **A relay refuses, under `Payment`, a receipt whose adapter tag names no member of its declared `economic.payment_adapters`**, and the refusal's value carries that set; otherwise the presenting party picks which of the relay's verifiers runs.
