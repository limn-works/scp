...
**The DID-record value carries the log.** The BEP44-signed `value` of an identity's DID record carries the key-event log the record projects: the chain from the inception event to the latest state-carrying event, or — when the resolver already holds a state-carrying event on that chain — the segment from that event forward. A publisher that cannot tell what a resolver holds publishes the chain from inception, which is the form every first-contact resolution reads.

**How a resolver authenticates a served record.** When resolving an identifier:

1. The client queries the Mainline DHT (§3.10.3) and every relay it knows for the record at the identifier's routing ID.
...
**Attack: relay list substitution.** A compromised DHT node or relay could serve a stale relay list, directing messages to relays the recipient no longer uses. Defense: a client takes the relay list from the latest state-carrying event of the chain it adopted (§9.7.4.2 R6, R8), and a chain that is a head of the accepted chain at a lower sequence is `RejectedStale` (§9.7.4.2 R12).
...
- **For every identifier, whatever its method:** the SDK records a `ContinuityStanding` for the identifier on first contact (§9.11) and sets it `PendingReverify` on every `RootRecovery` and every unauthored operational-key rotation it observes **after** that first contact. A first contact applies trust-on-first-use to the chain as Alice resolved it, whatever that chain contains: a `RootRecovery` already on the chain at first contact is part of the history Alice is trusting on first use, and Alice is not observing one. TOFU is not a did:web-only measure.
...
**Key conditions, the content boundary, and the relying party's obligation.** The latest state-carrying event of an identity's key-event log (§9.7.4.2 definitions, R8) lists every key that has appeared on the chain as a root member or an operational key, and each listed key is in exactly one of three conditions: **`current`** — listed in a role; **`Superseded`** — listed as a historical key; or **`Compromised{from:M}`** — listed with an asserted boundary M, a position in the key-event log at or before the sequence of the event that carries the entry. A state-carrying event lists every key any event at or before it installed, each key once and with one condition; R8 states that obligation and this section does not restate it, so a verifier applies R8's text and never a paraphrase of it. `Superseded` is derived: a key becomes `Superseded` when a state-carrying event lists it outside every role. The only assertion a controller makes about a key is `Compromised{from:M}`, and it withdraws a false alarm by a later state-carrying event that lists the key as `Superseded` or in a current role. An entry identifies its key by the public key bytes; the verification-method fragment a signature's `signing_key_id` names (§9.8.1) resolves to a key through the latest state-carrying event, so a verifier locates a `Superseded` key by that lookup and never from a current document alone.
...
**The boundary travels with the content it bounds.** A context snapshot and a context export carry, for every identity whose key the context retired, that key's boundary epoch in that context, and the producer signs those values with the key the snapshot or export is signed under. A member that joins after the boundary Commit therefore learns the boundary from the snapshot it syncs rather than from an observation it could not have made. A member holding no boundary value for a retired key **rejects** content signed under that key and reports `Invalid{no_boundary}` — the fail-closed direction, because a member that accepted such content by default would accept everything the attacker signed after the boundary.

**The relying party's obligation, stated over evidence it holds.** For a key listed `Superseded` or `Compromised{from:M}`, a relying party accepts a signature **iff** the content reached it under an epoch **before that context's boundary** for that key. One test decides it, and the test reads two inputs the party holds: the epoch the content came in under, and the boundary value. The epoch is the MLS epoch under which the party decrypted the content, with a valid `membership_tag` (§9.8.1 check 2), for content the party received live; for content inside a snapshot or an export it is the epoch the producing context recorded for that item, attested by the producer's `current` key on the snapshot. The boundary value is the one the party observed at the boundary Commit, or the one the snapshot or export carried to it. A party holding neither rejects, as the paragraph above states.
...
This bounds `Superseded` and `Compromised` alike: a retired key is not a live signing capability, and a compromise entry never unmakes what the party already accepted before the boundary. The controller's key event is never rejected on account of content.
...
| **Content** — accepted before the boundary | the relying-party obligation above: verifies under a current key, and under a `Superseded` or `Compromised{from:M}` key only for content that reached the party under an epoch before that context's boundary | `SCP-INNER-ENVELOPE-V1:`, `SCP-BROADCAST-ENVELOPE-V1:`, `SCP-VOTE-V1:`, `SCP-PROPOSAL-V1:`, `SCP-RECEIPT-V1:`, `SCP-XCTX-RECEIPT-V1:`, `SCP-XCTX-STREAM-RECEIPT-V1:`, `SCP-XCTX-DIVERGENCE-V1:`, `SCP-OUTLET-CHUNK-SIG-V1:`, `SCP-OUTLET-CREDIT-V1:`, `SCP-OUTLET-CANCEL-V1:`, `SCP-RESET-REQUEST-V1:`, `SCP-COMMIT-RANGE-REQ-V1:`, `SCP-COMMIT-RANGE-RESP-V1:`, `SCP-CONTEXT-SNAPSHOT-V1:`, `SCP-CONTEXT-EXPORT-V1:`, `SCP-CHECKPOINT-V1:` |
| **Log-anchored evidence** — anchored to a key-state position | the artifact carries the digest of the signer's key-state head at the moment it signed; the signature verifies **iff** the signing key was `current` at that position and, for a key the state lists `Compromised{from:M}`, that position is before M. A `Superseded` key's signature stays valid for this class, because the artifact names when it was made | `SCP-KEY-DESTRUCTION-V1:` |
| **Key event** — the log's own rules | §9.7.4.2 R3 | `SCP-KEL-EVENT-V1:` |

**Why the log-anchored class exists.** An artifact a party publishes as evidence against a future version of itself cannot verify under the attestation class, because the signer would void it by any routine rotation of the key it controls. §9.15's destruction attestation is such an artifact: it is published to relays, outside any context, so no context epoch orders it and the content class has no test to apply. Anchoring it to a key-state position gives it an ordering the signer cannot revise after the fact, because the key-state head digest is fixed by the log. `SCP-CHECKPOINT-V1:` (§9.9.3) needs no such anchor: a member accepts a consistency checkpoint through MLS inside the context, so the content class already orders it.

Governance votes and proposals are content: a vote cast under a key that is later marked compromised stays counted for the members who accepted it before the boundary, because the alternative — every vote in a context's history rejected the moment a key is asserted compromised — would let a compromise assertion rewrite governance outcomes. The DID-record frame's BEP44 signature (§9.10.12) is outside this table: it is not made under an SCP domain separator, and no relying party reads it as evidence. It is the relay and DHT write rule, made by the operational key the key state designates for the service record and checked by a relay against the `current` designated key of the chain it holds, and a resolver authenticates the record by the key-event log the frame carries instead (§9.6.1). A key-continuity fingerprint (§9.11) is a hash, not a signature, and is outside this table. Rationale for the default: a structure nobody classified is safer read as current-only than as content, because the content class admits historical keys.
...
- `Confirmed` and `Adopted` are the **success** branch, and a `RejectedStale` candidate leaves the verifier on its accepted baseline, which is also the success branch.
...
- `Invalid{at_event}` **rejects on both triggers and never reaches the grace.** A chain the verifier has judged invalid is not a transient resolution failure, so an Add rejects, an already-admitted member's Update rejects, and the verifier MUST NOT fall back to a last-known-good document for it. Routing it to the failure branch would hand the pin-to-stale attack to any party that can serve one invalid chain.
- `Contested` is its own branch: no new Add of that identity is admitted, no UCAN is issued to it, and no sender key is distributed to it while the contest stands. Its existing leaves are not evicted by the verdict alone, because R7 gives a contested identity no `current` key state to test them against; §9.12 step 1a runs on an adopted state-carrying event and proposes Remove for no leaf on a `Contested` verdict. Content verification for that identity uses the shared prefix's key state as its basis for content the context's boundary places before the fork, and verifies nothing after it (§9.7.4.2 R7).

A `RootRecovery` that is `Adopted`, and a `Contested` verdict, both set the identifier's `ContinuityStanding` to `PendingReverify` under §9.11, and every gate this paragraph names reads that one standing.
...
4. **Approved custody methods.** Under the key-event log a copy of a pre-rotation key alone does more lasting damage than a copy of the root alone: a root leak is recoverable by a reveal, while a leaked pre-rotation key lets its holder contest the identity at any later date and, held exclusively, take it over and then kill it permanently (the failure-mode table below). The pre-rotation key is therefore the backstop against root compromise and at the same time the key whose leak is least recoverable, and its custody is ranked on two axes with **post-spend destructibility** as the primary one: whether the controller can destroy every copy of a spent key (§9.7.4.2 R5) and, where the substrate supports it, obtain the §9.15 attested destruction; and resistance to theft at rest. Any one method is sufficient **subject to the recovery-authority-residence requirement of item 3a**. Every method MUST hold an Ed25519 key that signs the raw §9.5.1 preimage of a key event (§9.7.4.2 R3). A WebAuthn/FIDO2 authenticator signs only `authenticatorData || SHA-256(clientDataJSON)` under its own counter, and Apple's Secure Enclave and Android StrongBox hold P-256 keys only, so none of those substrates can produce the signature R3 requires, and this table names none of them.
   ...
   **Method × profile conformance.** A method conforms on a profile when that profile can hold the method's substrate and reach it at reveal time under item 3a. "Conforms" means the SDK MAY offer the method on that profile; "does not conform" means the SDK MUST NOT offer it there.
   ...
   | HSM-resident key (Ed25519-capable HSM) | Conforms — the primary server method | Conforms — network HSM under a separate principal | Does not conform — no mobile profile holds an HSM principal separate from operational custody | Does not conform — same reason |
   ...
   | Paper backup (BIP39 mnemonic) | Does not conform — an unattended server cannot re-enter a mnemonic at reveal time | Conforms | Conforms | Conforms |

   Every conforming cell is still subject to item 3a: the SDK MUST reject a selection whose recovery authority is reachable from the profile's operational custody, whatever this table says.
...
- a signer index list naming fewer than t root members or fewer than n next-set members, where n is the standing commitment's next threshold (definitions);
...
**The store.** A validating relay keeps one slot per **(routing id, standing root set)**, not one per routing id: a chain whose head is signed under a different standing root than another chain's head occupies a different slot, so an attacker holding a superseded root cannot evict the owner's recovered head by out-appending it, and every verifier receives both heads and runs R6. **A slot holds the full chain from the inception event to its head**, not a head alone, and a relay serving a head serves every predecessor of that head — R6 ranks chains, so a slot that held only heads would leave a verifier unable to rank the head it was served. §9.10.12 and §3.10.2 state the relay's write rules and are authoritative for them; this rule states the slot key and the chain-holding property those sections implement, and they cite it. **Opening a new root-set slot costs a reveal**, because only a `RootRecovery` installs a root set and every `RootRecovery` carries a reveal (R3). A party that opens many slots under one routing id has therefore revealed the standing commitment many times, which puts its suffixes at rank 1 and makes the identity contested by R7 whatever the relay stores; a flood of slots is a symptom of an already-contested or already-taken identity, never a way to take an intact one.

**The retention bound and its eviction rule.** A verifier retains at most `MAX_RETAINED_SUFFIXES` divergent suffixes per identity (§9.18.17: 8). A candidate that would exceed the bound is **admitted**, and the verifier evicts the retained suffix of lowest rank against the shared prefix's standing commitment, breaking a tie among equal ranks by evicting the suffix it observed first. **A rank-1 suffix is never evicted.** When every retained suffix is rank 1 and another rank-1 candidate arrives, the verifier keeps the retained set and returns `Contested{…, residue: n}`, where n counts the rank-1 candidates it could not retain — the identity is contested either way, and the residue tells the consumer that more tied suffixes exist than the verdict enumerates. Each eviction records the evicted head's digest so that a re-served copy is recognized as one the verifier already ranked. A validating relay, which verifies chains under R2 and R3, applies this same eviction rule to its root-set slots for one routing id; §9.10.12 and §3.10.2 cite this rule rather than restating it. R7's obligation to retain every tied head, and R14's persistence obligation, are scoped to the retained set. Rationale for the value: two devices plus several attacker forks fit with margin; the bound caps the pairwise comparison a flood would otherwise drive quadratic. Rationale for admitting rather than refusing: a bound with no eviction rule makes a flood the winning move, because a party that publishes eight rank-2 forks first would fill every slot before the owner's rank-1 recovery arrived and the recovery would never be ranked. Indicator, not criterion: key-event logs are short, so a verifier that retains its full accepted chain satisfies R11's evidence obligation with no fetch.
...
1. **A root thief forks early; the owner's device holding R0 is gone.** The owner's chain is e0 through e9 (R0-signed `KeyState` events). The thief, holding R0, forks at e5 and appends a5 through a50, all R0-signed. The owner no longer holds R0, so it signs a `RootRecovery{Lost}` at e10 that reveals C, installs a fresh root K′, carries a complete key state, and fixes C′. The shared prefix is e0 through e4 and its standing commitment is C. The owner's suffix reveals C, so its rank is 1; the thief's suffix reveals nothing, so its rank is 2. The owner's chain wins under R6 wherever e10 sits, and the thief's head sequence 50 decides nothing. Under R8 the derived key state after e10 is e10's snapshot, so nothing the thief installed survives; under R9 the owner's K′-signed chain occupies its own store slot, keyed to the root set K′ installs, and a relay serving that slot serves the chain from e0, so the thief cannot evict the recovery by appending and every verifier receives both chains in full.
...
2. **Confirms the identifier→routing_id binding**: `routing_id` equals the registered routing derivation over the identifier the frame's `identifier` field names. The derivation over an inception-derived identifier is fixed in a later revision of this section — the identifier byte layout revision, which the recovery-model work queue carries as item 2 — because it waits on the identifier's byte layout; the identifier encodes no key (§9.7.4.2 R13), so no key is derived from it and no field of the frame is hashed to the routing ID. This binding is the discriminant: a validating relay recognizes a DID record by the frame decoding *and* the binding holding — it needs no new wire type and no a-priori knowledge of `routing_id` semantics, and a context `OuterEnvelope` (whose first byte is a MessagePack map marker, never `version = 1`) is never mistaken for one. Being a plain hash, it runs **before** any signature verify.
...
4. **Verifies the BEP44 signature** over `bencode(seq, value)` against the **designated service key of the chain in the slot it is writing to** — the operational key that chain's latest state-carrying event designates for the service record — or, where the routing id has no slot for that chain's standing root set, against the designated service key of the chain the `value` itself carries, which step 3 has already verified from inception. The signature and `seq` are the write rule and never a resolver's trust input (§9.6.1).
5. **Keeps one slot per (routing id, standing root set)** — the store rule of §9.7.4.2 R9, which this section implements. A frame extends the chain in the slot for the standing root set whose digest it names, and the relay rejects a frame whose `seq` is `≤` the stored slot's `seq` **unless** an equal-`seq` frame is byte-identical (idempotent TTL refresh), replacing the slot only on a strictly-higher valid `seq` whose chain extends the stored chain. A frame naming a standing root set that has no slot at that routing id **opens a new slot**, and only a chain whose head is a reveal-valid `RootRecovery` installing that root set may open one (§9.7.4.2 R3), so a party cannot open a slot without revealing the standing commitment. The slot holds the chain from inception to its head, and a QUERY returns every slot at the routing id, because a resolver runs the fork-precedence rule (§9.7.4.2 R6) over the chains it is served and a relay that returned one slot would decide that rule for it. Slot count per routing id is bounded by `MAX_RETAINED_SUFFIXES` (§9.18.17) under R9's rank-eviction rule, which the relay applies to its slots exactly as a verifier applies it to retained suffixes.

**Slot-exclusivity.** The base relay stores multiple opaque blobs per `routing_id` with no per-`routing_id` cap (ADR-004), so validating only frame-shaped blobs would leave non-frame or pre-seeded junk sitting alongside the slot and returnable by QUERY. A validating relay therefore makes a `routing_id` **slot-exclusive** once a binding-valid frame first establishes a slot there: (a) it rejects any later PUBLISH there that is not a frame passing steps 1–5 above; (b) it evicts any pre-existing opaque blobs at that `routing_id` when the first slot is established; (c) QUERY there returns the slots and nothing else — every slot the routing id holds, one per standing root set; (d) it **rejects a DELETE of any slot blob** — only a superseding PUBLISH may replace a slot, and only R9's rank eviction may remove one. Rule (d) is **storage-derived, not index-derived**: because DELETE addresses a blob by `blob_id` and the in-memory slot index is a cache that a relay restart or a store-sharing peer leaves cold, the relay decides rule (d) by re-reading the blob at `blob_id` and rejecting the DELETE if it decodes as a frame whose chain verifies from inception under R2 and R3 (the chain is self-certifying, so protection is reconstructible from the bare bytes) — immune to a cold index. The DELETE gate runs behind the per-IP rate limit and fails closed on a storage read error (§3.10.8 "Attacker DELETEs the slot blob"). Before the first valid frame the relay cannot recognize the `routing_id` as DID-domain (`SHA-256` is one-way), so pre-seeded junk simply sits as ordinary opaque blobs until the first valid publish establishes the slot and evicts it (§3.10.2). This is a relay **storage** behavior; the relay-storage mechanics are transcribed in the companion **ADR-004 "DID-Record Slot-Exclusivity" subsection** (implemented under #482 / SCP-RELAYRES-003). This section and §3.10.2/§3.10.8 are authoritative for the behavior and its threat model.
...
DID-RECORD (DidRecordV1) :=
  ...
  root_set_digest:  [u8; 32]                 # digest of the standing root set of the chain in `value` — names the slot
  seq:              u64                      # 8 bytes big-endian — BEP44 sequence
  signature:        [u8; 64]                 # Ed25519 BEP44 signature, raw fixed-width, NO length prefix
  value:            [u8]                     # trailing remainder = BEP44-signed payload = the key-event log the record projects (§9.6.1)
```
...
- **root_set_digest** — the digest of the standing root set of the chain the `value` carries. It names which slot the frame writes to (step 5), so a relay picks the slot without parsing the chain first, and the chain the relay then verifies must carry the standing root set this field names. A frame whose chain's standing root set digests to a different value is rejected.
- **seq** / **signature** — the BEP44 sequence and signature, written raw with no length prefix (fixed-width fields, §9.5.1).
- **value** — the sole variable-length field, carried as the **trailing remainder** of the frame (no `value_len` prefix). Because every preceding field is fixed-width, `value` is unambiguously `frame_bytes[137..]`; a length prefix would be redundant with the blob's own length and a determinism footgun (two disagreeing lengths). `value` MUST be non-empty and its length MUST NOT exceed `Max blob size` (262144, §9.18.11) `− 137`.

The fixed prefix is `1 + 32 + 32 + 8 + 64 = 137` bytes, and the total frame length is `137 + len(value)`. There is deliberately **no** `sig_len` field: a BEP44 Ed25519 signature is unconditionally 64 bytes, and §9.5.1 gives fixed-width fields no length prefix; a different signature length would be a new version, not a per-frame field. **The frame carries no public key**: a relay verifies a write against the designated service key of the chain it holds in the slot, or of the chain the `value` itself carries when it opens a slot, so a key supplied by the writer would authorize the writer against itself.

**Framing is outside the signed authority.** The BEP44 signature covers only the **BEP44-canonical bencoded buffer `bencode(salt?, seq, value)`** — the sequence number precedes the value, serialized as `3:seqi<seq>e1:v<value>` (with an optional preceding `4:salt<salt>` where the DID method uses one; did:dht uses none). **BEP44 (BitTorrent BEP 44) is authoritative for this field ordering** — `seq` before `value` — and every SCP binding MUST reproduce that exact octet sequence, since the signature is hand-rolled across bindings and must be byte-reproducible. The frame's `version`, `identifier`, `root_set_digest`, and its field boundaries are unsigned transport framing, and a decoder MUST NOT derive any security-relevant conclusion from them. After decoding a frame, the resolver authenticates the record by the key-event log the `value` carries — recompute the identifier from the inception event and verify every event under §9.7.4.2 R2 and R3 (§9.6.1) — and reads the BEP44 signature for no trust purpose at all; a frame whose chain fails that verification is discarded exactly as an invalid DHT record is (§3.10.4). Record substitution fails because a chain served under an identifier recomputes to that identifier only if its inception event is the one that identifier names.
...
2. **Require the fixed prefix in full.** A decoder MUST reject any frame shorter than the 137-byte fixed prefix (`version + identifier + root_set_digest + seq + signature`) — truncation is never a partially-valid frame.
3. **Bound-check the `value` length only after the prefix check.** `value` is the trailing remainder, `len(value) = total_frame_len − 137`. A decoder MUST NOT compute `total_frame_len − 137` before rule 2 has confirmed `total_frame_len >= 137`: computing it first can underflow, which diverges between a debug-build panic and a release-build wrap across bindings. With rule 2 satisfied the subtraction cannot underflow. A decoder MUST reject an empty `value` (`total_frame_len == 137`) and MUST reject `len(value) > Max blob size (262144, §9.18.11) − 137`. No widening is required: `len(value)` is an actual buffer length (a valid `usize`, already bounded by the transport's Max blob size) and the bound is a compile-time constant — there is no wire-supplied length field to overflow (unlike a length-prefixed layout).
...
- **Publish** = wrap `(identifier, root_set_digest, seq, signature, value)` in a DID-record frame and PUBLISH the frame bytes at the DID routing ID via the existing PUBLISH operation (§3.10.5). The DID relay blob MUST be the DID-record frame — not the bare key-event log bytes, and not an `OuterEnvelope`.
- **Query** = the existing QUERY operation (§3.10.4) with `limit: N` (N = 16) returns the blob(s) stored at the routing ID; the resolver decodes each at the **single** decode-and-verify site, verifies the chain each `value` carries under §9.7.4.2 R2 and R3, and settles two chains that diverge by the fork-precedence rule (§9.7.4.2 R6). It reads no key from the frame, and it treats the BEP44 signature as the relay's write rule, never as evidence (§9.6.1). Against a validating relay the routing ID holds one slot per standing root set, so `N` bounds how many of those slots one QUERY returns; against non-validating or foreign storage `limit: N` lets the resolver retrieve up to N candidates and sift them by R2, R3, and R6 (§3.10.4 step 5). See §3.10.2 for why `limit: N` dominates `limit: 1`.
...
**Fingerprint format.** Both forms take the parties' **32-byte identifiers** (§9.7.4.2 R13), never their DID strings, and both are defined now. The identifier is already a fixed-length 32-byte digest, so under §9.5.1 it carries no length prefix and neither form waits on the identifier's textual encoding.

The **1-of-1 form**, for two identities each of whose root set has exactly one member:

```
fingerprint = SHA256("SCP-KEY-CONTINUITY-V1:" || id_a || a_root_member || a_active_key || a_agent_key || id_b || b_root_member || b_active_key || b_agent_key)
```

The **multi-member form**, which covers every root set of any size and reduces to the 1-of-1 form's inputs when each set holds one member:
...
Where `id_a, id_b` are the two 32-byte identifiers ordered by unsigned byte comparison, and the block for the lower identifier comes first; `count()` is a 4-byte big-endian member count (§9.5.1's repeated-field rule); `a_root_set_members` is every member of that root set as a raw 32-byte key, **in the list order the key state carries**, because the root set is an ordered list and its order is signed; and every key is a raw 32-byte Ed25519 public key. An SDK computes the multi-member form whenever either party's root set holds more than one member, and MUST NOT compute the 1-of-1 form over one member of a larger set. Every root member and both operational keys are included so that substitution of any single key changes the fingerprint.

**Sentinels for an absent key.** If a key state names no `#agent`, the domain-derived sentinel `SHA-256("SCP-ABSENT-AGENT-KEY")` (truncated to 32 bytes) stands in its place. If a key state names no `#active` — which R8 permits, because a snapshot may name no operational role — the sentinel `SHA-256("SCP-ABSENT-ACTIVE-KEY")` (truncated to 32 bytes) stands in its place. Each sentinel avoids collision with the Ed25519 identity point (all zeros) and provides domain separation from legitimate key values, and each makes the fingerprint defined for every key state R8 admits. The `"SCP-KEY-CONTINUITY-V1:"` domain separator prevents cross-protocol signature confusion.
...
The gates that read `ContinuityStanding`: §9.7.1's admission and grant gates (no Add, no UCAN, no sender key to a new leaf); §9.16.2's key response (no sender key to a new leaf of an identifier whose standing is `PendingReverify`); §9.6.4's first-contact bootstrapping (the TOFU record above applies to every identifier); §9.6.2's did:web mitigations (the same standing, keyed to the identifier, with the verification-method key set as the change trigger); and the standing-pair consent gate and the known-identity allowlist of the contexts spec, which MUST read the standing on a peer's `RootRecovery` because a stable identifier keeps matching an allowlist entry that a rename used to invalidate.
...
**1a. Leaf re-verification after a key-state change.** Every member of a context who **adopts** any state-carrying event for a fellow member — a `KeyState` or a `RootRecovery` — after which a leaf's attesting key is no longer `current` MUST re-verify every leaf in that group carrying that identity against the adopted key state. **The Remove test is the signature test:** the member proposes Remove for any leaf whose KeyPackage attestation does not verify against the key the adopted state lists `current` for the `signing_key_id` that leaf names (§9.7.1 check 3). Comparing the `signing_key_id` fragment instead would evict nothing, because every leaf names `#active` or `#agent` and the snapshot lists a fresh key under that same fragment. A PCS Update heals a leaked leaf secret and does not evict a member, so this step is the only thing that removes a leaf the attacker added — and the two most common compromises, `#agent` and `#active`, are remediated by a `KeyState`, so a step that fired only on a `RootRecovery` would leave the attacker's leaf in place for exactly those cases.
...
KeyDestructionAttestation {
  ...
  keyStateHead:          [u8; 32]              // digest of the signer's key-state head at signing (§9.7.1, log-anchored evidence class)
...
4. Attestations are published to relays, outside the now-destroyed context, so they remain readable after the context keys are destroyed. They are **log-anchored evidence** (§9.7.1): the `keyStateHead` field carries the digest of the signer's key-state head at the moment it signed, and a verifier accepts the signature iff the signing `#active` was `current` at that position and, where the key state lists that key `Compromised{from:M}`, the position is before M. A later routine rotation of `#active` therefore leaves every earlier destruction attestation verifiable, which is the property the attestation class would not give it — a signer that could void its own past destruction claims by rotating a key it controls would be publishing no evidence at all.
...
| DID-record frame fixed prefix | 137 | Fixed-width prefix length in bytes (`version` 1 + `identifier` 32 + `root_set_digest` 32 + `seq` 8 + `signature` 64); `value` is the trailing remainder, `total_frame_len = 137 + len(value)` | §9.10.12 |
...
| `MAX_RETAINED_SUFFIXES` | 8 | Maximum divergent suffixes a verifier retains per identity, and maximum root-set slots a validating relay keeps per routing id; a candidate beyond it is admitted and the lowest-ranked retained suffix is evicted, a rank-1 suffix never | §9.7.4.2 R9 |
