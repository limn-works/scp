...
**A verifier resolves a `signing_key_id` fragment under one of three arms, and the artifact's class in §9.7.1's classification table selects the arm.** **Attestation arm:** the verifier reads the `Current` key of the named role at the latest state-carrying event and reads no other key. **Log-anchored arm:** the verifier replays the chain to the position the artifact's `key_state_head` names and reads, at that position, the key that held the named role and the condition it held there. **Content arm:** the verifier replays the adopted chain, takes every key that held the named role at any position, takes the one key among them whose bytes verify the signature, and then applies that key's condition and that context's boundary under §9.7.1's content rule. The class selects the arm, and the presence or the absence of a `key_state_head` field selects nothing: no attestation-class separator carries that field, so a field-presence test would send every one of them to the content arm, whose procedure admits a key the controller has already dropped. No arm other than the attestation arm resolves a fragment from the latest key state's current keys, and the key state carries no set of every key that ever held a role for the log-anchored arm or the content arm to read.
...
**A relay that charges also declares which refusal scopes a verified receipt exempts.** `relay_config` carries `receipt_exempts`, a set whose members are scope names, and ADR-004's `POLICY` query returns it, so a publisher reads what its money buys before it spends. **A relay that charges and declares nothing exempts nothing**: an absent or empty `receipt_exempts` exempts no scope, and an implementer reads that declaration rather than assuming a set. **Alec ruled on 2026-09-14: "it should really only apply to storage (amount, duration). not to traffic."** What a relay sells is therefore storage, in amount and in duration, and it sells no relief from traffic. **`Identifier` is the one member a relay may declare**, because it is the one scope of the four that names a storage amount a relay can sell relief from. **`RateLimit` names a rate of writes, which is traffic, so no declaration exempts any party from the rate limit.** `Payment` names a receipt rather than storage, so a declaration naming it would make the receipt the object that exempts the check reading the receipt. `Total` names a storage amount and stays undeclarable for a reason about the operator's own host rather than about policy: it is the capacity that host holds, and an exemption from it names an outcome no relay can produce whatever its operator declared. `18-addressability-and-deployment.md` §18.3.3 states the one permitted member, and a `relay_config` naming a second is invalid.

**A declared `Identifier` exemption stops both arms of the per-identifier budget** — the suffix eviction this paragraph states and the refusal that follows it — **for the identity whose rent is current and for as long as that rent is current**, because both arms read the one term the exemption names and because a relay sells storage in duration as well as in amount. An exemption an identity bought lapses with the rent that bought it, and the per-identifier budget binds that identity again from that moment.

**The rate-limit check counts every PUBLISH, reads no field of a receipt, and no declaration exempts any party from it.** The relay counts the arriving PUBLISH against the publishing address and refuses it where the count passes `rate_limit_publish`, whatever that PUBLISH carries and whatever the relay declared. A check that admitted a PUBLISH for carrying a `payment_receipt` field would admit every PUBLISH carrying arbitrary bytes under that name, and each one would reach `PaymentAdapter::verify`, which hands an unauthenticated party one outbound rail query per request. A relay that skipped the check would have to know a receipt is valid before it ran the check, which drives that call ahead of the limit and reaches the same flood. `03-identity.md` §3.10.2 states the order and gives that reason.
...
**Under `Identifier` pressure the relay evicts before it refuses:** it evicts that identifier's retained divergent suffixes in the eviction-rank order below, lowest-ranked first, and refuses only where evicting every suffix it may evict still leaves the per-identifier budget exceeded. **The two pressures act on two units and neither is an exception to the other.** The ring acts when the retained capacity fills and the unit it moves is one identity; the `Identifier` eviction acts when one identity exceeds its per-identifier budget and the unit it moves is one divergent suffix. **`Identifier` bounds what the relay holds for one identity**, so one identity holds no more than the per-identifier budget wherever an operator declares one.
...
**Coverage is rent.** Alec ruled on 2026-09-14: "it's rent. as long as you keep paying. operator says, \"pay x at y frequency, get quota a of b thing\"." **A relay that sells coverage declares three terms in `relay_config` — a rent price, a rent period, and the quota of stored bytes one rent payment covers — and an identity is covered up to that quota while its rent is current.** A receipt the relay verifies against the declared rent price buys the declared quota for the declared period, counted from the moment the relay verified it, and a further verified rent payment before that period elapses extends the term. **An identity whose rent lapses is uncovered from that moment and is an ordinary displacement candidate again**, so no payment buys coverage that outlives the period it paid for. **A relay that declares no rent terms sells no coverage, and every identity it holds is uncovered.** **Coverage is a cap on what rent protects and never a credit a write earns**: the relay reads, for each identity, the bytes it holds for that identity against that identity's current quota, and the identity is uncovered where the first figure exceeds the second or where the rent has lapsed. **The rent terms supersede `per_byte_stored` as what a controller pays for storage of a retained kind**, because that member is a one-time fee and a retained kind never expires; `per_byte_stored` keeps its meaning for a blob the relay expires under a TTL, and `per_publish` keeps its meaning as the fee for the publish operation and buys no coverage. **The relay records each identity's current quota and the instant its rent expires, both durably beside the records they cover**, and it reads its own clock to decide whether a rent term has elapsed. **That clock decides one thing and orders nothing**: the ring's order is the monotonic counter above, no relay measures how long it has held a record, and two relays whose clocks differ disagree only about the moment one identity returns to the displaceable set, which is a disagreement about an operator-declared term and never about which identity the ring displaces next.

**The coverage test counts the bytes the identity's own controller published — its chains at `scp:did:` and its service record at `scp:svc:` — and counts no byte of the `scp:wit:` and `scp:wcf:` records other parties write about it.** A displacement still takes all four kinds together, which is what keeps a chain with its inception event and a fault proof's two heads at one relay, and the witness-written bytes count against `Total` like any other byte the relay holds. Counting them against the coverage test made a controller uncovered at its first designated witness's cosignature, for bytes it cannot compose, cannot refuse and cannot buy a receipt for, and let a party driving every witness to a conflict statement strip a paying controller's coverage with one event.

**The ring displaces uncovered identities alone, and it never splits a partly covered one.** An identity holding more published bytes than its current quota, and an identity whose rent has lapsed, are each an ordinary displacement candidate, and the ring displaces either whole, taking every record the relay holds for it together, because a split would leave a chain without its inception event and would separate a fault proof's two cosigned heads. **The ring reaches every wholly uncovered identity, in establishment order, before it reaches any partly covered one, and reaches partly covered identities in establishment order after that**, so a controller that rented most of its bytes loses them only after no identity that rented none remains. **A displacement takes the records the relay holds for an identity and leaves that identity's quota and rent expiry standing until the rent lapses**, so a controller that re-publishes inside the period it paid for is covered again at that relay and pays no second rent for it. **An identity whose held bytes pass its quota, or whose rent lapses, keeps the establishment position it already holds**: the relay refuses no write on that ground and answers no distinct error, because no declared term refused it. **A controller keeps its identity covered by paying rent at the frequency the relay declares, and it raises its quota by renting more**; neither act requires it to publish a byte, and the rent paragraph below states how a receipt that pays rent reaches the relay. **Coverage's rule is protocol and coverage's terms are the operator's declaration**: the `receipt_exempts` declaration above names refusal scopes and a displacement is not a refusal, so a relay's rent terms rather than its exemption set decide whether an identity is covered, and a relay that declares no rent terms holds every identity uncovered. **There is no idle clock, no refresh write and no pressure ordering**: no relay measures how long it has held a record, no party re-publishes a record to reset a clock, and the relay displaces by coverage and then by establishment order and by nothing else.

**What a displacement leaves each party to do.** A controller whose self-check finds its record absent at a fallback-set entry re-publishes its chain there (R10), which establishes that identity at the newest position in the ring, and it re-publishes its service record with the chain, because the displacement took every record the relay held for it. A witness re-cosigns the subject's head on the next offer that subject's controller makes it, which the self-observation cadence above drives, because a witness fetches nothing on its own account and cosigns nothing it was not offered (§9.7.4.3); that cosignature is an ordinary write into what the relay holds for the subject, and its bytes count against `Total` and against no identity's coverage. **A relay that holds no chain for a subject refuses every `scp:wit:` and `scp:wcf:` write naming that subject** (`03-identity.md` §3.10.2), so the controller's re-publication of its chain is what lets a witness record land there again. A conflict statement is held by the witness that wrote it and by the controller that fetched it. **No party pays to keep another party's identity covered**: coverage reads the bytes a controller published, so a third party's payment buys the subject nothing. Every controller holds its full log (R10), so a displacement changes where a record is stored and destroys no record, and a controller that lost the device holding that log has lost the log.

**The one attack the ring does not bound.** A party minting distinct identities displaces every uncovered identity at a relay by writing binding-valid frames at that relay's full ingest rate, for as long as it wants honest records absent: free at a relay that charges nothing, and at the declared price per write at a relay that charges. The per-address rate limit above bounds the rate, and no rule of this protocol bounds the duration. That is the bandwidth attack every free public service faces. What answers it is free capacity on the list under distinct operators, or paid entries (`19-economic-governance.md` §19.8); who curates the shipped list and what obligations that curator carries are open (`18-addressability-and-deployment.md` §18.5.1).

**Rent, and the wire fields a relay reads a receipt from.** A PUBLISH carries an optional `payment_receipt` (§9.10.12): the `PaymentReceipt` bytes of `19-economic-governance.md` §19.2.1, tagged with the adapter that issued them. A relay declaring `per_publish` or `per_byte_stored` verifies that receipt through the named adapter, which is `PaymentAdapter::verify` of §19.2.1 — "relays can already charge rent." **"Paid" means "carried a receipt this relay verified" in every storage rule of this section, and "covered" means what the ring-buffer paragraph above states over an identity**, so the `Payment` scope reads a field the wire carries and the ring reads the bytes the relay holds against the bytes a controller rented.

**Four checks decide a receipt, and every one of the four reads a term `PaymentAdapter::verify` returned.** That call takes the receipt together with the relay's own declared payee, currency and price, and returns in a `VerificationResult` the terms it verified against the rail (`19-economic-governance.md` §19.2.1); the relay's checks read those returned values and never the receipt's own fields, because a presenting party composes every field of the receipt. The four: the **verified payee** equals this relay's declared `economic.payee`; the **rail-bound receipt identifier** is one this relay has not already spent, and the relay spends it at the check that places the frame's events on a chain and at no earlier check, so a frame the relay rejects costs the publisher no receipt; **the relay holds that spent set durably**, because a restart that lost it would let one settled payment buy a second write; the **verified amount** covers the declared price for what the receipt pays, which is the `per_publish` fee for a write, the declared rent price for a rent payment, or both where one receipt pays both; and the **verified currency** equals the relay's declared `economic.currency`, because two rails denominate one integer in two monies and an amount compared without its currency compares nothing. The relay refuses `Payment` where the PUBLISH carried no receipt, where the verification failed, or where any of the four checks fails. **The rail-bound receipt identifier and the relay's spent set are what stop one receipt being used twice.** **Nothing on the relay's path verifies the payer's signature over the receipt**, because that preimage waits on the identifier's textual form and because a relay resolving `payer_did` would resolve a key the presenting party chose; the receipt's signed payload therefore joins §9.5.2's register of preimages that wait on that form, and that is the residue this rule leaves. **`economic.payee` is the relay identifier every charging relay declares** and every publisher reads through the `POLICY` query, so a self-hosted relay that charges has one and a relay that charges nothing needs none.

**A receipt that pays for a write accompanies one PUBLISH, and the relay spends it once, for the identity that PUBLISH names.** **A receipt that pays rent alone reaches a relay on `RENT`, an ADR-004 client operation of its own, and never on a PUBLISH**, because an identity that publishes nothing for a whole rent period would otherwise hold no way to pay and would go uncovered by the next period. **`RENT` carries the identifier the rent is credited to and the `PaymentReceipt` bytes, and carries nothing else.** The relay runs the receipt's four checks on it, credits that identity's quota and rent expiry, and answers with that identity's rent state after the payment, so a controller learns what it now holds without a second round trip. **`RENT` stores no bytes, so it sits outside the ring, outside the four retained kinds and outside every budget scope**: no `Identifier`, `Total` or `Payment` refusal reads it and it displaces nothing. **The declared rate limit counts `RENT` like every other message**, because a relay sells storage and never traffic, so no payment buys a party a higher message rate. The rejected alternative was a PUBLISH whose blob is absent: the retained-kind path reads that blob at the `retain` flag's scope, at the routing-id derivation and at chain verification, so each of those checks would need an arm for a frame that is not there.

**`03-identity.md` §3.10.2 orders every check a relay can run before it decides whether to spend an outbound query ahead of `PaymentAdapter::verify`, the one call that leaves the relay's host, and gives that reason: the call is an asynchronous query to a payment rail, a PUBLISH carries no key, and a relay that called the rail first would drive one outbound query per request an unauthenticated party sent.** **Every check after that call reads a value it returned or a decision it produced**, which is why the storage budgets and the chain verification sit behind it. **The declared-write-policy paragraph above states the declaration a charging relay makes and the one scope it may name, and the ring-buffer paragraph above states the coverage against displacement rent buys.** **A verified receipt buys no exemption from `Total`: rented bytes count against the declared total budget**, and the declared-write-policy paragraph above states when `Total` refuses, so the ceiling an operator declared bounds what that operator stores against a funded party. The blob-TTL clause of `19-economic-governance.md` §19.8 reaches no retained kind.
