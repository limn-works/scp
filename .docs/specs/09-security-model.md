...
**Step ordering and failure isolation:** Steps 1-6 are ordered by dependency: key rotation (1) must complete before MLS Updates (2) because Updates use the new key material; MLS Updates (2) must complete before UCAN revocation/reissuance (3) because new UCAN tokens are signed by the new key; KeyPackage attestation rotation (4) must follow to prevent new group additions using old key material; steps 5 and 6 are cleanup and can execute in any order after step 4.

**Step scope — per-context vs identity-scoped.** Steps 2 and 3 are **per-context**: failure in one context does not block recovery in other contexts, and the SDK retries failed contexts independently. A context where MLS Update cannot succeed (e.g., member has been offline too long and requires Tier 3 re-join per ADR-029) is flagged for manual re-join and does not block recovery in other contexts.

Steps 4, 5 and 6 are **identity-scoped**: each executes exactly once per recovery, never once per context. Step 4 is identity-scoped because the artifact it deletes and republishes — the **published** KeyPackage — is keyed by **owner DID** and carries no context field, independent of which contexts that identity belongs to. (Node-local, *private* KeyPackage material is a different thing and IS namespaced per context — MLS storage keys its entries under `mls/{context_id}/`, and a pending-join key package is bound to both owner DID and context id. That material is never published, so it is not what step 4 retracts; the claim here is about the published artifact only.) "Retry the failed contexts" is therefore not a meaningful remediation for step 4; retrying step 4 means retrying step 4, once, for the identity.

Because step 4 is identity-scoped it **gates completion for every context**: a KeyPackage that survives in the relay-fetchable population carries an attestation signed by the retired key and can be used to add the compromised member to a *new* group. That add vector is reason enough for the gate. No context may be reported as fully recovered while step 4 has not succeeded, and a step-4 failure is a **whole-recovery failure** rather than a per-context one. This holds even when the identity belongs to **zero** contexts — steps 4, 5 and 6 still execute, and a step-4 failure still fails the recovery.

**What step 4 does and does not bound.** Step 4 removes the **relay-fetchable distribution channel** for KeyPackages carrying attestations signed by the retired key, shrinking the population of parties who can obtain one. It introduces **no new revocation mechanism and no new DID-document field** — rotation (step 1) remains the revocation act, per §9.7.3 and the leaf-key/MLS-state bullet of step 1 above.

Step 4 does **NOT** make the rotation-propagation window unexploitable, and MUST NOT be described as doing so. Verifiers resolve the signer's **current** verification method from a document that may be up to `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (§9.18.7 — 5 min) stale (§9.7.1 checks 1–2). Within that window, two vectors survive step 4 entirely:

- **Bytes already distributed out-of-band.** A KeyPackage handed directly to an inviter has left the node; no relay delete reaches it. It stays presentable for the remainder of the window.
- **Freshly minted material.** The tiers step 4 addresses are `#active`/`#agent` compromise — the attacker holds the *signing* key itself, so within the window they can mint a **brand-new** KeyPackage and a **brand-new** attestation over a leaf of their choosing. No fetchable artifact is involved, so deleting published copies is structurally incapable of closing this.

This is the same bound §9.7.3 states for the leaf-key vector — an attacker "can present that leaf to a different group until either the attestation's capped lifetime expires ... or the victim rotates" — and it is not narrowed by step 4. The two controls that actually bound the window are the ones §9.7.3 names: rotation propagation (`MAX_ATTESTATION_KEY_RESOLUTION_STALENESS`) and the attestation lifetime cap (`MAX_KEYPACKAGE_ATTESTATION_LIFETIME`). Step 4 reduces reach, not duration.

**Steps 5 and 6 are non-fatal — but never silently skipped, and never reported as success when they did not run.** They execute regardless of per-context outcomes (they are identity-wide cleanup, and a per-context failure must not cancel them), and, unlike step 4, a failure of either does **not** fail the recovery: step 5 is best-effort by construction — the protocol requires no delivery confirmation — and step 6 is independently re-drivable without repeating steps 1–4. Their outcomes MUST be reported to the caller on both the success and the fail-closed path, distinguishing three states: *ran and succeeded*, *did not apply* (e.g. no known contacts; an agent-key compromise, which does not affect the PSK), and *ran and failed*. "Did not apply" MUST NOT be reported as success. Step 5 MUST additionally report **which** contacts were reached and which were not: an unreached contact still holds a §9.11 Key Continuity Verification binding to the compromised key and has not been told to re-verify — precisely the impersonation window recovery exists to close — so "at least one contact was reached" is not a sufficient report. A recovery in which step 5 or step 6 failed, or in which any contact was unreachable, is therefore **not fully recovered** even though it is not a whole-recovery failure; a caller MUST NOT read the absence of a fatal error as completion.
