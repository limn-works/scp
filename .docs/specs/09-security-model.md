...
R10's one-reveal-at-a-time rule reads a store set this value selects. **On `Browser` that store is the credential's own `largeBlob`, which the platform syncs with the credential and which every origin whose effective domain is the credential's relying-party identifier, or a registrable-domain suffix of it, reaches — the same span the `Passkey` row of the conformance table below states. Where the authenticator supports no `largeBlob`, the platform gives the SDK no durable store spanning those origins and the profile is per-device for that rule.** A profile is a property of a running SDK and never of an identity. The SDK MUST re-surface the copyable-custody disclosure whenever it opens an identity whose standing pre-rotation custody type is one of the three copyable values, on whatever profile it runs, because an identity incepted headless under a copyable method keeps that key when its operational key migrates to a phone.
...
| Two of the controller's devices each reveal the standing commitment | Rank 1 on both sides; terminal. R10's one-reveal-at-a-time rule stops the race wherever one store spans every SDK the credential serves: on `Mobile` and on `Desktop` under a platform account, and on `Browser` where the authenticator supports WebAuthn's `largeBlob`. The race stays open on `Headless`, on `Mobile` or `Desktop` under no platform account, and on `Browser` where the authenticator supports no `largeBlob` — and on that last profile it reaches two origins under one relying-party identifier and a private window, not two devices alone |
...
**The retention bound.** A verifier and a validating relay retain per identifier at most `MAX_RETAINED_SUFFIXES` divergent suffixes and at most `MAX_RETAINED_BYTES` of divergent-suffix bytes. Those two bounds are the whole of the retention rule, each reads the eviction rank below, and neither refuses a suffix on arrival without reading that rank. Both count divergent suffixes and count no byte of the accepted chain, which the storage budget below governs instead. A relay MAY configure a lower per-identifier byte value and MUST NOT configure a higher one. **The byte bound governs a rank-2 and a rank-3 suffix and governs no rank-1 suffix**: a rank-1 suffix is admitted whatever value the relay configured, so a lower configured value evicts only rank-2 and rank-3 suffixes and changes no verdict any party reaches. **That exemption is from a configured value and never from the protocol ceiling**, and it is no exemption from the fetch bound below: where the retained rank-1 suffixes fill the protocol ceiling the party refuses the arrival, as the rank-1 flood paragraph states. Without a byte bound the count bound leaves a suffix's bytes unbounded: a holder of one spent pre-rotation key forks below the event that spent it, signs a `RootRecovery{Lost}` installing its own root, and appends four million `KeyState` events on that rank-2 suffix, so every verifier that resolves the victim fetches about 1.1 GB and performs four million signature verifications before R6 can rank. A byte bound with no rank term would be defeated by the cheapest suffix to manufacture, which is why the rank term the eviction rank defines is what this bound reads.

**The relay's declared write policy.** Alec settled the relay's storage rules on 2026-09-13. A per-identifier byte ceiling misses the actor — "the risk is bad actors not theives" — because a party minting identities fills a relay under any per-identifier figure, so the protocol fixes a policy frame and never a figure, and it fixes eviction and denial together because "we need both eviction and denial". `MAX_ACCEPTED_CHAIN_BYTES` is withdrawn and no per-chain byte ceiling stands.

A validating relay declares its write policy in `relay_config` (`18-addressability-and-deployment.md` §18.3.3): `rate_limit_publish`, a per-identifier storage budget, a total storage budget, `per_publish` and `per_byte_stored` (`19-economic-governance.md` §19.8), and `pow_difficulty`. A publisher reads that policy through ADR-004's `POLICY` query (§9.10.12). The relay refuses a key-event PUBLISH that fails any declared term with `IdentityError::StorageBudgetExceeded{scope, value}` (`03-identity.md` §3.10.10), whose five scopes are `Identifier`, `Total`, `RateLimit`, `Payment` and `ProofOfWork`, and it keeps serving every record it already holds. **`value` carries the declared term the refusal names — the difficulty in bits, the price, the budget figure, the rate — so the one object a publisher holds after a refusal states what would satisfy it**, and the wire code `4041` carries the same field. **The protocol fixes each term's shape and never its value.** A term declared zero is off, and an absent term is off at every term but one: an absent `rate_limit_publish` is the 6,000-per-minute default `18-addressability-and-deployment.md` §18.3.3 states, and a relay turns that term off by declaring zero. **A refusal is never a verdict**: the publisher's recourse is the next entry of its fallback set, and a publish cycle is confirmed when at least one entry accepts it rather than when one answers (`03-identity.md` §3.10.6). **The exemption that lets an owner's recovery land reads the event's kind and never a rank: a reveal-authorized event — a `CommitmentRollover`, or a `RootRecovery` at either `standing_root` value — and the segment carrying it are never refused for `Identifier` or for `Total`.** A relay reads the kind off R3's type discriminator before any rank exists, so the exemption fires on the ordinary recovery that extends the accepted chain and carries no rank, and on a recovery arriving as a divergent suffix alike. Keying it on the kind admits no flood a rank term was holding back, because R1 lets a commitment change only under a reveal and R5 destroys the revealed keys. **No per-publisher scope exists**, because a PUBLISH carries no key (ADR-004, the SCP native relay protocol) and a minting party's signing key is fresh for every identity it mints.

**Proof of work.** A PUBLISH of a key-event record carries `nonce: u64` (§9.10.12). A relay declaring `pow_difficulty = N` greater than zero accepts that PUBLISH only where **`SHA-256(routing_id ‖ value_digest ‖ relay_id ‖ nonce_be64)`** carries N leading zero bits. `value_digest` is `SHA-256` over the `value` bytes that PUBLISH carries, which is the frame field §9.10.12's layout defines and not the relay proof's response digest of the same name; `relay_id` is the 32-byte `operator` identifier that relay's community-relay-list entry declares; and `nonce_be64` is the `nonce` written as 8 bytes big-endian. **The `relay_id` term is what makes one solution serve one relay**, so a party publishing to N listed entries solves N puzzles. **The puzzle binds a key-event PUBLISH and binds no other retained kind**: a service record, a cosigned head and a conflict statement each pass a relay-side signature filter against a chain that relay already holds (`03-identity.md` §3.10.2, §3.10.13), so a party writing at those three addresses first mints an identity through the kind the puzzle gates, and a witness owes no solve on the heads it cosigns. `N` MUST NOT exceed `MAX_POW_DIFFICULTY` (§9.18.17), and **a client MUST NOT solve a declared difficulty above that ceiling: it treats that entry as refusing and moves to the next entry of its fallback set.** The client's rule is the half a party executes against an untrusted relay, because the relay's own MUST NOT binds an honest relay and binds nothing a publisher relies on. **The ceiling is a bound on the honest client's cost and is stated as that and as nothing else**: one solution costs `2^N` hashes for every party, so the attacker's cost is `2^N` per identity per relay, and this section claims nothing further about what a flood pays. **Difficulty is per relay**: a publisher reads `relay_config` before it publishes, and a relay that raises its difficulty reaches its own publishers and no others.

**Eviction.** A relay MAY evict a retained record it was not paid to hold when it has held that record for `EVICTION_IDLE_SECONDS` (§9.18.17) without the identity re-publishing it. **Eviction's scope is the four retained kinds**, `scp:did:`, `scp:svc:`, `scp:wit:` and `scp:wcf:`. **The window is measured in the relay's own clock against a registered relay constant, and never as a multiple of the `witnessing_interval` the subject's own key state names**, because a party holding a threshold of a victim's root writes that field and would otherwise set the victim's window to forty minutes and its own to fifty-six days. **A validating relay treats a PUBLISH of a record whose bytes it already holds as a refresh: it charges no budget term, requires no proof of work, stores no new bytes, and resets that record's idle clock.** R10's self-observation cadence performs that refresh at every relay that serves the identity, which is the duty this window reads. Under storage pressure the relay evicts losing suffixes by the eviction rank below first, **then unpaid records by idle time, longest idle first** — and never the idlest identity, which ranks a continuous flooder last and a quiet honest identity first, so a conforming relay under the pressure a flood creates would evict the honest chains and keep the flood. **A paid record is never evicted for budget; an identity's accepted chain at a relay it pays is never evicted, and at a relay it does not pay that chain is evictable under the idleness rule above.** Every controller holds its full log (R10), so an eviction changes where a record is stored and never destroys it. A controller that lost the device holding that log stops refreshing by construction, and this rule then evicts the one copy a stranger could read: that is the residue the rule leaves and not a case it covers.

**Rent.** A PUBLISH carries an optional `payment_receipt` (§9.10.12): the `PaymentReceipt` bytes of `19-economic-governance.md` §19.2.1, tagged with the adapter that issued them. A relay declaring `per_publish` or `per_byte_stored` verifies that receipt through the named adapter, which is `PaymentAdapter::verify` of §19.2.1, and refuses `Payment` where the PUBLISH carried no receipt or where the verification failed — "relays can already charge rent." **"Paid" means "carried a receipt this relay verified" in every storage rule of this section.** `per_byte_stored` on a retained record buys exemption from eviction and from refusal for `Identifier` and for `RateLimit`. **It buys no exemption from `Total`: a paid record counts against the total budget, and a relay MAY refuse a PUBLISH for `Total` whoever offers to pay**, so the ceiling an operator declared bounds what that operator stores against a funded party. The blob-TTL clause of `19-economic-governance.md` §19.8 reaches no retained kind.

**Free listed relays** exist for reads and for first contact, and they accept writes under the policy they declare (`18-addressability-and-deployment.md` §18.5.1). **A free listed entry accepts an inception PUBLISH**: `pow_difficulty` and `rate_limit_publish` apply to that write, and neither budget scope refuses it for an identifier the relay does not yet hold, because a per-identifier budget has nothing to count against a first write and a total budget refusing one would make the entry's freeness deliver nothing. A stranger reads an identity only where some relay stored it, so an entry that refused every inception would deliver neither of the two acts the free-entry invariant names. The service record at `scp:svc:` is one record per identity, replaced on a higher sequence, and it counts against the same budgets (`03-identity.md` §3.10.2, §3.10.13).

**The fetch bound.** A verifier reads an accepted chain newest-first: the page walk's cursor runs backward from the `stored_at` of the head the relay serves, so the verifier reads the events nearest the head before any event a party appended earlier. **A verifier MUST stop fetching once the accepted-chain bytes it has read for one identifier, from one source, in one resolution, reach `MAX_RETAINED_BYTES`.** The bound is per source and per resolution: it neither spends one source's allowance on another, which would leave R11's two-source floor unmeetable for every long chain, nor carries a count forward from an earlier resolution, which would take the bound from a first contact to every established peer. It treats the unread remainder as absent rather than as a copy-level defect, so the verdict is **`Inconclusive{MissingEvents{range: [0, first_read), failed_sources: []}}`** and never `Invalid{at_event}`: a chain the verifier did not read is not a chain it found a defect in. **Both fields of that payload carry a value the verifier holds** — the range runs from sequence 0 to the first sequence the verifier read, and `failed_sources` is empty, because the verifier stopped itself and no source failed to serve. Naming the relay it was reading would record an honest listed relay as having failed to serve, which is the signal §9.9.2's suppression reasoning reads. **The backward read is what keeps a pad from freezing the identity.** A party holding a threshold of a victim's standing root appends root-signed `KeyState` events to the one genuine chain and produces no divergence, so R6 never runs; under a forward read its pad sits between every reader and the controller's later reveal-authorized event, and under the backward read that event is at the head and every reader reaches it before any pad. **An `Inconclusive` verdict blocks no recovery either**, because R10 composes a reveal-authorized event from the log the controller itself retains and never from a resolution verdict. **The bound reaches every suffix a verifier fetches, a rank-1 suffix included**, whose exemption above is from retention and from eviction alone; a reveal sits at the first event of the suffix, so a verifier decides the rank before the bound binds. The fetch bound counts the accepted-chain bytes a verifier has read, where the byte bound above counts divergent-suffix bytes, and a verifier runs both at once. The fetch bound is not a sequence ceiling, for the reason the segmentation paragraph above gives.

**The eviction rank** is defined for a divergent suffix and for nothing else. It is a scalar: the R6 rank the suffix carries against the accepted chain, read against the commitment standing at that suffix's own fork position. Eviction reads that one term and nothing else, and R6's pairwise comparison decides adoption while no eviction reads it. **The accepted chain sits outside the retained set and carries no eviction rank.** It is the chain the verifier held before it observed any divergence; at a first-contact verifier, which held none, it is the chain that verifier adopts under R11, and until that moment every chain it fetches is bounded by the fetch bound above, which is a MUST, and by nothing else; and at a validating relay it is the chain whose slot the routing id's first binding-valid frame established. **A verifier never evicts that chain. A validating relay never evicts it for a budget term, and never evicts it at all at a relay the identity pays; at a relay the identity does not pay it is evictable on the idleness arm of the eviction rule above and on nothing else**, so one record has one fate under both rules. R9 counts every arrival against it. Without the accepted-chain clause the rank term was defined only by reference to a chain the suffix forked from, so the chain every fork hangs off had either no rank or eight, and one conforming reading evicted it first. A suffix is rank 1 in one pair and rank 2 in another, so "never evict a rank-1 suffix" had no referent and two relays evicted different slots from identical inputs.
...
**Self-observation.** The SDK MUST fetch its own identity's log at a stated cadence, no less often than the witnessing interval the key state names and on every launch, from at least one relay in the fallback set, and MUST treat an inability to complete that check as an alert. **On each cadence it MUST also re-PUBLISH its current head to every entry of its fallback set.** A validating relay takes that write as the refresh R9's eviction rule defines: no budget term, no proof of work, no new bytes, and that record's idle clock reset. The fetch establishes what the controller is standing on, and the re-publication is what the eviction window reads. That cadence is also the submission cadence of §9.7.4.3.
...
**One reveal at a time.** An SDK MUST NOT compose a reveal-authorized event for an identity, and refuses with `IdentityError::RevealAlreadyPending`, while it can observe, for that identity, either an unpublished reveal-authorized event or a pending `RecoveryHandle`. **The SDK MUST write the pending handle, and the signed event once it signs one, into every store the identity's credentials sync through, before it signs; and it MUST read every one of those stores before it composes.** **The store set is the running profile's**: the platform account's synced credential store on `Mobile` and on `Desktop` under a platform account; **on `Browser`, the credential's own blob store — WebAuthn's `largeBlob` extension, which the platform syncs with the credential and which every origin whose effective domain is the credential's relying-party identifier, or a registrable-domain suffix of it, reaches — and never the page's origin storage**; and the SDK's local store on `Headless` and on a `Mobile` or `Desktop` profile under no platform account (`17-persistence-and-storage.md` §17.8). **Origin storage is the wrong home on `Browser` for two reasons a page cannot work around**: it is partitioned per origin, so two applications under one relying-party identifier read two stores and each composes a reveal believing none is pending, and §17.8 calls it ephemeral in a private window. **Where the authenticator supports no `largeBlob` the profile is per-device for this rule**, and §9.7.4.1's failure-mode table says so rather than leaving a reader to conclude the rule closed the race there. **The rule therefore observes across a controller's devices exactly where a synced store spans them and observes one device alone everywhere else**, and §9.7.4.1's failure-mode table names the profiles on which the two-device reveal race stays open. Without the rule the shipped consumer default destroys an identity with no attacker and no leaked key: the root credential and the pre-rotation credential share one platform account and the platform syncs both to every device of that account, so a controller told to roll over on suspicion signs a `CommitmentRollover` on a phone and another on a laptop, each revealing the same standing commitment under the same standing root at the same predecessor. Neither may be discarded, R6 ranks both at 1, and R7 makes a rank-1 tie terminal.

**The recovery handle** is one enum named `RecoveryHandle` whose variants are `FixesCommitment` and `Abandons`, each carrying a `RecoveryPhase` and the fields its resume consumes. The continuation is the variant tag so that `Abandons` has no next-set field by construction. `RecoveryPhase`'s five values are `Composed`, `Signed`, `PublishSent`, `Confirmed`, and `Submitted`. Both type names and all seven variant names are name-bound under the criterion the definitions state. Exactly one accessor, `load_pending_recovery`, loads a pending handle, and exactly one entry point, `resume_recovery`, consumes it by value. **`load_pending_recovery` reads every store the write duty above names for the profile and returns a handle any device of that store set wrote, at every phase from `Composed` through `Submitted`. A `resume_recovery` that completes removes the handle from every store it was written to, exactly as `abort_recovery` does**, so no completed ceremony leaves an object that refuses the controller's next reveal: a `Submitted` handle left in a synced store returns from `load_pending_recovery` on every later device, and `abort_recovery` cannot clear it, because abort is available at `Composed` alone.

**Abort** is a third entry point named `abort_recovery`, available at `Composed` alone and in no phase after the signature. **It deletes the persisted handle from every store the write duty above named**, and `load_pending_recovery` returns nothing for that identity afterwards; a handle an abort left in place would refuse every later reveal with `RevealAlreadyPending`, so the controller could never roll over again. The boundary abort respects is the signature and not the publish, because the signature is the authorizing act: a software zeroize of a signed event is not verifiable, and a surfaced copy beside a recomposed one is two reveals of one commitment and a permanent self-contest. The remedy for a wrong device-boundary establishment discovered after signing is to publish the signed event and sign a further reveal-authorized event under the commitment it fixed.
...
**The one deferral.** What waits is display, URLs, and every signed structure that takes an identifier as UTF-8 bytes, which §9.5.2 enumerates. Because that form is unfixed, no prose example anywhere in the corpus prints an identifier: an illustrative body writes `<scp-identifier:name>`, and a field whose encoding is this deferral writes `<the … identifier, in the textual form 09 §9.7.4.2 R13 defers>`. `25-test-vectors.md` §25.1 carries the one carve-out, for the fixture strings its vectors pin, and states the reason there.

**R14 — the verdicts.** A resolution returns exactly one of six verdicts of the type `ResolutionVerdict`, name-bound with its six variant names under the criterion the definitions state: `Confirmed`; `Adopted{head, baseline_decreased}`; `Contested{tie, heads, shared_prefix_head, fork_position, residue}`, whose `tie` field carries a `TieClass`, whose three variants are `Rank1Terminal`, `Rank2Terminal` and `Rank3Pending`; `Inconclusive{cause}`, whose cause type is `InconclusiveCause` with three variants, `MissingEvents{range, failed_sources}`, `CopyDefect{at_event}`, and `SingleSource{reached_sources, proven_sources, clock_skew_rejected}` — and R9's fetch bound states what each field of `MissingEvents` carries on a verifier that stopped itself; `Invalid{at_event}`; and `Discarded{accepted_head}`. A discard is a verdict so that a consumer routes it, because the resolution delivered nothing newer than the baseline.
...
**Clock model:** SCP does not require synchronized clocks. Timestamps are best-effort for message-ordering hints and replay detection, and no rule outside the two readers named below decides anything security-critical on one.
...
| `MAX_POW_DIFFICULTY` | 24 | The ceiling on the `pow_difficulty` a relay declares and on the difficulty a client solves. **It is a bound on the honest client's cost and is derived as that and as nothing else**: twenty-four leading zero bits is about sixteen million SHA-256 evaluations, about one second on a slow phone in 2026. One solution costs `2^N` hashes for every party, so an attacker pays `2^N` per identity per relay, and this registry claims nothing further about what a flood pays | §9.7.4.2 R9's proof-of-work rule, on both the relay's ceiling and the client's refusal to solve above it; the `relay_config` shape of `18-addressability-and-deployment.md` §18.3.3 |
| `EVICTION_IDLE_SECONDS` | 2,592,000 s (30 d) | The span a relay holds a retained record it was not paid to hold, without the identity re-publishing it, before the record becomes evictable. **It is measured in the relay's own clock, so no publisher sets its own window**: a multiple of the subject's `witnessing_interval` would let a party holding a threshold of a victim's root shrink that victim's window to forty minutes and set its own to fifty-six days. Thirty days: a device off for a month is a device its controller has stopped carrying, and R10's self-observation cadence refreshes every record on every cycle, so an identity that reaches this window has published nothing and fetched nothing for thirty days | §9.7.4.2 R9's eviction rule; `03-identity.md` §3.10.2 |
...
| Per-identifier storage budget, total storage budget | Off (absent) | Any value the operator sets; zero or absent means the term is off | The relay declares each term in `relay_config` and refuses a key-event PUBLISH that fails it with `StorageBudgetExceeded{scope, value}`, whose `value` carries the declared figure. The protocol fixes each term's shape and never its value, and its one floor reads the event's kind: a reveal-authorized event, and the segment carrying it, is never refused for `Identifier` or `Total` | §9.7.4.2 R9; §18.3.3 |
| `pow_difficulty` | 0 (off) | 0 to `MAX_POW_DIFFICULTY` (§9.18.17); a client refuses to solve above that ceiling and moves to the next fallback-set entry | Leading-zero-bit requirement on the four-operand puzzle §9.7.4.2 R9 states, for a key-event PUBLISH and for no other retained kind. Per relay, read from `relay_config` through ADR-004's `POLICY` query before publishing | §9.7.4.2 R9; §9.10.12; §18.3.3 |
