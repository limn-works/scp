...
| `"SCP-OUTLET-CANCEL-V1:"` | Outlet streaming cancel (`OutletStreamCancel`) invoker signature (binds context_id, outlet_id, request_id, next_seq, caveats_binding) | §5.4.5 |
...
| `"SCP-OUTLET-IKM-ROTATE-V1:"` | Ed25519 signature preimage over `InterfaceSaltRotated` events emitted at admin-removal time. Each rotation signs `SHA-256("SCP-OUTLET-IKM-ROTATE-V1:" \|\| len_be32(interface_id) \|\| interface_id \|\| len_be32(context_local_id) \|\| context_local_id \|\| len_be32(context_peer_id) \|\| context_peer_id \|\| epoch_local_be \|\| new_ikm_local \|\| len_be32(trigger_removal_did) \|\| trigger_removal_did \|\| len_be32(removal_event_id) \|\| removal_event_id)` under the signing admin's `#active` key. Binds the rotation to (a) the specific interface, (b) the ordered context pair, (c) the local epoch, (d) the fresh IKM, (e) the removed admin's DID, and (f) the event-log id of the triggering admin-removal event — denying any admin the ability to fire unwarranted rotations as a covert-channel and denying reuse of an accept-time IKM signature under this context. A separate domain separator from `SCP-OUTLET-IKM-COMMITMENT-V1:` prevents cross-preimage replay. Closes Round-6 BLOCKER-2 and MAJOR-3. | §6.2.0.1 |
...
| Outlet message-key LRU capacity | 4 entries per outlet | `MESSAGE_KEY_LRU_CAPACITY` — bounded receiver-side cache mapping `OutletRegistration` event-log id to the `outlet_message_key` pinned at that registration's acceptance. Keeps up to the four most recent registrations resolvable concurrently so in-flight `OutletError` envelopes signed under a prior registration are not silently rejected when the outlet is re-registered mid-flight. Sized by the catalog-rotation dwell (≥ 24h) — 4 × 24h = 96h covers reasonable cross-context propagation windows. | §5.4.4 |
...
| Outlet stream credit window default | 32 chunks | [1, u32 max] | `ContextParams::stream_window_default`. Initial credit on `OutletStreamOpen`. | §5.4.5 |
...
| Interface base-cost scale | Currency-dependent minimum meaningful fee (e.g., `100` cents for USD, `10,000` satoshis for BTC) | [currency_atomic_unit, 1000 × currency_atomic_unit] | `ContextParams::base_cost_scale` — scalar multiplier combined with `ceil(log2(member_count + 1))` to produce the population-weighted interface-spam floor `interface_base_cost_floor(ctx) = max(currency_atomic_unit, ceil(log2(member_count + 1)) × base_cost_scale)`. Replaces the round-5 flat `interface_base_cost_minimum`. Closes Round-6 MAJOR-5 cluster-detection bypass residual-risk: a larger context pays a proportionally larger floor on its first same-cluster interface. | §6.2.0.1 |
| Outlet-error buffer max seconds | 30s | [5, 300] | `ContextParams::outlet_error_buffer_max_secs` — maximum time an `OutletError` envelope stays in the outbound queue during the `Frozen` window of an admin-removal-induced `InterfaceSaltRotated` commit (§6.2.0.1). Overflow drops the buffered envelope with an internal audit-log entry and escalates the governance-removal timeout. | §6.2.0.1 |
