...
**Reference implementation:** the key-continuity fingerprint (§9.11, `"SCP-KEY-CONTINUITY-V1:"`) uses this exact construction and enumerates every field in place — a length prefix on each variable-length DID string, none on the six fixed-length 32-byte keys; the key-event signature preimage uses the same construction under `"SCP-KEL-EVENT-V1:"` (§9.7.4.2 R13), with its field order fixed by the substrate byte-layout amendment to this spec.
...
| 6 | `signing_key_id` | 4-byte BE length + UTF-8 bytes (`#active` or `#agent` — the DID verification method that signed this attestation) |
...
Note: the `KeyPackageAttestation` binds **all four** of the leaf's own public keys to the member's `did` (field 1), and is signed by the DID verification method named in `signing_key_id` (field 6, `#active` or `#agent` — never `#0`). The attestation must vouch for the *whole* leaf, not a subset of its keys — each of the leaf's HPKE public keys is a distinct decryption capability, and any key left unbound is one an attacker who holds only the leaf **signing** key can substitute with a key of their own:
...
**Stale document prevention:** BEP44 records include a sequence number. The client MUST reject DID documents with a lower sequence number than previously observed for the same DID. This prevents serving outdated documents. This rule governs a record on the chain the client has already accepted. Two key-event chains that diverge from a shared prefix are not comparable by sequence, and a client settles them by the fork-precedence rule of §9.7.4.2 R6, which may adopt a chain whose head sequence is lower than the head the client previously held (§9.7.4.2 R12).
...
| LeafNode credential | DID + UCAN + signing_key_id | The MLS credential field contains the member's DID, their context-scoped UCAN token, and the `signing_key_id` (`#active` or `#agent`) identifying which verification method signed the leaf's **KeyPackage attestation** (§9.7.1) — the leaf node itself is self-signed by its own ephemeral MLS key, not by a DID key (ADR-039). |
...
It is the **attestation** that is verified against the DID document — never the leaf signature itself. **Fail-closed scope (positive whitelist).** Attestation verification is MANDATORY and fail-closed for **all DIDs**: a leaf whose attestation is absent, malformed, expired, or failing any check above MUST be rejected. This applies to **every** production DID method without exception — `did:dht:z*` **and** `did:web:*` (§9.6.2) are both in scope; enforcement MUST NOT be keyed on a single method prefix such as `did:dht:z`, which would let a `did:web`-claiming leaf bypass verification. The **only** exemption is a narrow testing carve-out — mirroring the existing creator-identity validation carve-out for non-resolvable test DIDs, and **gated behind the `testing` feature so it is never present in shipped production artifacts** — which permits the `did:test:` and `did:key:` prefixes to skip DID-document resolution. No production DID method is exempt.

**Resolution failure policy — Add is fail-closed; an already-admitted member's Update gets a bounded last-known-good grace.** When the verifier cannot resolve the signer's DID document — DHT timeout, unreachable authority, `did:web` HTTPS/TLS failure, or any other resolution error — the policy depends on whether the leaf's DID is a **new member** (Add) or an **already-admitted member** (Update / their own leaf-changing Commit), because the availability trade-off is opposite in the two cases. In **both** cases a resolution *success* takes precedence: a document that resolves and shows the attesting `#active`/`#agent` **rotated away** fails check 1 and rejects — the grace below is for resolution *failure* only, never for a successful resolution that returns a new key.

- **New member (Add) — fail-closed, no stale fallback.** A resolution failure on an Add is a **REJECT** (fail-closed), never accept-if-uncertain, and the verifier **MUST NOT fall back to a stale or pre-rotation cached DID document**. Falling back would open a **rotation-bypass**: an attacker who can *induce* resolution failures (intermittently disrupting DHT/`did:web` reachability) could pin verifiers onto a **pre-rotation** cached document in which the retired `#active`/`#agent` still resolves, so old attestations continue to verify past a rotation — up to the cache TTL — defeating the revocation-by-rotation lever of §9.12. This is the cross-group **Add** path — the actual leaf-reuse threat (§9.7.3 "Scope of the PCS bound") — so it keeps the tight, fail-closed **≤ 5-min** current-key bound. Delaying a not-yet-member forks and censors nothing, so the correct availability posture is to **retry/queue the join**, not to admit on a possibly-retired key. The resolver cache is retained **only as a positive same-or-fresher optimization** for this current-key check (never as a fallback *for* a failed resolution), with a freshness TTL **no longer than `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (§9.18.7 — 300s / 5 min; check 2)** — a hard bound decoupled from, and far tighter than, the §9.10.7 DID-resolution privacy cache TTL (24h / 7d), which MUST NOT be used to satisfy the current-key check beyond this bound.
- **Already-admitted member (Update) — bounded last-known-good grace on transient failure.** An existing member replacing their own leaf is a different posture: they already passed fail-closed verification at their own Add, and their Commits carry the group forward. Here, on a transient resolution **failure** (the document cannot be fetched at all), the verifier uses the member's **last-known-good** DID document — the most recent one it successfully resolved — within a **bounded grace** (bounded by that document's own retention, §9.10.7, and always overridden by the next successful resolution) **instead of** hard-rejecting, so a transient DID-resolution outage cannot fork the epoch or censor an existing member (the BLACK-C22-10 liveness/censorship vector). This last-known-good retention bound (§9.10.7) holds **only under the Rollback-resistance assumption below**: a rollback-capable attacker who re-serves an equal-`seq` pre-rotation document under *sustained* resolution failure could keep refreshing the last-known-good entry and reset its retention timer, degrading the effective grace toward `MAX_KEYPACKAGE_ATTESTATION_LIFETIME` (§9.18.7 — the 84-day backstop is the honest upper bound); the resolution-layer mitigation is did:dht `seq` monotonicity (§9.6.1), which rejects the equal-or-lower-`seq` rollback once a higher `seq` has been observed. This grace applies **only** to resolution failure: a resolution **success** that returns a rotated key still rejects (see check 1). The grace does not weaken revocation, either: revoking an already-admitted, genuinely-compromised member is a **governance Remove** (as in MLS — the standard mechanism for evicting a compromised member), not a per-commit hard reject driven by an induced resolution outage. This presupposes a **governance authority other than the compromised member**. In a **SingleAdmin** context (§5.9) where the sole admin's full MLS state is compromised, no other party can issue the Remove; that case is a **context-re-creation** event (§5.9 governance / §5.11A migration), not a grace-recoverable one — compromise of the sole governance authority is game-over for the context regardless of this mechanism. The reuse threat the tight ≤ 5-min fail-closed bound defends against is the cross-group **Add**, which this grace does not touch.

**Rollback-resistance assumption.** The ≤ 5-min freshness guarantee (check 2) presupposes **rollback-resistant DID resolution**. An attacker who serves an **OLD but validly-signed, lower-`seq`** DID document as if it were a "fresh resolution" would return a **pre-rotation** key that the freshness check cannot catch — the document is genuinely signed, only stale, so it passes as fresh. This is out of scope for the attestation verifier; it is mitigated at the **resolution layer** — did:dht BEP44 `seq` monotonicity (§9.6.1: reject any document with a lower sequence number than previously observed) and did:web fetch integrity (§9.6.2) — which the attestation verifier **assumes**, rather than re-checking as a separate attestation check.

On key rotation (§9.7.3, §9.12), it is the **attestation** that is re-issued under the new `#active`/`#agent` key (a fresh signature over the leaf keys), not a leaf key made equal to the rotated DID key; outstanding KeyPackages carrying an attestation signed by the old key MUST be deleted from relays and replaced. Because verifiers resolve the **current** verification method only (check 1) and the resolved document must be no more than `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` old (check 2), rotating `#active`/`#agent` invalidates every outstanding attestation signed by the retired key **within `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (§9.18.7 — 5 min), not the 24h §9.10.7 privacy cache TTL** — so the effect is near-immediate but not instantaneous, because a verifier still holding a fresh-enough (≤ 5-min-old) *pre-rotation* cached document would keep resolving the retired key until that entry ages out. This strict ≤ 5-min bound governs the cross-group **Add** path — outstanding KeyPackages fetched from relays, where the actual leaf-reuse threat lives; an already-admitted member's own leaf-changing **Updates** follow the Update posture of the **Resolution failure policy** above (rotation still revokes on any resolution *success*, and genuine compromise of an admitted member is a governance Remove). This is the revocation lever exercised in §9.12.
...
**Key-status vocabulary and the content-signature rule.** The latest state-carrying event of an identity's key-event log (§9.7.4.2 definitions, R8) lists every key that has appeared on the chain as a root or an operational key, and each listed key is in exactly one of three conditions: **current** — listed in a role (`#0`, `#active`, `#agent`); **`Superseded`** — listed as a historical key whose content still verifies; or **`Compromised{from:M}`** — listed with an asserted boundary M, the sequence of the key-event log from which the controller asserts an attacker held the key, bounded M ≤ the sequence of the event that carries the entry. A key that has appeared on the chain and is absent from the latest state-carrying event has **no status**, and a verifier MUST reject every content signature by it: the default is deny, and the list is the positive whitelist of keys whose content verifies. `Superseded` is derived, not asserted: a key becomes `Superseded` when a state-carrying event lists it outside every role. The only assertion a controller makes about a key is `Compromised{from:M}`, and it withdraws a false alarm by a later state-carrying event that lists the key as `Superseded` or, if the key is still in use, in its current role. An entry identifies its key by the public key bytes; the verification-method fragment (`#active`, `#agent`) that a content signature's `signing_key_id` names (§9.8.1) resolves to a key through the latest state-carrying event, so a verifier locates a `Superseded` key by that lookup and never from a current document alone. The two kinds of signature a relying party verifies read the condition differently. A KeyPackage attestation verifies against the identity's **current** `#active` or `#agent` only (check 1 above). A content signature — an inner-envelope signature (§9.8.1) over content a key signed while it was current — verifies against a key listed as current or `Superseded`; for a key listed `Compromised{from:M}`, a verifier accepts a content signature **iff** the content is causally before the **compromise anchor** — the point in the content's context at which the `Compromised{from:M}` entry took effect — and rejects it otherwise, including when its order against the anchor is indeterminate. The anchor MUST be a value that converges across the context's honest members, so that two honest verifiers reach the same verdict on the same content — the every-field-convergent requirement the equivocation test of §9.9.3 states. How M maps onto a per-context anchor, and the exact causal-reachability rule, is not yet fixed in this spec; with no anchor fixed, no content is causally before it, so every content signature by a `Compromised` key is rejected until the mapping lands. When a later state-carrying event lists the key as `Superseded` or current, the anchor derived from the withdrawn entry is retired. Content committed after the actual compromise but before the anchor is a residual that an asserted boundary cannot recover; the entry is a root-signed judgment, not a guarantee. Rationale for rejecting indeterminate order: an attacker holding the compromised key can manufacture content at any later wall-clock time whose causal references name only pre-anchor heads.
...
- Identity Key (Ed25519): Generated in hardware security module where available (Secure Enclave, Android Keystore). Private key never exported from the secure element. Used ONLY for establishment events (§9.7.4.2 definitions). It fixes a pre-rotation commitment only inside the inception event; after inception, only a reveal-authorized event fixes a new commitment, and `#0` alone never re-commits (§9.7.4.2 R1). The identifier is the inception event's digest (§9.7.4.2 R2), not a derivation of this key, and it never changes.
- Active Signing Key (Ed25519): Generated via KeyCustody. Used for MLS KeyPackage attestations (§9.7.1), inner-envelope signatures, UCAN issuance. It does NOT sign the MLS leaf/credential directly — the leaf is self-signed by the ephemeral MLS leaf signature key (below), and the DID↔leaf binding is carried by a `#active`/`#agent`-signed KeyPackage attestation. Rotatable via DID document update signed by the Identity Key (ADR-003 §4a). The DID string does NOT change on active key rotation.
- Pre-Rotation Key (Ed25519): Generated at identity creation and again as the successor at every reveal (§9.7.4.1 items 1, 6), held in custody independent of the operational path (§9.7.4.1 item 3a). It is a one-shot authorizer: it signs exactly one reveal-authorized event (§9.7.4.2 R3) and is then spent and destroyed (§9.7.4.2 R5); it never becomes `#0` or any other key.
...
- MLS leaf signature key (Ed25519): The leaf's `signature_key` — an **ephemeral, context-scoped** key generated by the MLS layer (`SignatureKeyPair::new()`), one per group, re-generated on every Update/PCS rotation (§9.7.3). It self-signs the LeafNode and is DISTINCT from the Identity Key (`#0`), Active Signing Key (`#active`), and Agent Signing Key (`#agent`) — it is never a DID verification method and MUST NOT be expected to resolve to one. It is bound to the member's DID out-of-band by the `#active`/`#agent`-signed KeyPackage attestation (§9.7.1, §9.5.2), which is re-issued whenever this key is re-generated. Never persisted beyond the MLS group state; destroyed with the leaf on Update or group exit.
...
- Identity Key (`#0`): Changes only by a `RootRecovery` (§9.7.4.2 R3), which installs a fresh root and carries the complete post-recovery key state (§9.7.4.2 R8) — a rare operation. The identifier does not change (§9.7.4.2 R2). No migration proof exists: the recovery event is signed under `"SCP-KEL-EVENT-V1:"` like every other key event (§9.7.4.2 R13). §9.12 step 5 states the key-continuity re-verification a recovery triggers.
...
The pre-rotation key is the security backstop for the entire identity system — the last resort for recovery after root-key (`#0`) compromise. §9.7.4.2 states the events that consume it and the rule that ranks competing chains; §9.12 states the ordered recovery protocol around those events. Its custody MUST be specified with the same rigor as the Identity Key.
...
3a. **Recovery-authority residence (normative).** Item 3's storage-isolation requirement constrains *where the recovery authority resides*, not merely which provider object holds the key handle. The **recovery authority** is the minimal secret, key handle, or authorization capability sufficient to recover the pre-rotation private key. It MUST reside in a substrate whose compromise is **independent** of operational-custody compromise: there MUST NOT exist any secret, key handle, or authorization capability reachable **(directly or transitively, following any key-wrapping or key-derivation chain to its root)** from the operational `KeyCustody` provider or the daily-operations authentication flow that — alone or in combination with publicly-stored artifacts (including the encrypted-offline ciphertext of item 4) — suffices to recover the pre-rotation private key. Wrapping the key in an approved cipher (item 4) does NOT by itself satisfy this requirement: the §3 adversary is one who has already compromised operational custody, and if the decryption authority for that cipher is itself reachable from operational custody, the cipher provides zero protection against that adversary. The load-bearing property is **principal-distinctness of the recovery authority**, not the strength of the cipher wrapped around the key.
   ...
   **A reveal is not a daily operation.** A reveal-authorized event (§9.7.4.2) is a rare, separately-authorized event, not a daily operation. A distinct reveal-time authorization principal — even one that is fully automated (e.g., a separate KMS role assumed only for a rollover or recovery, which the operational path holds no grant to assume) — satisfies this requirement. What does NOT satisfy it is the daily operational signing flow itself being able to reach the pre-rotation key.
   ...
   The fail-closed-no-fallback rule above and the recovery-authority-residence property are canonical and normative here. What is NOT settled here — the per-profile conformance *realization* of this rule (the per-profile floors, which of the item-4 methods conform on which profile, and the profile-specific selection ceremony of item 5) — is PROPOSED in RFC #2130 (pre-rotation recovery custody), to be validated at execution (tracked by #1729; #1777 is closed). Items 4 (approved custody methods) and 5 (SDK presentation) below remain canonical; only their per-profile conformance filtering is the RFC #2130 realization. That re-scope MUST add post-spend destructibility (item 4's second axis) as a conformance axis, because RFC #2130 was drafted before destruction of a spent key was a protocol obligation (§9.7.4.2 R5).

4. **Approved custody methods.** Two axes rank a method: resistance to theft at rest, and **post-spend destructibility** — whether the controller can destroy every copy of a spent key (§9.7.4.2 R5) and, where the substrate supports it, obtain the §9.15 attested destruction. Any one method is sufficient **subject to the recovery-authority-residence requirement of item 3a**. Every method MUST hold an Ed25519 key that signs the raw §9.5.1 preimage of a key event (§9.7.4.2 R3). A WebAuthn/FIDO2 authenticator signs only `authenticatorData || SHA-256(clientDataJSON)` under its own counter, and Apple's Secure Enclave and Android StrongBox hold P-256 keys only, so none of those substrates can produce the signature R3 requires, and this table names none of them.
   ...
   The SDK MUST warn at custody selection that a method marked not destructible leaves the spent-key rows of the failure-mode table below open for every reveal the key later authorizes, and that the witness layer is the only other mitigation (§9.7.4.2 R5).
...
8. **Standing-key retention.** The controller MUST retain the standing pre-rotation private key — its backup under item 4 — from the moment its commitment is fixed until a reveal-authorized event consumes that commitment, on every code path. Loss of the standing key forecloses both reveal-authorized event kinds, because `#0` alone cannot fix a new commitment (§9.7.4.2 R1); the failure-mode table below states the outcome. Item 7's periodic verification is the client-level check on this obligation.

**Failure modes.** Each row names the keys the attacker holds and the outcome the rules of §9.7.4.2 produce over key material alone. Where a row says the root cannot be recovered by key material, the witness layer (§9.7.4.2 R6's Layer-B slot) MAY still resolve the divergence for a party that applies a witness policy; where no such policy applies, the person establishes a new identity and each context's admins remove the old identity and admit the new one. The social recovery of `03-identity.md` §3.3 does not apply to those rows, because §3.3 re-establishes custody of the same identity, and a contested or captured root leaves no identity to re-establish. Rank numbers refer to §9.7.4.2 R6.

- **Pre-rotation backup lost, `#0` intact.** The identity keeps operating under `#0`. The controller can sign no `CommitmentRollover` and no `RootRecovery`, because both require the pre-rotation private key, and `#0` alone cannot fix a new commitment (§9.7.4.2 R1). The SDK MUST warn the controller that root recovery is no longer possible. Item 8 is the obligation whose breach produces this state.
- **Pre-rotation backup lost, `#0` compromised.** The attacker's `#0`-signed suffix and the controller's `#0`-signed suffix tie at rank 3 (§9.7.4.2 R7), and no key event can end the tie because no party can reveal the standing commitment. The root cannot be recovered by key material.
- **Pre-rotation key copied, `#0` intact and trusted.** The attacker can sign a `RootRecovery{Lost}` (rank 2). The controller signs a `CommitmentRollover` (rank 1) as soon as it suspects the copy; the rollover wins under §9.7.4.2 R6 wherever the attacker forked, and it fixes a new commitment whose preimage the attacker does not hold.
- **Pre-rotation key copied, `#0` lost (the device is gone, not in the attacker's hands).** Both parties can sign only a `RootRecovery{Lost}`. Two such events revealing the same commitment tie at rank 2 once every signature verifies (§9.7.4.2 R3, R7); the tie is terminal by key material.
- **Pre-rotation key taken and the controller's copy lost, `#0` intact.** The attacker's `RootRecovery{Lost}` (rank 2) beats the controller's `#0`-only suffix (rank 3) and the attacker wins uncontested: relying parties see a valid, uncontested root change, not a contested verdict. Item 3a exists to keep this case from arising through operational-custody compromise; no key-material mitigation exists once the key is exclusively the attacker's.
- **`#0` compromised, pre-rotation key held only by the controller.** A controller that still holds `#0` signs a `RootRecovery{CoSigns}` (rank 1); a controller whose `#0` is lost signs a `RootRecovery{Lost}` (rank 2). Either wins under §9.7.4.2 R6 against the attacker's `#0`-only suffix (rank 3) wherever the attacker forked, and the recovery's state snapshot (§9.7.4.2 R8) replaces every install the attacker made.
- **`#0` compromised and pre-rotation key copied.** The attacker can sign every event kind, including a `CommitmentRollover` and a `RootRecovery{CoSigns}` at rank 1. A controller that still holds `#0` ties at rank 1; a controller whose `#0` is lost signs only a `RootRecovery{Lost}` at rank 2 and **loses outright** — relying parties adopt the attacker's chain. The loss is asymmetric, not a tie. The root cannot be recovered by key material.
- **`#0` compromised and pre-rotation key exclusively taken.** The attacker signs every kind and the controller none. The attacker's `RootRecovery` wins uncontested (rank 2 or 1 against rank 3): a clean, valid takeover with no contested verdict. The root cannot be recovered by key material.
- **`#0` compromised, owner recovered, and a backup of the spent pre-rotation key later reaches the `#0` thief.** The owner signed a `RootRecovery` at some position r that revealed P_prev and installed K′; the thief, who holds the superseded root K0 by the recovery's own premise, later obtains a copy of P_prev — a Shamir share set, a paper mnemonic, a cloud-store snapshot, or a copy taken before r — and forks before r with a `CommitmentRollover{P_prev, K0}` (rank 1). If the owner's recovery was `RootRecovery{Lost}` (rank 2), the thief wins outright, at any later date, and relying parties adopt the thief's chain. If the owner's recovery was `RootRecovery{CoSigns}`, the tie is at rank 1 and terminal by key material. Destruction that reaches every backup medium (item 4's destructibility axis, §9.7.4.2 R5) and the witness layer (§9.7.4.2 R6's Layer-B slot) are the only mitigations; this is the residual §9.7.4.2 names at R6.
- **A backup of a spent pre-rotation key reaches an attacker who does not hold the superseded root.** The attacker forks before the event that spent the key and signs a `RootRecovery{Lost}` revealing its commitment (rank 2). If the owner's event at that position was a `CommitmentRollover` or a `RootRecovery{CoSigns}`, the owner's rank 1 wins. If it was a `RootRecovery{Lost}`, the tie is at rank 2 and terminal by key material. §9.7.4.2 R5 destruction and item 5f destroy, and item 5d verifies, only the controller's own copies.
- **Two devices both holding `#0`, no attacker.** The two suffixes tie at rank 3 and the identity is contested with a pending tie (§9.7.4.2 R7). The controller ends the tie by signing a `CommitmentRollover` or a `RootRecovery{CoSigns}` on the suffix it keeps (§9.7.4.2 R10).
...
This section states how an identity's root key `#0` recovers from compromise and how a verifier decides between two key-event chains that diverge from a shared prefix. §9.7.4.1 supplies the recovery key's custody rules; this section states the events that consume that key and the rule that ranks competing chains. Every statement below is a verifier obligation or a controller obligation. The identity spec and the identity ADRs cite this section and do not restate its rules. The rules of this section read **key material only**; the witness layer (definitions below) supplies a second, independent axis of evidence, and R6 names the slot where a party's witness policy reads it.

**Definitions.**

- The **root key** `#0` is the identity's establishment authority. The **standing `#0`** at a position in a chain is the root the latest root-installing event at or before that position installed: the inception event installs the first, and a `RootRecovery` installs each later one.
- An **operational key** is a signing key other than `#0` — currently the Active Signing Key `#active` and the Agent Signing Key `#agent`.
- The **pre-rotation key** P is a one-shot authorizer: the chain fixes a commitment to it (R1, R2, R4), it signs exactly one reveal-authorized event, and it is then spent and destroyed (R5). It never becomes `#0` or any other key. The name is retained because §9.7.4.1 item 3a, ADR-054, RFC #2130, and the shipped custody trait carry it; this definition, not the name, governs.
- The **key state** of an identity is: the standing `#0`; every operational key by role (`#active`, `#agent`) and by custody-tier declaration; every key that ever appeared on the chain as a root or an operational key, each with a status from the vocabulary of §9.7.1 (`Superseded`, or `Compromised{from:M}`); the **witness set** — the relays the controller designates to cosign the identity's log head — and their cosigning parameters; and the **relay list** — the relays that serve the identity's log — together with the designation of the operational key that signs the identity's service record. The cosigning protocol the witness set runs is specified separately in this corpus; this section names the witness set as state and R6 names the slot its evidence fills.
...
- A **pre-rotation commitment** is `SHA-256("SCP-PREROTATION-COMMITMENT-V1:" || pre_rotation_public_key)`. The public key is a fixed-length 32-byte Ed25519 key, so under §9.5.1 it carries no length prefix. The separator is registered in §9.18.2 as a commitment-hash domain.
- The **standing commitment** at a position in a chain is the commitment fixed by the latest commitment-fixing event at or before that position — the inception event (R2) or a reveal-authorized event (R4).
- A **reveal-authorized event** carries a pre-rotation public key P whose commitment equals the standing commitment at the event's predecessor, and carries a signature over the event by P's private key.
...
- Every key event's signed preimage binds the identifier, the event's sequence, the digest of its predecessor event, the event-type discriminator, and every field a rule in this section reads (the revealed key P, the `standing_root` field, an installed root, the key-state snapshot, and the next commitment, where the kind carries them). Every signature an event carries is over that one identical preimage. In the inception event the identifier field and the predecessor-digest field are the all-zero 32-byte placeholder (R13 states the identifier's construction). Rationale: without predecessor binding, a party holding `#0` could copy the owner's genuine P-signed `RootRecovery` bytes onto its own fork at the same sequence and tie the owner at rank 2 without ever holding P. The substrate byte-layout amendment to this spec fixes the field order and widths.

**R1 — Closure: only a reveal moves the commitment.** The standing commitment MUST change only in a reveal-authorized event. A verifier MUST reject a chain in which the commitment changes at an event that reveals no preimage. Rationale: if an event signed by `#0` alone could fix a new commitment, an attacker holding `#0` would fix a commitment to a key the attacker holds, and the owner's later reveal would fail to match the standing commitment.

**R2 — Inception fixes the first commitment and the identifier is bound to the chain.** The inception event fixes the first commitment and carries the initial key state. A verifier MUST reject an inception event that fixes no commitment. A verifier MUST recompute the identifier from the inception event (R13) and MUST reject a chain whose recomputed identifier differs from the identifier under which the chain was served. Rationale: the identifier is the inception event's digest, so an inception event without a commitment would create a permanently valid identifier with no recovery path; and without the recomputation a relay could serve any self-consistent chain, including one whose inception is the attacker's own, under any identifier.

**R3 — Four event kinds; each kind fixes its signature set; every signature MUST verify.** A verifier MUST derive an event's kind from the type discriminator in its preimage, never from which signatures it carries. The kinds are `Inception`, `KeyState`, `CommitmentRollover`, and `RootRecovery`; these four names, and the status names of §9.7.1, are the protocol's identifiers, and every SDK binding carries them unchanged as its variant names. The set of discriminator values is not closed until the planned-root-move question of `03-identity.md` §3.2.1 is decided.

| Kind | Signatures the event carries | Effect |
|---|---|---|
| `Inception` | the first `#0` | installs the first `#0`; carries the initial key state; fixes the first commitment |
| `KeyState` | the standing `#0` | carries a complete key state; changes no root and no commitment |
| `CommitmentRollover` | P and the standing `#0` | fixes the next commitment; changes no root and carries no key state |
| `RootRecovery` with `standing_root: CoSigns` | P, the fresh root K′, and the standing `#0` | installs K′ as `#0`; carries the complete post-recovery key state; fixes the next commitment |
| `RootRecovery` with `standing_root: Lost` | P and the fresh root K′ | the same effect, without the standing `#0`'s signature |

A `RootRecovery` carries a required field `standing_root` with exactly two values, `CoSigns` and `Lost`, bound in the preimage. The kind together with that field fixes the signature set, so a `RootRecovery{CoSigns}` whose `#0` signature is missing or fails to verify is a copy-level defect (a required signature that does not verify), never a `RootRecovery{Lost}`: the value in the preimage, not the presence of a signature, says which set the event carries. The SDK derives the value from whether it holds `#0`, and MUST refuse with a typed error to sign `standing_root: Lost` while it holds `#0`. A `RootRecovery` also carries P's public key, K′'s public key with K′'s signature over the event as proof of possession, the complete post-recovery key state (definitions), and the next commitment (R4); P is never installed as a key, and no `#0` is ever the preimage of any commitment, because K′ is generated fresh and the commitment names a separate key. A `CommitmentRollover` carries no key state: it changes the commitment and nothing else, so one operation has one shape.

**Every signature MUST verify against the key its kind names for it**: P's signature against the revealed public key, the `#0` signature against the `#0` standing at the event's predecessor, and K′'s signature against the root the event installs. A signature that does not verify is absent, and the signature set this rule compares is the set of signatures that verify. Two classes of invalidity follow, and a verifier MUST treat them differently. A **copy-level defect** — a required signature absent or failing to verify, a surplus signature, a corrupted byte — is producible by any relay with no key, so the verifier discards that copy, treats the position as unserved under R11 (`Inconclusive`), and never demotes the suffix or records anything against the identity; an intact copy from another source supersedes the mutilated one, and events are identified by preimage digest for that purpose. An **author-attributable defect** — a revealed key whose commitment does not equal the standing commitment at the predecessor, or a violation of R4's two invariants — is signed content, so the verifier rejects the chain at that event (`Invalid{AuthorAttributable}`, R14). Authorization is the signature by P's private key and never the reveal alone: the revealed public key is public the instant the event appears, so a matching value without the signature proves nothing about who produced the event. Rationale for two reveal-authorized kinds: a controller whose pre-rotation key was copied while `#0` stays intact and trusted needs the rollover; a controller whose `#0` is compromised needs the recovery, co-signing with `#0` when it still holds it. Rationale for the fresh root: if the recovery installed P itself as the new `#0`, the live root after recovery k would be the preimage of the commitment recovery k consumed, so a thief who later took that root could re-reveal recovery k's commitment on a fork anchored before recovery k and tie the owner at rank 2 — recovery would defend against the first root theft and never a second. Rationale for deriving the kind from the discriminator and verifying every signature: if the kind were read from which signatures are present, a relay could strip the `#0` signature from a rollover so it read as a recovery and tied a thief's recovery, a thief holding `#0` could append its signature to the owner's recovery so it read as a rollover that left the thief's root in place, and a thief holding only a copy of P could mint a rollover with a junk `#0` slot that a presence check would rank 1. What this pair of kinds does not close: an owner who recovered with `standing_root: Lost` remains exposed to a `#0` thief who later obtains a backup of the spent P (the failure-mode table of §9.7.4.1, and R6's Layer-B slot).

**R4 — Every reveal-authorized event fixes the next commitment, under two closed invariants.** The controller MUST generate the next pre-rotation key in a substrate satisfying §9.7.4.1 item 3a before it signs the event (R10 states when). A verifier MUST reject a reveal-authorized event that fixes no successor commitment. Two invariants hold over the whole chain, and a verifier MUST reject the event that breaks either: **key uniqueness** — a public key appears on a chain in at most one event and in one role (an installed root, an installed operational key, or a revealed P); and **commitment freshness** — a fixed commitment equals no commitment fixed earlier on the chain and equals the commitment of no key that appears on the chain. Rationale: a chain whose standing commitment has been consumed and not replaced has no recovery path, and a follow-on event to fix the next commitment would have to be signed by `#0` alone, which R1 forbids; a key that reappears (K′ equal to P, K′ equal to a prior root, a root re-used as an operational key) or a commitment that repeats or names a present key would let a spent or stolen key satisfy a later reveal, which breaks the uniqueness R6 relies on.

**R5 — The controller destroys the spent pre-rotation key.** After the controller has retained the signed event (R10's retention invariant), the controller MUST destroy P's private key from every custody location and backup medium it controls, for a `RootRecovery` and a `CommitmentRollover` alike, and MUST NOT destroy it before that retention. R5 reaches only the controller's own copies: a Shamir share held by a contact, a paper mnemonic already photographed, and a cloud key store's historical snapshot are outside its reach (§9.7.4.1 item 4's destructibility axis). Rationale: a spent key that survives can sign a second reveal of a commitment already consumed and tie or, when its holder also holds the superseded root, defeat the identity at that fork (R7, the failure-mode table). Destruction is the only key-material mitigation for that class; the witness layer supplies the other (R6).

**R6 — Fork precedence over key material: the first event that consumes the shared prefix's standing commitment decides, and a witness policy fills the slot key material leaves open.** When a verifier holds two chains that diverge from a shared prefix, it MUST rank them as follows. Let C be the standing commitment at the shared prefix. On each divergent suffix, the verifier locates the first event that reveals C's preimage; at most one such event exists per suffix, because R1 makes C unique on the shared prefix, a reveal consumes it, and R4 forbids fixing it again. The verifier ranks each suffix by that event's kind:

1. the suffix carries a `CommitmentRollover` or a `RootRecovery{CoSigns}` that reveals C — the kinds whose set includes the `#0` standing at the shared prefix (Pin A);
2. the suffix carries a `RootRecovery{Lost}` that reveals C;
3. the suffix carries no event that reveals C.

The suffix with the lower rank number wins, and the winning chain supersedes the losing chain in its entirety, whatever the two head sequences are, because a chain is the unit of authority and no hybrid of two chains is a valid log. A verifier MUST NOT decide a divergence by head sequence, by the first event after the shared prefix, or by the highest rank found anywhere on a suffix. When a verifier holds more than two chains, it ranks every pair against the standing commitment of that pair's own shared prefix; the winner is the chain that wins every pairwise comparison it is party to, and the identity is contested only when no such chain exists (R7).

- *Pin A.* Rank 1 names the `#0` standing at the shared prefix — the key whose verified signature a rollover or a `RootRecovery{CoSigns}` carries — because each suffix may have installed a different root after the fork.
- *Pin B.* Once a suffix carries a C-revealing event, its rank is fixed and does not change when the suffix is extended. A suffix with no C-revealing event is rank 3 until one appears (R7's pending tie). A reveal on a suffix that consumes a commitment the suffix itself fixed confers no rank, because the shared prefix never committed to that key.

**This rule reads key material only, and its ranks are a static order over a signal whose meaning changes after a recovery.** A `RootRecovery` leaves its superseded root in an adversary's hands by its own premise, so after every recovery the signature of the `#0` standing at the pre-recovery prefix is evidence for the adversary as well as for the owner; the rank order that protects the copied-P case (rank 1 over rank 2) is the order that exposes the superseded-root-plus-spent-P-backup case, and the reverse order would expose the copied-P case. No static order over key material satisfies both, and the failure-mode table of §9.7.4.1 names the residual. The witness layer is the axis that does: a witness that cosigned the owner's head refuses to cosign a fork anchored behind it under its consistency rule. **A party applying a witness policy over a set it recognizes as independent of the `#0` holder MAY, on a Layer-A tie or on the residual case, adopt the suffix that set has cosigned; a party applying no such policy holds the key-material verdict.** Detection by the witness layer is the mandatory base of that layer; a witness gate is a relying party's opt-in policy and never a condition of an identity's resolvability. The witness protocol itself is specified separately in this corpus.

Rationale, stated as the three comparisons this rule replaces: a comparison of the first event after the shared prefix lets the party that chooses the fork point choose which events are compared, so an attacker who forks among the owner's ordinary rotations ties or wins before the owner's recovery is read; a comparison of the highest rank found anywhere on a suffix lets a party whose recovery installed its own root and its own commitment mint a rollover on its own suffix and tie the owner; a comparison of head sequences lets the party that appends the most events win.

**R7 — Ties and the contested verdict.** Equal rank resolves no winner over key material, and the identity is **contested**. The tie's class is a function of the tied rank: a tie at rank 1 or rank 2 is **terminal by key material** — C is consumed on both suffixes, so no key event on either suffix changes the comparison; a tie at rank 3 is **pending** — neither suffix has revealed C, and a later C-revealing event on either suffix decides the divergence under R6. A party's witness policy MAY resolve a tie of either class for that party (R6's slot). For a contested identity a verifier MUST return the `Contested` verdict (R14), which carries the tied rank and the digests of every tied head; MUST NOT adopt any tied suffix as the current key state; MUST NOT return the shared prefix's key state as the current key state; and MUST retain every tied head and suffix as evidence. Criterion for what a tie proves: a rank-2 tie proves that the pre-rotation private key signed two divergent events; a rank-1 tie proves that both the pre-rotation private key and the `#0` standing at the shared prefix signed two divergent events; a rank-3 tie proves that `#0` signed two divergent suffixes. Indicator, not criterion: two parties holding the key is the usual cause of a double signature, and one holder with two devices also produces one. When R6 later resolves a divergence that a verifier had recorded as a rank-3 tie, the verifier MUST retire that equivocation record and MUST NOT count it against the identity. Rationale for refusing the shared prefix's key state: if `#0` was compromised before the fork point, the shared prefix already carries the attacker's events, so serving its key state as current would serve a hostile key state.

**R8 — Key state is the latest state-carrying event; nothing is voided.** The derived key state at any position IS the key state the latest state-carrying event at or before that position carries; no earlier event contributes, and a verifier applies no diff, no undo, and no voiding. A `RootRecovery` therefore replaces the entire key state in one event: every operational key the attacker installed, every status the attacker asserted, every witness-set and relay-list change the attacker made, is absent from the recovery's snapshot and so is not current after it. **The default is deny (§9.7.1):** a key that appears on the chain and is absent from the latest state-carrying event has no status, and a verifier rejects every content signature by it; the controller lists its own historical keys as `Superseded`, and a key it forgets to list has its content rejected — the fail-safe direction, corrected by a later state event. The recovery's operational-key set replaces the derived set in full: an operational role the snapshot does not name is not current after the recovery. Each `Compromised{from:M}` entry carries its own boundary M, bounded M ≤ seq(the event that carries it); a verifier MUST reject a state-carrying event whose entry names an M above its own sequence. M is a content boundary only (§9.7.1): an M set too high leaves the attacker's content under that key accepted, and an M set too low rejects the controller's own content under it. A `CommitmentRollover` carries no key state and changes the derived state in no way. Chain validity under R1–R4 is evaluated over the full chain with every event present. Rationale: an attacker holding `#0` who appends `KeyState` events to the one genuine log produces no divergence, so R6 never runs; a rule that voided the attacker's events by boundary needed an expanding list of exemptions (reveal-authorized events, commitment fixes, the controller's own statuses and witness set, the relay list) and re-opened for each of them the window it was written to close, and one recovery can express `#agent` compromised at 30 and `#0` at 50, which a single boundary could not. A state-carrying event costs O(keys) bytes — a few kilobytes on a log whose key events are rare — and the relay list becoming key state is a consequence of the snapshot: the DID document of the identity spec is a projection of the latest state-carrying event, not a separately signed artifact.

**R9 — Retention of evidence.** A verifier MUST NOT bound the suffix it retains at its last reveal-authorized event, because a party holding a spent pre-rotation key can anchor a fork before that event (R5), and the comparison then reads earlier events. Indicator, not criterion: key-event logs are short, so a verifier that retains its full accepted chain satisfies R11's evidence obligation with no fetch, and a verifier that retains only a head digest re-fetches its own chain from a relay, which cannot forge a self-certifying chain but can withhold it.
...
| The controller… | signs | rank under R6 |
|---|---|---|
| holds `#0` and must retire it (any `#0`-signed event on its chain that it did not author, fork or not; or any compromise it asserts) | `RootRecovery{CoSigns}` | 1 |
| holds `#0`, trusts it, and suspects only the pre-rotation key copied | `CommitmentRollover` | 1 |
| has lost `#0` | `RootRecovery{Lost}` | 2 |

The SDK MUST treat any reveal-authorized event on its identity's chain that it did not sign, and any `#0`-signed event it did not author, as evidence of compromise and alert at maximum severity; and it MUST be able to extend a suffix the controller authored even after its own verifier adopted a competing chain, because the controller's answer to a hostile adoption is an event on its own suffix. A controller SHOULD roll over on a schedule, because a standing copy of P converts a later recoverable `#0` theft into a rank-1 tie.

*Device boundary.* A reveal-authorized event MUST be produced on a device, and its successor keys — K′, the operational keys the snapshot installs, and the next pre-rotation key — generated in custody, that the controller has established the asserted compromise did not reach; a controller that cannot establish this MUST NOT proceed. The controller MUST confirm, on the substrate that holds P or on an independent device, that the event P is about to sign carries the commitment of the key it just backed up and the K′ it generated, because the compromised path can otherwise assemble an event whose next commitment names a key the attacker holds and present it to P, which signs whatever it is shown. The SDK's `RootRecoveryConfig` names that custody as a required field with no default and no fallback to the daily provider; §9.12's "trusted device" is defined by this paragraph.

*Ceremony order.* Generate the next pre-rotation key under §9.7.4.1 items 1, 3a, 4, and 5b–5d; for a `RootRecovery`, generate K′ and the operational keys under the device boundary; sign the event — P signs where P resides and is never imported into operational custody; persist the recovery handle (below); publish; retain; destroy; submit to the witness set. **Retention is the invariant that makes destruction safe:** the controller MUST durably retain the byte-identical signed event as the head of its own chain BEFORE it destroys the spent key (R5), because once retained it can republish the event without P, and publication is therefore repeatable and gates nothing but R5's ordering. **Confirmed publication** is durable self-retention together with acceptance of the event by at least one relay in the identity's relay list AS THE EVENT'S OWN SNAPSHOT STATES IT (never a relay that only a superseded `#0` introduced) or by the DHT; the controller MAY publish to any additional relay at any time. The witness set the controller submits to is the set the event's snapshot names; witness submission never gates destruction, because an identity's resolvability never depends on a witness (the witness layer detects; a witness gate is a relying party's opt-in policy).

*The recovery handle.* The controller MUST durably persist a typed recovery handle BEFORE it publishes the event, and phase 1 begins only after that persist. The handle is a tagged union with one variant per phase — signed but published to no relay; published but the spent key not yet destroyed; destroyed but not yet submitted to the witness set — and each variant carries only what its resume consumes: the byte-identical signed event in every phase, the spent key's custody handle in the first two, the next pre-rotation key's custody handle, and the expected next commitment so a resume re-checks it. Exactly one named accessor loads a pending handle after a restart, and exactly one resume entry point consumes the handle by value and continues from its phase without re-signing. Rationale: the reveal is irreversible once the event is public; a handle that is surfaced and not persisted is lost to a crash between signing and surfacing, leaving a spent P live with no signal; a flat handle admits invalid states (a spent-key handle in a phase where the key is gone); a resume that re-derives a key or re-signs produces artifacts that differ from the ones already published.

**R11 — Divergence is a positive finding.** A candidate head is **on the accepted chain** when either chain is a prefix of the other. A candidate head is **divergent** when the verifier holds both chains back to a common ancestor and they diverge (definitions). A candidate that is neither — because the events linking it to the accepted chain were not served, or because a served copy failed verification as a copy-level defect (R3) — is **inconclusive**: the verifier holds its baseline, records nothing, and returns the `Inconclusive` verdict (R14), which names the candidate head, the accepted head, the missing sequence range, and every source that failed to serve the link; before returning it the verifier MUST have tried at least one source that the candidate's own chain does not name, because a relay named only by the candidate is chosen by the candidate's author. A verifier MUST record equivocation evidence only from two chains it holds in full back to their common ancestor, and only when R6 resolves them as a tie (R7). Rationale: a single relay that withholds linking events, or serves a mutilated copy, could otherwise cause a verifier to record a durable verdict against an honest identity, and a relay list the candidate's author controls could make the withholding permanent.

**R12 — Sequence governs the accepted chain only.** For a candidate head on the accepted chain, a verifier MUST reject a head at a sequence strictly lower than the highest it has accepted (`RejectedStale`, R14), and an equal sequence with an identical head confirms the baseline (`Confirmed`). A candidate whose chain is not a prefix of the accepted chain and of which the accepted chain is not a prefix is classified under R11 and decided under R6 and R7. A divergent candidate's head sequence decides nothing. Adopting an R6 winner MAY lower the accepted head sequence (`Adopted{baseline_decreased}`), and a verifier's stored baseline MUST be able to express that decrease. Rationale: a monotone maximum cannot record the adoption of a lower-sequence winner, and a verifier that stored only a maximum would discard the outcome R6 reached.

**R13 — Domain separators and the identifier's construction.** The key-event signature preimage uses `"SCP-KEL-EVENT-V1:"` under the §9.5.1 construction, and the first field after the separator is the event-type discriminator byte, because a verifier derives the kind before it parses the fields the kind carries. The pre-rotation commitment uses `"SCP-PREROTATION-COMMITMENT-V1:"` as a commitment-hash domain. The **identifier** is `SHA-256("SCP-KEL-ID-V1:" || inception_signed_preimage)`, where `inception_signed_preimage` is the inception event's complete signed preimage with its identifier and predecessor-digest fields set to the all-zero placeholder (definitions); a verifier recomputes it under R2. The identifier separator is distinct from the event separator so that no signature preimage's digest is ever an identifier and no identifier is ever a digest a key signed: the two constructions cannot be confused for one another at any verifier. All three separators are registered in §9.18.2.

**R14 — The verdict taxonomy.** A resolution returns exactly one of the following, and every rule above cites its variant; every SDK binding carries the six names unchanged. `Confirmed` — the candidate is the accepted head (R12). `Adopted{head, baseline_decreased}` — the verifier adopted a new head, on the accepted chain or by R6, and the flag says whether the accepted sequence decreased (R12). `RejectedStale` — a head on the accepted chain at a sequence strictly lower than the accepted head's (R12). `Contested{rank, heads}` — equal rank under R6, with the tied rank and the digests of every tied head; the tie's class (pending for rank 3, terminal by key material for ranks 1 and 2) is derived from the rank (R7). `Inconclusive{candidate, accepted, missing_range, failed_sources}` — the candidate could be neither placed on the accepted chain nor shown divergent (R11). `Invalid{at_event, cause}` — the chain fails R1–R4 at a named event, with `cause` one of `AuthorAttributable` or `CopyLevel` (R3). The contested verdict, the retained suffixes it rests on, and the accepted-head baseline MUST survive the verifier's restart and MUST be persisted before any verdict is acted on; a verifier that loses them re-seeds from whatever a relay serves, which is the fail-open direction across a restart.

**Worked examples.** Notation: e0 is the inception event, which installs K0 as `#0` and fixes commitment C over the pre-rotation key P; a number is a sequence; the owner is the legitimate controller.

1. **A `#0` thief forks early; the owner's device holding K0 is gone.** The owner's chain is e0 through e9 (K0-signed `KeyState` events). The thief, holding K0, forks at e5 and appends a5 through a50, all K0-signed. The owner no longer holds K0, so it signs a `RootRecovery{Lost}` at e10 that reveals C with P's signature, installs a fresh root K′, carries a complete key state, and fixes C′. The shared prefix is e0 through e4 and its standing commitment is C. The owner's suffix carries e10, which reveals C, so its rank is 2. The thief's suffix carries no C-revealing event, so its rank is 3. The owner's chain wins under R6 wherever e10 sits, and the thief's head sequence 50 decides nothing. Under R8, the derived key state after e10 is e10's snapshot; nothing the thief installed on the shared prefix survives, because the snapshot does not list it.
2. **A copied pre-rotation key, thief forks early.** The owner's chain is e0 through e60 with K0 intact and trusted. The thief holds a copy of P, forks at e10 with a `RootRecovery{Lost}` a11 (rank 2, installing a root the thief holds), and appends to a50. The owner signs a `CommitmentRollover` at e61 that reveals C and carries K0's signature. The shared prefix is e0 through e10. The owner's suffix carries e61, so its rank is 1. The thief's suffix carries a11, so its rank is 2. The owner wins under R6 wherever e61 sits. The rollover fixes a new commitment whose preimage the thief does not hold.
3. **`#0` compromised and the pre-rotation key copied; the owner still holds K0.** The owner's chain is e0 through e30, and a thief holding both K0 and a copy of P forks at e12 with a `RootRecovery{Lost}` a13. The owner, who still holds K0, signs a `RootRecovery{CoSigns}` at e31 that reveals C, installs K′, and carries K0's signature. The shared prefix is e0 through e11. The owner's suffix carries e31, rank 1; the thief's suffix carries a13, rank 2; the owner wins. The thief, holding both keys, answers with a `CommitmentRollover{P, K0}` or a `RootRecovery{CoSigns}` on a fork anchored at e12: both suffixes now carry a rank-1 event revealing C, the tie is at rank 1 and terminal by key material (R7), and only a party's witness policy resolves it (R6's slot) — the "`#0` compromised and pre-rotation key copied" failure mode.
4. **Two `#0`-only suffixes.** The owner's two devices both hold K0. Device A appends a `KeyState` e10a and device B appends a different `KeyState` e10b. The shared prefix is e0 through e9. Neither suffix reveals C, so the tie is at rank 3 and pending (R7). The owner, holding P, signs a `CommitmentRollover` e11a on device A's suffix; that suffix is now rank 1 against rank 3, and it wins. A thief holding K0 alone could not have ended the tie. An owner who has lost P faces a permanent rank-3 tie, and the root cannot be recovered by key material.
5. **A backup of a spent key later reaches the `#0` thief.** The owner's chain carries a `RootRecovery{Lost}` r20 that reveals an earlier commitment C_prev over P_prev and installs K′, because K0 was stolen and the owner's device holding it was gone; the chain continues to e40. The thief, who holds K0 by r20's own premise, later obtains a backup of P_prev — a Shamir share set the owner's contacts still hold, a paper mnemonic, or a cloud-store snapshot — and forks at e19 with a `CommitmentRollover{P_prev, K0}` a20. The shared prefix is e0 through e19, and its standing `#0` is K0 and its standing commitment C_prev. The owner's suffix carries r20, a `RootRecovery{Lost}`, rank 2. The thief's suffix carries a20, a rollover co-signed by the shared prefix's `#0`, rank 1. Over key material the thief wins outright and relying parties adopt the thief's chain, at any later date. Had the owner held K0 at r20 and signed `RootRecovery{CoSigns}`, the tie would be at rank 1 and terminal. This is the residual R6 names: destruction that reaches every backup medium (§9.7.4.1 item 4's destructibility axis, R5) and a party's witness policy over a recognized-independent set (R6's slot — a witness that cosigned e40 refuses a fork anchored at e19) are the mitigations; key material alone has none.
...
**Inner check 1 — Ed25519 identity signature.** The sender signs the payload with their Active Signing Key (`#active`) or Agent Signing Key (`#agent`) (see ADR-003 §4a, ADR-039). The signature is over a domain-separated, length-prefixed canonical hash: `SHA256("SCP-INNER-ENVELOPE-V1:" || version || message_type || len(context_id) || context_id || len(sender_did) || sender_did || epoch || generation || sequence || timestamp || len(payload_hash) || payload_hash || len(provenance_hash) || provenance_hash || len(signing_key_id) || signing_key_id)` where `payload_hash` covers the original plaintext (before padding), `provenance_hash` covers serialized provenance metadata (or `SHA256(0x00)` if absent), and `signing_key_id` is the `#active`/`#agent` verification-method fragment the sender signed under (ADR-039). Because `signing_key_id` is inside the signed preimage, a tampered or forged verification-method claim invalidates the signature: the verifier resolves the public key for the *declared* `signing_key_id` from the sender's DID document, and the signature can only verify if that VM's private key actually signed — preventing cross-VM (`#active`↔`#agent`) confusion. Including `version` and `message_type` (a discriminator byte) prevents downgrade and type-flipping attacks. Processing order: hash plaintext -> hash provenance -> sign -> pad -> sender-key encrypt -> MLS encrypt. Reverse on receipt: MLS decrypt -> sender-key decrypt -> strip padding -> resolve sender VM key by `signing_key_id` -> verify signature -> verify payload_hash -> verify provenance_hash. A failed signature means the envelope was tampered with or forged and MUST be rejected.
...
- **Public-record format (not a metadata-privacy protection):** the DID-record relay frame (§9.10.12) is the *format* for the unencrypted, self-certifying DID document record — the structural counterpart to the encrypted outer envelope (§9.10.2). It does **not** conceal metadata: a DID record is a public record that exposes the DID, its keys, and its relay endpoints in the clear by design. It is catalogued alongside the privacy layers above only because it is a relay-stored record format; it addresses no attack surface in the privacy sense.
...
### 9.10.12 Relay DID-Record Frame

The **DID-record frame** is the unencrypted counterpart to the §9.10.2 Minimal Outer Envelope: where the outer envelope is *confidential by encryption*, a DID record is *self-certifying by signature*. It carries a DID document (§3.10.2) as a public relay record. It is specified here in §9.10 because §9.10 is where relay-stored record formats live: the outer envelope (§9.10.2) is the encrypted format, and this is the public-record format. This design is issue #482. (See also §9.18.11 Transport and Relay, which fixes the shared relay blob-size and blob-TTL bounds this frame reuses.)

The frame is **minimal and DID-specific — there is no magic tag and no record-kind byte.** DID records live at their own `routing_id` domain (`SHA-256("scp:did:" || did_string)`, §3.10.2), so the address is the type discriminant; a frame needs no self-describing tag to say what it is. (A future MLS KeyPackage relay record, if built — issue #2202 — would define its OWN minimal frame at its OWN `routing_id` domain; DID records and KeyPackages do NOT share a tagged multi-kind envelope family, and this frame introduces no such taxonomy.)

**Storage model — a raw relay blob.** A DID record is stored as a **raw relay blob** at its routing ID. It is **NOT** wrapped in an `OuterEnvelope`, and its bytes are **NOT** MLS-encrypted — it is a public, self-certifying record whose authenticity comes from its BEP44 signature, not from encryption. Publication and resolution use the existing PUBLISH/QUERY operations (ADR-004) verbatim — no new wire types. `ClientMessage::Publish { routing_id, blob_ttl, blob }` writes the frame bytes at the DID routing ID, and `RelayMessage::Blob { …, blob }` returns them.

**Relay-side validation (SCP-native relays, OPTIONAL).** Unlike an opaque context blob, a DID record is a public, self-certifying record an SCP-native relay MAY validate on PUBLISH. The whole path sits behind the existing per-IP PUBLISH rate limit (ADR-004) and runs **cheapest-first**, so junk is rejected before any Ed25519 verify. A validating relay:

1. **Structurally decodes** the blob as a DID-record frame. A blob that does not decode is not a candidate DID record (it is governed by slot-exclusivity, below).
2. **Confirms the DID→routing_id binding** `SHA-256("scp:did:" || did(public_key)) == routing_id`, where `did(public_key)` is the `did:dht` string derived from the frame's `public_key` (§9.6.1). A frame whose `public_key` does not hash to the `routing_id` it is published at is rejected. This binding is the discriminant: a validating relay recognizes a DID record by the frame decoding *and* the binding holding — it needs no new wire type and no a-priori knowledge of `routing_id` semantics, and a context `OuterEnvelope` (whose first byte is a MessagePack map marker, never `version = 1`, and whose bytes do not hash a `public_key` to a context pseudonym) is never mistaken for one. Being a plain hash, it runs **before** the signature verify.
3. **Verifies the BEP44 signature** over `bencode(seq, value)` against the frame's `public_key` — only for a blob that passed steps 1–2, so a mis-addressed or non-frame blob never costs an Ed25519 verify.
4. Keeps a **single highest-sequence slot** per `routing_id`: rejects a frame whose `seq` is `≤` the stored slot's `seq` **unless** an equal-`seq` frame is byte-identical (idempotent TTL refresh), and replaces the slot only on a strictly-higher valid `seq`.

**Slot-exclusivity.** The base relay stores multiple opaque blobs per `routing_id` with no per-`routing_id` cap (ADR-004), so validating only frame-shaped blobs would leave non-frame or pre-seeded junk sitting alongside the slot and returnable by QUERY. A validating relay therefore makes a `routing_id` **slot-exclusive** once a binding-valid frame first establishes a slot there: (a) it rejects any later PUBLISH there that is not a binding-valid, `seq`-advancing frame; (b) it evicts any pre-existing opaque blobs at that `routing_id` when the slot is first established; (c) QUERY there returns only the single slot; (d) it **rejects a DELETE of the current slot blob** — only a superseding PUBLISH may replace a slot. Rule (d) is **storage-derived, not index-derived**: because DELETE addresses a blob by `blob_id` and the in-memory slot index is a cache that a relay restart or a store-sharing peer leaves cold, the relay decides rule (d) by re-reading the blob at `blob_id` and rejecting the DELETE if it decodes as a binding- and signature-valid DID-record frame (self-certifying, so protection is reconstructible from the bare bytes) — immune to a cold index. The DELETE gate runs behind the per-IP rate limit and fails closed on a storage read error (§3.10.8 "Attacker DELETEs the slot blob"). Before the first valid frame the relay cannot recognize the `routing_id` as DID-domain (`SHA-256` is one-way), so pre-seeded junk simply sits as ordinary opaque blobs until the first valid publish establishes the slot and evicts it (§3.10.2). This is a relay **storage** behavior; the relay-storage mechanics are transcribed in the companion **ADR-004 "DID-Record Slot-Exclusivity" subsection** (implemented under #482 / SCP-RELAYRES-003). This section and §3.10.2/§3.10.8 are authoritative for the behavior and its threat model.

This mirrors, and extends from the control plane to a stored public record, the exact check `BRIDGE_REGISTER` already performs — an Ed25519 signature plus the same `SHA-256("scp:did:" || did_string) == routing_id` binding (§10.12.4). It is what Mainline DHT BEP44 nodes already do for mutable items. Relay-side validation is an **OPTIONAL capability**: the protocol MUST NOT require a validating relay. Foreign transports and adapters that cannot validate store the frame opaquely; resolution stays correct via client-side verification, the DHT, and multi-relay publishing. Validation is an **availability and anti-suppression measure, never a trust dependency** — the resolver re-verifies every record independently (see "Framing is outside the signed authority" below), so a relay that skips, botches, or lies about validation degrades availability only, never integrity.

**Why raw binary, not MessagePack/CBOR.** The frame uses a raw, fixed-layout binary encoding under the §9.5.1 length-prefix discipline rather than a self-describing codec (MessagePack, CBOR). Self-describing codecs admit multiple valid encodings of the same logical value (map-key ordering, integer width, string-vs-binary tags), which would (a) break byte-identical cross-binding decoding — two SDKs could emit different bytes for the same record — and (b) perturb the exact `value` bytes handed to BEP44 verification, since the signed payload must be reproduced octet-for-octet. The frame therefore fixes exactly one canonical encoding.

**Frame layout:**

```
DID-RECORD (DidRecordV1) :=
  version:    u8         = 1                # frame version
  ...
  value:      [u8]                          # trailing remainder = BEP44-signed payload = encoded DID document (opaque; owned by did:dht §18.2.2A)
```
...
- **public_key** — the identity's raw 32-byte Ed25519 public key. It is carried **for the relay's benefit**: a relay holds only the one-way `routing_id` hash and cannot recover the DID or its key from it, so — exactly as `BRIDGE_REGISTER` carries `public_key` for the relay to verify against (§10.12.4) — the frame supplies it for the relay's signature + binding check. The **client ignores this field for trust** and verifies with the key it derives from the DID string itself (§9.6.1).
- **seq** / **signature** — the BEP44 sequence and signature, written raw with no length prefix (fixed-width fields, §9.5.1).
- **value** — the sole variable-length field, carried as the **trailing remainder** of the frame (no `value_len` prefix). Because every preceding field is fixed-width, `value` is unambiguously `frame_bytes[105..]`; a length prefix would be redundant with the blob's own length and a determinism footgun (two disagreeing lengths). `value` MUST be non-empty and its length MUST NOT exceed `Max blob size` (262144, §9.18.11) `− 105`.

The fixed prefix is `1 + 32 + 8 + 64 = 105` bytes, and the total frame length is `105 + len(value)`. There is deliberately **no** `sig_len` field: a BEP44 Ed25519 signature is unconditionally 64 bytes, and §9.5.1 gives fixed-width fields no length prefix; a different signature length would be a new version, not a per-frame field.

**Framing is outside the signed authority.** The BEP44 signature covers only the **BEP44-canonical bencoded buffer `bencode(salt?, seq, value)`** — the sequence number precedes the value, serialized as `3:seqi<seq>e1:v<value>` (with an optional preceding `4:salt<salt>` where the DID method uses one; did:dht uses none). **BEP44 (BitTorrent BEP 44) is authoritative for this field ordering** — `seq` before `value` — and every SCP binding MUST reproduce that exact octet sequence, since the signature is hand-rolled across bindings and must be byte-reproducible. The frame's `version`, `public_key`, and its field boundaries are unsigned transport framing. A decoder MUST NOT derive any security-relevant conclusion from the framing bytes — in particular, the **client MUST NOT trust the frame's `public_key`**. After decoding a frame into `(value, signature, seq)`, the resolver MUST BEP44-verify the triple against the Ed25519 key encoded in the DID string being resolved (§9.6.1) **before** any use; a frame whose triple does not verify against *that* key is discarded exactly as an invalid DHT record is (§3.10.4). Because the routing ID is derived from the DID string while verification is against that DID's key, record substitution is cryptographically impossible (§3.10.8). (A validating relay verifies against the frame's `public_key` — appropriate for the relay, which has no other source of the key — but the relay's acceptance is never a client trust input.)

**Decoder determinism (normative).** Because the frame is decoded by hand-rolled, byte-identical decoders across every binding, a conformant decoder MUST enforce the following. Any frame that fails any rule is discarded exactly as an invalid DHT record is (§3.10.4) — never trusted, never partially parsed:
...
2. **Require the fixed prefix in full.** A decoder MUST reject any frame shorter than the 105-byte fixed prefix (`version + public_key + seq + signature`) — truncation is never a partially-valid frame.
3. **Bound-check the `value` length only after the prefix check.** `value` is the trailing remainder, `len(value) = total_frame_len − 105`. A decoder MUST NOT compute `total_frame_len − 105` before rule 2 has confirmed `total_frame_len >= 105`: computing it first can underflow, which diverges between a debug-build panic and a release-build wrap across bindings. With rule 2 satisfied the subtraction cannot underflow. A decoder MUST reject an empty `value` (`total_frame_len == 105`) and MUST reject `len(value) > Max blob size (262144, §9.18.11) − 105`. No widening is required: `len(value)` is an actual buffer length (a valid `usize`, already bounded by the transport's Max blob size) and the bound is a compile-time constant — there is no wire-supplied length field to overflow (unlike a length-prefixed layout).
4. **Decode-and-verify at exactly one site.** Decoding a frame and BEP44-verifying its `(value, signature, seq)` triple happen at exactly **one** site (mirroring SCPM's single decode-and-verify site, §9.16.1). No other layer may test, branch on, or depend on the framing bytes.

**Publish / query contract.**

- **Publish** = wrap `(public_key, seq, signature, value)` in a DID-record frame and PUBLISH the frame bytes at the DID routing ID via the existing PUBLISH operation (§3.10.5). The DID relay blob MUST be the DID-record frame — not the bare DID-document bytes, and not an `OuterEnvelope`.
- **Query** = the existing QUERY operation (§3.10.4) with `limit: N` (N = 16) returns the blob(s) stored at the routing ID; the resolver decodes each at the **single** decode-and-verify site into `(value, signature, seq)` and then BEP44-verifies against the DID-derived key. Against a validating relay the routing ID is slot-exclusive, so one record is returned regardless of `N`; against non-validating or foreign storage `limit: N` lets the resolver retrieve up to N candidates and sift them to the highest-seq *valid* one (§3.10.4 step 5). See §3.10.2 for why `limit: N` dominates `limit: 1`.
- **Distinct from the outer envelope.** A DID routing ID carries DID-record frames, not `OuterEnvelope`s; a resolver MUST NOT deserialize one as the other. The two are disjoint at two independent levels: **(a) routing-ID space** — DID records occupy `SHA-256("scp:did:" || did_string)` (§3.10.2), disjoint from context routing IDs; **(b) byte level** — a DID-record frame begins with `version = 0x01`, whereas an `OuterEnvelope` is MessagePack-serialized as a map (`rmp_serde::to_vec_named`) whose first byte is always a map marker (fixmap `0x80`–`0x8f`, or `0xde`/`0xdf` for larger maps) — never `0x01`. A DID-record frame therefore can never be mistaken, byte-for-byte, for an `OuterEnvelope`, or vice versa — a defense-in-depth backstop beneath the routing-ID separation.

**BEP44 byte-identity across layers (§3.10.5).** The frame wraps `value` for relay transport only; it never enters, reorders, or alters the signed bytes. A DHT-native `(value, signature, seq)` triple and the same triple decoded from a DID-record frame are byte-identical and verify identically. The §3.10.4 rule that two valid records sharing a sequence number MUST be byte-identical holds across both layers.

**TTL.** DID records reuse the shared relay `blob_ttl` (§9.10.2), bounded by `Max blob TTL` = 604800s / 7d (§9.18.11). There is no record-specific TTL and no change to `Max blob TTL`. TTL governs storage lifetime; the BEP44 `seq` governs supersession (§3.10.7), and a validating relay's single-slot rule enforces supersession on write (§3.10.2). Record permanence is achieved by republication on the 6-day cycle (§3.10.2), not by TTL.

**Alternative rejected — the canonical BEP44 mutable item as the blob.** A tempting alternative is to store the canonical bencoded BEP44 mutable item (the `d1:k…3:seqi…e1:v…1:sig…e` dictionary) directly as the relay blob, reusing an existing serialization. Rejected on **determinism** grounds: a fixed-layout binary frame is trivially byte-reproducible across **all** bindings — including the wasm-safe `scp-protocol` decoder — with nothing but fixed-width reads and a trailing remainder, and **without** importing a canonical *bencode container codec* into every binding. Bencode appears in the protocol only as the hand-rolled BEP44 *signed-buffer* snippet (`3:seqi<seq>e1:v<value>`) emitted at the one signing/verifying site — never as a general serialization the frame would reuse. Storing the bencoded mutable item as the container would instead force every binding to carry, and byte-agree on, a full bencode encoder/decoder for a job a fixed layout does with no codec at all. So the fixed-layout binary container, not a bencode container, is the determinism-safe choice; the frame carrying `public_key` (parity with the mutable item's `k`) does not change that. The BEP44 *signature* is still computed over the BEP44-canonical `bencode(salt?, seq, value)` buffer above; only the *storage container* differs.
...
- Legitimate key changes (rotation, recovery) are distinguishable: a rotation of an operational key is a `KeyState` event signed by the standing `#0`, and a root change is a `RootRecovery` whose revealed key's commitment equals the standing commitment and whose chain wins the fork precedence of §9.7.4.2 R6 (authorization chain). For social recovery, trusted contacts independently confirm the rotation.
...
- **Agent Signing Key compromise (most common case):** The agent runtime is typically less secure than device HSM, making `#agent` the most likely key to be compromised. Recovery: (1) Human uses `#0` (Identity Key) to publish a new DID document removing or replacing the `#agent` verification method. (2) Revoke all UCANs with `fct.scp_key_scope: "#agent"` — add to per-context `RevocationList` and distribute via MLS application messages. (3) Issue MLS Update proposals in all active contexts with new credentials (updated `signing_key_id`). (4) Publish fresh KeyPackages carrying attestations re-issued under the new `#agent` key (§9.7.1). The human's `#active` key, root UCANs, and Identity Key are unaffected — only agent-scoped material is rotated (ADR-039). This is the cheapest recovery scenario: no root change, no root UCAN reissuance.
- **Active Signing Key compromise (common case):** Call `rotate_active_key` (ADR-003 §4a). Generate new active signing keypair, update DID document signed by Identity Key, publish to DHT. The DID string does NOT change. No root change is needed.
- **Root key (`#0`) compromise (rare, severe), with or without operational-key compromise:** The controller signs a `RootRecovery` (§9.7.4.2 R3) with the pre-rotation key from independent custody — `standing_root: CoSigns` while it still holds `#0`, `Lost` otherwise (§9.7.4.2 R10). The event installs a fresh root K′ generated under R10's device boundary and carries the complete post-recovery key state (§9.7.4.2 R8): every operational key by role — a fresh key for any key the compromised device also held, with the replaced key listed `Compromised{from:M}`, and the controller's own key re-listed otherwise; every historical key listed `Superseded`; the witness set; the relay list. The recovery supersedes any chain the attacker's `#0` signed, wherever the attacker forked (§9.7.4.2 R6), and nothing the attacker installed survives, because the snapshot does not list it. The identifier does not change: no new identity is created and no forwarding record exists. Steps 2–6 then run under the operational keys the snapshot names.
- **Pre-rotation key copied, `#0` intact and trusted:** The controller signs a `CommitmentRollover` (§9.7.4.2 R3) immediately. The rollover outranks any `RootRecovery{Lost}` the attacker signs (§9.7.4.2 R6) and fixes a commitment the attacker cannot reveal. No operational key changes, so steps 2–6 do not run.
- **Pre-rotation key copied, `#0` lost:** The controller signs a `RootRecovery{Lost}`; against the attacker's `RootRecovery{Lost}` it ties at rank 2 (§9.7.4.1 failure modes), and key material recovers the root only if the attacker never reveals.
- **Pre-rotation key taken and the controller's copy lost, `#0` intact:** The attacker's `RootRecovery{Lost}` wins uncontested over the controller's `#0`-only chain — rank 2 over rank 3 (§9.7.4.2 R6; §9.7.4.1 failure modes). The root cannot be recovered by key material.
- **`#0` compromised with the pre-rotation key copied:** A controller still holding `#0` ties at rank 1 (§9.7.4.2 R7); a controller whose `#0` is lost loses outright (§9.7.4.1 failure modes). **`#0` compromised with the pre-rotation key exclusively taken:** the attacker's `RootRecovery` wins uncontested (§9.7.4.2 R6). In every row where the root cannot be recovered by key material, a party's witness policy MAY still adopt the owner's cosigned suffix (§9.7.4.2 R6's slot); otherwise the person establishes a new identity, each context's admins remove the old identity and admit the new one, and §3.3's social recovery does not apply, because it re-establishes custody of the same identity.
- **Leaf-key / MLS-state compromise (ephemeral leaf key leaked, identity keys intact):** An attacker who extracts a member's MLS leaf state holds the leaf `signature_key`/`encryption_key` and the standalone **KeyPackage attestation** over them (§9.7.1). Beyond in-group PCS (§9.7.3), the standalone attestation lets the attacker reuse the leaf to join **other** groups until it expires. Remediation is an **existing operation, not a new mechanism**: the victim rotates `#active` (or `#agent`, for an agent leaf) via a DID-document update signed by `#0` (ADR-003 §4a / ADR-039) — the same rotation used for active/agent-key compromise above. This **invalidates every outstanding KeyPackage attestation within `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (§9.18.7 — 5 min, the hard current-key freshness bound of §9.7.1 check 2, NOT the 24h §9.10.7 privacy cache TTL)** — near-immediate, not instantaneous — because every one of them was signed by the now-retired verification method and verifiers resolve the DID's **current** `#active`/`#agent` only (§9.7.1 check 1), from a document no more than 5 minutes stale (§9.7.1 check 2); once the retired key no longer resolves (and no fresh-enough pre-rotation cache entry remains), every attestation signed by it fails verification everywhere. The victim then re-issues fresh KeyPackages/attestations under the new key (step 4) and issues MLS Updates in active contexts (step 2). **Containment property:** a leaked *leaf* key does NOT expose `#active`/`#agent` — separating the ephemeral, context-scoped leaf key from the identity signing keys is the entire point of the attestation model (§9.7.4), so this recovery is a **routine key rotation, not an identity-key exposure**: no recovery event (§9.7.4.2), no pre-rotation-key consumption, no change of identifier. This is a **Tier-1 revocation**: the DID document already carries the current verification method, and rotating it is the sole, sufficient revocation act — **no `attestations_valid_after` watermark or other new DID-document field is introduced** (a document-level revocation watermark was considered and rejected as over-engineering; rotation of an already-present key is a complete and cheaper revocation).
...
| `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` | 300s (5 min) | Maximum age of the resolved DID document used to satisfy the KeyPackage-attestation **current-key** check (§9.7.1 verifier checks 1–2). Tied to the §9.14 clock-skew tolerance (also 5 min). A resolver-cache entry older than this MUST NOT satisfy the current-key check — it MUST trigger a fresh resolution (on an Add a resolution failure then rejects; on an already-admitted member's Update a transient failure falls to the bounded last-known-good grace — §9.7.1 "Resolution failure policy"). This hard-bounds attestation-revocation latency on the Add path after `#active`/`#agent` rotation (§9.12) and is **decoupled from — and far tighter than — the §9.10.7 DID-resolution privacy cache TTL (24h / 7d)**, which MUST NOT be used to satisfy the current-key check beyond this bound. | §9.7.1 |
...
| DID-record frame version | 1 | Current DID-record relay frame version (§9.10.12); bumped on any field-encoding change. The frame has no magic tag and no record-kind byte — the DID `routing_id` domain is the type discriminant. Unsigned framing — grants no authority. See also §9.18.11 Transport and Relay for the shared blob-size/TTL bounds the frame reuses. | §9.10.12 |
| DID-record frame fixed prefix | 105 | Fixed-width prefix length in bytes (`version` 1 + `public_key` 32 + `seq` 8 + `signature` 64); `value` is the trailing remainder, `total_frame_len = 105 + len(value)` | §9.10.12 |
