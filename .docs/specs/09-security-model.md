...
An absent optional field encodes as the 32 bytes `SHA-256(0x00)` under the same rule the present form takes: length-prefixed where the present form is variable-length, bare where the present form is fixed-length. The key-event preimage takes no field under this rule: the definitions of §9.7.4.2 fix a sentinel of their own for a field a kind does not carry.
