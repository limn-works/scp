...
**`signer_key_id` names a verification method the identity model superseded.** The shipped `SigningKeyId` type renders a second role beside `"#active"`, the shared agent key ADR-039 defined, and §9.7.4.2 gives a human identity no such key, because an agent holds its own key-event log and its own `#active`. No section states which key a `CategoryAViolation` names once the offending signature comes from a delegated agent identity, and this section decides nothing about it.
