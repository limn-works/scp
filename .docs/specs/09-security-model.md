...
Steps 4, 5 and 6 are **identity-scoped**: each executes exactly once per recovery, never once per context. Step 4 is identity-scoped because outstanding KeyPackages are published and stored per **owner DID**, independent of which contexts that identity belongs to — there is no per-context KeyPackage state, so "retry the failed contexts" is not a meaningful remediation for it; retrying step 4 means retrying step 4, once, for the identity.

Because step 4 is identity-scoped it **gates completion for every context**: a surviving KeyPackage carrying an attestation signed by the retired key can be used to add the compromised member to a *new* group, so no context may be reported as fully recovered while step 4 has not succeeded, and a step-4 failure is a **whole-recovery failure** rather than a per-context one. This holds even when the identity belongs to **zero** contexts — steps 4, 5 and 6 still execute, and a step-4 failure still fails the recovery. Steps 5 and 6 likewise execute regardless of per-context outcomes: they are identity-wide cleanup, and a per-context failure must not silently cancel them.
