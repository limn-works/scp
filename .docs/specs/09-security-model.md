...
**The storage budget, and the fetch bound.** A validating relay bounds the key-event records it stores by three budgets it configures — one per identifier, one per publisher, which is the key that signed the PUBLISH, and one in total — and refuses a PUBLISH that would exceed any of the three with `IdentityError::StorageBudgetExceeded` naming the scope it exceeded (`03-identity.md` §3.10.10), while it keeps serving every record it already holds and evicts no accepted-chain byte. **A refusal is never a verdict**, and the publisher's recourse is the next entry of its fallback set (`03-identity.md` §3.10.6). A relay that charges refuses an unpaid PUBLISH the same way (`19-economic-governance.md` §19.8). No per-chain byte ceiling stands beside those budgets: Alec ruled on 2026-09-12 that a per-identifier ceiling misses the actor — "the risk is bad actors not theives" — because a party minting identities fills a relay under any per-identifier figure, and that what bounds a relay instead is what it charges, "relays can already charge rent." A verifier fetching a chain MAY stop once the accepted-chain bytes it has read reach `MAX_RETAINED_BYTES`, and it treats the unread remainder as absent rather than as a copy-level defect, so a truncated read produces `Inconclusive{MissingEvents}` and never `Invalid{at_event}`. That bound is not a sequence ceiling, for the reason the segmentation paragraph above gives.
