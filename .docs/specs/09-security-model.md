...
3a. **Recovery-authority residence (normative).** Item 3's storage-isolation requirement constrains *where the recovery authority resides*, not merely which provider object holds the key handle. The **recovery authority** is the minimal secret, key handle, or authorization capability sufficient to recover the pre-rotation private key. It MUST reside in a substrate whose compromise is **independent** of operational-custody compromise: there MUST NOT exist any secret, key handle, or authorization capability reachable **(directly or transitively, following any key-wrapping or key-derivation chain to its root)** from the operational `KeyCustody` provider or the daily-operations authentication flow that — alone or in combination with publicly-stored artifacts (including the encrypted-offline ciphertext of item 4) — suffices to recover the pre-rotation private key. Wrapping the key in an approved cipher (item 4) does NOT by itself satisfy this requirement: the §3 adversary is one who has already compromised operational custody, and if the decryption authority for that cipher is itself reachable from operational custody, the cipher provides zero protection against that adversary. The load-bearing property is **principal-distinctness of the recovery authority**, not the strength of the cipher wrapped around the key.
   ...
   a. **Non-interactive (server / `scp-node`) profile.** The recovery authority MUST be held under a principal DISTINCT from the operational-custody principal — a KMS/HSM key policy, secret-manager IAM role, or equivalent — and the operational principal MUST hold no read, use, decrypt, **assume, or delegate** grant over it **(nor over any KEK, secret, or role in its wrapping/derivation chain)**. An auto-generated encrypted-offline passphrase, a locally-held Shamir share set, or a server-readable BIP39 seed does NOT satisfy this requirement and MUST NOT be offered as a conforming standalone backend on this profile. An encrypted-offline backup conforms on this profile ONLY when the passphrase or key-encryption key it derives from itself resides in an independent-principal substrate (KMS/HSM/second device) — in which case that substrate, not the ciphertext, is the recovery authority. If no independent-principal substrate is available, identity creation MUST fail closed; there is no fallback to co-located storage.

   b. **Interactive-client profile.** A client with a present human MAY use an encrypted-offline backup whose passphrase is supplied by the user per-recovery and is never persisted in any server-reachable store; here the human presence is the independent principal. This is the encrypted-offline method rendered §3a-conforming by construction (the passphrase never resides server-reachably). Because §3a treats the encrypted-offline ciphertext as adversary-visible, a user-supplied passphrase is a §3a-conforming principal only if it resists offline brute-force against the published Argon2id parameters: the SDK MUST enforce a minimum-strength check on user-chosen passphrases and reject low-entropy inputs. Residence (never persisted server-reachably) is necessary but not sufficient when the ciphertext is public.

   c. **Migration is not a daily operation.** Identity migration (§9.12) is a rare, separately-authorized event, not a daily operation. A distinct migration-time authorization principal — even one that is fully automated (e.g., a separate KMS role assumed only for migration, which the operational path holds no grant to assume) — satisfies this requirement. What does NOT satisfy it is the daily operational signing flow itself being able to reach the pre-rotation key.
...
4. **Approved custody methods** (ordered by security, any one is sufficient **subject to the recovery-authority residence requirement of item 3a — not every method conforms on every profile; see item 3a(a)/(b)**):
...
5. **SDK presentation.** At identity creation, the SDK MUST:
   ...
   b. Present the user with custody options ordered by security — **filtered to those §3a-conforming for the active deployment profile (item 3a(a)/(b)); a backend the per-backend §3-soundness table marks unsound/degenerate for the profile MUST NOT be offered as a standalone conforming choice.**
   ...
   **Non-interactive (server) profile.** On a profile with no present human (§3a(a)), steps (b)–(d) are replaced by selection of an operator-configured independent-principal backend (KMS/HSM/cloud-vault). Creation MUST fail closed if none is configured — there is no fallback to co-located storage. The `PreRotationCommitment` is published only after the key is confirmed resident in the independent-principal substrate.

6. **Post-rotation key cycling.** After a pre-rotation key is used for Identity Key migration (§9.12), the protocol MUST immediately generate a new pre-rotation keypair, guide the user through custody selection again, and publish a new `PreRotationCommitment`. The old pre-rotation key is destroyed after migration completes. The new pre-rotation key MUST re-enter §3a-conforming custody for the active profile (re-run §5 selection, including its non-interactive-server branch); on a server this means a fresh independent-principal substrate, established non-interactively.
