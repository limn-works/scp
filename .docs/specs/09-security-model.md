...
**Ceremony order.** Read the standing commitment and the current key list from the adopted key state, **from the log the controller durably retains itself and never from a resolution verdict**; generate the next set under §9.7.4.1 items 1, 3a, 4, and 5b through 5d; for a `RootRecovery` generate K′ and the operational keys under the device boundary; for an abandoning rollover issue the retirement Update in every context; persist the recovery handle carrying the composed preimage, into every store the one-reveal-at-a-time rule below names for the identity's pre-rotation credential; sign; publish; retain; destroy. The persist precedes the signature, because a controller that crashed between signing and persisting would resume by composing a fresh next set, and its second event would reveal the standing commitment a second time.
...
**What a relay's ring buffer leaves a witness to do.** A witness whose cosigned head a relay displaced re-cosigns the subject's head on the next offer that subject's controller makes it, which is an ordinary write against that subject at that relay and carries no new duty; §9.7.4.2 R9 states the duties a displacement leaves each party. **The subject's controller pays for a `scp:wit:` or a `scp:wcf:` write at a relay that charges for one.** A witness holds no payment adapter and mints no receipt, and `03-identity.md` §3.10.2's own witness filter already computes the condition that would exempt one, because it accepts such a write only where the relay holds a chain for that subject and that subject's key state names the witness. **Those bytes count against `Total` and against no identity's coverage** (§9.7.4.2 R9), so a controller that rents its own published bytes rents nothing for them and loses no coverage to them. **A conflict statement is held by the witness that wrote it and by the controller that fetched it**, so neither party depends on a relay retaining one.
...
**The wire change this section makes to ADR-004** is six items: `retain` and `payment_receipt` on PUBLISH, `proof_nonce` on QUERY, `relay_proof` on BLOB, the `POLICY` query a publisher reads the relay's declared write policy through, and the `RENT` operation a controller pays rent on (§9.7.4.2 R9). ADR-004 carries all six. **No wire field binds a receipt to a frame**: R9's four receipt checks read the terms `PaymentAdapter::verify` returns, so the deleted `write_binding` field and the frame digest it read are gone from `PaymentMetadata`, from `PaymentReceipt` and from every check a relay runs.
...
|-----------|---------|-------|-----------|----------------|
| `rate_limit_publish` | 6,000/min, the default `18-addressability-and-deployment.md` §18.3.3 states | Any value the operator sets; zero turns the term off | The relay declares the term in `relay_config` and refuses a key-event PUBLISH that exceeds it with `StorageBudgetExceeded{scope: RateLimit, value}`, whose `value` carries the rate. An operator that charges buys no party relief from this term, because §9.7.4.2 R9 admits no declaration that exempts one | §9.7.4.2 R9; §18.3.3 |
| Per-identifier storage budget, total storage budget | Off (absent). An operator that declares no total budget still runs the ring, over the retained capacity its own host holds | Any value the operator sets; zero or absent means the term is off | The relay declares each term in `relay_config` and refuses a key-event PUBLISH that fails it with `StorageBudgetExceeded{scope, value}`, whose `value` carries the declared figure. §9.7.4.2 R9 states when each scope refuses and what the relay displaces or evicts first | §9.7.4.2 R9; §18.3.3 |
| `receipt_exempts` | Absent, which exempts no scope | The scope names §9.7.4.2 R9 permits a relay to declare | The relay declares the set in `relay_config` and ADR-004's `POLICY` query returns it, so a publisher reads which refusal a verified receipt lifts before it spends | §9.7.4.2 R9; §18.3.3 |
| Rent price, rent period, rent quota | Off (absent), and an operator that declares none sells no coverage | Any values the operator sets | The relay declares the three terms in `relay_config`, and a controller pays rent against them on ADR-004's `RENT` operation. §9.7.4.2 R9 states what rent covers and when it lapses | §9.7.4.2 R9; §18.3.3 |
