...
Because step 4 is identity-scoped it **gates completion for every context**. This does **not** contradict step 1's designation of `#active`/`#agent` rotation as the sole, sufficient revocation act (§9.7.3, and the leaf-key/MLS-state bullet of step 1 above). Rotation *is* the revocation; it simply does not take effect instantaneously. Verifiers resolve the signer's **current** verification method from a document that may be up to `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (§9.18.7 — 5 min) stale (§9.7.1 checks 1–2), so within that window a verifier holding a still-fresh cached document will accept an attestation signed by the retired key. Step 4 is what makes that residual window unexploitable: it removes the still-fetchable KeyPackages carrying those attestations, so there is nothing to present during the window. Step 4 introduces **no new revocation mechanism and no new DID-document field** — it retracts published artifacts, and rotation remains the revocation act. Consequently no context may be reported as fully recovered while step 4 has not succeeded, and a step-4 failure is a **whole-recovery failure** rather than a per-context one. This holds even when the identity belongs to **zero** contexts — steps 4, 5 and 6 still execute, and a step-4 failure still fails the recovery.
