...
**Reference implementation:** the inner-envelope signature preimage (§9.8.1) uses this exact construction and enumerates every field in place; the key-event signature preimage uses it under `"SCP-KEL-EVENT-V1:"` (§9.7.4.2 R13), with its field order fixed by the substrate byte-layout amendment to this spec.
...
**Key-status vocabulary and the content-signature rule.** A key-status assertion (§9.7.4.2 definitions) assigns a key one of three statuses: `Superseded` (a later key replaced it in the ordinary course), `Retired` (the controller withdrew it with no successor), or `Compromised{from:N}` (the controller asserts that the key was under an attacker's control from sequence N of the key-event log onward). A later assertion by the standing `#0` over the same key supersedes an earlier one, so a false alarm is withdrawn by a new assertion and never by deletion. The two kinds of signature a relying party verifies read the status differently. A KeyPackage attestation verifies against the identity's **current** `#active` or `#agent` only (check 1 above) and rejects every retired key whatever its status. A content signature — an inner-envelope signature (§9.8.1) over content a key signed while it was current — verifies against a key later marked `Superseded` or `Retired`, so content stays verifiable after ordinary rotation; but a verifier MUST reject a content signature by a key marked `Compromised{from:N}` for content whose position in its context is at or after the **compromise anchor**, and MUST reject content whose order against the anchor is indeterminate, accepting only content that is causally before the anchor. The compromise anchor is the point in a context at which the `Compromised{from:N}` assertion took effect for that context; it MUST be a value that converges across the context's honest members, so that two honest verifiers reach the same verdict on the same content — the every-field-convergent requirement the equivocation test of §9.9.3 states. How N maps onto a per-context anchor, and the exact causal-reachability rule, is not yet fixed in this spec. When a later assertion withdraws a `Compromised{from:N}` assertion, the anchor derived from it is retired, and verifiers re-derive the key's status from the latest assertion. Content committed after the actual compromise but before the anchor is a residual that an asserted boundary cannot recover; the carve-out is a root-signed judgment, not a guarantee. Rationale for rejecting indeterminate order: an attacker holding the compromised key can manufacture content at any later wall-clock time whose causal references name only pre-anchor heads, so accepting concurrent content would leave the carve-out bypassable.
...
- Pre-Rotation Key (Ed25519): Generated at identity creation, stored in cold/offline custody (see §9.7.4.1 for custody requirements). Its commitment (§9.7.4.2 definitions) is a field of the inception event and of every reveal-authorized event. The key is revealed only inside a reveal-authorized event — a `CommitmentRollover` or a `RootRecovery` (§9.7.4.2 R3) — where it signs the event in place and is never installed as an operational key, and the controller destroys it after that event is published (§9.7.4.2 R5).
...
- Identity Key (`#0`): Changes only by a `RootRecovery` (§9.7.4.2 R3) — a rare operation. The recovery event reveals the pre-rotation public key P whose commitment equals the standing commitment, carries P's signature, installs a fresh root K′ that the controller generated in operational custody for the event (K′'s signature over the event is its proof of possession), installs the operational keys, fixes the next commitment (§9.7.4.2 R4), and carries the compromise boundary N (§9.7.4.2 R8). P is never installed as a key; it is spent by the event and destroyed after publication (§9.7.4.2 R5). The identifier does not change, because it derives from the inception event and a recovery event does not rewrite the inception event. No migration proof exists: the recovery event is signed under `"SCP-KEL-EVENT-V1:"` like every other key event (§9.7.4.2 R13). A relying party re-verifies key continuity (§9.11) on a recovery event, because the identifier no longer signals that the root moved.
...
2. **Commitment publication.** The pre-rotation commitment (§9.7.4.2 definitions) is a field of the inception event and of every reveal-authorized event; the controller publishes it as part of that event and nowhere else. Only the commitment is published. The public key appears only inside the reveal-authorized event that consumes the commitment.
...
4. **Approved custody methods** (ordered by security; any one is sufficient **subject to the recovery-authority-residence requirement of item 3a** — a method whose recovery authority is reachable from operational custody does not satisfy item 3, regardless of its position in this list):
   ...
   |--------|---------------|-------------|
   | Hardware security key (FIDO2/U2F) | Highest | Pre-rotation key stored on a dedicated hardware security key (e.g., YubiKey). The key never leaves the hardware: it signs the reveal-authorized event on the device and is never installed as an operational key (§9.7.4.2 R3, R10). Requires physical possession for recovery. |
...
5. **SDK presentation.** At identity creation, the SDK MUST:
   ...
   e. Sign and publish the inception event, which carries the first pre-rotation commitment (§9.7.4.2 R2), only after backup verification succeeds.
   f. Destroy the pre-rotation private key from the creating device's memory after backup is confirmed.

6. **Next commitment and destruction of the spent key.** Every reveal-authorized event fixes the next pre-rotation commitment inside the same event (§9.7.4.2 R4), so the SDK MUST generate the next pre-rotation keypair in a substrate satisfying item 3a and guide the user through custody selection for it before it signs a `CommitmentRollover` or a `RootRecovery`. §9.7.4.2 R10 states the ceremony order, the definition of confirmed publication, and the recovery handle; §9.7.4.2 R5 states the destruction of the spent key.
...
**Failure modes.** Each row names the keys the attacker holds and the outcome the rules of §9.7.4.2 produce. Where a row says the root cannot be recovered, the person establishes a new identity and each context's admins remove the old identity and admit the new one; the social recovery of `03-identity.md` §3.3 does not apply to that row, because §3.3 re-establishes custody of the same identity, and a contested or captured root leaves no identity to re-establish.
...
- **Pre-rotation backup lost, `#0` compromised.** The attacker's `#0`-signed suffix and the controller's `#0`-signed suffix tie at rank 3 (§9.7.4.2 R7), and the tie is permanent because no party can reveal the standing commitment. The root cannot be recovered.
- **Pre-rotation key copied, `#0` intact.** The attacker can sign a `RootRecovery` (rank 2). The controller MUST sign a `CommitmentRollover` (rank 1) as soon as it suspects the copy; the rollover wins under §9.7.4.2 R6 wherever the attacker forked, and it fixes a new commitment whose preimage the attacker does not hold.
- **Pre-rotation key taken and the controller's copy lost, `#0` intact.** The attacker's `RootRecovery` (rank 2) stands, because the controller can sign no reveal-authorized event. This is root takeover, and the root cannot be recovered. Item 3a exists to keep this case from arising through operational-custody compromise; no mitigation exists once the key is exclusively the attacker's.
- **`#0` compromised, pre-rotation key held only by the controller.** The controller signs a `RootRecovery` (rank 2) carrying the compromise boundary N and installing a fresh root K′. The recovery wins under §9.7.4.2 R6 against any `#0`-signed suffix wherever the attacker forked, and §9.7.4.2 R8 voids the attacker's `#0`-alone establishment events from N on the controller's own chain. A controller that still holds `#0` and is not certain it is compromised signs a `CommitmentRollover` first (§9.7.4.2 R10).
- **`#0` compromised and pre-rotation key copied.** Both parties can sign every event kind. Two rollovers tie at rank 1 and two recoveries tie at rank 2, and either tie is terminal (§9.7.4.2 R7). The root cannot be recovered.
- **Spent key's copy held by an attacker, `#0` and the standing pre-rotation key intact.** A backup that survived §9.7.4.2 R5 — Shamir shares, a paper mnemonic, a hardware key — or a copy taken before the reveal lets its holder fork before the event that spent the key and reveal that event's commitment again. If that event was a `CommitmentRollover`, the owner's rank 1 wins. If it was a `RootRecovery`, the tie is at rank 2 and terminal, and the root cannot be recovered. R5 and item 5d, which destroy and verify the controller's own copies, are the only mitigation, and neither reaches a copy outside the controller's control.
- **Two devices both holding `#0`, no attacker.** The two suffixes tie at rank 3 and the identity is contested but the tie is pending (§9.7.4.2 R7). The controller ends the tie by signing a `CommitmentRollover` on the suffix it keeps (§9.7.4.2 R10).

**Partial-publish recovery.** A reveal-authorized event has one irreversible step, the reveal; §9.7.4.2 R10 states the ceremony order, the definition of confirmed publication, and the typed recovery handle that a failure after signing MUST surface.

### 9.7.4.2 Root-Authority Recovery and Fork Precedence

This section states how an identity's root key `#0` recovers from compromise and how a verifier decides between two key-event chains that diverge from a shared prefix. §9.7.4.1 supplies the recovery key's custody rules; this section states the events that consume that key and the rule that ranks competing chains. Every statement below is a verifier obligation or a controller obligation. The identity spec and the identity ADRs cite this section and do not restate its rules.

**Definitions.**

- The **root key** `#0` is the identity's establishment authority. The **standing `#0`** at a position in a chain is the root key the latest root-installing event at or before that position installed: the inception event installs the first, and a `RootRecovery` installs each later one.
- An **operational key** is a signing key other than `#0` — currently the Active Signing Key `#active` and the Agent Signing Key `#agent`.
- A **key-status assertion** is a key event signed by the standing `#0` that assigns one key a status from the vocabulary of §9.7.1 (`Superseded`, `Retired`, `Compromised{from:N}`); a later assertion over the same key supersedes an earlier one, so a false alarm is withdrawn by a new event and never by deletion.
- A **witness-set update** is a key event signed by the standing `#0` that names the relays the controller designates to cosign the identity's log head and the cosigning parameters they follow. The cosigning protocol those relays run is not yet specified in this corpus; this section names the event only as a kind R8 voids.
- An **establishment event** is a key event that changes the identity's key state or its declared witness set. The kinds are: the inception event, an operational-key rotation, a key-status assertion, a witness-set update, a `CommitmentRollover`, and a `RootRecovery`.
...
**R2 — Inception fixes the first commitment.** The inception event fixes the first commitment. A verifier MUST reject an inception event that fixes no commitment. Rationale: the identifier is the inception event's digest, so an inception event without a commitment would create a permanently valid identifier with no recovery path.

**R3 — Exactly two reveal-authorized event kinds exist, and every kind fixes its signature set.** A verifier MUST derive an event's kind from the type discriminator in its preimage, never from which signatures it carries.

| Kind | Signatures the event carries, exactly | Effect on the root |
|---|---|---|
| Inception | the first `#0` | installs the first `#0` |
| Operational-key rotation, key-status assertion, witness-set update | the standing `#0` | none |
| `CommitmentRollover` | P and the standing `#0` | none — `#0` is unchanged |
| `RootRecovery` | P and a fresh root K′ | installs K′ as the new `#0` |

A `RootRecovery` carries P's public key, P's signature, the compromise boundary N (R8), the public key of a fresh root K′ that the controller generated in operational custody for this event, K′'s signature over the event as proof of possession, the operational keys the recovery installs (one `#active` and, when the identity uses one, one `#agent`; no other event installs operational keys during a recovery), and the next commitment (R4). P is never installed as a key: it authorizes the event and is then a spent key, exactly as in a `CommitmentRollover`. No `#0` is ever the preimage of any commitment, because K′ is generated fresh and the commitment names a separate key held in independent custody (§9.7.4.1 item 3a). An event whose signature set differs from its kind's set, whose revealed key's commitment does not equal the standing commitment at its predecessor, or whose P signature is absent or fails to verify is invalid, and a verifier MUST reject the chain at that event. Authorization is the signature by P's private key and never the reveal alone: the revealed public key is public the instant the event appears, so a matching value without the signature proves nothing about who produced the event. Rationale: a controller whose pre-rotation key was copied while `#0` stayed intact needs the rollover; a controller whose `#0` was compromised needs the recovery; no third compromise case needs a reveal. Rationale for the fresh root: if the recovery installed P itself as the new `#0`, the live root after recovery k would be the preimage of the commitment recovery k consumed, so a thief who later took that root could re-reveal recovery k's commitment on a fork anchored before recovery k and tie the owner at rank 2 — recovery would defend against the first root theft and never a second. Rationale for deriving the kind from the discriminator: if the kind were read from which signatures are present, a relay could strip the `#0` signature from a rollover so it read as a recovery and tied a thief's recovery, or a thief holding `#0` could append its signature to the owner's recovery so it read as a rollover that left the thief's root in place.

**R4 — Every reveal-authorized event fixes the next commitment in the same event, and no commitment is fixed twice.** The controller MUST generate the next pre-rotation key in a substrate satisfying §9.7.4.1 item 3a before it signs the event. A verifier MUST reject a reveal-authorized event that fixes no successor commitment. A verifier MUST reject a commitment-fixing event whose commitment equals any commitment previously fixed on the chain, or equals the commitment of the standing `#0`, or equals the commitment of the root the event installs. Rationale: a chain whose standing commitment has been consumed and not replaced has no recovery path; a follow-on event to fix the next commitment would have to be signed by `#0` alone, which R1 forbids; and a commitment that repeats an earlier one or names the root itself would let a spent or stolen key reveal it, which breaks the uniqueness R6 relies on.

**R5 — The controller destroys the spent pre-rotation key.** After the controller has published a reveal-authorized event (R10 defines confirmed publication) and consumed any recovery handle for it, the controller MUST destroy P's private key from every custody location it controls, for a `RootRecovery` and a `CommitmentRollover` alike. Rationale: a spent key that survives can sign a second reveal of a commitment already consumed and tie the identity at that fork (R7). The controller cannot destroy a copy an attacker holds or a backup outside its control; R5 removes the copies it controls and closes the two-device case.

**R6 — Fork precedence: the first event that consumes the shared prefix's standing commitment decides.** When a verifier holds two chains that diverge from a shared prefix, the verifier MUST decide between them as follows and by no other comparison. Let C be the standing commitment at the shared prefix. On each divergent suffix, the verifier locates the first event that reveals C's preimage. At most one such event exists per suffix, because R1 makes C unique on the shared prefix, a reveal consumes it, and R4 forbids fixing it again. The verifier ranks each suffix by that event's kind:

1. the suffix carries a `CommitmentRollover` that reveals C — the C-revealing event also carries the signature of the `#0` standing at the shared prefix;
2. the suffix carries a `RootRecovery` that reveals C;
3. the suffix carries no event that reveals C.

The suffix with the lower rank number wins. The winning chain supersedes the losing chain in its entirety, whatever the two head sequences are, because a chain is the unit of authority and no hybrid of two chains is a valid log. A verifier MUST NOT decide a divergence by head sequence, by the first event after the shared prefix, or by the highest rank found anywhere on a suffix.

- *Pin A.* Rank 1 names the `#0` standing at the shared prefix, because each suffix may have installed a different root after the fork.
...
**R7 — Ties and the contested verdict.** Equal rank resolves no winner, and the identity is **contested**. A tie at rank 1 or rank 2 is **terminal**: C is consumed on both suffixes, so no later event on either suffix changes the comparison. A tie at rank 3 is **pending**: neither suffix has revealed C, and a later C-revealing event on either suffix decides the divergence under R6. For a contested identity a verifier MUST return a typed contested verdict that carries the tie class (pending or terminal), the tied rank, and both head digests; MUST NOT adopt either suffix as the current key state; MUST NOT return the shared prefix's key state as the current key state; and MUST retain both heads and both suffixes as evidence. Criterion for what a tie proves: a rank-2 tie proves that the pre-rotation private key signed two divergent events; a rank-1 tie proves that both the pre-rotation private key and the `#0` standing at the shared prefix signed two divergent events; a rank-3 tie proves that `#0` signed two divergent suffixes. Indicator, not criterion: two parties holding the key is the usual cause of a double signature, and one holder with two devices also produces one. Rationale for refusing the shared prefix's key state: if `#0` was compromised before the fork point, the shared prefix already carries the attacker's events, so serving its key state as current would serve a hostile key state. Rationale for the verdict payload: a pending tie tells the controller to act (R10) and a relying party to re-resolve later, and a terminal tie tells both that no event will resolve it.

**R8 — A `RootRecovery` voids the superseded `#0`'s sole-signed establishment events from the compromise boundary.** A `RootRecovery` MUST carry N, the sequence from which the controller asserts the superseded `#0` compromised. N MUST satisfy s < N ≤ seq(recovery), where s is the sequence of the event that installed the superseded `#0` (the inception event for the first recovery); a verifier MUST reject a `RootRecovery` whose N lies outside that range. When a verifier replays a chain through a `RootRecovery`, every establishment event on that chain at a sequence greater than or equal to N and before the recovery that carries the superseded `#0`'s signature alone MUST NOT contribute to the derived key state: an operational key such an event installed is not current, and a witness-set change such an event made does not take effect. A reveal-authorized event is never voided, and a commitment fix is never voided. Chain validity under R1–R4 is evaluated over the unvoided chain; R8 applies to the derived key state only. The key state after the recovery is the key state at sequence N−1, with every reveal-authorized event between N and the recovery still applied, plus the recovery event's own content. Content signed by an operational key that a voided event installed is evaluated under the `Compromised{from:N}` rule of §9.7.1. A `CommitmentRollover` carries no N, because a rollover supersedes a competing suffix in its entirety under R6 and voids nothing on its own chain. The two error directions are not symmetric: an N set too low voids the controller's own rotations, which the recovery's own operational-key installation re-establishes, while an N set too high leaves the attacker's installs current until another recovery; a controller uncertain of N MUST choose the earliest sequence it cannot rule out. Rationale: an attacker holding `#0` who appends rotations and witness-set changes to the one genuine log produces no divergence, so R6 never runs, and without R8 the controller's recovery would extend the attacker's events and leave them current. The alternative this rule rejects — leaving those events in force and requiring the new root to retire each one by a follow-on event — requires the controller to enumerate the attacker's events and leaves a window open between the recovery and the last retirement.

**R9 — A verifier holds the evidence the comparison reads.** To apply R6 a verifier MUST hold, or obtain, both divergent suffixes back to their common ancestor; a verifier that cannot treats the candidate under R11. A verifier MUST NOT bound the suffix it retains at its last reveal-authorized event, because a party holding a spent pre-rotation key can anchor a fork before that event (R5), and the comparison then reads earlier events. Indicator, not criterion: key-event logs are short, so a verifier that retains its full accepted chain satisfies R9 with no fetch, and a verifier that retains only a head digest re-fetches its own chain from a relay, which cannot forge a self-certifying chain but can withhold it.

**R10 — The controller's procedure: ceremony order, confirmed publication, the recovery handle, and which event to sign first.**

*Ceremony order.* A controller MUST produce a reveal-authorized event in this order: generate the next pre-rotation key in a substrate satisfying §9.7.4.1 item 3a; for a `RootRecovery`, generate the fresh root K′ and the operational keys the event installs in operational custody; sign the event — P signs it where P resides and is never imported into operational custody; publish the event to the identity's relays; on confirmed publication, destroy the spent key (R5); then submit the event to the identity's witness set as it stands after R8 voiding. **Confirmed publication** is acceptance of the event by at least one of the identity's relays. Witness submission never gates destruction, because the identity's resolvability never depends on a witness (the witness layer detects and does not gate).

*The recovery handle.* A failure after signing MUST surface a typed recovery handle that carries the byte-identical signed event, the phase that failed (published to no relay; published but the spent key not yet destroyed; destroyed but not yet submitted to the witness set), the custody handle of the spent key pending R5, and the custody handle of the next pre-rotation key; exactly one resume entry point consumes the handle by value and continues from the failed phase without re-signing. The controller retains the spent key between signing and confirmed publication. Rationale: the reveal is irreversible once the event is public, a resume that re-derives a key or re-signs the event produces artifacts that differ from the ones already published, and a handle that carried only the signed bytes could neither destroy the spent key nor tell which fallible step failed.

*Which event to sign first.* A controller that still holds `#0` and needs to consume the standing commitment MUST make a `CommitmentRollover` its first C-consuming event, and MUST NOT sign a `RootRecovery` as its first. Rationale: against an attacker who copied P and forked with a `RootRecovery`, a `RootRecovery` from the controller ties at rank 2 and the tie is terminal, while a rollover wins at rank 1; against an attacker who holds `#0`, a rollover wins at rank 1 against rank 3; only an attacker holding both keys ties a rollover, and that attacker ties every choice. A rank-3 tie whose other suffix the controller did not author is evidence that `#0` is compromised: after the rollover secures the fork, the controller MUST sign a `RootRecovery` on the same suffix to retire the compromised `#0`, because a rollover leaves that `#0` standing and the attacker can fork again past each rollover at one signature's cost. A rollover alone ends a rank-3 tie only when the controller authored both suffixes (the two-device case).

**R11 — Divergence is a positive finding.** A candidate head is **on the accepted chain** when its chain links to the verifier's accepted head. A candidate head is **divergent** when the verifier holds both chains back to a common ancestor and each chain carries, at the first position after that ancestor, an event the other does not. A candidate that is neither — because the events linking it to the accepted chain were not served — is **inconclusive**: the verifier holds its baseline, requests the missing events, and records nothing; the inconclusive verdict names the candidate head and the accepted head it could not be linked to. A verifier MUST record equivocation evidence only from two chains it holds in full back to their common ancestor, and only when R6 resolves them as a tie (R7). Rationale: a single relay that withholds linking events could otherwise cause a verifier to record a durable equivocation verdict against an honest identity.

**R12 — Sequence governs the accepted chain only.** For a candidate head on the accepted chain, a verifier MUST reject a head at a sequence strictly lower than the highest it has accepted, and an equal sequence with an identical head confirms the baseline. A candidate at an equal or lower sequence whose head differs from the accepted chain's head at that sequence is not on the accepted chain: the verifier classifies it under R11 and, when it holds both chains to the common ancestor, decides under R6 and R7. A divergent candidate's head sequence decides nothing. Adopting an R6 winner MAY lower the accepted head sequence, and a verifier's stored baseline MUST be able to express that decrease. Rationale: a monotone maximum cannot record the adoption of a lower-sequence winner, and a verifier that stored only a maximum would discard the outcome R6 reached.

**R13 — Domain separators.** The key-event signature preimage uses the separator `"SCP-KEL-EVENT-V1:"` under the §9.5.1 construction, and the first field after the separator is the event-type discriminator byte. The pre-rotation commitment uses `"SCP-PREROTATION-COMMITMENT-V1:"` as a commitment-hash domain. Both are registered in §9.18.2. Whether the identifier derivation uses the inception event's canonical digest under the event separator or a distinct identifier-construction prefix is not yet fixed in this spec; the substrate byte-layout amendment fixes it, and §9.18.2 marks that row pending until then.
...
1. **A `#0` thief forks early.** The owner's chain is e0 through e9 (ordinary K0-signed rotations), and the owner signs a `RootRecovery` at e10 that reveals C with P's signature, installs a fresh root K′, fixes C′, and carries N = 6. The thief, holding K0, forks at e5 and appends a5 through a50, all K0-signed. The shared prefix is e0 through e4 and its standing commitment is C. The owner's suffix carries e10, a `RootRecovery` that reveals C, so its rank is 2. The thief's suffix carries no C-revealing event, so its rank is 3. The owner's chain wins under R6 wherever e10 sits, and the thief's head sequence 50 decides nothing. Under R8, the owner's own K0-signed events e6 through e9 are voided as key-state contributors, because N = 6, and e10 installs the owner's operational keys.
2. **A copied pre-rotation key, thief forks early.** The owner's chain is e0 through e60 with K0 intact. The thief holds a copy of P, forks at e10 with a `RootRecovery` a11 (rank 2, installing a root the thief holds), and appends to a50. The owner signs a `CommitmentRollover` at e61 that reveals C and carries K0's signature. The shared prefix is e0 through e10. The owner's suffix carries e61, a rollover, so its rank is 1. The thief's suffix carries a11, a recovery, so its rank is 2. The owner wins under R6 wherever e61 sits. The rollover fixes a new commitment whose preimage the thief does not hold, so the thief cannot repeat the attempt.
3. **Two recoveries revealing the same commitment.** The owner's chain is e0 through e30, and K0 was compromised at e12 by a thief who also holds a copy of P. The thief forks at e12 with a `RootRecovery` a13 that reveals C and installs a root the thief holds. The owner signs a `RootRecovery` e31 that reveals C and installs K′. The shared prefix is e0 through e11. Both suffixes carry a `RootRecovery` that reveals C, so both are rank 2 and the tie is terminal (R7): P signed two divergent events. This is the "`#0` compromised and pre-rotation key copied" failure mode, and the identity is unrecoverable. Had the owner still trusted K0, R10's procedure — a rollover at e31 instead — would have won at rank 1; the owner lost the option when K0 was compromised, which is why the failure mode is stated as unrecoverable.
4. **Two `#0`-only suffixes.** The owner's two devices both hold K0. Device A appends an operational-key rotation e10a and device B appends a different rotation e10b. The shared prefix is e0 through e9. Neither suffix reveals C, so the tie is at rank 3 and pending (R7). The owner, holding P, signs a `CommitmentRollover` e11a on device A's suffix; that suffix is now rank 1 against rank 3, and it wins. A thief holding K0 alone could not have ended the tie. An owner who has lost P faces a permanent rank-3 tie, and the identity's root cannot be recovered (failure modes).
5. **A fork anchored before the latest reveal, by a holder of a spent key's copy.** The owner's chain carries a reveal-authorized event r20 that reveals an earlier commitment C_prev over the earlier pre-rotation key P_prev and fixes C, and continues to e40. An attacker who holds a copy of P_prev — a backup the owner did not destroy, or a copy taken before r20 — forks at e19 and signs a `RootRecovery` a20 that reveals C_prev. The shared prefix is e0 through e19, and its standing commitment is C_prev. The owner's suffix carries r20, which reveals C_prev; the attacker's suffix carries a20, which also reveals C_prev. If r20 was a `CommitmentRollover`, the owner's rank is 1 against the attacker's rank 2 and the owner wins. If r20 was a `RootRecovery`, both ranks are 2 and the tie is terminal — the attacker never needed `#0`, only the spent key's copy, which is the "spent key copied" failure mode. R5 and §9.7.4.1 item 5d, which destroy and verify the owner's own copies, are the only mitigation; neither reaches a copy an attacker holds.
...
- **Root key (`#0`) compromise (rare, severe), with or without operational-key compromise:** The controller signs a `RootRecovery` (§9.7.4.2 R3) with the pre-rotation key from independent custody. The event carries the compromise boundary N, installs a fresh root K′ generated in operational custody, and installs the operational keys — fresh keys where any operational key is suspected compromised, otherwise the controller's current keys re-installed, because §9.7.4.2 R8 voids every install the attacker's `#0` made from N. The recovery supersedes any chain the attacker's `#0` signed, wherever the attacker forked (§9.7.4.2 R6). The identifier does not change: no new identity is created and no forwarding record exists. Where the controller still holds `#0` and learned of the compromise from a rank-3 tie it did not author, it signs a `CommitmentRollover` first and the `RootRecovery` on the same suffix (§9.7.4.2 R10). Steps 2–6 then run under the operational keys the recovery installed.
- **Pre-rotation key copied, `#0` intact:** The controller signs a `CommitmentRollover` (§9.7.4.2 R3) immediately. The rollover outranks any `RootRecovery` the attacker signs (§9.7.4.2 R6) and fixes a commitment the attacker cannot reveal. No operational key changes, so steps 2–6 do not run.
- **Pre-rotation key taken and the controller's copy lost, `#0` intact:** The attacker's `RootRecovery` stands (§9.7.4.2 failure modes), and the root cannot be recovered. The person establishes a new identity, and each context's admins remove the old identity and admit the new one. The social recovery of `03-identity.md` §3.3 does not apply, because it re-establishes custody of the same identity.
- **`#0` compromised with the pre-rotation key copied or lost, or a spent key's copy used to tie a past `RootRecovery`:** The root cannot be recovered (§9.7.4.2 R7, a terminal or permanent tie). The person establishes a new identity, and each context's admins remove the old identity and admit the new one; §3.3 does not apply, for the reason above.
...
| `"SCP-KEL-EVENT-V1:"` | Key-event signature preimage for every establishment-event kind (§9.7.4.2 definitions); the first field after the separator is the event-type discriminator byte | §9.7.4.2 R13 |
...
| *(identifier derivation)* | **Pending.** Whether the inception-derived identifier is the inception event's canonical digest under `"SCP-KEL-EVENT-V1:"` or a distinct identifier-construction prefix is fixed by the substrate byte-layout amendment to this spec; no separator is registered for it until then | §9.7.4.2 R13 |
