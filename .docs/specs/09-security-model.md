...
A rule that lifted a refusal on a receipt a write carried would lapse when that write ended, which is the one-time purchase Alec's rent ruling replaced; an unbounded exemption would let one rent payment fill a relay's declared capacity at no marginal cost, which "get quota a of b thing" does not say. **A `relay_config` whose `receipt_exempts` names `Identifier` and which declares no rent terms is invalid**, because rent is the one thing that buys the exemption and a publisher refused at such a relay holds a remedy nothing can satisfy.

A stop over the eviction arm would shelter bytes the coverage test does not count, and it would leave a controller that wants a third party's suffix evicted no act but stopping its rent, which arms the ring against that controller's whole record set in the same act.
...
The check runs before any check that leaves the relay's host. A check that admitted a message for carrying a `payment_receipt` field would admit every message carrying arbitrary bytes under that name, and each one would reach `PaymentAdapter::verify`, which hands an unauthenticated party one outbound rail query per request; a relay that skipped the check would have to know a receipt is valid before it ran the check, which drives that call ahead of the limit and reaches the same flood. `rate_limit_publish` is a declared term, so the refusal it produces is `StorageBudgetExceeded{RateLimit, value}` on a PUBLISH and on a `RENT` alike, and the wire code is `4041`.
