...
R10's one-reveal-at-a-time rule reads a store set this value selects. A profile is a property of a running SDK and never of an identity. The SDK MUST re-surface the copyable-custody disclosure whenever it opens an identity whose standing pre-rotation custody type is one of the three copyable values, on whatever profile it runs, because an identity incepted headless under a copyable method keeps that key when its operational key migrates to a phone.
...
| Two of the controller's devices each reveal the standing commitment | Rank 1 on both sides; terminal. R10's one-reveal-at-a-time rule stops the race on `Mobile` and on `Desktop` under a platform account, and the race stays open on `Browser`, on `Headless`, and on `Mobile` or `Desktop` under no platform account, where no store spans the controller's devices |
...
**The name-binding criterion.** An enumeration this design defines is name-bound, meaning its type name and every variant name are identical in every SDK binding, where a binding surfaces it to callers or where it serializes on the wire. Without one criterion, four bindings inventing four names would register one later-added variant under four different names. That sentence decides membership, and no enumeration of instances stands beside it, so an enumeration added later is name-bound the moment it meets the criterion. The criterion reaches every section of this design and is not scoped to the section that states it. **A name is bound as this spec spells it, and each binding maps that spelling to its own language's enum convention by one deterministic rule**: PascalCase in Rust, Swift, Kotlin and TypeScript, and SCREAMING_SNAKE_CASE in Python, each derived from the spec's spelling by splitting on word boundaries and recasing, so no binding author chooses a spelling. A TypeScript enum member takes the spec's own PascalCase spelling, because TypeScript fixes no single convention for enum members: naming none leaves `WitnessRead`, `witnessRead`, `WITNESS_READ` and the string-literal union `'WitnessRead'` all defensible, and leaves the shared conformance fixtures unable to compute the TypeScript side of a cross-language comparison. The naming table of `.docs/standards/conventions.md` carries a row for enum variants in each of the five languages.
...
A validating relay declares its write policy in `relay_config` (`18-addressability-and-deployment.md` §18.3.3): `rate_limit_publish`, a per-identifier storage budget, a total storage budget, `per_publish` and `per_byte_stored` (`19-economic-governance.md` §19.8), and `pow_difficulty`. It refuses a key-event PUBLISH that fails any declared term with `IdentityError::StorageBudgetExceeded{scope}` (`03-identity.md` §3.10.10), whose five scopes are `Identifier`, `Total`, `RateLimit`, `Payment` and `ProofOfWork`, and it keeps serving every record it already holds. **The protocol fixes each term's shape and never its value**, and a term whose value is zero or absent is off. **A refusal is never a verdict**: the publisher's recourse is the next entry of its fallback set, and a publish cycle is confirmed when at least one entry accepts it rather than when one answers (`03-identity.md` §3.10.6). **A rank-1 reveal-authorized event, and the segment carrying it, is never refused for `Identifier` or for `Total`**, so an owner's recovery lands while a flood is running. **No per-publisher scope exists**, because a PUBLISH carries no key (ADR-004, the SCP native relay protocol) and a minting party's signing key is fresh for every identity it mints.

**Proof of work.** A PUBLISH of a key-event record carries `nonce: u64` (§9.10.12). A relay declaring `pow_difficulty = N` greater than zero accepts that PUBLISH only where `SHA-256(routing_id ‖ blob_digest ‖ nonce)` carries N leading zero bits, and `N` MUST NOT exceed `MAX_POW_DIFFICULTY` (§9.18.17), so no relay sets a difficulty no client clears. **Difficulty is per relay and never a network constant**: a publisher reads `relay_config` before it publishes, and a relay that raises its difficulty reaches its own publishers and no others, so the protocol governs nothing. Nothing else stops a fill on a relay that takes an unauthenticated permanent write for free, because eviction drains a flood after it stops and does nothing while it runs, and proof of work is the one cost that needs no account and no money.

**Eviction.** A relay MAY evict a key-event record it was not paid to hold where the identity has not re-published it within `EVICTION_IDLE_INTERVALS` multiplied by that identity's witnessing interval (§9.18.17); the self-observation cadence of R10 re-publishes inside that window for a live identity. Under storage pressure it evicts losing suffixes by the eviction rank below first, then the idlest unpaid identities. **A paid record is never evicted for budget**, and an identity's accepted chain at a relay it pays is never evicted. Every controller retains its own full log (R10), so an eviction changes where a record is stored and destroys no history.

**Rent.** `per_byte_stored` on a key-event record buys exemption from eviction and from budget refusal — "relays can already charge rent." — and the blob-TTL clause of `19-economic-governance.md` §19.8 reaches no retained kind. A relay that charges refuses an unpaid PUBLISH under the `Payment` scope.

**Free listed relays** exist for reads and for first contact, and they accept writes under the policy they declare (`18-addressability-and-deployment.md` §18.5.1). The service record at `scp:svc:` is one record per identity, replaced on a higher sequence, and it counts against the same budgets (`03-identity.md` §3.10.2, §3.10.13).

**The fetch bound.** A verifier **MUST** stop fetching once the accepted-chain bytes it has read for one identifier reach `MAX_RETAINED_BYTES`, and it treats the unread remainder as absent rather than as a copy-level defect, so a truncated read produces `Inconclusive{MissingEvents}` and never `Invalid{at_event}`: a chain the verifier did not read is not a chain it found a defect in. **The bound reaches every suffix a verifier fetches, a rank-1 suffix included**, whose exemption above is from retention and from eviction alone; a reveal sits at the first event of the suffix, so a verifier decides the rank before the bound binds. **The fetch bound and the byte bound are two counters that share one value and nothing else**: the byte bound counts divergent-suffix bytes and counts no accepted-chain byte, the fetch bound counts the accepted-chain bytes the verifier has read, and a verifier runs both at once. The fetch bound is not a sequence ceiling, for the reason the segmentation paragraph above gives.

**The eviction rank** is defined for a divergent suffix and for nothing else. It is a scalar: the R6 rank the suffix carries against the accepted chain, read against the commitment standing at that suffix's own fork position. Eviction reads that one term and nothing else, and R6's pairwise comparison decides adoption while no eviction reads it. **The accepted chain sits outside the retained set, carries no eviction rank, and is never evicted.** It is the chain the verifier held before it observed any divergence; at a first-contact verifier, which held none, it is the chain that verifier adopts under R11, and until that moment every chain it fetches is bounded by the fetch bound above, which is a MUST, and by nothing else; and at a validating relay it is the chain whose slot the routing id's first binding-valid frame established. R9 counts every arrival against it. Without that clause the rank term was defined only by reference to a chain the suffix forked from, so the chain every fork hangs off had either no rank or eight, and one conforming reading evicted it first. A suffix is rank 1 in one pair and rank 2 in another, so "never evict a rank-1 suffix" had no referent and two relays evicted different slots from identical inputs.

**The eviction rule, over both bounds.** A candidate that would exceed the count bound or the byte bound is admitted wherever the retained set holds a suffix the verifier may evict whose rank number is at least the candidate's, and the verifier evicts the retained suffix carrying the highest rank number, breaking a tie among equal ranks by evicting the suffix it observed most recently. Evicting the earliest-observed equal-rank suffix would evict the owner's own chain first at every verifier that had been following the identity. A rank-1 suffix is never evicted. A candidate the retained set leaves no evictable suffix for is refused, and the party returns `Contested{…, residue: n}` counting the candidates it could not retain, which is the one signal a refusal produces. An eviction returns no signal to the evicted suffix's publisher, and a suffix a party evicted and a party presents again later is re-ranked as if the party saw it for the first time.
...
**Ceremony order.** Read the standing commitment and the current key list from the adopted key state; generate the next set under §9.7.4.1 items 1, 3a, 4, and 5b through 5d; for a `RootRecovery` generate K′ and the operational keys under the device boundary; for an abandoning rollover issue the retirement Update in every context; persist the recovery handle carrying the composed preimage, into every store the one-reveal-at-a-time rule below names for the running profile; sign; publish; retain; destroy. The persist precedes the signature, because a controller that crashed between signing and persisting would resume by composing a fresh next set, and its second event would reveal the standing commitment a second time.
...
**A witness that lost its store takes case 1.** It holds no record naming the subjects it served and no address at which to look for its own published heads, because those heads sit at each subject's `scp:wit:` address and that address derives from the subject identifier, so it can enumerate nothing and no rule may tell it to. It therefore treats every subject as newly designated. **It seeds only on a chain the designating controller offers it** — the subject identity whose own key state names this witness in its witness set — having verified that chain from the inception event exactly as any relying party does, which is case 1. **A chain any other party offers it is refused**, and the witness reports the refusal and keeps no baseline, until the designating controller offers one. A party holding a forked suffix would otherwise reach a restarted witness first, get it to seed on that suffix, and then have it refuse the honest controller and emit a conflict statement naming the honest chain as the offered one. A re-seed mints no fault proof, because the fault-proof predicate below excludes a seed marker. **A store that is gone sends the witness here; a store that is present and fails to read sends it to the refusal above.**
...
**The fault proof** is two cosigned heads carrying one `witness` value, one `subject` value, one non-zero `previous_cosigned_digest` value, two different `event_digest` values, and a `previous_cosigned_digest` that is not the preimage digest of the event designating that witness for that subject. Those five conditions prove the witness released two successors of one baseline. Two heads for two subjects share no baseline, and the never-zero field keeps a witness's first head after a seed from colliding with a zero placeholder. Excluding the designating event's digest is what stops a seed from minting a proof: one witness seeding twice at one designating event, once at designation and once after a store loss, releases two first heads that meet the other four conditions and indicts an operator that never equivocated. Two listed relays under one operator are not a second path to that outcome, because one operator key is one witness for this predicate (§9.7.4.2 definitions). **Every one of the five is checkable by any party holding the witness's prior head**: `previous_cosigned_digest` MUST equal the preimage digest of the head that witness last published at that subject's `scp:wit:` address, and a relay stores heads under the key `03-identity.md` §3.10.2 states so that a third party fetches both the prior head and the pair from one address. The proof is portable and self-contained against the shipped list, because verifying either signature takes the P-256 key the witness's community-relay-list entry declares. It decides the operator's conduct and never the fork, and it reaches a witness that equivocates while recording one baseline truthfully; a witness that writes a previous digest of its own choosing fails condition three or condition five and is not attributable by this predicate.
...
Every key state a verifier accepts names exactly one `#active` key, because §9.7.4.2 R3 rejects a snapshot listing more than one key `current` in any operational role, and rejects a snapshot whose service-key role discriminator resolves to no key that snapshot lists `current` in that role. No sentinel stands in for an absent operational key.
...
| `MAX_RETAINED_BYTES` | 512 MiB | The product of three registered constants: `MAX_RETAINED_SUFFIXES` (8) × `MAX_EVENTS_PER_SUFFIX_RETAINED` (1,024) × `MAX_EVENT_BYTES` (65,536) = 536,870,912 bytes. **Its two readers are two counters that share this value and nothing else**: R9's byte bound counts divergent-suffix bytes per identifier and counts no accepted-chain byte, and R9's fetch bound counts the accepted-chain bytes one verifier has read for one identifier, so a verifier runs both at once | §9.7.4.2 R9's byte bound; §9.7.4.2 R9's fetch bound; the per-routing-id retention of `03-identity.md` §3.10.2 |
...
| `MAX_EVENTS_PER_SUFFIX_RETAINED` | 1,024 | **The events-per-suffix factor of `MAX_RETAINED_BYTES`'s derivation and nothing else.** It sizes one suffix an honest identity could not produce, because a suffix records key changes and no identity makes a thousand of them. **No retention rule reads it and no party compares a suffix's event count against it**: R9 states its retention rule in the count bound and the byte bound | The derivation of `MAX_RETAINED_BYTES` |
...
| `MAX_POW_DIFFICULTY` | 24 | The ceiling on the `pow_difficulty` a relay declares, so no relay sets a difficulty no client clears. Twenty-four leading zero bits is about sixteen million SHA-256 evaluations, which a phone clears in seconds and a flood pays for on every one of its writes | §9.7.4.2 R9's proof-of-work rule; the `relay_config` shape of `18-addressability-and-deployment.md` §18.3.3 |
| `EVICTION_IDLE_INTERVALS` | 8 | The multiplier on an identity's own witnessing interval past which a relay MAY evict a key-event record it was not paid to hold. R10's self-observation cadence re-publishes at least once per witnessing interval, so eight intervals of silence separate an identity that stopped from one whose controller missed a cycle | §9.7.4.2 R9's eviction rule |
...
|-----------|---------|-------|-----------|----------------|
| `rate_limit_publish`, per-identifier storage budget, total storage budget | Off (absent) | Any value the operator sets; zero or absent means the term is off | The relay declares each term in `relay_config` and refuses a key-event PUBLISH that fails it with `StorageBudgetExceeded{scope}`. The protocol fixes each term's shape and never its value, and its one floor is the rule that a rank-1 reveal-authorized event is never refused for `Identifier` or `Total` | §9.7.4.2 R9; §18.3.3 |
| `pow_difficulty` | 0 (off) | 0 to `MAX_POW_DIFFICULTY` (§9.18.17) | Leading-zero-bit requirement on `SHA-256(routing_id ‖ blob_digest ‖ nonce)` for a key-event PUBLISH. Per relay, read from `relay_config` before publishing, never a network constant | §9.7.4.2 R9; §9.10.12; §18.3.3 |
