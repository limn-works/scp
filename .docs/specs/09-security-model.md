...
**One signature encoding, and one signature per message and key.** An SCP signature under the §9.5.1 construction is the **64-byte raw form**: `r` then `s`, each a 32-byte big-endian integer, SEC1 fixed-width, never DER. A signer that runs in software MUST derive its per-signature nonce deterministically under RFC 6979 with SHA-256, and every verifier MUST reject a signature whose `s` exceeds half the group order. Together those two rules make one message under one key produce one accepted signature: RFC 6979 removes the nonce as a source of variation and a source of private-key leakage under a weak random source, and low-`s` enforcement removes the second, equally valid `(r, n − s)` encoding that a relay could otherwise substitute to change a signature's bytes without holding the key. **A hardware signer that draws a random nonce conforms**, because the deterministic rule binds the software case that RFC 6979 exists for; the low-`s` rule binds every verifier without exception.
...
4. **Approved custody methods.** Under the key-event log a copy of a pre-rotation key alone does more lasting damage than a copy of the root alone: a root leak is recoverable by a reveal, while a leaked pre-rotation key lets its holder contest the identity at any later date and, held exclusively, take it over and then kill it permanently (the failure-mode table below). The pre-rotation key is therefore the backstop against root compromise and at the same time the key whose leak is least recoverable, and its custody is ranked on two axes with **post-spend destructibility** as the primary one: whether the controller can destroy every copy of a spent key (§9.7.4.2 R5) and, where the substrate supports it, obtain the §9.15 attested destruction; and resistance to theft at rest. Any one method is sufficient **subject to the recovery-authority-residence requirement of item 3a**. Every method holds a P-256 key (§9.5); a method that produces a WebAuthn assertion rather than a raw signature satisfies §9.7.4.2 R3 through the assertion form R3 admits.
   ...
   **The residual these three carry, disclosed here and in no other section.** Each of them yields key bytes, so a copy can outlive the reveal that spent it. Where a copy of a spent pre-rotation key later reaches any party, that party forks below the event that spent the key and reveals the same commitment there. What happens next is the root rule of §9.7.4.2 R6 and nothing else: where the controller's own revealing event on that shared prefix carried the standing root's signature, the controller stands at rank 1 and the copy holder at rank 2 and the controller wins; where the controller's revealing event carried no root signature — a `RootRecovery{Lost}`, signed because the root was already gone — both suffixes stand at rank 2, R7 makes the identity contested, and the contest is terminal by key material at any later date. **The full precondition of the terminal case is therefore two losses together: the root lost, and a copy of a spent pre-rotation key leaked.** The SDK MUST state that outcome, in those terms, at custody selection on this profile. **On the consumer profile the case cannot arise**, because a passkey and a FIDO2 credential are non-exportable and an HSM key is non-extractable, so no copy exists to leak. This section discloses the choice and does not forbid it: a headless controller MAY select a copyable method, having read the disclosure.
   ...
   **A reveal may be human-attended on every profile, the headless one included.** Item 3a already states that a reveal-authorized event is a rare, separately authorized event and not a daily operation, so no profile's cell turns on whether a human is present when the reveal runs. The three headless methods each need a human act at reveal time — re-entering a mnemonic, supplying an Argon2id passphrase and mounting the offline medium, collecting three Shamir shares from their holders — and the table answers all three the same way. What a cell does turn on is whether the profile can hold the substrate and reach it under item 3a's principal-distinctness test.

   | Method | Headless (server, CLI, daemon) | Desktop | Mobile | Browser |
   |---|---|---|---|---|
   | Passkey (the default) | Does not conform — the profile is defined by the absence of a passkey API | Conforms — the platform provider | Conforms — the platform provider | Conforms — the browser's WebAuthn API |
   | FIDO2 security token | Conforms — USB or smartcard reader attached to the host | Conforms — USB or NFC | Conforms — NFC or USB-C token | Conforms — the browser's WebAuthn API reaches an external authenticator |
   | HSM-resident key | Conforms — the primary headless method | Conforms — network HSM under a separate principal | Conforms — network HSM under a separate principal, reached over the network the same way | Does not conform — a browser holds no credential for a principal the page's own origin cannot reach |
   | Encrypted offline backup | Conforms | Does not conform — the method leaves the consumer profile | Does not conform — the method leaves the consumer profile | Does not conform — the method leaves the consumer profile |
   | Shamir secret sharing (3-of-5) | Conforms | Does not conform — the method leaves the consumer profile | Does not conform — the method leaves the consumer profile | Does not conform — the method leaves the consumer profile |
   | Paper backup (BIP39 mnemonic) | Conforms | Does not conform — the method leaves the consumer profile | Does not conform — the method leaves the consumer profile | Does not conform — the method leaves the consumer profile |

   Every conforming cell is still subject to item 3a: the SDK MUST reject a selection whose recovery authority is reachable from the profile's operational custody, whatever this table says. That test, not the table, is what rejects a mobile HSM arrangement whose KMS grant the mobile application's operational signing flow can assume. **It also decides the passkey cells:** a passkey the daily operational signing flow can itself assert against is co-located custody, so the SDK MUST hold the pre-rotation passkey as a credential that flow holds no path to.
...
- **P copied, root intact and trusted.** Both parties can reveal, and the root rule separates them. The controller signs a `CommitmentRollover`, which R3 makes the standing root co-sign, so the controller's suffix is rank 1. The copy holder holds no root, so the only reveal-authorized event it can produce is a `RootRecovery{Lost}`, which carries no standing-root signature and stands at rank 2. **The controller wins the comparison outright and the identity is not contested.** The rollover also fixes a commitment the copy holder cannot reveal, which forecloses a second attempt on the controller's suffix. This is the case Alec's root-wins ruling of 2026-09-07 decided, and it is the reason an intact root is worth holding.
...
- **Root compromised, P held only by the controller.** The controller signs a `RootRecovery` — `CoSigns` while it still holds a threshold of its root, `Lost` otherwise. The attacker cannot reveal, so its suffix is rank 3 and the controller's is rank 1 or rank 2; the controller wins wherever the attacker forked. The recovery's snapshot (§9.7.4.2 R8) replaces every install the attacker made, and its K′-signed chain has its own store slot, keyed to K′ (R9), so the attacker cannot evict it.
...
- **Root compromised, owner recovered, and a backup of a spent pre-rotation key later leaks — the residual this design carries.** A party that obtains that backup forks below the event that spent the key and reveals the same commitment there. **The full precondition is two losses together: the root was lost, so the owner's own recovery was a `RootRecovery{Lost}` and stands at rank 2 on that shared prefix; and a copyable backup of the spent key leaked.** Where the owner's recovery instead carried the standing root's signature, that suffix is rank 1, the later fork loses, and this row does not arise. Under the full precondition the outcome splits on one further fact. **Where the leaking party also holds the root the owner lost**, its fork carries that root's signature — the root standing at the fork position is the pre-recovery one — so the fork is rank 1 against the owner's rank 2 and **the fork wins outright**; the root cannot be recovered by key material, and §9.7.4.2's fourth worked example traces it. **Where the leaking party holds only the spent key**, both suffixes are rank 2 and the identity is contested at any later date, terminal by key material. **On the consumer profile neither case can arise**, because item 4's three methods are non-copyable, so no backup of a spent key exists to leak; the row is a headless-profile residual and item 4 discloses it there. Destruction that reaches every backup medium (item 4's destructibility axis, §9.7.4.2 R5) is what prevents it on that profile, and §9.7.4.2 R5 destruction and item 5f destroy, and item 5d verifies, only the controller's own copies.
- **Two devices both holding the root, no attacker.** Each device appends a `KeyState`, neither of which reveals, so both suffixes are rank 3 and the tie is pending (§9.7.4.2 R7). The controller ends it by signing a `CommitmentRollover` or a `RootRecovery{CoSigns}` on the suffix it keeps (§9.7.4.2 R10), which puts that suffix at rank 1 and decides the divergence. **That exit costs a commitment**: it mints a spent key whose later leak reaches the residual row above on a headless profile, and reaches nothing on the consumer profile. R10 also states the cheaper avoidance, which is for a controller's second device to fetch the chain and re-sign its intended change on the head it finds rather than publishing a second event at one sequence.
...
- The **root** of an identity is a **root set** — an ordered list of at most `MAX_ROOT_SET_SIZE` public keys (§9.18.17) — with a **signing threshold** t. A **root signature** is an **indexed signature group**: the signed preimage names the list of root-set indices that sign, the signature field carries exactly one 64-byte slot per named index in index order, every indexed signature MUST verify against the member at its index, and the named indices MUST number at least t and MUST be distinct. Every statement below that says a root "signs" means a root signature. A personal identity is the 1-of-1 case: one key, threshold one, one index, one slot. An organization sets a list and a threshold so that no one officer acts alone. **A root signature MAY be carried as a WebAuthn assertion** in place of a raw signature: the authenticator signs `authenticatorData || SHA-256(clientDataJSON)` with the event's §9.5.1 preimage digest carried as the WebAuthn challenge, the slot carries that assertion's `authenticatorData`, its `clientDataJSON`, and its 64-byte P-256 signature, and a verifier accepts the assertion form beside the raw form. A verifier MUST recompute the challenge from the event's own preimage and MUST reject an assertion whose challenge is anything else; it reads no other field of `clientDataJSON` and it reads the `rpId` hash inside `authenticatorData` as public information about the credential's custody rather than as a claim about the identity. The assertion form exists because the SDK's default root custody is a passkey (§9.7.4.1 item 4), and a WebAuthn authenticator signs its own envelope rather than a preimage handed to it. The **standing root** at a position in a chain is the root set and threshold the latest root-installing event at or before that position installed: the inception event installs the first, and a `RootRecovery` installs each later one.
...
**The outcomes are stated over the first differing position, and the enumeration is exhaustive by construction.** For an accepted frame the relay compares the frame's events, in order, against the events the slot already holds at those positions. Exactly one of three cases applies:

1. **Every event of the frame is byte-identical to the event the slot holds at its position, and the frame's range lies inside the slot's chain.** The frame is an idempotent TTL refresh and changes no slot. This is the case a 6-day republication of an inception-only chain takes.
2. **Every event of the frame that the slot already holds is byte-identical, and the frame continues past the slot's head.** The frame's remaining events extend that slot and the slot's chain grows by them. A publisher whose chain outgrew one frame publishes segment two this way (`03-identity.md` §3.10.5 step 3), and a healing frame that overlaps a slot and extends past it takes this case too.
3. **A first position exists where the frame's event differs from the event the slot holds there.** The two chains diverge (definitions) at that position, so the frame's events form a second divergent suffix over the shared prefix, and that suffix occupies its own slot under the admission and eviction rule below. The relay stores the prefix once and both suffixes over it. A relay replaces no event it already holds.

**Serving order and the page walk.** A relay serves a slot's frames in ascending order of each segment's first sequence, and stores each frame with a `stored_at` preserving that order. A resolver therefore pages a slot by re-issuing QUERY with `since` set to the `stored_at` of the last frame it accepted, until it holds inception through head or a page returns no frame it does not already hold. This sentence is the one home of the page walk; §9.10.12 and `03-identity.md` §3.10.2 cite it, and no rule reads a wire parameter ADR-004's QUERY does not carry.

**The retained set holds two further object kinds, and both stay bounded by construction.** A verifier retains, per retained suffix, at most one cosigned head per witness the key state at that suffix's head names — the highest sequence it verified from that witness on that suffix — and at most one conflict statement per (witness, offered head digest) pair. `MAX_WITNESS_SET_SIZE` × `MAX_RETAINED_SUFFIXES` (§9.18.17) bounds each product at 256 objects per identity, which is 61,440 bytes of cosigned heads and 63,488 bytes of conflict statements at the fixed widths §9.7.4.3 states, so this rule needs no constant of its own. **Both kinds are evidence a party may read and never an input to a verdict** (definitions), so a verifier that retains none of them reaches every verdict of this section unchanged.
...
**The one check.** Before it cosigns, a witness looks up the head it last cosigned for this subject, and the chain it is now asked to cosign MUST carry that exact event digest at that exact sequence. Where the chain carries it, the witness cosigns. Where the chain does not, the witness refuses and emits a conflict statement. **The check is unconditional and it has no carve-out**, including for a chain that supersedes the witness's last-cosigned chain through a reveal-authorized event: at a witness a genuine recovery and a theft of a spent pre-rotation key are the same object, and a witness holds no evidence that separates them. SCP's log is a hash chain rather than a Merkle tree, so the segment from the previously cosigned sequence to the new head is itself the consistency proof, which the witness verifies by walking predecessor digests. Two preconditions sit under the check and neither is a second judgement: the witness verifies the chain exactly as any relying party does, under §9.7.4.2 R2 and R1 through R4, and it refuses a subject whose key state at the head does not name it, because signing for undesignated subjects is an unbounded workload. Its `observed_at` MUST exceed that of its last cosigned head for the subject and MUST sit no further ahead of its own clock than §9.14's skew tolerance; a witness whose stored `observed_at` exceeds its clock by more than that tolerance re-seeds its baseline.

**The witness's durable per-subject state.** A witness retains, per subject it has ever cosigned, the subject identifier, the sequence and event digest of the head it last cosigned, the preimage digest of that cosigned head, and its `observed_at`. Each record is about 112 bytes and a witness holds at most `MAX_SUBJECTS_PER_WITNESS` of them (§9.18.17), which bounds the store by construction. **The witness persists the record durably before it releases the cosignature that record accounts for.** A witness whose record is absent or unprovable — a cold restart, a storage fault, a restore from an older backup, or the future-stamp trigger above — MUST NOT cosign that subject until it has re-verified the chain from the inception event and re-seeded the baseline, because a witness cosigning on a cold baseline could sign both sides of a fork with no malice. `17-persistence-and-storage.md` §17.17.4 classifies an in-memory witness store as a security nullifier for that reason.

**Seeding a baseline reads evidence and never the witness's own memory.** A witness with no baseline fetches that subject's cosigned heads from the subject's `scp:wit:` address **from at least two entries of the community relay list under distinct declared operators** — §9.7.4.2 R11's first-contact floor, which applies because a seeding fetch is a first contact over a different record kind, and without which one relay serving only heads below a fork sets the floor the attacker chose. It verifies each head against the key its signer's own key-event log lists `current` in the `#active` role at the position that object names, and from the highest-sequence head it verified it adopts that head's `sequence`, `event_digest` and `observed_at`. **It counts a verified head from any witness of any set the chain ever carried, at any age**, because it needs a floor rather than current evidence. Reaching fewer than two such sources, or finding no head it can verify, it cosigns only an offered chain whose head is the inception event itself and otherwise refuses and says so.

**A recovered identity reaches witnesses by naming a fresh set.** The outgoing witnesses hold a last-cosigned head the recovered suffix does not carry, so the one check makes them refuse it and go on refusing it. The recovery event names a fresh set; those witnesses hold no last-cosigned head for the subject, so the seeding path above admits them and each cosigns from a chain it verified. **A party holding a spent pre-rotation key takes the same path**, so both suffixes end up carrying cosignatures from disjoint sets and **neither set's cosignatures decide anything**: the relying party ranks the two suffixes under §9.7.4.2 R6's root rule, exactly as it would holding none. A controller whose witnesses merely went silent replaces them with a `KeyState`, which spends no commitment (§9.7.4.2 R10).
...
**`previous_cosigned_digest` is read here, and this is its one reader.** A witness's cosigned heads for one subject chain through that field, so **two of them naming the same `previous_cosigned_digest` and different `event_digest`s prove, from the two objects alone, that the witness released two successors of one baseline** — a witness signing both sides of a fork, which its durable store and the one check together forbid. **That proof is about the operator's conduct and never about the fork**: a party holding it ranks the two chains under R6 exactly as it would without it.

**What a relying party may read a cosignature for.** Three things, and no fourth.

1. **Freshness, as one source of two.** A cosigned head from a recognized operator bounds how stale the head that party holds may be, and §9.11 reads that bound. **It is never the only source of it**: §9.7.4.2's definitions give a party a second and independent one, its own two-relay read of the adopted head, and §9.11's standing is set only where a party holds neither and cleared by either. A cosignature therefore spares a party that read; its absence withholds nothing and closes no gate.
2. **First contact.** A party resolving an identity for the first time holds no reject-lower baseline, so a cosigned head from a recognized operator is evidence that operator served this chain at a stated time. It supplements R11's two-relay floor and replaces no part of it.
...
**A witness operator's own chain is resolved without recursion.** A party verifying a cosignature resolves that operator's chain under §9.7.4.2 R1 through R4, and under R6 and R7 where it holds two divergent chains for it; the trust anchor is the identifier the operator's community-relay-list entry declares. **The floor here is that entry plus one other list entry** under a different declared operator, and a party reaching only one reads no cosignature. Applying R11 unchanged would make counting the first source require resolving an operator's log, which would require two proven sources, each requiring another log.

**Which subjects a witness admits, and what it does at capacity.** A witness admits a subject when that subject's key state first designates it, in arrival order, to `MAX_SUBJECTS_PER_WITNESS` subjects. **It never evicts an admitted subject**, because eviction would put that subject into the no-baseline state at every witness under the same storage pressure. **A witness at capacity refuses and names capacity as the reason**, so a controller designates a different operator rather than reading the refusal as a fork.

**What the SDK ships.** The SDK's default witness set for a new identity is the entries of the community relay list under distinct declared operators, capped at `MAX_WITNESS_SET_SIZE` (§9.18.17), at a witnessing interval of `DEFAULT_WITNESSING_INTERVAL` (§9.18.17). **An identity is usable from the moment its inception event is published**, because no rule reads a cosignature, so identity creation completes at publication and waits on no witness. **The SDK MUST warn a controller that replaces the default set with unlisted operators, or that designates no witness at all**, and MUST state the consequence in these terms: the identity's freshness then rests on relay reads alone, so each peer corroborates its head by reading that head from two community-relay-list entries under distinct declared operators (§9.7.4.2 definitions) rather than from a cosignature. **The warning MUST NOT say that such an identity is unresolvable, unadmittable, or held at `Unresolved`**, because no witness choice withholds the two-relay read and §9.11 sets its standing only against a party holding neither source. What the controller gives up is portability — a cosigned head travels to a party that did not read, and a relay read does not — and that cost falls on a peer whose network reaches fewer than two of those operators. A self-hoster's own relays therefore serve its transport while the community set serves its witnessing.

**Cosigning is free.** `18-addressability-and-deployment.md` §18.5.1 states that rule and this section reads it. Economic rules (`19-economic-governance.md`) may price a witness's cadence and availability guarantees; they may never price the act.

**What this layer does not cover, stated plainly rather than implied.**

1. **It resolves no fork.** A witness signs or refuses and never adjudicates, so §9.7.4.2 R6's root rule settles two divergent chains and nothing this layer produces does. A conflict statement says a fork exists and says nothing about which side to take.
2. **The one check gives no evidence at or above a witness's last cosigned head.** Where two events at one sequence each extend that head, the witness cosigns whichever it sees first and refuses the other, so the objects record a race rather than a wrong. R6 ranks the two suffixes regardless.
3. **Independence is named and never proven**, per the paragraphs above. A relying party's recognized set is a judgement it makes out of band.
4. **A silenced witness set costs the identity the portable half of its corroboration and nothing else.** A peer that can still read the identity's head from two community-relay-list entries under distinct declared operators corroborates it that way (§9.7.4.2 definitions) and admits and grants as before. **A peer reaching fewer than two such operators holds no corroboration and moves to `ContinuityStanding::Unresolved`**, at that peer alone and for a reachability reason rather than a witnessing one. The remedy for the witnessing half is a `KeyState` naming a fresh set (§9.7.4.2 R10).
5. **The witness's durable per-subject store is a correctness dependency** on infrastructure the protocol does not control, and the evidence-seeded baseline above mitigates rather than proves: it is exactly as good as what two relays under distinct declared operators chose to serve at the subject's `scp:wit:` address.
6. **No rule bounds the rate at which one party designates witnesses**, so a party minting identities in bulk drives every operator to `MAX_SUBJECTS_PER_WITNESS`. This section states that rather than implying a bound: a sound rate limit would have to bound designations per designating party without pricing the act and without reading any field an inception event's author chooses, and nothing in this corpus supplies one. Because no rule reads a cosignature, that flood costs new identities their §9.11 standing at peers and never their ability to resolve.
...
| 4 | `events` | `event_count` records in ascending sequence order, each a 4-byte big-endian length prefix over the event's §9.5.1 signed preimage bytes, followed by the signature field the event's kind and index lists fix (§9.7.4.2 R3) |

A decoder MUST reject a frame whose `event_count` is zero, whose `last_sequence` is below its `first_sequence`, or whose declared range does not equal the sequences its events carry — `last_sequence − first_sequence + 1` events, each one sequence above its predecessor. The signature field's length is a function of the event's kind and its signer index lists, both of which the preimage binds, so the decoder derives it from the preimage it just read rather than from a second length on the wire (§9.7.4.2 R3, the closed signature layout).
...
- **`Unresolved` means nothing was learned about the identifier, not that the identifier is suspect.** **Two causes set it, and each carries its own clearing condition.** The first is a resolution that reached too few sources, which R14 reports as `Inconclusive`; it clears on any later resolution that adopts a chain, with no human step, because a verifier that reached one relay learned nothing about the identity that a verifier reaching two would have contradicted. The second is an identity whose adopted head **this party has corroborated from neither of the two sources §9.7.4.2's definitions state** — neither a fresh cosigned head from an operator its recognized set carries, nor its own read of that head from two community-relay-list entries under distinct declared operators inside the staleness window. **A missing cosignature alone never sets this standing.** An identity whose witnesses are silent, whose witnesses the running release does not list, or which designates no witness at all is corroborated by the two-relay read like any other, so none of those three states reaches this cause on its own; what reaches it is a party that holds no corroboration of either kind, which is a party that also could not read the identity's head from two independently-operated relays. **That second cause reports no `Inconclusive` verdict of any kind**: no rule of §9.7.4.2 reads either source, so an uncorroborated identity resolves normally and returns `Adopted` or `Confirmed`, and the party keeps deriving key state from the head it adopted (§9.7.4.2 R8). **It clears on either source**, and on neither the bare fact that a resolution adopted a chain: a clearing rule reading adoption alone would clear this standing in the same act that set it, and a clearing rule reading the witness source alone would hold an identity with no listed witness at `Unresolved` forever, which is the gate on a cosignature this corpus forbids. The second cause reads the same way as the first, and for the same reason: a witnessing outage is no evidence against the tokens the identity issued, so withholding them would reach every token the victim ever issued on the strength of a relay outage.
...
Each of the three MUST read the standing on a peer's `RootRecovery` when it lands, because a stable identifier keeps matching an allowlist entry that a rename used to invalidate.
...
|----------|-------|-------|----------------|
| Signature algorithm | ECDSA on NIST P-256 with SHA-256 (FIPS 186-5) | All identity keys, envelope signatures, UCAN (ES256), MLS leaf credentials. RFC 6979 nonces in software signers; verifiers reject high-`s` | §9.5 |
...
| Key algorithm | `0x01` = ECDSA on NIST P-256 with SHA-256 | The one registered value for v1 (§9.7.4.2 definitions). Every state-carrying event carries it beside each key and each commitment, and a verifier rejects a chain carrying any other value. The field exists from the first schema version because the inception preimage is frozen by the identifier construction, so a later revision registers a second algorithm without renaming any existing identity | §9.5, §9.7.4.2 definitions, R3 |
...
| `MAX_SUBJECTS_PER_WITNESS` | 10000000 (10 million) | Maximum subjects one witness holds durable per-subject state for (§9.7.4.3). At about 112 bytes per record the store stays near a gigabyte, so the bound is what makes that figure a derivation rather than an illustration. A witness admits subjects in arrival order to this bound, **never evicts an admitted subject** — eviction puts the subject into the no-baseline state at every witness that evicted it, which converts free storage growth into a targeted freeze of that identity's key events — and refuses a designating subject at capacity with capacity as the stated cause, which the identity-creation flow reads at inception | §9.7.4.3 |
| `WITNESS_STALENESS_INTERVALS` | 3 | Multiplier on the witnessing interval that fixes the **staleness window** (§9.7.4.2 definitions). It bounds both corroboration sources: a cosigned head is fresh inside it, and a party's own two-relay read of the adopted head counts inside it. A witness misses two consecutive rounds without the identity going stale, and a third miss is a signal rather than noise. **The one rule that reads corroboration is §9.11's lapse standing**, which fires only where a party holds neither source, so this constant decides how often a peer re-reads and decides no verdict | §9.7.4.2 definitions, §9.11 |
