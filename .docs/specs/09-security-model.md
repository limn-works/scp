...
**Signature form.** A signature's form is a one-byte discriminator carried in a signature-form list that the preimage binds beside the signer index list of its group. Its type name is `SignatureForm` and its two variant names are `Raw` and `WebAuthnAssertion`, name-bound under the criterion below. A verifier MUST reject any other value.

`SignatureForm::Raw`, discriminator `0x01`: the signature a named index contributes is the 64-byte raw ECDSA signature that §9.5 defines, low-`s` normalized by the signer, verified against the key at that index over the event's preimage digest.

`SignatureForm::WebAuthnAssertion`, discriminator `0x02`: the signature a named index contributes is
...
**Rent.** A PUBLISH carries an optional `payment_receipt` (§9.10.12): the `PaymentReceipt` bytes of `19-economic-governance.md` §19.2.1, tagged with the adapter that issued them. A relay declaring `per_publish` or `per_byte_stored` verifies that receipt through the named adapter, which is `PaymentAdapter::verify` of §19.2.1 — "relays can already charge rent." **"Paid" means "carried a receipt this relay verified and bound to this write" in every storage rule of this section.** **One receipt pins one record at one relay, and four checks against bytes the relay already holds decide it**: the receipt's `payee` equals this relay's declared `economic.payee`; the receipt's `write_binding` equals `SHA-256(payee_identifier ‖ frame_value_digest)`, where `frame_value_digest` is `SHA-256` over the `value` bytes this PUBLISH carries and §9.10.12's layout fixes `value` as the frame's trailing remainder; the receipt's `receipt_id` is one this relay has not already spent; and `verified_amount` covers the declared price for this write's byte count. The relay refuses `Payment` where the PUBLISH carried no receipt, where the verification failed, or where any of those checks fails. **Three of them are the bindings**: the `payee` term binds the receipt to this relay, the `write_binding` term binds it to these bytes, and the unspent `receipt_id` binds it to one use. **The operand is named `frame_value_digest` and never `value_digest`**, because the relay proof's 200-byte layout carries a `value_digest` taken over a response's blob bytes and one SDK computes both. **No check binds the publisher**, because a PUBLISH carries no key and the receipt's `payer` names a party the connection need not be; spending the `receipt_id` at this relay is what bounds re-use, and that is the residue this rule leaves. **`economic.payee` is the relay identifier every charging relay declares** and every publisher reads through the `POLICY` query, so a self-hosted relay that charges has one and a relay that charges nothing needs none. `03-identity.md` §3.10.2 orders the payment verification ahead of every check a receipt exempts. `per_byte_stored` on a retained record pins that record outside the ring and buys exemption from refusal for `Identifier` and for `RateLimit`. **It buys no exemption from `Total`: pinned bytes count against the total budget, and a relay refuses a paid PUBLISH for `Total` whoever offers to pay**, so the ceiling an operator declared bounds what that operator stores against a funded party. The blob-TTL clause of `19-economic-governance.md` §19.8 reaches no retained kind.
...
**Self-observation.** The SDK MUST fetch its own identity's log at a stated cadence, no less often than the witnessing interval the key state names and on every launch, from at least one relay in the fallback set, and MUST treat an inability to complete that check as an alert. **The one publication duty this cadence carries: where the fetch shows that a fallback-set entry holds no record for this identity, the SDK re-publishes to that entry.** **There is no unconditional re-PUBLISH on every cadence**, because the ring buffer of R9 displaces an identity on arrival rather than on age, so a write that changes nothing costs the identity its own place at the head of the ring and buys it nothing. The fetch establishes what the controller is standing on, and the conditional re-publication is what restores a record the ring displaced. That cadence is also the submission cadence of §9.7.4.3.
...
**The fault proof** is two cosigned heads carrying one `witness` value, one `subject` value, one shared `previous_cosigned_digest` value, two different `event_digest` values, and **neither head setting `seed`**. Those five conditions prove the witness released two successors of one baseline, and **the five conditions are the test**. Every condition reads the two heads' own fields, so any party holding the pair decides the predicate without the subject's chain and without a prior head of the witness. **A witness that sets `seed` on a head makes no proof out of that head**, which is what separates a re-seed from an equivocation: a witness that re-seeded declared it in the byte, and a witness that released two successors of one baseline declared nothing. Two heads for two subjects share no baseline, and the never-zero field keeps a witness's first head after a seed from colliding with a zero placeholder. Excluding the designating event's digest is what stops a seed from minting a proof: one witness seeding twice at one designating event, once at designation and once after a store loss, releases two first heads that meet the other four conditions and indicts an operator that never equivocated. Two listed relays under one operator are not a second path to that outcome, because one operator key is one witness for this predicate (§9.7.4.2 definitions). Conditions one through four read the two heads' own fields, and condition five reads the chain: a third party fetches the subject's chain, locates the event designating that witness for that subject, and compares that event's preimage digest against the shared `previous_cosigned_digest`. No condition reads a prior head of the witness, so the predicate is decidable by any party holding the pair and the chain, and a relay stores heads under the key `03-identity.md` §3.10.2 states so that a third party fetches both heads from one address. The proof is portable and self-contained against the shipped list, because verifying either signature takes the P-256 key the witness's community-relay-list entry declares. It decides the operator's conduct and never the fork.

**Worked example 6 — the predicate applied, and it states no sixth condition.** A witness seeds on a subject and publishes a first head carrying `seed: 0x01`, then a second head carrying `seed: 0x00` and the first head's preimage digest. It then releases two heads at one sequence for that subject, each carrying a different `event_digest`, both carrying the second head's digest as `previous_cosigned_digest`, and both carrying `seed: 0x00`. A third party holding the two heads reads one `witness` value, one `subject` value, one shared `previous_cosigned_digest` value, two different `event_digest` values, and `seed` clear on both, so all five conditions hold and the pair attributes the equivocation to that operator. **The same party holding the first head beside any later head reads `seed: 0x01` on one of them, so that pair makes no proof**, which is how a declared re-seed stays outside the predicate. A witness inventing a shared `previous_cosigned_digest` no head of its own ever carried is attributable under the same five conditions, because no condition reads what the witness published before.
...
A decoder MUST reject a frame whose `event_count` is zero, whose `last_sequence` is below its `first_sequence`, or whose declared range does not equal the sequences its events carry — `last_sequence − first_sequence + 1` events, each one sequence above its predecessor. The decoder reads the event's kind, its signer index lists, and its per-signature form lists out of the preimage it just read, and derives the signature field's length from those three together with the two length prefixes each assertion-form signature carries inside itself (§9.7.4.2 R3, the closed signature layout). It walks the field one signature at a time: a `0x01` signature is 64 bytes, and a `0x02` signature is a 4-byte length and that many bytes of `authenticatorData`, then a 4-byte length and that many bytes of `clientDataJSON`, then 64 bytes. A decoder MUST reject a signature whose `authenticatorData` length is below `MIN_AUTHENTICATOR_DATA_BYTES` or exceeds `MAX_AUTHENTICATOR_DATA_BYTES`, whose `clientDataJSON` length is zero or exceeds `MAX_CLIENT_DATA_JSON_BYTES` (§9.18.17), or whose declared length runs past the field, and MUST reject an event whose signature field does not end exactly where the last signature ends. **No length outside a signature states the field's size**, so the frame carries no second length for a decoder to disagree with. **The three lengths `value` itself supplies are bounded here too**, because `value`'s own trailing-remainder shape bounds none of them: a decoder MUST reject an event whose 4-byte preimage length prefix runs past the remaining `value` bytes; MUST reject a frame whose events do not end exactly at the end of `value`, so a relay cannot append bytes after the last event and have one binding accept the frame while another rejects it; and MUST NOT size any allocation from `event_count` before it has read that many events.
...
COSIGNED-HEAD-RECORD (CosignedHeadRecordV3) :=
  ...
  value:      [u8]                     # trailing remainder = one or more 208-byte cosigned heads (§9.7.4.3)

WITNESS-CONFLICT-RECORD (WitnessConflictRecordV4) :=
  version:    u8         = 4
  subject:    [u8; 32]                 # the subject identifier
  value:      [u8]                     # trailing remainder = one or more 216-byte conflict statements (§9.7.4.3)
...
- **Pre-rotation key copied, root lost:** The controller signs a `RootRecovery{Lost}`; §9.7.4.1's failure-mode table states the outcome. Step 1a proposes Remove for no leaf while the contest stands, because R7 gives no party a `current` key state to test leaves against, and steps 2–6 do not run while it stands.
...
| `MAX_RETAINED_BYTES` | 512 MiB | The product of three registered constants: `MAX_RETAINED_SUFFIXES` (8) × `MAX_EVENTS_PER_SUFFIX_RETAINED` (1,024) × `MAX_EVENT_BYTES` (65,536) = 536,870,912 bytes | §9.7.4.2 R9's byte bound, over divergent-suffix bytes per identifier; §9.7.4.2 R9's fetch bound, over the accepted-chain bytes one verifier has read for one identifier from one source in one resolution; the per-routing-id retention of `03-identity.md` §3.10.2 |
