...
- **Bootstrap channel for pseudonym announcements.** A member who has just joined (or just imported / restored a context) does not yet know any peer pseudonyms — their pseudonym registry is empty. To close this bootstrap gap, pseudonym announcement messages — and ONLY pseudonym announcement messages — are sent to the shared `context_routing_id = SHA-256("scp:context-routing:" || context_id)` EXCLUSIVELY. They are NOT also fanned out to known peer pseudonyms. Every group member already subscribes to the shared routing ID (for MLS management traffic), so a single publish to the shared RID reaches every current subscriber regardless of whether the sender has learned their pseudonym yet — there is no need to additionally address peer pseudonyms. Sending to the shared RID alone (rather than the union of the shared RID and peer pseudonyms) is precisely what avoids linking a pseudonym to its context at the relay: if the identical MLS ciphertext appeared at both the shared RID and a peer pseudonym RID, a relay that can derive `context_routing_id` could positively correlate that pseudonym to the context by matching the blobs. Application messages (all non-announcement payloads) continue to use strict pseudonym-only fan-out — they are never sent to the shared routing ID, preserving the unlinkability property for sensitive content. The announced pseudonym VALUE is validated at ingest: a member may not announce the zero sentinel, the shared `context_routing_id`, or the broadcast `SHA-256(context_id)` RID for their own DID; such announcements are rejected and the registry is left unchanged. Privacy tradeoff: an active relay can observe that announcement traffic flows over the shared RID, but the relay can already observe MLS membership and subscription patterns on the shared RID; the sensitive payload remains inside pseudonym-only traffic. Receivers that decode a shared-RID blob as an announcement update their registry and deliver the announcement through the normal reorder buffer pipeline.
