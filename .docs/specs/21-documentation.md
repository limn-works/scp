...
The `publish-docs` job in `docs.yml` handles aggregation and deployment. Rust, Python, TypeScript, and Kotlin docs are generated by jobs within `docs.yml` itself. Swift DocC docs are generated by the `swift-xcframework` job in `build-matrix.yml` (see §21.10.6) and collected via the shared `docs-swift` artifact name.
