...
The conformance suite validates the vectors against the **Rust core only** — `scp_protocol::context::outlets::registry::compute_outlet_registration_canonical_bytes` + `verify_outlet_registration_signature` + direct Ed25519 verification. The production registration path runs the same check: `register_outlet` and `update_outlet` call `verify_outlet_registration_provenance`, which derives the operator's key from `operator_did` and calls `verify_outlet_registration_signature`, so a registration with an absent, mis-sized, or forged signature never reaches a context registry. The PyO3, NAPI, and UniFFI bridges each attach a real signature before that call: they sign the canonical digest with the operator's own key from their key custody when `operator_did` names a local identity, and otherwise accept the operator's 64-byte `operator_signature` from the caller. The WASM bridge is genuinely N/A per ADR-057, the in-browser client. The suite:
