...
**Category C — governance sets the context's agent restrictions.** A context restricts what an agent may do inside it through `agent_keys_allowed`, an agent-restricted role, `agent_rate_limit`, `agent_cosign_required`, and `user_authentication_required` (§4.9.3). Each one is a context parameter or a role that the context's governance model sets at creation and changes afterward through the same propose/approve path it uses for any other parameter — ADR-039 introduces no governance primitive for them. A change to any of the four `ContextParams` fields is a metadata change and is subject to the `metadata:edit` capability (§5.3.1); a change to an agent-restricted role assignment is subject to `role:assign`. Because all four parameters are structural metadata (§5.7), every member sees the current values, and a member who joined under the old values sees the new ones the moment governance executes the change. A governance proposal that adds a capability to `user_authentication_required` reaches further than a change to the other four, in two ways a proposer needs to see before voting. It takes that capability from every member whose platform publishes no key-custody attestation, which on macOS, Linux, and Windows is every member. And because `metadata:edit` is itself a capability (§5.3.1), a proposal that lists it makes the capability that changes the parameter subject to the parameter, after which no member can propose the change back; listing `governance:vote` locks the context the same way through the tally. §4.9.3 states no guard against either lock, and the Category C entry in `.docs/specs/00-open-questions.md` carries the guard to a human alongside the prior question of whether the protocol permits the parameter at all. No shipped governance proposal type carries any of the five changes; stories SCP-AB-028 through SCP-AB-033, SCP-AB-036, and SCP-AB-037 of `.docs/prds/agent-binding.json` deliver them, and §4.9.3 states what a reader may not assume until they land.
