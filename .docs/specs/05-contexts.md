...
**Governance-level subscriber ban.** When the context's capability ceiling includes `member:ban` (§5.3), governance can execute `RevokeAccess { did, access: Read }` (§5.9, ADR-031) against broadcast subscribers. Unlike per-author blocking (which is unilateral and affects only one author's content), a governance ban removes the subscriber from the registry AND adds them to ALL authors' block lists simultaneously. All authors MUST rotate keys after a governance ban: the per-author epoch advance is a mandatory state transition that is durable before any observer sees the ban (Durability below), and each rotated author's `KeyEpochAdvance` event-log leaf follows the durable `ReadAccessRevoked` leaf. A `KeyEpochAdvance` leaf append that fails MUST NOT roll back the ban or the epoch advance, because both are already durable when the leaf is appended; *Event-log leaf append failure* below states the log state that the failure leaves behind and what the member does about it. This mirrors `RevokeAccess` semantics in encrypted contexts (MLS group removal), adapted for broadcast's per-author key model.
...
**`RotateContentKeys` broadcast key rotation.** When the `RotateContentKeys` governance action executes on a broadcast context, all authors MUST advance their broadcast key epoch — a mandatory epoch-state advance per author, durable before any observer sees the rotation (identical to the governance-ban rotation above). This is the governance-triggered key hygiene path: it does not add to block lists or modify the subscriber registry, but advances every author's epoch to forward-fence observers who hold cached broadcast keys. One `KeyEpochAdvance` leaf per rotated author, in ascending author-DID order, follows the durable `ContentKeysRotated` leaf in the event log. A `KeyEpochAdvance` leaf append that fails MUST NOT roll back the epoch advance, because the advance is already durable when the leaf is appended; *Event-log leaf append failure* below states the log state that the failure leaves behind and what the member does about it.
...
**Event-log leaf append failure.** This section places event-log leaves after a durable state transition: the `MemberBlocked` leaf after a per-author block, the `ReadAccessRevoked` leaf after a governance ban, the `ContentKeysRotated` leaf after a `RotateContentKeys` rotation, and the per-author `KeyEpochAdvance` leaves that follow the ban's and the rotation's anchor leaf. The rule in this paragraph binds every leaf that this section places after a durable state transition. The event log is append-only, and every honest member derives the same leaf sequence from the commit-ordered stream (§9.9.3, *Convergent-log requirement*), so a member cannot insert a leaf at an interior position once a later leaf has landed. When one of those appends fails, the member's local log lacks that leaf at that position, and every leaf the member appends afterwards sits one position earlier than the same leaf sits in the log of every member whose append succeeded. The member MUST keep the durable state transition (the epoch advance, the block-list entry, the registry removal): the transition is authorization-downward, and *Durability* above forbids rolling it back. The member MUST surface the failure with the context id, the leaf's actor DID, and the storage error. The member MUST NOT append the leaf at a later position: the member's leaf order still differs from every other honest member's at every count from the missing position onward, so a later-position append restores agreement at no count, and it adds a leaf that no other member appended at that position. The member MUST count only the leaves that landed toward the §9.9.3 checkpoint interval, so that the count it reports matches the log it holds. The protocol surfaces the divergence through §9.9.3 checkpoint comparison: when this member and a peer report the same `eventCount`, their `merkleRoot` values differ, §9.9.3 names log corruption beside equivocation as a cause of that signal, and its *Divergence resolution* step exchanges event-log proofs, which locate the missing position. No clause in this specification repairs the local log after the failure, so the divergence persists until the context's governance model acts on the §9.9.3 signal.
