...
- **Derivation.** `derived_context_id = SHA-256("standing:" ‖ len32(id_lo)‖id_lo ‖ len32(id_hi)‖id_hi)` over the two participants' identifiers, ordered by unsigned byte comparison over their 32-byte values (`len32` = 4-byte big-endian length prefix, §9.5.1). Both parties compute the identical id; no party "allocates" it.
...
```
derived_context_id   = SHA-256( "standing:" || len32(id_lo) || id_lo || len32(id_hi) || id_hi )
standing_context_id  = "standing-" || hex( derived_context_id )
```
where `len32(x)` is the 4-byte big-endian length prefix of `x` (the §9.5.1 variable-length-bytes encoding). `id_lo` and `id_hi` are the two participants' identifiers, ordered by unsigned byte comparison over their 32-byte values (`09-security-model.md` §9.7.4.2 R13). `hex(...)` is lowercase hex of the 32-byte digest; the raw 32-byte digest is `derived_context_id: [u8;32]`. Both parties order one pair one way, so `derive(A,B) == derive(B,A)` and no party "allocates" the id. **The preimage is not yet computable**: it takes each identifier in the textual form `09-security-model.md` §9.7.4.2 R13 defers, and `09-security-model.md` §9.5.2 enumerates this structure among those that wait on that form.

**Injectivity invariant (load-bearing, unconditional by construction).** Each `len32` prefix fixes its own field boundary, so the preimage re-parses uniquely back into the ordered pair `(id_lo, id_hi)` and no attacker can shift a boundary to forge a colliding pair. Injectivity holds for whatever textual form the deferred decision fixes, because the prefixes and not the field contents carry the boundaries.
