...
| `Execution` | `6130..6139` | Handler panic, timeout, non-determinism, credit/stream exhaustion, cancel-ack timeout, operator signature refusal. |
...
**The compact code registry.** Only eighteen codes are allocated across the `6100..6199` sub-block — the design target is "compact," roughly one to two codes per class. Every allocated code has a class, a canonical **default slug**, and a default retry policy:
...
| `SCP-OUTLET-6137` | `Execution` | `execution.signing-refused` | Backoff 1s..30s |
...
**Retry guidance.** Every envelope carries a `RetryPolicy` (`Never`, `Immediate`, `After{delay}`, `WithBackoff{min,max}`) so a caller can decide to retry without re-classifying the error. Deterministic rejections (Protocol, Authorization, Input, Output, Economic, Governance, and broken-handler Execution faults) default to `Never` — retrying the same request reproduces the error. Idempotent credit exhaustion (`SCP-OUTLET-6131`, shared with the receiver-side `execution.stream-gap` cancel-and-rerun mitigation) defaults to `Immediate` (the framework refreshes credits / the caller reruns). Back-pressure conditions — the node-level concurrent-pump ceiling (`SCP-OUTLET-6132`), credit-stall (`SCP-OUTLET-6133`), and transport faults (`SCP-OUTLET-6160`) — default to `WithBackoff` over `1s..30s`. The node-level ceiling rejection carries its own code (not `SCP-OUTLET-6131`) precisely so the retry policy — keyed on the code — is `WithBackoff` rather than the credit band's `Immediate`: retrying immediately against a saturated node just re-fails. The signature refusal (`SCP-OUTLET-6137`, §5.4.5) carries its own code for the same reason. The terminal reaches a caller only when the operator key signed it, so the key that refused the earlier chunk answered the very next call — which rules out the custody conditions that describe the key itself (absent, wrong type, unsupported) and leaves the transient backend fault, a condition the caller may find cleared on a later attempt. An implementation that admits payloads which can fail canonicalization (§5.4.5) reaches this terminal for a deterministic cause as well, and a caller retrying that one re-fails; the backoff band bounds the cost of finding out. Riding `SCP-OUTLET-6130` would key the refusal to that code's `Never`, telling every caller not to retry a condition the emitter demonstrated is not permanent, and would additionally resolve through `error_code_to_default_slug` to `execution.handler-panic` — attributing the operator's own signing failure to a bug in the executor's handler. Receivers MAY override the default from contextual signals (e.g. a `transport.rate-limited` envelope carrying a `retry_after_secs` hint).
...
**Signature refusal (normative).** An operator's signing seam can refuse a chunk for either of two reasons: the custody backend that holds the operator key returns an error, or the payload fails RFC 8785 canonicalization while composing the preimage. Whether the second is reachable depends on the payload types an implementation admits — a `ChunkPayload` whose leaves are strings, integers, booleans, and finite numbers canonicalizes unconditionally, so an implementation that admits only those never reaches it, and the custody error is the only cause it must plan for. An implementation MUST NOT emit a chunk whose `sig` field it did not obtain from the operator key, and MUST NOT substitute a zero, random, or otherwise synthesized value — a chunk bearing a signature no operator produced is an attestation the operator can neither be held to nor repudiate, and the equivocation binding above rests on every emitted `sig` being real.

Refusing to emit is necessary but not sufficient. A pump that answers a refusal by closing its channel leaves a member unable to distinguish a refused chunk from a completed stream, and the refusal is precisely the case where an operator suppressed output.

**The criterion.** Whenever the operator key will produce a signature over the refusal terminal defined below, the implementation MUST emit that terminal to the member and MUST NOT abandon the delivery while the member is still consuming. When the key will not, the implementation MUST end the stream in a way the member reads as abnormal rather than as completion. An implementation that meets both halves conforms, however it is structured internally.

The first half obligates emission and non-abandonment rather than arrival, because arrival is not the emitter's to guarantee — a member that stops consuming, or a transport that severs, defeats any emitter. What it forecloses is the emitter choosing to give up: an implementation MUST NOT discard a signed refusal terminal on a deadline of its own and then close the channel, because a member that is merely slow would then read the close as a completed stream, which is the ambiguity this clause exists to remove.

**The refusal terminal.** It is a terminal `Error` chunk carrying the `execution.signing-refused` slug under code `SCP-OUTLET-6137`, signed under the operator key at the sequence the refused chunk would have used. The refused chunk MUST NOT consume a sequence number, so the terminal takes its slot and the member reads a contiguous sequence. Its payload MUST carry only the code, the slug-prefixed message, and `terminal: true` — never text derived from the refused payload. The cross-context bridge below forwards this terminal verbatim to an invoker who is not a member of the operating context, so payload-derived text in it would move the refused chunk's content across a context boundary that no governed path authorized.

**Satisfying the criterion (guidance, not the test).** Attempting the terminal and reading the key's answer satisfies the first half by construction, and is what the reference implementation does. Predicting the answer from the earlier refusal does not: a custody backend that refuses one call can serve the next, so a prediction that the key is unavailable can cost a member a terminal the key would have signed. An implementation that predicts correctly in every case conforms — the criterion is what the member receives, not how the implementation chose.

The second half is an obligation on the delivery boundary the implementation owns, not on the wire, so no peer observes how it is met. `.docs/standards/sdk-common.md` fixes the shape for this project's SDKs. Whatever an implementation surfaces there is a local control value and not a protocol artifact: it MUST NOT be assigned a `sequence`, MUST NOT enter the `stream_manifest_hash`, and MUST NOT be forwarded across a context boundary.

**Bridge-authored terminals.** The cross-context bridge synthesizes a terminal of its own when the operating context's stream ends with none, or when its own checks reject a chunk, so the receiving context never truncates after a non-terminal `Data`. That chunk carries no operator signature, which is the one sanctioned exception to the MUST NOT above, and its `sig` field is all-zero so a member reads it as bridge-authored rather than operator-attested.

Because no key stands behind it, what it may claim is bounded: **a bridge-authored terminal MUST state only a fact the bridge itself established, and MUST NOT relay a report the bridge received and cannot verify.** A member holds no signature to check it against, so a claim the bridge did not establish is one nothing in the protocol can confirm or refute.

The bridge establishes a fact when it performs the check itself. Verifying a forwarded chunk's operator signature against the pinned descriptor and finding it invalid is such a fact, and so is validating a chunk against the governed output schema and finding it non-conforming; each is the bridge reporting the outcome of its own computation, and each may carry the class its own check belongs to. So is observing that the operating context's stream ended without a terminal chunk, which is a Transport-class fact about the crossing.

A signature refusal the operating context *reports* is not such a fact. The bridge ran no check; it was told. Relaying it would write an unattested accusation about the operating context's key into the receiving context's manifest, so a bridge that receives one records the ordinary end-without-a-terminal fact it did establish, and the report reaches the operator's own diagnostics instead.

A member that reads the refusal terminal learns that the operator produced output it did not sign and therefore did not deliver. It learns neither how many chunks were withheld nor what they contained: the `stream_manifest_hash` covers the chunks the operator emitted, so it proves the operator did not over-bill and proves nothing about completeness. §5.4.4's `execution.signing-refused` slug is the sole protocol record that the withholding happened.
