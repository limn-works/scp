...
**Roles.** A = the identity whose DID sorts lower (`did_lo`) is, by convention, the **initiator** (creator); B = the higher (`did_hi`) is the **peer** who joins via Welcome. This is purely a deterministic tie-break for the get-or-create path so concurrent initiations converge on a single creator — it is not a two-phase ordering. Either party may *initiate* the get-or-create call; the sort-order convention only fixes which side authors the group when both attempt creation at once, so the symmetric `derived_context_id` resolves to one creator.
...
2. **Add the peer.** A **fetches B's published MLS `KeyPackage`** and `add_member`s B, producing an MLS **Welcome**. B's KeyPackage single-use is enforced at B's *join* by the existing fused-join two-anchor mechanism (§2F / ADR-049 §9) — there is no separate Prepare-time reservation.
...
4. **Peer (B) receives the Welcome asynchronously and applies the consent gate on receipt.** When B's node receives the Welcome it applies the **inbound-contact consent gate** *before joining*: (a) **block list** — B refuses to join if A is globally blocked (§3.6 / §3.7 `is_globally_blocked`); (b) **opt-in policy** — if B's contact policy requires opt-in for standing pairs (the `AutoAcceptPolicy` / its `TrustRequirement`, §5.12.6), B joins only if A satisfies it. The consent gate is applied by the **joining peer on Welcome receipt** — it is NOT a synchronous Prepare-B reply to the initiator. If the gate accepts, B processes the Welcome to join (its KeyPackage single-use enforced at join by the fused-join two-anchor mechanism), registers the context **Active**, appends to its event log, and records the peer via `register_standing_context`. If the gate rejects, B simply **never joins** — there is no synchronous "Rejected" reply.

**Why consent-on-receipt is better for block-privacy.** Because the consent gate is applied by the *joining* peer asynchronously on Welcome receipt — not as a synchronous Prepare-B reply to the initiator — a blocked or unapproved initiator receives **no synchronous rejection** and therefore **no existence/block oracle**: the peer simply never joins, indistinguishable (to the initiator) from an offline or slow peer. A synchronous two-phase `Rejected` reply would have leaked a 1-bit "you are blocked / not approved" signal back to the initiator; async consent forecloses that.
