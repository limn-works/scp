...
| `suspended_capabilities[did]` | Open — no maximum is set | The collection is a set, so it holds at most one entry per capability per member, and nothing bounds how many distinct capabilities one member's entry holds: no write path tests a suspension entry against the ceiling, and §5.3.1 wildcard coverage puts an unbounded family of `outlet:query:{outlet_id}` and `outlet:call:{outlet_id}` capabilities inside a ceiling that declares `outlet:query:*` or `outlet:call:*`. `.docs/specs/00-open-questions.md` carries the maximum as an open question |
| `read_exclusion_list` | No artificial cap | Naturally bounded by membership count — cannot exclude non-members from CEK wrapping |
| `agent_cosign_required` | No artificial cap | Bounded by the number of entries the ceiling declares — §4.9.3 makes every write path reject an entry the ceiling does not declare and an entry the list already names, and it tests the ceiling's declared entries rather than the §5.3.1 and §5.3.1.1 wildcard coverage its match rule applies, so the list holds at most one entry per capability the ceiling declares |
...
`suspended_capabilities[did]` is the one row that carries no maximum, so the **Collection size limits** paragraph that introduces this table states a duty the table does not yet discharge for that collection. A plain numeric cap rejects a suspension once the list is full, which leaves granted a capability the context's governance voted to deny, so a cap needs a companion rule stating what a full list does. Narrowing which capabilities an entry may name bounds the list without that hazard, and each candidate narrowing costs a different piece of the mechanism: admitting only the ceiling's declared entries forecloses suspending one outlet in a context whose ceiling declares only `outlet:call:*`, because `ContextRoleState::member_has_capability` matches a suspension entry against the invoked capability by equality; admitting only registered outlets rejects a suspension entered before its outlet is registered. `.docs/specs/00-open-questions.md` carries the choice to a human.
