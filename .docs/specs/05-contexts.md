...
**The criterion.** Whenever the operator key will produce a signature over the refusal terminal defined below, the member MUST receive that terminal. When the key will not, the stream MUST still end in a way the member reads as abnormal rather than as completion. An implementation that meets both halves conforms, however it is structured internally.
...
**Bridge-authored terminals.** The cross-context bridge synthesizes a terminal of its own when the operating context's stream ends with none, so the receiving context never truncates after a non-terminal `Data`. That chunk carries no operator signature, which is the one sanctioned exception to the MUST NOT above, and its `sig` field is all-zero so a member reads it as bridge-authored rather than operator-attested. Because no key stands behind it, it MUST confine itself to the bridge's own observation and carry a Transport-class code. It MUST NOT assert a signature refusal or any other claim about a participant's key: the member could not verify such a claim, and recording it would put an unattested accusation against the operating context into the receiving context's manifest.
