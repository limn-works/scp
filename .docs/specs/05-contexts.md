...
2. **A custom capability** of the form `{resource}:{action}` (§7.2) — the entry contains **exactly one** colon, and both `{resource}` and `{action}` are non-empty **kebab-case tokens** (per the §7.2 custom-naming rule: lowercase ASCII alphanumerics and hyphens, `[a-z0-9-]+`). Neither token may contain a colon (`:`), an asterisk (`*`), whitespace, or any other character outside the kebab-case charset.
...
Ceiling-entry strings are subject to the same string sanitization as other context string fields (§9.18.6): implementations MUST reject any entry containing control characters (U+0000–U+001F, U+007F–U+009F), HTML-special characters (`<`, `>`, `&`, `"`, `'`), or whitespace, and MUST reject any entry exceeding the §9.18.6 string length cap (256 bytes). Whitespace is never permitted inside the `{resource}` or `{action}` tokens — this already follows from the kebab-case charset above. Any other unrecognized or ill-formed string (empty resource, empty action, a token outside the kebab-case charset, a stray `*`, etc.) is likewise rejected with `InvalidCeilingCategory`.
