...
**Consent gate runs FIRST in the collision atomic sequence (normative).** Because `did_hi`'s destroy-and-rejoin IS a Welcome-join, the step-4(b) consent gate is **not** waived by the simultaneous-create race — it runs as the **FIRST step** of the atomic `{consent-gate (block-list mandatory) + confirm-creator + fresh-join + destroy}` sequence above, **before** any `confirm-creator`, fused-join, or destroy. The **block-list check is MANDATORY and non-waivable**: a `did_lo` that `did_hi` has globally blocked (§3.7.1 `is_globally_blocked`) MUST NOT trigger any destroy or join, regardless of the simultaneous-create race. The block-list read is **point-in-time at the gate-read instant**: a block already propagated and visible to `did_hi`'s node when the gate reads it aborts the bundle outright (no destroy, no join). A block that has **not yet propagated** to `did_hi`'s node at that instant (`did_hi` blocked `did_lo` after auto-initiating its own create, or a propagation race per §3.7.1's best-effort propagation) is not observed by the gate and the bundle MAY proceed — but this **self-heals** and is not a bypass: the only group the bundle can converge to is the **same `did_hi`-self-initiated pair** under the identical `derived_context_id` (`did_lo` controls no other destination), and §3.7.1 post-join block propagation then enumerates that now-joined standing context and **severs `did_lo`** (sender-key rotation, cached-content destruction, access-key deletion), reaching the same end state as a block that had landed slightly earlier. So `did_lo` can **never** force a *durable* join to a blocked peer: an observed block aborts the bundle, and an unobserved-then-observed block is severed post-join. The **stranger default-deny is satisfied implicitly** on this path: a `did_hi` to which the collision rule applies **itself initiated a create** for this exact `(did_lo, did_hi)` pair, and its own create is an explicit out-of-band decision to form the pair, so `did_lo` is not a stranger to `did_hi` here. (A `did_hi` that did **not** itself initiate holds no self-created group, so the collision rule does not apply to it at all — the ordinary step-4(b) gate applies on Welcome receipt and there is nothing to destroy.) The **block-list arm is the only consent-reject cause on this path**: the *Anti-spam rate limit* below is a per-initiator-DID **stranger-path approval-prompt** limiter, and a canonical `did_lo` Welcome on the collision path is **not** from a stranger (the consent gate's stranger arm is satisfied implicitly, above) — so the per-DID cooldown MUST NOT throttle collision-resolution processing of a creator-bound canonical `did_lo` Welcome from the already-paired peer, and convergence is therefore never gated on the inbound cooldown. On consent **reject** — i.e. a block-list hit — `did_hi` performs **no** fused-join and **no** destroy: it keeps its own self-created group (if any) and never joins `did_lo`'s. This is observably identical to any other consent-rejected Welcome (no join, no synchronous reply, no destroy); `did_hi` additionally retains its own self-created group, which then follows the never-joined steady-state and reaper rules below. Only on consent **accept** does the atomic `{confirm-creator + fresh-join + destroy}` remainder proceed, with the destroy still sequenced **strictly after** the successful fresh-join.
...
3. **Publish / register (A).** A publishes the group, emits the **Welcome to B's personal routing id over the relay** asynchronously (A does not block on B) — B's personal routing id is the reserved invitations routing id of §5.12.3.3 (step 1), so this emit follows the §5.12.3.3 invitation-delivery path and the Welcome carries the §5.12.3.3 InvitationBundle relay-retention TTL (7 days, default); this is the `welcome_ttl` the reaper (item (d) below) reads — registers the context **Active**, appends the creation to its event log, and records the peer in its contact graph via `register_standing_context`.
