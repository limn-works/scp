...
**Roles (single-creator rule, normative).** A = the identity whose DID sorts lower (`did_lo`) is the **creator**; B = the higher (`did_hi`) is the **peer** who joins via Welcome. This is a deterministic tie-break for the get-or-create path so concurrent initiations converge on a **single creator** — it is not a two-phase ordering. Either party may *initiate* the `standing_context(peer)` get-or-create call, but the create role is normatively pinned: the higher-DID node (`did_hi`) MUST NOT create the 1-leaf group itself — its get-or-create issues as a request to **fetch / await `did_lo`'s Welcome**, and **only `did_lo` creates the group and `add_member`s the peer**. The per-node `create_mls_group` `Entry::Vacant` guard is per-node and cannot coordinate two distinct nodes, so without this rule a genuinely-concurrent both-initiate would produce two distinct MLS groups under the same `derived_context_id` that mutually reject each other's Welcome.

**Concurrent-creation collision resolution (normative).** If both nodes nonetheless created a group under the same `derived_context_id` (e.g. a `did_hi` node that violated the rule, or a partition that let both create before either's Welcome arrived), the conflict resolves **deterministically in favor of `did_lo`'s group**: on receiving a Welcome for a `derived_context_id` under which it already holds a **self-created, un-joined, peerless** group, the higher-DID node (`did_hi`) MUST **destroy its own group** (local-only state no peer observed — purely local key/state destruction) and process the incoming Welcome from `did_lo`; the lower-DID node retains its own group. Exactly **one** group survives per `derived_context_id`. (`did_hi`'s discarded group was never the surviving replica, so destroying it equivocates against no peer.)
...
- **(a) Decline is indistinguishable from offline.** A MUST NOT distinguish a declined consent gate from an offline or slow peer — there is no synchronous `Rejected` reply and no peer-state probe, so A observes only "B has not joined yet," never "B refused." Distinguishing them would reconstruct the existence/block oracle the async consent gate exists to foreclose.
...
- **(d) Reaper for orphaned single-member replicas.** A MAY garbage-collect a single-member standing replica that has seen **no peer join** after an implementation-defined idle bound (the `bilateral-persistent` TTL / operator-driven `close_context`, §5.12.6). Because such a replica is local-only state never observed by any peer (A never published anything B's absence makes meaningful, and no Commit from B ever advanced it), reaping it is purely local cleanup — it equivocates against no peer and breaks no consistency obligation.
...
Cross-refs: §5.12.6 (standing contexts / contact graph), §5.12.1 (`bilateral-persistent` template), §3.7.1 (block list), §17 (event-log consistency / checkpoints), ADR-049 §3 / §3a (standing-pair creation is single-context async creation, not a saga). Standing-pair creation journals nothing and is not a saga; the genuine cross-context sagas are §6.2.4 (cross-context tool invocation) and §5.14.13 (broadcast-hosting handshake).
