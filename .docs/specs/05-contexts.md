...
- Agent policy — `agent_keys_allowed`, `agent_rate_limit`, `agent_cosign_required`, and `user_authentication_required` (§4.9.3). A prospective member decides whether to join partly on whether the context admits their agent at all, on which of their agent's actions need a human signature, and on which capabilities the context forecloses to a member whose platform attests no user-authentication policy, so the protocol makes these four parameters visible before joining rather than after. `user_authentication_required` carries the whole of the legibility guarantee for a member on macOS, Linux, or Windows: §9.3 of `.docs/specs/09-security-model.md` records that those three platforms have "no App Attest or Play Integrity equivalent", so a member on one of them reads the capability list and learns before joining which capabilities they will never exercise in this context. `ContextParams` declares none of the four today and no metadata projection carries them; story SCP-AB-032 of `.docs/prds/agent-binding.json` adds the fields and this projection together, and §4.9.3 states what a reader may not assume until it lands.
...
**Category C — governance sets the context's agent restrictions.** A context restricts what an agent may do inside it through `agent_keys_allowed`, an agent-restricted role, `agent_rate_limit`, `agent_cosign_required`, and `user_authentication_required` (§4.9.3). Each one is a context parameter or a role that the context's governance model sets at creation and changes afterward through the same propose/approve path it uses for any other parameter — ADR-039 introduces no governance primitive for them. A change to any of the four `ContextParams` fields is a metadata change and is subject to the `metadata:edit` capability (§5.3.1); a change to an agent-restricted role assignment is subject to `role:assign`. Because all four parameters are structural metadata (§5.7), every member sees the current values, and a member who joined under the old values sees the new ones the moment governance executes the change. A governance proposal that adds a capability to `user_authentication_required` takes that capability from every member whose platform attests no user-authentication policy, so the proposal reaches further than the other four: §4.9.3 states what a verifier then does, and open question 1 of §4.9.3 carries the prior question of whether the protocol permits the change at all. No shipped governance proposal type carries any of the five changes; stories SCP-AB-028 through SCP-AB-033 and SCP-AB-037 and SCP-AB-038 of `.docs/prds/agent-binding.json` deliver them, and §4.9.3 states what a reader may not assume until they land.
