...
| `derived_context_id` | `[u8;32]` | Recomputed by A; a mismatch ⇒ `Rejected`. |
| `group_id` | `Vec<u8>` | MLS group id — **deterministically derived from the pair**, NOT allocated by either party: `group_id = SHA-256("scp-standing-group-v1:" ‖ len32(did_lo) ‖ did_lo ‖ len32(did_hi) ‖ did_hi)` (32 bytes), where `len32` is the 4-byte big-endian length prefix per §9.5.1's variable-length-bytes rule. This removes A's free choice (a malicious lower-DID A could otherwise pick a `group_id` colliding with an existing group in B's MLS storage). Both sides recompute and MUST agree. This derivation is new (the as-built code allocates the group id randomly), so it adopts the §9.5.1 length-prefix discipline — injective from the start. |
...
A validates in order: (1) it is not already Active under `derived_context_id` — if so it returns `Rejected { reason: AlreadyExists }` (the idempotent-success mapping is **scoped to verified-self-membership** under the deterministic `derived_context_id`, see *Replay*; a bare existence is not auto-success); (2) the `bilateral-persistent` template params are well-formed (§5.12.1); (3) `peer_did` resolves and is not blocked; (4) the derived `group_id` does **not** collide with an existing MLS group in A's storage (collision ⇒ `Rejected`), symmetric to Prepare-B's collision check below. On success A creates the MLS group locally (2-leaf init) plus a fresh sender key, recording handles into the staged `CreationReceipt`, but does **not** publish (deferred to Commit). It replies `Prepared`; the supervisor journals `PreparingB` before contacting B.

**Prepare-B (consent-gated, reservation-only).** The supervisor sends B `derived_context_id`, `group_id`, A's DID, and A's published MLS `KeyPackage` reference. B validates **in order, cheapest-and-consent-first**: (1) **inbound-contact gate** symmetric to A's block check — B rejects (→ `Aborting`) if A is on B's block list, or if B's contact policy requires opt-in for standing pairs and A is not approved; (2) `derived_context_id` recomputes from `(B_local_did, A_did)` and `group_id` recomputes from the pair; (3) B holds no Active context under it and `group_id` does **not** collide with any existing MLS group in B's storage (collision ⇒ `Rejected`); (4) A's KeyPackage resolves and is unconsumed. Only after (1)–(4) pass does B do MLS work: it **reserves** A's KeyPackage (it does NOT consume it — consumption is deferred to Commit so an abandoned Prepare cannot drain A's pool) and stages the join. B stages symmetric evidence — its `StandingPairCreatePrepared` carries `local_did=did_hi`, `peer_did=did_lo`, `derived_context_id`, and `group_id`, but does **not** carry an independent `CreationReceipt`: the `CreationReceipt` is A-authored only (its booleans are inherently A-local creation state, and B creates no group/sender-key as the Welcome-joiner), so there is no A/B receipt symmetry to reconcile. B replies `Prepared`. Any check failure ⇒ `Rejected` ⇒ `PreparingB → Aborting`. **Rationale:** the consent gate plus reservation-only discipline forecloses unsolicited resource consumption and KeyPackage-pool drain on arbitrary victims — Prepare-B is cheap and side-effect-free until Commit.
...
CreationReceipt (JCS object):
  { "context_id": <canonical standing context id, "standing-" + 64-hex per the §5.15.8 derivation>, "group_id": <hex>, "mode": "encrypted",
