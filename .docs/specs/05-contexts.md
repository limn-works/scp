...
**Anti-spam rate limit (normative).** B MUST rate-limit *inbound* standing-pair Welcomes per initiator DID as ordinary anti-spam on the consent gate — a per-peer cooldown between successive inbound-contact attempts from the same DID, so a single peer cannot flood B with repeated standing-pair Welcomes. **Absent a context-specific override it defaults to 60 seconds, and a context MUST NOT configure it below a 1-second hard floor** — a degenerate near-zero cooldown is non-conformant. This is an ordinary per-peer inbound-contact rate limit on the consent gate, **not** saga concurrency machinery. The residual is framed honestly per §9.3: the goal is to make spam **expensive to sustain, not impossible to attempt** — a fresh admissible DID may attempt one inbound contact, but a single DID cannot sustain unbounded Welcome churn. The per-DID cooldown is per-initiator-DID and so does **not** bound a **fresh-DID fleet** (many distinct DIDs, one Welcome each): that fleet is bounded instead by §9.3 earned-capacity applied at the **consent-gate-evaluation** layer (not only the join layer), so an attacker pays §9.3's "expensive to sustain" cost of minting admissible DIDs before any of their inbound contacts are even evaluated. This spec adds only the forward cross-reference; it does **not** author a new standalone limiter here (that bound lives in §9.3 and its enforcement layer). There is **no** standing-pair-specific KeyPackage reservation (the removed reserve-not-consume machinery): KeyPackage single-use is enforced at *join* by the fused-join two-anchor mechanism (step 2, ADR-049 §9), and a stranger-add draining A's published KeyPackage pool is the **general MLS KeyPackage-pool concern**, bounded by KeyPackage republication and the §9.3 admission cost — not by any standing-pair-local reservation.
...
Cross-refs: §5.12.6 (standing contexts / contact graph), §5.12.1 (`bilateral-persistent` template), §3.7.1 (block list), §9.3 (Sybil resistance / earned capacity), §17 (event-log consistency / checkpoints), §6.2.4 (cross-context tool invocation saga), §5.14.13 (broadcast-hosting handshake saga), ADR-049 §3 / §3a (standing-pair creation is single-context async creation, not a saga).
