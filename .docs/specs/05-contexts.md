...
**No privileged-built-in collision.** A custom entry (shape 2 or shape 3 above) is valid only if its canonical UCAN projection — the output of `Capability::ucan_capability_name` (the authoritative kebab-resource → UCAN-form projection, defined in code at `crates/scp-protocol/src/context/roles.rs`; §7.3.4.1 gives the kebab capability-URI naming convention, not this projection) — is **not** a member of the built-in UCAN-form set: the set of UCAN forms produced by the built-in categories of §5.3.1 (e.g. `tool_invoke:*`, `tool_invoke:{tool_id}`, `bridging:*`, `context_child:create`, and every other built-in's UCAN spelling — with the parameterized `tool_invoke:{tool_id}` family treated as a covered member for any concrete `tool_id`). Equivalently, the admissible custom UCAN projections are *closed by construction* as "all well-formed projections **minus** the built-in UCAN-form set." A custom entry whose projection lands inside that built-in set MUST be rejected at context creation with `InvalidCeilingCategory`. This is a **positive membership test against the authoritative built-in set** — the validator computes the entry's canonical projection and asserts non-membership — **not** a denylist of forbidden spellings: it is defined by *exclusion from* the built-in projection set, so it neither grows nor needs amending as new spellings are imagined. The kebab-only resource charset of shapes 2 and 3 (`[a-z0-9-]+`, no `_`) forecloses only the **underscore-bearing** built-in UCAN forms — `tool_invoke:*`, `tool_invoke:{tool_id}`, and `context_child:create`, whose resource tokens (`tool_invoke`, `context_child`) a charset-valid custom resource cannot spell. But the majority of built-in UCAN forms carry **no underscore** and are therefore fully valid kebab strings — `messages:read`, `messages:write`, `tool:register` (`tool_register`), `member:invite`, `member:remove`, `member:ban`, `role:assign`, `governance:propose`, `governance:vote`, `context:close`, `metadata:edit`, `media:voice`, `media:video`, `media:screen_share`, `bridging:*`, `tool:interface`. For every one of these, nothing in the charset stops a custom entry from being declared as that exact string; the explicit membership test is the **sole** mechanism preventing it and is therefore **required** — it MUST NOT be removed or treated as "redundant with the charset." The clause is stated here as the authoritative, normative invariant so the validator can cite §5.3.1.1 and a custom capability can never masquerade as a privileged built-in.
...
There is currently **no** secret-bearing saga; §9.4.3 stands as the contract any future secret-bearing saga MUST satisfy. All three sagas — standing-pair creation (§5.15.8), cross-context tool invocation (§6.2.4), and broadcast hosting handshake (§5.14.13) — are public-metadata-only: their journals and envelopes carry no bearer material (the tool invocation carries a UCAN *index*, not the token; the broadcast key is delivered out-of-band via HPKE after Commit; the standing pair journals only public plan-metadata). Each marks resolution with `secret_bearing=false`, so no synchronous on-disk evidence overwrite is required.
...
### 5.15.8 Standing-Pair Creation Saga

A **standing pair** is the `bilateral-persistent` context two identities create on first contact (§5.12.6). When neither side yet holds the pair, creation crosses two context-owning computations and executes as a cross-context saga under §5.15.4. This section fixes the wire-level protocol §5.15.4 leaves abstract for this use case.
...
```
`did_lo` / `did_hi` are the two participant DID strings — each in its **canonical DID string form as produced by DID resolution**, which by construction yields a single comparison form per DID method: a `did:dht` id is the lowercase z-base-32 encoding of the Ed25519 public key (§9.6.1 — canonical by construction, no case or padding variation), and a `did:web` id MUST be normalized per the W3C did:web method (host lowercased, path segments percent-encoded so a literal `:` is `%3A`). DIDs used as sort/derivation keys MUST be in this canonical form, so both parties feed byte-identical DID bytes into the preimage and a DID has exactly one comparison form. Sorted lexicographically (bytewise UTF-8); `hex(...)` is the lowercase hex of the 32-byte digest. Taking the canonical form *before* the lexicographic sort guarantees both parties feed byte-identical `did_lo` / `did_hi` into the derivation and therefore derive the identical `derived_context_id`. The 32-byte `derived_context_id: [u8;32]` used in saga evidence is the raw digest before prefix and hex. The derivation is symmetric: `derive(A,B) == derive(B,A)`. No participant "allocates" the id — both compute it.

**Injectivity invariant (load-bearing).** The `"standing:" ‖ did_lo ‖ ":" ‖ did_hi` colon-join is *not* an injective encoding in the abstract — a DID method permitting an attacker-placeable raw `:` at the join position could in principle produce two distinct DID pairs with the same preimage. This construction is safe **because of a property of the realizable DID grammars**, which MUST hold for any DID method SCP admits: `did:dht` identifiers are z-base-32 (no `:` in the method-specific id) and `did:web` identifiers are percent-encoded (a literal `:` in a host/path segment is `%3A`), so neither method permits an attacker-placeable raw `:` at the relevant positions and the join is unambiguous in practice. The security-critical identifier is the **`derived_context_id`**: the crypto provider indexes MLS group state by `derived_context_id` (a create-time `Entry::Vacant` collision guard rejects a second group under the same id), so MLS group isolation keys off `derived_context_id` — making the colon-join's DID-grammar safety the load-bearing property that protects that isolation. Precisely, the provider's `create_mls_group` `Entry::Vacant` guard keys on the canonical context-id digest `SHA-256("standing-" ‖ hex(derived_context_id))`, not the raw `derived_context_id` directly. Because that digest is a collision-resistant 1:1 function of `derived_context_id`, the guard fires across exactly the distinct `derived_context_id` values and the isolation chain holds end-to-end. A future DID method that admitted a raw `:` in its identifier would trip *this documented assumption* (a fail-loud spec violation to be caught at method-admission review) rather than silently producing a `derived_context_id` collision. Length-prefix framing of the `derived_context_id` derivation (§9.5.1, `len32(did_lo) ‖ did_lo ‖ len32(did_hi) ‖ did_hi`) is intentionally **not** used: the DID-grammar property already forecloses the only realizable ambiguity, so unconditional injectivity would add no security — deliberately omitted, not deferred.

**Participants.** A = the identity whose DID sorts lower (`did_lo`); B = the higher (`did_hi`). The §5.15.4 `PreparingA → PreparingB` order follows this fixed A-before-B ordering (not initiation order), so concurrent initiations converge and the §5.15.4 saga-busy serialization resolves the race deterministically.

**Prepare-A.** The supervisor sends A the staged-creation Prepare; A stages `SagaPreparedState::StandingPairCreate` into `saga_pending` (all fields PUBLIC, not secret-bearing):

| Field | Type | Meaning / validation |
|---|---|---|
| `peer_did` | `DID` | B's DID (`did_hi`); validated to resolve to a well-formed DID document with an Active Signing Key (§3). |
| `local_did` | `DID` | A's DID (`did_lo`), proven owned via `OwnedIdentityDid` (ADR-049 §5). |
| `derived_context_id` | `[u8;32]` | Recomputed by A; a mismatch ⇒ `Rejected`. The crypto provider indexes MLS group state by this id (create-time `Entry::Vacant` collision guard). |
| `creation_receipt` | `Option<CreationReceipt>` | The typed, A-authored receipt: `Some` on A's side, `None` on B's side. **A-authored only** (the booleans `mls_group_created`/`sender_key_created`/`event_log_created`/`published` are inherently A-local creation state — B, joining via Welcome, creates no group or sender key, so B stages `None`). Its canonical byte form is `CreationReceipt::to_bytes` = `jcs::to_string(CreationReceipt).into_bytes()` (UTF-8 of the RFC 8785 canonical JSON below), round-trippable via `to_bytes`/`from_bytes`. The journal evidence for the saga is the MessagePack of the whole public staged state (see *Commitment coverage*), not the receipt bytes in-place. "Byte-reproducible across implementations" means a *single* party's serializer is deterministic (RFC 8785 JCS) — NOT that A's and B's receipts are byte-identical; B authors no independent `CreationReceipt`. |

A validates in order: (1) it is not already Active under `derived_context_id` — if so it returns `Rejected { reason: AlreadyExists }` (the idempotent-success mapping is **scoped to verified-self-membership** under the deterministic `derived_context_id`, see *Replay*; a bare existence is not auto-success); (2) the `bilateral-persistent` template params are well-formed (§5.12.1); (3) `peer_did` resolves and is not blocked; (4) A's MLS provider holds no existing group under `derived_context_id` (enforced by the `create_mls_group` `Entry::Vacant` guard; collision ⇒ `Rejected`), symmetric to Prepare-B's collision check below. On success A creates the MLS group locally (2-leaf init) plus a fresh sender key, recording handles into the staged `CreationReceipt`, but does **not** publish (deferred to Commit). It replies `Prepared`; the supervisor journals `PreparingB` before contacting B.

**Prepare-B (consent-gated, reservation-only).** The supervisor sends B `derived_context_id`, A's DID, and A's published MLS `KeyPackage` reference. B validates **in order, cheapest-and-consent-first**: (1) **inbound-contact gate** symmetric to A's block check — B rejects (→ `Aborting`) if A is on B's block list, or if B's contact policy requires opt-in for standing pairs and A is not approved; (2) `derived_context_id` recomputes from `(B_local_did, A_did)`; (3) B holds no Active context under `derived_context_id` and B's MLS provider holds no existing group under it (collision ⇒ `Rejected`); (4) A's KeyPackage resolves and is unconsumed. Only after (1)–(4) pass does B do MLS work: it **reserves** A's KeyPackage (it does NOT consume it — consumption is deferred to Commit so an abandoned Prepare cannot drain A's pool) and stages the join. B stages symmetric evidence — its `StandingPairCreatePrepared` carries `local_did=did_hi`, `peer_did=did_lo`, and `derived_context_id`, but does **not** carry an independent `CreationReceipt`: the `CreationReceipt` is A-authored only (its booleans are inherently A-local creation state, and B creates no group/sender-key as the Welcome-joiner), so there is no A/B receipt symmetry to reconcile. B replies `Prepared`. Any check failure ⇒ `Rejected` ⇒ `PreparingB → Aborting`. **Rationale:** the consent gate plus reservation-only discipline forecloses unsolicited resource consumption and KeyPackage-pool drain on arbitrary victims — Prepare-B is cheap and side-effect-free until Commit.

**Anti-griefing bound (normative).** For contexts whose contact policy **requires opt-in**, the Prepare-B consent gate (check (1): block-list plus opt-in approval) is the primary Sybil bound — an unapproved Sybil DID is rejected *before* any KeyPackage resolution or reservation work (checks (2)–(4)), so reservation churn is foreclosed at the cheapest check. §5.15.4's per-participant-context-set serialization independently bounds *concurrency*: every inbound standing-pair saga `(A_i, B)` shares context B, so all of them overlap at B and a second inbound Prepare while one is in flight is rejected saga-busy — B admits at most one inbound standing-pair saga at a time across all peers, so unbounded *concurrent* reservation churn is structurally impossible. The residual is therefore *sequential* churn: for contexts with an **open** contact policy (which by policy choice accept standing pairs from any DID and so have no opt-in gate to reject Sybils), a Sybil fleet of distinct lower-DIDs could cycle rapid *repeated* initiations, each after the prior saga reaches a terminal state. To bound that, B MUST rate-limit inbound standing-pair initiations per peer DID (a per-peer cooldown between successive attempts), mirroring the per-caller §6.2.0.2 rate budget the cross-context tool saga (§6.2.4) carries and the freshness/nonce-dedup the broadcast hosting saga (§5.14.13) carries. So the bound is not implementor-discretionary, the cooldown is concretely parameterized: **absent a context-specific override it defaults to 60 seconds, and a context MUST NOT configure it below a 1-second hard floor** — a degenerate near-zero cooldown is non-conformant. (The per-peer cooldown bounds *repeated* attempts from a single DID; a fresh-DID Sybil fleet, each DID attempting once, remains bounded by the §9.3 honest "expensive to sustain" cost of minting admissible DIDs plus reserve-not-consume, not by this cooldown.) This composes with reserve-not-consume KeyPackage handling, which prevents actual pool drain. The residual is framed honestly per §9.3: the goal is to make Sybil attacks **expensive to sustain, not impossible to attempt** — a fresh unlinked DID satisfying admission can still attempt one reservation, but cannot sustain unbounded reservation churn.

**`CreationReceipt` canonical field set (JCS)** — public plan-metadata only (never MLS secrets / sender-key / ratchet):
```
CreationReceipt (JCS object):
  { "context_id": <canonical standing context id, "standing-" + 64-hex per the §5.15.8 derivation>, "mode": "encrypted",
    "template_id": "scp:template/bilateral-persistent", "creator_did": <did_lo>, "peer_did": <did_hi>,
    "mls_group_created": <bool>, "sender_key_created": <bool>,
    "event_log_created": <bool>, "published": <bool> }
```

The `context_id` field here is intentionally the full `"standing-"`-prefixed canonical/display id (the canonical standing context id) — the `CreationReceipt` is a journal/display artifact, NOT a signature-preimage field, so it carries the display form, not a `Fixed32` slot. This is distinct from `derived_context_id` (the raw 32-byte digest used in signature preimages and `Fixed32` slots, §5.15.8): a reader MUST NOT conflate the two forms. This mirrors §6.2.4's id-form discipline (`Fixed32` preimage slots always carry the raw digest, never the prefixed display string), applied here to the inverse case — a non-preimage display artifact correctly carrying the prefixed form.

**Commitment coverage.** The journal records **only public plan-metadata** — **no §9.4.3 secret commitment** (there is no bearer artifact; MLS secrets live only in actor-local crypto-provider state, as for any non-saga creation). Journal `evidence` is the MessagePack of the public `StandingPairCreatePrepared`, wrapped in the standard `Zeroizing` envelope for uniformity but classified **public**; `mark_resolved(secret_bearing=false)` — no synchronous on-disk overwrite.

**Receipt signing.** The `CreationReceipt` is **not** independently signed — authenticity derives from the MLS layer (B's Welcome processing binds B into A's group; the first application message carries the Ed25519 InnerEnvelope signature). A standalone receipt signature would be redundant and would create a divergent authenticity surface. The receipt is a journal/replay artifact, not a transmitted credential.

**Commit and idempotency.** On both-`Prepared`: journal `Committing`, send Commit to A then B. **Idempotent by `SagaId`** (each actor records the last applied standing-pair Commit `SagaId`; a duplicate Commit *bearing the same supervisor-minted `SagaId`* is a no-op re-ack). The `SagaId` is supervisor-minted (`SagaId::new()`), never caller-supplied, so it is not an attacker-chosen idempotency key. At Commit, B **consumes** the KeyPackage it reserved at Prepare. A publishes (flips `published`, emits the Welcome to B's personal routing id), registers Active, and records the peer in its contact graph via `register_standing_context`; B processes the Welcome, registers Active, and records the peer via `register_standing_context`; both append the creation to their event logs. (`register_standing_context` is idempotent — a redundant re-run on an already-registered pair is a no-op.) Journal `Committed` (terminal) after both ack.

**Prepare-B-failure rollback of the Prepare-A MLS group.** `PreparingB → Aborting` must reclaim A's un-published group. The Abort to A invokes `CreationReceipt::rollback` (reverse order, best-effort per step): transport `delete_published` (a no-op while `published==false`), `destroy_event_log`, `destroy_sender_key`, `destroy_mls_group`. A's group was never published, so no relay or peer observed it — this is purely local key/state destruction. The §9 respawn loads a snapshot predating the in-flight `saga_pending`, so a crash mid-Prepare also discards the staged group on restore (the actor-state reconciler is a backstop, not the primary path).

**Abort / compensation.** Abort is reached from `PreparingA` (A rejects or times out) or `PreparingB`. The outcome is all-or-nothing at the granularity of "both registered Active and published" versus "neither holds the context."

**Replay interaction with `register_standing_context`.** `register_standing_context` records a peer DID in the contact graph (§5.12.6) — separate local bookkeeping that lets `standing_context(peer_did)` resolve get-or-create without re-running the saga. On restart, §5.15.4 replay re-drives unresolved entries per the §17.16.4 crash-recovery contract:

- A **`Committed`** journal is **terminal** and is therefore **not** re-loaded by §17.16.1's unresolved-saga scan — no startup replay occurs for it. If a crash lands between Commit-apply (which now includes the `register_standing_context` write, above) and that registry write completing, the gap is closed **not** by saga replay but by the next `standing_context(peer_did)` get-or-create (below): its fresh Prepare-A surfaces `AlreadyExists`, maps to verified-self-membership success, and (re-)runs the idempotent `register_standing_context`.
- A **Commit-in-progress** journal ⇒ re-send Commit to both participants; each deduplicates by `SagaId` (A re-emits its Welcome / re-registers Active as a `SagaId` no-op; B re-processes the Welcome as a `SagaId` no-op) — per §17.16.4.
- A **Prepare-in-progress** journal (one actor already `Prepared`) ⇒ the `Prepared` actor is aborted (A's staged-but-unpublished MLS group is rolled back via `CreationReceipt::rollback`; B drops its KeyPackage reservation) and the saga is **discarded**, per §17.16.4. Recovery never re-runs Prepare. The initiator's `standing_context` call errored and **retries fresh** (a new saga, a new `SagaId`) — consistent with the "no resume" retry semantics of §5.2.1 (*Retry semantics*: "there is no 'resume' mechanism for partially created contexts").
- **Pre-Prepare** (no actor committed) ⇒ discard.

The legitimate idempotency path is a caller's get-or-create **retry** after a committed-but-unregistered pair: the retry re-runs Prepare-A as a FRESH saga (a distinct `SagaId`); its check (1) surfaces `AlreadyExists`, which maps to idempotent success based on **verified-self-membership under the deterministic `derived_context_id`** — id-determinism guarantees the existing context IS this exact A–B pair, so A (a verified member) registers it via `register_standing_context` and returns it. This success is gated on **verified-self-membership**, NOT on a `SagaId` match (the retry is a distinct saga). Non-members never reach this success path (they are not members of the derived context); an `AlreadyExists` surfaced to a non-member returns the generic rejection per the existence-oracle clause below (timing- and value-indistinguishable). Id determinism guarantees that "already exists under this id" means "this exact pair," never a collision.

**`AlreadyExists` is not an existence oracle.** Outside replay, an `AlreadyExists` for a `derived_context_id` the *caller is not a verified member of* MUST NOT return `Ok` (it would leak that a standing pair between two DIDs exists — the contact graph is private local bookkeeping, §5.12.6 — and would let a caller believe it created a context it is not a member of). When the caller is not a verified member of the `derived_context_id`, the FFI surface MUST return a response **indistinguishable from the generic rejection any other failure returns** — a typed `AlreadyExists` error is itself a 1-bit existence oracle (it tells a non-member that a context exists under that id) and is therefore forbidden on the non-member path. The indistinguishability requirement covers **timing as well as value**: the non-member path MUST NOT be distinguishable by response latency — it MUST NOT branch its timing on whether a context exists under `derived_context_id` (it must be constant-time with respect to existence), because an existence-dependent latency branch is itself a 1-bit existence oracle even when the returned error value is identical. The typed `AlreadyExists`-mapped-to-`Ok` outcome is reserved **strictly** for the verified-self-membership case (the caller is a verified member of the existing context under the deterministic `derived_context_id`); in every other case the non-member sees only the same generic rejection, returned without an existence-dependent timing difference.

Cross-refs: §5.12.6 (standing contexts / contact graph), §5.15.4 (saga FSM), ADR-049 §3a (FFI Saga Surface). No saga journals a commitment; all three sagas are public-metadata-only.
