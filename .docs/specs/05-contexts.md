...
**Roles (symmetric initiation, normative).** There is **no creator/peer asymmetry in the common case**: **either party MAY initiate** by running the ordinary create flow — create a 1-leaf group, fetch the peer's published `KeyPackage`, `add_member`, emit the Welcome. When only one party initiates (the common case), the other simply **joins on Welcome receipt** (consent permitting), and `did_hi`'s `standing_context()` returns the same `Ok` (its replica created + Welcome dispatched) as any other initiator's. **Send-capability caveat in the collision case.** That "same `Ok` as any other initiator's" claim holds for the *common* case (`did_hi` initiated and nobody collided with it); a `did_hi` that *loses* a simultaneous-create collision (below) destroys its own group and **Welcome-joins** `did_lo`'s canonical group, and a Welcome-joined replica — like any joiner — **cannot SEND** until the Phase-2E spawn-from-Welcome entrypoint lands (*Known limitation*, ADR-049 §Follow-ups). So a collision-losing `did_hi`'s `Ok` reflects *replica-created and `did_lo`'s-Welcome-dispatched/processed*, and its send for that pair is interim Phase-2E-gated exactly as any joiner's is; once Phase-2E lands this resolves uniformly. The `did_lo` / `did_hi` deterministic tie-break governs **ONLY collision resolution in the genuine simultaneous-create race** — the case where *both* parties created a group under the same `derived_context_id` before either's Welcome was processed (the per-node `create_mls_group` `Entry::Vacant` guard is per-node and cannot coordinate two distinct nodes, so a genuinely-concurrent both-initiate produces two distinct MLS groups under the same `derived_context_id`). It is not a creator pin and not a two-phase ordering.
...
- `did_hi`, on receiving a Welcome **authored by `did_lo`** for a `derived_context_id` under which `did_hi` already holds **its own self-created group** (regardless of leaf count — a 1-leaf group, or a 2-leaf group in which `did_hi` had already `add_member`'d `did_lo`), MUST — only after confirming the incoming Welcome's **creator credential resolves to `did_lo`** as a **cryptographically BOUND check, not a self-asserted-string match** (the lower DID of the pair, verified from the creator leaf after Welcome processing: BOTH the creator leaf's `ScpCredential.did` MUST equal `did_lo` AND the creator leaf's MLS signature key MUST equal the verification method resolved from `did_lo`'s DID document per the §9.7.1 KeyPackage-signature / DID-VM binding — a leaf carrying `did: did_lo` but a signature key **not** present in `did_lo`'s DID document MUST NOT satisfy the check; a self-asserted DID string alone is insufficient, foreclosing a forged-creator-string DoS. A Welcome whose bound creator is **not** `did_lo` MUST NOT trigger the destroy, foreclosing a targeted DoS in which an attacker addresses a Welcome under the pair's `derived_context_id` to tear down `did_hi`'s legitimate group) — **destroy that self-created group** and process `did_lo`'s Welcome (join the canonical group). The "is this group **self-created** vs. arrived via the peer's Welcome" predicate, the creator-credential confirmation, and the destroy MUST be evaluated **atomically** (because standing contexts use deterministic ids, "atomically" MUST be implemented as the per-context actor mutex held across {confirm-creator + destroy + join} together with a generation/identity check, so a concurrent context-recreate cannot slip a different group under the same `derived_context_id` between confirm and destroy — a confused-deputy that would otherwise let the destroy tear down a freshly-recreated legitimate group); a group reached via the peer's Welcome is never self-created and is never destroyed by this rule.

`did_hi`'s destroy-and-rejoin of `did_lo`'s canonical group **is a Welcome-join**: after destroying its own group `did_hi` joins `did_lo`'s by processing `did_lo`'s Welcome, so `did_hi` becomes a Welcome-joined replica. It therefore inherits the **same interim send-gating as any Welcome-joiner** — it can join and decrypt but **cannot SEND** in the standing context until the Phase-2E spawn-from-Welcome entrypoint lands (*Known limitation* below, ADR-049 §Follow-ups); once Phase-2E lands this resolves uniformly across the common and collision cases.
