...
| `derived_context_id` | `[u8;32]` | Recomputed by A; a mismatch ⇒ `Rejected`. The crypto provider indexes MLS group state by this id (create-time `Entry::Vacant` collision guard), so it is the identifier that keys MLS group isolation. |
| `creation_receipt_bytes` | `Vec<u8>` | `jcs::to_string(CreationReceipt).into_bytes()` (UTF-8 of the RFC 8785 canonical JSON below), round-trippable via `CreationReceipt::to_bytes` / `from_bytes`. The `CreationReceipt` is **A-authored only** (the booleans `mls_group_created`/`sender_key_created`/`event_log_created`/`published` are inherently A-local creation state — B, joining via Welcome, creates no group or sender key). "Byte-reproducible across implementations" means a *single* party's serializer is deterministic (RFC 8785 JCS) — NOT that A's and B's receipts are byte-identical; B authors no independent `CreationReceipt`. |
