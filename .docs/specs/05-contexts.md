...
**Signature refusal (normative).** An operator's signing seam can refuse a chunk: the payload fails RFC 8785 canonicalization while composing the preimage, or the custody backend that holds the operator key returns an error. An implementation MUST NOT emit a chunk whose `sig` field it did not obtain from the operator key, and MUST NOT substitute a zero, random, or otherwise synthesized value — a chunk bearing a signature no operator produced is an attestation the operator can neither be held to nor repudiate, and the equivocation binding above rests on every emitted `sig` being real.

Refusing to emit is necessary but not sufficient. A pump that answers a refusal by closing its channel leaves a member unable to distinguish a refused chunk from a completed stream, and the refusal is precisely the case where an operator suppressed output. An implementation MUST therefore close the stream on a signal the member reads, chosen as follows:

1. **Attempt a terminal `Error` chunk carrying the `execution.signing-refused` slug under code `SCP-OUTLET-6137`,** signed under the same operator key at the sequence the refused chunk would have used. The refused chunk MUST NOT consume a sequence number, so the terminal takes its slot and the member reads a contiguous sequence. The terminal's payload MUST carry only the code, the slug-prefixed message, and `terminal: true` — never text derived from the refused payload. The cross-context bridge below forwards this terminal verbatim to an invoker who is not a member of the operating context, so payload-derived text in it would move the refused chunk's content across a context boundary that no governed path authorized.
2. **When the operator key refuses that terminal as well,** the implementation MUST surface a typed, non-chunk value to its caller naming the refusal. That value is a local control signal, not a protocol artifact: it MUST NOT be assigned a `sequence`, MUST NOT enter the `stream_manifest_hash`, and MUST NOT be forwarded across a context boundary.

The implementation decides which of the two applies by attempting the terminal and reading the operator key's answer, never by classifying the first refusal — a custody backend that refuses one call can serve the next, and a member that would have received a signed terminal must not lose it to a classification.

A member that reads the terminal `Error` learns that the operator produced output it did not sign and therefore did not deliver. It learns neither how many chunks were withheld nor what they contained: the `stream_manifest_hash` covers the chunks the operator emitted, so it proves the operator did not over-bill and proves nothing about completeness. §5.4.4's `execution.signing-refused` slug is the sole protocol record that the withholding happened.
