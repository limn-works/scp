...
A **standing pair** is the `bilateral-persistent` context two identities create on first contact (§5.12.6). It is **ONE** MLS context — one MLS group with **two members**, NOT two contexts: the symmetric derivation below has both parties compute the *identical* `derived_context_id`, so they create and join the **same** context and each member's node holds a **replica** of it. Replicas are kept consistent by **MLS** (epoch-ordered Commits + the bootstrapping Welcome) over the event-log RFC-6962 consistency layer (§17, §5.15) — the same synchronization every single context uses. There is **no cross-context atomicity to coordinate**, so creating a standing pair is **ordinary single-context async creation**, *not* a cross-context saga.

> **Provenance (correction, 2026-06-18).** §5.15.8 was previously specified as a two-phase-commit cross-context saga (originally authored as the standing-pair saga). That was a miscategorization: a 2-member MLS group is one context, replica sync is MLS + the event-log consistency layer, and a saga coordinates atomicity across **2+ distinct** contexts sharing no sync protocol — which a standing pair is not. The genuine cross-context sagas are exactly §6.2.4 (cross-context tool invocation) and §5.14.13 (broadcast-hosting handshake). See ADR-049 §3 / §3a. (The prior claim that unconditional length-prefix injectivity "would add no security" is **retracted**: the colon-join was always the sole structural isolation anchor for `derived_context_id`; the `group_id` removed in the saga-cut was the *saga's* separate MLS group identifier, not an isolation co-anchor. Length-prefix framing replaces a human method-admission assumption with an unconditional guarantee — see *Injectivity invariant* below.)

**Determinism precondition.** The id is a pure function of the two DIDs:
```
standing_context_id = "standing-" || hex( SHA-256( "standing:" || did_lo || ":" || did_hi ) )
```
`did_lo` / `did_hi` are the two participant DID strings, **each in its §3 canonical DID string form as produced by DID resolution** (which yields a single comparison form per method: a `did:dht` id is lowercase z-base-32 of the Ed25519 public key per §9.6.1; a `did:web` id is normalized per the W3C did:web method — host lowercased, segments percent-encoded so a literal `:` is `%3A`), then sorted lexicographically (bytewise UTF-8). `hex(...)` is lowercase hex of the 32-byte digest; the raw 32-byte digest is `derived_context_id: [u8;32]`. Canonicalizing **before** the sort guarantees both parties feed byte-identical bytes into the preimage, so `derive(A,B) == derive(B,A)` and both compute the identical id; no party "allocates" it. This is the same canonical-DID-as-key rule §5.14.13 applies to snapshot keys.

**Injectivity invariant (load-bearing).** The colon-join `"standing:" ‖ did_lo ‖ ":" ‖ did_hi` is **not** injective in the abstract, but is safe here because the **method-specific identifier** (the only attacker-influenced segment) is self-delimiting over a fixed alphabet — `did:dht` is z-base-32 (no raw `:`), `did:web` percent-encodes any literal `:` as `%3A` — so the preimage re-parses **uniquely** back into the ordered pair `(did_lo, did_hi)` and no attacker can shift the boundary to forge a colliding pair. (Full DIDs of course contain method-prefix colons; the property is colon-freedom of the *specific-id*, not the whole string.) A future DID method admitting a raw `:` in its specific-id would trip this documented assumption as a **fail-loud method-admission-review violation**, not a silent collision. This rides on a human admission gate; the RECOMMENDED unconditional hardening is length-prefix framing of the derivation (§9.5.1, `len32(did_lo) ‖ did_lo ‖ len32(did_hi) ‖ did_hi`), which makes the encoding injective for *any* DID grammar and **retires the human gate**. It is deferred here only because changing the derivation is a coordinated spec-plus-code change (both parties must re-derive byte-identically) — a recommended follow-up, not a lost backstop.

**MLS-layer defense-in-depth.** `derived_context_id` is the security-critical identifier: the crypto provider's `create_mls_group` `Entry::Vacant` guard keys on the canonical digest `SHA-256("standing-" ‖ hex(derived_context_id))` (a collision-resistant 1:1 function of `derived_context_id`, so the guard fires across exactly the distinct ids), rejecting a second group under the same id per node. Even on a hypothetical id collision, the OpenMLS `GroupId`, the per-group key schedule, and per-member credentials are **independent** isolation barriers: a colliding pair would still need a valid Welcome plus the right credentials to read anything — **a collision alone grants no plaintext.** The colon-join hardening is defense-in-depth on top of that, not the sole line of defense.
...
1. **Initiator (A) creates the group.** A recomputes `derived_context_id` and validates: (a) it is not already Active under the id (else the *Get-or-create idempotency* path applies — bare existence is not auto-failure); (b) `peer_did` is canonically **distinct from A's own DID** (a self-pair is rejected with the same generic/typed malformed-peer rejection as any malformed peer DID, disclosing nothing); (c) the `bilateral-persistent` template params are well-formed (§5.12.1); (d) `peer_did` resolves to a well-formed DID document with an Active Signing Key (§3) and is not blocked; (e) A's provider holds no group under the id (the `Entry::Vacant` guard; collision ⇒ error). On success A creates a **1-leaf** MLS group plus a fresh sender key.
...
3. **Publish / register (A).** A publishes the group, emits the **Welcome to B's personal routing id** (the §5.12.3.3 invitations routing id) asynchronously (A does not block on B), so the Welcome rides the §5.12.3.3 invitation-delivery path and carries its relay-retention TTL (`welcome_ttl`, 7 days default — read by the reaper, item (d)); registers the context **Active**, appends the creation to its event log, and records the peer via `register_standing_context`.
4. **Peer (B) receives the Welcome and applies the consent gate on receipt** (*before* joining): (a) **block list** — refuse if A is globally blocked (§3.7.1 `is_globally_blocked`); (b) **default-deny for strangers** — a standing-pair Welcome from a **stranger** MUST NOT be auto-joined and carries the **same non-overridable intent** as §5.12.2's tool-bearing rule (a conformant SDK MUST NOT let an `AutoAcceptPolicy` override the deny), justified by the pair's `memory_scope: full` sensitivity (§5.12.1). The **stranger** test is **§9.3's not-self-created-by-EITHER-party predicate**: a qualifying shared context counts only if neither party is its creator/admin and it is distinct from the pair being evaluated — closing the circular self-clear bypass (a self-created context, including one minted in the same first-contact flow, does not qualify). This default-deny is a **MUST on conformant implementations at the SDK consent-gate layer**, not protocol-layer enforcement. **Per-arm residuals are inherent to each `TrustRequirement` (§5.12.2)** and are disclosed, not patched: `shared_context` inherently extends transitive trust to a colluding third-party confederate's members; `discovery_context` (whose self-manufacture guard MUST be symmetric — not-self-created by *either* party) inherently delegates curation trust to the context's curator; `known_did` is exactly as trustworthy as B's own allowlist hygiene. If B's policy configures an `AutoAcceptPolicy` whose `TrustRequirement` A satisfies, B MAY auto-join; else B joins only after explicit approval. The gate is applied by the **joining peer on Welcome receipt**, never as a synchronous Prepare-B reply: on accept B joins (single-use enforced at join), registers Active, appends, and runs `register_standing_context`; on reject B simply **never joins** — no synchronous "Rejected" reply.

**Block-privacy (consent-on-receipt).** Because consent is applied by the *joining* peer asynchronously, a blocked or unapproved initiator gets **no synchronous rejection** — a decline is indistinguishable from an offline/slow peer, foreclosing the **synchronous** block/pair-existence reply oracle a two-phase `Rejected` would leak. **Scope (precise):** this closes the *reply* oracle only. A's step-2 fetch of B's **published** `KeyPackage` runs *before* the gate and is relay-observable, so a network observer can distinguish "B has a published KeyPackage" from a peer that never published one; the async gate does not claim to hide that. That published-KeyPackage bit becomes a *targeting* primitive only when chained with a stranger-bar bypass, which the not-self-created qualifier (step 4(b)) blunts.

**Known limitation — wire-observable solicitation (BLACK-SP-01).** The "private contact graph" claim (§5.12.6) scopes to A's **local bookkeeping**. The standing-pair Welcome rides B's *publicly-computable* invitations routing id **unconditionally, before** the consent gate, so first-contact solicitation metadata + addressee DIDs are **observable to relays**. This is inherent to contacting a peer over an untrusted relay; consent-on-receipt closes only the synchronous *reply* arm, **not** solicitation visibility. The message *content* remains MLS-encrypted.
...
- **(a) Decline is indistinguishable from offline** — no synchronous `Rejected`, no peer-state probe; A observes only "B has not joined yet."
...
**Send-capability caveat — ALL Welcome-joiners (normative).** Per ADR-049 §Follow-up #1, **any party that obtains its replica via Welcome-join** — the non-initiating common-case peer AND the collision-losing `did_hi` — can join and **DECRYPT** but **cannot SEND** until the Phase-2E spawn-from-Welcome entrypoint lands. This is the most frequent path, not an edge case. A Welcome-joined `Ok` therefore reflects *replica-created and decryptable* but **interim send-gated**, resolving uniformly once Phase-2E lands; A's (initiator) sends are unaffected.

**Known limitation (Phase 2E follow-up) — security-relevant.** The **initiator→peer direction is fully functional today**; only the **joiner-originated** direction is gated until the Phase-2E spawn-from-Welcome actor entrypoint exists. This is **security-relevant, not merely a feature gap**: a simultaneous-create collision is attacker-influenceable — a peer who is `did_lo` relative to a victim can deterministically race a create to push the victim (`did_hi`) onto the send-gated path. Bounded — the attacker must already be a consent-passed pair member, and the worst case is the victim being receive-but-not-send **in that one pair** until Phase-2E, never a cross-pair effect or key exposure. **Self-heal scope (BLACK-SP-03):** §3.7.1 severance requires a sender-key rotation (a SEND, §3.7.1 / §9.16.3), so the unobserved-block self-heal above is performed by the **send-capable side** (the initiator) on next connection; a **send-gated Welcome-joined replica** (the common joiner, and a collision-losing `did_hi`) **cannot sever until Phase-2E**. The "`did_lo` can never force a durable join to a blocking peer" end-state convergence therefore holds **conditioned on the blocking side being send-capable** — a send-gated blocker's severance is Phase-2E-gated (disclosed Known-limitation, not immediate bilateral self-heal). See ADR-049 §Follow-ups.

**`standing_context` Ok-return contract (normative).** A successful return means the **initiator's replica is created and the Welcome dispatched** — it does **NOT** imply the peer joined or that a bidirectional channel exists. An offline, slow, blocking, and declining peer **all yield the same `Ok`** (join observed only out-of-band; intentionally no synchronous confirmation — block-privacy above). When the call resolves via a **Welcome-join** (the common-case non-initiating peer, or a collision-losing `did_hi`), its `Ok` reflects *replica-created and decryptable* but **interim send-gated** until Phase-2E (*Send-capability caveat* above). A reaped or never-joined handle does **not** dangle — the next call auto-revives under the deterministic id (ADR-049 §10). Get-or-create returns the **identical handle type** whether it created or found the pair: **no typed create-vs-found or `peer_joined` discriminant in the return value** (a verified member MAY observe found-vs-create *latency* for their own pair, §5.12.5, but a typed discriminant and any non-member observation are foreclosed). **FFI/SDK bindings MUST NOT enrich the return with a create-vs-found or `peer_joined` discriminant** (e.g. a `created: bool`) — the uniform `Ok` is what forecloses the synchronous block/pair-existence oracle; the shape MUST be identical across all 4 bindings. The sole non-`Ok` outcomes are genuine failures (malformed/self peer DID, a peer A itself blocked, or the existence-oracle generic rejection) — never a typed signal about the peer's join decision.

**Get-or-create idempotency.** `register_standing_context` records a peer DID in the contact graph (§5.12.6) — local bookkeeping that lets `standing_context(peer_did)` resolve get-or-create without re-creating the group (idempotent — a redundant re-run is a no-op). It is an **internal contact-graph operation, never an FFI export** (mirroring ADR-049 §3a); the FFI/SDK surface is `standing_context(peer_did)` get-or-create alone. A get-or-create surfacing `AlreadyExists` under the deterministic id maps to idempotent **success based strictly on verified-self-membership** — id-determinism guarantees the existing context IS this exact pair, so a verified member re-runs the idempotent registration and returns it; non-members get the generic rejection (existence-oracle clause below). A crash between create/join and the registration write is closed by the next get-or-create. **Self-only group is not success-without-re-drive:** a bare group where the **peer is not yet a member and no Welcome was durably emitted** (crash after the 1-leaf create but before `add_member` / Welcome-emit) is half-created — get-or-create MUST re-drive the **idempotent** `add_member` + Welcome-emit (+ publish / register) to completion for whichever party created it. (A node that instead received the canonical peer's Welcome resolves it per *Concurrent-creation collision resolution* rather than re-driving its own group.) Verified-self-membership maps to immediate success **only** once the peer is a member OR a Welcome was durably emitted. **Residual (disclosed):** a `did_hi` self-only re-drive completing before `did_lo`'s canonical Welcome arrives consumes one of `did_lo`'s published single-use KeyPackages for a Welcome `did_lo` will ignore and `did_hi` then destroys — a bounded, disclosed sub-case of the general MLS KeyPackage-pool concern. The bound applies at **`did_lo`'s inbound consent-gate per-initiator cooldown** (which throttles `did_hi`'s *outbound* Welcome-emit re-drive to `did_lo`), not at `did_hi`'s own cooldown; it is no new amplification vector, and the init-key single-use plus `did_lo`'s ignore-rule preserve single-group convergence.

**`AlreadyExists` is not an existence oracle.** An `AlreadyExists` for an id the caller is **not a verified member of** MUST NOT return `Ok` (it would leak that a pair between two DIDs exists — the contact graph is private local bookkeeping, §5.12.6). The non-member path MUST return a response **indistinguishable in value AND timing** from the generic rejection any other failure returns — it MUST be constant-time with respect to existence, because an existence-dependent latency branch is itself a 1-bit oracle even when the returned value is identical. The typed `AlreadyExists`→`Ok` outcome is reserved **strictly** for verified-self-membership. **Implementer mechanism:** resolve membership **first**, then branch only on *membership* — a non-member and a no-such-context input traverse the same fixed-cost lookup work (`derived_context_id` resolution + membership check) and return the same generic rejection; existence MUST NOT be a branch the latency depends on. The §5.12.5 found-vs-create latency hint (`~0ms` found vs `~200ms` create) applies **only to a verified member's own pair** on the success path, never to the constant-time non-member path.

Cross-refs: §5.12.6 (standing contexts / contact graph; private local bookkeeping), §5.12.1 (`bilateral-persistent` template), §5.12.2 (`AutoAcceptPolicy` / `TrustRequirement` arms), §5.12.3.3 (InvitationBundle relay-retention TTL), §5.12.5 (found-vs-create latency hint), §3 (canonical DID string form), §3.7.1 (block list / severance — sender-key rotation), §9.3 (Sybil resistance / earned capacity), §9.5.1 (length-prefix framing), §9.6.1 (did:dht self-certification / z-base-32 canonical form), §9.7.1 (MLS-to-SCP Concept Mapping — KeyPackage-signature / DID-VM binding rule), §9.16.3 (sender key rotation), §17 (event-log consistency / checkpoints), §6.2.4 (cross-context tool invocation saga), §5.14.13 (broadcast-hosting handshake saga; canonical-DID-as-key precedent), ADR-049 §3 / §3a / §9 / §10 / §Follow-ups (single-context async creation, not a saga; fused-join single-use; auto-revive; spawn-from-Welcome).
