...
- The `shared_context` trust requirement means strangers can never trigger auto-accept — the existing shared context provides the trust baseline. (When `shared_context` is used to clear a **first-contact** stranger bar — e.g. standing-pair Welcomes, §5.15.8 — the qualifying context MUST be **not self-created by either party and distinct**, mirroring §9.3's "(not self-created)" qualifier, so a manufactured self-created two-party context — created by *either* the evaluating party or the peer — cannot self-clear the bar.)
- **First-contact disposition of every `TrustRequirement` arm (normative — covers all three arms, not only `shared_context`).** When ANY `TrustRequirement` arm is used to clear a **first-contact** stranger bar (§5.15.8 step-4(b) cross-references this clause for all three arms), the arm MUST NOT be **self-clearable** by the candidate:
  - **`shared_context`** — the qualifying context MUST be not-self-created/admin'd by either party and distinct (above). A self-created context, including one minted in the same first-contact flow, does not qualify.
  - **`discovery_context`** — the discovery context used to clear the bar MUST itself be **not self-created/admin'd by the candidate** (the same §9.3 not-self-created discriminator). Otherwise an attacker could self-register their own DID in a discovery context they themselves created/admin and thereby **self-clear** the first-contact stranger bar — yielding a silent `memory_scope: full` standing pair with no human prompt. The not-self-created qualifier closes that self-registration self-clear path: a discovery context the candidate created/admins does not qualify.
  - **`known_did(list)`** — the bar is an **explicit operator allowlist**, so there is **no self-clear path**: the candidate cannot add itself to the evaluating party's allowlist. This arm is self-clear-resistant by construction (allowlist membership is set by the evaluator, not the candidate).
...
**MLS-layer defense-in-depth.** The `create_mls_group` `Entry::Vacant` guard keys on `SHA-256("standing-" ‖ hex(derived_context_id))` (a 1:1 function of `derived_context_id`), rejecting a second group under the same id per node. A colliding id cannot occur in the first place (length-prefixed injectivity forecloses it by construction); were one to, MLS `GroupId` + per-group key schedule + per-member credentials (RFC 9420) are independent isolation barriers — **a collision alone grants no plaintext.**
