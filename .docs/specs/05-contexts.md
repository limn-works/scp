...
1. **Proposal logged.** The `CeilingChangeProposed` event is recorded in the event log, containing the proposed new ceiling, the proposer's identifier, and the governance justification.
...
- **Operator.** The identity accountable for the outlet. Outlet misbehavior traces to that identity.
- **Cost metadata (optional).** Per-invocation cost declared by the outlet via an `OutletCost` struct (§5.4.1), additive with context-level costs (§19.3). An outlet calling an external API can pass through its cost. Outlet costs carry their own payee identifier, which may differ from the context payee. Outlets without cost metadata are free. Query outlets MUST declare either no cost or a zero-amount cost (§5.4.2 structural floor).
...
OutletRegistration {
  ...
  operator_did:     Identifier,      // The identity accountable for this outlet.
  schema: {
  ...
  signature:        P256Signature, // The operator signs the V2 canonical digest below.
}
...
OutletCost {
  ...
  payee:            Identifier,      // Who receives payment. May differ from operator_did.
...
caveats_binding = SHA-256(
  ...
  || invoker_did                         // the invoker's identifier (09-security-model.md §9.5.2)
...
|-----------|---------|-------|-----------|
| `max_concurrent_inbound_streams_per_invoker` | 8 | [1, 1024] | Maximum number of streams the *immediate-previous-hop* invoker may have open concurrently against any outlet in the context. Breach rejects the open with `OutletErrorClass::Transport::RateLimited` slug `transport.concurrent-streams-per-invoker`. |
| `max_concurrent_inbound_streams_per_origin_invoker` | 16 | [1, 1024] | Maximum number of streams the *outermost caller in the delegation chain* may have open concurrently against any outlet this operator hosts (tracked at operator scope, not per-context, so a caller cannot fan out across a cluster of interfaces hosted by the same operator to bypass the per-context limit). The outermost caller is the `iss` of the root UCAN in the delegation chain presented at open. Breach rejects with `OutletErrorClass::Transport::RateLimited` slug `transport.concurrent-streams-per-origin-invoker`. |
...
- **Subscriber** — holds `messages:read` (auto-granted on identity-authenticated registration in open broadcast contexts, or requiring an explicit admin-issued UCAN in gated broadcast contexts). Subscribers receive author broadcast keys on request. Subscribers are unbounded.

The author/subscriber distinction mirrors the writer/reader two-tier model from contexts with discovery outlets (§6.2.2B [no such section]). Open broadcast subscriber registration follows the same identity-authenticated pattern as context reader-tier access.
...
**Broadcast context membership.** Broadcast contexts use a two-tier membership model: authors (MLS-equivalent bounded writers) and subscribers (unbounded readers registered via identity-signed requests). Subscriber registration records membership via `MemberJoined` events with role `subscriber`. The member list includes both tiers. Subscriber count is visible in metadata.

### 5.6.1 Member Removal Is a Clean Teardown

**Member removal is a clean teardown.** When a member is removed from a context — by governance `RemoveMember`, by self-`leave`, or by a failed-join rollback — ALL per-identity role state for that identity is dropped in the same operation: the identity is removed from the member list (`members`), its role assignment (`assignments`), its cached granted capabilities (`member_capabilities`), AND its suspended-capability set (`suspended_capabilities`, §5.3.2). A suspension only ever DENIES authority (§5.3.2 step 5); once the identity holds no role it is meaningless, and a re-admitted member under the same identity is a fresh admission that MUST NOT inherit a phantom suspension — a residual suspension would otherwise wrongly deny the re-admitted member a capability their new role grants.

Per-identity content-access and routing state owned outside the role state is likewise dropped at removal: the member's `read_exclusion_list` entry (§5.9), its access-key store entry (§9.17), its MLS sequence counter, and its pseudonym routing entry (§9.10.4). The MLS group leaf for the removed identity is evicted by the MLS commit that precedes the state teardown; eviction is the cryptographic boundary and is non-negotiable — the state teardown described here is the bookkeeping that MUST converge with it, never a substitute for it.

Dropping the suspended-capability set on removal is a pure narrowing-side hygiene that never widens authority, consistent with the suspension-pruning rule at ceiling-change activation (§5.3.2 step 5): a suspension referencing a capability the member no longer holds is already droppable, and a removed member holds none. The runtime executes this teardown through the shared `ContextRoleState` removal primitive, clearing the four per-identity role fields together — so a context's **role state** is byte-identical after a remove-then-readmit of the same identity. Out-of-role per-identity state (read-exclusion entry, access-key store entry, MLS sequence counter, pseudonym routing entry) is dropped by whichever layer owns it, in the same operation.
...
**Signing key.** A metadata record is signed by the key a current context admin's key state lists `Current` in the `#active` role, which a verifier derives by replaying that admin's key-event log (`03-identity.md` §3.10.4). A human identity's key state names one operational role and names no agent key (`09-security-model.md` §9.1 invariant 1), so a delegated agent identity signs no metadata record for its human: metadata publication is a governance-adjacent operation that requires human-accountable authorization.
...
MetadataRecord {
    ...
    signer_did:       Identifier,     // Admin who signed this record
...
**Signer verification.** A prospective member verifying a metadata record: (1) resolves the `signer_did`'s key state (`03-identity.md` §3.10.4), (2) takes the key that state lists `Current` in the `#active` role, (3) verifies the P-256 signature, (4) cannot independently verify that the signer is a current admin without joining (admin status is internal context state). The signature guarantees that the metadata was produced by the claimed identity; the `signer_did` being an admin is a social-layer assurance, not a cryptographic one. Once a member joins and has access to the membership roster and role assignments, they can retroactively verify that the signer held admin status at the time of signing.
...
| `RotateContentKeys { reason }` | Context-wide key rotation | All members, not identity-targeted |
...
Content access actions go through the context's governance model (propose/vote/execute). In SingleAdmin contexts, the admin's proposal auto-executes. In multi-admin contexts, the action requires the configured quorum. Tiers 1-2 (identity-to-identity blocking) are unilateral identity-layer operations and do NOT go through governance.
...
Both `suspended_capabilities[did]` and the removed identity's `read_exclusion_list` entry are dropped when the member is removed from the context (§5.6.1 — member removal is a clean teardown), so neither retains entries for non-members.
...
EphemeralDeletionRequest {
  ...
  requester_did: Identifier,     // the member requesting deletion
  context_close_proof: {
    context_id:  ContextId,
  ...
  signature:    P256Signature, // Signed by the requester's #active key
}
```
...
**Enforcement honesty.** The protocol enforces memory scope through cryptographic key destruction — specifically, MLS group state destruction (tree secrets, all epoch key schedules, application key material). This is verifiable and absolute for protocol-level data. Platform-attested destruction (§9.15) provides hardware-backed evidence that keys were deleted where available, and §9.15 grants that evidence its High-confidence rating only when a verification of the platform attestation proof returns a pass — a member's written `hardwareBacked` claim rates as software-only on its own. However, the protocol cannot enforce memory scope above the protocol boundary. An agent's underlying model may retain information from an ephemeral interaction in its own memory. The spec is explicitly honest about this limitation: ephemeral memory scope destroys the protocol-level record and makes reproduction unverifiable, but does not guarantee the agent has forgotten. The absence of provenance on information an agent produces from memory is itself a signal — "this data has no verified origin." Participants in other contexts can evaluate unprovenanced information accordingly.
...
Template: "scp:template/public-broadcast"
  ...
  roles:
    ...
    subscriber:  messages:read (auto-granted on identity-authenticated registration)
  governance:    single-admin
  memory_scope:  full
  ttl:           optional
...
The ONLY difference between `public-broadcast` and `gated-broadcast` is whether the subscriber role's `messages:read` is auto-granted (identity-authenticated, following the context reader-tier pattern §6.2.2B [no such section]) or requires an explicit admin-issued UCAN (like encrypted context membership). The open/gated distinction is expressed through the template's role definitions, not through a new enum type.
...
- **Auto-accept is allowlist-only.** The sole auto-accept trigger is an identity on the operator's explicit `known_did` allowlist. Co-membership in a shared context and registration/discoverability in an open registry are **NOT** trust signals and never trigger auto-accept: inferring trust from either is unsound, and discovery is how strangers *reach* you, not whom you auto-trust (consistent with §09's "from a known identity"). The allowlist has **no self-clear path** — the candidate cannot add itself to the evaluating party's allowlist; allowlist membership is set by the evaluator, not the candidate.
- **The allowlist entry alone never auto-accepts: the evaluator MUST read the candidate's `ContinuityStanding` first** (`09-security-model.md` §9.11). It resolves the candidate's key-event log within `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (`09-security-model.md` §9.18.7) and applies §9.11's gate list, which states every standing that withholds an auto-accept; where that resolution fails or the list withholds the act, it MUST route the request to the manual path, surfacing the standing and its reason. This entry names no standing value. **The identifier is what an allowlist entry names and the identifier never changes**, so an entry keeps matching after a `RootRecovery` replaced the root behind it, and the standing check is the gate a rename used to supply. The `09-security-model.md` §9.10.7 privacy cache never satisfies the resolution, whose retention runs to 7 days for an inactive contact.
...
**Bilateral shorthand.** For bilateral templates, the SDK accepts a peer identifier directly and handles the invitation internally. The creator creates the context and immediately sends the invitation. If the peer has an auto-accept policy that matches, the join is automatic. If not, the peer's agent is prompted.
...
InvitationBundle {
  ...
  creator_did:        Identifier          // The context creator / inviter.
...
JoinResponse {
  ...
  joiner_did:       Identifier          // The joining member.
...
**First-contact optimization.** When two agents already share a context (e.g., both are members of a group), creating a standing context between them is faster: both agents already hold each other's resolved key state and MLS key packages, cached from the shared context. The SDK SHOULD use that cached key material rather than re-resolving the peer's key-event log, reducing first-contact setup to a single relay roundtrip.
   ...
   **Eligibility verification by context mode.** The mechanism for verifying parent membership depends on the parent context's mode:
   ...
   - **Encrypted contexts.** The relay cannot read MLS group state, so membership rosters are opaque. Eligibility is proven via a **MembershipAttestation** — a signed statement from a parent context member who holds the `governance` capability:
     ...
     MembershipAttestation {
       ...
       member_did: Identifier,            // The member whose eligibility is attested
       attester_did: Identifier,          // The governance-capable member signing this
     ...
     The child context creation request and membership addition messages include one `MembershipAttestation` per member per encrypted parent. The relay verifies: (1) the attester is a known member of the parent context with `governance` capability (the relay tracks governance-capable members via context metadata updates), (2) the signature is valid, (3) `valid_until > now` (attestation is fresh), (4) `member_did` matches the member being added. Attestations are short-lived (default 1 hour) to limit replay risk.

     **Privacy consideration.** The attestation reveals to the relay that one identity is a member of the parent context. This is acceptable because the relay already knows context membership for routing purposes — the relay receives `SUBSCRIBE` messages per context and tracks which connections are associated with which contexts. The attestation adds no new information the relay doesn't already have.
...
ChildCreationProposal {
    ...
    proposer_did:       Identifier,           // The member who initiated in this parent
...
ScpContextExtension {
    ...
    creator_did:              Identifier,           // The group's genesis creator and admin
...
8. The `creator_did` in the extension MUST match the `creator_did` the joiner is asked to install as the context's creator/admin — the bundle's `creator_did` on a Welcome-based join (§5.12.3), or the snapshot's `role_state.creator_did` on import/restore. A mismatch means the presented creator is not the party the group cryptographically committed at creation; the SDK MUST reject the MLS group **before** installing any authority (governance engine, admin set) for that creator. This binding pins the **genesis** creator (the initial `SingleAdmin` admin); admin *transfers* are not tracked in `0xFF02` — they apply from the authenticated event log after join. This rule is the sole cryptographic anchor of admin identity for governance models that name no identity (e.g. `SingleAdmin`, whose `governance_policy_hash` is a constant and therefore commits no identity): without it, an in-context member could sign an otherwise-valid invitation bundle naming an arbitrary `creator_did` and have the joiner install that party as admin.
...
In parent's event log:
  ChildCreated { child_id, co_parents: [contextID], creator: Identifier, ceiling, config }
...
5. On unblock: author can redistribute the key to the previously blocked identity on its next request.
...
**The subscriber's `author_keys` cache, and the standing check that gates it.** A subscriber holds, per broadcast context, one entry per author it has received a key from: the author's identifier, the broadcast key, and that key's epoch. The entry is what lets a subscriber decrypt an author's content without a fresh key request on every message, and it is the cache `09-security-model.md` §9.11 names among the acts a standing withholds. **A subscriber MUST read that author's `ContinuityStanding` before it accepts a key into this cache and before it decrypts under an entry already in it**, applying §9.11's gate list and naming no standing value of its own, resolving that author's key-event log within `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (`09-security-model.md` §9.18.7) as every other auto-accept gate does; where §9.11's list withholds the act the subscriber surfaces the author's content as unverified and takes no entry. Without that check a subscriber keeps decrypting under a key an attacker distributed while holding a stolen `#active`, for as long as that author publishes no epoch advance.
...
Broadcast contexts reuse the two-tier membership model from contexts with discovery outlets (§6.2.2B [no such section]):

- **Writer tier (authors):** Hold `messages:write` UCAN. Bounded. Manage content and key distribution.
- **Reader tier (subscribers):** identity-authenticated (open) or UCAN-authenticated (gated). Unbounded. Receive author broadcast keys on request.

Subscribers register via identity-signed requests — the same pattern context readers use:

```rust
pub struct SubscriberRegistration {
    pub subscriber_did: Identifier,
...
**Open broadcast** (`public-broadcast` template): The subscriber role's `messages:read` capability is granted on identity-authenticated registration — no admin-issued UCAN required. This mirrors context readers who query via identity-signed requests without UCAN.
...
| Subscriber (open) | No (identity-auth only) | Yes (identifier + wrapping key) | No | Yes |
| Subscriber (gated) | Yes (`messages:read`) | Yes (identifier + wrapping key + UCAN) | No | Yes |

### 5.14.8 Blocking

Author-level, cryptographic, pull-based — the same protocol as encrypted contexts (§9.16.3):

1. Author adds the identity to its block list, increments key epoch.
...
1. **Service-record entry.** Authors MAY publish an `SCPBroadcastContext` entry in their service record with the context ID and relay URLs (`03-identity.md` §3.10.13).
2. **Contexts with discovery outlets.** Authors register broadcast contexts via `agent_register` in contexts with discovery outlets (§6.2.2B [no such section]), with metadata indicating the context mode.
...
1. **Initiator (A) creates the group.** A recomputes `derived_context_id` and validates: (a) it is not already Active under the id (else the *Get-or-create idempotency* path applies — bare existence is not auto-failure); (b) `peer_did` is **distinct from A's own identifier** (a self-pair is rejected with the same generic malformed-peer rejection as any malformed peer identifier, disclosing nothing). **Sybil non-credit (normative, §9.3 cross-ref):** beyond the self-pair guard, a standing pair between **two distinct identities the same operator controls** MUST NOT count toward §9.3 earned-capacity participation records — otherwise a Sybil operator could self-deal two of its own identities into participation credit. This is the same **not-self-created** discriminator §9.3 applies to participation records (a participation record only counts from a context the counting identity did not itself create/admin); a two-party context both of whose members the operator controls is self-dealt and does not earn capacity for either. (c) the `bilateral-persistent` template params are well-formed (§5.12.1); (d) `peer_did` resolves to a key state listing a key `Current` in the `#active` role (`03-identity.md` §3.10.4) and is not blocked; (e) A's provider holds no group under the id (the `Entry::Vacant` guard; collision ⇒ error). On success A creates a **1-leaf** MLS group plus a fresh sender key.
...
4. **Peer (B) receives the Welcome and applies the consent gate on receipt** (*before* joining): (a-1) **key-continuity standing** — B MUST resolve A's key-event log within `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (`09-security-model.md` §9.18.7) and read A's `ContinuityStanding` (`09-security-model.md` §9.11). Where that resolution fails, or §9.11's gate list withholds the auto-accept, B routes the request to the manual path and surfaces the standing and its reason; this step names no standing value of its own. A stable identifier keeps matching whatever the standing, which is why this step precedes the checks below rather than following them. Then (a0) **`derived_context_id` agreement (cross-party canonicalization check, defense-in-depth)** — B MUST **re-derive** the standing-pair `derived_context_id` from its **own** inputs (`local_id = B`, `peer_id = A`, ordered and length-prefixed per *Determinism precondition*) and verify it **equals** the context id the inbound InvitationBundle / Welcome binds. A mismatch ⇒ **reject the Welcome (do NOT join)**, surfacing the same generic rejection as any other consent reject (no synchronous "Rejected" reply, per *Block-privacy* below). Two honest parties cannot derive two ids for one pair, because an identifier is a fixed-length digest that admits one encoding (`03-identity.md` §3.8.1), so a mismatch means the bundle's asserted `context_id` is not the pair's. It is **not** a cryptographic cross-party agreement proof — `InvitationBundle.context_id` (§5.12.3.1) is a creator-asserted, creator-signed label, not a value bound to the identifier pair, so a *malicious* A can always label the bundle with the id B will derive. Agreement against a dishonest creator on the collision path rests on the §9.7.1 bound-creator check and MLS membership binding (below), not on this equality test. **Non-mismatch resolution outcomes (a0).** If B cannot resolve `creator_did` (A's identifier) at all — distinct from an id *mismatch* — a resolution failure is a **retryable deferral**, NOT a permanent reject: B re-attempts within the `welcome_ttl` window rather than discarding the Welcome. That path is never a silent join, because a still-pending deferral does not join. (a) **block list** — refuse if A is globally blocked (§3.7.1 `is_globally_blocked`); (b) **default-deny for strangers (allowlist-or-prompt)** — a standing-pair Welcome from a **stranger** is **default-deny** (non-overridable, carrying the **same non-overridable intent** as §5.12.2's outlet-bearing rule: a conformant SDK MUST NOT let an `AutoAcceptPolicy` override the deny), justified by the pair's `memory_scope: full` sensitivity (§5.12.1). It MAY be auto-joined **ONLY** if A is on the operator's `known_did` allowlist (§5.12.2 — the sole auto-accept trigger); otherwise B joins **only after explicit human approval**. This default-deny is a **MUST on conformant implementations at the SDK consent-gate layer**, not protocol-layer enforcement. If B's policy configures an `AutoAcceptPolicy` whose `known_did` allowlist contains A, B MAY auto-join; else B joins only after explicit approval. The gate is applied by the **joining peer on Welcome receipt**, never as a synchronous Prepare-B reply: on accept B joins (single-use enforced at join), registers Active, appends, and runs `register_standing_context`; on reject B simply **never joins** — no synchronous "Rejected" reply.

#### Threat model and operational contracts

**Block-privacy (consent-on-receipt).** Because consent is applied by the *joining* peer asynchronously, a blocked or unapproved initiator gets **no synchronous rejection**, foreclosing the **synchronous** block/pair-existence reply oracle a two-phase `Rejected` would leak. (The *Ok-return contract* below states that a decline is indistinguishable from offline, slow and blocking, which is the identical-`Ok` oracle closure.) **Scope (precise):** this closes the *reply* oracle only. A's step-2 fetch of B's **published** `KeyPackage` runs *before* the gate and is relay-observable, so a network observer can distinguish "B has a published KeyPackage" from a peer that never published one; the async gate does not claim to hide that. That published-KeyPackage bit becomes a *targeting* primitive only when chained with a stranger-bar bypass, which the **step-4(b) allowlist-or-prompt default-deny blunts (a candidate cannot self-clear an operator-set allowlist)**.

**Known limitation — wire-observable solicitation.** The "private contact graph" claim (§5.12.6) scopes to A's **local bookkeeping**. The standing-pair Welcome rides B's *publicly-computable* invitations routing id **unconditionally, before** the consent gate, so first-contact solicitation metadata and addressee identifiers are **observable to relays**. This is inherent to contacting a peer over an untrusted relay; consent-on-receipt closes only the synchronous *reply* arm, **not** solicitation visibility. The message *content* remains MLS-encrypted.
...
- **(a) Decline is indistinguishable from offline** — no synchronous `Rejected`, no peer-state probe; A observes only "B has not joined yet" (the *Ok-return contract* below states the identical-`Ok` closure across offline, slow, blocking and declining).
...
**Anti-spam rate limit (normative).** B MUST rate-limit *inbound* standing-pair Welcomes **per initiator identity** on the consent gate — an ordinary per-peer cooldown, default **60 s**, hard floor **1 s** (a near-zero cooldown is non-conformant); operators surfacing approval prompts as interrupts SHOULD prefer the default. **Scope carve-out (gate-decidable).** The cooldown is evaluated at the consent gate using **only locally-available state**: a Welcome under a `derived_context_id` for which this node **already holds its own self-created group** is a convergence candidate and is **exempt** from the per-initiator cooldown (its authenticity is settled downstream by confirm-bound-creator + init-key single-use, which a forged variant fails before consuming an init key or destroying anything). **Disclosed cost of the exemption (honest):** a forged convergence-candidate Welcome under such an id is not free — confirm-bound-creator still performs one **identity resolution and one signature verification** before rejecting it, and that work is **not** rate-limited by this cooldown. This is bounded because the exemption's precondition is that the victim **already holds a self-created group under that exact id**, which requires the victim to have itself initiated the pair — so an attacker cannot manufacture the precondition on an arbitrary victim (no self-created group under the id ⇒ the Welcome is a stranger/approval Welcome and IS cooldowned). **The precondition is not exotic, however (honest amplifier disclosure):** it is satisfied for **every real standing pair the victim has ever initiated**, and the `derived_context_id` is **publicly computable** from the two participant identifiers — so **any** party (not only the actual peer) who knows both identifiers can forge convergence-candidate Welcomes under such a known id, forcing the **un-throttled** confirm-bound-creator resolution and signature-verification work for each. It remains a **bounded** DoS — exactly one identity-resolve + one signature-verify per Welcome, with **no amplification beyond that** (no join, no state change, no fan-out) — but the cost is real and is stated honestly rather than implying the precondition is hard to reach. **Resolution is a NETWORK operation, not merely local CPU:** resolving `creator_did` replays that identity's key-event log over the relay network (`03-identity.md` §3.10.4), so an off-path party who knows both identifiers (the id is publicly computable) can force the victim into **outbound resolution traffic against a third party's relays** — a bounded **reflected-resolution** vector, one resolution per forged Welcome (still no join / state-change / fan-out). All other inbound standing-pair Welcomes (no pre-existing self-created group under the id) are stranger/approval-prompt Welcomes and **ARE** subject to the per-initiator cooldown. Convergence is thus never gated on the inbound cooldown, and the carve-out needs no post-gate creator-binding to decide. This is not saga concurrency machinery. **Residual (honest, per §9.3):** the per-identity cooldown does not bound a **fresh-identity fleet** (many identities, one Welcome each). That fleet is an **approval-prompt-spam DoS, not an unauthorized-join flood**: the step-4(b) default-deny means each of N fresh strangers still requires explicit out-of-band approval, so the fleet yields at most N approval prompts, never N silent joins — bounded by the §9.3 per-identity minting cost each identity pays to become admissible. §9.3 defines no recipient-side inbound tier check, so this spec claims none and authors **no** new standalone limiter. There is **no** standing-pair-specific KeyPackage reservation: single-use is enforced at join (step 2); a stranger-add draining A's KeyPackage pool is the **general MLS KeyPackage-pool concern**, bounded by republication and §9.3.
