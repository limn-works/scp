...
**Signing key and verification.** The operator signs with the Ed25519 identity key that its `operator_did` encodes — the key a verifier recovers from the DID string itself, not a key the registrant supplies alongside the registration. A verifier therefore checks a registration with no resolver, no relay, and no network: it decodes `operator_did` to 32 key bytes and verifies the signature over the digest above. The §7.3.1 event log binds an event's signature to its `actor_did` the same way, so a reader applies one rule to both records. Binding the key to the DID string is what makes the signature establish the operator: a registration cannot name one operator and carry another party's signature.

**A registration MUST carry a signature.** `signature` is a required field, and an empty `signature` fails verification. There is no legacy sentinel value that skips the check — pre-migration signatures are not honored, so a registration that carries none names an operator the protocol has not verified. A context's outlet registry MUST reject such a registration rather than store it. The registry MUST run the same check when it replaces a stored registration, because a replacement carries its own registration body and so its own signature; without the check on the replacement path an admin could rewrite the `operator_did` of an outlet the named operator never signed for.

**Who signs, and who registers.** The registrant and the operator are separate parties: the registration event records `registrant_did` alongside `operator_did`. An SDK registering an outlet whose operator key it holds signs the digest itself. An SDK registering an outlet operated by another party carries that operator's 64 signature bytes, which the operator produced out of band, together with the `registered_at` value the operator signed over. An SDK that can do neither MUST fail the registration with a typed error; it MUST NOT substitute its own key for the operator's, and it MUST NOT write an empty signature.
