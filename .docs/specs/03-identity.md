...
| `publicKeyMultibase` multicodec fix (§18.2.2A) | Phase 2 (#2297) | `scp-did` | `multibase_encode` (`crates/scp-did/src/document.rs:1264`) omits the `0xed 0x01` header the `Ed25519VerificationKey2020` suite requires. Changes the signed relay-layer octets, so it lands with the canonicalization above. |
