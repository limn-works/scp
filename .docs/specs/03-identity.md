...
**D18 — ADR-039 states that identity creation publishes a custody attestation, and no shipped path writes one into a DID document.** ADR-039's Enforcement Stack layer 4 opens "At identity creation, the DID document includes a `ScpKeyCustodyAttestation` service entry" (`.docs/adrs/phase-1.md:1302`). The two functions that place the entry in a document have no caller outside tests: `ScpKeyCustodyAttestation::derive` and `DidDocument::set_custody_attestation` (`crates/scp-did/src/document.rs:671`) are reached only from `#[cfg(test)]` modules and from `crates/scp-runtime/tests/agent_binding_integration.rs`. §27.3.4 of the attestations spec records the same fact as contradiction C30 and lists every caller. The two cannot both hold, because a document either carries the entry or it does not.

The bridges' `identity_published_custody` operation is what a caller reaches for the published value: it reads the two facts off the backend running in that process, returns the value that backend would publish, and writes nothing (`crates/scp-ffi/src/identity.rs`; `crates/scp-ffi/napi/src/scp.rs`; `crates/scp-ffi/uniffi/src/bridge.rs`). It therefore answers for an identity the calling instance created and returns `SCP-IDENT-1001` for every other DID. A reader who resolves a stranger's DID document reads ADR-039's layer-4 absence signal, "Absence of attestation is itself a signal", and cannot distinguish that participant from one whose backend reports a pair the published vocabulary states no value for. Open question OQ-17 below asks which component writes the entry.
...
**OQ-17 — Which component writes the `ScpKeyCustodyAttestation` service entry into a DID document, and at what point?** *Undefined:* ADR-039's Enforcement Stack layer 4 states that identity creation publishes the entry, and `ScpKeyCustodyAttestation::derive` and `DidDocument::set_custody_attestation` have no caller outside tests (§3.2.2.1, divergence D18). `DidMethod::create` builds the document every creation path publishes and takes a `KeyCustody`, which exposes no `CustodySubstrate`; the three bridges hold the substrate instead, one per key handle, and expose it only through `identity_published_custody`. *Should define it:* ADR-039, the shared-DID human-agent identity model, naming the component that derives the entry and the point in creation at which it writes one, together with ADR-006, the platform-adapter decision, stating whether `KeyCustody` reports the substrate for a key handle. *Breaks meanwhile:* every DID a shipped SDK publishes carries no custody entry, so ADR-039's layer-4 custody signal is absent for every identity, and a participant on a non-extractable store cannot tell a reader that their key cannot leave it.
