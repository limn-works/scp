...
- **One layer fails, the other reports the DID absent.** The resolver returns an absent result that carries each layer's status: which layer answered, and which layer could not answer. The resolver MUST NOT return a bare not-found here. A caller that reads a bare not-found cannot tell a DID that nobody published from a DID whose only holder was unreachable, so the caller would treat a suppressed layer as proof of absence. The criterion a caller applies: a layer *answered* when a source in that layer gave the resolver usable evidence about this DID — either a record that verified against the DID-derived key and carried a sequence number at or above the last one the resolver cached, or an affirmative report from a source the resolver reached that the source holds no record for this DID. A layer *could not answer* in every other case, because the resolver learned nothing about the DID from it.

The following are evidence that a layer gave no usable answer, not the criterion itself; a reader decides membership by the criterion above and uses this list to recognize the cases it admits: every source in the layer errored or timed out; no source in the layer had a live connection; the layer is switched off, so it queried no source; every record the layer's sources served failed BEP44 signature verification, failed self-certification, failed to decode as a DID-record frame (§9.10.12), or carried a sequence number below the last one the resolver cached. The three bullets below — invalid signature, failed frame decoding, and stale sequence number — each state one of those cases, and each requires the same outcome the criterion gives it.

A layer that returns `Answered` when no source answered is the failure this criterion exists to prevent: two `Answered` layers make an absent result mean "nobody published this DID", so a layer that claims to have answered without asking anyone hands whoever suppressed that layer a proof of absence.
...
- **A layer returns a sequence number below the cached one.** The record is discarded as if the layer had failed (step 4c). A replayed record is an attempt to reinstate a superseded document, so the layer that served it reported nothing about the DID's current state, and the resolver MUST NOT read the discard as that layer holding no record.
- **A relay serves blobs of which none survives.** The relay reported nothing about the DID, so it did not answer. A relay reports that it holds no record for a DID only by returning no blob at all at the routing ID. Without this rule, anyone able to write one junk blob to a DID's routing ID converts a suppression attack into a caller-visible claim that nobody published that DID.
- **A layer is switched off.** A layer whose sources are all disabled — a node configured with the DHT layer off (§3.10.3), a client with no relay transport bound (§3.10.2) — queried no source, so it did not answer. It MUST NOT report that it holds no record for the DID.
