...
4. **The declared storage budgets.** **No event kind is exempt from any scope.** The relay evicts or refuses under `Identifier` and refuses under `Total` as `09-security-model.md` §9.7.4.2 R9 states, and it reads that section for what a declared exemption stops and for how long. The check runs here because every term it reads is one the relay already holds.
5. **`PaymentAdapter::verify`, its three comparisons, and recording the write as paid.** Where the relay declares `per_publish`, call the adapter the receipt is tagged with (`19-economic-governance.md` §19.2.1), passing it the receipt together with this relay's declared currency and the price it computed. Read the three answers it returns: the rail-bound receipt identifier against the set of identifiers this relay has already spent, `amount_covers`, and `currency_matches`. The relay compares no verified amount and no verified currency of its own, because the call performs both comparisons against the currency and the price this relay passed in. Record the write as paid. Refuse `Payment` where the write is charged and the receipt is absent, where the call returns invalid, where any of the three answers fails, or where the receipt's adapter tag names no member of this relay's declared `economic.payment_adapters`. **Every one of the three reads a term the call returned and none reads a field of the receipt**, because a presenting party composes every field of the receipt. **This is the one check that leaves the relay's host**, which is why every check above it runs first. The relay spends the receipt identifier at check 6, where it places the frame's events on a chain, and at no earlier check, so a frame it rejects costs the publisher no receipt.
...
**A PUBLISH at `scp:svc:`, `scp:wit:` or `scp:wcf:` runs the same six checks in the same order, with that kind's own acceptance test standing where chain verification and placement stand at check 6, and the relay spends the receipt identifier there.** The two witness kinds carry no payment check at all: **a relay charges no `scp:wit:` and no `scp:wcf:` write** (`09-security-model.md` §9.7.4.3), because the witness filter below already proves the write is one a designated witness made about a subject whose chain this relay holds.
...
**Publishing once does not leave an entry current.** `09-security-model.md` §9.7.4.2 R10 states the cadence at which the SDK re-reads its own identity's log from every entry of its fallback set, and states the one publication duty that cadence carries, so an implementer of this section wires that cadence rather than treating a confirmed publish cycle as the end of the obligation.
...
|---|---|---|
| `Payment` | The relay's declared `economic` terms — the currency, `per_publish`, `per_byte_stored`, the accepted adapters and the payee — together with that entry's three rent terms | Mint a receipt for the declared price through a configured adapter and retry at the same relay (`09-security-model.md` §9.7.4.2 R9) |
| `RateLimit` | The declared ceiling per minute and the seconds to wait | Wait the stated seconds and retry at the same relay (`09-security-model.md` §9.7.4.2 R9) |
| `Identifier` | The declared per-identifier budget in bytes | **Where the entry's `receipt_exempts` names `Identifier`**, read that entry's rent state through `read_rent_state` and its rent terms through `read_policy`, pay on ADR-004's `RENT` operation a `units` at least the ceiling of `RentState.held_bytes` plus the refused frame's bytes divided by the declared `rent.quota_bytes`, and retry at the same entry. **The remedy reaches a controller whose chain that entry already holds**, because `09-security-model.md` §9.7.4.2 R9 refuses a `RENT` naming an identifier the relay holds neither a chain nor a rent record for, so a publisher refused on the first frame it ever sent to that entry moves to the next fallback-set entry instead. **Where the set does not name `Identifier`**, move to the next fallback-set entry and record this one under the per-entry result below |
...
pub trait IdentityBackend: Send + Sync {
    ...
    /// rent exists for — holds no `PublishOutcome` to mine one out of.
...
pub enum IdentityError {
    ...
    /// SCP-IDENT-1109. This identity's `IdentityConfig` carries no `payment`
    /// slot, so `pay_rent` can mint no receipt
...
pub struct DeclaredWritePolicy {
    ...
    /// or more members than one**, because that is the one member a relay may
    /// declare, and records that entry as `Skipped`.
