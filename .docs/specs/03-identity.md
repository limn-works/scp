...
2. AUTHORIZE on device holding #0:
   ...
   b. Construct updated DID document:
      ...
      - Advance each layer's own sequence number (§3.10.5) — Mainline's is a Unix timestamp, the relay layer's a publish counter.
...
**The precedence costs latency, and §3.10.4 bounds the cost.** A resolver holding a Mainline answer waits for a relay answer before it settles, so a resolution that only Mainline can satisfy costs Mainline latency **plus** that wait — not Mainline latency alone. §3.10.4's settle rule caps the wait at 2 seconds, and the resolver settles earlier if every relay query has already failed. Under §3.10.11's day-one conditions, where relay queries mostly fail, that wait is paid on most resolutions; the alternative is settling on the layer that carries less, which §3.10.7 rule 1 rejects. A relay answer arriving after the resolver settles does not supersede retroactively (§3.10.7 rule 1).
...
- **Settle rule (replaces the earlier first-response optimization).** When Mainline answers first, the resolver continues waiting for a relay answer, whatever the caller asked for, because a relay answer supersedes Mainline's for every element (step 5d) and cancelling on Mainline's answer would settle on the layer that carries less. The resolver settles at whichever of these comes first: a relay answers; every relay query has failed or timed out; or **2 seconds** elapse. The 2-second grace is shorter than the 5-second per-layer timeout below, so a relay that answers between 2 and 5 seconds is inside its own budget and outside the grace — the resolver has already settled on Mainline, and step 5d's precedence does not reach that answer. This is a deliberate latency bound, not an oversight: waiting the full 5 seconds would put that cost on every Mainline-only resolution, which §3.10.11 says is most of them on day one. The resolver MAY use a late relay answer to update its cache, so the next resolution holds it. When a relay answers first, the resolver MAY cancel the Mainline query, because the relay layer already carries everything Mainline would have supplied.
...
**What that costs, stated plainly.** Under the rule this section replaces, a resolver that reached both layers could reject a stale Mainline record because a relay had returned a higher number. That specific correction is gone: within the Mainline layer a stale-but-genuine record is the highest number that layer holds, so a sequence comparison inside that layer gives the resolver no ground to reject it. Two things limit what the loss can do. Mainline carries the bootstrap core alone (§18.2.2C), so a stale record can return a rotated-out `#active` key and a stale relay list and nothing else — never a stale capability entry, attestation entry or `alsoKnownAs`, because the layer never carries those. And §3.10.10's two `ResolvedDidDocument` variants tell a caller which layer answered, so a caller cannot mistake a core for a full document.

**Two rules bound the Mainline staleness that remains, and neither is the deleted cross-layer comparison.** Neither compares a sequence number across layers, so both are available under the rule above.

1. **The relay layer's answer supersedes Mainline's for every element, among the answers the resolver holds when it settles.** The relay layer carries a superset of the core (§18.2.2C), so when both layers return a valid record the resolver takes the relay record and uses the Mainline record for nothing. This is a fixed precedence between layers, not a comparison of their numbers. It closes the case §3.10.1 would otherwise leave open, where a caller that needs only a core element takes whichever layer answered first and can take a stale Mainline copy while a relay record is in hand. **The precedence chooses the layer that carries more, not the layer that is fresher** — see the asymmetry below, which is why it is stated as completeness and not as freshness.

   **The precedence is not retroactive.** §3.10.4's settle rule bounds how long the resolver waits, so a relay answer that arrives after the resolver has settled on Mainline does not supersede the answer already returned. The resolver MAY use the late answer to update its cache, and the next resolution then holds it.
...
**The two layers' staleness bounds are asymmetric, and the asymmetry runs against the layer the precedence prefers.** Stating it plainly, because rule 1 sends a verifier to the relay layer and rule 2 bounds only the other one:

| | Relay layer | Mainline |
|---|---|---|
| Sequence number | A publish counter — carries no time | A Unix timestamp (§18.2.2C) |
| Wall-clock freshness check | **None available**, because a counter cannot be compared to a clock | Rule 2, at 2 h 30 m |
| Republish cycle | 6 days | 2 hours |
| Hard bound on a served record's age | 7 days — the `blob_ttl` of 604800 seconds (§3.10.2), after which a relay's blob expires and the relay serves nothing | 2 h 30 m by rule 2 |
| Reduction across sources | Highest-seq valid across **all** of the identity's relays (§3.10.4 step 5a), so one lagging relay cannot win | One slot per key; no equivalent |

So a 5-day-old relay record carrying a rotated-out `#active` beats a 10-minute-old Mainline core carrying the current one. Two things keep that from being a hole rather than a cost. The relay layer's own bound is the 7-day blob TTL, not "unbounded" — a relay physically cannot serve a record older than that. And the multi-relay selection means the stale record only wins if **every** relay the identity published to is lagging, which is the same condition that would make the identity's own republishing broken.

**Neither bound substitutes for a fresh resolution when the decision is about current key material.** §9.7.1 requires the document backing an attestation check to be no older than `MAX_ATTESTATION_KEY_RESOLUTION_STALENESS` (300 seconds), and that bounds when the resolver **fetched**, not how old the served record is. A verifier making such a decision therefore re-queries rather than reading either layer's cache, and what bounds the rotation's propagation is the republish cycle of the layer that answers.
...
- **Relay serves stale document.** Detected by comparing the record's sequence number against the last number observed **on the relay layer** (§3.10.7). The resolver falls through to the identity's other relays. It does not fall through to Mainline for the full document, because Mainline does not carry one — what Mainline supplies is the bootstrap core, and §3.10.4 step 5d makes any relay answer supersede it. Stale relay records delay propagation of a key rotation, bounded by the 6-day relay republish cycle; a stale Mainline core is bounded instead by §3.10.4 step 5e's 2-hour-30-minute wall-clock check against its timestamp sequence number.
- **Relay unresponsive, slow, or withholding.** Resolution queries all of an identity's relay URLs plus Mainline concurrently, each relay guarded by an independent per-relay timeout (§3.10.4). A single slow, hung, or withholding relay cannot block a result obtained from a faster relay, and suppression by any one relay does not prevent resolution — multi-relay publishing (§9.9.2) applies to DID documents as it does to context blobs. Mainline is not a substitute for a withholding relay on the full document; it answers with the bootstrap core, which is what lets the resolver reach a relay the attacker did not suppress.
...
- **Suppression resilience (validating SCP-native relays).** With single-slot validation, an attacker cannot evict the genuine record by flooding, and cannot reorder it out of a bounded QUERY window (there is at most one record to return). To prevent resolution **of the full document**, an attacker must suppress it on ALL of an identity's validating relays — and that alone suffices, because Mainline does not carry the full document (§18.2.2C), so a surviving Mainline record cannot substitute for a suppressed relay record. What Mainline independently resists suppression *for* is the **bootstrap core**, for the same structural reason relays do (BEP44 nodes validate on write and keep one highest-`seq` slot per key), and preventing core resolution requires suppressing it on all reachable Mainline nodes. So the conjunction is not "all relays AND the DHT" over one document; it is two separate suppression targets, each with its own bar. The practical effect of Mainline's survival against a relay-side attack is that the resolver keeps the **relay list**, from which it reaches a relay the attacker did not suppress — which is why suppressing *some* relays does not work, and suppressing all of them does. **On integrity:** relay misbehavior is availability-only, never integrity, *because* a set of controls hold together, each covering a distinct failure mode — the client's BEP44 re-verification against the DID-derived key (§9.6.1) rejects a **forged** record; the resolver's per-layer `seq`-monotonicity plus its highest-`seq`-across-relays selection (§3.10.4 step 5a, §3.10.7) rejects a **stale/replayed genuine** relay record on any warm-cache or multi-relay resolution; and the storage-derived, cold-index-immune establish reconciliation + **DELETE gate (rule (d))** close the **cold-cache DELETE-purge-then-replay rollback** that the first two do not cover on a single-source first contact. Mainline does not contribute to that relay-layer selection, because it carries the bootstrap core rather than the full document (§3.10); what Mainline contributes against relay-layer suppression is a second, independently-suppression-resistant copy of the relay list, from which the resolver reaches a relay the attacker did not suppress. It is not an unconditional property of the relay: it is delivered by those controls together. A relay that omits them (a foreign/non-validating relay) provides no integrity control of its own — integrity there rests entirely on the client-side checks (§9.6.1 + seq-monotonicity + DHT).
...
| Parallel dual-layer resolution | Phase 2 | `scp-core` | Orchestration of parallel queries with per-layer selection: the relay answer supersedes Mainline's whenever a relay answers, and no sequence number is compared across layers (§3.10.1, §3.10.4 step 5). |
...
| DID-string format fix (§18.2.2A `id` row) | Phase 2 (#2297) | `scp-did`, fixtures | `did_dht_from_public_key` (`crates/scp-did/src/lib.rs:173`) formats `did:dht:z{}`, emitting a 53-character suffix where the method's transformation yields 52. Fix the literal and its `strip_prefix`, ADR-046's parity regex, ADR-057's prefix gate, and the pasted fixtures in §25. Nothing is live; no migration. |
| `publicKeyMultibase` multicodec fix (§18.2.2A) | Phase 2 (#2297) | `scp-did` | `multibase_encode` (`crates/scp-did/src/document.rs:1213`) omits the `0xed 0x01` header the `Ed25519VerificationKey2020` suite requires. Changes the signed relay-layer octets, so it lands with the canonicalization above. |
| `capabilityDelegation` and `capabilityInvocation` fields (§18.2.2A) | Phase 2 | `scp-did` | `DidDocument` (`crates/scp-did/src/document.rs:168`) declares no field for either array, and §18.2.2A marks both Required: Yes, so every serialized document omits both. No file under `crates/` writes either name. Adding the fields changes the signed relay-layer octets, so this lands with the canonicalization above. No issue records this obligation. |
| Service-entry `id` and `type` conformance (§18.2.2A) | Phase 2 | `scp-did`, `scp-runtime`, `scp-protocol` | Every service-entry constructor formats the document's own `id` in front of the fragment where §18.2.2A's `service[].id` row requires the fragment alone, and three type strings — `ScpDeviceAttestation`, `ScpParticipationStatements`, `ScpKeyCustodyAttestation` — name no row of §18.2.2's table. Changes the signed relay-layer octets, so this lands with the canonicalization above. No issue records this obligation. |
| `SCPRelay` entry on the creation path (§18.2.2A `service` row) | Phase 2 | `scp-did`, `scp-identity` | `DidDocument::new_with_agent_key` (`crates/scp-did/src/document.rs:330`) builds `service` holding the `PreRotationCommitment` entry alone, and `DidDht::publish_document` (`crates/scp-identity/src/dht.rs:830`) publishes the document it receives without counting relay entries, so the shipped creation path emits a document carrying zero of the at-least-one entries the row requires. No issue records this obligation. |
| `ResolvedDidDocument` two-variant enum (§3.10.10) | Phase 2 (#2297) | `scp-identity` | Replaces the `DidDocument`-plus-flag struct at `crates/scp-identity/src/resolver.rs:69`. This is the mechanism every "unresolved rather than absent" rule depends on. ADR-062 §Decision 5 no longer conditions `NoOpRelayQuerier`'s classification on it: Alec classified that arm a nullifier on 2026-08-17 and ordered the real resolution wired. |
| `NoOpRelayQuerier` sever (ADR-062 §Decision 5) | Phase 2 | `scp-identity`, `scp-ffi`, `scp-node` | Pull request #2378 composes `RealMultiRelayQuerier<TransportRelayQuerier>` at every construction site and deletes `NoOpRelayQuerier`, so no production path constructs `DualLayerResolver` without a real querier. If that pull request does not land, ADR-062's Slice 11 severs the arm itself: production construction returns a typed error and the arm moves behind a testing cfg. Shipped today from `crates/scp-ffi/src/identity.rs:127` (PyO3), `crates/scp-ffi/napi/src/identity.rs:161` (napi-rs) and `crates/scp-node/src/self_host.rs:1405`; the UniFFI construction at `crates/scp-ffi/uniffi/src/bridge.rs:9430` is `#[cfg(feature = "testing")]`. |
| `RepublishEntry` per-layer split (§3.10.5) | Phase 2 (#2297) | `scp-identity` | `republish.rs`'s entry carries one `signature`/`sequence`/`document_bytes` and feeds both layer loops; §3.10.5 signs each layer over different bytes, so the entry gains per-layer fields. |
| `ContextDiscoverySource` rename (§22) | Phase 2 (#2297) | `scp-runtime`, all three FFI bridges, TS SDK | `DhtDidDocument` → `DidDocumentBroadcastList`, wire string `"dht_did_document"` → `"did_document_broadcast_list"`. The old name asserts a layer that never serves the `SCPBroadcastContext` entry. |
| `PkarrDhtClient` rename (ADR-003 amendment) | Phase 2 | `scp-dht` | Type, module `pkarr_client`, builder, and the `pkarr` keyword at `crates/scp-dht/Cargo.toml:10`. The name asserts a dependency that was never usable. Also raises the `mainline` requirement to 6.1.1. |
...
1. **DID resolution, in two parts.**
   ...
   b. *An `#agent`-signed response needs one more fetch, and that fetch is not an HTTPS GET of JSON.* `#agent` lives on SCP's relay layer, not on Mainline. Take a relay URL from the core's `SCPRelay` entries: §18.2.1 of `.docs/specs/18-addressability-and-deployment.md`, the `SCPRelay` service endpoint, fixes it as the SCP relay WebSocket endpoint `wss://<host>/scp/v1`, which a self-hosted relay resolved from a DID document MAY serve over `ws://` at an IP literal, so reach it with the relay's QUERY operation (ADR-004) and not with an HTTPS request. Address the query at `did_routing_id = SHA-256("scp:did:" || did_string)` (§3.10.2), and send `limit: N` with N = 16. A relay that does not validate what it stores keeps every blob anyone published at that routing ID rather than one slot, so a query there returns up to N blobs and the first blob a relay hands back is not necessarily the record you want (§3.10.2). Four steps turn what a relay returns into a document, and a relying party runs steps i and ii over **every** blob the query returned:
      ...
      iii. *Take the highest-sequence survivor, and check it against what you already hold.* Among the blobs that survived steps i and ii, across every relay you queried, take the one carrying the highest `seq` and discard the rest — §3.10.4 step 5a requires that selection whenever a relay returns more than one valid record, and a relying party that stops at the first decodable blob accepts a stale or attacker-planted record while a higher-sequence genuine one sits beside it. Then accept that `seq` only when it is at least the highest `seq` you have already accepted for this DID on the relay layer, and never compare it against a Mainline sequence number (§3.10.7).

      iv. *Parse the value.* `value` is the full DID document as canonical JSON (§18.2.2A), and a JSON parser reads it.

   **The core's silence about `#agent` is not evidence that the identity has no agent key** — a relying party that treats it that way rejects valid authentications (§3.10.10). If no relay answers, or no blob any relay returned survives steps i through iii, fail the verification rather than concluding anything.
...
No SCP SDK, no MLS, no context management. For an `#active`-signed response the entire verification path is: one Mainline lookup + one did:dht DNS-packet decode + one SHA-256 + one Ed25519 verify. An `#agent`-signed response adds one relay QUERY over a WebSocket, then a DID-record frame decode and a BEP44 signature verify for each blob that query returned — up to the 16 a non-validating relay may hold — and one JSON parse of the highest-sequence survivor.
