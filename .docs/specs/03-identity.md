...
2. **Root key change (rare).** If the root must change, the root changes by a `RootRecovery` event (`09-security-model.md` §9.7.4.2 R3): the standing pre-rotation key authorizes the event, a fresh root generated in operational custody is installed, and the identifier does not change — no new DID, no `alsoKnownAs` forwarding record, and no `DidRotationEvent`. Relying parties re-verify key continuity (§9.11) on the recovery event. A planned root move with no compromise — a custody substrate being decommissioned with its key unexportable — is the same event: a `RootRecovery{CoSigns}` whose snapshot lists every key current or `Superseded` and asserts no compromise (`09-security-model.md` §9.7.4.2 R10).
