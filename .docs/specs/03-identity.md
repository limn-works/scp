...
**Resolution delivers a key state derived from a chain the resolver verified itself.** The resolver recomputes the identifier from the chain's inception event, verifies every later event under the standing root (`09-security-model.md` §9.6.1, §9.7.4.2 R2 and R3), settles any divergence under the root rule (§9.7.4.2 R6), and derives the key state from the latest state-carrying event at or before the head it adopted (§9.7.4.2 R8). **No witness cosignature gates any of those steps**; what a cosignature supplies is one of the two corroboration sources §9.11's lapse standing reads, the other being the resolver's own two-relay read of the adopted head (`09-security-model.md` §9.7.4.2 definitions, §9.7.4.3). Every relay is untrusted. Trust derives from the cryptographic binding between the identifier and the inception event that produced it, not from the infrastructure serving it. An SCP-native relay MAY additionally validate the records it stores (§3.10.2) — a validating relay verifies the chain and keeps one slot per divergent suffix, which resists suppression — but this is an availability property layered on top, never a trust dependency: the resolver re-verifies every record independently and accepts nothing on the relay's word (§3.10.4).
...
4. Submit the new head to every relay the identity's own key state
   ...
   the corroboration §9.11's lapse standing reads. **An identity that
   designates no witnesses skips this step and loses no standing**, because
   step 3 already put its head on the fallback set and a peer corroborates
   from its own read of two of those relays instead (09 §9.7.4.2
   definitions).
...
**The invariant reaches the witness set differently, and this states the difference rather than restating the rule.** An identity that publishes its chain everywhere and submits it to no witness is still resolvable by every stranger and keeps its §9.11 standing at every peer that can read its head from two fallback-set relays under distinct declared operators, because that read is the second corroboration source (`09-security-model.md` §9.7.4.2 definitions). What it loses is the portable half: a cosigned head travels to a party that did not perform the read, and a peer's own relay read does not. A publish cycle that reached no witness MUST therefore be reported to the caller as a **degraded** publication naming that narrower consequence, and never as a failure. **A publish cycle that reached no relay in the fallback set is the failure**, because it is the one that leaves a peer with no corroboration source at all. RepublishManager publishes to both relay sets on every cycle (§3.10.5), and this section adds no mechanism of its own. A publish cycle that reached no relay in the fallback set MUST be reported to the caller as a failed publication, never as a success: an SDK that reported success would leave the controller believing its chain is resolvable by a stranger when no stranger can reach it, which is the segmentation this invariant forbids.
