...
**Relay-side validation.** `09-security-model.md` §9.7.4.2 R9 states what a relay keeps for one identity, what a chain holds, the write rule, the serving order and the ring buffer. On PUBLISH at a routing id an SCP-native relay runs seven checks **in the order below, which is the order and not an enumeration**, cheapest first, so junk is rejected before any expensive work. **This section states the number of checks once, and every other artifact that names the order takes the number by citation to this section.** The three policy checks lead because each reads a field the relay already holds and decodes nothing: a relay that ran chain verification first would perform one P-256 verification per signature slot on a frame the sixth check rejects for free:
