...
**What it signs.** §9.7.4 of the security spec lists the operations: "Used for MLS KeyPackage attestations (§9.7.1), inner-envelope signatures, UCAN issuance. It does NOT sign the MLS leaf/credential directly — the leaf is self-signed by the ephemeral MLS leaf signature key … and the DID↔leaf binding is carried by a `#active`/`#agent`-signed KeyPackage attestation" (`.docs/specs/09-security-model.md:702`). ADR-039 assigns root UCAN issuance to `#active` alone and gives the reason: "Root UCANs are the origin of all delegation chains; an agent that can issue root UCANs can grant itself arbitrary capabilities" (`.docs/adrs/phase-1.md:1283`). §3.11 of this spec adds DID authentication challenges, which `#active` or `#agent` signs (`.docs/specs/03-identity.md:174`).
...
This section therefore states no fixed custody value for `#active`. A participant's choice is recorded in the `active_key_custody` field of the `ScpKeyCustodyAttestation` service entry, which admits `HardwareBiometric`, `HardwarePin`, and `Software` (`crates/scp-did/src/attestation.rs:43`, `crates/scp-did/src/attestation.rs:86`). §27.4.4 of the attestations spec governs what a consumer reads off that declaration; this section states no reading rule of its own.
...
A DID owner records the agent key's declared custody in the `agent_key_custody` field of the `ScpKeyCustodyAttestation` service entry, which is absent when no agent key exists (`crates/scp-did/src/attestation.rs:47`). §27.4.4 of the attestations spec governs what a consumer reads off that declaration.
...
**OQ-9 — Does `CustodyType.platform` name an OS keystore or an encrypted key file?** *Undefined:* the Swift and Kotlin SDKs document it as platform-native secure storage and the Python SDK documents it as an alias for file-backed custody (§3.9.8, divergence D13). *Should define it:* §3.2 of this spec, which owns the custody options a participant chooses among, with the three SDK enums following it. *Breaks meanwhile:* a participant who asks for platform custody receives an encrypted file on one SDK, and the custody model published in their `ScpKeyCustodyAttestation` entry then misdescribes where their `#active` key sits.
