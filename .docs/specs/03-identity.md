...
| `publicKeyMultibase` multicodec fix (§18.2.2A) | Phase 2 (#2297) | `scp-did` | `multibase_encode` (`crates/scp-did/src/document.rs:1253`) omits the `0xed 0x01` header the `Ed25519VerificationKey2020` suite requires. Changes the signed relay-layer octets, so it lands with the canonicalization above. |
| `ResolvedDidDocument` two-variant enum (§3.10.10) | Phase 2 (#2297) | `scp-identity` | Replaces the `DidDocument`-plus-flag struct at `crates/scp-identity/src/resolver.rs:69`. This is the mechanism every "unresolved rather than absent" rule depends on, and ADR-062 §Decision 5 makes `NoOpRelayQuerier`'s classification conditional on it. |
| `NoOpRelayQuerier` sever (ADR-062 §Decision 5) | Phase 2 | `scp-identity`, `scp-ffi` | Production `DualLayerResolver` construction fails closed without a real querier, and the arm moves behind a testing cfg. Reachable today from `crates/scp-ffi/uniffi/src/bridge.rs`'s `ensure_did_resolver_initialized_on`. |
