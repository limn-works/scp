...
3. PUBLISH:
   ...
   a-bis. Re-sign the identity's service record under the new #active and
...
`limit: N` (N = 16) **dominates `limit: 1`** and costs nothing where it does not help. Against a **validating** SCP-native relay the routing ID is slot-exclusive (below) and holds at most `MAX_RETAINED_SUFFIXES` slots, one per divergent suffix. **A slot is an ordered sequence of frames** (`09-security-model.md` §9.7.4.2 R9), so `N` bounds one page of frames and bounds no slot: the resolver reads the frames one QUERY returned, finds the highest contiguous sequence they cover, and issues further QUERYs by sequence range until it holds inception through head or the relay serves no more. Against a **non-validating or foreign** transport that accumulates multiple blobs per `routing_id`, `limit: N` lets the resolver retrieve up to N candidates and sift them by chain verification and fork precedence (§3.10.4 step 5) — defeating an intra-relay shadowing attempt that a single-record fetch would miss, Under an *active* flood on a non-validating relay this remains best-effort (§3.10.8 residual): N candidates may all be junk. The resolver's selection across relays (§3.10.4) still returns the genuine chain whenever any queried relay holds it.
...
4. **Slot placement.** For a frame that passed steps 1–3, place its events under `09-security-model.md` §9.7.4.2 R9, which is authoritative for the slot key, what a slot holds, the write rule over the assembled chain, and the eviction rule; this section restates none of it. **The frame carries no signature** (`09-security-model.md` §9.10.12): a signature over the frame would be checked against a key the relay reads out of the writer's own chain, and no frame that passes step 3 could fail such a check except by a byte change step 3 already rejects, so chain verification plus R9 is the whole write decision. The frame carries no sequence number and the rule compares none.
...
- **(c)** QUERY at that `routing_id` returns **the slots and nothing else** — every slot the routing id holds, one per divergent suffix, because a resolver runs fork precedence over the chains it is served and a relay that returned one slot would decide that rule for it;
...
2. **Relay restart or store-sharing peer node** (the slot index is not persisted, so it starts empty even though the durable blob is **still present** in storage): the genuine record is *present*, only the cache forgot it. This is a real, bounded, availability-only window on that one relay until the next binding-valid publish re-warms the index. Two properties keep it availability-only, never integrity: (i) the **read path is storage-authoritative** — QUERY (rule (c)) re-derives the slot from the stored self-certifying frames, so a cold-index QUERY still returns only the genuine record whose chain extends every other chain in storage and never surfaces co-located junk as genuine; and (ii) the **DELETE gate (rule (d)) and establish reconciliation are storage-derived**, so a replayed frame carrying a prefix of the stored chain cannot delete or roll back the record that extends it, even against a cold index. The residual — a junk flood that pushes the genuine record outside a narrow QUERY `limit` window on that single relay before the index warms — is covered by the resolver's selection across the identity's other relays and the fallback set (§3.10.4). The earlier claim that "the genuine record is already absent" applies **only** to case 1; for case 2 the record is present and the mitigation is the storage-authoritative read and DELETE gates plus multi-relay resolution, not absence. **Both gates are relay-side code**, so neither closes anything against a relay whose operator omits them; the control that covers a first contact on a single relay is the two-relay rule of `09-security-model.md` §9.7.4.2 R11.

Slot-exclusivity is a relay **storage** behavior (the base relay stores multiple opaque blobs per `routing_id` with no per-`routing_id` cap, ADR-004). This spec section is authoritative for the behavior; the relay-storage mechanics (one slot per divergent suffix holding the longest chain, eviction, cold-index reconciliation, the storage-derived DELETE gate) are transcribed in the companion **ADR-004 "DID-Record Slot-Exclusivity" subsection**, whose own text still names the superseded sequence rule and the superseded frame name (implemented under #482 / SCP-RELAYRES-003). The threat-model conclusions — the flood-inert enumeration and the DELETE-rollback vector — are owned by §3.10.8; ADR-004 records how the relay storage layer realizes them.
...
- **Why the frame carries no public key and no signature.** A relay authorizes a write by verifying the chain the frame's `value` carries against the standing root of the assembled chain, which it reads out of events it already verified rather than out of the writer's bytes (`09-security-model.md` §9.7.4.2 R9). A writer-supplied key would authorize the writer against itself, and a signature checked against such a key would add nothing chain verification has not already decided, which is why the frame carries neither.
...
5. Where the resolver holds no accepted baseline for this identifier, it
   MUST have obtained a chain that verifies from two relays under distinct
   declared operators, one of them in the fallback set. A relay that failed,
   timed out, or served a chain the resolver discarded counts toward
   neither number. Otherwise the resolver returns `Inconclusive{SingleSource}`
   and adopts no head (09 §9.7.4.2 R11). A resolver holding an accepted
   baseline resolves against one relay, and this step does not apply to it.
...
2. Wrap (identifier, value) in a key-event record frame (§9.10.12) per
   segment. The publisher signs no frame: the chain inside `value` is what
   authorizes the write (09 §9.6.1, §3.10.2 step 3).
...
- **Attacker floods junk at the DID routing ID.** The `routing_id = SHA-256("scp:did:" || identifier_bytes)` is publicly derivable, so any party can PUBLISH to it. On a **validating SCP-native relay every flood variant is inert**, because the relay verifies the chain each frame carries, keeps one slot per divergent suffix, and makes the `routing_id` slot-exclusive once claimed (§3.10.2):
  - **Junk frame** (malformed, wrong binding, or a chain that fails verification) — rejected at validation; never enters a slot.
  - **Valid-looking frame carrying a prefix of the chain in an existing slot** — the relay already holds every event it carries, so it replaces nothing (`09-security-model.md` §9.7.4.2 R9, case 3: byte-identical events are an idempotent TTL refresh). Extending the slot requires an event the attacker cannot sign, because R3 verifies every appended event against the standing root of the assembled chain.
  - **Frame carrying events that diverge from a stored chain at a position the relay holds** — they form a second divergent suffix in their own slot (`09-security-model.md` §9.7.4.2 R9, case 2), and R9's rank eviction is what bounds the total (`09-security-model.md` §9.7.4.2 R9). A flooder that opens slots without revealing the standing commitment produces rank-2 suffixes, which R9 evicts first, so a rank-2 flood cannot displace the owner's rank-1 recovery. A flooder that opens many rank-1 slots has revealed the standing commitment on each, which has already contested the identity (`09-security-model.md` §9.7.4.2 R7); the relay then keeps its retained set and the resolver reads the residue count R9 defines.
...
pub enum ResolutionSource {
    ...
    /// Chain served by two relays under distinct declared operators, at least one in the
...
`DidResolver` queries the identity's own relays and the fallback set in parallel and settles the chains they serve under §3.10.4. A first contact that reaches fewer than two relays returns `Inconclusive{SingleSource}` rather than a key state (`09-security-model.md` §9.7.4.2 R11).

### 3.10.11 Bootstrap and Network Growth

The relay network is designed to be self-reinforcing as it grows:

- **Day one.** Few relays exist, so an identity publishes its chain to the community relay list of §18.5.1 and lists no relay of its own. A resolution reads two entries of that list under distinct declared operators. That satisfies R11's two-relay rule, because an identity that lists no relay of its own names none of them, which puts every entry in that identity's fallback set (`09-security-model.md` §9.7.4.2 definitions).
...
- **Maturity.** Most identities list several relays of their own, so a resolver reaches an identity relay and a fallback relay quickly and R11's second relay costs no perceptible latency. The fallback set stays what the definitions section of `09-security-model.md` §9.7.4.2 defines it to be — the relays the identity's own service record does not name — which is the property R11 reads. An identity whose service record grew to name every bootstrap relay would leave that set empty and make its own first-contact resolutions return `Inconclusive{SingleSource}`, so a controller lists the relays it uses and not the whole bootstrap list.
...
**Who signs it.** The record carries an Ed25519 signature by the operational key the identity's latest state-carrying key event designates for the service-record role, never by a root member. `09-security-model.md` §9.7.4.2 R3 rejects a state-carrying event whose designation names a key that same event does not list `current`, so the designated key is always a key the current key state lists. A reader verifies in two steps and never one: it verifies the key-event log under `09-security-model.md` §9.7.4.2 R2 and R3, reads the designation out of the verified chain, then verifies the record's signature against the designated key (`09-security-model.md` §9.6.3). **A reader holding no accepted service record for the identity obtains one from two relays under distinct declared operators, one of them in the fallback set** (`09-security-model.md` §9.7.4.2 R11), and returns `Inconclusive{SingleSource}` where it cannot: a single relay can serve a genuine record truncated to an older sequence exactly as it can serve a truncated chain, and a first-contact reader holds no high-water mark to compare against.

**How a reader settles two copies: last writer wins on the record's own sequence.** The record carries a monotonic sequence of its own, unrelated to the key-event log's sequence. The sequence is a `u64`, and `09-security-model.md` §9.7.1's attestation class rejects a record at the maximum representable value as an author-attributable defect, so no writer can exhaust the space. **The high-water mark is keyed to the pair (identifier, designated key), never to the identifier alone.** Among copies whose signature verifies under the currently designated key, a reader takes the highest sequence, and it rejects a copy at a sequence lower than the highest it has already accepted **under that same designated key**. **The mark resets when the log designates a different key**: the reader clears it and accepts the first record the newly designated key signs at any sequence. The reset is sound because the designation change is itself root-signed (`09-security-model.md` §9.7.4.2 R3) and the old key's holder cannot produce one; a mark scoped to the identifier alone would let one record written at a high sequence by a briefly-held key block the controller's own recovery record forever, which is the recovery the last paragraph of this section states. Plain sequence ordering is complete here, and the fork-precedence rule of `09-security-model.md` §9.7.4.2 R6 does not apply and MUST NOT be applied: a service record has no reveal-authorized event class, so no legitimate update ever lowers the sequence, and two signature-valid records at one sequence are the designated key's holder equivocating rather than a recovery superseding a fork. A reader that holds two such records rejects both and reports the identity's transport metadata as unresolved. **A reader holding a `Contested` verdict for the identity (`09-security-model.md` §9.7.4.2 R7) accepts no new service record for it**, because a contested identity has no adopted key state and therefore no authorized designation. It keeps routing on the last record it accepted before the divergence, with the caching bound below suspended for that identity, and it surfaces the identity as contested. Reporting the identity unroutable instead would take away the reachability §9.11's standing rule promises to leave alone, and it would let any party that contests an identity cut its transport.
...
**What an attacker holding the designated key can and cannot do.** It can substitute the relay list, so it can re-point traffic and shrink the fallback set toward empty (`09-security-model.md` §9.7.4.2 definitions). Each of those outcomes is fail-closed rather than a substitution of identity: MLS group keys, not relay reachability, enforce membership (`10-infrastructure-and-self-hosting.md` §10.5, encryption-as-access-control), and a first-contact resolver that cannot reach two relays under distinct declared operators returns `Inconclusive{SingleSource}` and adopts no head (`09-security-model.md` §9.7.4.2 R11). It cannot change a key, a key's condition, the witness set, or the designation itself, because a root signature covers each of those. The controller recovers by signing a key event that designates a fresh service key and then writing a service record under it (`09-security-model.md` §9.12). That recovery works at every reader because the designation change resets the high-water mark (the settlement paragraph above), so a reader accepts the fresh record whatever sequence the attacker's record reached.

**What the witness layer covers.** The service record inherits both owner-signed guarantees the key-event log carries. **Layer A it has now**: an owner-signed monotonic sequence, highest-wins, and the two-relay first contact above. **Layer B it gains when `09-security-model.md` §9.7.4.3 lands**: the cosigned head covers the identity's current service-record digest beside the log head, in one witness signature, and §9.7.4.3's enumeration names that coverage. Until §9.7.4.3 lands, a service record's freshness rests on its signature, its sequence, the two-relay first contact above, and the caching bound above, and on nothing else.
