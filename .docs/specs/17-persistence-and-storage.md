...
**Re-seeding does not rescue an in-memory arm.** A witness that lost its store cannot enumerate the subjects whose heads it published, because those heads sit at addresses derived from each subject's identifier, so it seeds under `09-security-model.md` §9.7.4.3's case 1 from the chain the requester offers. An arm that starts empty takes that path on every restart, refuses nothing, and seeds afresh from the next divergent chain. A durable arm carries its own floor across a restart and refuses that chain.
