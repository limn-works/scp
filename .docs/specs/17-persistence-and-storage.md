...
A relay an identity designates as a witness keeps, per subject it has ever cosigned, the sequence, the event digest, the service-record sequence, the `observed_at`, and the designated key bytes of its own last cosigned head, for at most `MAX_SUBJECTS_PER_WITNESS` subjects (`09-security-model.md` §9.7.4.3, §9.18.17). That store is a provider capability, so its backend selection is governed by §17.17.1 exactly as storage is.

**SCP-CAPSEL-8014 — An in-memory witness per-subject store is a security nullifier.** The store is the input to the one check that carries the witness layer's security property: a witness refuses to cosign a chain that does not carry the head it last cosigned. A witness that lost that record and cosigns anyway signs a fork, and its cosignature then counts toward the accountability threshold of an event no honest witness would have established — so the arm keeps producing signatures while those signatures stop meaning what they claim. The harm has the same two parts SCP-CAPSEL-8013 names for identity resolution:
...
- **Loss of the non-equivocation the layer supplies.** Against a witness with a durable store, a party holding a spent pre-rotation key must corrupt a threshold of named operators to establish a fork; against witnesses whose stores restart empty, it must wait for a restart and suppress the subject's `scp:wit:` address at two distinct declared operators, which is a floor a durable store never obliges anyone to hold. The relying party's evidence — the count of recognized cosignatures — is unchanged in form and no longer carries the property it names.

The arm therefore fails **open** and is governed by SCP-CAPSEL-8012, provably absent from shipped production artifacts, rather than by SCP-CAPSEL-8011. `09-security-model.md` §9.7.4.3 states what a witness holding no baseline does — it seeds one from the subject's `scp:wit:` address, fetched from at least two entries of the community relay list under distinct declared operators — and this section restates none of it. **That seeding is stated over evidence rather than over the witness's own memory, which is what keeps this classification honest without freezing every inception**, and it does not rescue an in-memory arm. An arm that starts empty on every restart reaches the seeding fetch instead of a record it does not hold, so it cosigns an inception and refuses a fork wherever it reaches that evidence, and refuses outright where it does not. What it loses is the unconditional form of the property: a durable arm carries its own floor and needs no fetch, so no suppression of one address reaches it, while the in-memory arm's floor is exactly as good as what two relays chose to serve it on every restart. The shipped-feature-graph gate of ADR-062 therefore sees the witness store as a classified capability.