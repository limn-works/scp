...
**Where a pending recovery handle lives.** `09-security-model.md` §9.7.4.2 R10 obliges an SDK to write a pending `RecoveryHandle`, and the signed reveal-authorized event once it signs one, into every store the identity's credentials sync through, and to read every one of those stores before it composes. R10 names that store set per custody profile, and this section's own local store is the member it names on `Headless` and on a `Mobile` or `Desktop` profile under no platform account. **On `Browser` the handle lives in the credential's own `largeBlob` and never in the origin storage this section's Browser row describes**, which R10 states with its two reasons — origin storage is partitioned per origin, and the row above calls it ephemeral in a private window. **`abort_recovery` deletes the handle from every store the write duty named, and a `resume_recovery` that completes deletes it the same way**, so a finished ceremony leaves nothing in a synced store to refuse the controller's next reveal.
