...
**Where a pending recovery handle lives.** `09-security-model.md` §9.7.4.2 R10 obliges an SDK to write a pending `RecoveryHandle` into every store the identity's pre-rotation credential syncs through, to update that handle's `RecoveryPhase` in every one of them after it signs, and to read every one of them before it composes. **R10 keys that store set on the identity's pre-rotation credential and never on the running profile**, and it names two members: the platform account's synced credential store, and WebAuthn's `largeBlob` store of that credential. This section's own local store is the set where a credential syncs through neither member.

**What each member holds.** **`RecoveryHandle` is the one object every store of that set holds**, at the width `RECOVERY_HANDLE_BYTES` registers (`09-security-model.md` §9.18.17), which is why an authenticator's large-blob array and never the object's own size is what bounds what this set can hold. **The object, as `09-security-model.md` §9.7.4.2 R10 declares it**: <!-- scp:include id="recovery-handle-declaration" from=".docs/specs/09-security-model.md" -->**`RecoveryHandle` is one enum whose variants are `FixesCommitment` and `Abandons`, each carrying the identity's 32-byte identifier, a `RecoveryPhase`, and the 32-byte preimage digest of the composed event, and `RecoveryPhase`'s five values are `Composed`, `Signed`, `PublishSent`, `Confirmed` and `Submitted`.**<!-- scp:end id="recovery-handle-declaration" --> It carries the composed preimage and the signed event never, because <!-- scp:fragment id="large-blob-arithmetic" -->CTAP 2.1 floors an authenticator's whole large-blob array at 1,024 bytes across every credential it holds, and `MAX_EVENT_BYTES` is 65,536, so a duty to write the event itself would ask for a write between one and two orders of magnitude larger than the store.<!-- scp:end id="large-blob-arithmetic" --> **The composed preimage, the next commitment list, the next threshold and the signed event sit in this section's own local store, keyed on that digest**, and `09-security-model.md` §9.7.4.2 R10 states what an SDK whose local store holds no preimage under a handle's digest does. **The per-device case**: where a credential syncs through neither synced member — a `Headless` profile, or a `Mobile` or `Desktop` profile under no platform account with an authenticator supporting no `largeBlob` — the set is this section's own local store alone and spans one device. **The origin storage this section's Browser row describes is never a member** (`09-security-model.md` §9.7.4.2 R10).

**`abort_recovery` deletes the handle from every store the write duty named, and `resume_recovery` deletes it the same way once the ceremony completes at `Confirmed` plus destruction of the spent keys**, so a finished ceremony leaves nothing in a synced store to refuse the controller's next reveal. **A removal that does not land, and a `largeBlob` write that does not land, each have an arm R10 states**: the SDK surfaces the condition, and on a failed write it does not sign.
