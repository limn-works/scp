...
**Where a pending recovery handle lives.** `09-security-model.md` §9.7.4.2 R10 obliges an SDK to write a pending `RecoveryHandle`, and the signed reveal-authorized event once it signs one, into every store the identity's credentials sync through, and to read every one of those stores before it composes. **R10 keys that store set on the identity's pre-rotation credential and never on the running profile**, and it names two members: the platform account's synced credential store, and WebAuthn's `largeBlob` store of that credential. This section's own local store is the set where a credential syncs through neither member.

**What each member holds.** `largeBlob` holds a fixed-width 65-byte marker — the identifier, the `RecoveryPhase`, and the composed event's preimage digest — and never the composed preimage or the signed event, because CTAP 2.1 floors an authenticator's whole large-blob array at 1,024 bytes across every credential it holds while `MAX_EVENT_BYTES` is 65,536. The composed preimage and the signed event sit in this section's own local store beside the marker. **The origin storage this section's Browser row describes is never a member**, and R10 states the span a `largeBlob` store reaches and the two reasons origin storage is the wrong home; this section states neither and cites R10 for both.
