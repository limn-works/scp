...
**Where a pending recovery handle lives.** `09-security-model.md` §9.7.4.2 R10 obliges an SDK to write a pending `RecoveryHandle`, and the signed reveal-authorized event once it signs one, into every store the identity's credentials sync through, and to read every one of those stores before it composes. The store set follows the custody profile of `09-security-model.md` §9.7.4.1 item 4: the platform account's synced credential store on `Mobile` and on `Desktop` under a platform account, the browser's origin storage on `Browser`, and this section's own local store on `Headless` and on a `Mobile` or `Desktop` profile under no platform account. A handle written only to the local store makes the rule observe one device, which is why R10 names the set rather than a single location.
