...
**Per-entry binding.** An implementation MUST pass `key_type ‖ entry_index` as AES-256-GCM associated data when it encrypts an entry, where `key_type` is that entry's stored byte and `entry_index` is that entry's position written as a little-endian `u64`. A read path MUST read the stored `key_type` byte and compare it against the key type a caller's handle names before it decrypts anything, and MUST return an error naming both of those types when the two differ. Without both rules a handle whose recorded index no longer names the entry that handle was minted against decrypts whichever key now occupies that index, and an Ed25519 constructor accepts any 32 bytes, so an X25519 static secret produces an Ed25519 signature under a caller's identity key and no check reports an error. A rewrite that moves an entry to a different index MUST decrypt that entry and re-encrypt it under its new index, because associated data covers that index.
