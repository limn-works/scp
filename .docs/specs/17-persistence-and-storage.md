...
- **Loss of the equivocation evidence the layer supplies.** Against a witness with a durable store, a party that forks a subject's chain below a cosigned head is refused and a conflict statement records the attempt. Against witnesses whose stores restart empty, that party waits for a restart and suppresses the subject's `scp:wit:` address at two distinct declared operators, and it then collects a cosignature over its fork. The relying party's evidence is unchanged in form and no longer carries the property it names. **The fork itself is still settled by the root rule** (`09-security-model.md` §9.7.4.2 R6), so what the nullifier destroys is the detection this layer exists to supply and never the fork verdict.
