...
A relay an identity designates as a witness keeps, per subject it has ever cosigned, the sequence, the event digest, the service-record sequence, and the `observed_at` of its own last cosigned head (`09-security-model.md` §9.7.4.3). That store is a provider capability, so its backend selection is governed by §17.17.1 exactly as storage is.
...
- **Loss of the non-equivocation the layer supplies.** Against a witness with a durable store, a party holding a spent pre-rotation key must corrupt a threshold of named operators to establish a fork; against witnesses whose stores restart empty, it must only wait for a restart. The relying party's evidence — the count of recognized cosignatures — is unchanged in form and no longer carries the property it names.

The arm therefore fails **open** and is governed by SCP-CAPSEL-8012, provably absent from shipped production artifacts, rather than by SCP-CAPSEL-8011. `09-security-model.md` §9.7.4.3 states the rebuild obligation a witness follows when it loses the record by any other cause, and this section restates none of it. That obligation is the fail-closed behavior a durable arm delivers and an in-memory arm defeats, because an arm that starts empty on every restart meets the obligation's precondition every time it starts and would therefore never cosign anything, which is why an implementation reaching for it silently skips the check instead. The shipped-feature-graph gate of ADR-062 therefore sees the witness store as a classified capability.