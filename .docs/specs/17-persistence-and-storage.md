...
- **Loss of Mainline's independent suppression resistance (§3.10.8).** With a real Mainline backend, an attacker who suppresses a DID record on all of an identity's relays still leaves the **bootstrap core** resolvable, so a resolver recovers the relay list and reaches a relay the attacker missed, and a non-SCP BEP44 client (§3.10.6) still resolves `#active`. An in-memory backend contributes nothing to that, so relay-only suppression takes out both the full document and the core, and any resolver that must fall back to Mainline (all of an identity's relays unreachable, §3.10.3) gets stale-or-absent data that still verifies by signature. Note the two layers carry different payloads (§18.2.2A), so this is not one document with two copies — suppressing the full document requires suppressing all relays, and Mainline's contribution is the core, not a second copy of the document.

**The two-encoding model sharpens this harm rather than softening it.** §3.10.7 replaced cross-layer highest-sequence-wins with per-layer resolution, so a relay-layer sequence number no longer overrules a stale Mainline record — there is no cross-layer comparison left to perform. A resolver that reaches only Mainline (all of the identity's relays unreachable, §3.10.3, or a non-SCP BEP44 client) therefore reads the stale bootstrap core and has nothing that would reject it, because within the Mainline layer that record is the highest sequence number the layer holds. Under the earlier cross-layer rule such a resolver could at least be corrected once it also reached a relay. The harm-bounding argument above survives this change: it never rested on the cross-layer comparison, only on the relay layer carrying the rotated key, which the two-encoding model makes explicit by naming the relay layer authoritative for the full document. What no longer holds is the earlier sentence's mechanism, which is why this section states the relay layer's authority instead.
