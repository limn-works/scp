...
**2. Multi-key verification method architecture (§3.9, ADR-039).** Standard did:dht uses a single Ed25519 keypair (the one encoded in the DID string) for everything — signing documents, authenticating, operating. SCP defines multiple verification methods per DID document. §3.9 of the identity spec governs the four keys; the summary below records what this prior-art comparison needs and states no rule of its own:

- **Identity Key (`#0`)** — the Ed25519 key encoded in the DID string. Long-lived root of trust. Used for BEP44 signing and DID document modifications only — never for day-to-day operations. §3.9.2 of the identity spec states where its key material lives.
- **Human Signing Key (`#active`)** — the human's operational key for protocol actions (signing inner envelopes, MLS operations, capability delegation). Rotatable without changing the DID. Published in the DID document, authorized by the Identity Key. §3.9.3 of the identity spec states where its key material lives and who chooses that.
- **Pre-Rotation Key** — a commitment to the next Identity Key. The hash of the pre-rotation key is published in the DID document before it's needed, and identity migration installs the revealed key as the new `#0` (§9.7.4 of the security spec, §3.9.5 of the identity spec). This enables safe recovery even if the current Identity Key is compromised: the pre-rotation commitment was made before compromise, so an attacker who steals the Identity Key cannot forge a valid migration (they would need the pre-rotation private key, which was generated separately and stored apart from operational custody).
