...
**5. Two DID-document encodings, one per layer.** Standard did:dht specifies DNS packet encoding (TXT/SRV records) within the 1000-byte BEP44 payload. SCP keeps that encoding on the Mainline layer, where it carries the bootstrap core, and adds a JSON-LD encoding on the relay layer, where the 262,039-byte relay bound (not the 1000-byte BEP44 bound) lets the full document carry attestations, service endpoints and key material. The two encodings differ in a way the method itself forces: the did:dht method states that it does not use JSON-LD and that a did:dht document "MUST NOT include the `@context` property," while §18.2.2A requires `@context` on the relay-layer document. The relay-layer form is therefore not a did:dht document and does not claim to be — it is SCP's own encoding at an SCP routing ID, which is what lets it carry `@context` and exceed 1,000 bytes without contradicting the method. This subsection is where SCP's two-encoding model was first written down, in a prior-art survey. It does not govern: §18.2.2A of the addressability spec and §3.10.5 of the identity spec carry the normative statement, and this subsection records the model's origin so a reader can trace it.
...
2. **SCP's extensions are additive, not contradictory.** Nothing this design asks SCP to do violates the did:dht specification, and two things the shipped code does today violate it — it prepends a `z` to the DID string and it publishes JSON where the method requires a DNS packet, both of which §18.2.2A records as defects the code fixes. SCP adds capabilities on top (dual resolution, multi-key verification methods, per-layer healing, agent signing keys). What a did:dht-compliant resolver sees is the Mainline layer, and the Mainline layer carries a conformant DNS packet: the bootstrap core (§18.2.2C), holding `#active`, the pre-rotation commitment and the relay list as ordinary verification-method and service entries that a standard resolver parses. SCP's richer relay-layer document lives outside the did:dht method entirely, at an SCP routing ID a did:dht resolver never queries, so it extends SCP without changing what a conformant resolver reads.
