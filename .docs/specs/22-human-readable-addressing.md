...
The Verified bootstrap context requires endorsement independence checking as part of its `ContextSybilPolicy::high_trust()` admission policy. This is a spec clarification, not a new feature — §9.3 already describes independent endorsements as a trust signal, and the `check_threshold_attestation` function (ADR-017 acceptance criterion 7) already implements pairwise independence scoring via `ThresholdRequirement`.
