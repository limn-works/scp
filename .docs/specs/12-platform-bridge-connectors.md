...
3. On approval, the context appends a `BridgeRegistered` leaf (event type `BridgeRegistered`, ADR-011, verifiable event log) to its Merkle event log. The leaf payload is a `BridgeRegistrationEvent` with `action: Approved` (bridge registration wire table, §12.12.2). That payload carries the assigned `bridge_id` (lowercase hex-encoded SHA-256 of `context_id || operator_did || platform || timestamp`, a 64-character hex string carried as `String` on the wire), the `operator_did` from the proposal, the `governance_did`, which copies the `executor_did` that the `GovernanceActionExecuted` leaf for the same proposal records (governance event types, ADR-031 acceptance criterion 7) — one DID under every governance model, SingleAdmin and the three vote-based models alike, because every member already writes that one value into its `GovernanceActionExecuted` leaf and §7.3.1 already requires that leaf to be byte-identical across honest members, the `context_id` of the context that executed the proposal, and the committer-assigned `timestamp` (verifiable event logs, §7.3.1). Every member executes the approved `RegisterBridge` payload under the context's governance model (governance, §5.9), and step 4 publishes the resulting bridge metadata.
...
**Suspension** uses `SuspendBridge { bridge_id, reason, duration: Option<u64> }`. On governance approval, the context appends a `BridgeSuspended` leaf (event type `BridgeSuspended`, ADR-011, verifiable event log) to its Merkle event log. The leaf payload is a `BridgeRegistrationEvent` with `action: Suspended { reason, duration }` (bridge registration wire table, §12.12.2). Suspension stops message processing but retains shadow state and credentials (§12.11.1). Reactivation appends a `BridgeReactivated` leaf (event type `BridgeReactivated`, ADR-011, verifiable event log) whose payload is a `BridgeRegistrationEvent` with `action: Reactivated`, on one of two triggers. If `duration` is set, every member appends the `BridgeReactivated` leaf when the deadline `timestamp + duration` of the `BridgeSuspended` payload passes, and only when that `BridgeSuspended` leaf is still the highest-sequence lifecycle leaf for that `bridge_id` at the deadline: a later `BridgeReactivated`, `BridgeSuspended`, or `BridgeRevoked` leaf for the same `bridge_id` cancels the deadline of every earlier `BridgeSuspended` leaf, so an elapsed `duration` of an earlier suspension never lifts a later suspension and never appends a second `BridgeReactivated` leaf after an explicit reactivation. The leaf `timestamp` is that deadline, because a timer-triggered leaf carries its pre-computed deadline (verifiable event logs, §7.3.1); the leaf `actor_did` is the timer sentinel `system:timer` (ADR-011, verifiable event log), the sentinel that the TTL-expiry `ContextExpired` leaf carries, because no member's commit produced the leaf; and the payload `governance_did` copies the `governance_did` of the `BridgeSuspended` leaf whose `duration` elapsed. Otherwise, an explicit `ReactivateBridge { bridge_id }` governance action is required, and on its approval the context appends the `BridgeReactivated` leaf whose `governance_did` is the `executor_did` of the `GovernanceActionExecuted` leaf for the `ReactivateBridge` proposal (ADR-031 acceptance criterion 7), as §12.2.1 step 3 defines for the `BridgeRegistered` leaf. Neither trigger reactivates a bridge whose highest-sequence lifecycle leaf is `BridgeRevoked`, because `Revoked` is terminal (bridge status state machine, §12.2.1).
...
1. **Registration and admission.** The bridge operator submits a `RegisterBridge` proposal, whose payload carries the platform's webhook URL and webhook signing key, and the context's governance model decides that proposal (bridge registration protocol, §12.2.1). A bridge node MUST admit a bridge only on a governance approval that the node itself verifies. The node verifies an approval by executing it. The bridge operator is a member of the context, and the operator's membership lives on the bridge node, because the node decrypts SCP messages as that member (bridge encryption model, §12.6.1; SCP-to-platform message flow, §12.10.5). As a member, the node derives the context's event log from the MLS-commit-ordered stream (verifiable event logs, §7.3.1; equivocation detection, §9.9.3) and executes each approved governance action under the governance model the context declared at creation (governance, §5.9). The four bridge lifecycle leaves — `BridgeRegistered` (§12.2.1 step 3), `BridgeSuspended` and `BridgeReactivated` (suspension, §12.2.2), and `BridgeRevoked` (§12.2.2 step 2), each carrying a `BridgeRegistrationEvent` payload (ADR-011, verifiable event log; bridge registration wire table, §12.12.2) — therefore enter the node's copy of the log only after the node itself executed the governance action that produced each one. The criterion the node applies before it admits a bridge, and applies again before it verifies a webhook signature, constructs an SCP envelope, or creates a shadow for that bridge: among the bridge lifecycle leaves in the event log that the node holds as a member of the context whose payload `bridge_id` names that bridge and whose payload `context_id` names the context whose log holds the leaf, the leaf with the highest committer-assigned sequence (event sequencing, §7.3.1) is a `BridgeRegistered` leaf with `action: Approved` or a `BridgeReactivated` leaf with `action: Reactivated`, and the payload `operator_did` of that bridge's `BridgeRegistered` leaf equals the DID under which the node holds its membership in the context (bridge operator as MLS group member, §12.10.5). A bridge with no `BridgeRegistered` leaf fails the criterion. A bridge whose highest-sequence lifecycle leaf is `BridgeRevoked` fails it, because §12.2.2 makes revocation terminal. A bridge whose highest-sequence lifecycle leaf is `BridgeSuspended` fails it until a later `BridgeReactivated` leaf arrives; for that bridge the node retains shadow state and credentials (credential requirements, §12.11.2) and answers each webhook delivery that claims that bridge with `BRIDGE_SUSPENDED` (error format, §12.10.3). A bridge whose `BridgeRegistered` leaf names another member as `operator_did` fails it on every node but that member's, because every member's log holds the same leaf and only the operator's node runs the bridge. The node reads admission from that log and from no other input. A registration that reaches the node by any other path is not in that log, so the node MUST refuse it; the paths this rule refuses include a file the node's operator writes, a request body, and a leaf that any party hands to the node, the node's own operator included. Bridge connectors as protocol entities (§12.2) gives admission to the context's governance model and gives it to no node operator. A node whose operator is not a member of a context holds no event log for that context, so the node admits no bridge into that context. A node that holds no `BridgeRegistered` leaf for a bridge holds no platform key for that bridge, because the node stores the platform key when governance approves the registration (authentication, §12.10.2). The node verifies no webhook signature for a bridge that fails the criterion, answers each webhook delivery that claims a bridge with no `BridgeRegistered` leaf, or with `BridgeRevoked` as its highest-sequence lifecycle leaf, with the rejected webhook response (`accepted: false`; `POST /v1/scp/bridge/webhook`, endpoints, §12.10.4), constructs no SCP envelope that carries that bridge's provenance, and creates no shadow for that bridge.
...
| `Reactivated` | `"Reactivated"` | — | Governance reactivated a suspended bridge, or the suspension `duration` elapsed. |
...
| `governance_did` | `String` (DID) | Yes | The `executor_did` of the `GovernanceActionExecuted` leaf for the proposal that produced this event (ADR-031 acceptance criterion 7; §12.2.1 step 3). A deadline-triggered `Reactivated` copies the `governance_did` of the `BridgeSuspended` leaf whose `duration` elapsed (suspension, §12.2.2). |
