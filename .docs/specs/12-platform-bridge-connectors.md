...
1. **Registration and admission.** The bridge operator submits a `RegisterBridge` proposal, whose payload carries the platform's webhook URL and webhook signing key, and the context's governance model decides that proposal (bridge registration protocol, §12.2.1). A bridge node MUST admit a bridge only on a governance approval that the node itself verifies. That approval is the signed `BridgeRegistered` leaf the context emits in its Merkle event log when governance approves the registration (§12.2.1 step 3); the verifiable event logs section of the trust spec (§7.3.1) defines that leaf's canonical hash and states that the acting agent signs it. Before the node admits the bridge it MUST establish three facts about that leaf, and it MUST refuse the registration when it cannot establish any one of them: the leaf's signature verifies under a signing key in the DID document of the governance actor the leaf names, which the node resolves through the DID resolution layers (§3.10); the leaf names the `bridge_id` the node is asked to admit; and the leaf names the context the node is admitting that bridge into. A node that holds no such verified approval for a bridge answers `BRIDGE_NOT_AUTHORIZED` (error format, §12.10.3) to every request naming that bridge. A registration that any other party presents to the node — the node's own operator included — carries no verified approval, so the node MUST refuse it: bridge connectors as protocol entities (§12.2) gives admission to the context's governance model and gives it to no node operator.
