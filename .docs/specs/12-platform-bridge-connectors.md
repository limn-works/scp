...
The platform verifies the JWT signature against a key the operator's DID document authorizes, following the verification procedure §3.11.4 of the identity spec states for a DID-authentication response. A bridge login token proves control of a DID to the platform, so `authentication` is the verification relationship that governs it. The platform applies every step of that procedure, and MUST NOT skip one: each step rejects a document or a key the steps around it accept, so a verifier missing any one accepts a key the protocol does not authorize. Two of them decide which verification method a token may name:
...
- Step 8 — the operator's DID document MUST reference that method under `authentication`. The platform MUST reject a token whose named method that array omits. This step is what stops a rotated key from authenticating: ADR-003, DID creation, item 4a moves the `authentication` and `assertionMethod` references to the new key on a Layer 1 rotation and retains the old one as `#retired-{sequence}` for historical verification. Moving that reference revokes the old key for a live session, and it revokes nothing else — §23.13 paragraph 1 of the sync spec accepts the same retained method on an event-log leaf, because a leaf records what an actor did at the sequence it occupies. §9.12 of the security-model spec states that hygiene-rotation versus compromise-removal distinction, and assigns compromise recovery to removing the method from `verificationMethod` entirely.

ADR-039 gives `#active` to a human operator and `#agent` to agent software, and §3.11.4 step 11 authenticates a caller as the holder of the method that verified. A platform MAY therefore read which of the two verified and gate on it, requiring `#active` for an action ADR-039 reserves to a human. This permission is stated here rather than in §3.11.4, which states the authentication conclusion and grants no authorization.
