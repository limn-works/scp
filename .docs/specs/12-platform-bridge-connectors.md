...
6. Key identifier: `X-SCP-Platform-Key-Id` carries the lowercase hex-encoded SHA-256 of the 32 `platform_key` bytes that the approved `RegisterBridge` or `UpdateBridgePlatformKey` action registered, a 64-character hex string. The bridge node computes that digest for every key it stores, so the header resolves to one stored key without the node verifying any signature first, and through that key to the bridge whose governance approval registered it (bridge node lifecycle, §12.10.6 step 1). A delivery whose `X-SCP-Platform-Key-Id` matches the digest of no key the node stores resolves to no bridge, and the node rejects it (§12.10.6 step 1).
