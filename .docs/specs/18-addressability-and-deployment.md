...
Community relays SHOULD support STUN service (§10.12.3) — this makes them available as NAT type detection endpoints for self-hosted relays behind residential NAT. They also serve as identity-resolution endpoints: identity owners MUST publish their key-event record and their service record to the community relays of their fallback set (`03-identity.md` §3.10.6), and resolvers query them on every first contact (`09-security-model.md` §9.7.4.2 R11). **A community relay entry is expected to witness**, because the SDK's default witness set for a new identity is drawn from this list (`09-security-model.md` §9.7.4.3), and an entry that declines to witness reduces the set every new identity of that release draws from and therefore the freshness evidence those identities can offer a peer.

**A deployment whose shipped list is empty has an empty recognized set, so it reads no cosigned head as evidence and holds every identity it resolves at `ContinuityStanding::Unresolved`** (`09-security-model.md` §9.11). Its identities still resolve and still derive key state, because no verdict reads a cosignature. Reachability is a separate property and does not enter it: the recognized set is the party's own policy held locally (`09-security-model.md` §9.7.4.2 definitions), so an air-gapped deployment that carries this list and holds cosigned heads its own relay mirrors reads them, because a cosignature verifies offline.
