...
The method leaves SCP one mapping question. Its service record writes `t=N` where "`N` is the Service's `type`", and SCP's service types (`SCPRelay`, `PreRotationCommitment`, and the rest of §18.2.2's table) are SCP-defined strings that the did:dht registry does not list, so SCP chooses which token each type writes into `t=`. The encoder slice makes that choice and records it in this section; issue #2297 fix item 3 carries the work. Nothing in the bootstrap core's membership depends on the answer.
