...
| `rent.quota_bytes` | integer | bytes | <!-- scp:include id="rent-arithmetic-quota" from=".docs/specs/09-security-model.md" -->**One verified rent payment of `units` sets the identity's quota to the greater of its standing quota and `units` times the declared quota, and the standing quota is the figure the relay recorded while that identity's rent is current and is zero for an identity whose rent has lapsed.**<!-- scp:end id="rent-arithmetic-quota" --> |
