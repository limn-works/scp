...
|-------|------|------|-------------|
| `max_blob_size` | integer | bytes | Maximum blob size the relay accepts. **The value is this relay's own**, so a publisher splits a key-event chain into frames against the value declared by the entry it is writing to and re-segments for the next entry rather than segmenting once for the whole fallback set (`03-identity.md` §3.10.5). |
| `max_blob_ttl` | integer | seconds | Maximum blob TTL the relay enforces. |
| `rate_limit_publish` | integer | per minute | PUBLISH rate limit per IP address. Absent = 6000/min = 100/sec, which is the default `09-security-model.md` §9.18.B cites here rather than stating a second one; zero turns the term off. |
| `rate_limit_subscribe` | integer | per connection | Maximum concurrent subscriptions per connection (default: 100). |
| `storage_budget_identifier` | integer | bytes | Retained-kind bytes the relay stores for one identifier (`09-security-model.md` §9.7.4.2 R9). Absent or zero = the term is off. |
| `storage_budget_total` | integer | bytes | Retained-kind bytes the relay stores across every identifier. **Absent or zero leaves the relay running its ring over the retained capacity its own host holds, and it is not "no limit"**: such a relay displaces uncovered identities as a declaring relay does and answers a write it still cannot take with ADR-004's `5001` STORAGE_FULL (`09-security-model.md` §9.7.4.2 R9). |
| `economic` | object | — | Relay economic configuration (§19.8). Optional. Absence = free relay. **These five members are what a `Payment` refusal carries in `BudgetValue::Payment`** (`03-identity.md` §3.10.10), because a publisher cannot mint a receipt from a price alone. |
...
| `economic.payee` | string | — | The identifier the relay operator receives payments under, as its 32 raw digest bytes in lowercase hexadecimal. **Every charging relay declares it**, a self-hosted relay included, and a relay compares the verified payee its adapter returns against it (`09-security-model.md` §9.7.4.2 R9). |
| `rent.price` | integer | smallest unit | The price of one rent payment as `Amount` in the smallest unit of `economic.currency` (§19.1.1). |
| `rent.period_seconds` | integer | seconds | How long one rent payment covers an identity, counted from the moment the relay verified the receipt. |
| `rent.quota_bytes` | integer | bytes | The stored bytes one rent payment covers for one identity. **A relay that declares none of the three sells no coverage, and every identity it holds is displaceable** (`09-security-model.md` §9.7.4.2 R9). |
| `receipt_exempts` | array of wire tokens | — | The refusal scopes a receipt this relay verified exempts an identity from (`09-security-model.md` §9.7.4.2 R9). **A member takes the wire token `03-identity.md` §3.10.10 pairs with its `BudgetScope` variant and never the variant's own spelling**, so `identifier` and not `Identifier`, which is the spelling ADR-004's `4041` refusal already sends. **`identifier` is the one permitted member**, and a `relay_config` naming a second token, or a token that maps to no `BudgetScope` variant, is invalid. **Absent or empty exempts no scope**, so a relay that charges and declares nothing exempts nothing. **A publisher that reads a `relay_config` as invalid records that entry as `Skipped` and writes nothing to it** (`03-identity.md` §3.10.6, §3.10.10), because a relay whose declaration it cannot parse states no price it could pay and no exemption it could buy. |

All fields are optional, and an absent field leaves the relay on the protocol default this table states for that field. Absent `economic` field indicates a free relay. **`storage_budget_total` is the one storage ceiling a relay declares, and this table carries no second storage term**: it is the whole retained capacity, of which the identities R9's ring holds covered and the identities that ring may displace are two partitions. **A publisher reads every price and every budget before it publishes a key-event record**, because each term is this relay's own and never a network constant. **It reads them through ADR-004's `POLICY` query and not through this document**: the relay answers that query with these fields verbatim, a community-relay-list entry carries a URL and no domain, §18.3 calls this document an optional web on-ramp the core protocol operates without, §18.3.2 states that a party holding DNS or a CA chain serves a fraudulent copy of it, and the relay this document belongs to has no rule deriving its URL from a list entry. A client that cannot reach the `POLICY` query publishes anyway and lets the relay's own typed refusal be the authority. **Who may still read this document, and for what**: a human operator evaluating a relay before deploying against it, and an agent choosing a relay to connect to, both of which are inspection acts that decide nothing the protocol verifies. A publisher's write path reads the `POLICY` query alone.
