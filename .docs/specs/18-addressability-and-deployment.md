...
| `rate_limit_publish` | integer | per minute | <!-- scp:include id="rate-limit-check" from=".docs/specs/09-security-model.md" -->**The rate-limit check counts every PUBLISH and every `RENT` against the publishing address, reads no field of a receipt, and decodes nothing.** No declaration exempts any party from it, because a relay sells relief from a storage refusal and sells no relief from the rate limit.<!-- scp:end id="rate-limit-check" --> |
| `rate_limit_subscribe` | integer | per connection | Maximum concurrent subscriptions per connection (default: 100). |
| `storage_budget_identifier` | integer | bytes | <!-- scp:include id="identifier-bound" from=".docs/specs/09-security-model.md" -->**`Identifier` bounds what the relay holds for one identity**, so one identity holds no more than the per-identifier budget wherever an operator declares one and no exemption stands, and the bytes it bounds are every byte the relay holds for that identity at that routing id: the accepted chain, the service record, the two witness kinds, and every retained divergent suffix.<!-- scp:end id="identifier-bound" --> <!-- scp:include id="identifier-eviction" from=".docs/specs/09-security-model.md" -->**Under `Identifier` pressure the relay evicts before it refuses:** it evicts that identifier's retained divergent suffixes in the eviction-rank order below, **evicting the retained suffix carrying the highest rank number first, so it reaches a rank-3 suffix before a rank-2 one and reaches no rank-1 suffix**, and refuses only where evicting every suffix it may evict still leaves the per-identifier budget exceeded. **The suffixes it may evict include the one a re-ranking creates**: where an accepted frame leaves the relay holding a chain that wins every pairwise R6 comparison against the chain it served, the displaced chain's events past the shared prefix become a divergent suffix at the rank R6 gave them, and that suffix is evictable in the same act. **The relay evicts only for a frame whose events have verified**, so a frame no key authorized destroys nothing.<!-- scp:end id="identifier-eviction" --> <!-- scp:include id="coverage-test" from=".docs/specs/09-security-model.md" -->**The coverage test counts the bytes of the accepted chain the relay holds at `scp:did:` for that identity, the bytes of its service record at `scp:svc:`, and the bytes of every rank-1 divergent suffix the relay holds at that routing id.** It counts no byte of a rank-2 or rank-3 divergent suffix and no byte of the `scp:wit:` and `scp:wcf:` records other parties write about it. **The bytes it counts are the `value` bytes of the frames the relay stores**, and the relay's own establishment position, its chain index and its rent record count against neither the coverage test nor the per-identifier budget.<!-- scp:end id="coverage-test" --> |
| `storage_budget_total` | integer | bytes | <!-- scp:include id="total-scope" from=".docs/specs/09-security-model.md" -->**`Total` refuses a write where displacing every uncovered identity the relay may displace still leaves the retained capacity short of the arriving frame's bytes, and refuses no write while one such identity remains**, because the ring below displaces the oldest-established uncovered identity instead. That one refusal covers the write that carried a receipt and the write that carried none, and it returns `StorageBudgetExceeded{Total, value}` carrying the declared total budget, which is the figure the `POLICY` query returns and the figure that states what would satisfy the refusal.<!-- scp:end id="total-scope" --> |
| `operator_key` | 33-byte SEC1 compressed point, lowercase hexadecimal | — | The key this relay proves control of, and the operand a controller composes the `RENT` signature preimage over (`09-security-model.md` §9.7.4.2 R9). **This is the one row a validating relay's declaration must carry**: a relay that omits it leaves every controller addressing it unable to compose that preimage, and an implementer that filled the slot with a constant would give every such relay one preimage. A relay declares one whether or not the community relay list carries an entry for it, and declaring one is not declaring a listed operator identity. |
| `economic` | object | — | <!-- scp:include id="economic-object-declaration" from=".docs/specs/09-security-model.md" -->**Every relay that charges anything, a rent price included, declares an `economic` object.**<!-- scp:end id="economic-object-declaration" --> |
| `economic.currency` | string | — | Currency code for all amounts in this economic config (e.g., `"USD"`). |
| `economic.per_publish` | integer | smallest unit | <!-- scp:include id="publish-charge-trigger" from=".docs/specs/09-security-model.md" -->**A relay charges a PUBLISH at `scp:did:` or `scp:svc:` where it declares `per_publish`, and it charges one on no other declaration.** **It charges no `scp:wit:` and no `scp:wcf:` write** (§9.7.4.3), which are the other two retained kinds, and no party pays for either.<!-- scp:end id="publish-charge-trigger" --> |
| `economic.per_byte_stored` | integer | smallest unit | Cost per byte stored as `Amount` in smallest currency unit (§19.1.1). |
| `economic.payment_adapters` | array of `adapter_id` strings | — | <!-- scp:include id="payment-adapter-registry-rows" from=".docs/standards/construction.md" -->**Each row of that registry carries the variant's own spelling and the `adapter_id` string that variant rides as on the wire**, because a backend author that invents either mints a receipt carrying a tag no relay's configured adapter matches.<!-- scp:end id="payment-adapter-registry-rows" --> <!-- scp:include id="receipt-checks" from=".docs/specs/09-security-model.md" -->The three: the **rail-bound receipt identifier** the call returned is one this relay has not already spent; **`amount_covers`** is true, which is the call's answer to whether the settlement covers the price this relay supplied; and **`currency_matches`** is true, which is its answer to whether the settlement is denominated in the currency this relay supplied.<!-- scp:end id="receipt-checks" --> |
| `economic.payee` | string | — | The identifier the relay operator receives payments under. No check on the relay's path reads it (`09-security-model.md` §9.7.4.2 R9). |
| `rent.price` | integer | smallest unit | <!-- scp:include id="rent-arithmetic-price" from=".docs/specs/09-security-model.md" -->**The relay verifies the receipt against `units` times the declared rent price, so the arithmetic multiplies the declared quota and the declared price by `units` and adds the period once.**<!-- scp:end id="rent-arithmetic-price" --> |
| `rent.period_seconds` | integer | seconds | <!-- scp:include id="rent-arithmetic-period" from=".docs/specs/09-security-model.md" -->**The same payment sets the identity's rent expiry to one declared period past the later of its standing expiry and the instant the relay verified the payment, and the declared period is the duration one payment buys whatever `units` that payment names.**<!-- scp:end id="rent-arithmetic-period" --> |
| `rent.quota_bytes` | integer | bytes | <!-- scp:include id="rent-arithmetic-quota" from=".docs/specs/09-security-model.md" -->**One verified rent payment of `units` sets the identity's quota to `units` times the declared quota, and the standing quota is the figure the relay recorded while that identity's rent is current and is zero for an identity whose rent has lapsed.** **A relay refuses, under the `Payment` scope, a `RENT` whose `units` times the declared quota is below that identity's standing quota while that identity's rent is current**, and the refusal's value carries that entry's rent terms, so a controller computes from them and from the quota its rent state reports the smallest `units` that entry accepts.<!-- scp:end id="rent-arithmetic-quota" --> |
| `receipt_exempts` | array of wire tokens | — | <!-- scp:include id="receipt-exempts-declaration" from=".docs/specs/09-security-model.md" -->**A relay that charges declares which refusal scopes a current rent exempts an identity from.** `relay_config` carries `receipt_exempts`, a set whose members are scope names, and ADR-004's `POLICY` query returns it, so a publisher reads what its money buys before it spends. **A relay that charges and declares nothing exempts nothing**: an absent or empty `receipt_exempts` exempts no scope, and an implementer reads that declaration rather than assuming a set. **`Identifier` is the one member a relay may declare.**<!-- scp:end id="receipt-exempts-declaration" --> <!-- scp:include id="identifier-exemption" from=".docs/specs/09-security-model.md" -->**A declared `Identifier` exemption stops the refusal arm of the per-identifier budget and stops no eviction**, for the identity whose rent is current, for as long as that rent is current, and up to the quota that rent bought and no further. **Its refusal test is one arithmetic: the relay tests, against the declared per-identifier budget, the bytes it holds for that identity less the smaller of that identity's counted bytes and its current quota.** **A rank-2 and a rank-3 divergent suffix stay evictable in the eviction-rank order below whatever that identity's coverage**, because the coverage test counts none of their bytes, so an exemption over them would shelter bytes nobody paid for.<!-- scp:end id="identifier-exemption" --> |

**Every field but `operator_key` is optional, and a validating relay's declaration carries `operator_key`.** **`storage_budget_total` is the one storage ceiling a relay declares.** What an absent term means, one statement per term:

- <!-- scp:fragment id="relay-config-rate-limit-absence" -->An absent `rate_limit_publish` is 6,000 a minute, and a relay turns the term off by declaring zero.<!-- scp:end id="relay-config-rate-limit-absence" -->
- <!-- scp:fragment id="relay-config-storage-absence" -->An absent or zero `storage_budget_identifier` turns that term off, and an absent `storage_budget_total` is the retained capacity the operator's own host holds with the ring beneath it and never "no limit".<!-- scp:end id="relay-config-storage-absence" -->
- <!-- scp:fragment id="relay-config-receipt-exempts-absence" -->An absent or empty `receipt_exempts` names no scope.<!-- scp:end id="relay-config-receipt-exempts-absence" -->
- <!-- scp:fragment id="relay-config-rent-absence" -->An absent `rent` member, and an absent `economic` object, each mean the relay sells no coverage and charges nothing.<!-- scp:end id="relay-config-rent-absence" -->

**A `relay_config` member takes the wire token and never the `BudgetScope` variant name**, so `identifier` and not `Identifier`: <!-- scp:include id="budget-scope-token-mapping" from=".docs/specs/03-identity.md" -->`BudgetScope` serializes under one spelling, one wire token per variant: `Identifier` takes `identifier`, `Total` takes `total`, `RateLimit` takes `rate_limit`, and `Payment` takes `payment`.<!-- scp:end id="budget-scope-token-mapping" --> **`economic.payee` arrives on the wire as the 64-character lowercase hexadecimal rendering of an identifier's 32 raw digest bytes, and a party that reads it reads those 32 bytes.**

**Six declarations are invalid.** <!-- scp:fragment id="relay-config-invalid-declarations" -->The first is a `relay_config` whose `receipt_exempts` names a token other than `identifier`, or a token that maps to no `BudgetScope` variant. The second is a `relay_config` that declares a rent price and no `economic` object. The third is a `relay_config` whose `receipt_exempts` names `identifier` and which declares no rent terms, because a current rent is the one thing that buys the exemption, so a publisher refused at such a relay holds a remedy nothing can satisfy. The fourth is a `relay_config` whose `storage_budget_identifier` is below the floor `MIN_STORAGE_BUDGET_IDENTIFIER` registers (`09-security-model.md` §9.18.17). The fifth is a `relay_config` declaring rent terms of which `quota_bytes` or `period_seconds` is zero, because a zero quota sells coverage that covers nothing — the coverage test reads held bytes against a quota of zero and finds every identity uncovered the instant its payment lands — and a zero period sells one clock-skew tolerance of coverage at the full declared price. The sixth is a validating relay's `relay_config` that carries no `operator_key`.<!-- scp:end id="relay-config-invalid-declarations" --> (`09-security-model.md` §9.7.4.2 R9, §9.18.17). **A publisher that reads a `relay_config` as invalid records that entry as `Skipped` and writes nothing to it** (`03-identity.md` §3.10.6, §3.10.10), because a relay whose declaration it cannot parse states no price it could pay and no exemption it could buy. **A relay refuses to serve while its own `relay_config` is invalid**, so an operator's misdeclaration surfaces at start rather than as every publisher silently skipping the entry. **A publisher reads every price and every budget before it publishes a key-event record**, because each term is this relay's own and never a network constant. **It reads them through ADR-004's `POLICY` query and not through this document**: the relay answers that query with these fields verbatim, a community-relay-list entry carries a URL and no domain, §18.3 calls this document an optional web on-ramp the core protocol operates without, §18.3.2 states that a party holding DNS or a CA chain serves a fraudulent copy of it, and the relay this document belongs to has no rule deriving its URL from a list entry. A client that cannot reach the `POLICY` query publishes anyway and lets the relay's own typed refusal be the authority. **Who may still read this document, and for what**: a human operator evaluating a relay before deploying against it, and an agent choosing a relay to connect to, both of which are inspection acts that decide nothing the protocol verifies. A publisher's write path reads the `POLICY` query alone.
...
**The community relay list is one shipped artifact.** The SDK ships a fixed list of relays, identical in every language binding of one release. The artifact is one JSON document named `community-relays.json`, shipped inside every binding's package. It holds a JSON array of entries, carries no insignificant whitespace, and gives each entry four members in this order: `operator`, the operator's identifier as its 32 raw digest bytes in lowercase hexadecimal; `key`, that operator's non-transferable P-256 public key as the 33-byte SEC1 compressed point in lowercase hexadecimal; `url`, the relay's URL as a string; and `free`, a boolean that is true where the relay charges nothing for basic protocol operation. Entries appear in the curator's publication order, and a party reads them in any order (`09-security-model.md` §9.7.4.2 definitions). The four language bindings ship byte-identical copies. **The shipped list is a release artifact the curator publishes, and no release process compares a binding's shipped copy against a test vector's digest**: Vector 49 of `25-test-vectors.md` is a fixture whose operator private scalars §25.2 prints, so it pins the document's encoding — member order, lowercase hexadecimal, no insignificant whitespace, the digest construction — and never the bytes a binding ships. A release process computes that digest over the four bindings' copies and compares them to each other. `free` is a boolean rather than an enumeration because it gates one two-state invariant, that the list carries at least one free entry. A verifier reads an operator's key from its entry and resolves no chain of the operator's own (`09-security-model.md` §9.7.4.2 definitions).

**The list MUST carry at least one free entry**, a protocol invariant that puts no price on basic protocol operation; `19-economic-governance.md` §19.8 states what that invariant bounds and what it leaves to a relay's declared write policy. **The `free` member asserts one property and only one: that the relay charges nothing for basic protocol operation.** A free listed relay accepts writes under its whole declared policy, the total storage budget included, and owes no party any particular write (`09-security-model.md` §9.7.4.2 R9). **The obligation the invariant carries falls on the curator**, who keeps enough free capacity on the list, under distinct operators, that a flood at one entry does not take first contact with the list. That obligation binds the curator and no party verifies it on the wire, because nothing in the protocol detects a curation breach.

**Two entries whose `key` members hold equal bytes are one operator**, and a rule that counts independent sources counts them once, whatever operator identities the two entries declare; `09-security-model.md` §9.7.4.2 R11 states that count. Two entries declaring one operator identity are likewise one operator. An inception-derived identifier is a self-signed digest, so one party mints two at no cost and a floor counting declared fields alone would count one key twice. What a party does when it finds one `operator` value under two entries declaring different `key` bytes is `09-security-model.md` §9.7.4.2's definitions' rule, and this section states the entry shape that rule reads.

**The artifact carries two roles, and neither decides a verdict**: the source of the fallback set, and a relying party's default recognized operator set, both of which `09-security-model.md` §9.7.4.2's definitions define.
...
**The list is versioned with the SDK release that ships it**, so a party takes an added operator only from a release and re-reads its held cosigned heads against that release's set, changing no key state, because none was derived from a cosignature.

**No runtime removal channel exists, and an SDK implements no removal listener until a revision specifies one.** A removal subtracts an entry from both roles at once, so a forged one would push a party below R11's floor and deny it every first contact it has not already made.
...
**A deployment whose shipped list is empty reaches no first contact**, because R11's floor takes two entries, and it keeps resolving every identity whose baseline it already holds. **An air-gapped deployment reaches no first contact and loses no act** for such an identity, because a mirrored entry cannot sign as the operator it mirrors and counts toward no floor.
