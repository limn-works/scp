...
| `storage_budget_total` | integer | bytes | Retained-kind bytes the relay stores across every identifier. Absent or zero = the term is off, and a relay that declares no total budget runs no ring (`09-security-model.md` §9.7.4.2 R9). |
...
- The node publishes its key-event record and its service record on `.build()` and on relay URL changes. **It does not continuously re-publish**: on each self-observation cadence it fetches its own log, and it re-publishes to a fallback-set entry only where that fetch shows the entry holding no record for this identity (`09-security-model.md` §9.7.4.2 R10). A re-PUBLISH of bytes the relay already holds moves nothing, for the reason `09-security-model.md` §9.7.4.2 R9 states, so it costs the node a rate-limit allowance and buys it nothing.
