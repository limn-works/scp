...
| `verificationMethod` | Yes | MUST include `#0` (Identity Key). MUST include `#active` (Active Signing Key). MAY include `#agent` (Agent Signing Key, optional per ADR-039). **This row does not currently bind `#retired-*` entries either way.** It previously read "No other verification methods permitted," which contradicted ADR-003 §4a and §4a′ — those require `rotate_active_key` and `rotate_agent_key` to retain `#retired-{sequence}` and `#retired-agent-{sequence}`, and `retire_active_key` emits them. Two `Decided` artifacts disagreed, and neither side was authorized to overrule the other, so the prohibition is suspended for `#retired-*` until the open question named in `.docs/specs/00-open-questions.md` ("Retired verification methods") is decided. Until then this row neither permits nor forbids them, and no implementation may read its silence as either answer. Every other verification method beyond `#0`, `#active`, `#agent` and `#retired-*` remains forbidden. |
...
A value that satisfies neither MUST NOT appear in the document. Two kinds fail. A bare self-declared judgement — an owner writing its own trust score — is a fact the owner does not control, published without a proof, so no reader can check it. And a value other participants compute by recording this identity's behaviour — participation history, participation records, economic activity — is checkable only against the records those participants hold, which is why §9.3 ("Sybil Resistance and Identity Uniqueness") assigns it to context state. For the second kind the document MAY carry an owner-asserted pointer naming where a reader fetches the value; the pointer is configuration, and the fetched value arrives with the provenance of whoever produced it.
...
| Participation history, participation record, economic activity | Checkable only against the records the participating contexts hold. | Excluded. §9.3 puts them in context state; the document may point at them. |
...
**Criterion 2 does not decide retired verification methods, and neither does any other clause here.** Retained `#retired-*` entries accrue one per rotation, so criterion 2 applies to them, and it cannot answer until a prior question is answered: whether a content signature verifies against a verification method the identity has retired. `.docs/specs/00-open-questions.md` registers that question, records that ADR-003 §4a and §4a′ contradict §18.2.2A's `verificationMethod` row today, and assigns the decision to a human.

This specification therefore takes no position, and takes none in either direction. §18.2.2A's row is suspended for `#retired-*` rather than left standing, because leaving it standing would have been the negative answer written as a `MUST` inside a paragraph claiming to decide nothing. An implementation MUST NOT read this section's silence, or that row's suspension, as permission to emit retired entries or as a prohibition on them; the capability stays unbuilt until the question is answered. Two clauses move when the answer lands, and a reader settling it changes them together with §18.2.2A's row: ADR-003 §4a and §4a′, and CONF-003 in §26.3 ("Old `#active` key is absent"). CONF-001 in §26.3 does not move — it asserts three verification methods on a freshly created identity, which has rotated nothing.
...
**The one mapping the method leaves to SCP, decided here.** The method's service record writes `t=N` where "`N` is the Service's `type`", and SCP's service types are SCP-defined strings the did:dht registry does not list. **`t=` carries the SCP service type string verbatim** — `t=SCPRelay`, `t=PreRotationCommitment` — which is what the method's own clause asks for, since it names the field's content as the service's type. Two properties follow: a did:dht resolver reconstructing the document recovers exactly the `type` value §18.2.2's table defines, and the encoding round-trips without a lookup table that would have to be versioned. The cost is the type string's length, which §18.2.2D prices; `PreRotationCommitment` is the longest at 21 characters and the measurements below include it.

This decision belongs here rather than in the encoder, because nothing about it requires code. An earlier draft assigned it to the encoder slice, which would have let an implementation choose the wire tokens and then back-fill this specification — the artifact-flow inversion.
...
- **A TXT rdata longer than 255 bytes splits into multiple character-strings**, which the method permits explicitly. No record in the bootstrap core reaches that length — the longest is the `PreRotationCommitment` service record at 104 bytes — so the split path is unexercised by the core and an encoder still implements it, because §18.2.2's other service types are not length-bounded.
...
|--------------------|---------|-----------------------------------|------------------------------------------------------|
| 1 | 5 | 479 bytes | 498 bytes |
| 3 (the minimum §18.2.3 recommends) | 7 | 619 bytes | 676 bytes |
| 5 | 9 | 759 bytes | 854 bytes |

Each additional `SCPRelay` entry costs 70 bytes with the short hostname and 89 bytes with the realistic one. The cap admits **8 relay entries** at the short hostname (969 bytes) and **6** at the realistic one (943 bytes), so §18.2.3's recommended minimum of 3 fits with 324 bytes to spare at a realistic hostname, and an identity publishing more relay entries than 6 is what first presses the cap. A publisher that exceeds it is rejected by §18.2.2D's gate rather than by a Mainline node.
