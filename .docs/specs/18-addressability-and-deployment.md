...
| `pow_difficulty` | integer | leading zero bits | Proof-of-work difficulty a key-event PUBLISH must clear at this relay, at most `MAX_POW_DIFFICULTY` (`09-security-model.md` §9.18.17, §9.10.12). Absent or zero = the term is off. |
...
All fields are optional. Absent fields indicate the relay uses protocol defaults or has no limit. Absent `economic` field indicates a free relay. **A publisher reads `pow_difficulty` and every price before it publishes a key-event record**, because the difficulty is this relay's own and never a network constant. **It reads them through ADR-004's `POLICY` query and not through this document**: the relay answers that query with these fields verbatim, a community-relay-list entry carries a URL and no domain, §18.3 calls this document an optional web on-ramp the core protocol operates without, §18.3.2 states that a party holding DNS or a CA chain serves a fraudulent copy of it, and the relay this document belongs to has no rule deriving its URL from a list entry. A client that cannot reach the `POLICY` query publishes anyway and lets the relay's own typed refusal be the authority. **A publisher that reads a `pow_difficulty` above `MAX_POW_DIFFICULTY` solves nothing**: it treats that entry as refusing and moves to the next entry of its fallback set (`09-security-model.md` §9.7.4.2 R9).
...
**The list MUST carry at least one free entry**, a protocol invariant that puts no price on basic protocol operation (§19.8, §19.14). It bounds what an entry may charge and bounds nothing about what an entry stores: a free entry charges nothing and still refuses a PUBLISH that fails a term of the write policy it declares, under R9's refusal rule and its recourse. **A free entry accepts an inception PUBLISH**, which is the listing criterion the `free` member asserts: `pow_difficulty` and `rate_limit_publish` apply to that write, and neither budget scope refuses it for an identifier the entry does not yet hold (`09-security-model.md` §9.7.4.2 R9). An entry that refuses inceptions breaches the criterion, which binds the curator and which no party verifies on the wire, because nothing in the protocol detects a curation breach. Alec settled the frame on 2026-09-13 together with the eviction and denial rules R9 states, and the curation obligation it implies — keeping enough free entries under distinct operator keys that a flood at one does not take first contact with it — is the curator's, like every other curation obligation this section records as open.
...
**Two rules read the artifact.** The first-contact floor of `09-security-model.md` §9.7.4.2 R11, so a relay this list names MUST answer a QUERY carrying a `proof_nonce` with a relay proof of control; and the SDK's default witness set, which §9.7.4.3 draws from this list.
...
- The node publishes its key-event record and its service record on `.build()`, on relay URL changes, and on every self-observation cadence, which is the re-publication `09-security-model.md` §9.7.4.2 R10 obliges and R9's eviction window reads. A validating relay takes a cadence write of bytes it already holds as a refresh and charges nothing for it.
