...
**The community relay list is one shipped artifact, and this section defines it.** The SDK ships a fixed list of well-known community relays, identical in every language binding of one release. **Every entry declares three things: the relay's URL, the operator identity that runs it (the operator's 32-byte inception-derived identifier, `09-security-model.md` §9.7.4.2 R13), and whether the relay is free** (an entry with no `economic` field in `relay_config` is free). **An entry declares no cosigning key**, and a verifier reads the operator's key from that operator's own key-event log. **Which position it reads that log at differs by object, and this states both:** a cosigned head and a conflict statement are log-anchored evidence, so each verifies against the key that was `current` at the position its own `witness_key_state_head` names, which is what keeps an operator's routine rotation from voiding its whole historical record (`09-security-model.md` §9.7.1, §9.7.4.3). A relay proof of control is an attestation, so it verifies against the key the log's latest state-carrying event lists `current` in the `#active` role and against no earlier key, which is what makes that rotation a revocation (`09-security-model.md` §9.7.4.2 definitions). An operator rotates that key on its own schedule while an entry changes only at a release boundary, so an entry carrying key bytes would name a retired key at every party running a release older than the rotation, and it would stand as a second authority over a key state the operator's own log already fixes. The list MUST carry at least one free entry, which is a protocol invariant that prevents economic gatekeeping of basic protocol operation (§19.8, §19.14). **An entry of this list MUST cosign free for the identities that designate it**, and that is an obligation this list's curation carries rather than a bar on every relay in the protocol. Economic rules MAY price a witness's cadence and its availability guarantees; an entry of this list MAY NOT price the act. **What pricing the act would cost, stated at its true size:** an identity that cannot pay produces no cosigned head, so the portable half of its corroboration is unavailable to a peer that did not read its head itself. It costs that identity no resolution, no key-state derivation, no admission and no grant, because no rule reads a cosignature (`09-security-model.md` §9.7.4.2 definitions). The 2026-09-07 reason for a wider prohibition — that charging for the act would make identity establishment a paid feature — rested on the required-witnessing ruling Alec superseded on 2026-09-10 with "watch and report as well", so this sentence states the obligation over the entries this list ships and no further. Two entries declaring one operator identity are two relays under one operator, and a rule that counts independent sources counts them once. **The entries of a shipped release are established as independent parties at release**, and the release process admits no two entries whose declared operators are the same party. **Every entry carries a proof-of-control obligation.** A relay this list names MUST answer a QUERY carrying a `proof_nonce` with a **relay proof of control** — the object `09-security-model.md` §9.7.4.2's definitions state, signed under the operator identity this entry declares, binding that nonce, the routing id queried, and the digest of the bytes served. A resolver verifies it by the five checks those definitions state, resolving that operator's own chain under their non-recursive floor, before it counts that relay as a source (§9.7.4.2 R11 states what a resolver does with a relay that serves no such proof). An inception-derived identifier is a self-signed digest, so one party mints two of them at no cost and presents two entries under two operator identities; a floor that counted unproven operator fields would count that one party twice, and it would deliver its independence against nobody. These relays are not privileged: they are default suggestions a deployer overrides.

**This artifact carries two roles.** Beside the fallback set below, the list is the source of a relying party's **recognized operator set**: a party reads a cosigned head as evidence only where the cosigning operator sits on its own copy of this list (`09-security-model.md` §9.7.4.2 definitions, §9.7.4.3). **What that evidence decides is nothing**, so an identity whose designated witnesses are absent from a party's copy still resolves at that party and keeps its §9.11 standing there. **Curation is a security specification and not an editorial preference**, because this list is the source of both corroboration sources and of the fallback set a resolver reaches at all, and `09-security-model.md` §9.7.4.3 states plainly that a badly curated list is a supply-chain risk no protocol check covers. **Anti-self-witnessing is one of the properties curation carries, and no protocol check delivers it.** A relying party reads a cosigned head from any operator its own copy of this list carries, and `09-security-model.md` §9.7.4.2 R3's only mechanical bar is a witness set naming the subject identifier itself, which an operator identity never is. So a controller whose own operator identity sits on this list supplies its own freshness evidence at every default-configured party — and that reaches every entry's operator, the curator of this list included. **The curator MUST therefore keep an entry's operator identity off this list for any identity that operator's controller also witnesses through it**, and the release process that admits no two entries under one party is the same process that carries this obligation. Nothing in the protocol detects a breach of it. **The list is versioned with the SDK release that ships it**, so a party's recognized set changes with the SDK version it runs, and a party takes an added operator only from a release.
...
**A deployment whose shipped list is empty reaches no first contact**, because R11's floor takes two entries under distinct declared operators and an empty list carries none (`09-security-model.md` §9.7.4.2 R11); every first contact returns `Inconclusive{SingleSource}`, which §9.6.4 records as `ContinuityStanding::Unresolved`. An identity that deployment has already resolved keeps resolving, keeps deriving key state, and keeps its standing, because R11's floor binds a verifier holding no baseline. An empty list also costs that deployment its freshness record and costs it no act (`09-security-model.md` §9.7.4.2 definitions, §9.11).

**An air-gapped deployment reaches no first contact and loses no act for an identity it already resolved.** A mirrored community-relay-list entry cannot sign as the operator it mirrors, so it serves no relay proof of control and counts toward no R11 floor (`09-security-model.md` §9.7.4.2 definitions). Such a deployment resolves every identity whose baseline it already holds and admits and grants exactly as a connected deployment does.
