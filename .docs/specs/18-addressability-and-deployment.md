...
| `rate_limit_publish` | integer | per minute | The rate limit per publishing address, counting every PUBLISH and every `RENT` (`09-security-model.md` §9.7.4.2 R9). Absent = 6000/min = 100/sec, which is the default `09-security-model.md` §9.18.B cites here rather than stating a second one; zero turns the term off. |
...
| `storage_budget_total` | integer | bytes | Retained-kind bytes the relay stores across every identifier. **Absent is the retained capacity the operator's own host holds with the ring beneath it, and it is not "no limit"** (`09-security-model.md` §9.7.4.2 R9). |
| `economic` | object | — | Relay economic configuration (§19.8). Absent = the relay charges nothing and sells no coverage. **Every relay that charges anything, a rent price included, declares it**, and a declaration carrying `rent` and no `economic` is invalid (`09-security-model.md` §9.7.4.2 R9). |
...
| `economic.payee` | string | — | The identifier the relay operator receives payments under. **It arrives on the wire as the 64-character lowercase hexadecimal rendering of an identifier's 32 raw digest bytes, and a party that reads it reads those 32 bytes.** No check on the relay's path reads it (`09-security-model.md` §9.7.4.2 R9). |
| `rent.price` | integer | smallest unit | The price of one unit of rent as `Amount` in the smallest unit of `economic.currency` (§19.1.1). A relay verifies a `RENT` receipt against `units` times this figure (`09-security-model.md` §9.7.4.2 R9). |
| `rent.period_seconds` | integer | seconds | The duration one rent payment buys, whatever `units` that payment names (`09-security-model.md` §9.7.4.2 R9, which states the one arithmetic that reads all three terms). |
| `rent.quota_bytes` | integer | bytes | The stored bytes one unit of rent covers for one identity. Absent, for all three `rent` rows, means the relay sells no coverage (`09-security-model.md` §9.7.4.2 R9). |
| `receipt_exempts` | array of wire tokens | — | The refusal scopes a current rent exempts an identity from (`09-security-model.md` §9.7.4.2 R9). Absent or empty = no scope. **A member takes the wire token `03-identity.md` §3.10.10 pairs with its `BudgetScope` variant and never the variant's own spelling**, so `identifier` and not `Identifier`, which is the spelling ADR-004's `4041` refusal already sends. |

All fields are optional, and an absent field leaves the relay on the protocol default this table states for that field. **`storage_budget_total` is the one storage ceiling a relay declares.**

**Three declarations are invalid.** The first is a `relay_config` whose `receipt_exempts` names a token other than `identifier`, or a token that maps to no `BudgetScope` variant. The second is a `relay_config` that declares a rent price and no `economic` object. The third is a `relay_config` whose `receipt_exempts` names `identifier` and which declares no rent terms, because a current rent is the one thing that buys the exemption, so a publisher refused at such a relay holds a remedy nothing can satisfy (`09-security-model.md` §9.7.4.2 R9). **A publisher that reads a `relay_config` as invalid records that entry as `Skipped` and writes nothing to it** (`03-identity.md` §3.10.6, §3.10.10), because a relay whose declaration it cannot parse states no price it could pay and no exemption it could buy. **A relay refuses to serve while its own `relay_config` is invalid**, so an operator's misdeclaration surfaces at start rather than as every publisher silently skipping the entry. **A publisher reads every price and every budget before it publishes a key-event record**, because each term is this relay's own and never a network constant. **It reads them through ADR-004's `POLICY` query and not through this document**: the relay answers that query with these fields verbatim, a community-relay-list entry carries a URL and no domain, §18.3 calls this document an optional web on-ramp the core protocol operates without, §18.3.2 states that a party holding DNS or a CA chain serves a fraudulent copy of it, and the relay this document belongs to has no rule deriving its URL from a list entry. A client that cannot reach the `POLICY` query publishes anyway and lets the relay's own typed refusal be the authority. **Who may still read this document, and for what**: a human operator evaluating a relay before deploying against it, and an agent choosing a relay to connect to, both of which are inspection acts that decide nothing the protocol verifies. A publisher's write path reads the `POLICY` query alone.
