...
- The node publishes its key-event record and its service record on `.build()` and on relay URL changes. **It does not continuously re-publish**: on each self-observation cadence it fetches its own log, and it re-publishes to a fallback-set entry only where that fetch shows the entry holding no record for this identity (`09-security-model.md` §9.7.4.2 R10). R9's ring buffer displaces an identity when a newer write arrives rather than when a clock runs out, so a write that changes nothing moves the node to the newest position in the ring and buys it nothing.
