...
**This artifact carries two roles, and the second is load-bearing for identity acceptability rather than for bootstrap reachability alone.** Beside the fallback set below, the list is the source of a relying party's **recognized witness set**: a party counts a cosignature toward an identity's accountability threshold only where the cosigning operator sits on its own copy of this list (`09-security-model.md` §9.7.4.2 definitions, §9.7.4.3). An identity whose designated witnesses are absent from a party's copy establishes no event at that party, so what this list carries decides which identities resolve at all. **Curation is therefore a security specification and not an editorial preference**, and `09-security-model.md` §9.7.4.3 states plainly that a badly curated list is a supply-chain risk no protocol check covers. **The list is versioned with the SDK release that ships it**, so a party's recognized set changes with the SDK version it runs, and a party takes an added operator only from a release.

**Operator removal is a runtime channel, and that channel subtracts only.** A party may learn between two releases that the curator removed an operator, and from the moment it learns it MUST stop counting that operator's cosignatures toward any identity's accountability threshold (`09-security-model.md` §9.7.4.3). **A party MUST NEVER learn of an added operator through that channel.** An addition widens the set of operators whose observations a party trusts, and a party reviews a release before it runs it, so an operator added between two releases would widen a party's trust basis past what that party reviewed. A removal narrows that basis, so a forged removal takes availability from a party and takes no trust from it: at worst the party falls below some identity's accountability threshold and returns `Inconclusive{NotEstablished{…}}` (`09-security-model.md` §9.7.4.2 R14), which is the fail-closed direction.

**An event a party established before a removal stays established after it.** The party keeps every head it established under the removed operator's cosignatures, and it applies the removal to the cosignatures it counts from that point forward. Un-establishing on removal would let one operator's removal retroactively invalidate every honest identity that operator had cosigned, so removing one compromised operator would strip key state from every identity that had named it, which is a wider failure than the one the removal answers. **A release boundary is not this case.** A party running a new release re-derives establishment against the recognized set that release carries (`09-security-model.md` §9.7.4.3), because that party reviewed the release before it ran it and a runtime removal reaches it unreviewed.

**No section of this corpus states what authenticates the removal channel.** A sound channel must carry the identifier of the operator it removes, the release of this list the removal applies to, a signature a party verifies against a key that release already ships, and a bound that stops a replaying party from walking a reader back onto a removal it already applied. Until a revision of this section states the channel, a party's recognized set changes at a release boundary and at no other moment.
...
**A deployment that reaches no community relay has an empty recognized set and therefore establishes nothing.** Direct mode is the answer for such a deployment (`09-security-model.md` §9.7.4.3), and it is an explicitly named choice at identity creation rather than what an unreachable list degrades into.
