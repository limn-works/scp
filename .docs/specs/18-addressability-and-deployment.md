...
All fields are optional. Absent fields indicate the relay uses protocol defaults or has no limit. Absent `economic` field indicates a free relay. **A publisher reads `pow_difficulty` from this document before it publishes a key-event record**, because the difficulty is this relay's own and never a network constant.
...
**The list MUST carry at least one free entry**, a protocol invariant that puts no price on basic protocol operation (§19.8, §19.14). It fixes what an entry may charge and fixes nothing about what an entry stores: **a free entry charges nothing and still refuses a PUBLISH that fails a term of the write policy it declares**, and such a refusal sends the publisher to the next entry of its fallback set (`09-security-model.md` §9.7.4.2 R9; `03-identity.md` §3.10.6). **Free listed relays exist for reads and for first contact, and they accept writes under their declared policy.** Alec settled that frame on 2026-09-13 together with the eviction and denial rules R9 states, and the curation obligation it implies — keeping enough free entries under distinct operator keys that a flood at one does not take first contact with it — is the curator's, like every other curation obligation this section records as open.

**Two entries whose `key` members hold equal bytes are one operator**, and a rule that counts independent sources counts them once, whatever operator identities the two entries declare; `09-security-model.md` §9.7.4.2 R11 states that count. Two entries declaring one operator identity are likewise one operator. An inception-derived identifier is a self-signed digest, so one party mints two at no cost and a floor counting declared fields alone would count one key twice. **A party that finds one `operator` value under two entries declaring different `key` bytes reads neither entry as that operator** and treats that operator as absent from its list, because every verification sentence in this corpus reads "the key that operator's entry declares" in the singular, and two entries would otherwise give one witness two keys, hand a fault proof two signatures no single key holder produced, and let two verifiers reach two verdicts on one cosigned head.
