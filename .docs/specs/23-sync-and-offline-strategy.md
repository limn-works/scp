...
1. During reconciliation, each received event MUST be verified against a signing key the claimed actor's DID document authorizes, before the local event log accepts that event. A verifier reads the `actor_did` field of the `Event` struct (ADR-011), resolves that actor's DID document, and accepts a signature only from a verification method the document lists under `assertionMethod` (W3C DID Core section 5.3.3) — `#active` or `#agent`, the two operational signing keys ADR-039 grants an acting agent. An `Event` carries no `signing_key_id` field naming one of those two, which ADR-011 criterion 1 states under "Signing-key identification (ADR-039)": the seven fields are `event_type`, `actor_did`, `timestamp`, `sequence`, `payload`, `prev_hash`, and `signature`. A verifier therefore tries each authorized method and reports which one verified, so a caller applying ADR-039 Category A policy can tell a human's signature from agent software's. Every other verification method fails, including `#0`, which ADR-039's key-property table marks as signing no operational action, and including a `#retired-{n}` method that `DidDocument::retire_active_key` withdrew from `assertionMethod` on rotation. Events that fail signature verification MUST be rejected and MUST NOT be added to the local log.
