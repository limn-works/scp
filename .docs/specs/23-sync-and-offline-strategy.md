...
- `JCS(ContextSnapshot)` is the RFC 8785 (JSON Canonicalization Scheme) canonical-JSON serialization of the *entire* `ContextSnapshot` value embedded in the export — every field, not a subset. This is the repo's canonical-JSON convention (serde_json_canonicalizer; cf. `scp-protocol::jcs`). The reference implementation is the WASM bridge `manager.rs:5152-5181`: serialize the snapshot to canonical JSON, then `SHA-256(domain-bytes || snapshot-json-bytes)`, then sign the 32-byte digest.
- The signature is produced over that 32-byte digest (not over the raw JSON), using Ed25519.

**Set/Map canonicalization (normative).** Any field of the snapshot whose Rust type is a set or map with non-deterministic iteration order (e.g. `HashSet`, `HashMap`) MUST be canonicalized to a deterministic ordering — sorted by key for maps, sorted by element for sets (the `BTreeMap`/`BTreeSet` convention) — in the value that is fed to JCS, so that the canonical JSON and therefore the digest are byte-identical across runs of a single serializer/bridge-family (this is NOT a cross-family byte-equivalence claim — see the scoping clause below). RFC 8785 already fixes object-member ordering by key, but the producing implementation MUST NOT rely on a set/map's incidental iteration order for array-valued fields: array elements derived from a set MUST be emitted in sorted order before serialization. The determinism requirement is scoped **within a single serializer/bridge-family**: a given implementation MUST produce the same digest for the same logical snapshot across runs and regardless of incidental set/map insertion or iteration order. It is NOT a cross-implementation byte-equivalence requirement. The native runtime and the WASM bridge serialize structurally different snapshot value types (the native MessagePack `ContextExport` snapshot carries fields the WASM JSON snapshot does not model, and vice versa), so the same logical context does NOT yield a byte-identical digest across families. What converges across implementations is the *construction* — domain separator, full-JCS digest over that family's own snapshot value, Ed25519, `creator_did` signer, verify-before-restore, and `exporter_did == creator_did` — not the digest bytes. Each bridge family verifies only the exports it produces (see *Cross-implementation import* below).
...
3. **Verify the signature before restore.** Recompute `SHA-256(domain || JCS(snapshot))` over the *received* snapshot bytes and verify the Ed25519 signature against the resolved verifying key (strict verification). Verification MUST happen before any field of the snapshot is read into authoritative state. A failed signature MUST abort the import with a signature error distinct from the version error.

**Signed vs. wiped fields.** The signature covers the *entire* `ContextSnapshot`. However, certain per-instance fields carried in the snapshot are deliberately NOT trusted from the import even though they are signed: the importer intentionally wipes or sanitizes them because they are local-instance anti-abuse or accounting state with no cross-instance meaning, and inheriting them from a (possibly hostile, possibly merely foreign) exporter would let the exporter pre-load enforcement state against the importing node. Per `import_context` (`lifecycle.rs:1233-1308`), the importer WIPES `approved_proposals` (rebuilt from the imported event log), resets `next_proposal_seq` to `0`, WIPES `budget_tracker`, WIPES `participation_cache`, starts a FRESH spending-nonce tracker (the import path diverges from the local `restore_context` reload, which rehydrates the nonce tracker), WIPES `proposal_timestamps`, and validates/sanitizes the anti-spam snapshot state (hard-rate-limit and velocity trackers rejected if they carry future timestamps; `cooldown_until` clamped to a bounded horizon). The signature guarantees these fields were not tampered in transit; the wipe guarantees they cannot be weaponized regardless. The fields enumerated under *Construction* above (ceiling, member/suspended capabilities, assignments, threshold set/value, governance model config, economic policy, consequence rules, read-exclusion list, access-key store, pending ceiling modification, tool registrations) are by contrast trusted verbatim and depend entirely on the full-snapshot signature for their integrity.

**Format version.** The native export format `version` (§17.5, `StoredValue`-wrapped MessagePack envelope) increments to **3** for the full-snapshot signed construction. The WASM bridge ships an independent per-serializer integer envelope `version` (a JSON envelope distinct from the native MessagePack one); the two version counters are not required to share a value. What converges across all implementations is the *construction* — domain separator, full-JCS digest, Ed25519, `creator_did` signer, verify-before-restore, and `exporter_did == creator_did` — not the envelope integer. Imports MUST reject any version below the current signed format with a version error distinct from a signature-verification failure (§17.5, error code `SCP-CTX-2093`).

**Cross-implementation import (out of scope).** Native (MessagePack envelope, `version` 3) and WASM (JSON envelope, independent `version` integer) export envelopes are NOT cross-importable: each bridge family imports only exports produced by its own family, and the version/format gate rejects a foreign envelope (a native MessagePack blob is not a valid WASM JSON envelope and conversely). The two snapshot value types are not structurally identical — each carries trusted fields the other does not model — so there is no defined transcoding between them. Cross-platform export-on-one-family / import-on-another is explicitly out of scope; per ADR-034 the WASM bridge is a constrained reimplementation, not a wire-compatible peer of the native runtime. Export/import is a within-family operation (back up and restore, or migrate, using the same implementation family on both ends).
