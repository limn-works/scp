...
1. A client MUST verify the signature on every received `ConsistencyCheckpoint` before it accepts that checkpoint for comparison. The client resolves the DID document the `sender_did` field names, and accepts a signature only from a verification method that document lists under `assertionMethod` (W3C DID Core, the verification relationships section 5.3.3) — `#active` or `#agent`, the two operational signing keys ADR-039, the shared-DID human-agent identity model, grants an acting agent. A `ConsistencyCheckpoint` carries no field naming which of the two signed it: ADR-011, the verifiable event log, defines the struct with seven fields — `context_id`, `sender_did`, `event_count`, `merkle_root`, `epoch`, `timestamp`, `signature`. The client therefore tries each authorized method and MUST accept the checkpoint when either one verifies, matching what section 23.13 paragraph 1 states for an event signature. A client MUST reject every other verification method. That covers `#0`, the Identity Key ADR-039's key-property table marks as signing no operational action, and it covers a `#retired-{n}` method: ADR-003, DID creation, item 4a moves the `authentication` and `assertionMethod` references to the new key on a Layer 1 rotation and retains the old one as `#retired-{sequence}` for historical verification, and section 9.12 of the security-model spec makes that rotation the revocation act.
