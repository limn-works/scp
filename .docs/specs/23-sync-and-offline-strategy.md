...
| `key_bytes` | bytes (32) | The retired key's raw Ed25519 public key |
...
1. **Resolve the verifying key from `creator_did` under the log-anchored evidence class.** Resolve the snapshot's `creator_did` to its key state by replaying its key-event log (`03-identity.md` §3.10.4; the protocol produces no DID document), then take the key the state at the position `key_state_head` names listed `current` in the `#active` role, and accept the signature **iff** that key was `current` at that position (`09-security-model.md` §9.7.1, the log-anchored evidence class, which also states the further test for a key the state lists `Compromised{from: N}`). The verifying key is derived from `creator_did` and the signed `key_state_head` — never from an unauthenticated envelope field, and never from the creator's present `current` key, which a routine rotation would change and which would reject every export signed before it. **The position `key_state_head` names MUST be established** (`09-security-model.md` §9.7.4.2 definitions): the importer accepts the signature only where a threshold of witnesses it recognizes cosigned the creator's chain at or above that position. **Establishment at a historical position needs no fresh cosignature**, which is what lets an offline importer verify at all — it holds no fresh cosignature by construction — and `09-security-model.md` §9.7.4.2's freshness definition states that distinction. **A `ContextExport` MUST therefore carry, in a `witness_cosignatures` field of the export envelope, the cosigned heads that establish the position its `key_state_head` names**, so that an importer with no network reaches the same verdict as one with a network. A `SHOULD` here would leave every conforming offline import rejecting, because an importer that holds neither those cosigned heads nor a network path to the creator's `scp:wit:` address rejects rather than accepting an unestablished anchor, and it would leave two implementations disagreeing on which field carries them. **The field rides the envelope rather than the signed snapshot, and the full-snapshot-coverage invariant above does not reach it.** That invariant governs state the importer restores verbatim and therefore trusts from the exporter; a cosigned head is neither. Each one carries its own witness signature, which the importer verifies against that witness's own key-event log and counts only where its own recognized set carries the operator (`09-security-model.md` §9.7.4.3), so a tampered or added entry fails that verification and a stripped entry makes the import fail closed on an unestablished anchor. Binding them under the creator's signature would also fix the establishment evidence at the moment the creator signed, which would leave an export produced before its witnesses cosigned unable ever to carry them.
