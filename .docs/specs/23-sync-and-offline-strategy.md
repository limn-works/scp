...
1. A client MUST verify the signature on every received `ConsistencyCheckpoint` before it accepts that checkpoint for comparison. The client resolves the DID document the `sender_did` field names, and accepts a signature only from a verification method that document lists under `assertionMethod` (W3C DID Core, the verification relationships section 5.3.3) — `#active` or `#agent`, the two operational signing keys ADR-039, the shared-DID human-agent identity model, grants an acting agent. A `ConsistencyCheckpoint` carries no field naming which of the two signed it: ADR-011, the verifiable event log, defines the struct with seven fields — `context_id`, `sender_did`, `event_count`, `merkle_root`, `epoch`, `timestamp`, `signature`. The client therefore tries each authorized method and MUST accept the checkpoint when either one verifies. Section 23.13 paragraph 1 tells an event verifier to try each accepted method the same way; the two paragraphs differ in which methods they accept, and the note below states why that difference is unsettled. A client MUST reject every other verification method. That covers `#0`, the Identity Key ADR-039's key-property table marks as signing no operational action, and it covers a `#retired-{n}` method: ADR-003, DID creation, item 4a moves the `authentication` and `assertionMethod` references to the new key on a Layer 1 rotation and retains the old one as `#retired-{sequence}` for historical verification. Section 9.12 of the security-model spec calls that rotation soft — the retained method keeps verifying an event-log leaf under section 23.13 paragraph 1 — and assigns compromise recovery to removing the method from `verificationMethod` entirely. This paragraph accepts neither the retained method nor a removed one, for the reason the note below states.

   **Open — whether a retired key verifies a checkpoint.** Section 23.13 paragraph 1 accepts a retired `#retired-{n}` / `#retired-agent-{n}` method for an event-log leaf, on the ground that a leaf is content and a later rotation must not retroactively unmake authorship. A `ConsistencyCheckpoint` reads both ways and no artifact settles which one governs it. It is a statement about the past — this log carried this root at this event count — which is the property that makes a leaf content. It is also an input to a decision a client makes now: section 9.9.3 of the security-model spec compares two checkpoints at equal event counts and raises an equivocation alert on a root mismatch, so accepting a retired key lets a key an owner rotated away raise that alert today. This item keeps the current-key rule until a human decides, because that rule is the narrower of the two.
...
1. During reconciliation, a verifier MUST verify each received event against a signing key the claimed actor's DID document authorizes, before the local event log accepts that event. The verifier reads the `actor_did` field of the `Event` struct, resolves that actor's DID document, and accepts a signature only from a verification method the document lists under `assertionMethod` (W3C DID Core, the verification relationships section 5.3.3) — `#active` or `#agent`, the two operational signing keys ADR-039, the shared-DID human-agent identity model, grants an acting agent. ADR-011, the verifiable event log, defines `Event` in its acceptance criterion 1 with seven fields — `event_type`, `actor_did`, `timestamp`, `sequence`, `payload`, `prev_hash`, `signature` — so no field names a verification method; its note "Signing-key identification (ADR-039)" states that the credential and signature apparatus carries one instead. The verifier therefore tries each authorized method, and MUST return the one that verified: ADR-039 gives `#active` and `#agent` distinct holders, so a caller reads that value to attribute the event to a human holder or to agent software. A verifier MUST also accept a signature from a **retired** verification method the resolved document still carries — `#retired-{n}` and `#retired-agent-{n}`, the identifiers ADR-003, DID creation, item 4a and ADR-039 assign to a key a Layer 1 rotation moved aside — and MUST return `#active` for a `#retired-{n}` method and `#agent` for a `#retired-agent-{n}` method, because rotation moves a key between identifiers without moving it between holders. An event-log leaf is content: it records what an actor did at the sequence it occupies, and a later rotation MUST NOT retroactively unmake that authorship. **The acceptance carries no sequence condition, and no verifier can impose one.** A retained retired method verifies a leaf at every sequence, including a sequence an event log records long after the rotation; a verifier has no pair of values to compare, because a DID document's publish sequence counts that document's revisions and an event log's `sequence` counts one context's events. Removal is therefore what stops a retained key from signing new content, not the rotation that retired it. This is the opposite of the rule section 9.7.1 check 1 of the security-model spec states for a KeyPackage attestation, which is a bearer capability a holder presents now and which therefore binds to the current key only. Section 3.11.4 step 7 of the identity spec states the same current-key rule for a live DID authentication. A verifier MUST reject `#0`, the Identity Key ADR-039's key-property table marks as signing no operational action. A verifier MUST reject a verification method the resolved document does not carry: removing a method from `verificationMethod` is the act that revokes it for content, and section 9.12 of the security-model spec assigns that act to key compromise. A verifier MUST reject an event whose signature no accepted method verifies, and MUST NOT add that event to the local log.

   **How a verifier finds a retired method.** A retired method is referenced by neither `authentication` nor `assertionMethod`: ADR-003 item 4a moves both references to the new key, so gating a historical-verification path on `assertionMethod` membership finds nothing. A verifier accepts a retired method on three document facts instead. **The criterion:** the resolved document's own Layer 1 rotation produced the method. **The three facts that decide it:** (a) an identifier equal to `{document.id}#retired-{n}` or `{document.id}#retired-agent-{n}`, carried exactly once, where `{n}` is the decimal rendering of an unsigned 64-bit sequence with no leading zero — the two identifier shapes ADR-003 item 4a and ADR-039 assign, and no other; (b) a `type` of `Ed25519VerificationKey2020`; and (c) a `controller` equal to `{document.id}`. A method failing any of the three supplies no key. An event signed by a key pruned out of the document verifies against nothing.

   **The three facts are indicators, and they do not decide the criterion above.** A DID document records no rotation history — no counter, no signature by the outgoing `#active` over its successor, no Layer 1 pre-rotation commitment — so whoever holds `#0` writes all three fields for any Ed25519 public key, with no rotation having occurred, and that key then verifies content as the document's owner. Two things follow that an implementer MUST NOT assume otherwise. First, bounded retention (ADR-003 item 4a, at most the 2 most recent retired keys of each kind) is a property of the two rotation operations, NOT a bound a verifier can enforce on a document it receives: a verifier MUST NOT cap the retired set it accepts, because dropping the lowest-sequence entries would let a publisher revoke two genuinely rotated keys by publishing two higher-sequence ones, and section 9.12 of the security-model spec assigns revocation to removal alone. Second, this paragraph states no document-freshness bound, unlike section 9.7.1 check 2, which caps an attestation's document at 300 seconds so a stale copy cannot resurrect a withdrawn key. Removal is the only revocation act here, so a verifier reading a pre-removal document accepts a removed key for as long as it goes without re-resolving. An attacker who forges nothing, and who holds no key, therefore extends a removed key's life by suppressing the higher-sequence document: degrade a verifier's DHT reachability while a relay serves the pre-removal revision, and both revisions carry a valid signature by the owner's Identity Key, so the verifier has nothing to reject. Both gaps close the same way, with a rotation a verifier can check. `.docs/specs/00-open-questions.md` carries that question, and it is open.

   **Open — why an `Event` names no verification method.** Every other signed structure in SCP binds one into its signed preimage: `InnerEnvelope`, `ScpCredential`, `KeyPackageAttestation`, `ScpIdResponse`, `BlockNotification`, and `SenderKeyEpochAdvance` each carry a `signing_key_id` or a `signer_key_ref`, and §9.5.2 of the security-model spec states the reason: a field inside the signed preimage makes a forged verification-method claim invalidate the signature, which prevents confusion between `#active` and `#agent`. `Event` and `ConsistencyCheckpoint` are the two exceptions, and no artifact records why. Trying each authorized method, as this paragraph and §23.12 item 1 both require, recovers which method signed but binds nothing to the signature. One document that carries a single key under two fragments with different holders is caught — `verify_event_signature` rejects that document outright — but a reattribution spread across two published versions is not. Version N carries key K under `#active`; version N+1 carries K under `#retired-agent-{n}` and no `#active` holding K. Each version is internally consistent, so the one-document check finds nothing in either, and every signature K ever produced then reads as agent software rather than as the human who holds the DID. Closing that gap means adding the field to ADR-011 acceptance criterion 1, which changes every leaf hash and every §25 test vector. A human decides whether the binding is worth that cost.
