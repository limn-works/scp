...
Every agent is bound to its human through the agent's own key-event log: an agent is a separate identity whose inception event names the human as its delegator, and the human's log anchors the agent's establishment events by cooperative delegation (`09-security-model.md` §9.1 invariant 1). Binding is structural rather than an external proof, so any participant that resolves the agent's chain reads the human behind it. **The delegation model is unspecified as of 2026-09-10** (`00-open-questions.md`), and `09-security-model.md` §9.7.4.2 R3 rejects every chain whose delegator field is nonzero until it lands, so no agent identity resolves today.

- **Personal agents:** Bound to a single human. The common case. A context admits at most one delegated agent identity per human identity (`09-security-model.md` §9.1 invariant 4).
...
**Institutional agent key governance.** An institutional agent is one delegated identity the institution's log anchors, with operational control over that identity distributed among authorized humans. Key governance follows these rules:

1. **Key generation.** The designated key administrator generates the agent identity's root set and its `#active` key. Either MAY be generated in an HSM accessible to the institutional infrastructure, which `09-security-model.md` §9.7.4.1 item 4 admits as a custody substrate.
2. **Key rotation authority.** Only a threshold-satisfying subset of the agent identity's own root set rotates its `#active` key, and the institution decides who holds those members. Rotation is the migration of `03-identity.md` §3.2.1 case 1: the standing root signs a `KeyState` carrying the new key `current` and the old key's `Superseded` entry, old scoped UCANs are revoked, and MLS Updates are issued in every active context.
3. **Multi-operator access.** A root set is an ordered list of at most 16 public keys with a signing threshold (`09-security-model.md` §9.7.4.2 definitions), so an institution several officers jointly control expresses that control in the root set itself, each member in its own custody, rather than by splitting one key. The protocol does not mandate a multi-operator key management scheme beyond that shape.
4. **Revocation cascade.** When an authorized operator is removed from the institution, the institution MUST rotate the agent identity's `#active` key, and all scoped UCANs delegated to the old key are revoked and reissued. Where the removed operator held a root member, the institution installs a fresh root set by a `RootRecovery` (`09-security-model.md` §9.7.4.2 R10). No institution-specific revocation mechanism exists.
5. **Audit trail.** Institutional agent actions are signed by the agent identity's `#active` key and are indistinguishable from personal agent actions at the protocol level. The institutional governance structure — who authorized what — is an organizational concern above the protocol boundary. The protocol guarantees only that every action traces to the institution's identity through the delegator the agent's own key state names.

## 4.3 One Agent Per Person Per Context

At the protocol level, each human has exactly one agent per context. This is structurally enforced: a context admits at most one delegated agent identity per human identity, and a verifier reads the human behind an agent from the delegator the agent's own key state names (`09-security-model.md` §9.1 invariant 4). The agent can be arbitrarily capable internally — parallel execution, complex orchestration, sophisticated reasoning. The constraint is on presence: one seat per person per table.
...
The human and the agent are two identities, and their actions carry distinct provenance. A message signed under the human identity's `#active` key is human-direct; one signed under the agent identity's `#active` key is agent-autonomous (`09-security-model.md` §9.1 invariant 1). The distinction is structural, so a verifier reads provenance from the signing identity alone and needs no out-of-band metadata. The human can always act directly, and the human decides what the agent does — from full autonomy to direct manual control — a decision that is local and outside protocol scope.
...
Human-bound agents are **protocol consumers** — separate identities their human's log anchors, but consumers nonetheless. They use apps. They use context outlets. They interact with contexts through the protocol. They have zero responsibility for enforcing protocol rules.
