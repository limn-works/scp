...
Every agent is bound to its human through the DID document itself — the agent's signing key is a verification method (`#agent`) on the human's DID. Binding is structural, not an external proof: the DID document is the single source of truth for which key is the human's (`#active`) and which is the agent's (`#agent`). The binding is verifiable by any participant who resolves the DID. **[Superseded 2026-09-10 — a human identity's key state names one operational role, `#active`, and names no agent key (`09-security-model.md` §9.1 invariant 1); an agent is a separate identity whose establishment events the human's log anchors, and that delegation model is unspecified as of 2026-09-10 (`00-open-questions.md`).]**

- **Personal agents:** Bound to a single human. The common case. The human's DID document contains at most one `#agent` verification method. **[Superseded 2026-09-10 — a human identity's key state names one operational role, `#active`, and names no agent key (`09-security-model.md` §9.1 invariant 1); an agent is a separate identity whose establishment events the human's log anchors, and that delegation model is unspecified as of 2026-09-10 (`00-open-questions.md`).]**
...
**Institutional agent key governance.** Institutional agents share one `#agent` verification method on the institution's DID document, with operational control distributed among authorized humans. Key governance follows these rules: **[Superseded 2026-09-10 — a human identity's key state names one operational role, `#active`, and names no agent key (`09-security-model.md` §9.1 invariant 1); an agent is a separate identity whose establishment events the human's log anchors, and that delegation model is unspecified as of 2026-09-10 (`00-open-questions.md`).]**

1. **Key generation.** The institution's `#agent` key is generated by the designated key administrator (a human with `#0` or `#active` access on the institution's DID). The key MAY be generated in an HSM accessible to the institutional infrastructure. **[Superseded 2026-09-10 — a human identity's key state names one operational role, `#active`, and names no agent key (`09-security-model.md` §9.1 invariant 1); an agent is a separate identity whose establishment events the human's log anchors, and that delegation model is unspecified as of 2026-09-10 (`00-open-questions.md`).]**
2. **Key rotation authority.** Only holders of the institution's Identity Key (`#0`) can rotate the `#agent` key. Rotation follows the same protocol as personal agent key rotation (ADR-039) — publish a new DID document with a replacement `#agent` verification method, revoke old scoped UCANs, issue MLS Updates in all active contexts. **[Superseded 2026-09-10 — a human identity's key state names one operational role, `#active`, and names no agent key (`09-security-model.md` §9.1 invariant 1); an agent is a separate identity whose establishment events the human's log anchors, and that delegation model is unspecified as of 2026-09-10 (`00-open-questions.md`).]**
3. **Multi-operator access.** When multiple humans operate an institutional agent, the private key material for `#agent` is distributed via threshold key sharing (t-of-n Shamir secret sharing, GF(2^8)) or stored in a shared HSM with access controls. The protocol does not mandate a specific multi-operator key management scheme — it requires only that the `#agent` key is singular in the DID document (one `#agent` VM per DID, per ADR-039). **[Superseded 2026-09-10 — a human identity's key state names one operational role, `#active`, and names no agent key (`09-security-model.md` §9.1 invariant 1); an agent is a separate identity whose establishment events the human's log anchors, and that delegation model is unspecified as of 2026-09-10 (`00-open-questions.md`).]**
4. **Revocation cascade.** When an authorized operator is removed from the institution, the institution MUST rotate the `#agent` key. All scoped UCANs delegated to the old key are revoked and reissued. This is the standard key rotation path — no institution-specific revocation mechanism exists. **[Superseded 2026-09-10 — a human identity's key state names one operational role, `#active`, and names no agent key (`09-security-model.md` §9.1 invariant 1); an agent is a separate identity whose establishment events the human's log anchors, and that delegation model is unspecified as of 2026-09-10 (`00-open-questions.md`).]**
5. **Audit trail.** Institutional agent actions are signed by the `#agent` key and are indistinguishable from personal agent actions at the protocol level. The institutional governance structure (who authorized what) is an organizational concern above the protocol boundary. The protocol guarantees only that every action traces to the institution's DID. **[Superseded 2026-09-10 — a human identity's key state names one operational role, `#active`, and names no agent key (`09-security-model.md` §9.1 invariant 1); an agent is a separate identity whose establishment events the human's log anchors, and that delegation model is unspecified as of 2026-09-10 (`00-open-questions.md`).]**

## 4.3 One Agent Per Person Per Context

At the protocol level, each human has exactly one agent per context. This is structurally enforced: a DID document contains exactly one `#agent` verification method. Verifiers reject DID documents with multiple `#agent` VMs. The agent can be arbitrarily capable internally — parallel execution, complex orchestration, sophisticated reasoning. The constraint is on presence: one seat per person per table. **[Superseded 2026-09-10 — a human identity's key state names one operational role, `#active`, and names no agent key (`09-security-model.md` §9.1 invariant 1); an agent is a separate identity whose establishment events the human's log anchors, and that delegation model is unspecified as of 2026-09-10 (`00-open-questions.md`).]**
...
The human and agent share a single DID but their actions carry distinct provenance. Messages signed with `#active` are human-direct; messages signed with `#agent` are agent-autonomous. This distinction is structural — verifiers can determine provenance from the signing key alone, without out-of-band metadata. The human can always act directly through `#active`, and the agent acts through `#agent`. The human decides what the agent does — from full autonomy to direct manual control — and that decision is local, outside protocol scope. **[Superseded 2026-09-10 — a human identity's key state names one operational role, `#active`, and names no agent key (`09-security-model.md` §9.1 invariant 1); an agent is a separate identity whose establishment events the human's log anchors, and that delegation model is unspecified as of 2026-09-10 (`00-open-questions.md`).]**
...
Human-bound agents are **protocol consumers** — the same identity as their human, distinguished only by verification method (`#agent` vs `#active`), but consumers nonetheless. They use apps. They use context outlets. They interact with contexts through the protocol. They have zero responsibility for enforcing protocol rules. **[Superseded 2026-09-10 — a human identity's key state names one operational role, `#active`, and names no agent key (`09-security-model.md` §9.1 invariant 1); an agent is a separate identity whose establishment events the human's log anchors, and that delegation model is unspecified as of 2026-09-10 (`00-open-questions.md`).]**
