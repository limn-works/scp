...
2. **Key rotation authority.** Only a threshold-satisfying subset of the agent identity's own root set can rotate its `#active` key, and the institution decides who holds those members. Rotation is the migration of `03-identity.md` §3.2.1 case 1: the standing root signs a `KeyState` listing the new key `current` and the old one `Superseded`, old scoped UCANs are revoked, and MLS Updates are issued in every active context.
