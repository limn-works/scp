...
- **Sanitized at creation.** All string fields in consequence rules (`Custom` trigger keys, `CapabilitySuspension` capability names, `RoleDemotion` target roles) are validated when the context is created. Implementations MUST reject strings containing control characters (U+0000-U+001F, U+007F-U+009F), HTML-special characters (`<`, `>`, `&`, `"`, `'`), or strings exceeding 256 bytes. Role-typed fields (`RoleDemotion` target roles) use the role name limit of 64 bytes (§9.18.6) rather than the general 256-byte cap. This prevents injection attacks when consequence events are serialized for SDK consumers or rendered in user interfaces.
