...
Tier-1 validation (§7.2.1) has two callers with different needs, and the protocol exposes a distinct operation for each. They run the **same** 11 checks; they differ in what they return and in their side effects. (Decision: ADR-055.)
...
2. **Per-context extraction.** For each context log, scan events matching the target DID and extract:
   ...
   - `role_progression_count`: Count of events with type `RoleAssigned` where `subject_did == target_did`. The `subject_did` is carried in the `RoleAssigned` leaf payload (`RoleAssignedPayload`, ADR-011), so the affected member — not the assigning governance actor — is the subject this fact is attributed to.
