...
- **Reconciled atomically on ceiling change:** When the capability ceiling is lowered (a governed `ModifyCeiling` activation, §5.3.2 step 5), the cache and every role definition's permission set are intersected with the new ceiling in the same operation that installs it, dropping any capability no longer within it. The same reconciliation also prunes `suspended_capabilities` — a suspension referencing a capability no longer granted to (or in the ceiling of) that member is dropped as dead weight; this is pure hygiene, never a grant, since a suspension only ever denies. Combined with the role-change update above, there is **no window where stale capabilities are served**: the cache never holds a capability outside the current ceiling, because every write that populates it is ceiling-bounded at write time (role assignment mints from ceiling-validated tokens; ceiling lowering eagerly prunes) — the gate therefore does not re-intersect against the ceiling at read time, and none is needed.
