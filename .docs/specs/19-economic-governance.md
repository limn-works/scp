...
**Free relays MUST exist.** The bootstrap relay list (`18-addressability-and-deployment.md` §18.5.1, priority level 5) MUST include free relays. **A free relay refuses a PUBLISH past a storage budget it configured**, because that invariant fixes what a relay may charge and fixes nothing about what it stores; a charging relay refuses an unpaid PUBLISH the same way, and either refusal sends the publisher to the next entry of its fallback set (`09-security-model.md` §9.7.4.2 R9). Self-hosted relays (§10.2, §10.4), community relays, and bundled relays remain free. Economic config is optional. Absence = free.
