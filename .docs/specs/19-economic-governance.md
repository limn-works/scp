...
**Durability and scope of enforcement (local, per-instance).** Spending enforcement is the payer's self-governance over their own agent — payees do not enforce spending limits, so only the payer's own instances run the paid-action gate; a revocation therefore does not converge across context members. Both authoritative stores are **local, durable, per-instance** state:
