...
- **Per-context Class-S `revoked_spending_ucan_cids` set** — **convergent governance state**: it converges to the context's members via the append-only `SpendingUcanRevoked` leaf (§19.6.1) and is covered by the signed export digest (§23.16.8). It is therefore deliberately **NOT** time-garbage-collected. Lazy per-instance expiry pruning would (a) break export-digest convergence — members would prune moot CIDs at different wall-clock times, diverging their sets and their digests — and (b) could not in any case shrink the set below what the immutable, append-only `SpendingUcanRevoked` log already implies. Its growth is instead bounded by (i) the scope-matched authorization model — a context-scoped spending UCAN may be revoked ONLY by its issuer or by the creator of its scope context (SCP-ECON-12067) — and (ii) the number of context-scoped revocations actually committed. A residual remains and is **not** closed here: an issuer can still commit many revocations of self-issued (even never-granted) context-scoped tokens, bloating that context's convergent set and export. The principled bound for that case — restricting revocation to spending UCANs actually observed/granted in the context — is a separate convergent mechanism, tracked as its own work item; this section does **not** claim to solve it.
