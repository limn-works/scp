...
**Paid-action gate invariants.** The in-memory global revoked-CID cache the paid-action gate reads, and the authorization of the revoke path that maintains it, are governed by the following normative invariants. The implementation cites these labels (`invariant 1a`, `2b`, etc.); they are grounded here:

- **(1a) Fail-closed on an un-hydrated global store.** The gate reads a lock-free in-memory snapshot of the DID-scoped global revoked-CID set, which carries a hydration state: `NotConfigured`, `NeedsHydration`, `Hydrated`, or `Failed`. When a global store IS configured but has NOT been successfully hydrated (`NeedsHydration` or `Failed`, e.g. the durable read failed), that snapshot is not yet trustworthy, so the gate MUST treat EVERY `scp:spending:*` (global-scope) spending UCAN as revoked and fail the spend **closed** — for **all** contexts on the instance, including any joined or created *after* the failure — until a later hydration succeeds. Only `NotConfigured` (no store ⇒ the empty set is authoritative) and `Hydrated` (snapshot mirrors the durable store) are "status known" and may authorize a global spend. **Context-scoped** spends are unaffected: their revocations ride the per-context Class-S `revoked_spending_ucan_cids` set restored with the context's governance snapshot, not this global cache. Hydration runs **FIRST** at instance startup — before any context is restored — so the interval in which global spends fail closed is bounded by the startup hydration itself.
- **(1b) Retain-on-upgrade.** A durable revoked-CID record persisted by an older build **without** the `revocation_moot_after_secs` expiry field MUST deserialize with a RETAIN (never-moot) default, so it is NOT garbage-collected on sight — a fail-closed upgrade path: an un-dated record is kept, never dropped.
- **(2a) Bounded incremental cache update.** On each global revocation the payer DID's in-memory cache entry is RE-DERIVED from the freshly-pruned durable store (a single-DID, expiry-GC'd re-read), never blind-inserted. This keeps the in-memory cache equal to the bounded durable store instead of growing monotonically until the next restart's full hydration.
- **(2b) Atomic re-read-and-store under the cache write lock.** A global revocation is applied by calling the payer's supervisor **directly** — it is NOT serialized through a context actor's mailbox — so two concurrent revocations for the same payer DID can race. The durable re-read (2a) and the subsequent cache store MUST therefore be performed **atomically under the single cache write lock** (the same lock hydration holds across its full-store re-read *and* store). Re-reading outside the lock admits a lost update: an earlier revocation's stale re-read overwrites a later revocation's just-committed CID, dropping it from the gate cache (while it stays durable) and re-authorizing that revoked token until the next restart. Holding the lock across the re-read serializes the two writers so neither clobbers the other. If the bounding re-read itself fails after the durable record already succeeded, the just-revoked CID is still inserted into the cache under the held lock before the error is surfaced, so a confirmed revocation is never absent from the gate.
- **(3a) Membership gate for context-scoped revocation.** A context-scoped spending UCAN may be revoked only by the **scope-context creator** or by a **current member** of that context (the creator need not be a listed member). This shrinks *who* can flood the convergent per-context revoked-CID set to members; it is **defense-in-depth**, NOT a size bound (self-issuance is unbounded — see above and issue #2072). Because the gate keys off *current* membership, an issuer who has **left** the context can no longer revoke a token there — an **availability-conservative, security-safe** posture: the gate can only decline an otherwise-authorized revoke, never grant an unauthorized one.
- **(3b) Non-empty authenticated principal.** The revoker DID MUST be a non-empty, caller-authenticated principal. An empty revoker is rejected outright, and the creator-authorization branch additionally requires a non-empty recorded creator DID — so a blank principal can never spuriously match an empty creator field and authorize an unauthenticated revoke.
