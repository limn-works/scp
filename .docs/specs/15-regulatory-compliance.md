...
- **Key-event-log identity (§3.1)** is self-sovereign. No identity provider holds user data.

**GDPR and data portability:**

- **Right to erasure.** A user can abandon their identity (`09-security-model.md` §9.7.4.2) and leave all contexts. Their identity private state is encrypted and under their control. The protocol supports erasure at three levels:
  ...
  Content authored in contexts remains on other participants' nodes, attributed to a now-abandoned identifier. Apps and context governance can implement content deletion policies, and the protocol provides the abandonment primitive.
- **Right to data portability.** Protocol state (membership, roles, attestations, participation records) is inherently portable — it's bound to the user's identifier, not to any service provider. App state portability depends on the app (§8.3).
