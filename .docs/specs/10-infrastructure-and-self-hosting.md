...
**Relay conformance testing.** Relay storage backends implement the `BlobStorage` trait, which §17.7 of the persistence and storage spec specifies alongside its first-party adapter roster, and are verified by the `blob_store_conformance!()` macro (§16.12.6), which tests store/retrieve roundtrips, TTL expiry, listing, deletion, and concurrent access. The `InMemoryRelay` (§16.4.2) is the reference implementation; every production backend (SQLite, redb, etc.) must pass the same suite. First-party `BlobStorage` adapters include `SqliteBlobStore` (personal relays), `RedbBlobStore` (medium relays, pure Rust), `PostgresBlobStore` (production/enterprise), and `S3BlobStore` (large-scale/cloud). Multi-relay adversarial scenarios — suppression, equivocation, delay, replay, Commit suppression — are tested via `BehaviorMode` fault injection in the network simulation harness (§16.4.3).
