...
**Relay conformance testing.** Relay storage backends implement the `BlobStorage` trait, which §17.1 of the persistence and storage spec, the storage architecture overview, names together with its five methods and the file that declares it. §17.11 of that spec, the extension points section, states the adapter contract: the five required methods, the two streaming methods that carry default implementations, and a passing run of the `blob_store_conformance!()` macro (§16.12.6), which tests store/retrieve roundtrips, TTL expiry, listing, deletion, and concurrent access. §17.7 of that spec, the first-party adapter roster, lists five `BlobStorage` backends: `InMemoryBlobStore` (testing and development, shipped in phase 1), `SqliteBlobStore` (personal relays), `RedbBlobStore` (medium relays, pure Rust), `PostgresBlobStore` (production/enterprise), and `S3BlobStore` (large-scale/cloud). Every one of the five must pass the same suite. The `InMemoryRelay` of §16.4.2 of the test infrastructure spec is not one of them, because §16.4.1 of that same test infrastructure spec states that the in-memory relay stores blobs in its own `HashMap` and implements no `BlobStorage` method. Multi-relay adversarial scenarios — suppression, equivocation, delay, replay, Commit suppression — are tested via `BehaviorMode` fault injection in the network simulation harness (§16.4.3).
...
**The self-hosted website binary (`scp-node --self-host`, §10.12.11) is the bundled shape.** Publishing the operator's site is not the node participating in a context — it is a co-located SDK participant client, running inside the node process and sharing the node's custody, that creates a broadcast context and publishes the site assets as `BroadcastContent` (§5.14, ADR-042); the node then decrypts and projects them over HTTP. The participant and the node are distinct roles co-located in one process, never a single "participating node."
